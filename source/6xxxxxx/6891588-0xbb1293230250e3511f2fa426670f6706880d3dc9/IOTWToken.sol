/** 
 * Developer Notes
 * Company: AnApp Blockchain Technologies Limited
 * Compiler: 0.4.24+commit.e67f0147
 * Optimization: Enabled
 * Description: Token based on EIP-20 accepted by ethereum community.
...
contract MintableToken is BurnableToken, Ownable {
  ...
  uint256 public maximumMintLimit;
...
/**
  ...
  modifier canMint(uint _amount) {
    ...
    require(totalSupply.add(_amount) <= maximumMintLimit);
    _;
  }
  
  constructor(uint256 _maxLimit) public {
     maximumMintLimit  = _maxLimit * 10 ** 18;
  }
 ...
 * @title IOTWToken
 */
contract IOTWToken is MintableToken, PausableToken {
  string public constant name = "Internet Of Things World";
  string public constant symbol = "IOTW";
  uint8 public constant decimals = 18;
  
  /* Sub units for the tokens will be added automatically. */
  /* Inital Tokens 1.5 Billion = 1,500,000,000*/
  uint256 totalTokens = 1500000000;

  constructor() public MintableToken(totalTokens){}
  ...
  /**
   * @dev Make the contract non payable so no ether is directly accepted.
   */
