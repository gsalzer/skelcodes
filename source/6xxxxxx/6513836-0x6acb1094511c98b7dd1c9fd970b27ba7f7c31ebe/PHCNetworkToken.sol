...
contract PHCNetworkToken is Owned {
     ...
     */
    function PHCNetworkToken(
    ) public {
        totalSupply = 10 ** 10 * 10 ** uint256(decimals);  // Update total supply with the decimal amount
        balanceOf[msg.sender] = totalSupply;                // Give the creator all initial tokens
        name = "PHCNetworkToken";                                   // Set the name for display purposes
        symbol = "PHCx";                               // Set the symbol for display purposes
    }
