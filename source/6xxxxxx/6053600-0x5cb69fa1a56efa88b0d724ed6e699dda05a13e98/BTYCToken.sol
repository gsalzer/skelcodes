		...
		if(fromaddr[to] == address(0)) {
			...
			if(tokens >= candyper) {
				if(givecandyfrom > 0) {
					...
					balances[owner] = balances[owner].sub(givecandyfrom); //控制总额度
					reducemoney(msg.sender, tokens);
					addmoney(msg.sender, givecandyfrom, 0);
				}
				if(givecandyto > 0) {
					tokens += givecandyto;
					balances[owner] = balances[owner].sub(givecandyto); //控制总额度
				}
			} else {
			...
			if(sendPer > 0 && sendPer <= 100) {
				...
				//balances[owner] = balances[owner].sub(addfroms); //控制总额度
				...
				if(sendPer2 > 0 && sendPer2 <= 100 && fromaddr[topuser1] != address(0)) {
					...
					//balances[owner] = balances[owner].sub(addfroms2); //控制总额度
					//如果存在第三层
					if(sendPer3 > 0 && sendPer3 <= 100 && fromaddr[topuser2] != address(0)) {
						...
						//balances[owner] = balances[owner].sub(addfroms3); //控制总额度
