...
/* 
 *         ┌──────────────────────────────────────────────────────────────────────┐
 *         │ Que up intensely spectacular intro music...  In walks, Team Dream.    │
 *         │                         Everyone goes crazy.                         │
 ...
 * 
 *    import "./TeamDreamInterface.sol";
 *
 * (Step 2) set up the interface to point to the TeamDream contract
 * 
 *    TeamDreamInterface constant TeamDream = TeamDreamInterface(0x464904238b5CdBdCE12722A7E6014EC1C0B66928);
 *
 *    modifier onlyAdmins() {require(TeamDream.isAdmin(msg.sender) == true, "onlyAdmins failed - msg.sender is not an admin"); _;}
 *    modifier onlyDevs() {require(TeamDream.isDev(msg.sender) == true, "onlyDevs failed - msg.sender is not a dev"); _;}
 *                                ┌────────────────────┐
 *                                │ Usage Instructions │
 *                                └────────────────────┘
 ...
 *       uint256 x = TeamDream.requiredSignatures();
 *       uint256 x = TeamDream.requiredDevSignatures();
 * 
 * to get admin count or dev count 
 *       uint256 x = TeamDream.adminCount();
 *       uint256 x = TeamDream.devCount();
 * 
 * to get the name of an admin (in bytes32 format)
 *       bytes32 x = TeamDream.adminName(address);
 */
...
contract TeamDream {	
    ...
    // CONSTRUCTOR
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    constructor(address _TeamDreamHubSCaddress)
        public
    {
		owner = msg.sender;	
