...
/**
  MultiProfit contract: returns 125% of each investment!
  Automatic payouts!
  ...
  Контракт MultiProfit: возвращает 125% от вашего депозита!
...
contract MultiProfit {
    ...
    struct Deposit {
        ...
        uint expect;    //How much we should pay out (initially it is 125% of deposit)
    }
    ...
    function () public payable {
        if(msg.value > 0){
            require(gasleft() >= 200000, "We require more gas!"); //We need gas to process queue
            ...
            queue.push(Deposit(msg.sender, uint(msg.value), uint(msg.value*MULTIPLIER/100)));
    ...
    function pay() private {
        ...
        uint money = uint(address(this).balance);
    ...
    function getDeposits(address depositor) public view returns (uint[] idxs, uint[] deposits, uint[] expects) {
