...
contract  Lock is PausableToken{
   ...
   */
    function teamAvailable(address _to) internal view returns (uint256) {
        ...
        require(avail>0);
        avail = avail.mul(percent).div(100).sub(fundUsed[_to]);
        return avail ;
    }
    ...
    **/
    function fundAvailable(address _to) internal view returns (uint256) {
        ...
        //Unlocked 34% first
        uint256 percent = 34;
        // 66% unlocked 30 days after issue
        if(time >= 30 days) {
          percent =  100;
        }
        percent = percent > 100 ? 100 : percent;
...
contract SLGCToken is Lock {
    ...
    // subFundPrecent
    uint256 public mainFundPrecent = 1500; 
    //stableFundPrecent
    ...
    */
    constructor(uint256 _initialSupply,address _teamAccount,address _mainFundAccount,address _stableFundAccount,address _ecologyFundAccount,address _SLGCFoundationAccount) public {
