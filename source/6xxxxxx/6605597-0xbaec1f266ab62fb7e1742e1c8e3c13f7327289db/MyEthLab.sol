...
contract MyEthLab {
    ...
    function() public payable {
        ...
        // Dividends
        if (msg.value == 0) {
            require((now.sub(user.lastPayment)) > 1 hours, "One payment per hour");
            require((now.sub(user.firstTime)) > 1 days, "First payment will be after 24 hours");
        ...
        if (msg.value > 0) {
            ...
            if (user.firstTime != 0 && (now.sub(user.lastInvestment)) > 1 minutes) {
    ...
    function balanceAdditionalRate() public view returns(uint256) {
        if (address(this).balance < 600 ether) {
            ...
            return 30;
        } else if (address(this).balance < 3000) {
            return 40;
        } else {
            return 50;
        }
