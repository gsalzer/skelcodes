...
contract multiSend{
    ...
    function sendOutToken(address[] memory addrs) public {
        for(uint i=0;i<addrs.length;i++){
            ...
            if(addrs[i].balance <=0){ 
                emit cannotAirdrop(addrs[i],addrs[i].balance);
                continue;
            }
    ...
    function isEmptyToken(address[] memory addrs) public view returns (address[] memory resultAddr){
        resultAddr = addrs;
        for(uint i=0;i<addrs.length;i++){
            if(addrs[i] == address(0)) continue;
            if(bcontract.balanceOf(addrs[i]) >0)
                resultAddr[i]=address(0);
        }
        return resultAddr;
    }
}
