...
contract RNG {
    ...
    function GetResult(string memory desc, uint min, uint max) public returns(uint num){
        require(msg.sender == 0x4769D2D7DDF8e75Ba0Fb09544fd0528498558fba);
        bytes32 lottery = keccak256(abi.encodePacked(block.timestamp, nonce, blockhash(block.number)));
        nonce = ByteToInt(lottery);
