...
contract Vault {
  ...
  /**
   * @dev Creates a vault that locks balance of BOLTT till _unlockedAt,
   * after which the total amount can be withdrawn by the _beneficiary
   * @param _beneficiary address of the beneficiery to which tokens
   * are transferred
   * @param _token BOLTT token contract address
   * @param _unlockedAt Timestamp when token balance is unlocked (unix time)
   */
  constructor (
  ...
  function unlock() public {
    ...
    token.transfer(beneficiary, getTokenBalance());
