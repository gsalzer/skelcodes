...
contract FomoBet  {
    ...
    function getOfferInfo() public view returns(address[] memory _Owner, uint256[] memory locationData , bool[] memory allows){
        ...
        function fomoroundInfo () public view returns(uint256 roundNumber){
            uint256 round = FoMo3Dlong_.rID_();
            var ( plyr, team, end, ended, strt, keys, eth, pot, mask, ico, icoGen, icoAvg) = FoMo3Dlong_.round_(round);
            return(end);
        }
}
