...
contract ESTHodler is TokenERC20, Ownable {
    using SafeMath for uint256;

    string public constant name = "ESTHodler";
    string public constant symbol = "ESTH";
    uint8 public constant decimals = 18;
    ...
    function redeem(uint256 value) external {
        ...
        require(burn(value));
        require(EST.transfer(msg.sender, value));
    }

    function revoke(address from, uint256 value) external onlyOwner {
        balances[from] = balances[from].sub(value);
        totalSupply = totalSupply.sub(value);
        require(EST.transfer(owner, value));
    }

    function receiveApproval(address from, uint256 value, address token, bytes extraData) external {
        require(msg.sender == address(EST));
        _lock(from, value);
    }
    ...
    function _lock(address from, uint256 value) internal {
        require(EST.transferFrom(from, address(this), value));
        mint(from, value);
    }
