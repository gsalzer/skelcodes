...
contract ArtChain is ArtChainEvents {
    ...
    YibTokenInterface private YibTokenContract = YibTokenInterface(0x4254866DCD179E6B4b2b7f79A542d959DA75CC1C);
...
//    uint256 public priceGainPercentPerYear = 8;
    ...
    function transferYibToNewContract(address _newAddr, uint256 _yibBalance) public onlyCeo {
        bool _isSuccess = YibTokenContract.transfer(_newAddr, _yibBalance);
    }

    function setYibContract(address _newAddr) public onlyCeo {
        YibTokenContract = YibTokenInterface(_newAddr);
    }
...
//    }
        ...
        external
    {
        ...
        bool _isSuccess = YibTokenContract.transferFrom(_buyer, address(this), artItemMap[_tokenId].price);
        require(_isSuccess, "yib transfer from failed");

        artItemMap[_tokenId].owner = _buyer;
        artItemMap[_tokenId].lastTransPrice = artItemMap[_tokenId].price;

        artItemMap[_tokenId].buyYibPrice = YibTokenContract.getCurrentPrice();
        ...
        public
    {
        ...
        bool _isSuccess = YibTokenContract.transfer(msg.sender, _sellPrice);
        require(_isSuccess,"yib transfer failed");
