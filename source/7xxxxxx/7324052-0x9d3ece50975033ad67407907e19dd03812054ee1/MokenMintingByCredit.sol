...
contract MokenMintingByCredit  is Storage9 {
    ...
    function mint(address _tokenOwner, string _mokenName, uint256 _categoryIndex, bytes32 _linkHash) external returns (uint256 tokenId) {
        ...
        uint256 currentMintPrice = categoryMintPrice[_categoryIndex];
        ...
        uint256 data = uint256(_linkHash) & MOKEN_LINK_HASH_MASK | _categoryIndex << 176 | ownedTokensIndex << 160 | uint160(_tokenOwner);
        ...
        emit Mint(address(this), _tokenOwner, categories[_categoryIndex], _mokenName, bytes32(data), tokenId, "Ether", currentMintPrice);
