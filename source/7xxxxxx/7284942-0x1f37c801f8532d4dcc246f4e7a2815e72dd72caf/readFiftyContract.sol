...
contract readFiftyContract{
	...
	function getCurrentTree(address r, uint t) public view returns(address[8] memory addrs){
		address[8] memory Adds;
		if(bcontract.nodeIDIndex(r,t) > (2 ** 32) -2 || !bcontract.currentNodes(r,t)) 
		    return Adds;
		uint cc=bcontract.nodeIDIndex(r,t) - 1;
		Adds[0]=r;
		for (uint8 i=0; i < bcontract.spread(); i++) {
		    (address k,uint p,uint m) = bcontract.treeChildren(r,t,cc,i);
			if(p != 0){
				Adds[i+1]=k;
				for (uint8 a=0; a < bcontract.spread(); a++) {
