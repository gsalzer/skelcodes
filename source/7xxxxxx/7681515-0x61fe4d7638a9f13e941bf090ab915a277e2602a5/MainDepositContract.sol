...
contract DepositContract {
    ...
    uint256 public _payment_id;

    constructor(uint256 payment_id) public {
        _payment_id = payment_id;
        _main_contract = MainDepositContract(msg.sender);
    }

    function () external payable {
        _main_contract.log_deposit.value(msg.value)(_payment_id);
    }
}
...
contract MainDepositContract {
    ...
    event Deposit(uint256 _payment_id, uint256 _amount);
    ...
    function create_deposit_address(uint256 payment_id) _onlyManager public returns (DepositContract created_contract) {
        DepositContract c = new DepositContract(payment_id);
        _deposit_contracts[payment_id] = c;
        return c;
    }

    function log_deposit(uint256 payment_id) public payable {
        require(address(_deposit_contracts[payment_id]) == msg.sender, 'Sender is not a deployed deposit contract');
        emit Deposit(payment_id, msg.value);
    }
