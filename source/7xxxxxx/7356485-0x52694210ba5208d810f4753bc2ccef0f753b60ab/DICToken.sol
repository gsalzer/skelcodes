/*
DIC Token - Official Smart contract
https://dicresortcoin.com
Powered by Nodeberry Blockchain Solutions(https://nodeberry.com)
*/
...
{ 
    ...
    // Name of the token
    string public constant name = "DIC Token";
    // Symbol of token
    string public constant symbol = "DIC";
    uint8 public constant decimals = 8;
    uint public _totalsupply = 87000000 * (uint256(10) ** decimals);  //87 million DIC
    ...
    function DICToken() public
    {
        owner = msg.sender;
        balances[owner] = 0 * (uint256(10) ** decimals);
        balances[address(this)] = 87000000 * (uint256(10) ** decimals);
    ...
    function () public payable atStage(Stages.ICO)
    {
        ...
        uint256 tokPrice = ethCent.mul(100); //1DIC = 100 USD cent
        ...
        if (now < ico_first)
        {
            ...
            {
                bonus_token = no_of_tokens.mul(12).div(100); 
            }
        ...
        else if (now >= ico_first && now < ico_second)
        {
            if (no_of_tokens >=  2000 * (uint256(10)**decimals) &&
                no_of_tokens <= 19999 * (uint256(10)**decimals))
            {
                bonus_token = no_of_tokens.mul(8).div(100); 
            }
            else if (no_of_tokens >   19999 * (uint256(10)**decimals) &&
                     no_of_tokens <= 149999 * (uint256(10)**decimals))
            {
        ...
        else if (now >= ico_third && now < ico_fourth)
        {
            if (no_of_tokens >=  2000 * (uint256(10)**decimals) &&
                no_of_tokens <= 19999 * (uint256(10)**decimals))
            {
                bonus_token = no_of_tokens.mul(0).div(100); 
            }
            else if (no_of_tokens >   19999 * (uint256(10)**decimals) &&
                     no_of_tokens <= 149999 * (uint256(10)**decimals))
            {
    ...
    // what is the total supply of DIC
