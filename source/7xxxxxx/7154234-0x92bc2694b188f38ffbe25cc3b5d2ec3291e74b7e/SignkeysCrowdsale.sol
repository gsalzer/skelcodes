...
contract SignkeysBonusProgram is Ownable {
    ...
    mapping(address => bool) private _oneTimeBonusesWereSent;
    ...
    uint256[] public firstPurchaseTokensAmountRanges = [199, 1000, 10000, 100000, 1000000, 10000000];
    uint256[] public referrerRewards = [5, 50, 500, 5000, 50000];
    uint256[] public buyerRewards = [5, 50, 500, 5000, 50000];

    uint256[] public purchaseAmountRangesInCents = [100000, 10000000, 100000000000];
    ...
    function calcBonus(uint256 tokensAmount, uint256[] rewards) private view returns (uint256) {
        ...
        if (tokensAmount <= multiplier.mul(firstPurchaseTokensAmountRanges[0])) {
            return 0;
        }
        for (uint i = 1; i < firstPurchaseTokensAmountRanges.length; i++) {
            uint min = firstPurchaseTokensAmountRanges[i - 1];
            uint max = firstPurchaseTokensAmountRanges[i];
    ...
    function calcPurchaseBonus(uint256 amountUSD, uint256 tokensAmount) private view returns (uint256) {
        if (amountUSD <= purchaseAmountRangesInCents[0]) {
            return 0;
        }
        for (uint i = 1; i < purchaseAmountRangesInCents.length; i++) {
            if (amountUSD > purchaseAmountRangesInCents[i - 1] && amountUSD <= purchaseAmountRangesInCents[i]) {
                return tokensAmount.mul(purchaseRewardsPercents[i]).div(100);
            }
        }
        if (amountUSD > purchaseAmountRangesInCents[purchaseAmountRangesInCents.length - 1]) {
            return tokensAmount.mul(purchaseRewardsPercents[purchaseAmountRangesInCents.length - 1]).div(100);
        }
    }


    function sendBonus(address referrer, address buyer, uint256 _tokensAmount, uint256 _valueUSD, uint256 _couponCampaignId) external returns (uint256)  {
        ...
        uint256 purchaseBonusAmount = calcPurchaseBonus(_valueUSD, _tokensAmount);

        if (referrer != 0x0 && !_oneTimeBonusesWereSent[buyer]) {
            if (referrerBonusAmount > 0 && token.balanceOf(this) > referrerBonusAmount) {
                token.transfer(referrer, referrerBonusAmount);
                _oneTimeBonusesWereSent[buyer] = true;
        ...
        if (token.balanceOf(this) > purchaseBonusAmount.add(buyerBonusAmount)) {
            purchaseBonus = purchaseBonusAmount;
        }

        if (_couponCampaignId > 0 && !_couponWereUsed[buyer][_couponCampaignId]) {
            if (token.balanceOf(this) > purchaseBonusAmount.add(buyerBonusAmount).add(_couponToBonuses[_couponCampaignId])) {
                _couponWereUsed[buyer][_couponCampaignId] = true;
    ...
    function getFirstPurchaseTokensAmountRanges() public view returns (uint256[]) {
        return firstPurchaseTokensAmountRanges;
    }
    ...
    function setFirstPurchaseTokensAmountRanges(uint[] ranges) public onlyOwner {
        firstPurchaseTokensAmountRanges = ranges;
    }

    function setReferrerRewards(uint[] rewards) public onlyOwner {
        require(rewards.length == firstPurchaseTokensAmountRanges.length - 1);
