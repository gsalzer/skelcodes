...
contract Invest is MarginWithPresignedWithdraw{
    ...
    event RegisterTradeProfile(address indexed trader, address indexed profileAddr, uint256 profitSharingPercentage);
    ...
    * @param _profitSharingPercentage uint256 Additional argument added in modified Invest contract.
    *           Determine how many profit trader made should share with us.
    ...
    */
        ...
        returns (bool)
    {
        ...
        localVariables.profitSharingPercentage = _profitSharingPercentage;
        localVariables.profileAddr = new TradeProfileProxy(
            tradeProfileImplementation,
            abi.encodeWithSignature(
                "init(address,uint256,uint256,uint256,uint256,uint256)",
                localVariables.from,
                _periodLength,
                _maxMarginDeposit,
                _minMarginDeposit,
                _rewardPercentage,
                localVariables.profitSharingPercentage
            )
        );
        ...
        emit RegisterTradeProfile(localVariables.from, localVariables.profile, _profitSharingPercentage);
        return true;
    }
    ...
    */
    function registerPreSignedHashing(
        ...
        returns (bytes32)
    {
        /* "14cd2b3d": registerPreSignedHashing(address,bytes32,uint256,uint256,uint256,uint256,uint256,uint256,uint256) */
        return keccak256(
            abi.encodePacked(
                bytes4(0x14cd2b3d),
                _investContract,
    ...
    */
    ...
    function getInitInputDataBytes(address _bincentive, address _tokenAddress, address _bincentiveErrand, address _tradeProfileImplementation) public pure returns (bytes){
...
contract TradeProfile{
    ...
    uint256 public profitLeftPercentage;
    ...
    */
        ...
        returns (uint256 amountToTrader, uint256 amountToFollower)
    {
        ...
        if(_profitAmount <= 0) {
            ...
            amountToTrader = uint256(_profitAmount).mul(rewardPercentage).div(100);
        ...
        // Deduct the amount shared with us
        amountToTrader = amountToTrader.mul(profitLeftPercentage).div(uint256(100));
    }
    ...
    function init(address _owner, uint256 _periodLength, uint256 _maxMarginDeposit, uint256 _minMarginDeposit, uint256 _rewardPercentage, uint256 _profitSharingPercentage) public {
        ...
        // NOTE: profitLeftPercentage is the percentage of profit left for trader
        // while _profitSharingPercentage is the percentage of profit shared with us.
        profitLeftPercentage = uint256(100).sub(_profitSharingPercentage);
        isActive = true;
    }
}
...
contract TradeProfileProxy is Proxy {
    ...
    */
    ...
    function bincentive_trade_profile_contract_url() external returns (string) {
        bytes32 slot = INVEST_CONTRACT_ADDRESS_SLOT;
        address invest_contract_addr;
        assembly {
            invest_contract_addr := sload(slot)
        }
        return InterfaceInvest2(invest_contract_addr).trade_profile_contract_url();
    }
}
