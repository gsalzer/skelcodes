...
contract PositionTokenLogic is SplittableToken {
        ...
        returns (uint256)
    {
        ...
        // calculate mintAmount after entry slippage
        ...
        // new escrow should always be greater than current escrow
        uint256 actualDepositAmount = ERC20(loanTokenAddress).balanceOf(address(this))
            .add(netCollateralAmount)
            .add(interestDepositRemaining)
            .sub(currentEscrow);

        uint256 mintAmount = actualDepositAmount
            .mul(10**18)
           .div(currentPrice);
        ...
        returns (uint256)
    {
        ...
        uint256 currentEscrow = loanAmountAvailableInContract
            .add(netCollateralAmount)
            .add(interestDepositRemaining);
        ...
        // new escrow should always be less than or equal to current escrow
        uint256 slippageLoss = currentEscrow    // old escrow
            .sub(                               // new escrow
                loanAmountAvailableInContract
                .add(netCollateralAmount)
                .add(interestDepositRemaining)
            );

        if (loanAmountOwed > slippageLoss) {
            loanAmountOwed = loanAmountOwed.sub(slippageLoss);
        } else {
            loanAmountOwed = 0;
        }
