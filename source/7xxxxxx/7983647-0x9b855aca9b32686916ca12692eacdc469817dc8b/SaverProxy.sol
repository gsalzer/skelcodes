...
contract SaverProxy is DSMath {
    ...
    address public constant UNISWAP_WRAPPER = 0xB19d2123350bDb9887F0B19240576F3FF23556be;
    ...
    function repay(bytes32 _cup, uint _amount, uint _minPrice) public {
        address exchangeWrapper;
        uint ethDaiPrice;

        (exchangeWrapper, ethDaiPrice) = getBestPrice(_amount, ETHER_ADDRESS, DAI_ADDRESS);
    ...
    /// @param _minPrice Minimum acaptable ETH/DAI price
    function boost(bytes32 _cup, uint _amount, uint _minPrice) public {
        address exchangeWrapper;
        uint daiEthPrice;

        (exchangeWrapper, daiEthPrice) = getBestPrice(_amount, DAI_ADDRESS, ETHER_ADDRESS);

        require(wdiv(1000000000000000000, daiEthPrice) > _minPrice, "Slppage hit");
    ...
    function getBestPrice(uint _amount, address _srcToken, address _destToken) internal returns (address, uint) {
        ...
        // expectedRateUniswap = 0; //TODO: REMOVE ON MAINNET
