...
/**
 * @title HoundAndHareGame contract
...
contract HoundAndHareGame is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _publicSaleStartTimestamp, address _moderator) ERC721("P2E", "p2e") {
    ...
    function reserveMINT(uint256 amountOfNFTs) external payable{
        ...
        require(reservedClaimed != RESERVED_NFT, "Already have claimed all reserved NFTs");
        require(reservedClaimed + amountOfNFTs <= RESERVED_NFT, "Minting would exceed max reserved NFTs");
        require(msg.sender != address(0), "Cannot add null address");
        ...
        require(_totalClaimed[msg.sender] + amountOfNFTs <= MAX_NFT_WALLET_PRESALE, "Purchase exceeds max allowed per wallet");
        require(NFT_PRICE * amountOfNFTs == msg.value, "ETH amount is incorrect");

        for (uint256 i = 0; i < amountOfNFTs; i++) {
            uint256 tokenId = numNFTMinted;
    ...
    function mintNFT(uint256 amountOfNFTs) external payable {
        ...
        require(totalSupply() < MAX_NFT_SUPPLY, "All tokens have been minted");
        require(totalSupply() + amountOfNFTs <= MAX_NFT_SUPPLY, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfNFTs <= MAX_NFT_WALLET, "Purchase exceeds max allowed per wallet");
        require(amountOfNFTs > 0, "Must mint at least one NFT");
        require(NFT_PRICE * amountOfNFTs == msg.value, "ETH amount is incorrect");
    ...
    function getRareTokenOwners() public view returns(address[] memory) {
            // tokenOfOwnerByIndex(_account, index);
