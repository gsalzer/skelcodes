...
// File: contracts/TheSecondBurn.sol
...
contract TheSecondBurn is ERC721, ReentrancyGuard, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_ELEMENTS = 4999;    
    uint256 public constant MAX_PUBLIC_ELEMENTS = 4000;    
    uint256 public constant MINT_PRICE = 8 * 10**16;
    

    event CreateBurnable(uint256 indexed id);

    constructor() public ERC721("TheSecondBurn", "BURN2") {}

    modifier saleIsOpen {
        require(_totalSupply() <= MAX_PUBLIC_ELEMENTS, "Sale end");
        _;
    }
    ...
    function getBurnablesOwner(uint256 burnablesId) public view returns (address) {
        return burnablesContract.ownerOf(burnablesId);
    }
    
    // Minting reserved for burnables owners
    function mintWithBurnable(uint256 burnablesId) public nonReentrant {
        require(burnablesContract.ownerOf(burnablesId) == msg.sender, "Not the owner of this burnable");
        require(!_burnableMinters.contains(burnablesId), "This burnable already has minted a second burn.");
    ...
    function mintPublic(uint256 _count) public payable saleIsOpen {
        ...
        require(total + _count <= MAX_PUBLIC_ELEMENTS, "Max limit");
    ...
    // Send ETH to make the burn value higher
    ...
    function burn(uint256 _tokenId) public {
        ...
        // pay token owner 
        uint256 supply = totalSupply();
        _widthdraw(msg.sender, balance.div(supply));
