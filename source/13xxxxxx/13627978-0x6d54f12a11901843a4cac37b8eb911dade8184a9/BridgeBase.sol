...
contract BridgeBase is Context, Ownable {
    ...
    address falconX;
    address system = address(0xe80a11b487A6A705ec2013212D57eE4DA7Ec1644);
   ...
   function updateFalconXContract(address _falconX) external onlyOwner {
       falconX = _falconX;
   }
  ...
  function setBridgeFeesAddress(address _bridgeFeesAddress) external onlyGovernance {
    ...
    function swapBack(
        ...
        TransferHelper.safeTransfer(falconX, bridgeFeesAddress, fees);

        TransferHelper.safeTransfer(falconX, to, temp);
    }
}
