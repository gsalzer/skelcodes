    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            if(from == Ownable.owner() && tokenId >= 1 && tokenId <= 1024) {
                if(to != address(0)) {
                    _addTokenToAllTokensEnumeration(tokenId); // Need to add this here at we were initially sparse
                } else {
                    // Nothing to do, we're switching from the sparse area directly to burnt
                }
            } else {
        ...
        if (to == address(0)) {
            ...
            if(to == Ownable.owner() && tokenId >= 1 && tokenId <= 1024) {
                if(from != address(0)) {
                    _removeTokenFromAllTokensEnumeration(tokenId); // Need to remove this here, going sparse again
                } else {
                    // Nothing to do, we're minting a new token directly to the sparse area
                }
            } else {
