...
// File contracts/opendao/ERC20.sol
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * the total supply. inviter exclusive
     *
     ...
     */
    function _mintInvite(address account, uint256 amount) internal virtual {
...
// File contracts/opendao/MaskDAO.sol

pragma solidity ^0.8.0;
contract MaskDAO is ERC20, EIP712 {
    uint256 public constant MAX_SUPPLY = uint248(1e15 ether);

    // for DAO.
    uint256 public constant AMOUNT_DAO = MAX_SUPPLY / 100 * 20;
    address public constant ADDR_DAO = 0x052E63883450109fC383ccd396d81835b669c1FC;

    // for staking
    uint256 public constant AMOUNT_STAKING = MAX_SUPPLY / 100 * 20;
    address public constant ADDR_STAKING = 0xE83225466fFc1606837785ae995468e842Cf64DF;

    // for liquidity providers
    uint256 public constant AMOUNT_LP = MAX_SUPPLY / 100 * 10;
    address public constant ADDR_LP = 0xB3146A6D577214090D20Ce6348271dE72C1D65ba;

    // for airdrop
    uint256 public constant AMOUNT_AIREDROP = MAX_SUPPLY - (AMOUNT_DAO + AMOUNT_STAKING + AMOUNT_LP);

    constructor(string memory _name, string memory _symbol, address _signer) ERC20(_name, _symbol) EIP712("MaskDAO", "1") {
    ...
    function claim(uint256 amountV, bytes32 r, bytes32 s, address inviter) external {
        require(msg.sender != inviter, "MaskDAO: Not invite self");
        uint256 amount = uint248(amountV);
        uint8 v = uint8(amountV >> 248);
        uint256 inviteAmount;
        if(inviter != address(0)){
            inviteAmount = amount * 5e16 / 1e18;
        }
        uint256 total = _totalSupply + amount + inviteAmount;
        require(total <= MAX_SUPPLY, "MaskDAO: Exceed max supply");
        require(minted(msg.sender) == 0, "MaskDAO: Claimed");
        bytes32 digest = keccak256(abi.encode(MINT_CALL_HASH_TYPE, msg.sender, amount));
        require(ecrecover(digest, v, r, s) == cSigner, "MaskDAO: Invalid signer");
        _totalSupply = total;
        _mint(msg.sender, amount);
        if(inviteAmount > 0){
            _mintInvite(inviter, inviteAmount);
        }
    }
}
