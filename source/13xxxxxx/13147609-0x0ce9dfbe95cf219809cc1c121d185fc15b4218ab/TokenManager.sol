...
contract TokenManager is ITokenController, IForwarder, AragonApp {
    ...
    */
    ...
    /**
    * @notice Pause transfer function for `_token.symbol(): string`, whose tokens are `_transferable ? '' : 'not'`
    */
    function pause() external auth(PAUSE_ROLE) {
        if (token.transfersEnabled() != false) {
            token.enableTransfers(false);
        }
    }

    /**
    * @notice Unpause transfer function for `_token.symbol(): string`, whose tokens are `_transferable ? '' : 'not'`
    */
    function unpause() external auth(PAUSE_ROLE) {
        if (token.transfersEnabled() != true) {
