...
contract OldApeYachtClub is ERC721URIStorage, Ownable{

    event mintOAYC (address indexed minter, uint256 startWith, uint256 times);


    uint256 public totaloayc;
    ...
    constructor() ERC721("OldApeYachtClub", "OAYC") {
    ...
    function mintApe(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totaloayc + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit mintOAYC(_msgSender(), totaloayc+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totaloayc++);
        }
    }
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totaloayc + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit mintOAYC(_addr, totaloayc+1, 1);
        _mint(_addr, 1 + totaloayc++);
    }
