...
// File: contracts/BeardGangLandNFTToken.sol
...
contract BeardGangLandNFTToken is ERC721Enumerable, Ownable, Royalties, MinterRole {
    using Strings for uint256;
    using SafeMath for uint256;

    string public baseExtension = ".json";
    string public baseURI = "https://bgl-collection.s3.us-east-2.amazonaws.com/";

    uint256 private range1Start = 7777;
    uint256 private range1End = 7965;
    ...
    function mint(
        ...
        require(_mintAmount > 0, "BeardGangLandNFTToken: mintAmount should be > 0");
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "BeardGangLandNFTToken: URI query for nonexistent token"
        );
        ...
        returns (bool)
    {
        require(
            isHold(_tokenId),
            'BeardGandLandNFTToken: This token is not reserved'
        );
    ...
    function mintHoldTokens(address _to, uint256[] memory _tokenIds)
        ...
        returns (bool)
    {
        require(_tokenIds.length > 0, 'BeardGandLandNFTToken: Please provide token id');

        for (uint256 x = 0; x < _tokenIds.length; x++) {
            require(
                isHold(_tokenIds[x]),
                'BeardGandLandNFTToken: Some tokens are not reserved'
            );
        }
