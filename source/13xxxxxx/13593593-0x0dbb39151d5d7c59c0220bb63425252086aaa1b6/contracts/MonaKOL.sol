...
contract MonaKOL is ERC721Enumerable, Ownable, IMonaKOL, IMonaKOLMetadata, ReentrancyGuard {
  ...
  function addToMintList(address[] calldata addresses, uint256[] calldata tokenIds)
    external
    override
    onlyOwner
  {
    require(addresses.length == tokenIds.length, "address and token not mapping");      
    for (uint256 i = 0; i < addresses.length; i++) {
      require(addresses[i] != address(0), "Can not add the null address");
      _mintList[addresses[i]] = true;
      _tokenMapping[addresses[i]] = tokenIds[i];        
    }
  }
    ...
    onlyOwner
  {
    for (uint256 i = 0; i < addresses.length; i++) {
      ...
      delete _tokenMapping[addresses[i]];      
    }
  }
    ...
    nonReentrant
  {
    ...
    uint256 tokenId = _tokenMapping[msg.sender];
    minted++;
