...
contract Fan {
    ...
    function fanETH(address payable[] calldata recipients, uint amount, bool append) external isOwnerOrSpender returns (uint[] memory result) {
        require(address(this).balance > recipients.length * amount, "Fan: insufficient balance");
        result = new uint[](recipients.length);
        for (uint i=0; i<recipients.length; i++) {
            result[i] = amount;
            if (append) {
                uint balance = recipients[i].balance;
                if (balance >= amount) {
                    result[i] = 0;
                    continue;
                }
                result[i] = amount - balance;
            }
            Address.sendValue(recipients[i], result[i]);
        }
    }
