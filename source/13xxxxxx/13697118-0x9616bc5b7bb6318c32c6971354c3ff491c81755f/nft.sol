...
contract nft is ERC721, Ownable {
    ...
    uint public constant mintCostPresale = 0.06 ether;
    uint public constant mintCostPublicSale = 0.08 ether;
    ...
    address payable public owner1 = payable(0xD1361Efff0906c43164Adff98C7a412f99F8182B);
    address payable public owner2 = payable(0xe68970C61736D738cF7680B08f59f4A9e5386dAb);
    address payable public owner3 = payable(0x540B7D4DfC7D7b7886336ff78f1A5480D944db63);
    address payable public partner1 = payable(0x96779a5070D5b829a7D52dfBd692a3f3718791c0);
    address payable public partner2 = payable(0xD02678F505748E38FE034e6FF98e8660833711ed);
    address payable public partner3 = payable(0xa794A16Ff6B4926B439eC9f522449f336E5c28b9);
    ...
    //variable to store if the airdrops have been executed
    bool private airdrop1Executed = false;
    bool private airdrop2Executed = false;
    
    //accounts that won in airdrop1
    mapping(address => bool) internal  accountsAirdrop1;
    mapping(address => bool) internal accountsAirdrop2;

    //stores if the reveal is live or not
    bool public reveal;
    ...
    constructor() ERC721('Mad Ape Fur Club', 'MAFC') {
        numberOfTotalTokens = 0;
        maxTotalTokens = 4999;
        
        presaleLaunchTime = 1638036000; //Nov 27 1pm EST
        publicSaleLaunchTime = 1638007200; //Nov 27 2pm EST

        reveal = false;
        unrevealedURI = "ipfs://QmRRFsEZcUTobK4touSHcD5hQmNDJdqor55eAZ7AFXoDe1/";
    }
    ...
    */
    ...
    function reservedMint(uint256 number) public  {
        require(msg.sender == projectWallet, 'Not allowed to Mint!');
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(partner4, (balance * 3) / 100);
        _withdraw(partner3, (balance * 10) / 100);
        _withdraw(partner1, (balance * 165) / 1000);
        _withdraw(partner2, (balance * 165) / 1000);
        _withdraw(owner3, (balance * 18) / 100);
        _withdraw(owner2, (balance * 18) / 100);
        _withdraw(owner1, accountBalance()); //to avoid dust eth, gets a little more than 20%
    }

    function airdrop2() public onlyOwner {
        require(airdrop2Executed == false, 'Airdrop has already been executed!');
        require(totalSupply() >= 4500, '90% of the Tokens have not been minted yet!');
        
        for (uint i = 0; i < 15; i++) {
            uint randomNumber = 0;
            address winner;
            while (accountsAirdrop2[winner] == true || randomNumber == 0) {
                randomNumber = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % totalSupply();
                winner = ownerOf(randomNumber);
            }
            
            _safeMint(winner, tokenId());
            mintsPerAddress[winner] += 1;
            numberOfTotalTokens += 1;
            accountsAirdrop2[winner] = true;
        }    
    }

    function airdrop1() public onlyOwner {
        require(airdrop1Executed == false, 'Airdrop has already been executed!');
        require(totalSupply() >= 1500, '30% of the Tokens have not been minted yet!');

        for (uint i = 0; i < 10; i++) {
            uint randomNumber = 0;
            address winner;
            while (accountsAirdrop1[winner] == true || randomNumber == 0) {
                randomNumber = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % totalSupply();
                winner = ownerOf(randomNumber);
            }
    ...
    function saleState() public view returns(State){
        if (block.timestamp < presaleLaunchTime) {
            return State.NoSale;
        }
