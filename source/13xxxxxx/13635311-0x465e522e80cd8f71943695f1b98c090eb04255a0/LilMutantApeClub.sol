...
contract LilMutantApeClub is ERC721URIStorage, Ownable{

    event MintLilMutant (address indexed minter, uint256 startWith, uint256 times);
   
    
    uint256 public totalLilMutant;
    ...
    function MintLilMutants(uint256 _times) payable public mintEnabled {
       
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalLilMutant + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintLilMutant(_msgSender(), totalLilMutant+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalLilMutant++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalLilMutant + _times <= presaleMax, "Mint amount will exceed total presale amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalLilMutant + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintLilMutant(_addr, totalLilMutant+1, 1);
        _mint(_addr, 1 + totalLilMutant++);
    } 
