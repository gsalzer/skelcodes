...
contract MoassAsses is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE = 3.5 * 10**16; // 0.035ETH Per MA
    uint256 private constant PRICE_PRESALE = 2 * 10**16; // 0.02ETH Per MA
    uint256 private constant PRICE_PREMINT = 0; // Free Per MA

    uint256 private constant MAX_ELEMENTS = 10000 ; // 10000 MAs in CrowdSale
    uint256 private constant MAX_ELEMENTS_PRESALE = 5 * 10**2; // 500 MAs in PreSale
    uint256 private constant MAX_ELEMENTS_PREMINT = 50; // 50 MAs for GiveAway.
    ...
    address private constant ownerAddress = 0xe0b435Ec076f0b0e4C56fbf6A47DB0eA94444781; // Wallet Address of Owner
    ...
    event CreateMoassAsses(address to, uint256 indexed id);

    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before buying MAs ;)");
        }
    ...
    constructor (string memory baseURI) ERC721("MOASS ASSES", "MA") {
        setBaseURI(baseURI);
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= _maxElements, "MINT: Please go to the Opensea to buy MA.");
        require(_ids.length <= _maxMint, "MINT: Current count exceeds maximum mint count.");
        
         if (_to != owner()) {
            require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of MA");
        }
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        _widthdraw(developerAddress, balance.mul(1).div(100));
