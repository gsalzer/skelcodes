...
contract CozyFrens is ERC721, Ownable {

    using SafeMath for uint256;

    string public FREN_PROVENANCE = ""; // IPFS URL WILL BE ADDED WHEN FRENS ARE ALL SOLD OUT

    string public LICENSE_TEXT = ""; // IT IS WHAT IT SAYS

    bool licenseLocked = false; // TEAM CAN'T EDIT THE LICENSE AFTER THIS GETS TRUE

    uint256 public constant frenPrice = 50000000000000000; // 0.05 ETH

    uint public constant maxFrenPurchase = 20;

    uint256 public constant MAX_FRENS = 2500;

    bool public saleIsActive = false;

    mapping(uint => string) public frenNames;

    // Reserve 125 Frens for team - Giveaways/Prizes etc
    uint public frenReserve = 125;

    event frenNameChange(address _by, uint _tokenId, string _name);

    event licenseisLocked(string _licenseText);

    constructor() ERC721("Cozy Frens", "FRENS") { }
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        FREN_PROVENANCE = provenanceHash;
    }
    ...
    function mintFren(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Fren");
        require(numberOfTokens > 0 && numberOfTokens <= maxFrenPurchase, "Can only mint 20 tokens at a time");
    ...
    // GET ALL FRENS OF A WALLET AS AN ARRAY OF STRINGS. WOULD BE BETTER MAYBE IF IT RETURNED A STRUCT WITH ID-NAME MATCH
