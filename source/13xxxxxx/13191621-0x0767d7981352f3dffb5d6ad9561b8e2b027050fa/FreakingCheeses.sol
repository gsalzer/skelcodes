...
contract FreakingCheeses is ERC721Enumerable, Ownable{
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;

    string public FC_PROVENANCE  = ""; 
    uint256 public constant cheesePrice = 10000000000000000; // 0.01 ETH
    ...
    constructor() ERC721("Freaking Cheeses","FC"){
    }
 ...
 function reserveCheeses(address _to, uint256 _reserveAmount) public onlyOwner {        
        uint supply = totalSupply();
    ...
    function mintCheese(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply of Cheeses");
        require(numberOfTokens <= MAX_NFT_PURCHASE,"Can only mint up to 20 per purchase");
        require(cheesePrice.mul(numberOfTokens) == msg.value, "Sent ether value is incorrect");
