...
// File: contracts/OceanIncNFTToken.sol

pragma experimental ABIEncoderV2;

pragma solidity ^0.8.0;

contract OceanIncNFTToken is ERC721Enumerable, Ownable, Royalties, MinterRole {
    using Strings for uint256;
    using SafeMath for uint256;
    ...
    string public baseURI = "https://thumbnail-ocean-collections.s3.us-east-2.amazonaws.com/";

    uint256 private range1Start = 9867;
    uint256 private range1End = 10000;
    ...
    function mint(address _to, uint256 _mintAmount)
        ...
        returns (bool)
    {
        require(_mintAmount > 0, "OceanIncNFTToken: mintAmount should be > 0");
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "OceanIncNFTToken: URI query for nonexistent token"
        );
        ...
        returns (bool)
    {
        require(
            isHold(_tokenId),
            'OceanIncNFTToken: This token is not reserved'
        );
    ...
    function mintHoldTokens(address _to, uint256[] memory _tokenIds)
        ...
        returns (bool)
    {
        require(_tokenIds.length > 0, 'OceanIncNFTToken: Please provide token id');

        for (uint256 x = 0; x < _tokenIds.length; x++) {
            require(
                isHold(_tokenIds[x]),
                'OceanIncNFTToken: Some tokens are not reserved'
            );
        }
