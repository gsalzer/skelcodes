...
// File contracts/UpsidedownRocks.sol
...
contract UpsidedownRocks is Ownable, ERC721Enumerable, ERC721Burnable, ReentrancyGuard {
  ...
  string private _baseTokenURI = "https://gateway.pinata.cloud/ipfs/Qmax6Qt8zNdUYZYSNzZkUPzJVPQxBqn4upEvMyERbsMVQA/";

  mapping (uint256 => uint256) public listings; // id to price

  constructor() ERC721("Upsidedown Rocks", "SKCOR") {
  }
  ...
  function listUpsidedownRock(uint256 tokenId, uint256 price) public {
    require(msg.sender == ownerOf(tokenId), "Must own upsidedown rock to sell");
    require(price > 0);
  ...
  function delistUpsidedownRock(uint256 tokenId) public {
    require(msg.sender == ownerOf(tokenId), "Must own upsidedown rock to delist");

    listings[tokenId] = 0;
    emit Delist(tokenId);
  }

  function buyUpsidedownRock(uint256 tokenId) public payable nonReentrant {
    require(listings[tokenId] > 0, "Upsidedown rock must be for sale to buy");
    require(msg.value >= listings[tokenId], "Must pay enough for upsidedown rock");
