...
// File contracts/CARTEL_LOOTS.sol
...
contract CARTELLOOTCRATES is Ownable, ERC721Enumerable, ReentrancyGuard {
    using SafeMath for uint256;

    string public CARTEL_LOOTS_PROVENANCE = "";
    ...
    uint256 public constant MAX_CARTEL_LOOTS = 10000;
    ...
    constructor() ERC721("CARTELLOOTCRATES", "CARTELLOOT") {}
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        CARTEL_LOOTS_PROVENANCE = provenanceHash;
    }

    // LICENSE
    function tokenLicense(uint256 _id) public view returns (string memory) {
        require(_id < totalSupply(), "CHOOSE TOKEN WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function claim(uint256 numberOfTokens) public payable nonReentrant {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_CARTEL_LOOTS, 'All tokens have been minted');
        require(numberOfTokens > 0 && numberOfTokens <= PURCHASE_LIMIT, 'Would exceed PURCHASE_LIMIT');

        for (uint256 i = 0; i < numberOfTokens; i++) {
            if (totalPublicSupply < MAX_CARTEL_LOOTS) {
