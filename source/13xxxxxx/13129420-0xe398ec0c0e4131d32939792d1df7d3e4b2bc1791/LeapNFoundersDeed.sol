...
// File: contracts/LeapNFoundersDeed.sol

pragma solidity 0.8.6;

contract LeapNFoundersDeed is ERC721, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;

    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public constant PER_ORDER_LIMIT = 10;
    uint256 public constant SALE_PRICE = 100000000000000000 wei; // 0.10 ETH
    ...
    address private constant _company = 0x557e5F91f60dc99aB499F6ffdbedFfBa44b441D9; // LeapN
    ...
    string private _baseUrl = "ipfs://QmdRtLu3jwjSxCMSx47JQnRzvQthnEFebZzNcr8tWAy3JG";
    bool private _baseUrlLocked = false;
    bool private _adminsSet = false;
    
    address[5] private _shareholders = [_picoPyro, _frederickLevins, _ferb, _wickedDig, _kingOfTheJungle];
    ...
    constructor() ERC721("LeapN Founders Deed", "DEED") {
        isActive = false;
        
        _shares[_picoPyro] = 900;
        _shares[_frederickLevins] = 100;
        _shares[_kingOfTheJungle] = 100;
        _shares[_ferb] = 51285;
        _shares[_wickedDig] = 27615;
    ...
    // The first 8 go to Ferb for giveaways and thank yous
    function mintGiveaways() private {
    ...
    /**
     * @dev Replaces the temp BaseUrl with the finalized version.  
     * To be called after the reveal
     * NOTE: This can only be called once and the base url cannot
     * be changed after this function has been called
     */
    function setRevealBaseUrl(string memory revealBaseUrl) external onlyOwnerOrAdmin {
        require(!_baseUrlLocked, "Nope, the base url has already been updated and locked");
        require(bytes(revealBaseUrl).length > 0, "revealBaseUrl is required");
        require(!endsWithForwardSlash(revealBaseUrl), "Don't include a trailing forward slash");

        _baseUrl = revealBaseUrl;
        _baseUrlLocked = true;
    }
    ...
    function endsWithForwardSlash(string memory _s) private pure returns (bool) {
        bytes memory stringBytes = bytes(_s);

        return keccak256(abi.encodePacked(stringBytes[stringBytes.length - 1])) == keccak256(abi.encodePacked("/"));
    }
