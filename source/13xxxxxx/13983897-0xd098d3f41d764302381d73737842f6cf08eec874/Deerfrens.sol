...
contract DeerFrens is ERC721URIStorage, Ownable{

    event mintDeer (address indexed minter, uint256 startWith, uint256 times);


    uint256 public totalDeers;
    uint256 public totalCount = 2222; //bruhTotal
    ...
    constructor() ERC721("DeerFrens", "DF") {
    ...
    function mintDeers(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalDeers + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit mintDeer(_msgSender(), totalDeers+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalDeers++);
        }
    }
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalDeers + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit mintDeer(_addr, totalDeers+1, 1);
        _mint(_addr, 1 + totalDeers++);
    }
