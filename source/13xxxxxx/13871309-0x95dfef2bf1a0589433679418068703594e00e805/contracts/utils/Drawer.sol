...
contract AtopiaDrawer {
	...
	function tokenURI(
		...
		uint16 specialTraitId = uint16(tokenTrait);
		uint16 traitAge = traits[11].getTraitByAge(age);
		pieces[10] = traits[11].getTraitContent(traitAge);

		// Furry Body = Face Colors
		if (specialTraitId < traits[10].itemCount()) {
			specialTraitId = ((tokenTrait >> 144) & 0xFFFF) == ((tokenTrait >> 128) & 0xFFFF) ? 1 : 0;
		}
		...
		string memory attributes = string(
			abi.encodePacked(
				...
				traits[10].getTraitName(specialTraitId),
				'"},{"',
				'trait_type":"Form","value":"',
				tokenId > 300 ? "Normal" : "Genesis"
			)
		);
		...
		for (uint256 i = 0; i < 10; i++) {
			...
			uint16 traitId = uint16(tokenTrait);
			attributes = string(
				abi.encodePacked(
					attributes,
					'"},{"',
			...
			string(
				abi.encodePacked(
					...
					Base64.encode(
						abi.encodePacked(
							'{"name":"',
							bytes(name).length > 0 ? name : string(abi.encodePacked("Atopia - ", tokenId.toString())),
