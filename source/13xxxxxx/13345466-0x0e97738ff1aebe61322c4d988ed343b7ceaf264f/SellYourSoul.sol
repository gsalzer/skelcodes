...
contract SellYourSoul is ERC721Enumerable, Ownable  {
    ...
    uint public constant TOKEN_LIMIT = 1000*1024; // 100*1024 for prod
    uint public constant ARTIST_PRINTS = 32; // 32 for testing, 32 for prod
    uint public constant PRICE = 30 * 1000000000000000; // 'finney':      200 * '1000000000000000',
    // The beneficiary is Transporter Control
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
     // 0xa5409ec958C83C3f309868babACA7c86DCB077c1 <- registerProxyAddress on mainnet 
     // 0xa5409ec958C83C3f309868babACA7c86DCB077c1 <- rinkeby 
    ...
    function mint(address toAddress) payable public {
        ...
        if (amount > 0) {
            payable(BENEFICIARY).transfer(amount); // send rest to BENEFICIARY
        }
    }
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory image = Base64.encode(bytes(generateImage(tokenId))); //generateBase64Image(tokenId);
    ...
    function generateImage(uint256 tokenId) public view returns (string memory) {
        //uint    morph = uint(bytes32(keccak256(abi.encodePacked(block.timestamp))));  // we are changing with time
