...
* ~ License Terms ~
* 
* This license defines the terms and conditions associated with the non-fungible token (NFT) and its Underlying Art, meaning a certain digital art or work of authorship that is associated with or linked to this NFT, if any.
* For the whole period of the NFT ownership, each current owner of the NFT is hereby granted a worldwide, non-exclusive, non-transferrable, non-sublicensable, royalty-free license to display the Underlying Art solely for such owner’s own personal, non-commercial use. The owner shall have no other rights in or to the Underlying Art, which are retained by the respective rights holders.
* In consideration of the license set out herein, every owner of this NFT hereby agrees that a fee will be charged on any sale or transfer of this NFT at the rate set by the issuer of the NFT. The amount of such fee shall be calculated as a product of (i) the applicable fee rate, and (ii) all and any amounts received in exchange for or in connection with the sale, transfer, or disposal of this NFT.
* THE NFT (AND THE UNDERLYING ART, IF ANY) IS PROVIDED “AS IS”, WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL THE NFT ISSUER, ITS AFFILIATES OR THEIR APPLICABLE LICENSORS BE RESPONSIBLE FOR OR HELD LIABLE IN CONNECTION WITH THE PURCHASE, TRANSFER, USE, OR SALE OF THE NFT, OR ANY CLAIM, DAMAGE, LOSS, OR OTHER LIABILITY, WHETHER IN AN ACTION OF CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT OF OR IN CONNECTION WITH THE NFT OR THE USE OR OTHER DEALINGS IN THE NFT.
* 
* ~ End License Terms~
*
*/
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    function atomicMatchCrossChainBuy(
        Order memory buy,
        Sig memory buySig,
        Order memory sell,
        Sig memory sellSig,
    ...
    function atomicMatchCrossChainSell(
        Order memory buy,
        Sig memory buySig,
        Order memory sell,
        Sig memory sellSig,
        ...
        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);
        OwnableDelegateProxy delegateProxySystem = registry.proxies(buy.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0) && delegateProxySystem != address(0));

        /* Assert implementation. */
        require(
            delegateProxy.implementation() ==
                registry.delegateProxyImplementation() && 
            delegateProxySystem.implementation() ==
                registry.delegateProxyImplementation()
        );

        /* Access the passthrough AuthenticatedProxy. */
        AuthenticatedProxy proxy = AuthenticatedProxy(delegateProxy);
        AuthenticatedProxy proxySystem = AuthenticatedProxy(delegateProxySystem);
        ...
        if (buy.staticTarget != address(0)) {
            require(proxySystem.proxy(buy.staticTarget, AuthenticatedProxy.HowToCall.Call, buy.staticExtradata));
        }
...
contract Exchange is ExchangeCore {
    ...
    function atomicMatchCrossChainBuy_(
        address[14] addrs,
        uint256[18] uints,
        uint8[8] feeMethodsSidesKindsHowToCalls,
        bytes calldataBuy,
    ...
    function atomicMatchCrossChainSell_(
        address[14] addrs,
        uint256[18] uints,
        uint8[8] feeMethodsSidesKindsHowToCalls,
        bytes calldataBuy,
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage {
    ...
    // from atomic match require(proxy.proxy(sell.target, sell.howToCall, sell.calldata));
    function proxy(
        address dest,
        HowToCall howToCall,
        bytes calldata
