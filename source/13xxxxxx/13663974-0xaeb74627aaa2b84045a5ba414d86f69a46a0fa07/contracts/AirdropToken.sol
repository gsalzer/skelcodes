...
//
// $AIRDROP is the first meme coin that redistribute a % of the daily traded volume
//
// Find out more on https://airdropfan.com
//
...
contract AirdropToken is Context, IERC20, Ownable {
    ...
    uint256 public _baseFee = 2;
    uint256 private _beforeBaseFee = _baseFee;
    uint256 public _liquidityFee = 8;
    uint256 private _beforeLiquidityFee = _liquidityFee;
    uint256 public _maxTokensForTx = 500000000000 * 10**18;
    uint256 private maxForLiquidity = 20000000 * 10**18;
    ...
    mapping (address => bool) private _allowedLiqIncrModifier;
    ...
    modifier onlyWithLiqIncrPermission {
        require(_allowedLiqIncrModifier[msg.sender], "Only allowed users can edit this status");
        _;
    }
    ...
    constructor (address payable stakingAddress) public {
        ...
        _allowedLiqIncrModifier[owner()] = true;
    ...
    function setLiquidityIncreaseEnabled(bool _enabled) external onlyWithLiqIncrPermission {
        liquidityIncreaseEnabled = _enabled;
        emit LiquidityIncreaseEnabledUpdated(_enabled);
    }

    function toggleLiquidityIncreaseEnabledModifier(address _newAdd, bool _enabled) public onlyOwner {
        _allowedLiqIncrModifier[_newAdd] = _enabled;
    }
    ...
    function removeAllFee() private {
        if(_baseFee == 0 && _liquidityFee == 0) return;
        _beforeBaseFee = _baseFee;
    ...
    function sendFundsToWallet(uint256 amount) private {
        swapTokensForEth(amount);
        uint balance = address(this).balance;
        _stakingAddress.transfer(balance / 2);
        _marketingAddress.transfer(balance / 2);
    }
    ...
    function _setMaxForLiquidity(uint _newMax) external onlyOwner() {
        maxForLiquidity = _newMax;
    }

    function restoreAllFee() private {
        _baseFee = _beforeBaseFee;
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        bool overMinTokenBalance = contractTokenBalance >= maxForLiquidity;
        if ( overMinTokenBalance && !inLiquidityIncrease && from != uniswapV2Pair && liquidityIncreaseEnabled ) {
            contractTokenBalance = maxForLiquidity;
    ...
    function uniswapAndAddLiquidity(uint256 contractTokenBalance) private pauseBrewerySwap {
        ...
        sendFundsToWallet(portionForFees);
