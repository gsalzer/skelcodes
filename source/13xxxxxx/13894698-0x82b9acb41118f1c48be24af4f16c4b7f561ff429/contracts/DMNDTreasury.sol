...
contract DMNDTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public rDMND;
  uint256 public rDMNDQueue; // Delays change to rDMND address

  uint256 public totalReserves; // Risk-free value of all assets
  uint256 public totalDebt;

  constructor(address _DMND, uint256 _blocksNeededForQueue) {
  ...
  /**
        @notice allow approved address to deposit an asset for DMND
        @param _amount uint
  ...
  function deposit(
    ...
    // mint DMND needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(DMND).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn DMND for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IDMNDERC20(DMND).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(rDMND).balanceOf(msg.sender); // Can only borrow against rDMND held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with DMND
        @param _amount uint
     */
  function repayDebtWithDMND(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IDMNDERC20(DMND).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, DMND, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(DMND).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(DMND).totalSupply().sub(totalDebt));
  }
  ...
  // get the amount of DMND mintable by treasury
  /**
        @notice returns DMND valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      value_ = _amount.mul(10**IERC20(DMND).decimals()).div(
      ...
      uint256 reserve = token0 == DMND ? reserve1 : reserve0;
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.RDMND) {
      // 9
      rDMNDQueue = block.number.add(blocksNeededForQueue);
    } else return false;
