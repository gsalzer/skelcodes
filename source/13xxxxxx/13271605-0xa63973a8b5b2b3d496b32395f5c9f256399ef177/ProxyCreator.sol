...
// File: contracts/ProxyCreator.sol
...
contract ProxyCreator is Ownable {
    ...
    function create(uint256 totalSupply, uint8 decimals, string calldata name, string calldata symbol, uint256 threshold, string calldata description) public onlyOwner returns (address) {
        return uToken = IUnicFactory(factory).createUToken(totalSupply, decimals, name, symbol, threshold, description);
    }

    function issue() public onlyOwner {
        Converter(uToken).issue();
    }

    function refund(address _to) public onlyOwner {
        Converter(uToken).refund(_to);
    }
    ...
    function deposit(uint256[] calldata tokenIDs, uint256[] calldata amounts, address contractAddr) public {
        for (uint8 i=0; i<200; i++) {
        ...
        if(isPublic) {
            Converter(uToken).deposit(tokenIDs, amounts, contractAddr);
        }
        else {
        ...
        if(reward != 0) {
            Converter(uToken).transfer(msg.sender, reward);
        }
    }
}
