...
// File contracts/WaLINKToken.sol
...
contract WaLINKToken is AccessControl, ERC20("Wasabi LINK", "waLINK") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "waLINK: YumVault is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "waLINK: YumVault is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"waLINK: YumVault's ceiling was breached.");
    require(!paused[msg.sender], "waLINK: user is currently paused.");
