...
contract Painda is ERC721Enumerable, Ownable{
    ...
    string private whitelistSecret;
    mapping (uint256 => string) _tokenURIs;

    constructor() ERC721("Pain Da","PDA"){
        whitelistSecret = "painda.io";
    }
    ...
    function whitelistMint(uint numberOfTokens, uint code) public payable {
        require(!msg.sender.isContract(), "Owner should not be a contract");
        require(whitelistSaleIsActive, "Whitelist sale is not active at the moment");
        require(numberOfTokens > 0, "Number of tokens can not be less than or equal to 0");
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(numberOfTokens <= MAX_NFT_PURCHASE,"Can only mint up to 10 per purchase");
        uint256 price = NFT_PRICE * 8 / 10;
        require(price.mul(numberOfTokens) == msg.value, "Sent ether value is incorrect");

        uint256 checkCode = getWhitelistCode(msg.sender);
        require(code == checkCode, "Whitelist code is wrong.");
    ...
    function getWhitelistCode(address addr) public view returns (uint256)  {
        bytes memory temp = abi.encodePacked(whitelistSecret, addr);
        string memory temp1 = Base64.encode(temp);
        bytes memory temp2 = abi.encodePacked(temp1);
        return uint256(keccak256(temp2)) / 2;
    }
    ...
    function hp(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        address owner = ownerOf(tokenId);
        bytes memory temp = abi.encodePacked(owner, tokenId);
        return uint256(keccak256(temp)) % 10000000;
    }
