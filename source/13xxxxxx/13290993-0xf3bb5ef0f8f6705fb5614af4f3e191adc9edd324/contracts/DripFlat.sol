...
contract Drip is ERC721Enumerable, IERC2981, ReentrancyGuard, Ownable {

    uint256 public PRESALE_PRICE = 35000000000000000; // 0.035 eth
    uint256 public FULL_PRICE = 50000000000000000; // 0.05 ETH

    uint public WIDTH = 16;
    uint public HEIGHT = 18;

    uint256 public SIZE = WIDTH * HEIGHT;

    uint internal NEXT_TOKEN_ID = 0;

    // Allow for starting/pausing sale
    bool public MINT_ENABLED = false;
    bool public PRESALE_ENABLED = false;

    // Royalty, in basis points
    uint internal ROYALTY_BPS = 1000;

    // sell 50 @ 0.035 ETH
    // sell 238 @ 0.05 ETH
    uint internal NUM_PRESALE = 50;

    string internal BASE_TOKEN_URI;

    constructor(string memory baseURI) ERC721("Drip", "DRIP") Ownable() {
    ...
    // ERC2891 royalty function, for ERC2891-compatible platforms
    function royaltyInfo(uint256, uint256 _salePrice) override external view returns (
        address receiver,
        uint256 royaltyAmount
    ) {
        return (owner(), (_salePrice * ROYALTY_BPS) / 10000);
    }

    // Update royalty rate, but unable to rug (> 30%)
    function updateRoyaltyBPS(uint8 newRoyaltyBPS) public onlyOwner {
        require(newRoyaltyBPS <= 3000, "No royalty greater than 30%");
        ROYALTY_BPS = newRoyaltyBPS;
    }

    function getX(uint256 tokenId) public view returns (uint256) {
        // X coordinate
        return tokenId % WIDTH;
    }


    function getY(uint tokenId) public view returns (uint) {
        // Y coordinate
        return tokenId / WIDTH;
    }
    ...
    function mint() public payable nonReentrant {
        require(MINT_ENABLED || PRESALE_ENABLED, "Minting is not currently enabled right now");
        require(balanceOf(msg.sender) < 10, "Cannot mint more than 10 per address");
        if (PRESALE_ENABLED && !MINT_ENABLED) {
            // Presale enabled, full sale not enabled yet
            // Only a certain amt can be minted
            require(NEXT_TOKEN_ID < NUM_PRESALE, "No more presale minting available");
            require(PRESALE_PRICE <= msg.value, "Must send 0.035 ETH for mint");
        } else if (MINT_ENABLED) {
            // Full sale now enabled
            // Yeehaw
            // If 50 still havent been minted yet, keep the prices at PRESALE_PRICE
            require(NEXT_TOKEN_ID < SIZE, "No more minting available");

            if (NEXT_TOKEN_ID < NUM_PRESALE) {
                require(PRESALE_PRICE <= msg.value, "Must send 0.035 ETH for mint");
            } else {
                require(FULL_PRICE <= msg.value, "Must send 0.05 ETH for mint");
            }
        }
    ...
    function startPresale() public onlyOwner {
        require(!MINT_ENABLED, "Cannot start presale while full sale is in effect");
    ...
    function startMint() public onlyOwner {
        require(!PRESALE_ENABLED, "Cannot start full sale while presale is in effect");
        MINT_ENABLED = true;
    }
