...
// File contracts/FlowCapacitor.sol
...
contract FlowCapacitor is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Strings for string;

    string private _baseUriString;
    bool public salesOpened = false;
    uint256 public maxMagnets = 3000;
    uint256 public lastMagnetID;
    uint256 public bonusesAmount = 30;
    uint256 public currentPrice = 0.049 ether;
    uint256 public constant MAX_PER_TRANSACTION = 20;

    constructor(string memory baseUriString) ERC721("Flow Capacitor", "FlowCapacitor") {
    ...
    function _mintMagnet(address to, uint256 count) private {
        uint256 _lastMagnetID = lastMagnetID;
        for (uint256 i = 0; i < count; i++) {
            _lastMagnetID++;
            _safeMint(to, _lastMagnetID);
        }
        lastMagnetID = _lastMagnetID;
    }

    function mintMagnet(uint256 count) external payable nonReentrant {
        require(salesOpened == true
        && count > 0
        && count <= MAX_PER_TRANSACTION
        && totalSupply().add(count) <= maxMagnets.sub(bonusesAmount)
            && msg.value >= currentPrice.mul(count)
        );
        _mintMagnet(msg.sender, count);
    }

    function mintBonusMagnet(address to, uint256 count) public onlyOwner {
        if (to == address(0)) {
            to = msg.sender;
        }
        require(count > 0 && count <= bonusesAmount);
        _mintMagnet(to, count);
        bonusesAmount -= count;
    }

    function start() public onlyOwner {
        require(salesOpened == false);
        salesOpened = true;
    }
