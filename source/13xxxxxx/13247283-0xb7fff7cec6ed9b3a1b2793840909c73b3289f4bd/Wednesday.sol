...
contract Wednesday is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, PaymentSplitter {
    
    using Strings for uint256;
    
    uint256 private _price = 0.006529 ether;

    string public _baseTokenURI = 'https://ipfs.io/ipfs/QmWC4ynsF4Lt6TyAeij4C2WQGbZpF32uK6hm7Y281VcPZi/';
    
    string public PROVENANCE = '';

    uint256 public MAX_TOKENS_PER_TRANSACTION = 20;

    uint256 public MAX_SUPPLY = 6529;

    mapping(uint => string) private _owners;

    // Withdrawal addresses
    address t1 = 0x7305ce3A245168dDc87c3009F0B4b719BC4519F5;
    address t2 = 0xD02bf9b3DA78BEc791014EB3cEecA65990cb046F;

    address[] addressList = [t1, t2];
    uint256[] shareList = [50, 50];

    constructor()
    ERC721("Wednesday", "WED")
    ...
    function airdrop(address _to, uint256 _tokenId) external onlyOwner {
        uint256 supply = totalSupply();
        require( supply + 1 <= MAX_SUPPLY, "Number of tokens exceeds supply" );
        _safeMint(_to, _tokenId);
    }

    function mint(uint256 _count) public payable {
        uint256 supply = totalSupply();
        require( _count <= MAX_TOKENS_PER_TRANSACTION, "You can mint a maximum of 20 tokens per transaction" );
        require( supply + _count <= MAX_SUPPLY,        "Number of tokens exceeds supply" );
