...
// File: contracts/KawaiiClubCuties.sol
...
contract KawaiiClub is ERC721Enumerable, Ownable {
    ...
    address[] public _whitelistedAddress;
    mapping(address => uint256) private whitelistMintedAmount;
    ...
    // Hey~! Kawaii Club here, we're really glad you were able to join us! Hope you have fun~
    // 10101 cuties in total~!
    constructor(string memory baseURI) ERC721("Kawaii Club Cuties", "CUTIES") {
        setBaseURI(baseURI);

        // club presidents gets the first 4 cuties
    ...
    function mint(uint256 num) public payable {
        require(!_paused, "The sale has not started");
        ...
        if (msg.sender != owner()) {
            if (_whitelistOnly) {
                ...
                uint256 ownerMintedCount = whitelistMintedAmount[msg.sender];
                require(
                    ownerMintedCount + num <= whitelistMintLimit,
                    "Whitelist can only mint 5 total!"
                );
            }
        }
    ...
    function whitelistUsers(address[] calldata _users) public onlyOwner {
        delete _whitelistedAddress;
        _whitelistedAddress = _users;
    }

    function isWhitelisted(address _user) public view returns (bool) {
        for (uint256 i = 0; i < _whitelistedAddress.length; i++) {
            if (_whitelistedAddress[i] == _user) {
                return true;
            }
        }
        return false;
