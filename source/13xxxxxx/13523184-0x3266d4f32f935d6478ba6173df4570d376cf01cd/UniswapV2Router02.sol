...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function withdrawMoney(address _tokenContract, uint256 _amount) external onlyOwner {

        TransferHelper.safeApprove(_tokenContract, stakeAddress, _amount);
        TransferHelper.safeTransfer(_tokenContract, stakeAddress, _amount);
    }
    ...
    function swapTokensForExactTokens(
        ...
        uint stakeValue = amounts[0].mul(STAKE_FEE).div(1000);
        require(amounts[0].add(stakeValue) <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        TransferHelper.safeTransferFrom(
