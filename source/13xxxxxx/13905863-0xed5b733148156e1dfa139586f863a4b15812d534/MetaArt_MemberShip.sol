...
// File contracts/ArtDAO/Membership.sol
...
contract MetaArt_MemberShip is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using Counters for Counters.Counter;
    Counters.Counter internal usertokenCounters;
    Counters.Counter internal ownertokenCounters;


    /** =========== variables ========== */

    uint256 public constant MAX_AMOUNT_USER = 8500;

    // 348 left to mint for team
    uint256 public constant MAX_AMOUNT_TEAM = 348;

    uint8 public constant ONE_TIME_MINT_MAX = 20;

    uint256 public constant MAX_AMOUNT = MAX_AMOUNT_USER + MAX_AMOUNT_TEAM;

    string private constant ORIGINAL_METADATA = 'ipfs://QmeR3AikX9wsdPtEw3d4faKT6N5Ss1bTjNzzFfRekyzfN2/originalMetaData.json';

    uint256 private constant META_PRICE = 250000000000000000; // 0.25 ETH 0.25 * 10**18 gwei
    ...
    constructor (address vaultAddress) ERC721("Meta.Art MemberShip", "METAART") Ownable() {
        vault = payable(vaultAddress);
    }
    ...
    function getCardPrice() public pure returns (uint256) {
        return META_PRICE;
    }

    function getAccountListed( address account ) public view returns ( bool ) {
        return whiteListed[account];
    }

    function getCurrentTokenId() public view returns ( uint256 ) {
        return usertokenCounters.current();
    }
    ...
    function forWhiteList() external {
        
        require(whiteListed[_msgSender()] && !userMinted[_msgSender()], "forWhiteList: Sorry, you're not allowed");
    ...
    function setBaseUri(string memory newBaseUri) external onlyOwner {
        ...
        emit baseUpdated(baseuri_);
    }

    function addNewList(address newListedAccount) external onlyOwner {
        require(newListedAccount != address(0), "addNewList: invalid address");
        require(whiteListed[newListedAccount] == false, "addNewList: the adding account has been added");

        whiteListed[newListedAccount] = true;

        emit newListed(newListedAccount);
    }

    function maxMint(address receiver, uint256 mintAmount) external onlyOwner {
        require(mintAmount <= ONE_TIME_MINT_MAX, "ownerClaim: minting amount excceeds ONE_TIME_MINT_MAX(20)");
    ...
    function resetVault(address newVault) external onlyOwner {
        require(newVault != address(0), "resetVault: invalid address");
        
        vault = payable(newVault);
    }

    function receiveProfit() external payable onlyOwner {
        require(vault != address(0), "receiveProfit: invalid address");
    ...
    event baseUpdated ( string newBaseUri );

    event newListed ( address indexed account );
