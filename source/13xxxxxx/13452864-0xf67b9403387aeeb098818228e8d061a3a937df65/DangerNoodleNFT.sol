...
// File contracts/DangerNoodleNFT.sol

pragma solidity ^0.8.0;

contract DangerNoodleNFT is Ownable, ERC721Enumerable, OpenSeaTradableNFT {
    using SafeMath for uint256;

    string public baseURI = '';

    uint256 public price = 0.04 ether;
    uint256 public preSaleBuyInPrice = 0.04 ether;

    uint256 public MAX_NOODLES = 10000;
    uint256 public MAX_NOODLE_PURCHASE = 10;
    uint256 public MAX_PRESALE_NOODLE_PURCHASE = 3;
    uint256 public PURCHASEABLE_PRESALES = 500;
    uint256 public NOODLE_RESERVE = 300;
    ...
    function maxAvailableForSell() public view returns (uint256) {
        return MAX_NOODLES.sub(NOODLE_RESERVE).sub(totalSupply());
    }

    function mintPreSaleNoodles(uint256 numberOfTokens) external payable {
        require(whitelist[msg.sender], 'Whitelist required for pre sale');
        require(preSaleIsActive, 'Pre sale must be active to mint Noodles');
        require(
            presalePurchases[msg.sender].add(numberOfTokens) <=
                MAX_PRESALE_NOODLE_PURCHASE,
            'Can only mint MAX_PRESALE_NOODLE_PURCHASE tokens during the presale'
        );
        require(
    ...
    function mintNoodles(uint256 numberOfTokens) external payable {
        require(saleIsActive, 'Sale must be active to mint Noodles');
        require(
            numberOfTokens <= MAX_NOODLE_PURCHASE,
            'Can only mint MAX_NOODLE_PURCHASE tokens at a time'
        );
        require(
            numberOfTokens <= maxAvailableForSell(),
            'Purchase would exceed max supply of Noodles'
        );
        require(
            msg.value >= price.mul(numberOfTokens),
            'Ether value sent is not correct'
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 devShare = balance.mul(30).div(100);
    ...
    function reserveNoodles(uint256 numberOfTokens) external onlyOwner {
        require(
            numberOfTokens > 0 && numberOfTokens <= NOODLE_RESERVE,
        ...
        NOODLE_RESERVE = NOODLE_RESERVE.sub(numberOfTokens);
    }
}
