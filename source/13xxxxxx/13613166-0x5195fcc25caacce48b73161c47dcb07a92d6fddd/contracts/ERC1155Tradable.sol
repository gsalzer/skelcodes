...
contract ERC1155Tradable is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable {
  ...
  function create_and_airdrop(
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      _mint(_addresses[i], _id, _quantities[i], "0x3333");
      _initialSupply++;
    }
  ...
  function airdrop(
    ...
    uint256 _initialSupply = tokenSupply[_id];
    for (uint256 i = 0; i < _addresses.length; i++) {
      _mint(_addresses[i], _id, _quantities[i], "0x3333");
      _initialSupply++;
    }
  ...
  function give_free_mint(
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      freeMints[_id][_addresses[i]] += _quantities[i];
    }
  }

  function claim_free_mint(
    uint256 _id,
    uint256 _quantity
  ) public {

    require(freeMints[_id][msg.sender] >= _quantity, "You don't have enough free mint passes");
    _mint(msg.sender, _id, _quantity, "0x3333");
    freeMints[_id][msg.sender] -= _quantity;
