...
contract CompensationVault is ICompensationVault, CompensationVaultStorage {
        ...
        onlySigner(compensationParams.signer)
    {
        ...
        //                                                                      quote
        //  actual compensation amount = expected compensation amount * ---------------------
        //                                                                  actual received
        uint256 compensationAmount = compensationParams.compensationAmount.mul(compensationParams.quote).div(
            amount1Out
        );

        // cap up to daily limit and tx limit
        compensationAmount = calcCompensationAmount(compensationAmount);

        uint128 _totalCompensation = totalCompensation;
        uint128 _debt = debt;

        totalCompensation = uint128(uint256(_totalCompensation).add(compensationAmount));
        debt = uint128(uint256(_debt).add(compensationAmount));
