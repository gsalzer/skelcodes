...
/**
 * UtilityToken Contract (The native token of MetaKnights)
 * @dev Extends standard ERC20 contract
 */
contract UtilityToken is Context, IERC20, Ownable {
    using SafeMath for uint256;

    // Constants
    uint256 public SECONDS_IN_A_DAY = 86400;

    uint256 public INITIAL_ALLOTMENT = 0;
    ...
    uint256 public emissionPerDay = 5 * (10 ** 18);
    ...
    /**
     * @dev When accumulated utility tokens have last been claimed for a testNFT index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(IERC721Enumerable(_NFTAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IERC721Enumerable(_NFTAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated utility tokens for a testNFT token index.
     */
    ...
    function setIntitalAllotment(uint256 _initialAllotment) public onlyOwner {

        INITIAL_ALLOTMENT = _initialAllotment;
    }

    function setEmissionPerDay(uint256 _emissionPerDay) public onlyOwner {

        emissionPerDay = _emissionPerDay;
    }

    /**
     * @dev Claim mints utility tokens and supports multiple testNFT token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IERC721Enumerable(_NFTAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IERC721Enumerable(_NFTAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
        ...
        require(totalClaimQty != 0, "No accumulated utility tokens");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the testNFTs contract. For better UX.
