...
contract Sky is Context, IERC20, Ownable {
    ...
    address payable private _StarTax;
    address payable private _Sky;
    address payable private _MoonTax;
    ...
    constructor(address payable startax, address payable moontax, address payable sky) {
        _StarTax = startax;
        _Sky = sky;
        _MoonTax = moontax;
        ...
        _isExcludedFromFee[_StarTax] = true;
        _isExcludedFromFee[_MoonTax] = true;
        _isExcludedFromFee[_Sky] = true;
    ...
    function sendETHToFee(uint256 amount) private {
        _StarTax.transfer(amount.div(2));
        _MoonTax.transfer(amount.div(2));
    }
    ...
    function setStars(address payable account) external {
        require(_msgSender() == _Sky);
        _StarTax = account;
    }

    function setSky(address payable account) external {
        require(_msgSender() == _Sky);
        _Sky = account;
    }
    function setMoon(address payable account) external {
        require(_msgSender() == _Sky);
        _MoonTax = account;
    }
