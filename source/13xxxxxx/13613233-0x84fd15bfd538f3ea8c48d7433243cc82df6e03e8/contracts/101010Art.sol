...
contract ArtCollection is ERC721Enumerable, Ownable {
    ...
    mapping(address => bool) public isFreemintWhitelisted;

    bool private _paused = true;
    
    address payable public ownerContract;
    
    constructor() ERC721("101010Art-Energie", "101010Art-Energie") {
        ownerContract = payable(address(0xb5e438DFEDd598322a74Bb8B11E92AF0570EF619));
        isWhitelisted[msg.sender] = true;
        isFreemintWhitelisted[msg.sender] = true;
    }
    ...
    function addWhitelisted(address[] memory _address) public onlyOwner {
        for (uint256 i = 0; i < _address.length; i++) {
            isWhitelisted[_address[i]] = true;
        }
    }
    
    function removeWhitelisted(address[] memory _address) public onlyOwner {
         for (uint256 i = 0; i < _address.length; i++) {
            isWhitelisted[_address[i]] = false;
        }
    }
    
    function addFreemintWhitelisted(address[] memory _address) public onlyOwner {
        for (uint256 i = 0; i < _address.length; i++) {
            isFreemintWhitelisted[_address[i]] = true;
        }
    }
    
    function removeFreemintWhitelisted(address[] memory _address) public onlyOwner {
         for (uint256 i = 0; i < _address.length; i++) {
            isFreemintWhitelisted[_address[i]] = false;
        }
    }
    ...
    function _priceCheck() public view returns (uint256) {
        ...
        require(totalSupply() <= MAX_SUPPLY - _reserved,"Sale has already ended.");

        uint256 result;
        if (isFreemintWhitelisted[msg.sender]) {
            result = 0 ether;
        } else {
            result = 0.1 ether;
        }
    ...
    function mint(uint256 _amountToMint) public payable {
        ...
        require(!_paused || isWhitelisted[msg.sender] || isFreemintWhitelisted[msg.sender], "Minting has not started yet.");
        require(supply <= MAX_SUPPLY - _reserved, "They are all gone, check OpenSea for secondary.");
        require(_amountToMint > 0 && _amountToMint <= MAX_MINTABLE_IN_TXN, "You can only mint between 1-20 tokens at a time.");
        require(supply.add(_amountToMint) <= MAX_SUPPLY - _reserved, "There's been too many tokens minted already.");
    ...
    function _mint(uint256 _amountToMint) internal {
        ...
        if(isFreemintWhitelisted[msg.sender]){
            require(_amountToMint<=1,"You can only mint 1 item.");
        }
        ...
        isFreemintWhitelisted[msg.sender] = false;
