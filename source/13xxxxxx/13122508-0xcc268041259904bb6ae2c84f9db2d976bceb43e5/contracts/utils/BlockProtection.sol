...
interface IBlockProtection {
    ...
    function callWithBlockProtection(address _to, bytes calldata _data, uint256 _blockNumber) external payable returns (bytes memory _returnData) ;
...
contract BlockProtection is MachineryReady, IBlockProtection {
    ...
    function callWithBlockProtection(address _to, bytes memory _data, uint256 _blockNumber) external payable override onlyMechanic() returns (bytes memory _returnData) {
