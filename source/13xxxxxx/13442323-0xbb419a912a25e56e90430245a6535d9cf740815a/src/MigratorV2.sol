...
contract MigratorV2 {
    ...
    function migrateAndFarm(uint256 tokenId) public {
        (address own, uint256 time) = _migrate(tokenId);
    ...
    function getStatus(uint256 tokenId) internal view returns (address owner, uint256 timestamp) {
        ...
        timestamp = time == 0 ? block.timestamp : time;
    }


    function _migrate(uint256 tokenId) internal returns (address owner, uint256 timestamp) {
        require(!migrated[tokenId], "already migrated");

        //Check what the orc is doing
        ( owner,  timestamp) = getStatus(tokenId);
