...
contract CyberGirlsCafe is IERC165, ERC721Metadata, Ownable {
    uint256 public immutable TOTAL_TOKENS;
    uint256 public constant MAX_TOKENS_PER_TXN = 20;
    uint256 public constant MAX_TOKENS_PER_WL = 4;
    ...
    function setMintState(bool publicSaleActive, bool presaleActive) public onlyOwner {
        _publicSaleActive = publicSaleActive;
        _presaleActive = presaleActive;
    }

    function mint(uint256 count) external payable {
        require(_publicSaleActive, "Public mint is not allowed");
    ...
    function presaleMint(uint256 count) external payable {
        require(_presaleActive, "Whitelist mint is not allowed");
        require(count > 0 && count <= MAX_TOKENS_PER_WL, "Count should be from 1 to 4");
