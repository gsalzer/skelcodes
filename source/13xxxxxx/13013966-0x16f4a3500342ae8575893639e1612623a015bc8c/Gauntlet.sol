...
contract Gauntlet is ERC721, Ownable {
    ...
    // The token ids are split into 10 tiers of 1000. If a tier sells quickly, the price will rise for the next tier. 
    uint public constant tierSize = 1000; 
    // When the first token from a tier is minted, we set the block number here, which will help us calculate the
    // estimated time it takes for the tier to sell.
    uint public currentTierStartBlockNum;

    uint public currentTier = 0;

    // A block takes about 13 seconds on average to get mined.
    uint public constant AVG_BLOCK_SECS = 13;
    ...
    mapping (address => bool) private earlyAccessClaimedToken;
    ...
    function setSaleState(uint newState) public onlyOwner {
        ...
        saleState = newState;
        if (saleState == 2) {
            currentTierStartBlockNum = block.number;
        }
    }
    ...
    function handlePriceAdjustment() internal {
        uint numBlocksInTier = block.number.sub(currentTierStartBlockNum);
        uint secondsInTier = numBlocksInTier.mul(AVG_BLOCK_SECS);
        if (secondsInTier < 60 * 30) {
            // If tier sold in less than 30 minutes, price increases by 0.25 ETH.
            nftPrice = nftPrice.add(250000000000000000);
        }
    ...
    function _checkEarlyAccess(address sender, uint numberOfTokens) view internal {
        ...
        require(!earlyAccessClaimedToken[sender], "Sender already claimed their gauntlet");
        require(numberOfTokens == 1, "Can only mint 1 token in the early access sale");
    }

    function isApprovedForEarlyAccess(address addr) view public returns (bool) {
        return earlyAccessAllowList[addr];
    }

    function hasClaimedEarlyAccess(address addr) view public returns (bool) {
        return earlyAccessClaimedToken[addr];
    }
    ...
    */
    function mintNft(uint numberOfTokens) public payable {
        ...
        if (saleState == 1) {
            ...
            earlyAccessClaimedToken[msg.sender] = true;
        } else if (saleState == 2) {
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                if (thisTier > currentTier) {
                    ...
                    // Reset the tier start block number.
                    currentTierStartBlockNum = block.number;
