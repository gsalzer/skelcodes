...
// File: contracts/ApeverseERC721.sol
...
contract ApeverseERC721 is ERC721, Ownable, ReentrancyGuard {
  using Address for address;
  ...
  uint256 constant public MINT_PRICE = 0.03 ether;
  uint256 constant public LAUNCH_MINT_PRICE = 0.05 ether;
  uint256 constant public MAX_SUPPLY = 6666;
  uint256 constant public MAX_RESERVED = 84;
  ...
  function mintPresale(uint256 amount) public payable nonReentrant {
   ...
   function mintPublicSale(uint256 amount) public payable nonReentrant {
    require(publicSaleActive,                       "Public sale has not started");
    require(msg.value == LAUNCH_MINT_PRICE * amount, "Invalid Ether amount sent");
    require(amount <= MAX_MINT,               "Exceeds the maximum amount to mint at once");
...
contract Apeverse is ApeverseERC721 {
  using Address for address;
  ...
  uint256 constant public UPDATE_NAME_PRICE = 500 ether;
  uint256 constant public UPDATE_DESCRIPTION_PRICE = 500 ether;

  IBana public Bana;
  mapping(uint256 => ApeInfo) public apeInfo;
  ...
  function setBanaAddress(address bana) public onlyOwner {
    Bana = IBana(bana);
  }
  ...
  {
    require(address(Bana) != address(0), "No token contract set");

    bytes memory n = bytes(name);
    require(n.length > 0 && n.length < 25, "Invalid name length");
    require(
      sha256(n) != sha256(bytes(apeInfo[tokenId].name)),
      "New name is same as current name"
    );

    Bana.burn(msg.sender, UPDATE_NAME_PRICE);
    apeInfo[tokenId].name = name;
    ...
    onlyTokenOwner(tokenId)
  {
    ...
    require(
      sha256(bytes(d)) != sha256(bytes(apeInfo[tokenId].description)),
      "New description is same as current description"
    );

    Bana.burn(msg.sender, UPDATE_DESCRIPTION_PRICE);
    apeInfo[tokenId].description = description;
