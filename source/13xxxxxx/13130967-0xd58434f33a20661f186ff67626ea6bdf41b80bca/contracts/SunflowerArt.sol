...
contract SunflowerArt is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  string public artCodeDependencies; //e.g. p5js@10.0.5; three@2.1.9;
  ...
  // The seed corresponding to a token can only be accesed on block after, and is equal to keccak256(blockhash + tokenID)
...
// Check that code can be set before sealed, and cannot be changed after sealed
// Check that owner can change metadata, and new metadataURI gets returned correctly.
// Check that owner can pause minting as well as unpause minting

// Check that the seed for a token depends on the blockhash of the previous block
// Check that funds are paid out to correct addresses, and in right quantity
// MAke sure token limit is reached correctly and token IDs change
// Make sure that minting can only happen if the right amount is paid
