...
// Bring on the FomoApes!
contract FomoApe is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  string private co1=', ';
  string private rl1='{"name": "FomoApe #';
  string private rl3='"}';
  ...
  function getTraits(Ape memory ape) internal view returns (string memory) {
    string memory o=string(abi.encodePacked(tr1,uint256(ape.bg).toString(), tr8));
    return string(abi.encodePacked(o));
  }
  ...
  function getAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    return string(abi.encodePacked(uint256(ape.bg).toString()));
  }


  function genSVG(Ape memory ape) internal view returns (string memory) {

    string memory output = string(abi.encodePacked(z[0],background[ape.bg],z[1],z[2]));
  ...
  function mintFOMO() public payable {
    require(totalSupply() < 10000, "Purchase would exceed max supply of Apes");
    require(msg.value >= apePrice, "Ether value sent is not correct");
  ...
  function reserveFOMO(address _to, uint256 _reserveAmount) public onlyOwner {
  ...
  constructor() ERC721("FomoApe", "FOMO") Ownable() {}
}
