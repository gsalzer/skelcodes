...
contract SpiritOrbPetsv1Pfp is ERC721, Ownable {
    ...
    constructor() ERC721("Spirit Orb Pets v1 PFP", "SOPV1PFP") {
      _paused = true;
    ...
    function claim(uint256 tokenId) external {
        ...
        require(msg.sender == SOPv1.ownerOf(tokenId), "You don't own this pet");
        require(!_exists(tokenId), "v1 pet claimed already");
    ...
    function claimMultiple(uint256[] memory tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            require(msg.sender == SOPv1.ownerOf(tokenIds[i]), "You don't own this pet");
            require(!_exists(tokenIds[i]), "v1 pet claimed already");
            numberOfPets++;
    ...
    /**
    * @dev Checks to see if a v1 pet claim has been taken yet.  Returns true if it has been taken
    */
    function hasV1PetClaimedWhitelist(uint256 tokenId) public view returns (bool) {
