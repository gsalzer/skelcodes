 ...
 */
contract Lorenz is AttractorSolver3D {
    /**
     ...
     */
    int256 private constant RHO = 28 * 2**96 - 2;
    int256 private constant SIGMA = 10 * 2**96;
    // int256 private constant BETA = (8 * 2**96 + 1) / 3;
    // almost 8/3, funny story...
    int256 private constant BETA = 211275100038038221854659837952;
        ...
        returns (StartingPoint memory startingPoint)
    {
        ...
        (randomSeed, randNumber) = _random(randomSeed, range);
        startingPoint.startingPoint[2] = unperturbed + randNumber;
    }
                ...
                // A lot of code is repeatedly inlined for better efficiency.
                {
                    // Compute intermediate steps and weighted rhs sum
                    {
                        ...
                        // RK4 intermediate step 0
                        {
                            ...
                            // x' = sigma * (y - x)
                            dxdt := sar(PRECISION, mul(SIGMA, sub(y, x)))

                            // y' = x * (rho - z) - y
                            dydt := sub(sar(PRECISION, mul(x, sub(RHO, z))), y)

                            // z' = x * y - beta * z
                            dzdt := sar(PRECISION, sub(mul(x, y), mul(BETA, z)))
