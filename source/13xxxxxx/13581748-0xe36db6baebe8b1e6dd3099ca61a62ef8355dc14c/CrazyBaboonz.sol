...
contract CrazyBaboonz is ERC721, ERC721Enumerable, Ownable {
  
  // Provenance
  string public CRAZY_BABOONZ_PROVENANCE_HASH = "";
  ...
  // Pre-Sale
  uint public preSaleStartDate = 1636606800; // November 11th 05:00 UTC 
  uint public preSaleFinishDate = 1636779600; // November 13th 05:00 UTC 
  uint256 public constant NFT_PRICE_PRESALE = 20000000000000000; //0.02 ETH
  uint public constant MAX_PRESALE_SUPPLY = 6000;
  ...
  // NFT Claims
  uint private MAX_NFT_CLAIMS = 60;
  bool private DEV_NFT_CLAIMED = false;

  constructor(string memory baseURI, bytes32 whitelist) ERC721("CrazyBaboonz", "CBB") {
    setBaseURI(baseURI);
    WHITELIST = whitelist;
  }
  ...
  function mintDev(address _to) public onlyOwner {
    require(!DEV_NFT_CLAIMED, "Already minted");
    for (uint i = 0; i < MAX_NFT_CLAIMS; i++) {
      uint mintIndex = totalSupply() + 1;
      _safeMint(_to, mintIndex);
    }
    DEV_NFT_CLAIMED = true;
  }

  function mintWhitelist(uint256 numNFTs, bytes32[] memory proof) public payable {
    require(block.timestamp < preSaleStartDate, "Whitelist sale closed");
  ...
  function mintPreSale(uint256 numNFTs) public payable {
    require(block.timestamp >= preSaleStartDate && block.timestamp < preSaleFinishDate, "Presale closed");
    require(msg.value >= NFT_PRICE_PRESALE * numNFTs, "Incorrect ether value");
  ...
  function mint(uint256 numNFTs) public payable {
    require(block.timestamp >= preSaleFinishDate, "Sale closed");
  ...
  function setProvenanceHash(string memory provenanceHash) public onlyOwner {
    CRAZY_BABOONZ_PROVENANCE_HASH = provenanceHash;
  }
}
