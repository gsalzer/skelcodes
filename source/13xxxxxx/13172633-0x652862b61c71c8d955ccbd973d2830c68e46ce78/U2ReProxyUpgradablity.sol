...
// File contracts/reproxy/V2Reproxy.sol

pragma solidity ^0.7.6;

abstract contract V2ReproxyAdmin {
...
abstract contract V2Reproxy is V2ReproxyAdmin {
...
// File contracts/U2ReProxyUpgradablity.sol

pragma solidity ^0.7.6;


contract U2ReProxyUpgradablity is Ownable {
    ...
    /// @notice u2Reproxy Instance
    V2Reproxy public u2Reproxy;
    ...
    /// @notice u2ReproxyUnstakeStatus return true or false basis of you active staking.
    mapping(address => mapping(uint256 => bool)) public u2ReproxyUnstakeStatus;
    ...
    function safeTransfer(address tokenAddress) internal {
        uint256 bal = IERC20(tokenAddress).balanceOf(address(u2Reproxy));
        if (bal > 0) u2Reproxy.safeWithdraw(tokenAddress, bal);
    }
    ...
    function setAllU2Addresses(address u2Address, address u2ProxyAddress,address u2ReproxyAddress)
        external
        onlyOwner
        returns (bool)
    {
        u2 = U2Legacy(u2Address);
        u2Proxy = U2Proxy(u2ProxyAddress);
        u2Reproxy = V2Reproxy(u2ReproxyAddress);
        return true;
    }
    ...
    function transferV2ProxyOwnership(address newOwner) external onlyOwner {
        u2Reproxy.transferOwnership(newOwner);
    }
    ...
    function getTotalStaking(address tokenAddress)
        public
        view
        returns (uint256 totalStaking)
    {
        totalStaking = u2.totalStaking(tokenAddress);
    }
    ...
    function unStake(address user, uint256 stakeId) external whenNotPaused {
        ...
        bool isAlreadyUnstakedFromReproxy = u2Reproxy.u2UnstakeStatus(user,stakeId);    
        ...
        // Checks
        if (
            u2ReproxyUnstakeStatus[user][stakeId] == false &&
            activeStatus[stakeId] == true &&
            isAlreadyUnstaked == false &&
            isAlreadyUnstakedFromReproxy == false
        ) u2ReproxyUnstakeStatus[user][stakeId] = true;
    ...
    function emergencyUnstake(
        ...
        bool isAlreadyUnstakedFromReproxy = u2Reproxy.u2UnstakeStatus(userAddress,stakeId);

        if (
            u2ReproxyUnstakeStatus[userAddress][stakeId] == false &&
            activeStatus[stakeId] == true &&
            isAlreadyUnstaked == false && 
            isAlreadyUnstakedFromReproxy == false
        ) u2ReproxyUnstakeStatus[userAddress][stakeId] = true;
