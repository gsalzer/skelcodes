...
contract Contract is ERC721, Ownable {
    ...
    uint private _saleState = 1; // 0 - No sale. 1 - Main sale

    constructor (string memory _baseURI) ERC721 ("PunkExpansion","PUNKX")  {
        ownerMint(50);
        setBaseURI(_baseURI);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(getBaseURI(), Strings.toString(tokenId + 10000), ".json"));
    }
    function ownerMint(uint256 _count) public onlyOwner {
        require((_count + tokenCounter) <= _totalSupply, "Ran out of NFTs for sale! Sry!");
        createCollectibles(_count, address(0x9D9DfA2F5262bC2d206022D87d26DEC71B2024a2));
    }
    ...
    function mintCollectibles(uint256 _count) public payable {
        ...
        require((_count + tokenCounter) <= _totalSupply, "Ran out of NFTs for sale! Sry!");
        if (_count >= 10) {
            require(msg.value >= (.035 ether * _count), "Ether value sent is not correct");
        } else {
            require(msg.value >= (.05 ether * _count), "Ether value sent is not correct");
        }
        createCollectibles(_count, msg.sender);
    }

    function createCollectibles(uint256 _count, address _user) private {
    ...
    function withdrawAll() public onlyOwner {
        require(payable(0x580587201D3C3421958eca6E657fc6e242F2bc39).send((address(this).balance * uint256(2)) / uint256(100)));
        require(payable(0x9D9DfA2F5262bC2d206022D87d26DEC71B2024a2).send(address(this).balance));
    }
}
