//  .----------------.  .----------------.  .----------------.  .----------------.  .-----------------.
// | .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
// | |  _________   | || |     ____     | || |  ___  ____   | || |  _________   | || | ____  _____  | |
// | | |  _   _  |  | || |   .'    `.   | || | |_  ||_  _|  | || | |_   ___  |  | || ||_   \|_   _| | |
// | | |_/ | | \_|  | || |  /  .--.  \  | || |   | |_/ /    | || |   | |_  \_|  | || |  |   \ | |   | |
// | |     | |      | || |  | |    | |  | || |   |  __'.    | || |   |  _|  _   | || |  | |\ \| |   | |
// | |    _| |_     | || |  \  `--'  /  | || |  _| |  \ \_  | || |  _| |___/ |  | || | _| |_\   |_  | |
// | |   |_____|    | || |   `.____.'   | || | |____||____| | || | |_________|  | || ||_____|\____| | |
// | |              | || |              | || |              | || |              | || |              | |
// | '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
//  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'
//  .----------------.  .----------------.  .----------------.
...
contract TMBA is ERC721, Ownable {
    ...
    function setYearbook(uint256 tokenId, string memory text) external {
        require(ERC721.ownerOf(tokenId) == msg.sender, "TMBA: sender is not owner of token");
        _yearbook[tokenId] = text;
        emit YearbookUpdated(msg.sender, tokenId);
    }

    function mint(uint256 count) public payable returns (bool) {
        require(count > 0 && count <= 5, "TMBA: Cannot mint more than 5 tokens");
        require(_tokenIds.current() + count <= MAX_NFTS, "TMBA: Cannot mint more than max supply");
        require(msg.value == PRICE_PER_NFT * count , "TMBA: Ether value sent is not correct");
