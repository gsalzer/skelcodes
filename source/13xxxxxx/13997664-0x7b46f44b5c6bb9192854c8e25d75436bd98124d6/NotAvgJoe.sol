...
contract NotAvgJoe is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Input: Key variables - giveaway
  uint256 public maxFreeMintSupply = 33;                     
  
  // Input: Key variables - whitelist sale
  uint256 public whitelistSalePrice = 0.03 ether;
  uint256 public maxWhitelistSupply = 250;                   
  uint256 public maxWhitelistSaleMintAmount = 20;            
  ...
  function mintWhitelist(uint256 _mintAmount) public payable {
    ...
    require(whitelistSaleIsActive, "!=AVG(JOE) minting not ready yet!");
    ...
    if (msg.sender != owner()) {
      ...
      require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "Exceeds number of tokens allowed per wallet");
      require(msg.value >= whitelistSalePrice * _mintAmount, "Lacks Ether");
    }
  ...
  function mintPublic(uint256 _mintAmount) public payable {
    uint256 supply = totalSupply();
    require(publicSaleIsActive, "!=AVG(JOE) minting is not ready yet!");
    require(_mintAmount > 0, "Let's mint more than one");
    ...
    if (msg.sender != owner()) {
      require(msg.value >= publicSalePrice * _mintAmount);
    }
...
//**************************************  PUBLIC  **************************************//
  ...
  // Giveaway control
  function freeMintStateSet(bool freeMintEnabled) public onlyOwner {
  freeMintIsActive = freeMintEnabled;
  }
  ...
  // Giveaway to those selected
  function safeMintTo(address _to, uint256 _mintAmount) public onlyOwner {
		uint256 supply = totalSupply();
        require(freeMintIsActive, "!=AVG(JOE) minting is not ready yet!");
        require(supply + _mintAmount <= maxFreeMintSupply, "Purchase would exceed maximum supply of whitelist mintable tokens");
