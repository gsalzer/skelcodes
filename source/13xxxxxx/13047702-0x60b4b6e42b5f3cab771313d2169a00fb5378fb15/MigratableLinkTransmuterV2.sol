...
// File contracts/libraries/alchemist/VaultV2WithIndirection.sol
...
/// @title BUSDVaultV2WithIndirection
///
/// @dev A library which provides the Vault data struct and associated functions.
library VaultV2WithIndirection {
  using VaultV2WithIndirection for Data;
  using VaultV2WithIndirection for List;
  using SafeMath for uint256;
...
// File contracts/MigratableLinkTransmuterV2.sol
 ...
 */
contract MigratableLinkTransmuterV2 is Context {
    using SafeMath for uint256;
    using SafeERC20 for IERC20Burnable;
    using Address for address;
    using VaultV2WithIndirection for VaultV2WithIndirection.Data;
    using VaultV2WithIndirection for VaultV2WithIndirection.List;
    ...
    uint256 public plantableThreshold = 8000000000000000000000; // 500k
    ...
    /// deposits and withdraws. VaultV2WithIndirection before the last element are considered inactive and are expected to be cleared.
    VaultV2WithIndirection.List private _vaults;
    ...
    constructor(address _waToken, address _token, address _governance) public {
        require(_governance != ZERO_ADDRESS, "MigratableLinkTransmuterV2: 0 gov");
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "MigratableLinkTransmuterV2: !whitelisted");
        _;
    }
    ...
    ///
    modifier onlyGov() {
        require(msg.sender == governance, "MigratableLinkTransmuterV2: !governance");
        _;
    }
    ...
    function claim() public {
        ...
        require(realisedTokens[sender] > 0, "MigratableLinkTransmuterV2: no realisedToken balance for sender");
    ...
    function unstake(uint256 amount) public updateAccount(msg.sender) {
        ...
        address sender = msg.sender;
        require(depositedWaTokens[sender] >= amount, "MigratableLinkTransmuterV2: unstake amount exceeds deposited amount");
    ...
    function stake(uint256 amount)
        ...
        checkIfNewUser()
    {
        require(!pause, "MigratableLinkTransmuterV2: emergency pause enabled");
    ...
    function transmute() public runPhasedDistribution() updateAccount(msg.sender) {
        ...
        require(pendingz > 0, "MigratableLinkTransmuterV2: need to have pending in bucket");
        ...
        checkIfNewUser()
    {
        ...
        require(
            ...
            "MigratableLinkTransmuterV2: !overflow"
        );
    ...
    function setPendingGovernance(address _pendingGovernance) external onlyGov() {
        require(_pendingGovernance != ZERO_ADDRESS, "MigratableLinkTransmuterV2: 0 gov");
    ...
    function acceptGovernance() external  {
        require(msg.sender == pendingGovernance, "MigratableLinkTransmuterV2: !pendingGovernance");
    ...
    function setActiveVault(IVaultAdapterV2WithIndirection _adapter) external onlyGov() {
        require(_adapter != IVaultAdapterV2WithIndirection(ZERO_ADDRESS), "MigratableLinkTransmuterV2: active vault address cannot be 0x0.");
        require(address(_adapter.token()) == token, "MigratableLinkTransmuterV2.vault: token mismatch.");

        _vaults.push(VaultV2WithIndirection.Data({
            adapter: _adapter,
            totalDeposited: 0
        }));
    ...
    function recallAllFundsFromVault(uint256 _vaultId) external {
        require(pause && (msg.sender == governance || msg.sender == sentinel), "MigratableLinkTransmuterV2: not paused, or not governance or sentinel");
    ...
    function _plantOrRecallExcessFunds() internal {
        ...
        if (bal > plantableThreshold.add(marginVal)) {
            ...
            VaultV2WithIndirection.Data storage _activeVault = _vaults.last();
    ...
    function setSentinel(address _sentinel) external onlyGov() {
        require(_sentinel != ZERO_ADDRESS, "MigratableLinkTransmuterV2: sentinel address cannot be 0x0.");
        sentinel = _sentinel;
    ...
    function setPause(bool _pause) external {
        require(msg.sender == governance || msg.sender == sentinel, "MigratableLinkTransmuterV2: !(gov || sentinel)");
        pause = _pause;
    ...
    function setRewards(address _rewards) external onlyGov() {
        ...
        require(_rewards != ZERO_ADDRESS, "MigratableLinkTransmuterV2: rewards address cannot be 0x0.");
    ...
    function migrateFunds(address migrateTo) external onlyGov() {
        require(migrateTo != address(0), "MigratableLinkTransmuterV2: cannot migrate to 0x0");
        require(pause, "MigratableLinkTransmuterV2: set pause first");
