...
abstract contract AccessControlUpgradeable is Initializable, ContextUpgradeable {
    ...
    /// @dev Return all members of a role
    /// @dev No ordering guarantees, as per underlying EnumerableSet sturcture
...
contract WrappedIbbtcEth is Initializable, ERC20Upgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    /// Initialize access control after creation. Function is intended to be called once and then upgraded away in a future iteration.
    function initializeAccessControl() external {
        ...
        // vault is admin, can grant and revoke other roles
        _setupRole(DEFAULT_ADMIN_ROLE, 0xB65cef03b9B89f99517643226d76e286ee999e77);
        _setupRole(DEFAULT_ADMIN_ROLE, 0xDA25ee226E534d868f0Dd8a459536b03fEE9079b);

        // vault and techOps can approve account access
        _setupRole(ALLOWLIST_MANAGER_ROLE, 0xB65cef03b9B89f99517643226d76e286ee999e77);
        _setupRole(ALLOWLIST_MANAGER_ROLE, 0x86cbD0ce0c087b482782c181dA8d191De18C8275);
        _setupRole(ALLOWLIST_MANAGER_ROLE, 0xDA25ee226E534d868f0Dd8a459536b03fEE9079b);
        
        // vault, techOps, and guardian can pause
        _setupRole(PAUSER_ROLE, 0xB65cef03b9B89f99517643226d76e286ee999e77);
        _setupRole(PAUSER_ROLE, 0x86cbD0ce0c087b482782c181dA8d191De18C8275);
        _setupRole(PAUSER_ROLE, 0x6615e67b8B6b6375D38A0A3f937cd8c1a1e96386);

        // vault and techOps can unpause
        _setupRole(UNPAUSER_ROLE, 0xB65cef03b9B89f99517643226d76e286ee999e77);
        _setupRole(UNPAUSER_ROLE, 0x86cbD0ce0c087b482782c181dA8d191De18C8275);

        // allowlist_managers (vault, techOps) can use normal grantRole() to add approved accounts
        _setRoleAdmin(APPROVED_ACCOUNT_ROLE, ALLOWLIST_MANAGER_ROLE);
    }
    ...
    function mint(uint256 _shares) external whenNotPaused onlyApprovedAccount {
        if (_shares == 0) {
            return;
        }
    ...
    function burn(uint256 _shares) external whenNotPaused onlyApprovedAccount {
        if (_shares == 0) {
            return;
        }
     ...
     * `amount`.
     */
    function transferFrom(address sender, address recipient, uint256 amount) public override whenNotPaused onlyApprovedAccount returns (bool) {
     ...
     * - the caller must have a balance of at least `amount`.
     */
    function transfer(address recipient, uint256 amount) public override whenNotPaused onlyApprovedAccount returns (bool) {
        /// The _balances mapping represents the underlying ibBTC shares ("non-rebased balances")
        /// Some naming confusion emerges due to maintaining original ERC20 var names
    ...
    /// @dev Static ibBTC price from core
