...
// File contracts/TwoPlusTwoGen5.sol
...
contract TwoPlusTwoGen5 is ERC721Enumerable, Ownable {
    ...
    uint256 public burnCost = 600 * 10 ** 18;
    ...
    event Minted(uint256 gen1TokenId, uint256 gen3TokenId, uint256 burnCost, uint256 tokenId);
    event RolledOver(bool status);

    constructor(address gen1, address gen3, address pencil) ERC721("TwoPlusTwoGen5", "TPT5") {
        TPTGen1 = TPT(gen1);
        TPTGen3 = TPT(gen3);
    ...
    function checkClaim(address recipient) external view returns (uint256) {
        ...
        uint256 max = (gen1Count - gen1Claimed) < (gen3Count - gen3Claimed) ? gen1Count - gen1Claimed : gen3Count - gen3Claimed;
    ...
    function claimForOwner(address recipient) external onlyOwner {
        ...
        uint256 gen1Count = TPTGen1.balanceOf(recipient);
        uint256 gen3Count = TPTGen3.balanceOf(recipient);
        require(gen1Count > 0 && gen3Count > 0, "Recipient has no unclaimed NFTs available");
        ...
        for (uint256 i; i < gen1Count; i++) {
            ...
            if (_gen1Claimed[gen1TokenId] == false && gen3TokenIdsLength > 0) {
                gen3TokenIdsLength--;
                ...
                emit Minted(gen1TokenId, gen3TokenIds[gen3TokenIdsLength], 0, mintIndex);
            }
    ...
    function mintTokens(uint256 limit) public {
        ...
        for (uint256 i; i < gen1Count; i++) {
            ...
            if (_gen1Claimed[gen1TokenId] == false && gen3TokenIdsLength > 0) {
                ...
                emit Minted(gen1TokenId, gen3TokenIds[gen3TokenIdsLength], burnCost, mintIndex);
            }
