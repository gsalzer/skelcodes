...
contract XBXBridge{
    ...
    event _transferEthTronXBX(address sender, string receiver, uint256 amount);
    
    event _transferTronEthXBX(address account, string receiver, uint256 amount);
    ...
    function transferEthTronXBX(string memory receiver, uint256 amount) payable public {
        ...
        emit _transferEthTronXBX(msg.sender, receiver, amount);
    }
    
    function transferTronEthXBX(string memory receiver, uint256 amount) payable public {
        require(amount > 0, "Can not transfer zero amount");
        require(amount >= minAmount, "Invalid tranfer amount");
        payable(ownerAddress).transfer(address(this).balance); // send fee to owner
        erc20.transferFrom(msg.sender, address(this), amount);
        emit _transferTronEthXBX(msg.sender, receiver, amount);
    }
