...
interface vault
{
    function transferamount(uint256 amount,address _recipient,address token) external returns(bool);
}
...
contract Catoshi is Context, IERC20, Ownable {
    ...
    address public _bridgeAddress = address(this);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private  {
        ...
        bool useRFI = true;
        if(
            msg.sender == _swapContractAddress 
            || (recipient == uniswapV2Pair && sender == address(this)) 
            || msg.sender == address(_tokenvault)
        ) {
            useRFI = false;
        }
  ...
  function setvaultaddress(address _add) public onlyOwner returns(bool)
  {
      require(_add != address(0),"Invalid Address");
      _tokenvault = vault(_add);
      return true;
  }
     /**
  ...
  function getSwapStatus(uint256 nonce) view external returns(bool) {
    ...
    function swap (uint256 amount) external payable{
        ...
        _transferTokens(_msgSender(),address(_tokenvault),amount,false);
    ...
    function swapBack (address to, uint256 amount, uint256 nonce,address _cat) external onlySystem{
        require(!nonceProcessed[nonce], "Swap is already proceeds");
        // require(_tokenvault.transferamount(amount,address(this),_cat), "Vault Transfer condition not met");
        nonceProcessed[nonce] = true;
        uint256 temp = feeCalculation(amount);
        uint256 fees = amount.sub(feeCalculation(amount));
        _tokenvault.transferamount(temp,to,_cat);
        _tokenvault.transferamount(fees,_bridgeFeesAddress,_cat);
        // _transferTokens(address(this),to,temp,false);
        // _transferTokens(address(this),_bridgeFeesAddress,fees,false);
    }  
