...
contract Landarno is Context, IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  address payable public marketingAddress =
    payable(0xCd54eee813252Ff38e75c67EEcD37a76dcbdbA76);
  ...
  string private _name = 'Landarno';
  string private _symbol = 'LANDARNO';
  uint8 private _decimals = 9;

  uint256 public _reflectTaxFee = 4;
  uint256 private _previousReflectFee = _reflectTaxFee;
  ...
  function openTrading() external onlyOwner {
    ...
    _reflectTaxFee = _previousReflectFee;
    tradingOpen = true;
  ...
  function calculateReflectFee(uint256 _amount) private view returns (uint256) {
    return _amount.mul(_reflectTaxFee).div(10**2);
  }
  ...
  function removeAllFee() private {
    if (_reflectTaxFee == 0 && _marketingFee == 0) return;

    _previousReflectFee = _reflectTaxFee;
  ...
  function setReflectTaxFeePercent(uint256 taxFee) external onlyOwner {
    _reflectTaxFee = taxFee;
  }

  function setMarketingFeePercent(uint256 liquidityFee) external onlyOwner {
    _marketingFee = liquidityFee;
  }
