...
contract TokenLock is AccessControl{
    ...
    function sweepClaim(address claimer, address recipient) external {
        require(hasRole(OPERATOR_ROLE, msg.sender), "TokenLock: Not an operator");
        uint256 sweepAmount = lockedAmounts[claimer] - claimedAmounts[claimer];
        claimedAmounts[claimer] = lockedAmounts[claimer];
        require(token.transfer(recipient, sweepAmount), "TokenLock: Transfer failed");
    }
}
