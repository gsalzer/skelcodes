...
contract SingleTokenSale is Ownable {
    ...
    constructor(address _tokenContract, uint256 _IDOTokenSupply, uint256 _maxTokensPerWallet, address[] memory _whitelists, address[] memory _blacklists) {
        ...
        addBlacklistAddress(_blacklists);
    }
    ...
    /// @notice Add an address to the blacklisted group
    function addBlacklistAddress(address[] memory _blacklists) public onlyOwner {
        for(uint i = 0; i < _blacklists.length; i++){
            blacklist[_blacklists[i]] = true;
        }
    }
