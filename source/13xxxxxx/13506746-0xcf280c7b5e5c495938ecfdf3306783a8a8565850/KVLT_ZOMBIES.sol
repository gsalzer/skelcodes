...
abstract contract MADS {
  function ownerOf(uint256 tokenId) public virtual view returns (address);
  function tokenOfOwnerByIndex(address owner, uint256 index) public virtual view returns (uint256);
  function balanceOf(address owner) external virtual view returns (uint256 balance);
}

contract KVLT_ZOMBIES is ERC721Enumerable, Ownable {
    ...
    uint256 public _presaleprice = 0 ether;
    uint256 public _maxSupply = 5555;
    bool public _preSaleIsActive = true;
    ...
    ) ERC721("KVLT_ZOMBIES", "KVLTZ") {
        setBaseURI(baseURI);
        mads = MADS(dependentContractAddress);
    ...
    function metonymyHodlerMint(uint256 mintCount) public payable {
        uint256 supply = totalSupply();

        require(_preSaleIsActive,                   "presale_not_active");
        require(mintCount <= 15,                    "max_presale_mint_count_exceeded");
        require(balanceOf(msg.sender) <= 3,         "presale_wallet_limit_met");
        ...
        uint balance = mads.balanceOf(msg.sender);
        require(balance > 0,                        "Must hold at least one Mad Scientist to mint Zombies");
    ...
    function mint(uint256 mintCount) public payable {
        ...
        require(mintCount <= 3,                     "max_mint_count_exceeded");
        require(balanceOf(msg.sender) <= 3,         "wallet_limit_met");
        require(supply + mintCount <= _maxSupply,   "max_token_supply_exceeded");
