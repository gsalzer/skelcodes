...
contract nft is ERC721Enumerable, Ownable {
    ...
    //mint the initial tokens that were minted already in the original collection
    //token 1 has already been minted and corresponds to the owner
    function initialMint(address[] memory listOfMinters) public onlyOwner {
        require(totalSupply() + listOfMinters.length <= 44, 'Initial Mint already Executed!'); //44 was the supply in the other collection
        for (uint i = 0; i < listOfMinters.length; i++) {
            uint tid = tokenId();
            _safeMint(listOfMinters[i], tid);
            mintsPerAddress[listOfMinters[i]] += 1;
        }
    }
