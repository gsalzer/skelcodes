...
contract MiningNFT is Poster,IMiningNFT,ERC1155{

    uint256 constant initialTokenId = 100000;
    uint256 public newTokenId = initialTokenId;

    constructor() ERC1155(""){

    }
    
    function getInitialTokenId() public pure override returns(uint256){
    ...
    function mint(address account, uint256 amount) external override onlyPoster returns(uint256){
        newTokenId += 1;
        _mint(account, newTokenId, amount, "");
        return newTokenId;
    }

    function burn(address account,uint256 tokenId, uint256 amount) external override onlyPoster {
