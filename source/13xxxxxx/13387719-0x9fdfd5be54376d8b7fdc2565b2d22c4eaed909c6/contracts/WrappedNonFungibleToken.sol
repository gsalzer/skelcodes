...
contract WrappedNonFungibleToken is ERC721, Ownable, ERC1155TokenReceiver {
    bytes32 internal _tokenIdArrayMerkleRoot;

    uint256 internal nextTokenId = 1;
    bool internal isContractFinalized = false;
    ...
    constructor(
        ...
        address wrappableContract,
        bytes32 tokenIdArrayMerkleRoot
    ) ERC721(name, symbol) {
        _tokenIdArrayMerkleRoot = tokenIdArrayMerkleRoot;
        _wrappableContract = wrappableContract;
    }

    function updateRoot(bytes32 tokenIdArrayMerkleRoot_) external onlyOwner {
        if(!isContractFinalized) {
            _tokenIdArrayMerkleRoot = tokenIdArrayMerkleRoot_;
        }
    }

    function setContractFinalized() external onlyOwner {
     ...
     * @dev Wraps NFTs
     */
    function wrapWithProof(uint256 legacyTokenId, bytes32[] memory merkleProof)
        public
        returns (bool)
    {
        ...
        require(
            MerkleProof.verify(
                merkleProof,
                _tokenIdArrayMerkleRoot,
                keccak256(abi.encode(legacyTokenId))
            ),
            "proof failure"
        );

        IERC1155(_wrappableContract).safeTransferFrom(
            msg.sender,
            address(this),
            legacyTokenId,
            1,
     ...
     * @dev Wraps NFTs
     */
    function wrapManyWithProof(
        uint256[] memory legacyTokenIds,
        bytes32[][] memory merkleProofs
    ) public returns (bool) {
        //require that the tokenId is in the tokenIdArray that was merkle hashed
        uint256 arrayLength = merkleProofs.length;
        for (uint256 i = 0; i < arrayLength; i++) {
            require(
                MerkleProof.verify(
                    merkleProofs[i],
                    _tokenIdArrayMerkleRoot,
                    keccak256(abi.encode(legacyTokenIds[i]))
                ),
                "proof failure"
            );
        }
        ...
        for (uint256 i = 0; i < arrayLength; i++) {
            uint256 legacyTokenId = legacyTokenIds[i];
            IERC1155(_wrappableContract).safeTransferFrom(
                msg.sender,
                address(this),
                legacyTokenIds[i],
    ...
    function unwrap(uint256 tokenId) public returns (bool) {
        ...
        IERC1155(_wrappableContract).safeTransferFrom(
            address(this),
            msg.sender,
            legacyTokenIdReverseRegister[tokenId],
            1,
            ""
        );
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    baseURI(),
                    uint2str(legacyTokenIdReverseRegister[tokenId]),
                    "?format=json"
                )
            );
    }
