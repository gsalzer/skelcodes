...
contract XanaPriceFeed is Ownable {
    /// @notice keeps track of oracles for each tokens
    mapping(address => address) public oracles;

    /// @notice xana address registry contract
    ...
    function registerOracle(address _token, address _oracle)
        ...
        onlyOwner
    {
        IXanaTokenRegistry tokenRegistry = IXanaTokenRegistry(
            IXanaAddressRegistry(addressRegistry).tokenRegistry()
        );
