...
contract reachRarity is ERC721, Ownable {
    uint public next_summoner;
    uint constant xp_per_day = 250e18;
    uint constant DAY = 1 days;
    uint SUMMON_PRICE = 30000000000000000;
    uint LEVEL_UP_PRICE = 8880000000000000;
    
    string constant public name = "Rich Rarity Manifested";
    string constant public symbol = "RRM";
    ...
    function level_up(uint _summoner) external payable {
        require(_isApprovedOrOwner(msg.sender, _summoner));
        require(msg.value == (level[_summoner] * LEVEL_UP_PRICE), "Payment too low, try 0.00888 ETH");
    ...
    function summon(uint _class) external payable {
        require(1 <= _class && _class <= 11);
        require(msg.value == SUMMON_PRICE, "Payment too low, try 0.03 ETH");
