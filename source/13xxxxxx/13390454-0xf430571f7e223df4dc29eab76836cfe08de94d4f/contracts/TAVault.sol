...
interface IStrategy {
    ...
    function switchMode(uint[] calldata tokenPrice) external;
...
contract TAVault is Initializable, ERC20Upgradeable, OwnableUpgradeable, 
    ...
    event Reimburse(address token, uint amount);
    ...
    function switchMode(uint[] calldata tokenPrice) external onlyOwnerOrAdmin whenNotPaused {
        strategy.switchMode(tokenPrice);
    }
    ...
    function swapTokenToWETH(uint USDTAmt, uint USDCAmt, uint DAIAmt) private returns (uint WETHAmt, uint tokenAmtToInvest, uint pool) {
        ...
        if (DAIAmt > DAIAmtKeepInVault + 1e18) {
            ...
            WETHAmt += sushiSwap(address(DAI), address(WETH), DAIAmt);
            tokenAmtToInvest += DAIAmt;
        }
    }
