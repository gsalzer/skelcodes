...
abstract contract Ownable is Context {
  ...
  function getOwnershipUnlockTime() public view returns(uint256) {
    return _lockTime;
  }

  /**
   * @dev Locks all `onlyOwner` functions in the contract until (`time`) has passed.
   */
  ...
  /**
   * @dev Unlocks the `onlyOwner` functions. Can only be called by owner after lock time has passed.
   */
  function unlockOwnership() public {
    require(_previousOwner == msg.sender, "Ownable: you do not have permission to unlock");
    require(block.timestamp > _lockTime, "Ownable: can't unlock contract yet");
