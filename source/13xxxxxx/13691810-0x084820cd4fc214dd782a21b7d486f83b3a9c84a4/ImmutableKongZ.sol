...
contract ImmutableKongZ is ERC721, Ownable, IMintable {
    address public imx;
    mapping(uint256 => bytes) public blueprints;

    uint256[] private _allBlueprints;
    uint256 public immutable maxTokenCount = 6666;

    event AssetMinted(address to, uint256 id, bytes blueprint);

    constructor(address _imx) ERC721("ImmutableKongZ", "ImmKongZ") {
        imx = _imx;
    }
    ...
    function mintFor(
        ...
        require(tokenCount() < maxTokenCount, "Cannot mint more tokens. Max supply reached");
