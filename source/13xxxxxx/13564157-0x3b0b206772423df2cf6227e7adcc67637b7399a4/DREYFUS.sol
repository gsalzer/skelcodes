...
contract DREYFUS is Context, IERC20, Ownable {
    ...
    string private _name = "Dreyfus Inu";
    string private _symbol = "DREYFUS";
    ...
    address public marketing = 0xbc3633F8230E58c287E7F35493E552ACDd384a07;
    ...
    uint private coolDownGap = 86400;
    uint256 public _maxSellAmount = 1000000000000 * 10**18;

    struct cool {
        uint time;
        uint256 sold;
    }
    mapping (address => cool) private cooldown;

    uint256 public _maxTxAmount = 1000000000000 * 10**18;

    uint256 public _maxBalAmount = 7500000000000 *10**18;
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && cooldownEnabled && !_isExcludedBal[from]) {
            if(cooldown[from].time < block.timestamp && cooldown[from].sold == _maxSellAmount) {
                cooldown[from].sold = 0;
            }
            require(cooldown[from].sold.add(amount) <= _maxSellAmount, "max sell limit");
            require(cooldown[from].time < block.timestamp, "sell gap to avoid bots");

            if(cooldown[from].sold.add(amount) == _maxSellAmount) {
                cooldown[from].time = block.timestamp.add(coolDownGap);
            }
            cooldown[from].sold += amount;
        }
