...
contract MetaCrocs is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x85901A4f6F2B53eA76dB309a921711A737175d1C);
    address payable private _DevAddress;

    uint256 public METACROC_MAX = 11111;
    ...
    uint256 private PRESALE_HOURS = 12 hours;
    uint256 public PURCHASE_LIMIT = 11;
    
    mapping(address => bool) _mappingWhiteList;
    mapping(uint256 => uint256) _mappingMintTimestamp;

    uint256 private _activeDateTime = 1639328400; // Date and time (GMT): Sunday, December 12, 2021 5:00:00 PM
    ...
    Counters.Counter private _publicMETACROC;

    constructor() ERC721("Meta Crocs", "METACROC") {
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicMETACROC.current();

            if (_publicMETACROC.current() < METACROC_MAX) {
                _publicMETACROC.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicMETACROC.current() < METACROC_MAX,"Purchase would exceed METACROC_MAX");
