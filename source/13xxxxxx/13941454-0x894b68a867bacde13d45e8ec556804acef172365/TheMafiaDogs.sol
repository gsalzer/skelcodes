    ...
    PaymentSplitter
{
    ...
    uint256 public maxSaleSupply = 7900;
    uint256 public maxPresaleSupply = 2000;
    uint256 public maxGiftSupply = 100;
    uint256 public preSalePrice = 40000000000000000;
    ...
    uint256[] private _teamShares = [2500, 2500, 2500, 2500];
    address[] private _team = [
        0xf80387CAC4f5eE9A7F3f72fC5BBbC95CA958b0C3,
        0xDf2f22Fce70F04354Ef450a76803Ae0dDA1a211E,
        0x646Fe0076543ae64cb0DbfeD2CD9Cacf46A39748,
        0xE41e95a4b3D1f0173656CbcFBEaeFA4596195385
    ];
    ...
    event PresaleMint(address indexed _minter, uint256 _amount, uint256 preSalePrice);
    event SaleMint(address indexed _minter, uint256 _amount, uint256 publicPrice);
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "The Mafia Dogs: Can't add a zero address"
            );
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "The Mafia Dogs: Can't remove a zero address"
            );
    ...
    function setUpPresale(uint256 _duration) external onlyOwner {
        require(
            ...
            "The Mafia Dogs: Unauthorized Transaction"
        );
    ...
    function setUpSale() external onlyOwner {
        ...
        require(
            ...
            "The Mafia Dogs: Sale not started"
        );
        uint256 _startTime = block.timestamp;
        uint256 _maxCount = 3;
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "The Mafia Dogs: max total supply exceeded"
        );

        require(
            giftCount + _addresses.length <= maxGiftSupply,
            "The Mafia Dogs: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "The Mafia Dogs: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) external payable nonReentrant {
        ...
        require(
            ...
            "The Mafia Dogs: You cant mint so much tokens"
        );
        require(
        ...
        require(
            ...
            "The Mafia Dogs: Presale must be active to mint Mafia Dogs NFTs."
        );
        require(
            block.timestamp >= _presaleConfig.startTime,
            "The Mafia Dogs: Presale not started"
        );
        require(
            block.timestamp <=
                _presaleConfig.startTime + _presaleConfig.duration,
            "The Mafia Dogs: Presale is ended"
        );
        require(
            totalNFT + _amount <= maxPresaleSupply,
            "The Mafia Dogs: max presale supply exceeded"
        );
        require(
            totalNFT + _amount <= maxSupply,
            "The Mafia Dogs: max supply exceeded"
        );
        require(
            preSalePrice <= msg.value,
            "The Mafia Dogs: Ether value sent is not correct"
        );
        require(!paused, "The Mafia Dogs: contract is paused");
        uint256 _newItemId;
    ...
    function saleMint(uint256 _amount) external payable nonReentrant {
        ...
        require(_amount > 0, "The Mafia Dogs: zero amount");
        require(
            _saleConfig.startTime > 0,
            "The Mafia Dogs: sale is not active"
        );
        require(
        ...
        require(
            ...
            "The Mafia Dogs:  Can only mint 10 tokens at a time"
        );
        require(
            totalNFT + _amount <= maxSupply,
            "The Mafia Dogs: max supply exceeded"
    ...
    function changesalePrice(uint256 _newSalePrice) public onlyOwner {
        publicPrice = _newSalePrice;
    }

    function changePresaleAmountLimit(uint256 _newPresaleAmountLimit) public onlyOwner {
        presaleAmountLimit = _newPresaleAmountLimit;
    }

    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "The Mafia Dogs: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "The Mafia Dogs: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
