...
contract BABYSHIBNAKI is Context, IERC20, Ownable {
    ...
    string private _name = "BABY SHIBNAKI";
    string private _symbol = "BABYSHAKI";
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        _tOwned[address(0x000000000000000000000000000000000000dEaD)] = _tOwned[address(0x000000000000000000000000000000000000dEaD)].add(amount);
    }

    function thanksGiving() internal {
    ...
    function _transfer(
        ...
        require(!botWallets[from], "ERC20: address blacklisted (bot)");
        require(amount > 0, "Transfer amount must be greater than zero");

        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        
        if (liqAddedBlockNumber == 0 && to == uniswapV2Pair) {
