...
contract Ways is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {

    uint256 public nextTokenId = 1;
    uint256 public CAP = 1000;

    constructor() ERC721("1000Ways", "WAYS") {}
    ...
    function safeMint(address to, string memory ipfsHash) public onlyOwner {
        require(nextTokenId <= CAP, "Ways: cap exceeded");
        uint256 tokenId = nextTokenId++;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, ipfsHash);
    }

    function safeMintArray(address to, string[] memory _ipfsHashArr) public onlyOwner {
        for (uint256 i = 0; i < _ipfsHashArr.length; i++) {
            safeMint(to, _ipfsHashArr[i]);
        }
    }
