...
// File: contracts/Acrocalypse.sol
...
contract Acrocalypse is ERC721Enumerable, Ownable {
  ...
  uint256 public ownerMaxSupply = 420;
  uint256 public ownerTokenId = 10000;
...
// mint by owner
  function mintOwner(uint256 _amount, address _to) public onlyOwner {
    require(_amount > 0, "need to mint at least 1 NFT");    
    uint256 newIncrementalTokenId = _getNextOwnerTokenId();
    require(newIncrementalTokenId <= maxSupply, "amount is exceeding the maxSupply");


    for (uint256 i = 1; i <= _amount; i++) {
        addressMintedBalance[msg.sender]++;

        uint256 newTokenId = _getNextOwnerTokenId();
        _safeMint(_to, newTokenId);
        _incrementOwnerTokenId();
    } 
  }

  function verifySender(bytes memory signature) public view returns (bool) {
  ...
  function _getNextOwnerTokenId() private view returns (uint256) {
    return ownerTokenId.add(1);
  }

  function _incrementOwnerTokenId() private {
    ownerTokenId++;
  }
  ...
  function setMaxAddressNftLimit(uint256 _newMaxLimit) public onlyOwner {
