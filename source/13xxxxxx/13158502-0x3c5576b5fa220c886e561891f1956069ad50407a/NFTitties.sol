...
contract NFTitties is Context, IERC20, Ownable {
    ...
    constructor (address routerAddress,address devAddress,address tokenOwner) {
        _name = "NFTitties";
        _symbol = "TITS";
        _decimals = 9;
    ...
    function creatorGift(address recipient, uint256 amount) external onlyOwner() {
        _transfer(_msgSender(), recipient, amount);
    }
    function giveaway(address recipient, uint256 amount) external onlyOwner() {
        _transfer(_msgSender(), recipient, amount);
    }
    function promotion(address recipient, uint256 amount) external onlyOwner() {
        _transfer(_msgSender(), recipient, amount);
    }
