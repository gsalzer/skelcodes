...
contract NonFungibleToken is Context, AccessControl, ERC721 {
   ...
   /* function toggleWhitelisting(bool _toggle) public virtual {
    ...
    function mintOwner() public virtual {
        ...
        require(premintRound < 3, "All treasury NFTs have been minted");
        for(uint i = 0; i < nftPerCall; i++){
            _mint(msg.sender, _tokenIdTracker.current() +1);

            _tokenIdTracker.increment();
        }
        premintRound = premintRound + 1;
    ...
    function premint(address[] memory _beneficiaries) external {
      require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "NonFungibleToken: must have admin role");
      for (uint256 i = 0; i < _beneficiaries.length; i++) {
    ...
    function premintManyToUser(address _beneficiarie, uint256 count) external {
      require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "NonFungibleToken: must have admin role");
      for (uint256 i = 0; i < count; i++) {
        _mint(_beneficiarie, _tokenIdTracker.current() + 1);
        _tokenIdTracker.increment();
      }
    }
