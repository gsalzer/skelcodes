...
contract Yamchainu is Context, IERC20, Ownable {
    ...
    string private _name = "Yamcha Inu";
    string private _symbol = "YAMCHA";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 0;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 9; //(2% liquidityAddition + 5% rewardsDistribution + 2% marketing)
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 forRewards = contractTokenBalance.div(5);
        // split the liquidity
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
         require(taxFee <= 16, "Maximum fee limit is 16 percent");
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 16, "Maximum fee limit is 16 percent");
        _liquidityFee = liquidityFee;
    }
   
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
         require(maxTxPercent <= 50, "Maximum tax limit is 16 percent");
