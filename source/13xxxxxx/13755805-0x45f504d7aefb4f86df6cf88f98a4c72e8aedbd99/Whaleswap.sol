...
contract Whaleswap is IPancakeRouter02, Ownable {
    using SafeMath for uint;

    address public immutable override factory;
    address public immutable override WETH;

    uint256 public whaleSwapFee = 6;
    ...
    function setWhaleSwapFee(uint256 _newFee) external onlyOwner() {
        whaleSwapFee = _newFee;
    }
    ...
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(msg.value >= amountOutMin.mul(whaleSwapFee).div(100), 'Insufficient Whaleswap Payment');
        require(path[path.length - 1] == WETH, 'PancakeRouter: INVALID_PATH');
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
