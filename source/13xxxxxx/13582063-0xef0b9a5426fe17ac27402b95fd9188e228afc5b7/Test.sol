...
contract Test is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    uint256[] private _teamShares = [50,25,25];
    address[] private _team = [
        0x1149900685C618eB37Ea285b06BBbe73B9CA2c7c,
	0xe9945D97EE9253020b7EFFb652dA4e1B2D31fc49,
	0x6e6f19f14F63Ce6991aDFC9540c635d9290B329f
    ];
    ...
    function getPrice() public view returns (uint256) {
        ...
        if (block.timestamp <= _presaleConfig.startTime + 2 hours) {
            _price = 10000000000000000; //0.10 ETH TEMP 0,01 24 hour
        } else if ((block.timestamp > _presaleConfig.startTime + 2 hours) && (block.timestamp <= _presaleConfig.startTime + 4 hours)) {
            _price = 20000000000000000; //0.20 ETH TEMP 0,02 24 hour 48 hour
        } else if (block.timestamp <= _saleConfig.startTime + 6 hours) {
            _price = 30000000000000000; //0.3 ETH TEMP 0,03
        } else {
            _price = 25000000000000000; //0.25 ETH TEMP 0,025
        }
        return _price;
    }
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            block.timestamp <=
                _presaleConfig.startTime + 4 hours,
