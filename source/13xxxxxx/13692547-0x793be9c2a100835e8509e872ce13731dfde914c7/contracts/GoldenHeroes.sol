...
contract GoldenHeroes is ERC721("GOLDEN HEROES", "GH"), ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    string private baseURI;
    uint256 public nftsNumber = 999;
    uint256 public nftsPresale = 100;
    uint256 public maxTokensPresale = 2;
    uint256 public maxTokensPerTransaction = 10;
    uint public presalePrice = 0.1 ether;
    uint public initialPrice = 0.3 ether;
    uint public finalPrice = 0.2 ether;
    uint public priceDrop= 0.05 ether;
    uint public priceDropTime = 360 minutes;
    uint public startTime = 1638154800;
    uint public whiteListId = 0;
    uint private devSup = 4.5 ether;
    address private devAddress = 0x0AF3f0461f2bEE2F18f405d0a4463A0cC131723D;
    bool public isActive;
    bool public isPresaleActive;
    bytes32 public root;

    mapping(uint => mapping(address => uint)) private presaleClaimed;
    ...
    */
    ...
    function setNewDrop(
        ...
        uint256 _finalPrice,
        uint256 _priceDrop,
        uint256 _priceDropTimeInSeconds,
        uint256 _startTime,
        uint _whiteListId
    )
        public
    ...
    {
        nftsNumber = _supply;
        nftsPresale = _presaleSupply;
        maxTokensPresale = _maxTokensPresale;
        maxTokensPerTransaction = _maxTokensPerTransaction;
        presalePrice = _presalePrice;
        initialPrice = _initialPrice;
        finalPrice = _finalPrice;
        priceDrop= _priceDrop;
        priceDropTime = _priceDropTimeInSeconds;
	 ...
	 */
	function getPriceOnTime(uint _time) public view returns(uint) {
		if(_time < startTime) {
			return 0;
		}
		uint maxRange = (initialPrice - finalPrice) / priceDrop;
		uint currentRange = (_time - startTime) / priceDropTime;

		if(currentRange >= maxRange) {
			return finalPrice;
		}

		return initialPrice - (currentRange * priceDrop);
	}

    /*
     * Function to withdraw collected amount
    */
    function withdraw(
        address _to
    )
     ...
     * Amount is calculated as per (tokenPrice.mul(_numOfTokens))
    */
    function mintNFT(
        uint256 _amount
    )
        ...
        payable
    {
        ...
        require(_amount <= maxTokensPerTransaction, 'Cannot mint above limit');
        require(totalSupply().add(_amount) <= nftsNumber, "Purchase would exceed max supply of NFTs");

        uint currentPrice = getCurrentPrice() * _amount;
		require(msg.value >= currentPrice, "Ether value sent is not correct");
    ...
    */
    function presaleMint (
      ...
      {
          require(isActive, 'Contract is not active');
          require(block.timestamp <= startTime, 'Open sale is started');

          require(nftsPresale >= _amount, "Purchase exceeds presale supply");
          require(presalePrice.mul(_amount) == msg.value, "Ether value sent is not correct");
          require(_amount <= maxTokensPresale, 'Cannot purchase this many tokens');
          require(presaleClaimed[whiteListId][msg.sender].add(_amount) <= maxTokensPresale, 'Purchase exceeds max presale');
          require(verify(_proof, bytes32(uint256(uint160(msg.sender)))), "Not whitelisted");

          for(uint i = 0; i < _amount; i++) {
              presaleClaimed[whiteListId][msg.sender] += 1;
              nftsPresale -= 1;
              _safeMint(msg.sender, totalSupply());
          }
    }
