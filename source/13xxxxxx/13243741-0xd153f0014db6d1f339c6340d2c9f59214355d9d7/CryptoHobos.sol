...
// File contracts/Crypto Hobos.sol
...
contract CryptoHobos is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public currentTokenPrice = 49000000000000000; //0.049E
    uint256 public constant MAX_MINTING_TOKENS_ON_TRANSACTION = 25;

    constructor(string memory baseUriString) ERC721("Crypto Hobos", "CryptoHobos") {
    ...
    function _mintHobos(address to, uint256 count) private {
    ...
    function mintReservedHoboTo(address to, uint256 count) public onlyOwner {
        if (to == address(0)) {
            to = msg.sender;
        }
        require(count > 0
    ...
    function reduceSupply(uint256 supply) public onlyOwner {
        require(supply < maxTokens && supply > lastTokenId.add(reservedTokensLeft));
        maxTokens = supply;
    }
}
