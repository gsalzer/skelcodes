...
contract BloodMoon is ERC721Enumerable, Ownable {
    ...
    uint256 private constant BLOOD_PUBLIC = 10000;
    uint256 private constant BLOOD_AIRDROP = 310;
    uint256 public constant BLOOD_MAX = BLOOD_PUBLIC + BLOOD_AIRDROP;
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = BLOOD_PUBLIC + _airdropBLOOD.current();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicBLOOD.current();
