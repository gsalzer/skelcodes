...
contract EthereumFear is ERC721Enumerable, ReentrancyGuard, Ownable {
  uint256 public constant PRICE = 69000000000000000; // 0.069 ETH

  constructor() ERC721("Ethereum Fear and Greed Index", "EFGI") Ownable() {}

  function mint(uint256 tokenId) external payable nonReentrant {
    require(tokenId > 0 && tokenId < 101, "Token ID invalid");
    require(PRICE <= msg.value, "Ether value sent is not correct. 0.069 ETH minumum.");
      ...
      bytes(
        string(
          ...
          abi.encodePacked(
            ...
            '", "description": "Multifactorial Ethereum Market Sentiment Analysis",',
            '"attributes": [',
              '{"trait_type": "Index", "value": ', Strings.toString(tokenId), '},',
              '{"trait_type": "Sentiment", "value": "', textValue, '"}',
            '],',
  ...
  function getTextValue(uint256 value) public pure returns (string memory) {
    if (value <= 23) {
      return "Extreme Fear";
    } else if (value >= 24 && value <= 37) {
      return "Fear";
  ...
  function getRotateDeg(uint256 value) public pure returns (uint256) {
    return 180 - 21 + (220 * value) / 100;
  }

  function getColor(uint256 value) public pure returns (string memory) {
    return interpolateColor([216, 115, 87], [122, 215, 93], value);
  }

  function interpolateColor(
    uint8[3] memory color1,
    uint8[3] memory color2,
    uint256 factor
  ) private pure returns (string memory) {
    uint256[3] memory result;
    // prettier-ignore
    result[0] = color1[0] + factor * color2[0] / 100 - factor * color1[0] / 100;
    // prettier-ignore
    result[1] = color1[1] + factor * color2[1] / 100 - factor * color1[1] / 100;
    // prettier-ignore
    result[2] = color1[2] + factor * color2[2] / 100 - factor * color1[2] / 100;
    return
      string(
        abi.encodePacked(
          "rgb(",
          Strings.toString(result[0]),
          ", ",
          Strings.toString(result[1]),
          ", ",
          Strings.toString(result[2]),
          ")"
        )
      );
  }
