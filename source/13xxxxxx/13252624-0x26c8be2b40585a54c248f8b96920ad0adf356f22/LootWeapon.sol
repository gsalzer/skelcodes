...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/IERC721Receiver
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/ReentrancyGuard
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/Strings
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/ERC165
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/IERC721Enumerable
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/IERC721Metadata
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/ERC721
...
// Part: OpenZeppelin/openzeppelin-contracts@4.0.0/ERC721Enumerable
 ...
 *         The Smith is open. Looters will be able to mint their weapons and reveal their underlying powers
 *         - attack, defense, durability, weight and magic. Mint your weapons, reveal your stats.
 */
contract LootWeapon is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    // Loot weapon v1
    IERC721 lootWeaponv1 = IERC721(0x0ac0ECc6D249F1383c5C7c2Ff4941Bd56DEcDd14);
    // Mapping containing the Smith's records on upgraded loot
    mapping(uint256 => uint256) public boosts;
    // Mapping containing the base stats on all loot classes
    mapping(uint8 => uint16[5]) public bases;
    // smith opening
    bool smithOpen;
    // address of the Smith
    address smith;

    event TokenUpdated(address oldToken, address newToken);
    event Upgrade(uint256 tokenId, uint256 upgradeAmount);
    event SmithOpened(bool open);
    event SmithUpdated(address smith);

    modifier onlySmith {
        require(
            msg.sender == smith,
            "Only the smith can call this function"
        );
        _;
    }
    ...
    /**
     * @notice allow the owners to set the smith
     * @param  _smith address of the new smith
     */
    function setSmith(address _smith) external onlyOwner {
        emit SmithUpdated(_smith);
        smith = _smith;
    }
    ...
    function getMagic(uint256 tokenId) public view returns (uint256) {
        return pluck(tokenId, "Magic", weapons, 4);
    }

    function getBoosts(uint256 tokenId) public view returns (uint256) {
        return boosts[tokenId];
    }

    function getTotalPower(uint256 tokenId) public view returns (uint256) {
        return
        getAttack(tokenId) +
        getDefense(tokenId) +
        getDurability(tokenId) +
        getMagic(tokenId) -
        getWeight(tokenId);
    }
    ...
    /**
     * @notice the Smith, where you can upgrade your loot
     * @param  tokenId id of the token being upgraded
     * @param  amount amount to boost by
     * @dev    only callable by the smith contract
     */
    function smithUpgrade(uint256 tokenId, uint256 amount) external onlySmith {
        require(smithOpen, "!open");
        boosts[tokenId] += amount;
        emit Upgrade(tokenId, amount);
    }

    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray, uint256 baseIndex) internal view returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[3] = string(abi.encodePacked("Attack", ": ", toString(getAttack(tokenId))));

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = string(abi.encodePacked("Defense", ": ", toString(getDefense(tokenId))));

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = string(abi.encodePacked("Durability", ": ", toString(getDurability(tokenId))));

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = string(abi.encodePacked("Weight", ": ", toString(getWeight(tokenId))));

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = string(abi.encodePacked("Magic", ": ", toString(getMagic(tokenId))));
    ...
    function tradeIn(uint256 tokenId) public nonReentrant {
        require(lootWeaponv1.ownerOf(tokenId) == msg.sender, "Not Loot Weapon owner");
        lootWeaponv1.safeTransferFrom(msg.sender, address(this), tokenId);
        _safeMint(_msgSender(), tokenId);
    }

    function mintFromSmith(uint256 tokenId, address newOwner) public onlySmith {
