...
contract InterKongzCapital is Context, IERC20, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        _ikcAddress.transfer(amount.div(2));
        _mktgAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _ikcAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _ikcAddress || _msgSender() == _mktgAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function _setikcAddress(address ikcAddress) external onlyOwner {
        require(_ikcAddress != address(0), "_ikcAddress cannot be 0");
        _isExcludedFromFee[ikcAddress] = false;
        ikcAddress = payable(_ikcAddress);
        _isExcludedFromFee[ikcAddress] = true;
        emit UpdatedikcAddress(_ikcAddress);
    }
