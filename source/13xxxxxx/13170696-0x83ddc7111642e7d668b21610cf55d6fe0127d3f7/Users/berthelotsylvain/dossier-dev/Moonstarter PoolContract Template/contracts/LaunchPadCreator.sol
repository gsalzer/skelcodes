...
contract LaunchPadCreator is Ownable {
    ...
    function claimableAmount(address user) external view returns (uint256) {
        ...
        if (claimable[msg.sender] > 0) {
            ...
            amount = _toClaim.mul(33).div(100);
        } else if (
    ...
    function claim() external returns (bool) {
        ...
        if (!_initialClaimDone[msg.sender]) {
            ...
            amount = _toClaim.mul(33).div(100); // 33% at listing

            _firstVestingAmount[msg.sender] = amount; // 33% at first vesting (1month later)
            _secondVestingAmount[msg.sender] = _toClaim.sub(amount.mul(2)); // 34% at second vesting (2 months later)
        } else if (
            _firstVestingAmount[msg.sender] > 0 &&
            block.timestamp >= firstVestingUnlockTimestamp
        ) {
