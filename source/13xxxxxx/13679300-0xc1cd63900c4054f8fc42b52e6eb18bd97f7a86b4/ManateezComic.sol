...
contract AlgaeToken is ERC20("AlgaeToken", "ALGAE"), Ownable {
    ...
    function updateRewardOnMint(address _user, uint256 _amount) external {
        ...
        if (timerUser > 0)
            rewards[_user] = rewards[_user].add(comicbook.userBalance(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
    ...
    function updateReward(address _from, address _to) external {
        ...
        if (timerFrom > 0)
            rewards[_from] += comicbook.userBalance(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
        if (timerFrom != END)
            lastUpdate[_from] = time;
        if (_to != address(0)) {
            uint256 timerTo = lastUpdate[_to];
            if (timerTo > 0)
                rewards[_to] += comicbook.userBalance(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
            if (timerTo != END)
                lastUpdate[_to] = time;
        }
    }
    ...
    function sendAlgaeToManateezOwners() public onlyOwner{
        uint256 count = mightymanateez.totalSupply();

        for(uint i = 0; i < count; i++){
            address owner = mightymanateez.ownerOf(i);
    ...
    function getTotalClaimable(address _user) external view returns (uint256) {
        uint256 time = min(block.timestamp, END);
        uint256 pending = comicbook.userBalance(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400);
        return rewards[_user] + pending;
    }

    constructor(address _comicbook, IERC721Enumerable _mightyManateez, IERC721Enumerable _merchPass) public {
        comicbook = IComics(_comicbook);
        mightymanateez = _mightyManateez;
        merchpass = _merchPass;
    }
}
...
contract ManateezComic is ERC721, Ownable {
    ...
    function mintComics(uint numberOfTokens) public payable {
        ...
        algaetoken.updateRewardOnMint(msg.sender, numberOfTokens);
    ...
    function getReward() external {
        algaetoken.updateReward(msg.sender, address(0));
        algaetoken.getReward(msg.sender);
    }

    function transferFrom(address from, address to, uint256 tokenId) public override {
        algaetoken.updateReward(from, to);
        userBalance[from]--;
        userBalance[to]++;
    ...
    function reserveForTeam(uint _amount, address _to) external onlyOwner {
        ...
        algaetoken.updateRewardOnMint(_to, _amount);
    ...
    constructor() ERC721("MightyManateez Comic Issue #1", "MMCOMIC") {}
}
