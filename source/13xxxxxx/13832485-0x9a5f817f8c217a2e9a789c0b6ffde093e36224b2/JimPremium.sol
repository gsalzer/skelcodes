...
// File contracts/PremiumLibrary.sol

pragma solidity ^0.8.2;

library PremiumLibrary {
...
// File contracts/ERC721WithSale.sol
...
contract ERC721WithSale is ERC721, ERC721Enumerable, Ownable {
  using SafeMath for uint256;
  using Strings for uint256;
  ...
  constructor(
    ...
    __baseURI = _baseUriValue;

    require(royaltiesBips < 2500, "JimPremium: Royalties cannot exceed 25%");
  }

  modifier onlyTradeableToken (address from, uint256 tokenId) {
    require(_exists(tokenId), "JimPremium: TokenId does not exist");
    require(ownerOf(tokenId) == from, "JimPremium: Token does not belong to user");
    _;
  }

  event PutTokenForSale(uint256 tokenId, uint256 price, address from);

  function putTokenForSale(uint256 tokenId, uint256 price) public onlyTradeableToken(_msgSender(), tokenId) {
    require(price > 0, "JimPremium: Cannot put zero as a price");
  ...
  function buyToken(uint256 tokenId) public payable {
    ...
    require(_exists(tokenId), "JimPremium: TokenId does not exist");
    require(offer.isForSale, "JimPremium: No Sale");
    require(msg.value == offer.price, "JimPremium: Incorrect amount");
    require(ownerOf(tokenId) == offer.seller, "JimPremium: Not seller"); // This is not possible but just in case
    address seller = offer.seller;

    // Transfer the NFT, this will automatically remove from market place
    _safeTransfer(seller, _msgSender(), tokenId, "");

    (uint256 amountAfterRoyalties, uint256 royaltiesAmount) = 
      PremiumLibrary.calculateRoyalties(msg.value, royaltiesBips);

    volume += msg.value;
    totalSales += 1;
    if (msg.value > highestSalePrice) {
      highestSalePrice = msg.value;
    }
  ...
  function tokenNoLongerForSale(uint256 tokenId) public onlyTradeableToken(_msgSender(), tokenId) {
    tokensForSale[tokenId] = Sale(false, tokenId, _msgSender(), 0);
    emit TokenNoLongerForSale(tokenId, _msgSender());
  }

  // Gets all tokenIds that are onsale
  function getOnSaleTokenIds(uint256 start, uint256 end) public view 
    ...
    uint256 max = totalSupply() > end ? end : totalSupply();
...
// File contracts/JimPremium.sol

pragma solidity ^0.8.2;


contract JimPremium is ERC721WithSale {
    ...
    ) ERC721WithSale("Jim's Crown", "JIMS", _baseUriValue, _royaltiesBips) {
    ...
    event TokenMinted(uint256 tokenId, address buyer);

    function mintToken() public payable {
        require(
            mintPrice == msg.value,
            "JimPremium: Payment amount is incorrect."
        );
        _mintToken(_msgSender());
    }

    function adminMintToken(address recipient) public onlyOwner {
        _mintToken(recipient);
    }

    function _mintToken(address recipient) private {
        uint256 balance = balanceOf(recipient);
        require(balance < 1, "JimPremium: Recipient already has a token");
