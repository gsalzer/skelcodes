...
contract SuperlativeApes is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x26F3103bF9721c07c89aC0151972398fD0cf679e);
    address payable private _DevAddress;

    uint256 public LAPE_MAX = 4444;
    uint256 public PRESALE_PRICE = 69_000_000_000_000_000; // 0.069 ETH
    uint256 public PUBLIC_PRICE = 69_000_000_000_000_000; // 0.069 ETH
    uint256 private DEV_FEE = 0;
    uint256 private REVEAL_DELAY = 24 hours;
    uint256 private PRESALE_START_HOURS = 24 hours; // Start 24hrs before 
    uint256 private PRESALE_END_HOURS = 4 hours; // And End 4hrs before
    ...
    uint256 private _activeDateTime = 1639710000; // Date and time (GMT): Friday, December 17, 2021 3:00:00 AM
    
    string private _tokenBaseURI = "";

    Counters.Counter private _publicLAPE;

    constructor() ERC721("SUPERLATIVE APES", "LAPE") {
    ...
    function setPresaleHours (uint256 presaleStartHours, uint256 presaleEndHours) external onlyOwner {
        PRESALE_START_HOURS = presaleStartHours;
        PRESALE_END_HOURS = presaleEndHours;
    }
    ...
    function PRICE(address addr) public view returns (uint256) {
        if (block.timestamp < _activeDateTime) {
            require(block.timestamp > _activeDateTime - PRESALE_START_HOURS && block.timestamp < _activeDateTime - PRESALE_END_HOURS,"Contract is not active for presale");
            require(_mappingWhiteList[addr] == true , "OnlyWhitelist member can mint now");
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicLAPE.current();

            if (_publicLAPE.current() < LAPE_MAX) {
                _publicLAPE.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicLAPE.current() < LAPE_MAX,"Purchase would exceed LAPE_MAX");
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory)
    {
        ...
        if ((_mappingMintTimestamp[tokenId] + REVEAL_DELAY < block.timestamp) && (_activeDateTime + REVEAL_DELAY < block.timestamp)) {
