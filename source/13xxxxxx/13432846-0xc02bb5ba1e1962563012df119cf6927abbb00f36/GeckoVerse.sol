...
contract GeckoVerse is Ownable, ERC721 {
    ...
    uint public maxPerTransaction_public=6;
    uint public maxPerTransaction = 3;  //Max Limit for pre-Sale
    uint public maxPerWalletPresale = 3; //Max Limit for Presale
  
         
    constructor() ERC721("The GeckoVerse Collection", "GVC"){}

   function mint(uint _count) public payable{
        require(_count > 0, "mint at least one token");
        require(_count <= maxPerTransaction_public, "max per transaction 6");
   ...
   function mint_presale(uint _count) public payable{
       ...
       require(msg.value >= tokenPrice * _count, "incorrect ether amount");
        ...
        require(balanceOf(msg.sender) + _count<= maxPerWalletPresale, "3 tokens per wallet allowed in presale");
          for(uint i = 0; i < _count; i++)
            _safeMint(msg.sender, totalSupply + 1 + i);
        totalSupply += 1;
    }
    ...
    function update_max_supply(uint total)public onlyOwner{
        maxSupply=total;
    }
