...
interface iFeather {
    ...
    function infos(uint256 id) external view returns (Character memory nft);
...
contract EVM is ERC721, ERC721Enumerable, Ownable {
    ...
    function claim() public {
        iFeather feather = iFeather(0x51d0B69886DcDe7a4fb9b39722868056804AFbca); //Feather
        uint256 holdings = feather.balanceOf(msg.sender);

        for (uint256 index = 0; index < holdings; index++) {
            uint256 id = feather.tokenOfOwnerByIndex(msg.sender, index);

            iFeather.Character memory info = feather.infos(id);

            if (claimed[id] == false && id < 1000 && !info.isFree) {
                //# of feather at time of drop
                _mint(msg.sender, id);
                claimed[id] = true;
            }
        }
