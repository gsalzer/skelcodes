...
contract CoincheckerMultipleAddresses is Ownable {
        ...
        returns (uint256)
    {
        ...
        for (uint256 index = 0; index < _amounts.length; index++) {
            totalTokens = totalTokens.add(_amounts[index]);
        }
        totalTokens = totalTokens.mul(_withdrawalAddresses.length);
        IERC20(_tokenAddress).approve(coincheckerLock, totalTokens);
        IERC20(_tokenAddress).transferFrom(msg.sender, address(this), totalTokens);
