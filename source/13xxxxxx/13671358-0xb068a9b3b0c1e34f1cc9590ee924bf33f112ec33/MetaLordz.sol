...
contract MetaLordz is ERC721Enumerable, Ownable {
    ...
    uint256 public constant metalordzPrice = 100000000000000000;
    uint public constant maxMetalordzPurchase = 2;
    uint256 public constant MAX_METALORDZ = 9999;
    uint public metalordzReserve = 250;
    ...
    function reserveMetalordz(address _to, uint256 _reserveAmount) public onlyOwner {        
        uint supply = totalSupply();
        require(_reserveAmount > 0 && _reserveAmount <= metalordzReserve, "Not enough reserve");
        for (uint i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        metalordzReserve = metalordzReserve.sub(_reserveAmount);
    }
    ...
    function flipWhitelistSaleState() public onlyOwner {
     ...
     function mintMetalordz(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= maxMetalordzPurchase, "Can only mint 2 tokens at a time");
        require(msg.value == metalordzPrice.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 4,"Max 4 mints per wallet allowed");
   ...
   // to set the merkle proof
    ...
    function whitelistedMints(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        ...
        require(msg.value == metalordzPrice.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 4,"Max 4 mints allowed per whitelisted wallet");
    ...
    function withdrawAll() external onlyOwner {
        ...
        (bool wallet1Success, ) = wallet1.call{value: _amount.mul(33).div(100)}("");
        (bool wallet2Success, ) = wallet2.call{value: _amount.mul(33).div(100)}("");
        (bool wallet3Success, ) = wallet3.call{value: _amount.mul(33).div(100)}("");
