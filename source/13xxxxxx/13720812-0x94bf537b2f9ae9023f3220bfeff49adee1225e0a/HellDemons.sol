...
contract HellDemons is ERC721, Ownable {
    ...
    uint256 public perOfGiveaway = 17;
    ...
    uint256 public presaleAmount = 3996;
    uint256 public presalePerWallet = 6;
    ...
    function setPresalePerWallet(uint256 _presalePerWallet) public onlyOwner {
        presalePerWallet = _presalePerWallet;
    }
    ...
    function mintByTeam() external onlyOwner {
    ...
    function mintForGiveaway() external onlyOwner {
        ...
        if (numberOfMint > perOfGiveaway) numberOfMint = perOfGiveaway;
    ...
    function mintDemon(uint256 _numberOfTokens) public payable {
        ...
        if (presale) {
            ...
            require(_tokenBalance[_msgSender()].add(_numberOfTokens) <= presalePerWallet, "Your wallet is limited in presale");
            require(currentSupply.add(_numberOfTokens) <= presaleAmount.add(numOfGiveaway).add(team.length), "No tokens available for presale");
            price = presalePrice;
        } else {
