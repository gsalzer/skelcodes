...
contract MerkleEligibility is IEligibility {
    ...
    function addGate(bytes32 _merkleRoot, uint _maxWithdrawals) external managementOnly returns (uint index) {
        // increment the number of roots
        numGates += 1;

        gates[numGates] = Gate(_merkleRoot, _maxWithdrawals);
        return numGates;
    }

    function getGate(uint index) external view returns (bytes32, uint) {
        return (gates[index].root, gates[index].maxWithdrawals);
    }

    function isEligible(uint rootIndex, address recipient, bytes32[] memory proof) public override view returns (bool eligible) {
        // We need to pack the 20 bytes address to the 32 bytes value
        bytes32 leaf = keccak256(abi.encode(recipient));
        bool countValid = timesWithdrawn[rootIndex][recipient] < gates[rootIndex].maxWithdrawals;
        return countValid && gates[rootIndex].root.verifyProof(leaf, proof);
    }
