...
abstract contract ERC721LazyMintWhitelistBase is ERC721SingleCreatorExtensionBase, ICreatorExtensionTokenURI, ReentrancyGuard {
    ...
    uint private MINT_PRICE = 0.5 ether; // to be changed
    uint private MAX_MINTS = 50; // to be changed
    bytes32 merkleRoot;
    mapping(address => bool) private ownsToken;
    ...
    function _mint(bytes32[] memory merkleProof) internal {
        ...
        require(ownsToken[msg.sender] == false, "Cannot mint twice");

        ownsToken[msg.sender] = true;
        _tokensMinted += 1;
        _tokenEdition[IERC721CreatorCore(_creator).mintExtension(msg.sender)] = _tokensMinted;
    }
