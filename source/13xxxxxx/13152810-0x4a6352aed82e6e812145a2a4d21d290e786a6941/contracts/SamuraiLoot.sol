...
contract SamuraiLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getClan(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "CLAN", clan);
    }
    ...
    function getArmorColor(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "ARMOR_COLOR", armor_color);
    }
    ...
    function pluck(
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("RACE") && greatness > 14) {
            output = string(abi.encodePacked(prfixes[rand % prfixes.length], " ", output));

            if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("RACE") && greatness >= 19) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: monospace; font-size: 14px; }</style><rect width="100%" height="100%" fill="#fA3637" /><text x="10" y="20" class="base">';

        parts[1] = getRace(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = getClan(tokenId);
        ...
        parts[9] = getArmorColor(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getWeapon(tokenId);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = getFace(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "SamuraiLoot #',
                        toString(tokenId),
                        '", "description": "SamuraiLoot is randomized samurai property and stored on chain. SamuraiLoot will be exchanged for Pixelized Samurai in the future, and will open war mode. Join Community Driven Samurai War!", "image": "data:image/svg+xml;base64,',
    ...
    function withdraw() public onlyOwner {
        payable( 0xf6BB1aD9b9A8134f383ff36cd181E72363348C90 ).transfer(address(this).balance);
    }
    ...
    constructor() ERC721("SamuraiLoot", "SAMURAI") Ownable() {}
}
