...
contract NFTTokenEngine {
    ...
    function setNftTokenValues(
        ...
        address newNftContract = address(
            new HUMBLNFT(name, owner, symbol, uri, cap, id, royaltyAmount, creator)
        );
...
contract HUMBLNFT is ERC721URIStorage, Royalties, Ownable {
    ...
    function mintNewId(address owner, uint256 id, string memory URI) public returns (uint256) {
        _mint(owner, id);
        _setTokenURI(id, URI);
