...
contract SpellActionCommon {
    ...
    function executeCommon(address changeLogAddr) internal {
        ...
        VowLike(MCD_VOW).file("bump", TWO_THOUSAND_RAD);
        VowLike(MCD_VOW).file("hump", THREE_HUNDRED);

        // PotLike(MCD_POT).drip();
        // PotLike(MCD_POT).file("dsr", THREE_PERCENT_RATE);
        

        ChainlogAbstract(changeLogAddr).setVersion("1.5.0");
    }
}
