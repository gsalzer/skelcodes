...
contract Variable is Context, IERC20, Ownable {
    ...
    function setmarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        ...
        _otherFee = _marketingFee.add(_devFee);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tOther);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tOther, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rOther);
