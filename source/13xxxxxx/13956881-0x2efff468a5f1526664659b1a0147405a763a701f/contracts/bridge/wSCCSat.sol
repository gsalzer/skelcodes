...
-You buy on Fantom or Ethereum, we farm on multiple chains and return the profits to $SCC holders.
Tokenomics:
...
contract wSCCSat is Context, IERC20, Ownable, Controller {
    ...
    uint256 public _tTotal = 0 * 10**9; //BRDIGED
    uint256 public _rTotal = MAX; //BRIDGED
    uint256 private _tFeeTotal;
    string private _name = "ScaryChainCapital (ETH)"; //BRIDGED
    ...
    address payable public _SCCWalletAddress = 0x287f0e1826D88D0D212fF4A0Ede781186DA575A2;
    address payable public _marketingWalletAddress = 0xb891202b6cbA6269F995a0bD337f03F9208efD35;
    uint256 private _SCCPercent = 80;
    ...
    constructor(
        ...
        _rOwned[_msgSender()] = _rTotal; //BRIDGED
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D 
        ); // Uniswap Router for ETH Network BRIDGED
        ...
        setOperator(msg.sender, true); //BRIDGED
    ...
    function mint(address account, uint256 amount) external onlyOperator() {
        require(account != address(0), "bridge: cannot send to the zero address");

        _tTotal = _tTotal.add(amount);
        _rTotal = (MAX - (MAX % _tTotal));

        uint256 currentRate = _getRate();
        uint256 bridgeAmount = amount.mul(currentRate);

        if (_isExcluded[account]) {
            _tOwned[account] = _tOwned[account].add(amount);
        } else {
            _rOwned[account] = _rOwned[account].add(bridgeAmount);
        }
    ...
    function burn(address account, uint256 amount) external onlyOperator() {
        ...
        if (_isExcluded[account]) {
            ...
            _rOwned[account] = _rOwned[account].sub(bridgeAmount);
        }
