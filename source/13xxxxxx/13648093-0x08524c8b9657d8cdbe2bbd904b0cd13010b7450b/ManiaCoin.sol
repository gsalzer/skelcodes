...
contract ManiaCoin {
    ...
    address payable public prizePool;
    ...
    string private _name = "ManiaOne";
    string private _symbol = "MANIA";
    uint8 private _decimals = 9;

    uint256 public __reflectFee = 1;
    uint256 public __marketingFee = 7;
    uint256 public __prizeFee = 2;
    ...
    uint256 public _maxWalletHolding = 10 * 10**6 * 10**9;
    ...
    constructor (address payable _marketing, address payable _prizePool) public {
      _owner = msg.sender;
    ...
    function setNumTokensToSwap(uint _numTokens) public onlyOwner {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256[3] memory rValues = _getRValuesArray(tAmount, tValues[1], tValues[2]);
        return (rValues[0], rValues[1], rValues[2], tValues[0], tValues[1], tValues[2]);
    }
    ...
    function _getRValuesArray(uint256 tAmount, uint256 tFee, uint256 tOperations) private view returns (uint256[3] memory val) {
        uint256 currentRate = _getRate();
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(tOperations.mul(currentRate));
    ...
    function calculateOperationsFee(uint256 _amount) private view returns (uint256) {
        uint256 operations_fee = __marketingFee.add(__prizeFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingBalancePart = __marketingFee.mul(newBalance).div(__marketingFee + __prizeFee);

        marketing.sendValue(marketingBalancePart);
        prizePool.sendValue(newBalance.sub(marketingBalancePart));
    }
