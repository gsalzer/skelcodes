...
interface VaultAPI is IERC20 {
    ...
    function deposit(uint256 amount, address recipient, bool stakeInstantly) external returns (uint256);
...
// File: StrategyONXankr.sol

contract StrategyONXankr is BaseStrategy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address public constant onxFarm = address(0x168F8469Ac17dd39cd9a2c2eAD647f814a488ce9);
    ...
    function _approveBasic() internal {
        want.approve(onxFarm, 0);
        want.approve(onxFarm, type(uint256).max);
    }

    function _approveDex() internal virtual {
        IERC20(onx).approve(dex, 0);
        IERC20(onx).approve(dex, type(uint256).max);
        IERC20(weth).approve(dex, 0);
        IERC20(weth).approve(dex, type(uint256).max);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        return IONXFarm(onxFarm).userInfo(pid, address(this)).amount;
    }
    ...
    function pendingONXRewards() public view returns (uint256) {
        return IONXFarm(onxFarm).pendingOnX(pid, address(this));
    }

    function claimONXRewards() internal {
        // Zero amount can trigger internal harvest in withdraw function to acquire ONX rewards
        IONXFarm(onxFarm).withdraw(pid, 0);
    }
        ...
        )
    {
        ...
        if (_onx > 0) {
            ...
            Uni(dex).swapExactTokensForTokens(_onx, uint256(0), path, address(this), now);
        }
        ...
        if (_weth > 0) {
            ...
            Uni(dex).swapExactTokensForTokens(_weth, uint256(0), path, address(this), now);
        }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (_want > 0) {
            IONXFarm(onxFarm).deposit(pid, _want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        IONXFarm(onxFarm).withdraw(pid, _amount);
    ...
    function liquidateAllPositions() internal override returns (uint256 _amountFreed) {
        uint256 _loss = 0;
        (_amountFreed, _loss) = liquidatePosition(estimatedTotalAssets());
    }

    function prepareMigration(address _newStrategy) internal override {
        IONXFarm(onxFarm).withdraw(pid, balanceOfPool());
    ...
    function _migrateRewards(address _newStrategy) internal {
        ...
        IERC20(weth).safeTransfer(_newStrategy, IERC20(weth).balanceOf(address(this)));
    }
        ...
        returns (uint256)
    {
        ...
        uint256[] memory amounts = Uni(dex).getAmountsOut(_amtInWei, path);
