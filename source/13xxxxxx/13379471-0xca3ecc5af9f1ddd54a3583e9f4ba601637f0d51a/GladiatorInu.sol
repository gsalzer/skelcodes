...
contract GladiatorInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 85 * 10**7 * (10**18);
    ...
    IERC20 eArena = IERC20(0xc30590A879C7Dd78bCd9A18E48de446A3c7c98b8);

    constructor() public ERC20("GladiatorInu", "GLAD") {    	
    ...
    function _transfer(
        ...
        if (!isTradingEnabled) {
            ...
            require(amount <= 185 * 10**7 * (10**18), "Over the max buy amount.");
        }
        ...
        if(takeFee && (to == uniswapV2Pair || from == uniswapV2Pair)) {
            uint256 fees;
            if (from == uniswapV2Pair && IERC20(eArena).balanceOf(to) >= 5 * 10**11 * (10**18)) {
                fees = amount.mul(15).div(1000);
