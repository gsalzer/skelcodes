...
contract ManifestGenesisERC1155 is ERC1155, Ownable {
    ...
    event HoodiesMinted(address sender, uint256 count);
    ...
    string public symbol = "MNFSTGENESIS";
    ...
    uint256 public MAX_PER_WALLET = 33;
    ...
    bool public treasuryHasMinted = false;
    ...
    constructor (address _treasury, address _sMNFST, string memory _uri) ERC1155(_uri) {
        treasury = _treasury;
        sMNFST = _sMNFST;
    }
    ...
    /**
        @dev uses information about the transaction, and a rolling nonce to try to generate
        a random number between 0 and the modulus.
     */
    ...
    function mint(uint256 _numHoodies) external payable {
        require(saleIsActive, "SALE_NOT_STARTED");
        require(_numHoodies > 0 && _numHoodies <= MAX_PER_MINT, "AMOUNT_INCORRECT");
        require(_numHoodies <= totalRemaining1 + totalRemaining2 + totalRemaining3, "AMOUNT_EXCEEDS_REMAINING");
        require(price * _numHoodies == msg.value, "NOT_ENOUGH_ETH");
        require(totalClaimed[msg.sender] + _numHoodies <= MAX_PER_WALLET, "EXCEEDS_MAX_PER_WALLET");
        ...
        // go through each requested mint
        for (uint256 i = 0; i < _numHoodies; i++) {

            // get a "random" number from 1-3
            uint256 hoodieNum = (randMod(123456) % 3) + 1;

            // find the correct hoodie chosen
            // make sure there's enough
            // increment and continue to next mint

            if (hoodieNum == TOKEN_HOODIE_1) {
                if (totalRemaining1 > 0) {
                    totals[0]++;
                    totalRemaining1--;
                    continue;
                }
            } else if (hoodieNum == TOKEN_HOODIE_2) {
                if (totalRemaining2 > 0) {
                    totals[1]++;
                    totalRemaining2--;
                    continue;
                }
            } else if (hoodieNum == TOKEN_HOODIE_3) {
                if (totalRemaining3 > 0) {
                    totals[2]++;
                    totalRemaining3--;
                    continue;
                }
            ...
            // if the assigned random hoodie choice wasn't available,
            // choose the next one that's available from the ones remaining
            ...
            // this shouldn't be possible...but adding a revert here just in case
            revert("ERROR_STATE");
        }
        
        totalClaimed[msg.sender] += _numHoodies;

        _mintBatch(msg.sender, tokens, totals, '');
        IERC20Contract( sMNFST ).transfer(msg.sender, _numHoodies * 33 * 10 ** 9);
        
        emit HoodiesMinted(msg.sender, _numHoodies);
    }
