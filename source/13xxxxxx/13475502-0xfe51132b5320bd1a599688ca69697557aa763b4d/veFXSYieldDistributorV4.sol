...
// File contracts/Staking/IveFXSYieldDistributor.sol
...
contract veFXSYieldDistributorV4 is Owned, ReentrancyGuard {
    ...
    // Old veFXSYieldDistributor
    IveFXSYieldDistributor public oldYieldDistributor = IveFXSYieldDistributor(0xed2647Bbf875b2936AAF95a3F5bbc82819e3d3FE);
    ...
    function getYield() external nonReentrant notYieldCollectionPaused checkpointUser(msg.sender) returns (uint256 yield0) {
        ...
        // Collect from previous contract, if applicable
        if (address(oldYieldDistributor) != address(0)){
            uint256 yieldOld = oldYieldDistributor.yields(msg.sender);
            if (yieldOld > 0){
                oldYieldDistributor.getYield();
                emit OldYieldCollected(msg.sender, yieldOld, emitted_token_address);
            }
        }
    ...
    function setOldYieldDistributor(address _old_yield_distributor) external onlyByOwnGov {
        oldYieldDistributor = IveFXSYieldDistributor(_old_yield_distributor);
    }
