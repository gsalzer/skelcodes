...
contract RevyInu is Context, IERC20, Ownable {
    ...
    string private _name = "Revy Inu";
    string private _symbol = "Revy";
    ...
    uint256 public limitChangeTime = 0;
    ...
    function changeLimit() public onlyOwner {
        ...
        limit = false;
        limitChangeTime = block.timestamp;
    }
    ...
    function expectedRewards(address _sender) external view returns (uint256) {
        ...
        uint256 rewards = holdersBal.mul(_balance).div(
            _tTotal.sub(balanceOf(pancakePair)).sub(totalExcludedBal)
        );
        return rewards;
    }
    ...
    function _transfer(
        ...
        if (limit == true && from != owner() && to != owner()) {
            ...
            require(
                amount <= 100 ether,
                "Transfer amount must be less than 100 tokens"
            );
        }

        // max tx limit for first 2 minutes of trading
        if (
            block.timestamp <= limitChangeTime + 120 seconds &&
            from != owner() &&
            to != owner()
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (block.timestamp <= limitChangeTime) {
            return _amount.mul(_liquidityFee).mul(6).div(10**2);
        } else {
