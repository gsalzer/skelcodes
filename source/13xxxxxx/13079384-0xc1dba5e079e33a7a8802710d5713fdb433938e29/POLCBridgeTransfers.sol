...
contract POLCBridgeTransfers is Context {
    ...
    function _bridgeSend(uint256 _amount) private {
        ...
        require(polcToken.transferFrom(msg.sender, polcVault, _amount));
    }
    
    function _bridgeWhitelisted(uint256 _amount) private {
        require(polcToken.transferFrom(msg.sender, polcVault, _amount));
    }
    ...
    function setPOLCVault(address _vault, bytes memory _sig) public isManager {
        uint8 mId = 2;
        bytes32 taskHash = keccak256(abi.encode(_vault, taskIndex, mId));
        verifyApproval(taskHash, _sig);
        polcVault = _vault;
    ...
    function whitelistWallet(address _wallet, bool _whitelisted, bytes memory _sig) public isManager {
        uint8 mId = 6;
        bytes32 taskHash = keccak256(abi.encode(_wallet, _whitelisted, taskIndex, mId));
        verifyApproval(taskHash, _sig);
