...
// File: Volumes/code/remix/my/distribution.sol
...
contract Distribution is ReentrancyGuard {
    function eth(uint amount, address payable[] calldata dst) public nonReentrant payable {
        for (uint i; i < dst.length; i++) {
            dst[i].transfer(amount);
        }
    }

    function erc20(uint amount, IERC20 token, address [] calldata dst) public nonReentrant {
        for (uint i; i < dst.length; i++) {
