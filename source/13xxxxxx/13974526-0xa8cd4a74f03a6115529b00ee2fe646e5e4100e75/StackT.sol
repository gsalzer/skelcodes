// hevm: flattened sources of src/stackt.sol
...
Stack Treasury
A revolutionary take on DeFi.

When you hold $STACKT, we invest it across multiple 
chains using our farming strategies.

All the profits go back to the holders.
...
contract StackT is Ownable, IERC20 {
    address UNISWAPROUTER = address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string private _name = "Stack Treasury";
    string private _symbol = "STACKT";
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "StackT: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "StackT: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "StackT: Sender is blacklisted");
        require(!isBlacklisted[recipient], "StackT: Recipient is blacklisted");

        require(sender != address(0), "StackT: transfer from the zero address");
        require(recipient != address(0), "StackT: transfer to the zero address");
        ...
        require(
            ...
            "StackT: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "StackT: approve from the zero address");
        require(spender != address(0), "StackT: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "StackT: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "StackT: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "StackT: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "StackT: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "StackT: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "StackT: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "StackT: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "StackT: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "StackT_DividendTracker";
    string private _symbol = "StackT_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "StackT_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "StackT_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "StackT_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "StackT_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("StackT_DividendTracker: method not implemented");
    }
