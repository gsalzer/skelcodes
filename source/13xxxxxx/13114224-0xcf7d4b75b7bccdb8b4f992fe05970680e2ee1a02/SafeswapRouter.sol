...
library SafeswapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = ISafeswapPair(pair).price0CumulativeLast();
        price1Cumulative = ISafeswapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = ISafeswapPair(pair).getReserves();
...
library SafeswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'7fc48862bb659c6079c67f949053514afd141b7fcc1dc2b0a9474d647c51d670' // init code hash
            ))));
    }
...
contract SafeswapRouter01 is ISafeswapRouter01 {
    address public immutable override factory;
    address public immutable override WETH;
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? SafeswapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            ISafeswapPair(SafeswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        uint amountIn,
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return SafeswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
