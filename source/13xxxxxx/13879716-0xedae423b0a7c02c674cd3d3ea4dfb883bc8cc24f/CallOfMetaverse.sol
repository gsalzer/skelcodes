...
contract CallOfMetaverse is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress;

    uint256 public COM_MAX = 10000;
    ...
    uint256 public PRESALE_MINT_LIMIT = COM_MAX;
    uint256 public PUBLIC_MINT_LIMIT = COM_MAX;
    uint256 private FEE_PERCENT = 0;
    
    mapping(address => bool) _mappingWhiteList;
    mapping(address => uint256) _mappingMintCount;

    uint256 private _activeDateTime = 1642377600; // (GMT): Monday, January 17, 2022 12:00:00 AM
    uint256 private _revealDateTime;
    
    string private _tokenBaseURI = "";
    string private _revealURI = "";
    
    Counters.Counter private _publicCOM;

    constructor() ERC721("Call Of Metaverse", "COM") {}
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCOM.current();

            if (_publicCOM.current() < COM_MAX) {
                _publicCOM.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicCOM.current() < COM_MAX,"Purchase would exceed COM_MAX");
