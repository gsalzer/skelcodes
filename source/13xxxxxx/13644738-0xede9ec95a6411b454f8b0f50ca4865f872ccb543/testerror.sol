...
contract testerror {
        ...
        )
    {
        ...
        uint256 calculatedPrice = (exchangeAmount * (ethPerToken)) / (10**18);
        uint256 decimals = ERC20(swapPath[0]).decimals();
        uint256 preDecimals = calculatedPrice;
        if (decimals < 18) {
            calculatedPrice = calculatedPrice * (10**(18 - decimals));
        }

        return (sushiAmount, calculatedPrice, preDecimals, portion, decimals);
    }
