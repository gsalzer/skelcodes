// File: contracts/IThreeNityNFTToken.sol
...
contract Crowdsale is Context, ReentrancyGuard  {
    ...
    IThreeNityNFTToken private _token;
    ...
    // Sale price is 0.5 ETH
    uint256 internal _rate = 500000000000000000;
     ...
     */
    constructor (address payable __wallet, IThreeNityNFTToken __token) public {
     ...
     */
    function token() public view virtual returns (IThreeNityNFTToken) {
        return _token;
    }
...
abstract contract TimedCrowdsale is Crowdsale {
    ...
    constructor (uint256 __openingTime, uint256 __closingTime) {
        ...
        require(__closingTime > __openingTime, "TimedCrowdsale: opening sale time is not before closing time");
...
// File: contracts/ThreeNityNFTICO.sol
...
contract ThreeNityNFTICO is Crowdsale, CappedCrowdsale, TimedCrowdsale, Ownable {
    using SafeMath for uint256;

    /* Track investor contributions */
    uint256 public investorTransactionHardCap = 20; // 20 NFT

    constructor(
        address payable _wallet,
        IThreeNityNFTToken _token,
        uint256 _cap,
    ...
    function _preValidatePurchase(
        ...
        require(currentMinted() <= _cap, "ThreeNityNFTICO: cap exceeded");

        require(weiAmount != 0, "ThreeNityNFTICO: ETH sent 0");

        require(
            weiAmount.div(mintAmount) == rate(),
            "ThreeNityNFTICO: Invalid ETH Amount"
        );

        require(
            _beneficiary != address(0),
            "ThreeNityNFTICO: beneficiary zero address"
        );

        // Check max transaction minting limit
        require(
            mintAmount <= investorTransactionHardCap,
            "ThreeNityNFTICO: You are exceeding the investor per transaction limit"
        );
