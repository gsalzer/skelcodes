...
contract APG is Context, IERC20, Ownable {
    ...
    uint256 private _total = 40000000000 * 10 ** 9;
    uint256 private _feeTotal;

    string private _name = "APG";
    ...
    constructor() {
        ...
        _isIncludedFromFee[uniswapV2Pair] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            takeFee = false;
        } else if (_isIncludedFromFee[from] || _isIncludedFromFee[to]) {
            takeFee = true;
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 transferAmount, uint256 fee) = _getValues(amount);
    ...
    function _reflectFee(uint256 fee) private {
        _owned[address(0)] = _owned[address(0)].add(fee);
        _feeTotal = _feeTotal + fee;
    }
