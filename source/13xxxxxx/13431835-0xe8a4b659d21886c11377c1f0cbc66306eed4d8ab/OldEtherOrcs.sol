...
interface OldMetadataHandlerLike {
...
interface OldListLike {
    function register(address buyer) external;
}

contract OldEtherOrcs is ERC721 {
    ...
    OldMetadataHandlerLike metadaHandler;
    OldListLike            list;

    function setAddresses(address meta, address list_) external {
        require(msg.sender == admin, "not allowed");
        list          = OldListLike(list_);
        metadaHandler = OldMetadataHandlerLike(meta);
    }
