...
contract NiftyBaseMerkle is ERC721Enumerable, PaymentSplitter, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint8;

    // Token Data
    uint256 public constant TOKEN_PRICE = 0.05 ether;
    uint256 public constant RESERVED_TOKENS = 69;
    ...
    function mintPreSaleTokens(uint8 numberOfTokens, bytes32[] calldata _merkleProof) external payable {
        ...
        require(MerkleProof.verify(_merkleProof, preSaleRoot, leaf), "Invalid MerkleProof");
    ...
    function mintOGSaleTokens(uint8 numberOfTokens, bytes32[] calldata _merkleProof) external payable {
        ...
        // check proof
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, ogSaleRoot, leaf), "Invalid MerkleProof");
