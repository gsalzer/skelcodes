...
contract CryptoBloxsNFT  is ERC721Enumerable, Ownable {
    ...
    uint256 public MAX_SUPPlY = 788;
    uint256 public RESERVE_COUNT = 100;
    uint256 public PRESALE_SUPPLY = 100;
    uint256 public MAX_ALLOWED = 10;
    uint256 public MAX_PRESALE_ALLOWED = 4;
    //0.88,1.00,1.25,1.50,1.75,2.00,2.15,2.25
    uint256[] public priceScale = [880000000000000000,1000000000000000000,1250000000000000000,1500000000000000000,
    1750000000000000000,2000000000000000000,2150000000000000000,2250000000000000000]; 
    uint256 public price=priceScale[0];
    string baseTokenURI;
    ...
    constructor(string memory baseURI) ERC721("Crypto Bloxs", "CRBLOXS") {
        setBaseURI(baseURI);
    }
    ...
    function currentPrice() public view returns (uint256) {
        return priceScale[(totalSupply()-_reserveId.current())/100];
    }
    ...
    function presaleMintNFT(uint256 _count) public payable {
        price = priceScale[0];
        if (msg.sender != owner()) {
            require(presaleOpen, "Presale is paused please try again later");
        }
        ...
        if (msg.sender != owner()) {
            require(
                totalSupply() - _reserveId.current() + _count <= (PRESALE_SUPPLY),
                "All NFTs sold"
            );
        }else{
            require(
        ...
        // uint256 tokenId = _tokenId.current()+100;
       // if ( tokenId>PRESALE_SUPPLY) presaleOpen = false;
    }
    ...
    function mintNFT(uint256 _count) public payable {
        if (msg.sender != owner()) {
         ...
         require(
                totalSupply()-_reserveId.current()  <= (MAX_SUPPlY),
                "All NFTs sold"
            );
        
        require(
            _count > 0 && _count <= MAX_ALLOWED,
            "Min 1 & Max 10 NFTs can be minted per transaction"
        );
        
        if (msg.sender != owner()) {
            require(
                totalSupply()-_reserveId.current() + _count <= (MAX_SUPPlY),
    ...
    function airdrop(address[] calldata _recipients,uint[] calldata tokenid) external onlyOwner {
        require(
            (totalSupply() + _recipients.length) <= (MAX_SUPPlY+RESERVE_COUNT),
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            specMint(_recipients[i],tokenid[i]);
        }
    }
    function specMint(address _to,uint tokenId) private {
