// mirrorcats.com
// discord.gg/mirrorcats
// opensea.io/collection/mirror-cats-nft
//
...
contract MC is ERC721Enumerable, Ownable, PaymentSplitter {
    ...
    address t1 = 0x634d032fAa3c38Bc867f5cF8ab78c07714a724d3;

    address[] addressList = [t1];
    uint256[] shareList = [100];

    constructor()
    ERC721("Mirror Cats", "MC")
    PaymentSplitter(addressList, shareList)  {}

    function mint(uint256 _count) public payable {
        uint256 supply = totalSupply();
        require( block.timestamp >= _startTime,        "General sale has not started yet" );
        require( _count <= MAX_TOKENS_PER_TRANSACTION, "You can mint a maximum of 100 Mirror Cats per transaction" );
        require( supply + _count <= MAX_SUPPLY,        "Exceeds current Mirror Cat supply limit" );
    ...
    function mint(address _wallet, uint256 _count) public onlyOwner {
        uint256 supply = totalSupply();
        require(supply + _count <= MAX_SUPPLY, "Exceeds maximum Mirror Cat supply");
