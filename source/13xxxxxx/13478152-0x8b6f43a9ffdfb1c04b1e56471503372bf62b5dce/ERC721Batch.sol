    ...
    IERC721Enumerable
{
    ...
    constructor(
        ...
        _symbol = symbol;
        _setBaseURI(baseURI);
        _royaltyOwner = _royaltyReceiver;
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external override view returns (address, uint256){
        uint256 royalty = _BatchRoyaltyAmount(_tokenId);
        return (_royaltyOwner, royalty.mul(_salePrice).div(10_000));
    }
    ...
    function _BatchRoyaltyAmount(uint256 _tokenId) internal view returns (uint256){
        if(royaltyAmounts[_tokenId] > 0){
            return royaltyAmounts[_tokenId];
        }else{ 
            uint256 min = _tokenId.div(MAX_MINT);
            uint256 max = (_tokenId.div(MIN_MINT)).add(1);
            if (max > _totalBatches) {
                max = _totalBatches;
            }
            for (uint256 i = min; i < max; i++) {
                if (_batchMax[i].length() >= 1) {
                    if (_batchMax[i].at(1) >= _tokenId) {
                        if (
                            _batchMintOwnersMap[i].start <= _tokenId &&
                            _batchMintOwnersMap[i].end >= _tokenId
                        ) {
                            return _batchMintOwnersMap[i].royalty;
                        }
                    }
                }
            }
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, uint256 _royalty) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        _totalSupply = _totalSupply.add(1); 
        _holderTokens[to].add(_totalSupply);
        _tokenOwners.set(_totalSupply, to);
        royaltyAmounts[_totalSupply] = _royalty;
        BalancesMap[to] = BalancesMap[to].add(1);
        emit Transfer(address(0), to, _totalSupply);
    }

    function mint(address to, uint256 _royalty) public onlyOwner returns (bool) {
        _mint(to, _royalty);
        return true;
    }

    function mintWithURI(address to, string memory url, uint256 _royalty) public onlyOwner returns (bool) {
        _mint(to, _royalty);
        _setTokenURI(_totalSupply, url);
        return true;
    }
    
       function _mintWithURI(address to, string memory url, uint256 _royaltyAmount) internal returns (bool) {
        _mint(to, _royaltyAmount);
        _setTokenURI(_totalSupply, url);
        return true;
    }

    function _batchMint(address to, uint256 count, uint256 _royaltyAmount) internal virtual {
        ...
        _batchMintOwnersMap[_totalBatches] = owners({
            start: _totalSupply,
            end: _totalSupply.add(count),
            owner: msg.sender,
    ...
    function batchMint(address to, uint256 count, uint256 _royaltyAmount) onlyOwner public returns (bool) {
        require(
            count >= MIN_MINT && count <= MAX_MINT,
            "Can only mint between 2 and 2000 tokens"
        );

        _batchMint(to, count, _royaltyAmount);
