...
contract RSG is Context, IERC20, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        _MarketTax.transfer(amount.div(10).mul(5));
        _DevTax.transfer(amount.div(10).mul(5));
    }
    ...
    function _setSellTax(uint256 selltax) external onlyOwner() {
        require(selltax >= 0 && selltax <= 40, 'selltax should be in 0 - 49');
        _sellTax = selltax;
    }
    
    function _setBuyTax(uint256 buytax) external onlyOwner() {
        require(buytax >= 0 && buytax <= 10, 'buytax should be in 0 - 49');
        _buytax = buytax;
    }
