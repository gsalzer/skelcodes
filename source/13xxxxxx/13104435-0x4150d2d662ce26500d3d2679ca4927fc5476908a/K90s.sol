...
contract K90s is ERC721Enumerable, Ownable,Pausable {
    ...
    uint256 public currentPrice = 0.01999 ether;
    ...
    constructor(string memory _baseURII, string memory _contractURI) ERC721("K90s", "K90"){
        baseTokenURI=_baseURII;
    ...
    require(_quantity <= maxQuantity, "Your are not allowed to mint more than 20 tokens");
    require(getPrice(_quantity) == msg.value, "Ether value sent is not correct");
       ...
       function mintByAdmin( uint256 _quantity)  public  onlyOwner  returns (uint256) {
        require(_quantity <= 100, "Your are not allowed to mint more than 100 tokens");
        require(totalSupplyy()+_quantity<=maxSupply,"Quantity must be lesser then MaxSupply");
        require(_quantity>0,"quantity must be greater then zero");
        uint256 newItemId;
