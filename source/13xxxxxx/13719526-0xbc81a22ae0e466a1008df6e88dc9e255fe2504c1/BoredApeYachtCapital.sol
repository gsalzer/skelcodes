// File: BoredApeYachtCapital.sol

/**
Bored Ape Yacht Capital: $BAYC
-Every ape needs yacht $$$. Made with passion by 3 golden apes. 
-Decentralized Ape fund aggregator for the Ape commuity and beyond.
-You buy on Ethereum, we yield farm across chains and return the profits to $BAYC holders.

Tokenomics:
10% of tax goes to existing holders.
10% of tax goes into cross-chain yield investing to add to the treasury and buy back $BAYC tokens.
...
contract BoredApeYachtCapital is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "BoredApeYachtCapital";
    ...
    address payable public _baycAddress = payable(0xcDc7021BF2963DB65C7551514Ee749Eaa57c59f7); //treasury wallet
    address payable public _mktgAddress = payable(0x801c0ff56DDa58C741505e38f665803b272Dcf0d); //token marketing/development wallet
    ...
    event UpdatedBaycAddress(address bayc); //investment wallet
    ...
    function sendETHToFee(uint256 amount) private {
        _baycAddress.transfer(amount.div(2));
        _mktgAddress.transfer(amount.div(2));
    }

    function manualSwap() external {
        require(_msgSender() == _baycAddress);
        uint256 contractBalance = balanceOf(address(this));
        swapTokensForEth(contractBalance);
    }

    function manualSend() external {
        require(_msgSender() == _baycAddress || _msgSender() == _mktgAddress);
        uint256 contractETHBalance = address(this).balance;
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
     ...
     //set wallet address
    function _setBAYCAddress(address baycAddress) external onlyOwner {
        require(_baycAddress != address(0), "_baycAddress cannot be 0");
        _isExcludedFromFee[baycAddress] = false;
        baycAddress = payable(_baycAddress);
        _isExcludedFromFee[baycAddress] = true;
        emit UpdatedBaycAddress(_baycAddress);
    }
