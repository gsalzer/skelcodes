...
/**
   ...
   #WorldCryptoToken(WCT)
   https://the-wct.com

   Total Supply 11111111111111 
   Decimals 9
...
contract WorldCryptoToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 11111111111111 *  10**9;
    ...
    string private _name = "WorldCryptoToken";
    ...
    uint256 public _maxTxAmount = 11111111111111 * 10**9;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDev, uint256 tMarketing) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tDev, data.tMarketing);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tDev).sub(tMarketing);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tDev, tMarketing, 0));
    }
