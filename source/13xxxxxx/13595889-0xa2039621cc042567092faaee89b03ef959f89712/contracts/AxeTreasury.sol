...
contract AxeTreasury is Ownable {
    ...
    address public sAXE;
    uint public sAXEQueue; // Delays change to sAXE address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAXE ).balanceOf( msg.sender ); // Can only borrow against sAXE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AXE
        @param _amount uint
     */
    ...
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AXE decimals
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAXE ) { // 9
