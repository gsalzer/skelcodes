...
/**
 * @title Indivisuals by Takens Theorem contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract IbTT1 is ERC721, Ownable {
    ...
    string private ipfs = 'https://ipfs.io/ipfs/';
    ...
    function renderDescription(uint256 tokenId) private view returns (string memory) {
        return string(abi.encodePacked(nftDescription[tokenId],"\\n\\nToken #",
            Strings.toString(tokenId),"\\n\\n- IPFS CID:\\n- ",ipfsCid[tokenId],"\\n\\n- Owner:\\n- 0x",
            Strings.toAsciiString(ownerOf(tokenId)))); 
    }
    
    function mintNft(string memory _name, string memory _description, string memory _ipfsCid) public onlyOwner {
        require(_tokenIds.current() < MAX_SUPPLY,'ERROR: minting complete');
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        
        nftDescription[newItemId] = _description;
        nftName[newItemId] = _name;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        bytes memory json = abi.encodePacked('{"name":"',nftName[tokenId],
                                            '", "description":"',renderDescription(tokenId),
                                            '", "created_by":"Takens Theorem", "image":"',ipfs,ipfsCid[tokenId],
                                        '"}');
    ...
    constructor() ERC721("Indivisuals by Takens Theorem", "IbTT1") {}    
}
