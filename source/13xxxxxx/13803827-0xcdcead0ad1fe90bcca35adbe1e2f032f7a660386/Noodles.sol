...
contract Noodles is ERC721Enumerable, Ownable {
  ...
  //stage 1: free mint
  mapping(address => uint8) public addressFreeMinted;

  //stage 2: presale mint
  mapping(address => uint8) public addressWLMinted;

  //stage 3: public mint
   mapping(address => uint8) public addressPublicMinted;
  ...
  // public
  function maxMint() public onlyOwner{
    uint256 supply = totalSupply();
    for (uint256 i = 1; i <= 5555 - supply; i++) {
  ...
  function mint(uint8 _mintAmount) public payable {
    ...
    if (mintState ==1){
      //free mint of 1 with 833 spots
          require(supply + _mintAmount <= 836);
          require(isWhitelisted(msg.sender), "You are not whitelisted");
          require(addressFreeMinted[msg.sender] + _mintAmount <= 1, "Max NFTs exceeded");
          addressFreeMinted[msg.sender]++;
 
    }

    else if (mintState ==2){
      //WL mint of 1, 2, or 3 with 1000 addresses whitelisted
          require(supply + _mintAmount <= 3836);
          require(isWhitelisted(msg.sender), "You are not whitelisted");
          require(addressWLMinted[msg.sender] + _mintAmount <= 3, "Max NFTs exceeded");
          addressWLMinted[msg.sender]+= _mintAmount;
    }
