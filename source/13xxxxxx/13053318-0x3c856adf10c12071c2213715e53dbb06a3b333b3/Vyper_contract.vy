...
VERSION: constant(uint256) = 2
PREFIX: constant(Bytes[28]) = b"\x19Ethereum Signed Message:\n32"
# 1000 blocks, roughly 4 hours
DEFAULT_BLOCK_WINDOW: constant(uint256) = 1000
...
def _make_offer(
    ...
    @param wanted_contract Contract address for the wanted token
    @param wanted_token_id Token ID for the wanted token
    @param expires Block number when the offer expires
    ...
    assert expires > block.number, "expires-too-low"
    ...
    self.offers[param_hash].expires = expires
...
def offer_can_complete(
    ...
    return (
        self.offers[param_hash].expires > block.number and
        not self.offers[param_hash].revoked and
...
def accept(
    ...
    assert self.offers[param_hash].signer != empty(address), "offer-does-not-exist"
    assert not self.offers[param_hash].revoked, "offer-revoked"
    assert self.offers[param_hash].expires > block.number, "offer-expired"
