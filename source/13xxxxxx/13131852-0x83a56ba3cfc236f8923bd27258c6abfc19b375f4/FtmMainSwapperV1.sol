...
contract FTMMainLevSwapperV1 {
    ...
    function swap(
        ...
        pair.swap(amountIntermediate, 0, address(FTM_WETH), new bytes(0));

        (uint256 reserve0, uint256 reserve1, ) = FTM_WETH.getReserves();
        
        uint256 amountThird = getAmountOut(amountIntermediate, reserve1, reserve0);

        FTM_WETH.swap(amountThird, 0, address(bentoBox), new bytes(0));

        (, shareReturned) = bentoBox.deposit(FTM, address(bentoBox), recipient, amountThird, 0);
