...
contract AnimeNFT is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal =  1000000000 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "AnimeNFT";
    string private constant _symbol = "ANFT";
    ...
    event minimumTokensBeforeSwapUpdated(uint256 _minBeforeSwap);
    ...
    constructor() payable {
        ...
        minimumTokensBeforeSwap = 500000*1e18; // 0.05% swap tokens amount

        marketingAddress = payable(0xeFe222Bfc476ADc0e91D6F67ef5a48372603c1b1); // Marketing Address

        liquidityAddress = payable(0x3d32b5B625871E8C6caB228515961951e4366D2d); // Liquidity Address (switches to dead address once launch happens)
    ...
    function enableTrading() internal onlyOwner { // Cannot be turned off after launch
        tradingActive = true;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // Normal transfers do not get taxed
                ...
                require(!boughtEarly[from] || earlyBuyPenaltyEnd <= block.timestamp, "Snipers can't transfer tokens until penalty timeframe is over.  DM a Mod.");
                removeAllFee();
        ...
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee <= 12, "Buy taxes shall never be above 12%");
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee <= 24, "Sell fee shall never be above 24%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee);
    }

    function setminTokensBeforeSwap(uint256 _minBeforeSwap) external onlyOwner {
        require(_minBeforeSwap >= 50000 && _minBeforeSwap <= 500000, "Must be between 50k and 500k");
        require(_minBeforeSwap != minimumTokensBeforeSwap, "Same Value");
        minimumTokensBeforeSwap = _minBeforeSwap * 1e18;
        emit minimumTokensBeforeSwapUpdated(_minBeforeSwap);
    }
