...
contract Multiswap {
    ...
    */
        ...
        external returns (uint256[] memory)
    {
        ...
        require(token.approve(address(uniswapRouter), _total), 'Multiswap: Uniswap approval failed');
    ...
    */
    ...
    ) external payable returns (uint256)
    {
        ...
        for (uint i = 0; i < _tokens.length; i++) {
            ...
            require(token.transferFrom(msg.sender, address(this), _amounts[i]), 'Multiswap: Transfer failed');
            uint256 transferAmount = _amounts[i];
            if (transferAmount > token.balanceOf(address(this))) transferAmount = token.balanceOf(address(this));
            require(token.approve(address(uniswapRouter), transferAmount), 'Multiswap: Approval failure');
            uniswapRouter.swapExactTokensForETH(transferAmount, _expected[i], path, address(this), block.timestamp + 1200);
        }
