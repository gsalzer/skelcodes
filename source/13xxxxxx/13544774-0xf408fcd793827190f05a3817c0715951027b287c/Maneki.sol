...
contract Maneki is Context, IERC20, Ownable {
    ...
    uint256 public _transactionsBeforePrize = 3;
    uint256 public _minHoldingForDraw = 1000 * 10**6 * 10**9;
    ...
    string private _name = "Maneki Neko";
    ...
    address payable public _devWalletAddress = payable(0x5cF8C839a456060D6ef750011E2d9Be51598e429);
    ...
    function setDevFeeDisabled(bool _devFeeEnabled ) public returns (bool){
        require(msg.sender == _governer, "Only Governer can disable dev fee");
        swapAndSendEnabled = _devFeeEnabled;
        return(swapAndSendEnabled);
    }
    
    
    /*Left Exposed for Governer to adjust fees as we grow 
    * Can never be higher than starting (12%)
    */
    function setOperationsFee(uint256 opFee ) public {
        require(msg.sender == _governer, "Only Governer can set tax fee");
        require(opFee<13, "Dev tax can not be greater than 12");
        _operationsFee = opFee;
    }
    
    /*Exposing incase token becomes highly valued and we need to change the min amount to hold*/
    function updateMinTokenForPrize(uint256 minHolding) external {
        require(_msgSender() == _governer);
        _minHoldingForDraw = minHolding;
    }
    
     function updateTransCountLimit(uint256 transBeforePrize) external {
        require(_msgSender() == _governer);
        _transactionsBeforePrize = transBeforePrize;
    }
    ...
    function manualswap(uint256 amount) external {
        require(_msgSender() == _governer);
        /*can't swap more than contract balance*/
    ...
    function setMaxTxPercent(uint256 maxTxPercent) public {
        require(msg.sender == _governer, "Only governer can set max transaction");
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateoperationsFee(tAmount);
    ...
    function swapAndSend(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 prizeBalance = newBalance.div(4);
        uint256 devBalance = newBalance.sub(prizeBalance);
        sendETHToDev(devBalance);
        
        /*after dev fee is taken send the remaining balance to winner
          this lets eth value build up in contract if no winner and lets
          the pot grows
          */
        if(_transactionCount > _transactionsBeforePrize) {
            sendToWinner(address(this).balance);
        }
    ...
    /*Draw randomly from list of buyers
    * if the selected has enough tokens send them prize and reset the count
    * otherwise pot will continue to grow and address is removed
    */
    function sendToWinner(uint256 amount) private {
       uint256 randomPosition = rand(buyers.length());
       winner = payable(buyers.at(randomPosition));
       if(balanceOf(winner)>_minHoldingForDraw){
