...
contract BearX is ERC721Enumerable, Ownable {
    ...
    BearToken public TokenContract;
    bool public breedStart = false;
    uint256 public breedPrice = 1000 ether;

    constructor(string memory baseURI, address _fundWallet) ERC721("BearX", "BearX")  {
        require(_fundWallet != address(0), "Zero address error");
        setBaseURI(baseURI);
        fundWallet = _fundWallet;
    }
    
    function setBearToken (BearToken _TokenContract) public onlyOwner{
        // require(_TokenContract != address(0), "Contract address can't be zero address");
        TokenContract = _TokenContract;
    }
    
    function getBearToken(address __address) public view returns(uint256) {
        require(__address != address(0), "Contract address can't be zero address");
        return TokenContract.balanceOf(__address);
    }
    
    function getBearAllowance(address __address) public view returns(uint256) {
        require(__address != address(0), "Contract address can't be zero address");
    ...
    function MINT(uint256 num) public payable {
        ...
        require( num <= _maxperTX,                               "You are exceeding limit of per transaction BEARX" );
        require( supply + num <= _limit - _reserved,             "Exceeds maximum BEARX supply" );
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        ...
        require( _amount <= _reserved, "Exceeds reserved bear supply");
    ...
    function WHITELIST_MINT(uint256 num) public payable {
        ...
        require( perWallet[msg.sender] + num <= _maxperWalletWhiteList,   "You are exceeding limit of per wallet BEARX" );
        require( supply + num <= _limit - _reserved,             "Exceeds maximum BEARX supply" );
        require( msg.value >= _price * num,                      "Ether sent is not correct" );
