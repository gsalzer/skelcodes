...
contract MoassAsses is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE_PER_MA = 5 * 10**16; // 0.05ETH Per MA
    uint256 private constant PRICE_PREMINT = 0; // Free Per MA

    uint256 private constant MAX_ELEMENTS_CROWDSALE = 10000 ; // 10000 MAs in CrowdSale
    uint256 private constant MAX_ELEMENTS_PRESALE = 5 * 10**2; // 500 MAs in PreSale
    uint256 private constant MAX_ELEMENTS_PREMINT = 50; // 30 MAs for GiveAway.
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= maxSalesAmount, "MINT: Please go to the Opensea to buy MAs.");
        require(_ids.length <= maxMintAmount, "MINT: Current count exceeds maximum mint count.");
        require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of MAs");
    ...
    function startPreSale() public onlyOwner {
        maxSalesAmount = MAX_ELEMENTS_PRESALE + MAX_ELEMENTS_PREMINT;
    ...
    function price(uint256 _count) public pure returns (uint256) {
        return PRICE_PER_MA.mul(_count);
    }
