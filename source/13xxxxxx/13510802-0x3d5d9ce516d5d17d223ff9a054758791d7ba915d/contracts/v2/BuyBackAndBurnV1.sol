...
/**
 * @title BuyBackAndBurnV1
 * This contract collects cross-chain bridge fees and orchestrates token buybacks and burns
 */
contract BuyBackAndBurnV1 is MyPausableUpgradeable, IBuyBackAndBurn {
  ...
  function buyBackAndBurnERC20(address collectedToken) external whenNotPaused nonReentrant {
    ...
    ERC20Burnable(address(burnToken)).burn(burnAmount);
    ...
    emit BoughtBackAndBurned(collectedToken, amount, address(burnToken), burnAmount);
  }
  ...
  function buyBackAndBurnNative() external whenNotPaused nonReentrant {
    ...
    emit BoughtBackAndBurned(address(0), amount, address(burnToken), burnAmount);
  }
