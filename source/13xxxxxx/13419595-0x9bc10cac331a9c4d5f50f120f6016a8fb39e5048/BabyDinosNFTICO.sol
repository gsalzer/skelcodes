...
contract Crowdsale is Context, ReentrancyGuard  {
    ...
    // Private Sale price is 0.03ETH
    uint256 internal _rate = 30000000000000000;
...
abstract contract TimedCrowdsale is Crowdsale {
    ...
    uint256 internal _breakTime = 86400; // 24hrs
     ...
     * @return the crowdsale opening time.
     */
    function breakTime() public view virtual returns (uint256) {
        return _breakTime;
    }
    ...
    function isBreak() public view virtual returns (bool){
        // solhint-disable-next-line not-rely-on-time
        return block.timestamp >= _openingTime + _breakTime;
    }
    ...
    WhitelistedRole
{
    ...
    function _preValidatePurchase(
        ...
        if (stage == CrowdsaleStage.PreICO) {
            require(
                ...
                "BabyDinosNFTICO: Wallet Address is not WhiteListed"
            );
        }
    ...
    function _updatePurchasingState(address _beneficiary, uint256 _weiAmount)
        ...
        override
    {    
        if ( isBreak() && stage == CrowdsaleStage.PreICO) {
            stage = CrowdsaleStage.ICO;
            _openingTime = secondaryTime();
            _rate = 50000000000000000;                  // 0.05ETH
        }
