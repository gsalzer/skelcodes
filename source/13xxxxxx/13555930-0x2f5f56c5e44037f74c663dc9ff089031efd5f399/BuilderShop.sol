...
contract BuilderShop {
   ...
   //FitusNFT registry is hard coded
   address fitusNftRegistryContract = 0x4d04Bebe0CE168F710594A7eeA529993F18Bb33d;

   modifier onlyValidSender() {
       FitusNFTRegistry nftg_registry = FitusNFTRegistry(fitusNftRegistryContract);
       bool is_valid = nftg_registry.isValidFitusNFTSender(msg.sender);
   ...
   function isValidBuilderShop(address builder_shop) public view returns (bool isValid) {
       //public function, allowing anyone to check if a contract address is a valid FitusNFT contract
   ...
   function createNewBuilderInstance(
       ...
       uint num_nifties,
       uint[] memory fitusNft_quantities,
       string memory token_base_uri,
       string memory creator_name)
   public onlyValidSender returns (FitusNFTBuilderInstance tokenAddress) {

       contractId = contractId + 1;

       FitusNFTBuilderInstance new_contract = new FitusNFTBuilderInstance(
           _name,
           _symbol,
           contractId,
           num_nifties,
...
contract FitusNFTBuilderInstance is ERC721Full {
...
//   string public creatorName;
   ...
   //FitusNFT registry contract
   address public fitusNftRegistryContract = 0x4d04Bebe0CE168F710594A7eeA529993F18Bb33d;

   //master builder - ONLY DOES STATIC CALLS
   address public masterBuilderContract = 0x7EB520f2727747910C544dCBeC4C12D7f3890c54;
   ...
   mapping (uint => Counters.Counter) public _numFitusNFTMinted;
   mapping (uint => uint) public _numFitusNFTPermitted;
   mapping (uint => uint) public _fitusNftPrice;
   mapping (uint => string) public _fitusNftIPFSHashes;
   mapping (uint => bool) public _IPFSHashHasBeenSet;

   //EVENTS

   //purchase + creation events
   event FitusNFTPurchased(address _buyer, uint256 _amount, uint _tokenId);
   event FitusNFTCreated(address new_owner, uint _fitusNftType, uint _tokenId);
   ...
   constructor(
       ...
       //offset starts at 1 - there is no fitusNftType of 0
       for (uint i=0; i<(num_nifties); i++) {
           _numFitusNFTPermitted[i+1] = fitusNft_quantities[i];
       }
   }
   
   function setFitusNFTIPFSHash(uint fitusNftType, 
                            string memory ipfs_hash) onlyValidSender public {
        //can only be set once
        if (_IPFSHashHasBeenSet[fitusNftType] == true) {
            revert("Can only be set once");
        } else {
            _fitusNftIPFSHashes[fitusNftType] = ipfs_hash;
            _IPFSHashHasBeenSet[fitusNftType]  = true;
        }
    }

   function isFitusNFTSoldOut(uint fitusNftType) public view returns (bool) {
       if (fitusNftType > numNiftiesCurrentlyInContract) {
           return true;
       }
       if (_numFitusNFTMinted[fitusNftType].current() > _numFitusNFTPermitted[fitusNftType]) {
           return (true);
       } else {
           return (false);
       }
   ...
   function giftFitusNFT(address collector_address, 
                      uint fitusNftType) onlyValidSender public {
       //master for static calls
       BuilderMaster bm = BuilderMaster(masterBuilderContract);
       _numFitusNFTMinted[fitusNftType].increment();
       //check if this FitusNFT is sold out
       if (isFitusNFTSoldOut(fitusNftType)==true) {
           revert("Fitus NFT sold out!");
       }
       //mint a FitusNFT
       uint specificTokenId = _numFitusNFTMinted[fitusNftType].current();
       uint tokenId = bm.encodeTokenId(contractId, fitusNftType, specificTokenId);
       string memory tokenIdStr = bm.uint2str(tokenId);
       string memory tokenURI = bm.strConcat(baseURI, tokenIdStr);
       string memory ipfsHash = _fitusNftIPFSHashes[fitusNftType];
       //mint token
       _mint(collector_address, tokenId);
       _setTokenURI(tokenId, tokenURI);
       _setTokenIPFSHash(tokenId, ipfsHash);
       //do events
       emit FitusNFTCreated(collector_address, fitusNftType, tokenId);
   }

}

contract FitusNFTRegistry {
   function isValidFitusNFTSender(address sending_key) public view returns (bool);
...
contract BuilderMaster {
   ...
   function encodeTokenId(uint contractId, uint fitusNftType, uint specificFitusNFTNum) public view returns (uint);
...
/**
* Contracts and libraries below are from OpenZeppelin, except FitusNFT builder instance
**/
