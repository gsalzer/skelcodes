...
// File contracts/og_otter_organization.sol

pragma solidity ^0.8.0;

contract OG_OTTER_ORGANIZATION is ERC721Enumerable, Ownable {
    uint256 public constant TOTAL_MAX_OTTERS = 10000;
    uint256 public constant PRESALE_MAX_OTTERS = 1500;
    uint256 public constant RESERVED_OTTERS = 100;
    uint256 public publicAmountMinted = 0;
    uint256 public privateAmountMinted = 0;
    uint256 public reservedAmountMinted = 0;


    bool private _paused = false;
    uint256 private _presalePurchaseLimit = 3;
    uint256 private _maxPerTx = 10; // 10 Otters max mint
    uint256 private _price = 70000000000000000; // .07 ETH
    uint256 private _saleTime = 1635609600; // Saturday, October 30th @ NOON EST, 9AM PST 
    // Presale Time: 1635436800 - Thursday, October 28th @ NOON EST, 9AM PST 

    // Faciliating the needed functionality for the presale
    mapping(address => bool) addressToPreSaleEntry;
    mapping(address => uint256) addressToPreSaleOttersMinted;

    string private _baseTokenURI;

    constructor(string memory baseURI, address[] memory preSaleWalletAddresses)
        ERC721("OG Otter Organization", "OG")
    {
        setBaseURI(baseURI);
    ...
    modifier preSaleIsOpen() {
        require(_paused == false, "Presale Paused");
        require(
            block.timestamp >= _saleTime - 172800,
    ...
    modifier saleIsOpen() {
        require(_paused == false, "Sale Paused");
    ...
    function setPresaleMaxRate(uint256 _val) public onlyOwner {
        _presalePurchaseLimit = _val;
    }

    function getPresaleMaxRate() public view returns (uint256) {
        return _presalePurchaseLimit;
    }
    ...
    function setPaused(bool _newPauseBoolValue) public onlyOwner {
        _paused = _newPauseBoolValue;
    }
    ...
    function addToPresaleList(address[] calldata entries) external onlyOwner {
        for(uint256 i = 0; i < entries.length; i++) {
            ...
            require(!addressToPreSaleEntry[entry], "DUPLICATE_ENTRY");

            addressToPreSaleEntry[entry] = true;
        }   
    }
    ...
    function removeFromPresaleList(address[] calldata entries) external onlyOwner {
        for(uint256 i = 0; i < entries.length; i++) {
            ...
            addressToPreSaleEntry[entry] = false;
        }
    }
    ...
    function preSaleOttersMinted(address _address)
        public
        view
        returns (uint256)
    {
        return addressToPreSaleOttersMinted[_address];
    }

    function preSaleMint(uint256 _count) external payable preSaleIsOpen {
        uint256 totalSupply = totalSupply();
        require(
          totalSupply < TOTAL_MAX_OTTERS, 
          "All Otters are already minted."
          );
        require(
            totalSupply + _count <= TOTAL_MAX_OTTERS,
            "This amount of Otters will exceed total supply."
        );
        require(
            privateAmountMinted + _count <= PRESALE_MAX_OTTERS,
            "This amount of Otters will exceed presale supply."
        );
        require(
            _count <= _presalePurchaseLimit,
        ...
        require(
            addressToPreSaleOttersMinted[msg.sender] + _count <=
                _presalePurchaseLimit,
            "Exceeds supply of presale Otters you can mint."
        );
    ...
    function mint(uint256 _count) external payable saleIsOpen {
        ...
        require(
            _count <= _maxPerTx,
            "Woah, You Otter slow down! That's too many Otters to mint!"
        );
        require(totalSupply < TOTAL_MAX_OTTERS, "All Otters are already minted.");
        require(
            totalSupply + _count <= TOTAL_MAX_OTTERS,
            "This amount of Otters will exceed max supply."
        );
        require(_price * _count <= msg.value, "Transaction value too low.");
    ...
    function reserveOtters(uint256 _count) external onlyOwner {
        uint256 totalSupply = totalSupply();
        require(totalSupply + _count <= TOTAL_MAX_OTTERS, "Beyond total otter limit");
        require(reservedAmountMinted + _count <= RESERVED_OTTERS, "Beyond reserved otter limit");
