...
contract LEGION is Context, IERC20, Ownable {
    ...
    address payable public tradingWallet = payable(0x350E32f045CC6fB4f928377675d73D3C0E244912);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _tradingFee==0 && _burnFee==0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_tradingFee); // trading + liquidity fees
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees); 
        
        uint256 tokensFortrading = contractTokenBalance.sub(tokensForLiquidity);
        ...
        swapTokensForEth(tokensFortrading);
        uint256 transferredBalance = address(this).balance.sub(contractBalance);
        
        //Send to trading address
        transferToAddressETH(tradingWallet, transferredBalance);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private 
    {
        ...
        (tTransferAmount, rTransferAmount) = taketrading(sender, tTransferAmount, rTransferAmount, tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
    ...
    function taketrading(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
        if(_tradingFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 ttrading = tAmount.div(100).mul(_tradingFee);
        uint256 rtrading = ttrading.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rtrading);
        tTransferAmount = tTransferAmount.sub(ttrading);
        _rOwned[address(this)] = _rOwned[address(this)].add(rtrading);
        emit Transfer(sender, address(this), ttrading);
        return(tTransferAmount, rTransferAmount); 
    }
    ...
    function settradingWallet(address payable newWallet) external onlyOwner() {
        tradingWallet = newWallet;
    }
