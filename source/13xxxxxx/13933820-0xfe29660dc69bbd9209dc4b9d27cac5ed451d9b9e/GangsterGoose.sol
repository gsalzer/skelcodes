...
contract GangsterGoose is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  event gooseNameChange(address _by, uint256 _tokenId, string _name);
  ...
  // add ETH addresses to whitelist
  function addWhitelistAddresses(uint8 numberOfTokens, address[] calldata addresses) external onlyOwner {
    require(!_privateSale, "Private sale is active now!");
    require(numberOfTokens <= maxMintAmount, "numberOfTokens is higher than max mint amount!");
  ...
  function privateMint(uint8 numberOfTokens) public payable {
    ...
    require(totalSupply() + numberOfTokens <= maxSupply, "Total Supply limit have reached!");
    require(numberOfTokens <= _whiteList[msg.sender], "Not enough presale slots to mint tokens!");
    require(msg.value >= whitelistSalePrice * numberOfTokens, "Ether value sent is not correct!");

    _whiteList[msg.sender] = uint8(_whiteList[msg.sender] - numberOfTokens);
    _mintTokens(msg.sender, numberOfTokens);

    emit addressPrivateSlotsChange(msg.sender, _whiteList[msg.sender]);
  }
  
  function publicMint(uint8 numberOfTokens) public payable {
    require(_publicSale, "Public sale is not active!");
    require(numberOfTokens > 0, "Number of tokens cannot be lower than, or equal to 0!");
    require(numberOfTokens <= maxSupply, "Trying to mint too many tokens!");
    require(totalSupply() + numberOfTokens <= maxSupply, "Total Supply limit have reached!");
    require(msg.value >= publicSalePrice * numberOfTokens, "Ether value sent is not correct!");

    _mintTokens(msg.sender, numberOfTokens);
  }

  function giveawayMint(address to, uint8 numberOfTokens) public onlyOwner {
    require(numberOfTokens > 0, "Number of tokens cannot be lower than, or equal to 0!");
    require(totalSupply() + numberOfTokens <= maxSupply, "Total Supply limit have reached!");
  ...
  function changeGooseName(uint256 _tokenId, string memory _name) public {
  ...
  function viewGooseName(uint256 _tokenId) public view returns( string memory ){
    require( _tokenId < totalSupply(), "Choose a goose within range" );
    return gooseNames[_tokenId];
  }
  
  // GET ALL GOOSES OF A WALLET AS AN ARRAY OF STRINGS
  function gooseNamesOfOwner(address _owner)
    public
    view
    returns ( string[] memory )
  {
    uint256 ownerTokenCount = balanceOf(_owner);
    string[] memory tokenNames = new string[](ownerTokenCount);
    for (uint256 i; i < ownerTokenCount; i++) {
      tokenNames[i] = gooseNames[tokenOfOwnerByIndex(_owner, i)];
    }
    return tokenNames;
  }
}
