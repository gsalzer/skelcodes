...
contract Token is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(
        string memory name_, string memory symbol_, string memory baseURI_, uint256 limitSupply_,
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        require(totalSupply() + tokensAmount <= limitSupply(), "DA: Minting would exceed max supply");
        if (presaleActive) {
