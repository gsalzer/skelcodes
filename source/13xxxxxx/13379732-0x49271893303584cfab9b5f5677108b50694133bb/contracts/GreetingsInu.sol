...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, uint256 closesAt_, uint256 opensAt_) {
        _name = name_;
        _symbol = symbol_;
    ...
    function transferFrom(
        ...
        if(!_enabled) {
            ...
            if(open()) {
                require(recipient != afterHours && sender != afterHours, "market is open");
            } else {
                require(recipient == afterHours || sender == afterHours, "after hours is open");
            }
        }
    ...
    function setOpen(uint256 closesAt_, uint256 opensAt_) public onlyOwner {
        if(_enabled) {
            if(closesAt_ > opensAt_) {
                require(closesAt_ - opensAt_ >= 8 hours, "must at least be open for 8 hours");
            } else {
                require(opensAt_ - closesAt_ >= 8 hours, "must at least be open for 8 hours");
            } 
        }
        closesAt = closesAt_;
        opensAt = opensAt_;
