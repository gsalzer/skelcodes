...
contract GamerMonkeys is ERC721, Ownable {
    ...
    uint256 public currentPrice = 0.025 ether;
    uint256 public totalGamerMonkeys = 5555;
    uint256 public maxFreeGamerMonkeys = 555;
    bool public paused = true;
    bool public revealed = false;

    constructor(string memory _initBaseURI, string memory _preRevealURI)
        ERC721("Gamer Monkeys", "GM")
    {
    ...
    function freeGamerMonkey(uint256 mintAmount) public {
        uint256 supply = tokenIdCounter.current();
        require(!paused, "Sale paused");
        require(supply + mintAmount < maxFreeGamerMonkeys, "No more free Gamer Monkeys left");
        mintGamerMonkeys(msg.sender, mintAmount);
    }

    function mint(uint256 mintAmount) external payable {
        uint256 supply = tokenIdCounter.current();
        require(!paused, "Sale paused");
        require(mintAmount < 21, "You can only mint 20 Gamer Monkeys");
        require(
            supply + mintAmount < totalGamerMonkeys + 1,
            "Exceeds maximum Gamer Monkey supply"
        );
        require(
    ...
    function mintGamerMonkeys(address addr, uint256 mintAmount) private {
