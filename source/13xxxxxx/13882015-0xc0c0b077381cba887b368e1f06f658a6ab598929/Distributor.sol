...
contract Distributor is Ownable {
  IDinoEggs private dinoEggsContract;

  address private _verifier = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266;

  mapping(address => uint256) public claimedEggs;

  constructor(address _dinoEggsContract) {
    dinoEggsContract = IDinoEggs(_dinoEggsContract);
  }
  ...
  function claim(uint256 _amount, bytes calldata _signature) external {
    ...
    require(claimedEggs[_msgSender()] < _amount, "Invalid eggs amount");

    address signer = _recoverWallet(_msgSender(), _amount, _signature);

    require(signer == _verifier, "Unverified transaction");

    uint256 claimAmount = _amount - claimedEggs[_msgSender()];

    claimedEggs[_msgSender()] = _amount;
    dinoEggsContract.withdraw(_msgSender(), claimAmount);
  }
