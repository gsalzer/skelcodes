    ...
    UUPSUpgradeable
{
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < _ticket.key.length; i++) {
            require(!minted[_ticket.key[i]][_ticket.tokenId], "Already minted");
        }
        ...
        for (uint256 i = 0; i < _ticket.key.length; i++) {
            minted[_ticket.key[i]][_ticket.tokenId] = true;
        }
        ...
        returns (bytes32)
    {
        bytes memory key;
        for (uint256 i = 0; i < _ticket.key.length; i++) {
            key = bytes.concat(key, keccak256(_ticket.key[i]));
        }
