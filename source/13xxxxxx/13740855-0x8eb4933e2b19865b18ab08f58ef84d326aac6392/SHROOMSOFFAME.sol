...
contract SHROOMSOFFAME is ERC721Enumerable, Ownable {
    using Strings for uint;

    uint public SHROOMS_MAX_COUNT =  7000;
    uint public SHROOMS_MAINSALE_PRICE = 0.035 ether;
    mapping (address => uint) public saleListPurchase;

    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmX93kf1C1WcwKxvZA8Dq5wqWifqvW1MBvM8h6ykNX5G8q/";
    ...
    constructor() ERC721("SHROOMS", "SHROOMIE") {}

    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= SHROOMS_MAX_COUNT, "EXCEED_MAX");
    ...
    function setPrice(uint newPrice) external onlyOwner {
        SHROOMS_MAINSALE_PRICE = newPrice;
    }


    function setMaxSuuply(uint newValue) external onlyOwner {
        SHROOMS_MAX_COUNT = newValue;
    }


    function buy(uint tokenQuantity) external payable {
        require (saleLive, "SALE_CLOSED");
        require (totalSupply() + tokenQuantity <= SHROOMS_MAX_COUNT, "EXCEED MAX");
        require (SHROOMS_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
