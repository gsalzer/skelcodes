...
contract SlothsNFT is ERC721, ERC721Enumerable,ERC721URIStorage, Ownable {
    ...
    uint256 public _startDate = 1627416000;
    string private _metadata =  "https://ipfs.io/ipfs/QmZ1WPnJEn1u3GfiP5zg8UBBe1f22RAYBuahEqF1qKCyYA";
    string private _slothMetadata = "https://ipfs.io/ipfs/QmYpQDwcKyUrV4fAUSwV4TZKt6kSmjhVusvCXaGbN7PsLT";

    constructor() ERC721("SlothsNFT", "SLO") {
    }
    ...
    function slothMetadata() public view returns (string memory) {
        return _slothMetadata;
    }
    
    function setSlothMetadata(string memory slothMetadata) public onlyOwner {
        _slothMetadata = slothMetadata;
    } 
    ...
    function setContractMetadata(string memory metadata) public onlyOwner {
        _metadata = metadata;
    }
    ...
    function adminMint(uint8 _quantityToMint) public onlyOwner{
        ...
        for (uint8 i = 0; i < _quantityToMint; i++) {
            ...
            _setTokenURI(newItemId, _slothMetadata);
        }
    }
    ...
    function slothsToAwake(address owner) public view returns(uint256) {
        uint256 _sloths = 0;
        
        for(uint256 i=0;i < balanceOf(owner);i++) {
            if((keccak256(abi.encodePacked((tokenURI(tokenOfOwnerByIndex(owner, i))))) == keccak256(abi.encodePacked((_slothMetadata))))) {
                _sloths = _sloths + 1;
            }
        }
    ...
    function slothToAwake(address owner, uint256 index) public view returns(uint256) {
        require(slothsToAwake(owner) > 0, 'No sloths available');
        for(uint256 i=0;i < balanceOf(owner);i++) {
            if((keccak256(abi.encodePacked((tokenURI(tokenOfOwnerByIndex(owner, i))))) == keccak256(abi.encodePacked((_slothMetadata))))) {
                if(i == index) {
    ...
    function awakeSloth(uint256 tokenId, string memory tokenUri) public {
        require((keccak256(abi.encodePacked((tokenUri))) != keccak256(abi.encodePacked(('')))), 'TokenUri is empty');
        require(ownerOf(tokenId) == msg.sender, 'You dont own this slot');
        require((keccak256(abi.encodePacked((tokenURI(tokenId)))) == keccak256(abi.encodePacked((_slothMetadata)))), 'Sloth already awaken!');
        _setTokenURI(tokenId, tokenUri);
    }
    
    function awakeAllSloths(uint256[] memory _tokenIds, string[] memory _tokenURIs) public {
        require(_tokenIds.length == _tokenURIs.length, 'Arrays doesnt have the same length');
        for(uint256 i=0;i<_tokenIds.length;i++) {
            if(ownerOf(_tokenIds[i]) == msg.sender && 
            (keccak256(abi.encodePacked((tokenURI(_tokenIds[i])))) == keccak256(abi.encodePacked((_slothMetadata))))) {
                awakeSloth(_tokenIds[i], _tokenURIs[i]);
            }
        }
    }
