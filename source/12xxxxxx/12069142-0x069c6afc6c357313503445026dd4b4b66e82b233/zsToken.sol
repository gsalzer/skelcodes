...
contract zsToken is ERC20("Stabilize Strategy Emerging USD Tokens", "zs-XUSD"), Ownable, ReentrancyGuard {
    ...
    function underlyingDepositAssets() public view returns (address, address, address, address, address, address) {
        // Returns all addresses accepted by this token vault
        return (address(tokenList[0].token), 
                address(tokenList[1].token), 
                address(tokenList[2].token),
                address(tokenList[3].token),
                address(tokenList[4].token),
                address(tokenList[5].token));
    }
