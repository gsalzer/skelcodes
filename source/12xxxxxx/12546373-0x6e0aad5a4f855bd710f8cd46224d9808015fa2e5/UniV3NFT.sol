...
// File: contracts/interfaces/uniswap-v3/UniV3Helper.sol
...
contract UniV3NFT is Ownable{
  ...
  function approve(address token,uint256 amount)external onlyOwner{
    IERC20(token).safeApprove(positionManager,amount);
  }
  ...
  function mint(INonfungiblePositionManager.MintParams calldata params,uint8 count)public{
    ...
    bool has=false;
    uint256 tokenId;
    for(uint8 i=1;i<count&&!has;i++){
      (tokenId,,,)=manager.mint(params);
      has=isRare(tokenId,pool);
    }
    require(has==true,'not has rate token');
  ...
  function isRare(uint256 tokenId, address poolAddress) public pure returns (bool) {
    ...
    return uint256(h) < type(uint256).max / (1 + UniV3Helper.mostSignificantBit(tokenId) * 2);
  }

  function withdraw(address to,uint256 tokenId)public onlyOwner{
    IERC721(positionManager).safeTransferFrom(address(this),to,tokenId);
  }
