...
// File contracts/interfaces/IWETHVaultAdapterV2.sol

pragma solidity ^0.6.12;

/// Interface for all Vault Adapter implementations.
interface IWETHVaultAdapterV2 {
...
// File contracts/libraries/alchemist/WETHVaultV2.sol
...
library WETHVaultV2 {
  using WETHVaultV2 for Data;
  using WETHVaultV2 for List;
  using SafeMath for uint256;

  struct Data {
    IWETHVaultAdapterV2 adapter;
    uint256 totalDeposited;
  }
  ...
  function token(Data storage _self) internal view returns (IWETH) {
    return IWETH(_self.adapter.token());
  }
...
// File contracts/YumEthVesperVault.sol
...
contract YumEthVesperVault is ReentrancyGuard {
  using CDP for CDP.Data;
  using FixedPointMath for FixedPointMath.uq192x64;
  using WETHVaultV2 for WETHVaultV2.Data;
  using WETHVaultV2 for WETHVaultV2.List;
  ...
  event ActiveVaultUpdated(
    IWETHVaultAdapterV2 indexed adapter
  );
  ...
  WETHVaultV2.List private _vaults;
  ...
  constructor(
    IWETH _weth,
    IMintableERC20 _waEth,
    address _governance,
    address _sentinel
  )
    public
  {
    require(_governance != ZERO_ADDRESS, "YumEthVesperVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumEthVesperVault: sentinel address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumEthVesperVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumEthVesperVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumEthVesperVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumEthVesperVault: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumEthVesperVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumEthVesperVault: collateralization limit above maximum.");
  ...
  function initialize(IWETHVaultAdapterV2 _adapter) external onlyGov {

    require(!initialized, "YumEthVesperVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumEthVesperVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumEthVesperVault: cannot initialize rewards address to 0x0");
  ...
  function migrate(IWETHVaultAdapterV2 _adapter) external expectInitialized onlyGov {
  ...
  function harvest(uint256 _vaultId) external expectInitialized returns (uint256, uint256) {
    ...
    WETHVaultV2.Data storage _vault = _vaults.get(_vaultId);
    ...
    if (_harvestedAmount > 0) {
      ...
      if (_feeAmount > 0) {
        weth.transfer(rewards, _feeAmount);
      }
  ...
  function flushActiveVault() internal returns (uint256) {
    ...
    WETHVaultV2.Data storage _activeVault = _vaults.last();
  ...
  /// @dev Wraps ETH in WETH and deposits into a CDP
  /// 
  /// This function reverts if an emergency exit is active. This is in place to prevent the potential loss of
  /// additional funds.
  function deposit() public payable nonReentrant noContractAllowed expectInitialized {
      require(!emergencyExit, "emergency pause enabled");
  ...
  /// @dev Handle deposit logic
  ///
  /// @param _amount the amount of the WETH to deposit.
  /// @param user the user's address
  function _deposit(uint256 _amount, address user) internal {
    CDP.Data storage _cdp = _cdps[user];
    _cdp.update(_ctx);
    ...
    emit TokensDeposited(user, _amount);
  }
  ...
  /// @dev Wraps ETH in WETH and repays debt with it
  function repay() external payable nonReentrant noContractAllowed onLinkCheck expectInitialized {
    CDP.Data storage _cdp = _cdps[msg.sender];
    _cdp.update(_ctx);

    // Wraps ETH in WETH
    weth.deposit{value: msg.value}();
    _distributeToTransmuter(msg.value);

    uint256 _totalAmount = msg.value;
    _cdp.totalDebt = _cdp.totalDebt.sub(_totalAmount, "");

    emit TokensRepaid(msg.sender, msg.value, 0);
  }
  ...
  function repay(uint256 _parentAmount, uint256 _childAmount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_parentAmount > 0) {
      weth.transferFrom(msg.sender, address(this), _parentAmount);
      _distributeToTransmuter(_parentAmount);
    }

    if (_childAmount > 0) {
      waEth.burnFrom(msg.sender, _childAmount);
      //lower debt cause burn
      waEth.lowerHasMinted(_childAmount);
    }
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumEthVesperVault: Loan-to-value ratio breached");
    } else {
      _cdp.totalCredit = _totalCredit.sub(_amount);
    }

    waEth.mint(msg.sender, _amount);
  }
  ...
  function getVaultAdapter(uint256 _vaultId) external view returns (IWETHVaultAdapterV2) {
    WETHVaultV2.Data storage _vault = _vaults.get(_vaultId);
  ...
  modifier expectInitialized() {
    require(initialized, "YumEthVesperVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumEthVesperVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IWETHVaultAdapterV2 _adapter) internal {
    require(_adapter != IWETHVaultAdapterV2(ZERO_ADDRESS), "YumEthVesperVault: active vault address cannot be 0x0.");
    require(_adapter.token() == weth, "YumEthVesperVault: token mismatch.");

    _vaults.push(WETHVaultV2.Data({
      adapter: _adapter,
      totalDeposited: 0
    }));
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumEthVesperVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
  ...
  function _withdrawFundsTo(address _recipient, uint256 _amount) internal returns (uint256, uint256) {
    ...
    uint256 _bufferedAmount = Math.min(_amount, weth.balanceOf(address(this)));

    if (_recipient != address(this)) {
      weth.transfer(_recipient, _bufferedAmount);
    }
