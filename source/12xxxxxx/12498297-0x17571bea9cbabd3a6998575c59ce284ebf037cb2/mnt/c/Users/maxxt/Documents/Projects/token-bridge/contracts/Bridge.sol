...
contract Bridge is ProxyStorage, BridgeStorageV1 {
    ...
    function initialize(address _courier, address _guardian, address _bridgeToken, uint _fee, uint[] memory newRoutes) public {
        ...
        require(_courier != address(0), "Bridge: courier address is 0");
        _setCourier(_courier);

        require(_guardian != address(0), "Bridge: guardian address is 0");
        _setGuardian(_guardian);

        require(_bridgeToken != address(0), "Bridge: bridgeToken address is 0");
    ...
    function cross(uint chainId, address to, uint amount) public returns (bool) {
        require(amount > fee, "Bridge: amount must be more than fee");
        require(to != address(0), "Bridge: to address is 0");
        require(checkRoute(chainId), "Bridge: chainId is not support");
    ...
    function deliver(uint fromChainId, address to, uint amount, uint nonce) public returns (bool) {
        require(msg.sender == courier, 'Bridge: Only courier can deliver tokens');
        require(amount > 0, "Bridge: amount must be positive");
        require(to != address(0), "Bridge: to address is 0");
        require(!deliverNonces[fromChainId][nonce], "Bridge: bad nonce");
    ...
    function _setCourier(address newCourier) public returns (bool) {
        // Check caller = admin
        require(msg.sender == admin, 'Bridge: Only admin can set courier');
    ...
    function _setGuardian(address newGuadrdian) public returns (bool) {
        // Check caller = admin
        require(msg.sender == admin, 'Bridge: Only admin can set guardian');
    ...
    function unsetCourier() public returns (bool) {
        ...
        require(msg.sender == guardian, 'Bridge: Only guardian can unset courier');
    ...
    function _setFee(uint newFee) public returns (bool) {
        // Check caller = admin
        require(msg.sender == admin, 'Bridge: Only admin can set fee');
    ...
    function _setRoutes(uint[] memory newRoutes) public {
        // Check caller = admin
        require(msg.sender == admin, 'Bridge: Only admin can set routes');
