...
contract DIREBEAR is Ownable, ERC721 {
    ...
    uint256 public constant SALE_DAYS = 10 days;
    ...
    constructor(address _bearWallet) ERC721("Dire Bear","DBEAR")  {
        bearWallet = payable(_bearWallet);
    ...
    function genrateBearTokenToOwner(uint256 _numberOfBear) external onlyOwner returns(bool){
        require(block.timestamp > SALE_END_DAY,"sale not ended");
        for(uint i=0;i < _numberOfBear;i++){
            _genrateBearToken(); 
        }
        return true;
    }
