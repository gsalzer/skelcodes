...
abstract contract Percentage {
    ...
    function onePercent(uint256 _value, uint256 _basePercent) public pure returns (uint256)  {
        ...
        uint256 _onePercent = SafeMath.div(SafeMath.mul(roundValue, basePercent), 10000);
        return _onePercent;
    }
}
...
abstract contract ERC20 is ClaimableOwnable, Context, IERC20  {
    ...
    function approveDai(uint256 _amount) public returns(bool) {
        ...
        return IERC20(DAI_ADDRESS).approve(address(this), _amount);

    }
    
    function allowanceOfDai() public view returns (uint256){
       return IERC20(DAI_ADDRESS).allowance(address(this),msg.sender);
    }
    ...
    function senderDaiBalance() public view returns (uint256){
         return IERC20(DAI_ADDRESS).balanceOf(msg.sender); 
    }
    ...
    function buy(uint256 amount) public {
        ...
        if(IERC20(DAI_ADDRESS).transferFrom(msg.sender, address(this), amount) == true){
            ...
            uint256 _fee = SafeMath.wdiv((SafeMath.wmul(amount,FEE)),10000);
    ...
    function getEstimatedDAIforUSDF(uint256 usdfAmount) public view returns (uint256) {
        ...
        if (usdfAmount > 0) {
            uint256 _fee = (usdfAmount.mul(FEE)).div(10000);
    ...
    function getEstimatedUSDFforDAI(uint256 daiAmount) public view returns (uint256) {
        if (daiAmount > 0) {
            uint256 _fee = (daiAmount.mul(FEE)).div(10000);
