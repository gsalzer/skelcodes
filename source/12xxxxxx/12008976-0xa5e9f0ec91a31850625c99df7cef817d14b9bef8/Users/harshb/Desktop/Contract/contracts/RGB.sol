...
import './util/IERC721Enumerable.sol';

interface IColors is IERC721Enumerable {
    function isMintedBeforeReveal(uint256 index) external view returns (bool);
}

/**
 *
 * RGBToken Contract (The native token of Colors)
 * @dev Extends standard ERC20 contract
 */
contract RGB is Context, IERC20 {
    ...
    address private _colorsAddress;
    ...
    /**
     * @dev When accumulated NCTs have last been claimed for a Colors index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(IColors(_colorsAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IColors(_colorsAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated NCT tokens for a Colors token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = IColors(_colorsAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setColorsAddress(address colorsAddress) public {
        require(_colorsAddress == address(0), "Already set");
        
        _colorsAddress = colorsAddress;
    }
    
    /**
     * @dev Claim mints NCTs and supports multiple Colors token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IColors(_colorsAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IColors(_colorsAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the Colors contract. For better UX.
        if (msg.sender != _colorsAddress) {
