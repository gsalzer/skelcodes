...
contract Fabricae is ERC20("Staked Jarvis Reward Token", "stJRT"){
    using SafeMath for uint256;
    IERC20 public jrt;

    // Define the Jarvis Reward Token contract
    constructor(IERC20 _jrt) public {
        jrt = _jrt;
    }
    ...
    // Claim back your JRT.
    // Unlocks the staked + gained JRT and burns stJRT
    function unstake(uint256 _share) public {
        // Gets the amount of stJRT in existence
        uint256 totalShares = totalSupply();
        // Calculates the amount of JRT the stJRT is worth
        uint256 finalAmount = _share.mul(jrt.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        jrt.transfer(msg.sender, finalAmount);
    }
}
