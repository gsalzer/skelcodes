...
// File: contracts/Explore.sol


pragma solidity ^0.6.0;


contract Explore is ERC20Burnable, Pausable, AccessControl, Ownable {
    
    mapping (address => uint256) private _balances;

    uint256 public initial_supply = 9 * (10**8) * (10**18);
  
    uint256 public public_allocation = (initial_supply * 65)/100;
    
    uint256 public founder_supply = (initial_supply * 5)/100;
    
    uint256 public growth_supply = (initial_supply * 10)/100;
    
    uint256 public country_lead_supply = (initial_supply * 10)/100;
    
    uint256 public core_team_supply = (initial_supply * 3)/100;
    
    uint256 public partners_supply = (initial_supply * 5)/100;
    
    uint256 public bounty_supply = (initial_supply * 2)/100;
    
    address public publicSaleAddress;
    address public founderAddress;
    address public growthAddress;
    address public countryLeadAddress;
    ...
    constructor(
        ...
        require(initial_supply > 0, "ERC20: supply cannot be zero");
        ...
        countryLeadAddress = _countryLeadAddress;
        coreTeamAddress = _coreTeamAddress;
        partnersAddress = _partnersAddress;
        bountyAddress = _bountyAddress;
        
        transfer(_publicSaleAddress, public_allocation);
        transfer(_founderAddress, founder_supply);
        transfer(_growthAddress, growth_supply);
        transfer(_countryLeadAddress, country_lead_supply);
        transfer(_coreTeamAddress, core_team_supply);
        transfer(_partnersAddress, partners_supply);
        transfer(_bountyAddress, bounty_supply);
    ...
    // function mint(address account, uint256 amount) internal virtual onlyOwner whenNotPaused returns(bool) {
    //     require(account != address(0), "ERC20, mint to the zero address");
    //     _mint(account, amount);
    // }
    
     /** lock the Account for security. Incase if token stolen or mistransfered to wrong address**/
