...
pragma solidity 0.6.12;
contract TimelockedTransfer is Context, Ownable {
	using SafeMath for uint256;
	using Address for address;
	AVOCADO public Token;
	constructor (AVOCADO _token) public {
		Token = _token;
	}
	
	mapping (address => bool) public isWaitingForTransfer;
	mapping (address => uint256) public pendingAmount;
	mapping (address => uint256) public txEta;

	event QueuedSend (address sender, address reciever, uint256 amount, uint256 eta);
	event ExecutedSend (address sender, address reciever, uint256 amount);
// NOTE SENDING A 2ND TRANSACTION TO THE SAME ADDRESS WHILE THE 1ST IS STILL PENDING WILL RESULT IN A LOSS OF FUNDS!
	function queueSend(address _sender, address _reciever, uint256 _amountSent, uint256 _delay) public onlyOwner {
		require(address(_reciever) != address(0), "TimelockedTransfer: reciever is the zero address");
		require(address(_sender) != address(0), "TimelockedTransfer: reciever is the zero address");
		uint256 eta = getBlockTimestamp().add(_delay);
		Token.transferFrom(_sender, address(this), _amountSent);
		isWaitingForTransfer[_reciever] = true;
		pendingAmount[_reciever] = _amountSent;
		txEta[_reciever] = eta;
		emit QueuedSend(_sender, _reciever, _amountSent, eta);
		eta = 0;
	}
	function executeSend (address _reciever_) public {
	require(isWaitingForTransfer[_reciever_] = true, "TimelockedTransfer: no transaction queued for this address");
		require(getBlockTimestamp()>= txEta[_reciever_], "TimelockedTransfer: delay hasnt ended yet");
		uint256 amountToSend = pendingAmount[_reciever_];
		Token.transfer(_reciever_, amountToSend);
		isWaitingForTransfer[_reciever_] = false;
		pendingAmount[_reciever_] = 0;
		txEta[_reciever_] = 0;
	}
