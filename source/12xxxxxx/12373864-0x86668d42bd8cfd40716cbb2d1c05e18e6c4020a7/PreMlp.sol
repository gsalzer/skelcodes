...
contract PreMlp is Initializable, OwnableUpgradeSafe {
    ...
    event PreMlpCreated(uint256 mlpId, uint256 id, address indexed token0, address indexed token1, address indexed pair, uint256 bonusTokenAmount);
    ...
    function takePreMlp(
        ...
        emit PreMlpCreated(mlpId, _PreMlpId, address(pendingPreMlp.token0), address(pendingPreMlp.token1), address(pair), _bonusTokenAmount);
    }
    ...
    function cancelMarketplaceMlp(uint256 _PreMlpId) external {
        ...
        if (pendingPreMlp.bonusToken1 > 0) {
            ...
            pendingPreMlp.token1.safeTransfer(pendingPreMlp.taker, pendingPreMlp.amountToken1);
        }
