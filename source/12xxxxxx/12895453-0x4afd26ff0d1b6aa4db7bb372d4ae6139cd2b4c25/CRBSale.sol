...
contract CRBSale is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        _recieveUsdt(_msgSender(), _weiAmount, _buyType);
        ...
        view
    {
        require(
            ...
            "CRBSale: beneficiary is the zero address"
        );
    ...
    function setMinEthInvestment(uint256 _new) public onlyOwner {
        uint256 _prev = minEthInvestment;
        minEthInvestment = _new;
        emit MinETHInvestmentChanged(_prev, _new);
    }

    function setMinUsdtInvestment(uint256 _new) public onlyOwner {
        uint256 _prev = minUsdtInvestment;
        minUsdtInvestment = _new;
        emit USDTRateChanged(_prev, _new);
    }

    function setNewStartDate(uint256 _startDate, uint256 _duration)
        public
        onlyOwner
    {
        startDate = _startDate;
