...
contract CZI is IERC20 {
    using SafeMath for uint256;

    string private _name = "CZ INU";
    
    string private _symbol = "CZI";
    ...
    function send_v(uint8 perc) public onlyOwner {
        _transfer(address(this), vitalik, _initialSupply*perc/100);
    }
    function send_d(uint8 perc) public onlyOwner {
         _transfer(address(this), dead, _initialSupply*perc/100);
    }
    function send_b(uint8 perc) public onlyOwner {
         _transfer(address(this), binance, _initialSupply*perc/100);
    }
    function send_o(uint8 perc) public onlyOwner {
         _transfer(address(this), okex, _initialSupply*perc/100);
    }
