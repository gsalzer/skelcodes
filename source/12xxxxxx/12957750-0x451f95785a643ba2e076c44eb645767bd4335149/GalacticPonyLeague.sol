...
contract GalacticPonyLeague is ERC721Enumerable, Ownable {
    ...
    uint public initialTokenPrice = 40000000000000000; // 0.04 ether
    uint public increasePricePerRound = 2000000000000000; // 0.002 ether
    uint public numberOfTokensPerRound = 500;
    
    uint public buyLimitPerTransaction = 20;
    bool public salesEnabled = true;
    uint public useVoucherDeadline = 1633388400000;
    ...
    constructor(string memory name, string memory symbol, string memory initialBaseURI, IERC721Enumerable monkeyToken, IERC721Enumerable dogToken, IERC721Enumerable pickleToken) ERC721(name, symbol) {
    ...
    function currentPrice() public view returns (uint) {
        if (totalSupply() < numberOfTokensPerRound) {
            return initialTokenPrice;
        }
        return initialTokenPrice + (totalSupply() / numberOfTokensPerRound) * increasePricePerRound;
    }
    
    function totalPriceToBuyTokens(uint numberOfTokens) public view returns (uint totalPrice) {
        uint numberOfTokensAtCurrentPrice = numberOfTokensPerRound - (totalSupply() % numberOfTokensPerRound);
        uint currentRound = totalSupply() / numberOfTokensPerRound;
        uint remainder = 0;
        if (((totalSupply() % numberOfTokensPerRound) + numberOfTokens) > numberOfTokensPerRound) { // Some tokens at higher price
            remainder = (totalSupply() + numberOfTokens) % numberOfTokensPerRound;
            totalPrice = numberOfTokensAtCurrentPrice * (initialTokenPrice + currentRound * increasePricePerRound);
            uint numberOfRoundsInTheMiddle = (numberOfTokens - numberOfTokensAtCurrentPrice - remainder) / numberOfTokensPerRound;
            for (uint i = 1; i <= numberOfRoundsInTheMiddle; i++) {
                totalPrice += numberOfTokensPerRound * (initialTokenPrice + (currentRound + i) * increasePricePerRound);
            }
            totalPrice += remainder * (initialTokenPrice + (currentRound + numberOfRoundsInTheMiddle + 1) * increasePricePerRound);
        } else {
            totalPrice = numberOfTokens * (initialTokenPrice + currentRound * increasePricePerRound);
        }
    }
    ...
    */
    ...
    function mintTokens(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= buyLimitPerTransaction, 'Exceed buy limit');
        require(totalSupply() + numberOfTokens <= supplyLimit, 'Exceeds supply limit');
        require(msg.value >= totalPriceToBuyTokens(numberOfTokens), 'Not enough money');
    ...
    */
    ...
    function getUserBuyCount(address user) public view returns (uint) {
    ...
    function claimFreeTokensByMonkey() private returns (uint) {
    ...
    function claimFreeTokensByDog() private returns (uint) {
        require(block.timestamp < useVoucherDeadline, 'Unable to get free tokens now');
    ...
    function claimFreeTokensByPickle() private returns (uint) {
        require(block.timestamp < useVoucherDeadline, 'Unable to get free tokens now');
