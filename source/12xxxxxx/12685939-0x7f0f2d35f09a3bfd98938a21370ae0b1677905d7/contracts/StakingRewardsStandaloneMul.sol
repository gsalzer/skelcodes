...
contract StakingRewardsStandaloneMul is IStakingRewards, Ownable, ReentrancyGuard {
    ...
    uint256 public rewardsMultiplier = 0;
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        uint256 reward = rewards[msg.sender].div(rewardsMultiplier);
        if (reward > 0) {
    ...
    function notifyRewardAmount(uint256 reward, uint256 multiplier) external onlyOwner updateReward(address(0)) {
        rewardsMultiplier = multiplier;

        if (block.timestamp >= periodFinish) {
            rewardRate = reward.mul(multiplier).div(rewardsDuration);
        } else {
            uint256 remaining = periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardRate);
            rewardRate = reward.mul(multiplier).add(leftover).div(rewardsDuration);
        }
        ...
        require(rewardRate <= balance.mul(multiplier).div(rewardsDuration), "Provided reward too high");
