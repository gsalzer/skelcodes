...
contract VaultOwned is Ownable {
  ...
  function setMigration(address migration_) external onlyOwner returns (bool) {
    _migration = migration_;

    return true;
  }

  function migration() public view returns (address) {
    return _migration;
  }
  ...
  modifier onlyVaultOrMigration() {
    require(_vault == msg.sender || _migration == msg.sender, "VaultOwned: caller is not the Vault or Migration");
    _;
  }
}

contract Hades is ERC20PresetMinterPauser, VaultOwned {
  using SafeMath for uint256;

  constructor() ERC20PresetMinterPauser("Hades", "HADES") {
    _setupDecimals(9);
  ...
  function mint(address account_, uint256 amount_) public override onlyVaultOrMigration {
