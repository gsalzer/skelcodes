...
contract PaymentSplitter is Context {
    ...
    function _topUp() internal {
        uint256 totalEthBalance = VESPER_DEPLOYER
        .balance
        .add(IERC20(WETH).balanceOf(VESPER_DEPLOYER))
        .add(IERC20(veth).balanceOf(VESPER_DEPLOYER));
        // transfer only when balance is < low mark
        if (totalEthBalance < LOW) {
            uint256 amount = IERC20(veth).balanceOf(address(this)) > (HIGH.sub(totalEthBalance))
