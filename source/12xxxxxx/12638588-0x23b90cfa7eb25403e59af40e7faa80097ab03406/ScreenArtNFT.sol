...
// Root file: contracts/ScreenArtNFT.sol
...
contract ScreenArtNFT is ERC1155(""), Ownable {
    // Contract name
    string public name = "ScreenArt NFT";
    // Contract symbol
    ...
    modifier creatorOnly(uint256 _id) {
        require(_creators[_id] == _msgSender(), "You are not the creator");
        _;
    }

    function uri(uint256 _id) public view override returns (string memory) {
        string memory tokenUri = idToUri[_id];
    ...
    function setTokenUri(uint256 _id, string memory newuri)
        public
        creatorOnly(_id)
    {
        _setTokenUri(_id, newuri);
    }
    ...
    function mint(
        ...
        _mint(_to, newTokenId, _quantity, bytes(_uri));
        _setTokenUri(newTokenId, _uri);
        _creators[newTokenId] = _to;
        _incrementTokenId();
    }

    function _getNextTokenId() private view returns (uint256 nextTokenId) {
