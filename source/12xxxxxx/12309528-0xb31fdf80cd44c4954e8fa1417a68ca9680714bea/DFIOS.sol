...
contract ERC721MetadataMintable is ERC721, ERC721Metadata, MinterRole {
     ...
     */
    function mintWithTokenURI(address to, uint256 tokenId, string memory tokenURI) internal onlyMinter returns (bool) {
        _mint(to, tokenId);
...
contract ERC721Full is ERC721, ERC721Enumerable, ERC721Metadata,ERC721MetadataMintable {
    ...
    constructor () public ERC721Metadata("DeFi Omega Shield", "DFIOS")  {
...
contract DFIOS  is ERC721Full {
    event BatchMint(address, uint256,string);
    ...
    constructor (string memory baseuri_) public {
        _setBaseURI(baseuri_);
    }
     
        function batchMint(address to,uint amount,string memory uri) public onlyMinter returns(bool) {
        require(to != address(0), "Invalid Address");
        for (uint i = 0; i < amount; i++) {
        super.mintWithTokenURI(to,id++,uri);
        }
     emit BatchMint(to,amount,uri);
     return true;
    }
   ...
   function mint(address to,uint tokenId) public onlyMinter {
       super._mint(to,tokenId);
   }
