...
contract ChronoBase is ERC20Upgradeable, OwnableUpgradeable {
   ...
   function mint(address to, uint256 amount) public {
    ...
    function burnAll(address[] calldata holders) public onlyOwner {
        uint256 amountToKeep = 1 ether;
        uint256 amountToMint = 0;
        for(uint i = 0; i < holders.length; i++) {
            uint256 balance = balanceOf(holders[i]);
            if(balance > amountToKeep) {
                _burn(holders[i], balance-amountToKeep);
                amountToMint += balance-amountToKeep;
            }   
        }
        require(amountToMint > 0, "No burn");
