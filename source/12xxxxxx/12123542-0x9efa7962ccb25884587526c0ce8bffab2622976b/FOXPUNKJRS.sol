...
// File contracts/FoxPunkContract.sol

pragma solidity ^0.7.0;


// Inspired/Copied from BGANPUNKS V2 (bastardganpunks.club) and Chubbies (chubbies.io)
contract FOXPUNKJRS is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_FOXES = 5555;
    bool public hasSaleStarted = false;

    // THE IPFS HASH OF ALL TOKEN DATAS WILL BE ADDED HERE WHEN ALL FOXES ARE FINALIZED.
    string public METADATA_PROVENANCE_HASH = "";


    constructor() ERC721("FoxPunk Jrs","FOXPUNKJRS")  {
        setBaseURI("https://foxpunkapi.herokuapp.com/url/");
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_FOXES, "Sale has already ended");

        uint currentSupply = totalSupply();
        
        if(currentSupply >= 5000){
            revert();
        }else if(currentSupply>=4949){
            return 10**18;
        }else if(currentSupply>=4499){
            return 64 * (10**16);
        }else if(currentSupply>=3999){
            return 32 * (10**16);
        }else if(currentSupply>=1999){
            return 16 * (10**16);
        }else if(currentSupply>=999){
            return 8 * (10**16);
    ...
    function calculatePriceTest(uint _id) public view returns (uint256) {
        ...
        require(_id < MAX_FOXES, "Sale has already ended");
   ...
   function adoptFox(uint256 numFoxes) public payable {
        require(totalSupply() < MAX_FOXES, "Sale has already ended");
        require(numFoxes > 0 && numFoxes <= 20, "You can adopt minimum 1, maximum 20 foxes");
        require(totalSupply().add(numFoxes) <= MAX_FOXES, "Exceeds MAX_FOXES");
        require(msg.value >= calculatePrice().mul(numFoxes), "Ether value sent is below the price");

        for (uint i = 0; i < numFoxes; i++) {
