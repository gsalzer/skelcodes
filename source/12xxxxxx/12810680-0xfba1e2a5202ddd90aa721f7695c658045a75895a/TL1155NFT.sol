...
contract ERC1155Mintable is ERC1155, Ownable{
    ...
    uint256 public nonce = 10000001000000;
    ...
    function create(uint256 _initialSupply, string calldata _uri) external onlyOwner returns(uint256 _id) {
        ...
        _balances[_id][msg.sender] = _initialSupply;
        tokenUris[_id] = _uri;
    ...
    function mint(uint256 _id, address[] calldata _to, uint256[] calldata _quantities) external onlyOwner{
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            if (to.isContract()) {
                super._doSafeTransferAcceptanceCheck(msg.sender, msg.sender, to, _id, quantity, '');
            }
        }
    }
    ...
    function mintSingle(uint256 _id, address _to, uint256 _quantities) external onlyOwner {
        require(msg.sender == creators[_id]);
        // Grant the items to the caller
        _balances[_id][_to] = _quantities.add(_balances[_id][_to]);
        ...
        if (_to.isContract()) {
            super._doSafeTransferAcceptanceCheck(msg.sender, msg.sender, _to, _id, _quantities, '');
        }
    }
...
contract TL1155NFT is ERC1155Mintable {

    constructor() ERC1155Mintable("TL NFT", "TL", "") {
