...
contract FEXPool is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. FEXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that FEXs distribution occurs.
        uint256 accFexPerShare; // Accumulated FEXs per share, times 1e12.
    ...
    // The FEX Token!
    IFEX public fex;
    // FEX tokens created per block.
    uint256 public fexPerBlock;
    // Info of each pool.
    ...
    // The block number when FEX mining starts.
    ...
    // Set the number of fex produced by each block
    function setFexPerBlock(uint256 newPerBlock) public onlyOwner {
        massUpdatePools();
        fexPerBlock = newPerBlock;
    }
    ...
    // Update the given pool's FEX allocation point. Can only be called by the owner.
    ...
    function reward(uint256 blockNumber) public view returns (uint256) {
        ...
        return fexPerBlock.div(2 ** _phase);
    }

    function getFexBlockReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blockReward = getFexBlockReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return;
        }
        uint256 fexReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        bool minRet = fex.mint(address(this), fexReward);
        if (minRet) {
            pool.accFexPerShare = pool.accFexPerShare.add(fexReward.mul(1e12).div(lpSupply));
        }
        pool.lastRewardBlock = block.number;
    }

    // View function to see pending FEXs on frontend.
    function pending(uint256 _pid, address _user) external view returns (uint256, uint256){
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            (uint256 fexAmount, uint256 tokenAmount) = pendingFexAndToken(_pid, _user);
            return (fexAmount, tokenAmount);
        } else {
            uint256 fexAmount = pendingFex(_pid, _user);
            return (fexAmount, 0);
        }
    }

    function pendingFexAndToken(uint256 _pid, address _user) private view returns (uint256, uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accFexPerShare = pool.accFexPerShare;
        ...
        if (user.amount > 0) {
            ...
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getFexBlockReward(pool.lastRewardBlock);
                uint256 fexReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accFexPerShare = accFexPerShare.add(fexReward.mul(1e12).div(pool.totalAmount));
                return (user.amount.mul(accFexPerShare).div(1e12).sub(user.rewardDebt), userPending);
            }
            if (block.number == pool.lastRewardBlock) {
                return (user.amount.mul(accFexPerShare).div(1e12).sub(user.rewardDebt), userPending);
            }
    ...
    function pendingFex(uint256 _pid, address _user) private view returns (uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accFexPerShare = pool.accFexPerShare;
        ...
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getFexBlockReward(pool.lastRewardBlock);
                uint256 fexReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accFexPerShare = accFexPerShare.add(fexReward.mul(1e12).div(lpSupply));
                return user.amount.mul(accFexPerShare).div(1e12).sub(user.rewardDebt);
            }
            if (block.number == pool.lastRewardBlock) {
                return user.amount.mul(accFexPerShare).div(1e12).sub(user.rewardDebt);
            }
    ...
    // Deposit LP tokens to FEXPool for FEX allocation.
    function deposit(uint256 _pid, uint256 _amount) public notPause {
        require(!isBadAddress(msg.sender), 'Illegal, rejected ');
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            depositFexAndToken(_pid, _amount, msg.sender);
        } else {
            depositFex(_pid, _amount, msg.sender);
        }
    }

    function depositFexAndToken(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pendingAmount = user.amount.mul(pool.accFexPerShare).div(1e12).sub(user.rewardDebt);
            if (pendingAmount > 0) {
                safeFexTransfer(_user, pendingAmount);
            }
        ...
        user.rewardDebt = user.amount.mul(pool.accFexPerShare).div(1e12);
        user.multLpRewardDebt = user.amount.mul(pool.accMultLpPerShare).div(1e12);
        emit Deposit(_user, _pid, _amount);
    }

    function depositFex(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        updatePool(_pid);
        if (user.amount > 0) {
    ...
    // Withdraw LP tokens from FEXPool.
    function withdraw(uint256 _pid, uint256 _amount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            withdrawFexAndToken(_pid, _amount, msg.sender);
        } else {
            withdrawFex(_pid, _amount, msg.sender);
        }
    }

    function withdrawFexAndToken(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdrawFexAndToken: not good");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accFexPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            safeFexTransfer(_user, pendingAmount);
    ...
    function withdrawFex(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdrawFex: not good");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accFexPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            safeFexTransfer(_user, pendingAmount);
    ...
    function emergencyWithdraw(uint256 _pid) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            emergencyWithdrawFexAndToken(_pid, msg.sender);
        } else {
            emergencyWithdrawFex(_pid, msg.sender);
        }
    }

    function emergencyWithdrawFexAndToken(uint256 _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
    ...
    function emergencyWithdrawFex(uint256 _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 amount = user.amount;
        user.amount = 0;
    ...
    // Safe FEX transfer function, just in case if rounding error causes pool to not have enough FEXs.
    function safeFexTransfer(address _to, uint256 _amount) internal {
        uint256 fexBal = fex.balanceOf(address(this));
        if (_amount > fexBal) {
            fex.transfer(_to, fexBal);
        } else {
            fex.transfer(_to, _amount);
        }
    }
    ...
    function harvestOf(address account) public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (isMultLP(address(pool.lpToken))) {
                depositFexAndToken(pid, 0, account);
            } else {
                depositFex(pid, 0, account);
            }
        }
    }
}
