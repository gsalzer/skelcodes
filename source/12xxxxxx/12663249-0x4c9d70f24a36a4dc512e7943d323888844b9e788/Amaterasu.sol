...
pragma solidity ^0.6.12;
    ...
    contract Amaterasu is Context, IERC20, Ownable {
        ...
        // Marketing and charity fees will start at 0 so we don't have a big impact when deploying to Uniswap
        ...
        function removeAllFee() private {
            if(_marketingFee == 0 && _charityFee == 0) return;
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            //transfer amount, it will take marketing and charity fee
        ...
        function SetNumOfTokensToExchangeForCharity(uint256 newAmt) external onlyOwner(){
            _numOfTokensToExchangeForCharity = newAmt;
        }
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tCharity) = _getTValues(tAmount, _marketingFee, _charityFee);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tCharity);
        }

        function _getTValues(uint256 tAmount, uint256 marketingFee, uint256 charityFee) private pure returns (uint256, uint256, uint256) {
