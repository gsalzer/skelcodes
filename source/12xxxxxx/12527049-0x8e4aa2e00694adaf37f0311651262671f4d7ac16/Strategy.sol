...
contract Strategy is ConvexETH {
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(fis).approve(uniswap, 0);
        IERC20(fis).approve(uniswap, type(uint256).max);
    }

    // >>> include other rewards
    function _migrateRewards(address _newStrategy) internal override {
        super._migrateRewards(_newStrategy);
        IERC20(fis).safeTransfer(_newStrategy, IERC20(fis).balanceOf(address(this)));
    }
    ...
    function _claimableInETH() internal override view returns (uint256 _claimable) {
        ...
        uint256 _fis = IERC20(fis).balanceOf(address(this));
        if (_fis > 0) {
            address[] memory path = new address[](2);
            path[0] = fis;
            path[1] = weth;
            uint256[] memory swap = Uni(uniswap).getAmountsOut(_fis, path);
        ...
        )
    {
        ...
        if (_fis > 0) {
            ...
            Uni(uniswap).swapExactTokensForTokens(_fis, uint256(0), path, address(this), now);
        }
