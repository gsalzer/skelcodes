// fDAI the first auto rewarding DAI ecosystem. 
...
contract fDAI is IERC20, ReentrancyGuard {
    using Address for address;
    using SafeMath for uint256;
    enum TxType { FromExcluded, ToExcluded, BothExcluded, Standard }

    mapping (address => uint256) private rDaiBalance;
    mapping (address => uint256) private tDaiBalance;
    mapping (address => mapping (address => uint256)) private _allowances;

    EnumerableSet.AddressSet excluded;

    uint256 private tDaiSupply;
    uint256 private rDaiSupply;
    uint256 private feesAccrued;
 
    string private _name = 'FEG Wrapped DAI'; 
    string private _symbol = 'fDAI';
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (EnumerableSet.contains(excluded, account)) return tDaiBalance[account];
        (uint256 r, uint256 t) = currentSupply();
        return (rDaiBalance[account] * t)  / r;
    }
    ...
    function ClaimOPfee() public {
        ...
        uint256 transferToAmount = (IERC20(DAI).balanceOf(address(this))) - (tDaiSupply);
        _pushUnderlying(DAI, op, transferToAmount);
        tDaiSupply -= transferToAmount;
    ...
    function deposit(uint256 _amount) public  {
        ...
        _pullUnderlying(DAI, msg.sender, _amount);
        (uint256 r, uint256 t) = currentSupply();
        ...
        if(isExcluded(msg.sender)){
            tDaiBalance[msg.sender] += (_amount- fee);
        } 
        feesAccrued += df;
        rDaiBalance[op] += ((df * r) / t);
        rDaiSupply += (((net + df) * r) / t);
        rDaiBalance[msg.sender] += ((net * r) / t);
    ...
    function withdraw(uint256 _amount) public  {
        ...
        if(isExcluded(msg.sender)) {
            tDaiBalance[msg.sender] -= _amount;
            rDaiBalance[msg.sender] -= ((_amount * r) / t);
        } else {
            rDaiBalance[msg.sender] -= ((_amount * r) / t);
        }
        tDaiSupply -= (net + wf);
        rDaiSupply -= (((net + wf) * r ) / t);
        rDaiBalance[op] += ((wf * r) / t);
        feesAccrued += wf;
        _pushUnderlying(DAI, msg.sender, net);
    ...
    function rDaiToEveryone(uint256 amt) public {
        require(!isExcluded(msg.sender), "not allowed");
        (uint256 r, uint256 t) = currentSupply();
        rDaiBalance[msg.sender] -= ((amt * r) / t);
        rDaiSupply -= ((amt * r) / t);
        feesAccrued += amt;
    }
    ...
    function excludeFromFees(address account) external {
        ...
        if(rDaiBalance[account] > 0) {
            (uint256 r, uint256 t) = currentSupply();
            tDaiBalance[account] = (rDaiBalance[account] * (t)) / (r);
        }
    ...
    function includeInFees(address account) external {
        require(msg.sender == op2, "op only");
        require(EnumerableSet.contains(excluded, account), "address excluded");
        tDaiBalance[account] = 0;
        EnumerableSet.remove(excluded, account);
    }
    
    function tDaiFromrDai(uint256 rDaiAmount) external view returns (uint256) {
        (uint256 r, uint256 t) = currentSupply();
        return (rDaiAmount * t) / r;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amt) private {
        ...
        if (tt == TxType.ToExcluded) {
            rDaiBalance[sender] -= ((amt * r) / t);
            tDaiBalance[recipient] += (amt - fee);
            rDaiBalance[recipient] += (((amt - fee) * r) / t);
        } else if (tt == TxType.FromExcluded) {
            tDaiBalance[sender] -= (amt);
            rDaiBalance[sender] -= ((amt * r) / t);
            rDaiBalance[recipient] += (((amt - fee) * r) / t);
    ...
    function currentSupply() public view returns(uint256, uint256) {
        if(rDaiSupply == 0 || tDaiSupply == 0) return (1000000000, 1);
        uint256 rSupply = rDaiSupply;
        uint256 tSupply = tDaiSupply;
        for (uint256 i = 0; i < EnumerableSet.length(excluded); i++) {
            if (rDaiBalance[EnumerableSet.at(excluded, i)] > rSupply || tDaiBalance[EnumerableSet.at(excluded, i)] > tSupply) return (rDaiSupply, tDaiSupply);
            rSupply -= (rDaiBalance[EnumerableSet.at(excluded, i)]);
            tSupply -= (tDaiBalance[EnumerableSet.at(excluded, i)]);
        }
        if (rSupply < rDaiSupply / tDaiSupply) return (rDaiSupply, tDaiSupply);
