...
contract StrategyTRUTusdV1 is StrategyBase {
    ...
    // staking constants
    address public constant trueFiPool = 0xa1e72267084192Db7387c8CC1328fadE470e4149;
    address public constant stakingFarm = 0x8FD832757F58F71BAC53196270A4a55c8E1a29D9;
    ...
    function deposit() public override {
        ...
        TrueFarm(stakingFarm).stake(_lpAmount);
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        uint256 _totalStaked = TrueFarm(stakingFarm).staked(address(this));

        _withdrawAmt  = _withdrawAmt > _totalStaked ? _totalStaked : _withdrawAmt;

        TrueFarm(stakingFarm).unstake(_withdrawAmt);
        uint256 _lpAmount = IERC20(trueFiPool).balanceOf(address(this));
    ...
    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        ...
        require(address(_asset) != stakingFarm, 'tfiLPFarming');
    ...
    function balanceOfPool() public override view returns (uint256){
        uint256 _lpAmount = TrueFarm(stakingFarm).staked(address(this));
        uint256 poolValue = TrueFiPool(trueFiPool).poolValue();
        uint256 totalSupply = TrueFiPool(trueFiPool).totalSupply();
    ...
    function getHarvestable() external view returns (uint256) {
        return TrueFarm(stakingFarm).claimable(address(this)); //TRU rewards
    }
