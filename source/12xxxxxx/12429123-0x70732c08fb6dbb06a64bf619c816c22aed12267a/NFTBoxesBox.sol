...
// Part: OpenZeppelin/openzeppelin-contracts@3.2.0/IERC1155
...
contract NFTBoxesBox is ERC721("NFTBox", "[BOX]"), Ownable, HasSecondaryBoxSaleFees {
	...
	function buyManyBoxes(uint256 _id, uint128 _quantity) external payable {
		...
		require(voucherValidityInterval[_id] != 0 && block.timestamp > voucherValidityInterval[_id],
			"Buy window !open");
		require(boxMould.price.mul(_quantity) == msg.value, "!price");
		require(currentEdition + _quantity <= max, "Too many boxes");
		require(boxBoughtMapping[_id][msg.sender].add(_quantity) <= boxMould.maxBuyAmount, "!buy");

		for (uint128 i = 0; i < _quantity; i++)
			_buy(currentEdition, _id, i, msg.sender);
		boxMould.currentEditionCount += _quantity;
		boxBoughtMapping[_id][msg.sender] = boxBoughtMapping[_id][msg.sender].add(_quantity);
	...
	function reserveBoxes(uint256 _id, uint256 _quantity) external payable {
		BoxMould memory boxMould = boxMoulds[_id];
		require(_id <= boxMouldCount && _id > 0, "ID !exist");
		require(voucherValidityInterval[_id] == 0, "Cannot reserve");
		require(boxMould.price.mul(_quantity).mul(gasFee).div(TOTAL_SHARES) == msg.value, "!price");
	...
	function distributeReservedBoxes(uint256 _id, uint256 _amount) external authorised {
		require(_id <= boxMouldCount && _id > 0, "ID !exist");
		require(!lockedBoxes[_id], "Box locked");
		require(voucherValidityInterval[_id] == 0, "Box distro over");
