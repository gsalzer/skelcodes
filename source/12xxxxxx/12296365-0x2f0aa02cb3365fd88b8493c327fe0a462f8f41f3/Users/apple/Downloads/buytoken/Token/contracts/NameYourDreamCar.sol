...
/**
 *
 * NameYourDreamCar Contract (The native token of Dreamcars)
 * @dev Extends standard ERC20 contract
 */
contract NameYourDreamCar is Context, IERC20 {
    ...
    address private _dreamcarsAddress;
    ...
    /**
     * @dev When accumulated NYDCs have last been claimed for a Dreamcars index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(IDreamCars(_dreamcarsAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IDreamCars(_dreamcarsAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated NYDC tokens for a Dreamcar token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = IDreamCars(_dreamcarsAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setDreamCarsAddress(address dreamcarsAddress) public {
        require(_dreamcarsAddress == address(0), "Already set");
        
        _dreamcarsAddress = dreamcarsAddress;
    }
    
    /**
     * @dev Claim mints NYDCs and supports multiple Dreamcar token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IDreamCars(_dreamcarsAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IDreamCars(_dreamcarsAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
        ...
        require(totalClaimQty != 0, "No accumulated NYDC");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the Dreamcars contract. For better UX.
        if (msg.sender != _dreamcarsAddress) {
