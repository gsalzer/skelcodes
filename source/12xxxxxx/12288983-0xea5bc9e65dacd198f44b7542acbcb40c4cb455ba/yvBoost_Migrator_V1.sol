...
// File: contracts/yEarn/yvBoost_Migrator_V1.sol
...
contract yvBoost_Migrator_V1 is Ownable {
    using SafeMath for uint256;
    ...
    {
        IERC20(yvBOOST_ETH_Sushi).safeApprove(yvBOOST_ETH_pJar, amount);
    ...
    {
        IERC20(yveCRV).safeApprove(address(yearnZapIn), uint256(amountIn));
    ...
    {
        IERC20 sushiPool = IERC20(yveCRV_ETH_Sushi);
    ...
    function _sushiDeposit(
        ...
        IERC20(toUnipoolToken0).safeApprove(
            address(sushiSwapRouter),
            token0Bought
        );
        IERC20(toUnipoolToken1).safeApprove(
            address(sushiSwapRouter),
            token1Bought
        );
