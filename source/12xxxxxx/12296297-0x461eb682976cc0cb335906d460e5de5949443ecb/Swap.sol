...
contract Swap is Ownable {
    ...
    mapping(bytes32 => SwapContract) contracts;
    ...
    /**
     * @dev Sender / Payer sets up a new swap contract and withdraw to owner
     *
     *
    ...
    returns (bytes32 contractId)
    {
        ...
        if (!ERC20(_tokenContract).transferFrom(msg.sender, owner(), _amount))
            revert("transferFrom sender failed");

        contracts[contractId] = SwapContract(
            msg.sender,
            owner(),
            _tokenContract,
            _amount,
    ...
    function withdraw(uint256 amount) payable public onlyOwner {
        ...
        require(amount < balance, "Balance to low");
        msg.sender.transfer(amount);
    }

    function withdrawToken(address tokenContract, uint256 amount) payable public onlyOwner {
        uint256 balance = ERC20(tokenContract).balanceOf(address(this));
        require(amount < balance, "Balance to low");
