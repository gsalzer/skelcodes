...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    function sendFundsToBMIDAIStaking(uint256 amount) external onlyOwner {
        ...
        emit TokensSent(bmiDaiStakingAddress, amount);
    }
    ...
    /// @notice attaches underlying DAI tokens to an NFT and initiates rewards gain
    function stake(
        address policyBookAddress,
        uint256 nftIndex,
        uint256 amount
    ...
    /// @notice returns how much DAI are using in rewards generation in the specific pool
    ...
    /// @notice returns how much DAI are used by an NFT
    ...
    /// @notice withdraws funds (rewards + BMIDAI tokens) of this NFT
