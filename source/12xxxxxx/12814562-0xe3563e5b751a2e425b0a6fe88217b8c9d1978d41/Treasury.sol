...
contract Treasury is IERC721Receiver, Context, Ownable {
    ...
    constructor(
        address theDogePound
    ) {
        _theDogePound = IERC721(theDogePound);
    }

    function sendDogeToPromoter(address promoter, uint256 amount) external onlyOwner {
    ...
    function sendDogeToPromoters(address[] memory promoters) external onlyOwner {
        require(promoters.length > 0);

        uint256[] memory tokens = _theDogePound.tokensOfOwner(address(this));

        for (uint256 i; i < promoters.length; i++) {
            _theDogePound.safeTransferFrom(address(this), promoters[i], tokens[i]);
        }
    }
