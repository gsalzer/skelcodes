...
contract Strategy is CurveVoterProxy {
    ...
    constructor(address _vault) public CurveVoterProxy(_vault) {
        ...
        gauge = address(0xd7d147c6Bb90A718c3De8C0568F9B560C79fa416);
        keepCRV = 1000;
    }
        ...
        )
    {
        ...
        IVoterProxy(proxy).claimRewards(gauge, pnt);
        uint256 _pnt = IERC20(pnt).balanceOf(address(this));
        if (_pnt > 0) {
            IERC20(pnt).safeApprove(uniswap, 0);
            IERC20(pnt).safeApprove(uniswap, _pnt);
