...
contract FRI is
    ...
    Freezable
{
    string constant private _name = "Food Research Institute for Mankind";
    string constant private _symbol = "FRI";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "FRI/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "FRI/transferFrom : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "FRI/approve : Should not approve zero address"
        );
