...
interface ICrossChainPolicy {
    function onTransfer(address token, address destination, uint256 amount, bytes memory extraData) external;
}
...
contract Custodian is Authorization {
    address public crossChainPolicy = address(0x0);
    mapping(bytes32 => mapping(uint16 => uint8)) public refunded;
    constructor() {
    }
    
    event CrossChainPolicyChanged(address newPolicy);
    event Refund(address token, address destination, uint amount);
    
    function setCrossChainPolicy(address crossChainPolicy_) external onlyOwner {
     ...
     */
    function refund(bytes32 txHash, uint16 transferIndex, address token, address destination, uint amount, bytes memory extraData) external auth {
        require(refunded[txHash][transferIndex] == 0, "This transfer was already processed");
        if (crossChainPolicy != address(0x0)) {
            ICrossChainPolicy(crossChainPolicy).onTransfer(token, destination, amount, extraData);
        }
        TransferHelper.safeTransfer(token, destination, amount);
        emit Refund(token, destination, amount);
        refunded[txHash][transferIndex] = 1;
    }
    
    function transferToken(address token, address destination, uint amount) external onlyOwner {
        TransferHelper.safeTransfer(token, destination, amount);
    }
    
    function transferETH(address payable destination, uint amount) external onlyOwner {
