...
// Root file: contracts/FMTPrivateDistribution.sol
...
contract FMTPrivateDistribution is Ownable {
    ...
    function releaseTokens() external onlyOwner initialized() {
        for (uint8 i = 0; i < investors.length; i++) {
            ...
            if (tokensAvailable > 0) {
                ...
                _fmtToken.safeTransfer(investors[i], tokensAvailable);
            }
        }
    }
        ...
        returns (uint256 tokens)
    {
        ...
        uint256 noOfTokens =
            _calculatePercentage(investor.tokensAllotment, availablePercentage);
