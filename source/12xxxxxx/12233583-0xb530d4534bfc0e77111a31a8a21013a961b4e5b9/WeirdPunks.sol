...
contract WeirdPunks is ERC721EnumerableSimple, Ownable {
    // Maximum amount of Weird Punks in existance. Ever.
    uint public constant MAX_WPUNK_SUPPLY = 10000;

    // The provenance hash of all Weirdos.
    string public constant METADATA_HASH = "QmdpFFRoid8kkrdM389Rjw349Cg73Bsqoh98EwUtR8j3fs";

    // Sale switch.
    bool public hasSaleStarted = false;

    // Bsae URI of WeirdPunk's metadata
    string private baseURI;

    constructor() ERC721("Weird Punks", "WEIRDPUNK") {}
    ...
    function calculatePriceForToken(uint _id) public pure returns (uint) {
        require(_id < MAX_WPUNK_SUPPLY, "Sale has already ended");

        if (_id >= 9900) {
            return 0.5 ether; //    9900-10000: 0.50 ETH
        } else if (_id >= 9500) {
            return 0.32 ether; // 9500-9500:  0.32 ETH
        } else if (_id >= 7500) {
            return 0.16 ether; // 7500-9500:  0.16 ETH
        } else if (_id >= 3500) {
            return 0.08 ether; // 3500-7500:  0.08 ETH
        } else if (_id >= 1500) {
            return 0.04 ether; // 1500-3500:  0.04 ETH
        } else if (_id >= 500) {
            return 0.02 ether; // 500-1500:   0.02 ETH
        } else {
            return 0.01 ether; // 0 - 500     0.01 ETH
        }
    }

    function adoptWeirdPunks(uint numWpunks) public payable {
        uint _totalSupply = totalSupply();
        require(_totalSupply < MAX_WPUNK_SUPPLY, "Sale has already ended");
        require(hasSaleStarted, "Sale hasn't started");
        require(_totalSupply + numWpunks <= MAX_WPUNK_SUPPLY, "Exceeds maximum WeirdPunk supply");
        require(numWpunks > 0 && numWpunks <= 30, "You can adopt minimum 1, maximum 30 WeirdPunks");
