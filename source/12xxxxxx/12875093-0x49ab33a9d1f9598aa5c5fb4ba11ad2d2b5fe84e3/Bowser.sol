...
contract Bowser is Context, IERC20, Ownable {
    ...
    string private _name = "Bowser Inu";
    string private _symbol = "BOWSER";
    uint256 private _tTotal = 100000000000000 * 10**9; // 100T

    // % to holders
    uint256 public defaultTaxFee = 3; //redistribution
    uint256 public _taxFee = defaultTaxFee;
    uint256 private _previousTaxFee = _taxFee;

    // % to swap & send to marketing wallet
    uint256 public defaultMarketingFee = 12; //buying only
    uint256 public _marketingFee = defaultMarketingFee;
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 public _marketingFee4Sellers = 12; //selling only

    bool public feesOnSellersAndBuyers = true;

    uint256 public _maxTxAmount = 5000000000 * 10**2 * 10**9; // max transaction amount
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to] && cooldownEnabled) {
                ...
                defaultMarketingFee = 12;
                _marketingFee4Sellers = 20; //ear;y penalty during cooldown
            }
        }
