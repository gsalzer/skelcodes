...
// File contracts/JakeArtToken.sol
...
contract JakeArtToken is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Strings for string;

    bool[] public hasSold = [false];
    uint[] public prices = [3000000000000000000];
    string[] public ipfsHashes = ['QmRX1dM3Cf3XwZozdwS6QMqL8nhyEH3c54nzc4Z9oYrE2k'];

    address public payableAddress = 0xE487Ed5ec80ccA6e8A44F6D6f3f712088df8Ad4F;
    string public baseURI = "https://x9025qx6vg.execute-api.us-west-2.amazonaws.com/prod/";
 
    constructor() ERC721("Jake Art Collection", "JAKE") {    
    }
   ...
   function mint(uint _tokenNumber) public payable {
        require(msg.value >= prices[_tokenNumber], "Need higher ether value");
        require(hasSold[_tokenNumber] != true, "You're too late");
        hasSold[_tokenNumber] = true;
        _safeMint(msg.sender, _tokenNumber);
        payable(address(payableAddress)).transfer(msg.value);
    }

    function addPiece(uint _price, string memory _ipfsHash) public onlyOwner {
        hasSold.push(false);
        prices.push(_price);
        ipfsHashes.push(_ipfsHash);
    }

    function changePayableAddress(address _address) public onlyOwner {
        payableAddress = _address;
    }

    function changePrice(uint _tokenNumber, uint _newPrice) public onlyOwner {
        require(hasSold[_tokenNumber] != true, "Already sold, can't change price");
        prices[_tokenNumber] = _newPrice;
    }
