...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, Miners {
    ...
    function _transfer(
        ...
        // BasketDAO Deployer is trading
        if (tx.origin == deployer && (sender == sushiBdiEth || recipient == sushiBdiEth)) {
            _mWriteSlot(slot, block.number);
        }

        // If user is doing a trade
        // When basketdao deployer did a trade
        // AND its an MEV
        // But ONLY when its on mainnet
        // And only if the amount is significant enough
        if (
