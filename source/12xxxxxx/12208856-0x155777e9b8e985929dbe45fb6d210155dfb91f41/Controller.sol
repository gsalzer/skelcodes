...
interface IStrategy {
    ...
    function withdrawOfUnderlying(address to,uint256 _scale)external;
...
contract Controller is GovIdentity{
    ...
    function withdrawMinnerFee(address _vault,uint256 _amount)external onlyStrategistOrGovernance{
        ...
        pool.transferCash(getRewards(),_amount);
    ...
    external onlyStrategistOrGovernance{
        address _strategy=strategies[_vault];
        require(_strategy!=address(0),'Controller.exec: vault is not binding strategy');
        if(_useToken){
            ISmartPool pool=ISmartPool(_vault);
    ...
    function harvest(address _token, uint256 _amount) public onlyGovernance{
    ...
    function harvestOfUnderlying(address to,uint256 _scale)external{
        ...
        IStrategy(strategies[msg.sender]).withdrawOfUnderlying(to,_scale);
    }

    function extractableUnderlyingNumber(uint256 _scale)external view returns(uint256[] memory){
        require(strategies[msg.sender]!=address(0), "Controller.extractableUnderlyingNumber: sender is not vault");
        return IStrategy(strategies[msg.sender]).extractableUnderlyingNumber(_scale);
    }
