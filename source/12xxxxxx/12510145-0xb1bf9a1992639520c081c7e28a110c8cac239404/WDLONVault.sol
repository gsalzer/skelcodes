...
contract WDLONVault{
    ...
    constructor(address _DLON) public{
        _wDLON = new wDLON();
        DLON = IERC20(_DLON);
    }
    
    function getwDLONaddress() public view returns(address){
        return address(_wDLON);
    }
    
    function getDLONaddress() public view returns(address){
        return address(DLON);
    }
    
    function wrapDLON(uint amount) public {
        
        require( amount>0, "Amount is 0" );
        DLON.transferFrom(msg.sender, address(this), amount);
        _wDLON.mint(msg.sender, amount);
        
    }
    
    function unWrapDLON(uint amount) public {
        
        require( amount>0, "Amount is 0" );
        _wDLON.burnFrom(msg.sender,amount);
        DLON.transfer(msg.sender,amount);
