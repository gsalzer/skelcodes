...
interface IERC2309 {
    // EvaverseNFTv2: We will no longer use this interface or event anymore.
    // Although it saved people money in gas, and it allowed us to send more batch mint tokens in 1 transaction, etherscan ignores the
    // message, and only listens to transfer message, so people don't see their tokens in the etherscan logs.
 ...
 * {ERC721Enumerable}.
 */
contract EvaverseNFTv2 is ProxyTarget, Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, IERC2309, Pausable, ReentrancyGuard {
    ...
    function MintNFT(uint count) payable external {
        MintInternal(_msgSender(), count);
    }
    
    function MintNFT(address to, uint count) payable external {
        MintInternal(to, count);
    }
    
    function MintInternal(address to, uint count) internal whenNotPaused nonReentrant {
    ...
    function approve(address to, uint256 tokenId) public virtual override whenNotPaused {
        address owner = EvaverseNFTv2.ownerOf(tokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(EvaverseNFTv2.ownerOf(tokenId) == from, "EvaNFT: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual whenNotPaused {
        ...
        emit Approval(EvaverseNFTv2.ownerOf(tokenId), to, tokenId);
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < EvaverseNFTv2.balanceOf(owner), "EvaNFT: owner index out of bounds");
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < EvaverseNFTv2.totalSupply(), "EvaNFT: global index out of bounds");
        return index + 1;
    }
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        ...
        uint256 length = EvaverseNFTv2.balanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = EvaverseNFTv2.balanceOf(from) - 1;
    ...
    // At the time of writing this function there are 16 NFTs that have been sent to invalid addresses that are unrecoverable without this.
    // 1 Was sent to a Giveaway and the person receiving it gave us a Binance wallet address, and they wouldn't help recover it.
    // 1 Was sent to a Coinbase wallet and is also unrecoverable. 
    // 14 Were sent to this contract due to a bug with turtles. That bug has now been fixed but the 14 NFTs left untouchable.
    function DevRecoverToken(address from, address to, uint256 tokenId) external onlyOwner {
