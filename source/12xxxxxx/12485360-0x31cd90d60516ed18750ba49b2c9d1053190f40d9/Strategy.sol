...
contract Strategy is CurveVoterProxy {
        ...
        )
    {
        ...
        IVoterProxy(proxy).claimRewards(gauge, alcx);
        uint256 _alcx = IERC20(alcx).balanceOf(address(this));
        if (_alcx > 0) {
            IERC20(alcx).safeApprove(sushiswap, 0);
            IERC20(alcx).safeApprove(sushiswap, _alcx);

            Uni(sushiswap).swapExactTokensForTokens(_alcx, uint256(0), pathReward, address(this), now);
        }
