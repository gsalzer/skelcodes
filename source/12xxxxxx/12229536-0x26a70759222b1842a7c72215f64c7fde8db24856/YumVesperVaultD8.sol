...
// File contracts/libraries/FixedPointMathD8.sol

pragma solidity ^0.6.12;

library FixedPointMathD8 {
...
// File contracts/libraries/alchemist/CDPD8.sol
...
// File contracts/YumVesperVaultD8.sol
...
contract YumVesperVaultD8 is ReentrancyGuard {
  using CDPD8 for CDPD8.Data;
  using FixedPointMathD8 for FixedPointMathD8.uq192x64;
  ...
  ///
  /// IMPORTANT: This constant is a raw FixedPointMathD8.uq192x64 value and assumes a resolution of 64 bits. If the
  ///            resolution for the FixedPointMathD8 library changes this constant must change as well.
  uint256 public constant MINIMUM_COLLATERALIZATION_LIMIT = 100000000;
  ...
  /// IMPORTANT: This constant is a raw FixedPointMathD8.uq192x64 value and assumes a resolution of 64 bits. If the
  ///            resolution for the FixedPointMathD8 library changes this constant must change as well.
  uint256 public constant MAXIMUM_COLLATERALIZATION_LIMIT = 400000000;
  ...
  /// @dev The context shared between the CDPD8s.
  CDPD8.Context private _ctx;

  /// @dev A mapping of all of the user CDPD8s. If a user wishes to have multiple CDPD8s they will have to either
  /// create a new address or set up a proxy contract that interfaces with this contract.
  mapping(address => CDPD8.Data) private _cdps;
  ...
  constructor(
    ...
    public
  {
    require(_governance != ZERO_ADDRESS, "YumVesperVaultD8: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumVesperVaultD8: sentinel address cannot be 0x0.");

    token = _token;
    xtoken = _xtoken;
    governance = _governance;
    sentinel = _sentinel;
    flushActivator = 10000000000000;// for 8 digit tokens

    uint256 COLL_LIMIT = MINIMUM_COLLATERALIZATION_LIMIT.mul(2);
    _ctx.collateralizationLimit = FixedPointMathD8.uq192x64(COLL_LIMIT);
    _ctx.accumulatedYieldWeight = FixedPointMathD8.uq192x64(0);
  }
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumVesperVaultD8: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumVesperVaultD8: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumVesperVaultD8: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumVesperVaultD8: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumVesperVaultD8: collateralization limit above maximum.");

    _ctx.collateralizationLimit = FixedPointMathD8.uq192x64(_limit);
  ...
  function collateralizationLimit() external view returns (FixedPointMathD8.uq192x64 memory) {
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumVesperVaultD8: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumVesperVaultD8: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumVesperVaultD8: cannot initialize rewards address to 0x0");
  ...
  function harvest(uint256 _vaultId) external expectInitialized returns (uint256, uint256) {
    ...
    if (_harvestedAmount > 0) {
      ...
      FixedPointMathD8.uq192x64 memory _weight = FixedPointMathD8.fromU256(_distributeAmount).div(totalDeposited);
  ...
  /// @dev Deposits collateral into a CDPD8.
  ///
  /// This function reverts if an emergency exit is active. This is in place to prevent the potential loss of
  /// additional funds.
  ///
  ...
  function deposit(uint256 _amount) external nonReentrant noContractAllowed expectInitialized {
    ...
    CDPD8.Data storage _cdp = _cdps[msg.sender];
    _cdp.update(_ctx);
  ...
  /// @dev Attempts to withdraw part of a CDPD8's collateral.
  ///
  /// This function reverts if a deposit into the CDPD8 was made in the same block. This is to prevent flash loan attacks
  ...
  /// @dev Attempts to liquidate part of a CDPD8's collateral to pay back its debt.
  ///
  ...
  ///
  /// This function reverts if the debt is increased and the CDPD8 health check fails.
  ///
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumVesperVaultD8: Loan-to-value ratio breached");
    } else {
  ...
  /// @dev Get the total amount of collateral deposited into a CDPD8.
  ///
  /// @param _account the user account of the CDPD8 to query.
  ///
  /// @return the deposited amount of tokens.
  function getCdpTotalDeposited(address _account) external view returns (uint256) {
    CDPD8.Data storage _cdp = _cdps[_account];
    return _cdp.totalDeposited;
  }

  /// @dev Get the total amount of alchemic tokens borrowed from a CDPD8.
  ///
  /// @param _account the user account of the CDPD8 to query.
  ///
  ...
  /// @dev Get the total amount of credit that a CDPD8 has.
  ///
  /// @param _account the user account of the CDPD8 to query.
  ///
  ...
  /// @dev Gets the last recorded block of when a user made a deposit into their CDPD8.
  ///
  /// @param _account the user account of the CDPD8 to query.
  ///
  ...
  modifier expectInitialized() {
    require(initialized, "YumVesperVaultD8: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumVesperVaultD8: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumVesperVaultD8: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumVesperVaultD8: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumVesperVaultD8: not an emergency, not governance, and user does not have permission to recall funds from active vault");
