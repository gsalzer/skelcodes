...
contract SHERLOCKHOLMES is Context, IERC20, Ownable {
    ...
    uint256 private _tBuyBackpoolTotal;

    string private _name = "Sherlock Holmes";
    ...
    uint256 public _buybackpoolFee = 20;
    uint256 private _previousBuyBackpoolFee = _buybackpoolFee;

    uint256 public _maxTxAmount = 1000000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 100 * 10**6 * 10**9;

    address payable public buybackpoolAddress = 0x00392153d513988147C2786b085d249ff91E7E91; // BuyBack Wallet
    ...
    function totalBuyBackpoolETH() public view returns (uint256) {
        // ETH has  18 decimals!
        return _tBuyBackpoolTotal;
    }
    ...
    function whitelistBot(address account) external onlyOwner() {
        require(_isExcluded[account], "Account is already excluded");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tBuyBackpoolTotal = _tBuyBackpoolTotal.add(address(this).balance);
        TransferBuyBackpoolETH(buybackpoolAddress, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_buybackpoolFee).div(
            10**2
        );
    }

    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _buybackpoolFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousBuyBackpoolFee = _buybackpoolFee;
    ...
    function restoreAllFee() private {
        ...
        _buybackpoolFee = _previousBuyBackpoolFee;
    }
    ...
    function setBuyBackpoolFeePercent(uint256 BuyBackpoolFee) external onlyOwner() {
        _buybackpoolFee = BuyBackpoolFee;
    }
    ...
    function TransferBuyBackpoolETH(address payable recipient, uint256 amount) private {
