...
/**
 *
 * Gallery0 G0 Token Contract (The native token of Gallery0)
 * Based on HashMasks NameChangingToken Contract (NCT)
 * @dev Extends standard ERC20 contract
 */
contract G0Token is Context, IERC20 {
    ...
    string private constant _name = "Gallery0 Token";
    string private constant _symbol = "G0";
    uint8 private _decimals;
    address private _artsquaresAddress;
    ...
    /**
     * @dev When accumulated G0s have last been claimed for a ArtSquare index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(IArtSquares(_artsquaresAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IArtSquares(_artsquaresAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated G0 tokens for a ArtSquare token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = IArtSquares(_artsquaresAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setArtsquaresAddress(address artsquaresAddress) public {
        require(_artsquaresAddress == address(0), "Already set");
        
        _artsquaresAddress = artsquaresAddress;
    }
    
    /**
     * @dev Claim mints G0s and supports multiple ArtSquare token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IArtSquares(_artsquaresAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IArtSquares(_artsquaresAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
        ...
        require(totalClaimQty != 0, "No accumulated G0");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the ArtSquares contract. For better UX.
        if (msg.sender != _artsquaresAddress) {
