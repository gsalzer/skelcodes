...
contract SpaceShibasTest is ERC721Enumerable {
  ...
  function buy(uint256 amountToBuy) public payable {
    ...
    require(msg.value == minPrice * amountToBuy, "Invalid amount of ether for amount to buy");
    ...
    while (tokensToMint < amountToBuy) {
      if (counter > 10000) {
        saleEnabled = false;
        payable(msg.sender).transfer((amountToBuy - tokenIdsToMint.length) * minPrice);
        break;
      }
      if (!_exists(i)) {
