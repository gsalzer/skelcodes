...
// File @openzeppelin/contracts-upgradeable/token/ERC721/IERC721ReceiverUpgradeable.sol@v3.4.1
  ...
  IERC721ReceiverUpgradeable
{
  ...
  event WithdrawnCoinviseBalance(address user, uint256 amount);
  ...
  // Wei charged per link for airdrop
  uint256 public weiPerAirDropLink;
   ...
   */
  function initialize(address _trustedAddress, uint256 _weiPerAirDropLink)
    public
    initializer
  {
    ...
    EIP712MetaTransactionUpgradeable._initialize("NFTAirdrop", "1");
    trustedAddress = _trustedAddress;
    weiPerAirDropLink = _weiPerAirDropLink;
  }
  ...
  function setWeiPerAirDropLink(uint256 _weiPerAirDropLink) external onlyOwner {
    weiPerAirDropLink = _weiPerAirDropLink;
  }

  function withdrawCoinviseBalance() external onlyOwner {
    uint256 totalBalance = address(this).balance;
    msg.sender.transfer(totalBalance);
    emit WithdrawnCoinviseBalance(msg.sender, totalBalance);
  }
  ...
  function _createCampaign(
    ...
    // Check approval
    require(
      _tokenAddress.isApprovedForAll(_sender, address(this)),
      "NOT_APPROVED_FOR_ALL"
    );
    require(
      _tokenIds.length == _linksAmount,
      "ERR_LINKS_COUNT_MUST_MATCH_TOKEN_IDS_COUNT"
    );

    // Transfer all nfts to self
    for (uint256 i = 0; i < _linksAmount; ++i) {
      _tokenAddress.safeTransferFrom(_sender, address(this), _tokenIds[i]);
    }
  ...
  function createCampaign(
    ...
    uint256 priceInWei = weiPerAirDropLink.mul(_linksAmount);
    require(msg.value == priceInWei, "ERR__CAMPAIGN_PRICE_MUST_BE_PAID");
    return _createCampaign(_tokenAddress, _linksAmount, _tokenIds);
  }
    ...
    )
  {
    ...
    return (
      ...
      _campaign.manager,
      address(_campaign.tokenAddress),
  ...
  function claim(
    ...
    IERC721 _token = _campaign.tokenAddress;
    uint256 _rewardedTokenId = _campaign.tokenIds[_campaign.linksRewardedCount];

    _token.safeTransferFrom(address(this), _claimer, _rewardedTokenId);
