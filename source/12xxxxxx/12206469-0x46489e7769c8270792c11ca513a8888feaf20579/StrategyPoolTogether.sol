...
contract StrategyPoolTogether is BaseStrategyInitializable {
    ...
    event MyVault(address vau);
    event BP(uint256 cv, uint256 amount);
    event BXXXX(uint256 cv, uint256 amount);
    function prepareReturn(uint256 _debtOutstanding)
        internal
        override
        returns (
    ...
    event WS(uint256 amount, uint256 balance);
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        uint256 balanceOfWantBefore = balanceOfWant();
        emit WS(_amount, balanceOfWantBefore);
