...
contract Minter is Ownable {
    ...
    event MinterDelete(address indexed minter);
    ...
    function addMinter(address minter) external onlyOwner {
        minters[minter] = true;
        emit MinterAdd(minter);
    }
    
    function deleteMinter(address minter) external onlyOwner {
        minters[minter] = false;
        emit MinterDelete(minter);
    }
    
    function mint(address to, uint256 amount) external onlyMinter {
        _swapp.mintSupply(to, amount);
    }
    
    function mintSupply(address to, uint256 amount) external onlyMinter {
        _swapp.mintSupply(to, amount);
    }
}
