...
// File contracts/aHadesMigration.sol
pragma solidity 0.7.5;


contract aHadesMigration {
  using SafeMath for uint256;

  address public immutable aHADES;
  ...
  constructor(
    ...
    require(_aHADES != address(0));
    require(_HADES != address(0));
    require(_endTime > 0);
    aHADES = address(_aHADES);
    HADES = address(_HADES);
    endTime = _endTime;
  }

  function migrate(uint256 _amount) external {
    require(block.timestamp >= endTime, "Current block is earlier than endTime");
    ERC20PresetMinterPauser(aHADES).burnFrom(msg.sender, _amount);
    ERC20PresetMinterPauser(HADES).mint(msg.sender, _amount);
  }
}
