...
contract CollToken is ERC20Burnable {
    using SafeMath for uint256;
     ...
     */
    event UpdateTaxInfo(address indexed owner, uint indexed feePercent);


    uint public constant PRECISION = 100;
    uint public constant INITIAL_SUPPLY = 50000000 * 10**18;

    address public _owner;
    address public _feeAccount;
    uint   public _feePercent;
    
    // List of no fee addresses when transfer token
    AddrArrayLib.Addresses noFeeAddrs;

    constructor(address feeAccount, uint feePercent) ERC20("Collateral", "COLL") {
        require(feeAccount != address(0), "Coll: fee account from the zero address");
     ...
     */
    function updateTaxInfo(address feeAccount, uint feePercent) external {
        require(feeAccount != address(0), "Coll: fee account from the zero address");
        require(feePercent <= PRECISION, "Coll: incorrect fee percent");
        require(_owner == _msgSender(), "Coll: updated tax info from no-owner address");
        _feeAccount = feeAccount;
        _feePercent = feePercent;
     ...
     */
    function addNoFeeAddress(address addr) external {
        require(_owner == _msgSender(), "Address: pushed address from no-owner address");
     ...
     */
    function removeNoFeeAddress(address addr) external {
        require(_owner == _msgSender(), "Address: pop address from no-owner address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(sender != address(0), "Coll: transfer from the zero address");
        require(recipient != address(0), "Coll: transfer to the zero address");
