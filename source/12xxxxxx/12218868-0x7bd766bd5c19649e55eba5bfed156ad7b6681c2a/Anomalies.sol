...
contract Anomalies is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapV2Router02 public immutable _uniswapV2Router;

    struct MadScientistInfo {
        uint256 harnessedFog;
    }

    address[] private _madScientist;

    address public _newFogPool;
    address public _plasma;
    address public _orbs;

    uint256 public _vaporizedChargeAmt;

    uint256 private _minPlasma;
    uint256 private _minPower;
    uint256 private _anomaliesInterval = 1 hours;
    uint256 public _lastAnomalie;

    event VaporizedChargeAmtUpdated(uint256 vaporizedChargeAmt);

    event Loged(address indexed madScientist, uint256 amount);
    event Unloged(address indexed madScientist, uint256 amount);
    event MinPowerUpdated(uint256 _minPower);
    event MinPlasmaUpdated(uint256 _minPlasma);
    event anomaliesIntervalUpdated(uint256 _anomaliesInterval);
    event FoggyBogUpdated(address _newFogPool);
    event OrbsAddressUpdated(address _orbs);

    mapping(address => uint256) public balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => MadScientistInfo) private _logBook;

    PLASMA public _plasmaContract;
    FogContract public _fogContract;

    constructor(
        IUniswapV2Router02 uniswapV2Router,
        FogContract fogContract,
        PLASMA plasmaContract,
        address plasma,
        address newFogPool
    ) public {
        _newFogPool = newFogPool;
        _plasma = plasma;
        _plasmaContract = plasmaContract;
    ...
    // harness the power of your fog by depositing it here.
    function logHarnessedFog(uint256 amount) public {
        require(
            _fogContract.transferFrom(msg.sender, address(this), amount),
            "Could not harness FOG"
        );

        uint256 HarnessedFog = amount;

        if (_logBook[_msgSender()].harnessedFog == 0) {
            _madScientist.push(_msgSender());
        }
        _logBook[_msgSender()].harnessedFog = _logBook[_msgSender()]
            .harnessedFog
            .add(HarnessedFog);
        emit Loged(_msgSender(), HarnessedFog);
    }

    // unHarness any remaining fog back to your wallet.
    function unlogHarnessedFog(uint256 amount) public {
        require(
            _logBook[_msgSender()].harnessedFog >= amount,
            "unlog amount exceeeds loged amount"
        );
        ...
        require(
            _fogContract.transfer(_msgSender(), amount),
            "unHarnessing Fog failed"
        );

        if (_logBook[_msgSender()].harnessedFog == 0) {
            for (uint256 i = 0; i < _madScientist.length; i++) {
                if (_madScientist[i] == _msgSender()) {
                    _madScientist[i] = _madScientist[_madScientist.length - 1];
                    _madScientist.pop();
                    break;
                }
            }
        }
        emit Unloged(msg.sender, amount);
    }

    function vaporize() public {
        uint256 vaporizeCharge = _vaporizedChargeAmt;

        if (IERC1155(_orbs).balanceOf(msg.sender, 2) >= 1) {
            privilegedVape();
        } else {
            require(
                _logBook[_msgSender()].harnessedFog >= vaporizeCharge,
                "More harnessed FOG needed"
            );

            require(
                _plasmaContract.balanceOf(_msgSender()) >= _minPlasma,
                "Access denied, need more PLASMA"
            );
            require(
                block.timestamp > _lastAnomalie + _anomaliesInterval,
                "Too Soon."
            );
            require(
                _plasmaContract.balanceOf(address(this)) >= _minPower,
                "need more powwwwahhh"
            );

            _lastAnomalie = block.timestamp;

            _logBook[_msgSender()].harnessedFog = _logBook[_msgSender()]
                .harnessedFog
                .sub(_vaporizedChargeAmt);

            if (_logBook[_msgSender()].harnessedFog == 0) {
                for (uint256 i = 0; i < _madScientist.length; i++) {
                    if (_madScientist[i] == _msgSender()) {
                        _madScientist[i] = _madScientist[
                            _madScientist.length - 1
                        ];
                        _madScientist.pop();
                        break;
                    }
            ...
            uint256 amtFogBeforeAnomalie =
                _fogContract.balanceOf(address(this));

            uint256 amtPlasmaToSwap = _plasmaContract.balanceOf(address(this));

            uint256 amtEthBeforeSwap = address(this).balance;

            swapTokensForEth(amtPlasmaToSwap);

            uint256 EthAmount = address(this).balance.sub(amtEthBeforeSwap);

            swapEthForFog(EthAmount);

            uint256 fogSupply = _fogContract.balanceOf(address(this));
            uint256 fogToBog =
                fogSupply.sub(amtFogBeforeAnomalie).add(vaporizeCharge);

            _fogContract.transfer(_newFogPool, fogToBog);

            emit Unloged(_msgSender(), vaporizeCharge);
        }
    }

    function privilegedVape() public {
        require(
            IERC1155(_orbs).balanceOf(msg.sender, 2) >= 1,
            "Anomalies: one much be holding the FOG orb to yeild such power"
        );
        ...
        uint256 amtFogBeforeAnomalie = _fogContract.balanceOf(address(this));
        ...
        uint256 fogSupply = _fogContract.balanceOf(address(this));
        uint256 fogToBog = fogSupply.sub(amtFogBeforeAnomalie);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(_plasma);
        path[1] = _uniswapV2Router.WETH();

        _plasmaContract.approve(address(_uniswapV2Router), tokenAmount);
    ...
    function swapEthForFog(uint256 EthAmount) private {
        address[] memory uniswapPairPath = new address[](2);
        uniswapPairPath[0] = _uniswapV2Router.WETH();
        uniswapPairPath[1] = address(_fogContract);
    ...
    function _setVaporizedChargeAmt(uint256 vaporizedChargeAmt)
        external
        onlyOwner()
    {
        _vaporizedChargeAmt = vaporizedChargeAmt;
        emit VaporizedChargeAmtUpdated(vaporizedChargeAmt);
    }

    function getHarnessedFogLogged(address madScientist)
        public
        view
        returns (uint256)
    {
        return _logBook[madScientist].harnessedFog;
    }

    function getPowerCellAmt(address madScientist)
        public
        view
        returns (uint256)
    {
        uint256 harnessedFogAmt = _logBook[madScientist].harnessedFog;
        uint256 vaporizeCharge = _vaporizedChargeAmt;
        uint256 powerCells = harnessedFogAmt.div(vaporizeCharge);
        return powerCells;
    }

    function getFogAddress() public view returns (address) {
        return address(_fogContract);
    }

    function getFoggyBogAddress() public view returns (address) {
        return address(_newFogPool);
    }

    function getLastAnomalie() public view returns (uint256) {
        return _lastAnomalie;
    }

    function getAnomalieInterval() public view returns (uint256) {
        return _anomaliesInterval;
    }

    function getMinPower() public view returns (uint256) {
        return _minPower;
    }

    function getMinPlasma() public view returns (uint256) {
        return _minPlasma;
    }

    function getVaperizedChargeAmt() public view returns (uint256) {
        return _vaporizedChargeAmt;
    }

    function _setAnomalieInterval(uint256 anomaliesInterval)
        external
        onlyOwner()
    {
        _anomaliesInterval = anomaliesInterval;
        emit anomaliesIntervalUpdated(anomaliesInterval);
    }

    function _setMinPower(uint256 minPower) external onlyOwner() {
        _minPower = minPower;
        emit MinPowerUpdated(minPower);
    }

    function _setMinPlasma(uint256 minPlasma) external onlyOwner() {
        _minPlasma = minPlasma;
        emit MinPlasmaUpdated(minPlasma);
    }

    function _setFoggyBogAddress(address foggyBog) external onlyOwner() {
        _newFogPool = foggyBog;
        emit FoggyBogUpdated(foggyBog);
    }

    function getPlasmaBalance() external view returns (uint256) {
        uint256 plasmaBalance = _plasmaContract.balanceOf(address(this));
        return plasmaBalance;
    }

    function _setOrbsAddress(address orbs) external onlyOwner() {
        _orbs = orbs;
        emit OrbsAddressUpdated(orbs);
    }
}
