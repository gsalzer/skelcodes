...
contract TokenVesting is Ownable {
    ...
    returns (uint256)
  {
    ...
    uint256 totalApproved =
      amountPerRelease(_recipient).mul(
        Math.min(elapsedTime.div(info.releaseInterval), info.totalReleaseCount)
      );
    return totalApproved.sub(info.paidAmount);
  }

  function amountPerRelease(address _recipient) public view returns (uint256) {
    VestingInfo memory info = vestingInfoMap[_recipient];
    return info.totalAmount.div(info.totalReleaseCount);
  }

  function nextReleaseTimestamp() public view onlyRecipient returns (uint256) {
    ...
    return info.startTime.add(info.releaseInterval.mul(pastReleaseCount + 1));
  }
