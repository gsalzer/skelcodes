...
contract Balancer is IExchange, AdapterStorage {
        ...
        override
    {
        ...
        if (address(fromToken) == Utils.ethAddress()) {
            IBalancerProxy(exchangeProxy).batchEthInSwapExactIn{value: fromAmount}(
                data.swaps,
                address(toToken),
                toAmount
            );
            ...
            IBalancerProxy(exchangeProxy).batchEthOutSwapExactIn(
                data.swaps,
                address(fromToken),
                fromAmount,
                toAmount
            ...
            IBalancerProxy(exchangeProxy).batchSwapExactIn(
                data.swaps,
                address(fromToken),
                address(toToken),
                fromAmount,
    ...
    function getKey() public override pure returns(bytes32) {
      return keccak256(abi.encodePacked("Balancer", "1.0.1"));
    }
