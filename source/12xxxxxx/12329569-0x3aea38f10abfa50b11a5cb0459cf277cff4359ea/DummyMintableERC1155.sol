    ...
    IMintableERC1155
{
    ...
    function collect(address _token) external only(DEFAULT_ADMIN_ROLE) {
        if (_token == address(0)) {
    ...
    function collectNFTs(address _token, uint256 _tokenId) external only(DEFAULT_ADMIN_ROLE) {
    ...
    function setUri(string calldata _uri) external only(DEFAULT_ADMIN_ROLE) {
        _setURI(_uri);
    }
