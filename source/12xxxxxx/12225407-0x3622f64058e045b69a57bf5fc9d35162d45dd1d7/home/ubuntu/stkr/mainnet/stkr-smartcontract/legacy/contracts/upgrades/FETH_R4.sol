...
contract FETH_R4 is Ownable, IERC20, Lockable {
    ...
    function mint(address account, uint256 shares, uint256 sentAmount) external onlyPool {
        /* amazing hotfix for balance overflow */
        if (sentAmount.div(shares) > 2) {
    ...
    function decreaseUserShares(address staker, uint256 amount) public onlyOwner {
        _shares[staker] = _shares[staker].sub(amount);
        _totalShares = _totalShares.sub(amount);
    }

    function increaseUserShares(address staker, uint256 amount) public onlyOwner {
        _shares[staker] = _shares[staker].add(amount);
