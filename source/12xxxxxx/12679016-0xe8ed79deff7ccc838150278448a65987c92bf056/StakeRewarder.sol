...
contract StakeRewarder is Ownable {
    ...
    // Total allocation points. Must be the sum of all allocation points across all staking tokens.
    ...
    event EmergencyReclaimRewards(address indexed user, uint256 amount);
    ...
    /**
     * @dev Adds a new token to the stack. Can only be called by the owner.
     * WARNING: DO NOT ADD TOKENS MORE THAN ONCE! Rewards will be messed up.
     */
    function add(
        uint256 _allocPoint,
        IERC20 _token,
    ...
    /**
     * @dev Update the given staking pool's allocation points. Can only be called by the owner.
     */ 
    function set(
        uint256 _pid,
        uint256 _allocPoint,
    ...
    /**
     * @dev View function to see pending rewards for an address (not for contract use).
     */
    ...
    /**
     * @dev Update rewards of the given pool to be up-to-date.
     */
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accPerShare).div(1e12).sub(
                    user.rewardDebt
                ).add(underpayment[msg.sender]);
            
            uint256 payout = safelyDistribute(msg.sender, pending);
            if (payout < pending) {
                underpayment[msg.sender] = pending.sub(payout);
            } else {
     ...
     */
    function safelyDistribute(address _to, uint256 _amount) internal returns (uint256 amount) {
    ...
    /**
     * @dev Withdraw undisbursed rewards tokens. This is a fail-safe.
     */
    function emergencyReclaimRewards(uint256 _amount) public onlyOwner {
        if (_amount == 0) {
            _amount = rewardToken.balanceOf(address(this));
        }
        
        rewardToken.transfer(msg.sender, _amount);
        emit EmergencyReclaimRewards(msg.sender, _amount);
    }
}
