...
// File contracts/YieldlyPools.sol

pragma solidity 0.6.6;
pragma experimental ABIEncoderV2;


contract YieldlyPools is RandomNumberConsumer, TwoStageOwnable  {
    ...
    struct Round {
        uint status;
        uint roundIndex;
        uint256 roundRandomnessSeed;
        uint256 winningRandomness;
        Program programInfo;
        Winner winnerInfo;
        uint256 roundOpenTimestamp;
        uint256 roundCloseTimestamp;
    }
    
    Round[] private rounds;
    
    uint256 roundTime = 0; // seconds
    ...
    event RoundRandomnessCalculated(
        uint256 indexed roundIndex,
    ...
    function roundsCount() public view returns (uint) {
        return rounds.length;
    }
    
    function roundData(uint round) public view returns (Round memory) {
        return rounds[round];
    }
    
    function roundRandomness(uint round) public view returns (uint256) {
        return rounds[round].winningRandomness;
    }
