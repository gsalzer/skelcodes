...
contract MultipleDistribution is Ownable, IMultipleDistribution {
    ...
    constructor(uint8 _pool) public {
        ...
        if (POOL_NUMBER == 3) {
            TOTAL_STAKE = 13_621_100 ether; // Private Offering supply. `ether` is used as the token has 18 decimals
        }
        if (POOL_NUMBER == 4) {
            TOTAL_STAKE = 7_066_668 ether; // Advisors Reward supply. `ether` is used as the token has 18 decimals
        }
    }
    ...
    function withdraw() external {
        ...
        if (POOL_NUMBER == 3) { // Private Offering timelock for the first 5 days
            require(_now() >= IDistribution(distributionAddress).distributionStartTimestamp().add(5 days), "Private Offering withdrawals are locked during LBP event");
        }
