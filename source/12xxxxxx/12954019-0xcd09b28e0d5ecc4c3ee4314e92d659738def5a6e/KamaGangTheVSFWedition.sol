// KamaGang: The VSFW Edition ETH CONTRACT
...
contract KamaGangTheVSFWedition is ERC721, Ownable {
    using SafeMath for uint256;
    
    IERC721 constant KamagangContract = IERC721(0xD049Ab4988962c8D3F25908E25baf7dd868A01C3);
    
    bool public canClaim = false;
    
    mapping(uint256 => bool) public ClaimedTokens;
    event Claim(uint256[] _id, address _from);
    // THE IPFS HASH OF ALL TOKEN DATAS WILL BE ADDED HERE
    string public METADATA_PROVENANCE_HASH = "";

    constructor() ERC721("KamaGang: The VSFW Edition","VSFW")  {
        setBaseURI("https://kamagang.com/api/token/vsfw/");  
    }
   ...
   function claimToken(uint256[] memory tokenIds) public payable {
        require(canClaim, "CLAIM PERIOD IS NOT STARTED YET.");
        for(uint i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < 3163, "INVALID ID.");
            require(KamagangContract.ownerOf(tokenIds[i]) == msg.sender && !ClaimedTokens[tokenIds[i]], "NOT OWNED OR ALREADY CLAIMED.");
            ClaimedTokens[tokenIds[i]] = true;
            _safeMint(msg.sender, tokenIds[i]);
        }
        emit Claim(tokenIds, msg.sender);
    }
