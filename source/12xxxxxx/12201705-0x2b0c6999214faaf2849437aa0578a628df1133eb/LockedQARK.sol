...
contract LockedQARK is ERC20, Ownable {
    ...
    constructor(IERC20 qark_) ERC20("Locked QARK", "LQARK") {
        _qark = qark_;
    }
    ...
    function makeOffer(address _toBuyer, uint256 _tokenAmount, uint256 _atPrice, uint32 _claimableUntil, uint32 _lockedUntil) public onlyOwner {
        ...
        if(_offers[_toBuyer].purchasePrice > 0){
            require(_offers[_toBuyer].tokenAmount > 0 && _offers[_toBuyer].claimableUntil > 0, "Buyer already claimed the offer!");
        }
    ...
    function cancelOffer(address _ofBuyer) public onlyOwner {
        ...
        require(_offers[_ofBuyer].tokenAmount > 0, "Buyer does not have an offer or claimed it already!");

        // TRANSFER THE OFFERED QARK AMOUNT BACK TO CONTRACT OWNER
        _qark.transfer(owner(), _offers[_ofBuyer].tokenAmount);

        // BURN THE LOCKED TOKENS
        _burn(owner(), _offers[_ofBuyer].tokenAmount);

        // REMOVE BUYER'S OFFER FROM MAPPING
        delete _offers[_ofBuyer];
    }
    ...
    receive() external payable {
        ...
        require(_offers[_msgSender()].purchasePrice == msg.value, "Incorrect purchase price sent");

        // OFFER MUST BE STILL CLAIMABLE
        require(_offers[_msgSender()].claimableUntil > block.timestamp, "Offer expired!");

        // PURCHASE SUCCESSFUL REGISTER BUYER'S BALANCE
        _transfer(owner(), _msgSender(), _offers[_msgSender()].tokenAmount);

        // UPDATE OFFER
        _offers[_msgSender()].claimableUntil = 0;
        _offers[_msgSender()].tokenAmount = 0;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(_offers[_msgSender()].lockedUntil < block.timestamp, "Tokens are still locked!");
