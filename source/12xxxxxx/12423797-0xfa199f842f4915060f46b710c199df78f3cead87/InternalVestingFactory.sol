...
contract InternalVestingFactory is Ownable {
    
    event VestingCreated(address vestingContractAddress, address beneficiary);
    
    uint256 public _start  = 1618707600; // start time 1618707600
    
    uint256 public _cliff  = 86400 * 30; //30 days
    
    uint256 public _duration  = 86400 * 180; //180 days
    
    address public constant LIX = address(0x33BbFF1Ab6CD5ddF36C9A905BC9fb308aa8f18D5); // LIX TOKEN 0xd0345D30FD918D7682398ACbCdf139C808998709
    
    
    function vest(address beneficiary, uint256 duration, uint256 amount) public {
        TokenVesting newVestingContract = new TokenVesting(beneficiary, _start, _cliff, duration*86400, true); 
        IERC20(LIX).transferFrom(msg.sender, address(newVestingContract), SafeMath.mul(amount, 1e18));
        VestingCreated(address(newVestingContract), msg.sender);
    }
    
    function changeStart(uint256 start) onlyOwner public {
        _start = start;
    }
    
    function changeCliff(uint256 cliff) onlyOwner public {
        _cliff = cliff;
    }
