...
contract MStable is IExchange {
    ...
    function getKey() public override pure returns (bytes32) {
        return keccak256(abi.encodePacked("MStable", "1.0.0"));
    }
    ...
    override
    {
        ...
        if (data.opType == uint(OpType.mint)) {
            IMStable(exchange).mint(address(fromToken), fromAmount, toAmount, address(this));
        } else if (data.opType == uint(OpType.redeem)) {
            IMStable(exchange).redeem(address(toToken), fromAmount, toAmount, address(this));
        } else if (data.opType == uint(OpType.swap)) {
            IMStable(exchange).swap(address(fromToken), address(toToken), fromAmount, toAmount, address(this));
        } else {
