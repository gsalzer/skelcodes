...
interface tokenRecipient {
    ...
    function isGov(address _from,address _to, address _token)
    external
    returns (bool);
}
...
contract RocketVault is ERC20Burnable, Ownable {
    ...
    constructor(address _spender) public ERC20("Rocket Vault", "RVF") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (address(spender) != address(0)) {
            spender.isGov(sender, recipient, address(this));
        }
