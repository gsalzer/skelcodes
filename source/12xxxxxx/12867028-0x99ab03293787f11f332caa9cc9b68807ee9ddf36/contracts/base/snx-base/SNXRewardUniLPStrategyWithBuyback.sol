...
contract SNXRewardUniLPStrategyWithBuyback is StrategyBase {
  ...
  */
  ...
  function setBuybackRatio(uint256 _newRatio) public onlyGovernance {
    require(_newRatio <= 10000, "Buyback ratio too high");
    buybackRatio = _newRatio;
  }

  function setDistributionPool(address _newPool) public onlyGovernance {
    require(IRewardPool(_newPool).lpToken() == vault, "Incompatible pool");
