...
contract NFTTemplateManager is ERC721Enumerable ,Ownable {
    ...
    function safeMint(uint256 _id, address _to, uint256 _uriTemplateId) public onlyManager {
    ...
    function safeBatchMint(uint256[] calldata _id, address[] calldata _to, uint256 _uriTemplateId) external onlyManager {
    ...
    function safeMultipleBatchMint(uint256[] calldata _id, address[] calldata _to, uint256[] calldata _uriTemplateId) external onlyManager {
        require(_id.length == _to.length, "_ids and _to array length must match.");
