...
contract DeflationaryTokenUpgradeableV1 is ERC20PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function transfer(
        address _to,
        uint256 _amount
    ) public override notBlacklistedAddress(msg.sender) notBlacklistedAddress(_to) returns (bool) {
        uint256 calculatedFee = calculateFee(_amount);
        if (calculatedFee > 0) {
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _amount
    ) public override notBlacklistedAddress(msg.sender) notBlacklistedAddress(_from) notBlacklistedAddress(_to) returns (bool) {
        uint256 calculatedFee = calculateFee(_amount);
        if (calculatedFee > 0) {
