...
/**
 *
 * LoveYourHottie Contract (The native token of Hotties)
 * @dev Extends standard ERC20 contract
 */
contract LoveYourHottie is Context, IERC20 {
    using SafeMath for uint256;

    // Constants
    uint256 public SECONDS_IN_A_DAY = 86400;

    uint256 public constant INITIAL_ALLOTMENT = 8369 * (10 ** 18);
    ...
    uint256 public emissionPerDay = 69 * (10 ** 18);
    ...
    address private _hottiesAddress;
    ...
    constructor (string memory name, string memory symbol, uint256 emissionStartTimestamp) {
        ...
        emissionEnd = emissionStartTimestamp + (86400 * 365 * 7);
    }
    ...
    /**
     * @dev When accumulated LYHs have last been claimed for a Hottie index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(IHotties(_hottiesAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IHotties(_hottiesAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated LYH tokens for a Hottie token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = IHotties(_hottiesAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setHottiesAddress(address hottiesAddress) public {
        require(_hottiesAddress == address(0), "Already set");

        _hottiesAddress = hottiesAddress;
    }

    /**
     * @dev Claim mints LYHs and supports multiple Hottie token indices at once.
     */
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IHotties(_hottiesAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IHotties(_hottiesAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
        ...
        require(totalClaimQty != 0, "No accumulated LYH");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the Hotties contract. For better UX.
        if (msg.sender != _hottiesAddress) {
