...
contract Strategy is ConvexStable {
    ...
    constructor(address _vault) public ConvexStable(_vault) {
        ...
        _setPathTarget(2, 0); // fxs path target
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(fxs).approve(dex[2], 0);
        IERC20(fxs).approve(dex[2], type(uint256).max);
    }

    // >>> include other rewards
    function _migrateRewards(address _newStrategy) internal override {
        super._migrateRewards(_newStrategy);
        IERC20(fxs).safeTransfer(_newStrategy, IERC20(fxs).balanceOf(address(this)));
    }
        ...
        )
    {
        ...
        if (_fxs > 0) {
            ...
            Uni(dex[2]).swapExactTokensForTokens(_fxs, uint256(0), path, address(this), now);
        }
