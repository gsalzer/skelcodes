...
contract RebalancingV2 is Storage, Constants, IO {
    ...
    function rebalance(
        ...
        // Swap on 1inch
        (uint256 toAmount, uint256[] memory distribution) =
            IOneInch(ONE_SPLIT).getExpectedReturn(fromUnderlying, toUnderlying, fromUnderlyingAmount, _paths, 0);
        IERC20(fromUnderlying).safeApprove(ONE_SPLIT, 0);
        IERC20(fromUnderlying).safeApprove(ONE_SPLIT, fromUnderlyingAmount);
        IOneInch(ONE_SPLIT).swap(fromUnderlying, toUnderlying, fromUnderlyingAmount, toAmount, distribution, 0);

        // Re-wrap to derivative
        if (_isCToken(_to)) {
            (, toAmount) = _toCToken(toUnderlying, toAmount);
        } else if (_isATokenV1(_to)) {
            (, toAmount) = _toATokenV1(toUnderlying, toAmount);
        } else if (_to == yveCRV) {
            // Should have CRV
            (, toAmount) = _toYveCRV(toAmount);
        } else if (_to == linkCRV) {
            (, toAmount) = _toLinkCRV(toAmount);
        }
