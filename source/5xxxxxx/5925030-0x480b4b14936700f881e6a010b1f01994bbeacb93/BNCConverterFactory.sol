...
/*
    BNC Formula interface
*/
contract IBNCFormula {
...
/*
    BNC Converter interface
*/
contract IBNCConverter {
...
/*
    BNC Converter Factory interface
*/
contract IBNCConverterFactory {
...
/*
    BNC Network interface
*/
contract IBNCNetwork {
...
/**
    Id definitions for BNC contracts
...
/**
    Id definitions for BNC contract features
...
/*
    ...
    The BNC version of the token converter, allows conversion between a smart token and other ERC20 tokens and between different ERC20 tokens and themselves.
...
*/
contract BNCConverter is IBNCConverter, SmartTokenController, Managed, ContractIds, FeatureIds {
    ...
    string public converterType = 'BNC';
    ...
    */
    ...
    // allows execution by the BNCNetwork contract only
    modifier BNCNetworkOnly {
        IBNCNetwork BNCNetwork = IBNCNetwork(registry.getAddress(ContractIds.BNC_NETWORK));
        require(msg.sender == address(BNCNetwork));
        _;
    }
    ...
    */
    ...
    /*
        ...
        @param _registry    address of a BNC converter registry contract
    */
    ...
    /*
        ...
        note that the whitelist check is actually done by the BNCNetwork contract
    ...
    */
    ...
    /*
        ...
        @param _path    new quick buy path, see conversion path format in the BNCNetwork contract
    */
    ...
    */
        ...
        returns (uint256)
    {
        ...
        IBNCFormula formula = IBNCFormula(registry.getAddress(ContractIds.BNC_FORMULA));
    ...
    */
    ...
    /**
        @dev converts the token to any other token in the BNC network by following a predefined conversion path
        note that when converting from an ERC20 token (as opposed to a smart token), allowance must be set beforehand

        @param _path        conversion path, see conversion path format in the BNCNetwork contract
    ...
    */
        ...
        returns (uint256)
    {
        ...
        // we need to transfer the source tokens from the caller to the BNCNetwork contract,
        // so it can execute the conversion on behalf of the caller
        if (msg.value == 0) {
            // not ETH, send the source tokens to the BNCNetwork contract
            // if the token is the smart token, no allowance is required - destroy the tokens
            // from the caller and issue them to the BNCNetwork contract
            if (fromToken == token) {
                token.destroy(msg.sender, _amount); // destroy _amount tokens from the caller's balance in the smart token
                token.issue(BNCNetwork, _amount); // issue _amount new tokens to the BNCNetwork contract
            } else {
                // otherwise, we assume we already have allowance, transfer the tokens directly to the BNCNetwork contract
                assert(fromToken.transferFrom(msg.sender, BNCNetwork, _amount));
            }
        }

        // execute the conversion and pass on the ETH with the call
        return BNCNetwork.convertForPrioritized2.value(msg.value)(_path, _amount, _minReturn, msg.sender, _block, _v, _r, _s);
    }
...
/*
    BNC Converter Factory
*/
contract BNCConverterFactory is IBNCConverterFactory {
    ...
    /**
        @dev constructor
    */
    function BNCConverterFactory() public {
    }
    ...
    */
    function createConverter(
        ...
        BNCConverter converter = new BNCConverter(
            _token,
            _registry,
            _maxConversionFee,
            _connectorToken,
