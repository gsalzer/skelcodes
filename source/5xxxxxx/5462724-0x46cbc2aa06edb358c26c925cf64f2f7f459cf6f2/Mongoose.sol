...
contract Mongoose is StandardToken, BurnableToken, Ownable {
    ...
    // Index of last token claimed by Mongoose holder, required for holder to claim unclaimed tokens
    ...
    modifier handleDerivativeTokens(address from) {
        ...
        for (uint256 i = lastDerivativeTokens[from]; i < derivativeTokens.length; i++) {
            // Since tokens haven't redeemed yet, mint new ones and send them to Mongoose holder
