...
/* once started, this process requires a total of at least 4 separate executions. Each execution is limited to 250 accounts, so if there
are more than 250 accounts in any step they will have to be executed multiple times (eg, 555 new accounts would require 3 executions of that step)
*/
        ...
        returns (bytes32 newsubkID)
    {
        ...
        require(rm < 6, "above max size"); // Some making this operation should remove this restriction
        uint takerLong;
        if (isTakerLong)
            takerLong = 1;
        else
    ...
    /** withdraw amounts are in gweis. Thus, 1234 is 1.234 Szabo
    ...
    * weekly PNL. They are capped at +/- 0.975e12 (the unit of account in this contract, szabo), so that the extreme case of a maximum funding rate, the liability
