/**************
 * A registry of provably authentic physical prints of Punks from the Larva Labs
 * CryptoPunks generative art project (https://www.larvalabs.com/cryptopunks).
 * In order to purchase a print you must be able to prove ownership of the Punk.
 * This is done by sending payment for the print from the account address that 
 * holds the Punk itself. There are three types of purchase. One is a print of your Punk
 * accompanied by an NFC that contains the details of your transaction. You may also 
 * purchase the NFC by itself if you want to print your own Punk. There is a Misc purchase
...
// File: contracts/PunkPrintRegistry.sol

pragma solidity ^0.5.0;

contract PunkPrintRegistry is CustomERC721Metadata, WhitelistedRole {
    using SafeMath for uint256;
    ...
    mapping(uint256 => uint256) public tokenIdToPunkId;
    ...
    function purchasePrint(uint256 _punkId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        ...
        tokenIdToPunkId[tokenId] = _punkId;
    ...
    function purchaseNFCOnly(uint256 _punkId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
    ...
    function purchaseMisc(uint256 _punkId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
    ...
    function mint(uint256 _punkId, string memory _nfc) public onlyWhitelisted returns (uint256 _tokenId) {
        uint256 tokenId = _mintToken(punkContract.punkIndexToAddress(_punkId));
        tokenIdToPunkId[tokenId] = _punkId;
    ...
    function updateTokenPunkId(uint256 _tokenId, uint256 _punkId) public onlyWhitelisted returns (bool) {
        require(_exists(_tokenId), "Token must exist");

        tokenIdToPunkId[_tokenId] = _punkId;
        return true;
    }
