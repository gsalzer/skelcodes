...
contract  ISusdCurve is ICurve {

    mapping(uint128 => address) public coins;
...
contract OneSplitUniswapPoolTokenView is OneSplitBaseView, OneSplitUniswapPoolTokenBase {
        ...
        )
    {
        ...
        uint256[] memory dist = new uint256[](9);
...
// File: contracts/OneSplitCurveSusdPoolToken.sol
...
contract OneSplitCurveSusdPoolTokenBase {
    ...
    function isPoolToken(IERC20 addr) internal view returns (bool) {
        return addr == curveSusdToken;
    }
        ...
        returns(PoolTokenDetails memory details)
    {
        ...
        for (uint256 i = 0; i < 4; i++) {
            details.tokens[i].token = IERC20(curve.coins(uint128(i)));
            details.tokens[i].reserveBalance = curve.balances(uint128(i));
        }
    }
...
contract OneSplitCurveSusdPoolTokenView is OneSplitBaseView, OneSplitCurveSusdPoolTokenBase {
        ...
        )
    {
        ...
        if (!disableFlags.check(FLAG_DISABLE_CURVE_SUSD_POOL_TOKEN)) {
            bool isPoolTokenFrom = isPoolToken(fromToken);
            bool isPoolTokenTo = isPoolToken(toToken);

            if (isPoolTokenFrom && isPoolTokenTo) {
                (
                uint256 returnDaiAmount,
                uint256[] memory poolTokenFromDistribution
                ) = _getExpectedReturnFromCurveSusdPoolToken(
                    fromToken,
                    dai,
                    amount,
                    parts,
                ...
                uint256 returnPoolTokenToAmount,
                uint256[] memory poolTokenToDistribution
                ) = _getExpectedReturnToCurveSusdPoolToken(
                    dai,
                    toToken,
                    returnDaiAmount,
                    parts,
        ...
        )
    {
        ...
        uint256[] memory dist;
        for (uint i = 0; i < 4; i++) {
            uint256 exchangeAmount = amount.div(4);
...
contract OneSplitCurveSusdPoolToken is OneSplitBase, OneSplitCurveSusdPoolTokenBase {
    ...
    function _swapToCurveSusdPoolToken(
        ...
        for (uint i = 0; i < 4; i++) {
            ...
            minMintAmount = tokenAmounts[i].mul(details.totalSupply).div(details.tokens[i].reserveBalance);

            _infiniteApproveIfNeeded(details.tokens[i].token, address(poolToken));
        }

        // 0.5% slippage
        curve.add_liquidity(tokenAmounts, minMintAmount.mul(995).div(1000));
    }
}
