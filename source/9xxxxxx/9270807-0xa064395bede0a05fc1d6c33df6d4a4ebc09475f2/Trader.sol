...
contract Trader{
   ...
   function swapEtherToToken (KyberNetworkProxyInterface _kyberNetworkProxy, ERC20 token, address destAddress) internal
   {
        ...
        require(token.transfer(destAddress, destAmount), "exception here...");
    }

    function swapTokenToEther1 (
        KyberNetworkProxyInterface _kyberNetworkProxy,
        ERC20 token,
        uint tokenQty,
        address destAddress) internal returns (uint)
    {
        token.transferFrom(msg.sender, this, tokenQty);
        token.approve(proxy, 0);
        token.approve(address(proxy), tokenQty);
        uint destAmount = proxy.tradeWithHint(
    ...
    function withdrawETHAndTokens() public onlyOwner
    {
        ...
        ERC20 daiToken = ERC20(usdcTokenAddress);
    ...
    function getKyberSellPrice() public view returns (uint256)
    {
     ...
     function getUniswapBuyPrice() public view returns (uint256){
        uint256 currentPrice = orfeed.getExchangeRate("ETH", "USDC", "BUY-UNISWAP-EXCHANGE", 1000000000000000000);
        return currentPrice;
    }
}
