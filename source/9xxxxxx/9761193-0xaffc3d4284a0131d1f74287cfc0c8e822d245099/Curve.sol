...
contract Curve is IExchange {
    ...
    function swap(
        ...
        if (curveData.underlyingSwap) {
            ICurve(exchange).exchange_underlying(curveData.i, curveData.j, fromAmount, toAmount);
        }
        else {
            ICurve(exchange).exchange(curveData.i, curveData.j, fromAmount, toAmount);
        }
