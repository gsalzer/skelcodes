...
// File: contracts/mocks/PauserRoleMock.sol
...
// File: contracts/ABEToken.sol

pragma solidity ^0.4.24;

// OpenZeppelin - Token Blueprint
...
contract ABEToken is ERC20Detailed, ERC20Pausable, PauserRoleMock, Ownable {
    ...
    modifier canTransfer(address _sender, uint256 _value) {
        ...
        uint256 remaining = balanceOf(_sender).sub(_value);
    ...
    constructor(address initialAddress, uint256 initialTokenSupply) 
        public 
        ERC20Detailed("Abelian", "ABE", 18) 
    {
        _mint(initialAddress, initialTokenSupply);
    }
    ...
    // this lockup will be released in 4 batches: 
    // 1. on delievery date
    // 2. three months after the delivery date
    // 3. six months after the delivery date
    // 4. nine months after the delivery date
		...
		returns (uint256)
	{
        ...
        if (now <= delieveryDate + 90 days) {
            return lockedAmt.mul(3).div(4);
        }
        if (now <= delieveryDate + 180 days) {
            return lockedAmt.mul(2).div(4);

        }
        if (now <= delieveryDate + 270 days) {
            return  lockedAmt.mul(1).div(4);
        }
	
        return 0;
    }

    // set lockup for investor
        ...
        onlyOwner
    {
        ...
        emit UpdatedLockingState(_investor, _value, _delieveryDate);
    }

    // show investor details
    function showLockupDetails_investors(address _investor) 
        public 
        view 
        returns (uint256, uint256) 
    {
        ...
        uint256 _value = investors_locked[_investor];
        uint256 _spendingDate = investors_deliveryDate[_investor];

        return (_spendingDate, _value);
    }
