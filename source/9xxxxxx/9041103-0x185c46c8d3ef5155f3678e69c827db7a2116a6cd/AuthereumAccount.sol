...
contract BaseAccount is AccountState, AccountInitialize, TokenReceiverHooks {
        ...
        internal
    {
        ...
        if (_feeTokenAddress == address(0)) {
            ...
            // NOTE: The return value is not checked because the relayer should not propogate a transaction that will revert
            ...
            uint256 totalTokenFee = _gasUsed.mul(_gasPrice).mul(_feeTokenRate);
    ...
    function _getRevertMsg(bytes memory _res) internal view returns (string memory) {
        // If the _res length is less than 68, then the transaction failed silently (without a revert message)
        if (_res.length < 68) return 'BA: Transaction reverted silently';

        string memory revertMsg;
        assembly {                                   // 36 is the location of the revert message in _res (_res.length (32 bytes) + topic (4 bytes))
            let offset := mload(add(_res, 36))       // Get the offset of the revert message by adding the location in _res
            revertMsg := add(_res, add(36, offset))  // Assign data ptr to the start of the revert message
        }
...
contract ERC1271Account is IERC1271, BaseAccount {
        ...
        returns (bytes4)
    {
        ...
        bytes4 sig;
        _isValidAuthKey(authKeyAddress) ? sig = VALID_SIG : sig = INVALID_SIG;
        return sig;
    }
        ...
        returns (bytes4)
    {
        ...
        bytes memory loginKeyAttestationSignature = _signatures.slice(65, 65);

        address _loginKeyAddress = _getEthSignedMessageHash(_msg).recover(
            msgHashSignature
        );
        ...
        bytes32 loginKeyAttestationMessageHash = keccak256(abi.encodePacked(
            _loginKeyAddress
        ...
        bytes4 sig;
        _isValidAuthKey(_authKeyAddress) ? sig = VALID_SIG : sig = INVALID_SIG;
        return sig;
    }
...
contract BaseMetaTxAccount is BaseAccount {
    ...
    function executeMultipleMetaTransactions(bytes[] memory _transactions) public onlyAuthKeySenderOrSelf returns (bytes32, bytes[] memory) {
        _executeMultipleMetaTransactions(_transactions);
    }
...
contract AuthereumAccount is BaseAccount, ERC1271Account, LoginKeyMetaTxAccount, AuthKeyMetaTxAccount, AccountUpgradeability {
    string constant public authereumVersion = "2019111500";
}
