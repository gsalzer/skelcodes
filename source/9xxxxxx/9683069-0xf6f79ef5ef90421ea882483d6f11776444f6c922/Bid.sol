...
contract Bid is ConstantAddresses {

    function bid(uint bidId, bool isEth) public {
        uint amount;
        uint lot;
        if (isEth) {
            (,lot,,,,,,amount) = Flipper(0xd8a04F5412223F513DC55F839574430f5EC15531).bids(bidId);
        } else {
            (,lot,,,,,,amount) = Flipper(0xaA745404d55f88C108A28c86abE7b5A1E7817c07).bids(bidId);
        }

        uint balance = ERC20(DAI_ADDRESS).balanceOf(msg.sender);

        if (balance > 0) {
            ERC20(DAI_ADDRESS).transferFrom(msg.sender, address(this), balance);
            ERC20(DAI_ADDRESS).approve(0x9759A6Ac90977b93B58547b4A71c78317f391A28, balance);
            Join(0x9759A6Ac90977b93B58547b4A71c78317f391A28).join(address(this), balance);
        }

        Vat(0x35D1b3F3D7966A1DFe207aa4514C12a259A0492B).hope(0xd8a04F5412223F513DC55F839574430f5EC15531);
        Vat(0x35D1b3F3D7966A1DFe207aa4514C12a259A0492B).hope(0xaA745404d55f88C108A28c86abE7b5A1E7817c07);

        if (isEth) {
            Flipper(0xd8a04F5412223F513DC55F839574430f5EC15531).tend(bidId, lot, amount);
        } else {
            Flipper(0xaA745404d55f88C108A28c86abE7b5A1E7817c07).tend(bidId, lot, amount);
        }
    }
