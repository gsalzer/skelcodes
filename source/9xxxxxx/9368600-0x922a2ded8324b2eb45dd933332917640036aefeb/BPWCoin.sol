...
/**
 * @title BPWCoin contract 
 */
contract BPWCoin is ERC20Mintable, ERC20Detailed {
    uint noOfTokens = 5000000000; // 5,000,000,000 (5B)

    // Address of bpwcoin vault (a BPWCoinMultiSigWallet contract)
    // The vault will have all the bpwcoin issued and the operation
    // on its token will be protected by multi signing.
    // In addtion, vault can recall(transfer back) the reserved amount
    // from some address.
    address internal vault;

    // Address of bpwcoin owner (a BPWCoinMultiSigWallet contract)
    // The owner can change admin and vault address, but the change operation
    // will be protected by multi signing.
    address internal owner;

    // Address of bpwcoin admin (a BPWCoinMultiSigWallet contract)
     ...
     *
     * The _from's BPW balance should be larger than the reserved amount(reserves[_from]) plus _value.
     *
     */
    ...
    function getOneBPWCoin() public view returns (uint) {
