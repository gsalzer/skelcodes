...
contract Roulette {
    ...
    address owner;
    uint maxBet = 110000000000000000;
    ...
    function play(string color) payable returns (uint256 result){
        
        require(msg.value < maxBet, "You must bet less than the maxBet");
        require(msg.value <= 0, "You cant bet zero");
        require(this.balance >= msg.value.mul(2), "There is not enough in the house to allow this bet to take palce");
        
        string memory theColor = toLower(color);
        bool userInputBool;
...
event Result(string bet, uint256 numb, string result);
}
