...
contract ErrorReporter {
    event Failure(uint error, uint detail);
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        GAME_STATUS,
    ...
    function fail(Error err, uint info) internal returns (uint) {
        emit Failure(uint(err), info);
...
contract richMan is SafeMath, ErrorReporter {
    ...
    * timer=倒计时，计数器单位为秒
    ...
    function setPaused(bool requestedState) public returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, 0);
        }
    ...
    function() payable public {
        if (msg.sender != admin) {
            fail(Error.UNAUTHORIZED, 0);
            return;
        }
        if (game[currentGameCount].status != 2) {
            msg.sender.transfer(msg.value);
            fail(Error.GAME_STATUS, game[currentGameCount].status);
            return;
        }
        currentGameCount += 1;
    ...
    function playGame(uint32 gameCount, address sponsorUser) payable returns (uint) {
        if (paused) {
            msg.sender.transfer(msg.value);
            return fail(Error.CONTRACT_PAUSED, 0);
        }
        if (game[currentGameCount].status != 1) {
            msg.sender.transfer(msg.value);
            return fail(Error.GAME_STATUS, game[currentGameCount].status);
        }
        if (game[gameCount].timer < safeSub(now, game[gameCount].lastTime)) {
            msg.sender.transfer(msg.value);
            return fail(Error.GAME_TIME_OUT, 0);
        }
        if (msg.value < game[gameCount].minAmount) {
            msg.sender.transfer(msg.value);
            return fail(Error.MIN_LIMIT, game[gameCount].minAmount);
        }
    ...
    function withdrawCommission(uint32 gameCount) public returns (uint){
        if (userWithdrawFlag[gameCount][msg.sender] == true) {
            return fail(Error.ALREADY_WITHDRAWAL, 0);
        }
        uint256 commission = calculateCommission(gameCount, msg.sender);
        if (commission <= 0) {
            return fail(Error.NOT_COMMISSION, 0);
        }
