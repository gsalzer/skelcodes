...
// File: original_contracts/MultiSwapper.sol
...
contract MultiSwapper is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    constructor(address whitelist, address gasToken) public {
   ...
   * @param toAmount Minimu destination token amount expected out of this swap
   ...
   */
    ...
    //Same as multiswap just allows different receiver then msg.sender
    function pay(
        ...
        returns (uint256)
    {
        ...
        Utils.transferTokens(address(toToken), receiver, toAmount);

        if (receivedAmount > toAmount) {
            uint rest = receivedAmount.sub(toAmount);

            Utils.transferTokens(address(toToken), payer, rest);
        }
        ...
        returns(uint256)
    {
        ...
        for (uint i = 0; i < path.length; i++) {
            ...
            //TODO: check that percents adds up to 100
            ...
            for (uint j = 0; j < path[i].routes.length; j++) {
                ...
                if (address(_fromToken) == Utils.ethAddress()) {
                    dex.swap.value(fromAmountSlice)(_fromToken, _toToken, fromAmountSlice, 1, route.targetExchange, route.payload);
                }
                else {
                    _fromToken.safeTransfer(route.exchange, fromAmountSlice);

                    dex.swap(_fromToken, _toToken, fromAmountSlice, 1, route.targetExchange, route.payload);
                }
            }
