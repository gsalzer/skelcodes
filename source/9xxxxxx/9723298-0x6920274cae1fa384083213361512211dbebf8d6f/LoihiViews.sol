...
contract LoihiRoot is DSMath {
    ...
    address constant exchange = 0x773943519f7cEa228EB618eA0C0DEe6d3840c68d;
    address constant liquidity = 0x329B4570c0422747Ce4017D52bad8d01D3816876;
    address constant views = address(0);
    address constant erc20 = address(0);
...
contract LoihiViews is LoihiRoot, LoihiDelegators {
    ...
    function viewTargetAmount (uint256 _oAmt, address _oAdptr, address _tAdptr, address _this, address[] memory _rsrvs, address _oRsrv, address _tRsrv, uint256[] memory _weights, uint256[] memory _globals) public returns (uint256 tNAmt_) {
        ...
        tNAmt_ = wmul(_oNAmt, WAD-(_globals[3]/2));
        ...
        if (_globals[5] > _psi) tNAmt_ = add(_oNFAmt, wmul(_globals[4], sub(_globals[5], _psi)));
        ...
        tNAmt_ = wmul(tNAmt_, WAD-(_globals[3]/2));

        return dViewRawAmount(_tAdptr, tNAmt_);

    }

    function viewOriginAmount (uint256 _tAmt, address _tAdptr, address _oAdptr, address _this, address[] memory _rsrvs, address _tRsrv, address _oRsrv, uint256[] memory _weights, uint256[] memory _globals) public returns (uint256 oNAmt_) {
        ...
        oNAmt_ = wmul(_tNAmt, WAD+(_globals[3]/2));
        ...
        oNAmt_ = wmul(oNAmt_, WAD+(_globals[3]/2));

        return dViewRawAmount(_oAdptr, oNAmt_);

    }

    function makeFee (uint256 _bal, uint256 _ideal, uint256 _beta, uint256 _delta) internal returns (uint256 _fee) {
    ...
    function viewSelectiveWithdraw (address[] calldata _rsrvs, address[] calldata _flvrs, uint256[] calldata _amts, address _this, uint256[] calldata _weights, uint256[] calldata _globals) external returns (uint256) {
        ...
        return wmul(shellsBurned_, WAD+(_globals[3]/2));

    }

    function viewSelectiveDeposit (address[] calldata _rsrvs, address[] calldata _flvrs, uint256[] calldata _amts, address _this, uint256[] calldata _weights, uint256[] calldata _globals) external returns (uint256) {
        ...
        return wmul(shellsMinted_, WAD-(_globals[3]/2));
    }

    function viewMintFees (uint256[] memory _balances, uint256[] memory _deposits, uint256[] memory _globals, uint256[] memory _weights, uint256 _nSum, uint256 _oSum) public returns (uint256 psi_) {
    ...
    function viewBurnFees (uint256[] memory _balances, uint256[] memory _withdrawals, uint256[] memory _globals, uint256[] memory _weights, uint256 _nSum, uint256 _oSum) public returns (uint256 psi_) {
    ...
    function viewBalancesAndAmounts (address[] memory _rsrvs, address[] memory _flvrs, uint256[] memory _amts, address _this) private returns (uint256[] memory, uint256[] memory) {
