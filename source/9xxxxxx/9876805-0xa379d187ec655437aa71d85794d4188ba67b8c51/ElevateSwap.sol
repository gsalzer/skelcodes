...
contract ElevateSwap is ERC1155TokenReceiver, CommonConstants {
  ...
  function erc1155Payment(
    ...
    require(token.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, bytes(msg.data)));
    emit PaymentSent(_id);
  }
  ...
  function receiverSpend(
    ...
    if (_tokenAddress == address(0)) {
      ...
      IERC1155 token = IERC1155(_tokenAddress);
      require(token.safeTransferFrom(address(this), msg.sender, _tokenId, _amount, bytes(msg.data)));
    } else {
      IERC20 token = IERC20(_tokenAddress);
