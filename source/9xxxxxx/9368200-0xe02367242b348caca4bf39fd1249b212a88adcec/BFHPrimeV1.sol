...
contract BFHPrimeV1 is Operatable {
    ...
    function buyPrimeRight(bytes calldata _signature, bytes32 _hash, uint256 _blockNum) external payable whenNotPaused() {
        require(isApplicable(msg.sender, _blockNum, block.number), "block num error");
        require(validateSig(msg.sender, _hash, _blockNum, msg.value, _signature), "invalid signature");
        lastSignedBlock[msg.sender] = _blockNum;
        emit BuyPrimeRight(msg.sender, _blockNum);
    }

    function validateSig(address _from, bytes32 _hash, uint256 _blockNum, uint256 _priceWei, bytes memory _signature) public view returns (bool) {
        require(validator != address(0));
        address signer = recover(ethSignedMessageHash(encodeData(_from, _hash, _blockNum, _priceWei)), _signature);
        return (signer == validator);
    }

    function encodeData(address _from, bytes32 _hash, uint256 _blockNum, uint256 _priceWei) public pure returns (bytes32) {
