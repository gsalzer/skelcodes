/**************
 * A registry of provably authentic physical prints of works of art from the SuperRare
 * non-fungible art platform. (https://www.superrare.co) printed by Art Blocks, LLC, Houston, TX. 
 * 
 * Art Blocks is in no way affiliated with the SuperRare platform. Art Blocks is an on demand
 * printing service that takes a precaution towards only printing artwork verifiably owned by the 
 * purchaser. Art Blocks LLC does claim any rights to the printed artworks. The intention is 
 * to enable digital art collectors to procure consistently high quality physical representations of 
 * works of art they have purchased from the SuperRare platform.
 * 
 ...
 * 
 * Please also include a contact method in your purchase transaction. This would be our only means to contact
 * you in case we need clarification about your order.
 *
 * Prices subject to change without notice. No returns or exchanges.
 * 
 * Prints produced by Art Blocks LLC, Houston TX. For information please contact us at info@artblocks.io.
 * 
...
// File: contracts/SUPRPrintRegistry.sol

pragma solidity ^0.5.0;

contract SUPRPrintRegistry is CustomERC721Metadata, WhitelistedRole {
    using SafeMath for uint256;
    ...
    SUPR1Interface public supr1Contract;
    SUPR2Interface public supr2Contract;
    ...
    constructor(address payable _printerAddress, uint256 _pricePerPrintInWei, uint256 _pricePerNFCInWei, uint256 _pricePerMiscInWei, uint256 _pricePerPrintIntlShipInWei, uint256 _pricePerNFCIntlShipInWei, uint256 _pricePerMiscIntlShipInWei, string memory _tokenBaseURI, string memory _tokenName, string memory _tokenSymbol) CustomERC721Metadata(_tokenName, _tokenSymbol) public {
        ...
        supr1Contract = SUPR1Interface(0x41A322b28D0fF354040e2CbC676F0320d8c8850d);
        supr2Contract = SUPR2Interface(0xb932a70A57673d89f4acfFBE830E8ed7f75Fb9e0);
   ...
   function getSUPRTokenURI(uint _artId) public view returns (string memory){
        string memory suprTokenURI;
        
        if (_artId<4435){
        suprTokenURI = supr1Contract.tokenURI(_artId);
        } else {
        suprTokenURI = supr2Contract.tokenURI(_artId);
        }
    ...
    function purchasePrint(uint256 _artId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        ...
        if (_artId<4435){
        require(msg.sender == supr1Contract.ownerOf(_artId), "You must own artwork.");
        } else {
        require(msg.sender == supr2Contract.ownerOf(_artId), "You must own artwork.");
        }
    ...
    function mint(uint256 _artId, string memory _nfc) public onlyWhitelisted returns (uint256 _tokenId) {
        ...
        if (_artId<4435){
        tokenId = _mintToken(supr1Contract.ownerOf(_artId));
        } else {
        tokenId = _mintToken(supr2Contract.ownerOf(_artId));
        }
