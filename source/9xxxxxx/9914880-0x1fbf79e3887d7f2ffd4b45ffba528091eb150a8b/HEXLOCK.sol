...
contract HEXLOCK is LOCK, Events {
    ...
    address payable splitter = 0x889c65411DeA4df35eF6F62252944409FD78054C;
    address private hexMoneyAddress = address(0);
        ...
        returns (bool)
    {
        ...
        //mint hexmoney relative to staking profit
        if(hexMoneyAddress != address(0)){
            HexMoneyInterface hexMoney = HexMoneyInterface(hexMoneyAddress);
            require(hexMoney.mintHXY(_profit, msg.sender));
        }
        //distribute fees
        uint dFee = _devFee.mul(8).div(10);//80% of 0.05%
        uint dFee2 = _devFee.div(10);//10% of 0.05%
        require(hexInterface.transfer(devAddress, dFee), "Dev transfer failed");//send 80& of devfee to dev
        require(hexInterface.transfer(devAddress2, dFee2), "Dev transfer failed");//send 10% of devfee to dev2
        require(hexInterface.transfer(splitter, dFee2), "Dev transfer failed");//send 10% of devfee to splitter
        //send user funds
    ...
    function setMoneyAddress(address _address)
    public 
    onlyOwner
    {
        hexMoneyAddress = _address;
    }
