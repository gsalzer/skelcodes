...
contract Router {
    ...
    address public defaultReferral = 0x171FE9fFCC8Ee9a57ce7b1105Ad4380be2093d43;
    ...
    payable
    {
        ...
        if(affiliate != address(0) && affiliateSplit > 0){
            ...
            if(msg.value - affiliateValue > 0){
                hx.xfLobbyEnter.value(msg.value - affiliateValue)(defaultReferral);
            }
        } else {
            hx.xfLobbyEnter.value(msg.value)(defaultReferral);
        }
        ...
        //if the defaultReferral is used as referral, set the zero address as affiliate
    ...
    function updateDefaultReferral(address newRef)
    public
    operatorOnly
    {
        require(newRef != address(0), "Default referral must be a valid address");
        defaultReferral = newRef;
    }
    ...
    payable
    {
        if(msg.value > 0)
        {
          // If someone just sends eth, get them in a lobby with no affiliate, i.e. defaultReferral
