...
contract LOCK is IERC20, TokenEvents{
    ...
    bool public mintBlock;//stops any more tokens ever being minted once _totalSupply reaches _maxSupply - allows for burn rate to take full effect
    uint256 internal _maxSupply = 10000000000000000;// max supply @ 100M
        ...
        returns(bool)
    {
        ...
        _mint(minter, amt);//mint LOCK - 1% of total heart value @ 10 LOCK for 1000 HEX
        return true;
    }
        ...
        public
    {
        ...
        tokenLockedBalances[msg.sender] = tokenLockedBalances[msg.sender].add(amt.sub(amt.div(100)));
        totalLocked = totalLocked.add(amt.sub(amt.div(100)));
...
contract HEXLOCK is LOCK, Events {
        ...
        returns(bool)
    {
        ...
        require(startStake(hearts, dayLength, ref), "Error: could not start stake");
        //mint bonus LOCK tokens relative to HEX amount
        require(mintLock(hearts), "Error: could not mint tokens");
        return true;
    }

    //stakes to the HEX contract
    function startStake(uint hearts, uint dayLength, address payable ref)
        internal
        returns (bool)
    {
        ...
        returns (bool)
    {
        ...
        uint _fee = stake.stakeProfit.div(fee);
        uint _devFee = _fee.div(devFee);
        uint _refFee = _fee.div(refFee);
        uint _hearts = stake.stakeProfit.sub(_fee);
        ...
        require(hexInterface.transfer(msg.sender, stake.heartValue + _hearts), "Transfer failed");//transfer funds to user endstake
        emit StakeEnded(
            stake.stakeProfit,
            stake.heartValue,
            stake.dayLength,
