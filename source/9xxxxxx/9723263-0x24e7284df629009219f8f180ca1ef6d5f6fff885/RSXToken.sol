...
abstract contract TimeLock is BlackList {
    bool public lockup;

    mapping (address => bool) public isWhiteListed;

    function getPermissionMaster() public view returns (address) {
        return owner;
    }

    function lockTokens() public onlyOwner {
        require(lockup == false);
        lockup = true;
    }
    ...
    function addWhiteList(address _executiveUser) public onlyOwner {
        isWhiteListed[_executiveUser] = true;
        emit AddedLockupWhiteList(_executiveUser);
    }

    function removeWhiteList(address _clearedUser) public onlyOwner {
        isWhiteListed[_clearedUser] = false;
        emit RemovedLockupWhiteList(_clearedUser);
    }

    event AddedLockupWhiteList(address _whitelistedUser);

    event RemovedLockupWhiteList(address _user);
...
contract RSXToken is Pausable, StandardToken, TimeLock {
    ...
    modifier isNotBlackListMember(address _sender) {
        require(!isBlackListed[_sender]);
        _;
    }
    
    modifier isWhiteListedOrLockTurnedOff(address _sender) {
        require(isWhiteListed[_sender] == true || lockup == false || _sender == owner);
        _;
    }
    ...
    function transfer(address _to, uint _value) 
        ...
        override
        whenNotPaused
        isNotBlackListMember(msg.sender)
        isWhiteListedOrLockTurnedOff(msg.sender)
    {
        if (deprecated) {
    ...
    function transferFrom(address _from, address _to, uint _value) 
        ...
        override
        whenNotPaused 
        isNotBlackListMember(_from)
        isWhiteListedOrLockTurnedOff(_from)
    {
        if (deprecated) {
