...
/**
 * @title Syt
 * @dev The Syt contract is Claimable, and provides ERC20 standard token.
 */
contract Syt is Claimable, Contactable, HasNoEther, FreezableToken {
    // @dev Constructor initial token info
    function Syt(){
        uint256 _decimals = 18;
        uint256 _supply = 2000000000*(10**_decimals);
        ...
        symbol = "SYT";
        decimals = _decimals;
        contactInformation = "Shuyi Token Email:info@shuyi.co";
    }
}


contract SytTokenLock is Ownable, HasNoEther {
    using SafeMath for uint256;
     ...
     *
     */
    function SytTokenLock(address _owner, address _token) {
        require(_owner != 0x0);
        require(_token != 0x0);
