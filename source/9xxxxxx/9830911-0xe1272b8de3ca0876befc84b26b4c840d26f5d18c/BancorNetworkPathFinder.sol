...
contract BancorNetworkPathFinder is ContractRegistryClient {
    ...
    */
    function generatePath(address _sourceToken, address _targetToken) public view returns (address[] memory) {
        ...
        for (uint i = 0; i < anchorTokens.length; i++) {
            address[] memory sourcePath = getPathForAnchor(anchorTokens[i], _sourceToken, converterRegistry);
            address[] memory targetPath = getPathForAnchor(anchorTokens[i], _targetToken, converterRegistry);
    ...
    function weightOf(address[] memory path) public view returns(uint256) {
        return IERC20Token(path[0]).balanceOf(path[1]) *
            IERC20Token(path[path.length - 1]).balanceOf(path[path.length - 2]);
    }
    ...
    */
    function getPathForAnchor(address anchorToken, address _token, IBancorConverterRegistry _converterRegistry) private view returns (address[] memory) {
        ...
        for (uint256 n = 0; n < smartTokens.length; n++) {
            ...
            for (uint256 i = 0; i < connectorTokenCount; i++) {
                ...
                if (connectorToken != _token) {
                    address[] memory path = getPathForAnchor(anchorToken, connectorToken, _converterRegistry);
                    
                    if (path.length > 0) {
                        address[] memory extendedPath = getExtendedArray(_token, smartTokens[n], path);
                        uint256 pathWeight = weightOf(extendedPath);
                        if (pathWeight > bestPathWeight) {
                            bestPathWeight = pathWeight;
