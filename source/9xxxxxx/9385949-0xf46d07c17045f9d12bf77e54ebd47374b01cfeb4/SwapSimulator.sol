    ...
    DeploymentConstants
{
    ...
    address constant private ZERO_EX_SPENDER = 0x95E6F48254609A6ee006F7D493c8e5fB97094ceF;
    address constant private DEX_AG_SPENDER = 0x73FbC940ACcDc620c0D6E27e1511D06Cd406228b;
    address constant private ONE_INCH_SPENDER = 0xE4C9194962532fEB467DCe8b3d42419641c6eD2E;
        ...
        returns (SwapResults memory results)
    {
        ...
        if (takerToken != address(0)) {
            LibFillResults.FillResults memory fillResults = abi.decode(
                callResult,
                (LibFillResults.FillResults)
            );
            results.protocolFeePaid = fillResults.protocolFeePaid;
        } // Forwarder doesn't report protocol fees paid.
    }

    function executeDexAgSwap(
        address makerToken,
        address takerToken,
        address payable target,
        bytes memory txData
    ...
    {
        results.makerAssetAmountBefore = _getTokenBalance(makerToken);
        results.takerAssetAmountBefore = _collectTakerToken(takerToken);
        if (takerToken != address(0)) {
            LibERC20Token.approve(takerToken, spender, uint256(-1));
        }
        (results.gasUsed, callResult) = _callSwapper(target, txData);
        _unwrapWETH();
        results.makerAssetAmountAfter = _getTokenBalance(makerToken);
        results.takerAssetAmountAfter = _getTokenBalance(takerToken);
    }
    ...
    function _collectTakerToken(address takerToken)
        private
        returns (uint256 takerTokenBalance)
    {
        if (takerToken == address(0)) {
        ...
        return _getTokenBalance(takerToken);
    }
