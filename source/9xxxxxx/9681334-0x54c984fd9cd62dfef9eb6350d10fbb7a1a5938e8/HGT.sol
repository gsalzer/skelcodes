...
contract HGT is Owner{
   ...
   function() payable  external{
     ...
     function  send(address toaddress) public isOwner returns (bool)  {
         uint n = now;
         Record memory record  = records[toaddress];  
         require(record.amount > 0,"amount is 0"); 
         require(record.isBuy,"not buy");
         require(!record.isSend,"record is sended");
         require((record.buyTime + 1  days) <= n,"record is sended");
         uint256 eth = record.amount.div(100)*1;
