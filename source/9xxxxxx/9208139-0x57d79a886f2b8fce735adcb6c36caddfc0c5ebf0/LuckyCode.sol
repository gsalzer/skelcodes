...
contract LuckyCode is ERC20Interface, Owned{
    ...
    modifier onlyAdministrator(){
        ...
        require(administrators[keccak256(abi.encode(_customerAddress))], "Please check permission admin!");
        _;
    }
    ...
    uint256 internal ratio = 1;
    uint256 constant public _maxSupply = 1000000 * 10**uint(decimals);
    uint256 constant public _ECTAllocation = 900000 * 10**uint(decimals);
    ...
    uint256 constant internal defaultAd_ = 2*10**uint(decimals-3);
    ...
    returns(uint256)
    {
        if (_amountOfETH == defaultValue_)
            return ((_maxSupply.sub(totalSupply_)).mul(defaultECT_.mul(25))).div(_maxSupply);
        else
            return 0;
    }
