...
contract EthMLM is Ownable {
    ...
    event IntroducerLostMoney(uint indexed _referrer, uint indexed _referral, uint _star, uint _money, uint _time);
                ...
                if(_star==1){
                    for(uint i=1;i<=6;i++){
                        if(referrerFinal==_introducerID){
                            if(users[referrerFinal].starActive[i] == false){//level paid or not by introducer
                                emit IntroducerLostMoney(referrerFinal, userList[msg.sender], _star, money, now);
                                introducer = mainAddress;
