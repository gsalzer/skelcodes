...
contract DfAirdrop is FundsMgr {
    ...
    function financeMgrCall(address to, bytes memory _data, uint256 amountEther) public payable onlyOwner {
        bytes32 response;
        assembly {
    ...
    function run(uint256 coef, uint8 profitPercent, bytes memory data, uint256 saiToBuyEther, uint8 ethType, uint256 maxUsers) public payable onlyOwner {
    ...
    function stop(bytes32 cup, uint256 etherNeeded, bytes memory _saiData, bytes memory _usdData, uint256 minAmountAllowed) public payable onlyOwner {
        require(isDealOpen);
    ...
    function finalize(uint256 depositedUSD) public onlyOwner {
        require(isDealOpen);
