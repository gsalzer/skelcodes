...
// File: contracts/base/WrappedTokenSwap.sol
...
contract WrappedTokenSwap is Withdrawable, Pausable, Destructible, WithFee, IErc20Swap
{
  ...
  // The getUnderlyingPayload() abstract function is used in the provided isWrap() implementation,
  // it must return the payload for a call against a wrapped token to get the underlying token address
  // If not applicable, you must reimplement the isWrap() function
  ...
  // If reimplemented it  must revert if the pair is not valid
...
// File: contracts/base/AdjustedRateWrappedTokenSwap.sol
 ...
 * @dev A swap asset contract. The offerAmount and wantAmount are collected and sent into the contract itself.
 */
contract BZxLoanTokenSwap is AdjustedRateWrappedTokenSwap
{
  using SafeMath for uint;
  using SafeStaticCallERC20 for IBadStaticCallERC20;
  ...
  function getUnderlyingPayload() private pure returns (bytes memory){
    return abi.encodeWithSignature("loanTokenAddress()");
  }

  function wrap(address src, uint unwrappedAmount, address dest) private {
    IBadStaticCallERC20(src).safeApprove(dest, unwrappedAmount);
    require(IBZxLoanToken(dest).mint(address(this), unwrappedAmount) > 0, "Cannot mint");
  }

  function unwrap(address src, uint wrappedAmount, address /*dest*/) private {
    require(IBZxLoanToken(src).burn(address(this), wrappedAmount) > 0, "Cannot burn");
  }

  function getExchangedAmount(address src, uint srcAmount, address dest) private view returns(uint) {
    if (isWrap(src, dest)) {
      uint rate = IBZxLoanToken(dest).tokenPrice();
      return srcAmount.mul(expScale).div(rate);
    } else {
      uint rate = IBZxLoanToken(src).tokenPrice();
