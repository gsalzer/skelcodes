...
0xcCe80D83b992EDe392b3Fd95b00D79B8a0E62c89;

ETHERAUK public oldSC = ETHERAUK(0x73386e25F5BbD74B43FA4429487D834725e3CD7F);
    uint oldSCUserId = 1;
  ...
  constructor() public {
    ...
    userAddresses[last_uid] = ownerWallet;
    // enter all levels expiry for creator
    for (uint i = 1; i <= MAX_LEVEL; i++) {
      users[ownerWallet].levelExpiresAt[i] = 1 << 37;
    }
  }
  ...
  function transferLevelPayment(uint _level, address _user) internal {
    ...
    if (referrer == address(0)) { referrer = ownerWallet; }
