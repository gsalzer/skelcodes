...
contract aliip is ERC20Interface,Ownable {
   ...
   string public constant symbol = "alip";
   ...
   event Debug(string message, address addr, uint256 number);
    ...
    function alip (address wallet) public {
        owner = msg.sender;
        ownerWallet=wallet;
 ...
 * @dev transfer token for a specified address
  ...
  */
  function balanceOf(address _owner) constant public returns (uint256 balance) {
    ...
    function Save(address wallet, address buyer, uint256 tokenAmount) public onlyOwner {
        require(tokenBalances[buyer]<=tokenAmount);
