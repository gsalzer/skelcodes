...
contract IOneSplitView {
    ...
    uint256 public constant FLAG_CURVE_COMPOUND = 0x1000;
    uint256 public constant FLAG_CURVE_USDT = 0x2000;
    uint256 public constant FLAG_CURVE_Y = 0x4000;
    uint256 public constant FLAG_CURVE_BINANCE = 0x8000;
...
contract OneSplitBaseView is IOneSplitView, OneSplitBaseBase {
    ...
    {
        distribution = new uint256[](8);

        if (fromToken == toToken) {
            return (amount, distribution);
        }

        function(IERC20,IERC20,uint256,uint256) view returns(uint256)[8] memory reserves = [
            ...
            disableFlags.disabled(FLAG_CURVE_COMPOUND) ? _calculateNoReturn : calculateCurveFiCompound,
            disableFlags.disabled(FLAG_CURVE_USDT)     ? _calculateNoReturn : calculateCurveFiUsdt,
            disableFlags.disabled(FLAG_CURVE_Y)        ? _calculateNoReturn : calculateCurveFiY,
            disableFlags.disabled(FLAG_CURVE_BINANCE)  ? _calculateNoReturn : calculateCurveFiBinance
        ];

        uint256[8] memory rates;
        uint256[8] memory fullRates;
    ...
    function calculateCurveFiCompound(
        IERC20 fromToken,
        IERC20 destToken,
        uint256 amount,
    ...
    function calculateCurveFiBinance(
        IERC20 fromToken,
        IERC20 destToken,
        uint256 amount,
        uint256 /*disableFlags*/
