...
// File: contracts/interfaces/IDfFinanceMgr.sol
...
interface IOtc {
    ...
    function getSellAmount(IToken dest, IToken src, uint256 srcAmount) external view returns (uint256);
    function getBuyAmount(IToken dest, IToken src, uint256 srcAmount) external view returns (uint256);
}

// File: contracts/interfaces/IPep.sol
...
// File: contracts/interfaces/ITub.sol
...
// File: contracts/interfaces/IAffiliateProgram.sol
...
// File: contracts/interfaces/IDfProxyBet.sol
...
contract DfFinanceMgrOpenCdp is Initializable, DSMath, FundsMgrUpgradable {
    ...
    IDfFinanceMgr public dfManagerAddress;
    ...
    function initialize() initializer public {
        ...
        tub = ITub(0x448a5065aeBB8E423F0896E6c5D525C040f59af3);  // MainNet SaiTub: 0x448a5065aeBB8E423F0896E6c5D525C040f59af3
        inFee = 0;
    ...
    function setDfManagerAddress(address payable newAddress) onlyOwner public {
        ...
        dfManagerAddress = IDfFinanceMgr(newAddress);
    ...
    function changeOtc(IOtc _newOtc, bool allow) onlyOwner public {
    ...
    function addOtc(IOtc newOtc) onlyOwner public {
        allOtc.push(newOtc);
        allowedOtc[address(newOtc)] = true;
    }

    function removeOtc(IOtc otcToRemove) onlyOwner public {
    ...
    function exchange(IToken fromToken, uint256 maxFromTokenAmount, IToken toToken, uint256 minToTokenAmount, bytes memory _data) internal returns (uint256)
    {
    ...
    function findBestOtc(address tokenPay, address tokenNeeded, uint256 amountNeeded) view public returns(IOtc bestOtc, uint256 bestAmount) {
        uint256 index = allOtc.length;
        bestOtc = allOtc[index - 1];
        bestAmount = bestOtc.getBuyAmount(IToken(tokenPay), IToken(tokenNeeded), amountNeeded);
        index--;
        while (index > 0) {
            uint256 amount = allOtc[index - 1].getBuyAmount(IToken(tokenPay), IToken(tokenNeeded), amountNeeded);
    ...
    function dealViaOtc(address newOwner, uint256 coef, uint8 profitPercent, IOtc otc, uint256 maxSai) public payable returns(bytes32 cup) {
    ...
    function _deal(address newOwner, uint8 profitPercent, uint256 coef, uint256 valueEth, bytes memory data, uint256 saiToBuyEther, uint8 ethType) internal returns(bytes32 cup)
    {
        ...
        // return an extra eth loan
        transferEthInternal(address(loanPool), extraEth);

        tub.give(cup, address(dfManagerAddress));
        dfManagerAddress.setupCup(newOwner, cup, valueEth, profitPercent, priceETH(), currentFeeForBonusEther);
    }

    function _deal_otc(address newOwner, uint256 coef, uint256 valueEth, IOtc otc, uint256 maxSai) internal returns(bytes32 cup)
    {
        if (newOwner == address(0x0)) newOwner = msg.sender;
    ...
    // additional variables (after deployment)
