...
contract Aavee is IExchange {
    ...
    constructor(uint16 _refCode) public {
        refCode = _refCode;
    }

    //TODO: restrict to owner
    function setRefCode(uint16 _refCode) external {
        refCode = _refCode;
        //TODO: fire an edit event
    }
    ...
    function swap(
        ...
        bool fromAToken = address(fromToken) == address(data.aToken);

        Utils.approve(address(data.aToken), address(fromToken));
        ...
        if (fromAToken) {
            ...
            if (address(fromToken) == Utils.ethAddress()) {
                IAaveLendingPool(data.aToken).deposit.value(fromAmount)(fromToken, fromAmount, refCode);
            }
            else {
                IAaveLendingPool(data.aToken).deposit(fromToken, fromAmount, refCode);
            }
        }
