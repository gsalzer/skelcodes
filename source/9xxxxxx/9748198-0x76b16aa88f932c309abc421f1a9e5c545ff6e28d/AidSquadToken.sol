...
contract ERC777PullPayment {

    mapping(address => uint) balances;
    
    // ERRORS
    string constant private amountBiggerThanBalance = "err01";
    string constant private mintDidNotSucceed = "err02";
    ...
    function withdrawBalance(uint amount) external {
        ...
        require(amount <= balance, amountBiggerThanBalance);
        balances[msg.sender] = balance - amount;
        require(internalMint(msg.sender, amount), mintDidNotSucceed);
        emit BalanceWithdrawn(msg.sender, amount);
    }

    function increaseBalance(address to, uint amount) internal {
        balances[to] += amount;
        emit BalanceIncreased(msg.sender, to, amount);
    }
    ...
    function internalMint(address to, uint amount) internal returns (bool);
...
contract AidSquadToken is Ownable, Pausable, ERC777, ERC777PullPayment {
  using SafeMath for uint256;
  using ECDSA for bytes32; // Lib de OpenZeppelin con helpers para ecrecovery

  // Token constants
  string constant private __name = "Aid Squad Token";
  string constant private __symbol = "AIDTKN";

  // ERRORS
  string constant private hashDoesNotMatch = "err01";
  string constant private signatureIsNotValid = "err02";
  string constant private arrayDymensionsDoNotMatch = "err03";
  string constant private tooManyCauses = "err04";
  string constant private hashAlreadyUsed = "err05";

  uint8 constant private __maxCausesPerBlock = uint8(33);
  ...
  constructor() ERC777(__name, __symbol, emptyOperatorInitFunction())
    public {
      _mint(msg.sender, msg.sender, 0, "", "");
  }

  function emptyOperatorInitFunction() private pure returns (address[] memory empty) {
    empty = new address[](0);
  }

  function hashGeneration(
    ...
    returns (bytes32)
  {
    ...
    bytes32 msgHash = keccak256(abi.encodePacked(causes, rewards, random, address(this)));
    return msgHash;
  }
