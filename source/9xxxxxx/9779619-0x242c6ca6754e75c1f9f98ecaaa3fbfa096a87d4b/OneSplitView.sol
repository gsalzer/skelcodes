...
contract IOneSplitView {
    ...
    uint256 public constant FLAG_DISABLE_UNISWAP_CHAI = 0x200000; // Works only when one of assets is ETH or FLAG_ENABLE_MULTI_PATH_ETH
...
contract OneSplitBaseView is IOneSplitView, OneSplitBaseBase {
    ...
    {
        distribution = new uint256[](11);

        if (fromToken == toToken) {
            return (amount, distribution);
        }

        function(IERC20,IERC20,uint256,uint256) view returns(uint256)[11] memory reserves = [
            ...
            disableFlags.check(FLAG_DISABLE_UNISWAP) ||
            !disableFlags.check(FLAG_ENABLE_UNISWAP_COMPOUND) ? _calculateNoReturn : calculateUniswapCompound,
            disableFlags.check(FLAG_DISABLE_UNISWAP) ||
            disableFlags.check(FLAG_DISABLE_UNISWAP_CHAI)     ? _calculateNoReturn : calculateUniswapChai
        ];

        uint256[11] memory rates;
        uint256[11] memory fullRates;
    ...
    function calculateUniswapCompound(
        ...
        if (!disableFlags.check(FLAG_ENABLE_UNISWAP_COMPOUND)) {
            return 0;
        }
    ...
    function calculateUniswapChai(
        ...
        if (disableFlags.check(FLAG_DISABLE_UNISWAP_CHAI)) {
            return 0;
        }
...
contract OneSplitBase is IOneSplit, OneSplitBaseBase {
    ...
    function _swap(
        ...
        function(IERC20,IERC20,uint256) returns(uint256)[11] memory reserves = [
            _swapOnUniswap,
            _swapOnKyber,
            _swapOnBancor,
            _swapOnOasis,
