...
contract DualSwap is Pausable, Destructible, Withdrawable, RateNormalization
{
  ...
  constructor (IErc20Swap [2] memory _swaps) public {
    swapsStore = _swaps;
//    swapsStore = [_swaps[0], _swaps[1]];
  }

  function () external payable {}

  function setSwaps(IErc20Swap [2] memory _swaps) public onlyOwner {
    swapsStore = _swaps;
  }

  function getRate(address src, address dest, uint256 srcAmount) external view returns(uint rate, uint slippage) {
    if (paused) return (0, 0);
    (rate, slippage,) = _getRateAndPath(src, dest, srcAmount);
  }

  function _getRateAndPath(address src, address dest, uint256 srcAmount) internal view
    returns(uint rate, uint slippage, IErc20Swap [2] memory path)
  {
    require(src != ETHER && dest != ETHER, "must be src != ETHER && dest != ETHER");
    (rate, slippage) = swapsStore[0].getRate(src, ETHER, srcAmount);
    if (rate > 0) {
      path = [swapsStore[0], swapsStore[1]];
    } else {
      path = [swapsStore[1], swapsStore[0]];
      (rate, slippage) = path[0].getRate(src, ETHER, srcAmount);
    }
    (uint stepRate, uint stepSlippage) = path[1].getRate(ETHER, dest, srcAmount.mul(rate).div(rateUnit));
    rate = rate.mul(stepRate).div(rateUnit);
    slippage = slippage.mul(stepSlippage).div(rateUnit);
  }
  ...
  function swap(address src, uint srcAmount, address dest, uint maxDestAmount, uint minConversionRate)
    ...
    whenNotPaused
  {
    (uint normalizedRate,, IErc20Swap [2] memory path) = _getRateAndPath(src, dest, srcAmount);
    require(normalizedRate > 0, "rate == 0");
    // validate maxDestAmount
    uint toTradeAmount;
    if (maxDestAmount != 0) {
      uint expectedRate  = denormalizeRate(src, dest, normalizedRate);
      uint expectedDestAmount = srcAmount.mul(expectedRate).div(rateUnit);
      toTradeAmount = expectedDestAmount > maxDestAmount
      ? maxDestAmount.mul(rateUnit).div(expectedRate)
      : srcAmount;
    } else {
      toTradeAmount = srcAmount;
    }

    // empty unexpected balances of all intermediate tokens
    _emptyBalance(src);
    _emptyBalance(ETHER);
    _emptyBalance(dest);
    ...
    // get user's tokens
    require(
      IBadStaticCallERC20(src).allowance(msg.sender, address(this)) >= toTradeAmount,
      "ERC20 allowance < toTradeAmount"
    );
    IERC20(src).safeTransferFrom(msg.sender, address(this), toTradeAmount);

    // do swaps
    if (IBadStaticCallERC20(src).allowance(address(this), address(path[0])) > 0) {
      IBadStaticCallERC20(src).safeApprove(address(path[0]), 0);
    }
    IBadStaticCallERC20(src).safeApprove(address(path[0]), toTradeAmount);
    path[0].swap(src, toTradeAmount, ETHER, BIGGEST_AMOUNT, 1);
    // workaround! address(this).balance throws "invalid opcode"
    uint ethersToSwap = address(uint160(address(this))).balance;
    path[1].swap.value(ethersToSwap)(ETHER, ethersToSwap, dest, BIGGEST_AMOUNT, 1);
    uint destAmount = IBadStaticCallERC20(dest).balanceOf(address(this));

    // check minConversionRate
    uint rate = destAmount.mul(rateUnit).div(toTradeAmount);
    require(normalizeRate(src, dest, rate) >= minConversionRate, "cannot satisfy minConversionRate");

    // finalize
    IERC20(dest).safeTransfer(msg.sender, destAmount);
    emit LogTokenSwap(
      msg.sender,
      src,
      toTradeAmount,
  ...
  function _emptyBalance(address token) private {
    if (token == ETHER) {
      // workaround! address(this).balance throws "invalid opcode"
      uint balance = address(uint160(address(this))).balance;
      if (balance > 0) {
        address payable target = address(uint160(owner()));
        target.transfer(balance);
        emit UnexpectedBalance(token, balance);
      }
    } else {
      uint balance = IBadStaticCallERC20(token).balanceOf(address(this));
      if (balance > 0) {
