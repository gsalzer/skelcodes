/*
Concept by Aaron Koenig
Art by Max Cryptohead
Smart Contracts by Andreas Petersson
*/
...
// File: contracts\TwentyoneHeads.sol

/*
Concept by by Aaron Koenig
Art by Max Cryptohead
Smart Contracts by Andreas Petersson, Capacity Blockchain Solutions
...
contract TwentyOneHeads is ERC721, ERC721Enumerable, ERC721Metadata("21 Heads - Portraits of Crypto Pioneers", "21HEADS") {
    ...
    address public aaronAddress;
    address public andreasAddress;

    mapping(address => bool) minters;

    constructor(address _createControl, address _tokenAssignmentControl, address _aaronAddress, address _andreasAddress)
    public
    {
        createControl = _createControl;
        tokenAssignmentControl = _tokenAssignmentControl;
        uribase = "https://21heads.com/metadata/";
        aaronAddress = _aaronAddress;
        andreasAddress = _andreasAddress;
        require(aaronAddress != address(0));
        require(andreasAddress != address(0));

        minters[createControl] = true;
        minters[aaronAddress] = true;
        minters[andreasAddress] = true;
    }

    modifier onlyCreateControl()
    {
        require(minters[msg.sender], "only for the 3 minters");
        _;
    }
    ...
    function mintCharacter(uint256 _characterId)
    public
    onlyCreateControl
    {
        require(_characterId >= 1);
        require(_characterId <= 21);

        uint256 _tokenIdStart = _characterId * 21 + 1;


        for (uint256 i = 0; i < 19; i++) {
            create(_tokenIdStart + i, aaronAddress);
        }

        for (uint256 i = 19; i < 21; i++) {
            create(_tokenIdStart + i, andreasAddress);
        }
