...
contract CRC is IERC20 {
    ...
    function buyref(uint16 coin_id, uint256 amount, address referral) public returns (bool) {
        Coin memory coin = _coins[coin_id];
        require(coin.status & 1 != 0, "Coin in not active");
        uint256 coin_amount = amount;
        if(amount == 0){
            coin_amount = IERC20(coin.token).allowance(msg.sender, address(this));
        }
        require(coin_amount > 0, "No funds avaialble");
        IERC20(coin.token).transferFrom(msg.sender, address(this), coin_amount);
    ...
    function transferCoinByID(uint16 coin_id, address to, uint256 amount) public onlyOwner returns(bool){
        Coin memory curcoin = _coins[coin_id];
        IERC20(curcoin.token).transfer(to, amount);
        return true;
    }
