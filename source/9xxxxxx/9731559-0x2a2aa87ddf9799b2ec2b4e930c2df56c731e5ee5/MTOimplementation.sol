...
contract MTO is ERC20Detailed, ERC20Pausable {
    ...
    modifier notFrozen(address _holder) {
        require(!frozenAccounts[_holder]);
        _;
    }
    
    constructor() ERC20Detailed("MTO", "MTO", 18) public  {
        
        _mint(msg.sender, 56000000 * (10 ** 18));
    }
    
    /*
    constructor() ERC20Detailed("", "", 0) public  {
    }
    */
    ...
    function transferTxFrom(address from, address to, uint256 value) public notFrozen(from) returns (bool) {
    ...
    function transferTx(address to, uint256 value) public notFrozen(msg.sender) returns (bool) {
    ...
    function lockAddress(address holder, uint256 value, uint256 releaseTime) public onlyPauser returns (bool) {
    ...
    function transferTxWithLock(address holder, uint256 value, uint256 releaseTime) public onlyPauser returns (bool) {
        _transfer(msg.sender, holder, value);
        _lock(holder,value,releaseTime);
        return true;
    }
    
    function unlockAddress(address holder, uint256 idx) public onlyPauser returns (bool) {
        require( timelockList[holder].length > idx, "There is not lock info.");
        _unlock(holder,idx);
        return true;
    }

    function freezeAddress(address holder) public onlyPauser returns (bool) {
        require(!frozenAccounts[holder]);
        frozenAccounts[holder] = true;
        emit Freeze(holder);
        return true;
    }

    function unfreezeAddress(address holder) public onlyPauser returns (bool) {
        require(frozenAccounts[holder]);
...
contract MTOimplementation is MTO, ERC20Burnable{
