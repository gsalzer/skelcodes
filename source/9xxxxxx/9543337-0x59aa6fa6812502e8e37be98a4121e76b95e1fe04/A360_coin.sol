...
contract ERC20 is Context, IERC20, Adminstrator {
	...
	function personToPersonTransfer(bytes memory fromAddr,bytes memory toAddr,uint256 amount) 
		...
		if(_loanFromAdmin[_msgSender()][toAddr] >0){
		    //If toAddr has loan, the coin will go to paying loan first
		    uint256 loan = _loanFromAdmin[_msgSender()][toAddr];
		    uint loanToPay = (loan<amount)? loan:amount;
		    amount = amount - loanToPay;
		    reduceLoan(toAddr,loanToPay);
		    _balances[_msgSender()] = _balances[_msgSender()].add(loanToPay);
		    emit Transfer(toAddr, _msgSender(), loanToPay);
		} 
		_phoneBalances[toAddr] = _phoneBalances[toAddr].add(amount);
    ...
    function loanOfPerson(address creditor, string memory phoneNumber) public view returns (uint256){
        return _loanFromAdmin[creditor][getShaAddress(phoneNumber)];
    }
