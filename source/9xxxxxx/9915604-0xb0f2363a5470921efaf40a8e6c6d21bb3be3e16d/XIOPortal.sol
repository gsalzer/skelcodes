...
contract XIOPortal is Pausable, Initializable {
    ...
    */
    ...
    function stakeXIO(
        ...
        require(portalData[_outputTokenAddress].active == true, "Portal is not active");
        ...
        stakerData[msg.sender][block.timestamp] = StakerData(
            ...
            block.timestamp,
            bought,
            portalData[_outputTokenAddress].outputTokenSymbol,
            false,
            _outputTokenAddress,
            msg.sender
        );
        ...
        for (uint256 i = 0; i < _timestampArray.length; i++) {
            if (
                (stakerData[msg.sender][_timestampArray[i]]
                    .stakeInitiationTimestamp
                    .add(
                    stakerData[msg.sender][_timestampArray[i]]
                        .stakeDurationTimestamp
                ) <= block.timestamp) &&
                (stakerData[msg.sender][_timestampArray[i]].unstaked == false)
            ) {
                if (
                    _amount >
                    stakerData[msg.sender][_timestampArray[i]].stakeQuantity
                ) {
                    ...
                    portalData[stakerData[msg.sender][_timestampArray[i]]
                        .outputTokenAddress]
                        .xioStaked = portalData[stakerData[msg
                        .sender][_timestampArray[i]]
                        .outputTokenAddress]
                        .xioStaked
                        .sub(
                        stakerData[msg.sender][_timestampArray[i]].stakeQuantity
        ...
        returns (bool)
    {
        ...
        emit PortalAdded(_tokenAddress, exchangeAddress, symbol);
        return true;
    }
     ...
     */
    function setXIOStakeQuantity(uint256 _quantity)
        public
        onlyOwner
        whenNotPaused
        returns (uint256)
