...
contract ReserveFund is Auth {
  ...
  IERC20 public lgcToken = IERC20(0x3b3A5557F119106270017A7662488C1FF6312A6b);
  ...
  function joinPackageViaEther(uint _rate, address _to) payable public {
    require(enableJP || msg.sender == 0xb69dd8C4d64B0cbED8048975756Eef35AA4c4042, "Can not buy via Ether now");
  ...
  function joinPackageViaToken(uint _amount, address _to) public {
    ...
    require(lgcToken.allowance(msg.sender, address(this)) >= tokenAmount, "You must call approve() first");
    uint userOldBalance = lgcToken.balanceOf(msg.sender);
    require(userOldBalance >= tokenAmount, "You have not enough tokens");
    require(lgcToken.transferFrom(msg.sender, address(this), tokenAmount), "Transfer token failed");
    require(lgcToken.transfer(commissionAdmin, tokenAmount / 10), "Transfer token to admin failed");
  ...
  function miningToken(uint _tokenAmount) public {
    ...
    uint userOldBalance = lgcToken.balanceOf(msg.sender);
    require(lgcToken.transfer(msg.sender, _tokenAmount), "Transfer token to user failed");
    require(lgcToken.balanceOf(msg.sender) == userOldBalance.add(_tokenAmount), "User token changed invalid");
  ...
  function swapToken(uint _amount) public {
    ...
    require(lgcToken.balanceOf(msg.sender) >= _amount, "You have not enough balance");
    ...
    require(lgcToken.allowance(msg.sender, address(this)) >= _amount, "You must call approve() first");
    require(lgcToken.transferFrom(msg.sender, address(this), _amount), "Transfer token failed");
  ...
  function TransferToken (address _to, uint amountToken) onlyMainAdmin public {
     lgcToken.transfer(_to, amountToken);
  }
