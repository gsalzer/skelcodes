...
//    function () external payable;  // equivalent to mint()
}
...
contract CEtherSwap is RateNormalization, NetworkBasedTokenSwap
{
  using SafeMath for uint;
  using SafeERC20 for IERC20;
  uint constant expScale = 1e18;
  uint constant expScaleSquare = 1e18 * 1e18;
  ...
  {
    setRateAdjustmentFactor(ETHER, _cether, 10 ** (18-8), true);
  ...
  function getNetworkRate(address src, address dest, uint256 /*srcAmount*/) internal view
    returns(uint expectedRate, uint slippageRate)
  {
    uint rateStored = cether.exchangeRateStored();
    uint rate = 0;
    if (src == ETHER && dest == address(cether)) {
      rate = expScaleSquare.div(rateStored);
    } else if (src == address(cether) && dest == ETHER) {
      rate = rateStored;
    }
    rate = normalizeRate(src, dest, rate);
    return (rate, rate);
  }
  ...
  {
    (uint rate, ) = getNetworkRate(src, dest, srcAmount);
    require(rate >= minConversionRate, "rate < minConversionRate");
    uint toUser = srcAmount.mul(rate).div(expScale);
    uint toTradeAmount = toUser > maxDestAmount
      ? maxDestAmount.mul(expScale).div(rate)
      : srcAmount;

    if (src == ETHER && dest == address(cether)) {
      cether.mint.value(toTradeAmount)();
      return cether.balanceOf(address(this));
    } else if (src == address(cether) && dest == ETHER) {
      require(cether.redeem(toTradeAmount) == 0, "Cannot redeem");
