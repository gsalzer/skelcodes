...
contract owned {
    ...
    event OwnershipTransferred(address indexed target);
...
contract SenCoin is owned, IERC20 {
    ...
    constructor(string memory name, string memory symbol, uint8 decimals, uint256 initialSupply, uint256 cap) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _cap = cap;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
        require(!_frozen[msg.sender], "Caller cannot be frozen");
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        require(_totalSupply < _cap, "ERC20: total supply exceeds its cap");
