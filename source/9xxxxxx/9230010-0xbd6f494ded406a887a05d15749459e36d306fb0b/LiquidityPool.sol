...
interface ILiquidityPool {
    // Deposit funds into the liquidity pool. These funds become accessible to
    // whitelisted adapters.
    function deposit(address _token, uint256 _amount) external returns (uint256);

    // Withdraw funds from the liquidity pool. These funds are no longer
    // accessible to the whitelisted adapters.
...
contract Token is ERC20Pausable, ERC20Mintable, ERC20Burnable, Ownable, CanReclaimTokens {
    ...
    function rename(string calldata _name, string calldata _symbol) external onlyOwner {
        name_ = _name;
        symbol_ = _symbol;
...
contract LiquidityPool is Ownable, ILiquidityPool, CanReclaimTokens {
    ...
    constructor(ERC20 _liquidityToken, Token _liquidityShareToken, ILender _lender) public {
        liquidityToken = _liquidityToken;
        liquidityShareToken = _liquidityShareToken;
        lender = _lender;
    }

    function updateLender(ILender _newLender) external onlyOwner {
        lender = _newLender;
    }

    function deposit(address _token, uint256 _amount) external returns (uint256) {
        require(_token == address(liquidityToken), "Invalid token address");
        // Transfer liquidityToken from sender to contract.
        liquidityToken.safeTransferFrom(msg.sender, address(this), _amount);

        // Approve before lending.
        liquidityToken.safeTransfer(address(lender), _amount);

        // Lend liquidityToken on Compound.
        uint256 lentAmount = lender.lend(_token, _amount);
        uint256 liquidity = calculateExpectedLiquidity(_token, lentAmount);

        // Mint liquidity pool token for sender.
    ...
    function withdraw(address _token, uint256 _amount) external {
        require(_token == address(liquidityToken), "Invalid token address");
        uint256 lentAmount = calculateExpectedCAmount(_token, _amount);
        liquidityShareToken.burnFrom(msg.sender, _amount);

        // Redeem base liquidityToken from Compound.
        lender.redeem(msg.sender, _token, lentAmount);
    }

    function calculateExpectedLiquidity(address _liquidityToken, uint256 _cTokenAmount) internal returns (uint256) {
        uint256 totalSupply = liquidityShareToken.totalSupply();
        if (totalSupply == 0) {
            return _cTokenAmount;
        }
        uint256 totalLiquidity = lender.balanceOf(_liquidityToken);
        return (totalSupply*_cTokenAmount)/totalLiquidity;
    }

    function calculateExpectedCAmount(address _liquidityToken, uint256 _liquidityAmount) internal returns (uint256) {
        uint256 totalSupply = liquidityShareToken.totalSupply();
    ...
    function destroy() external onlyOwner {
        Ownable(address(lender)).transferOwnership(msg.sender);
