...
contract OrchidLottery {
    ...
    function take(address funder, address signer, address payable recipient, uint128 amount, uint128 ratio, bytes memory receipt) private {
        Pot storage pot = find(funder, signer);
        ...
        if (verify != OrchidVerifier(0)) {
            ...
            if (pot.codehash_ == codehash)
                verify.book(funder, signer, pot.shared_, recipient, amount, ratio, receipt);
        }
