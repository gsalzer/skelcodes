...
// File: browser/dex-adapter-advanced.sol
...
contract UniswapExchangeAdapter is GSNRecipient {
        ...
        returns (uint256 ethBought)
    {
        ...
        bytes32 pHash = keccak256(abi.encode(_to, _msg));
        bytes32 signedMessageHash = getSignedMessageHash(pHash, _amount, _nHash);
        shiftInStatuses[signedMessageHash] = true;
    }
    ...
    // Admin
    function depositZBTC(uint256 _amount) external {
        IERC20(token).transferFrom(address(msg.sender), address(this), _amount);
    }
    
    function withdrawZBTC(uint256 _amount) external {
