...
contract KyberHelpers is DSMath, Stores  {
    ...
    function checkSlippage(
        ...
        uint slippageAmt = convert18ToDec(getDecimal(buyAddr), wmul(unitAmt, sellAmt18));

        (uint expectedAmt, ) = kyber.getExpectedRate(sellAddr, buyAddr, sellAmt);
...
contract KyberResolver is KyberHelpers {
    ...
    ) external payable
    {
        ...
        uint _buyAmt = kyberContract.trade.value(ethAmt)(
            sellAddr,
            _sellAmt,
            buyAddr,
            address(this),
