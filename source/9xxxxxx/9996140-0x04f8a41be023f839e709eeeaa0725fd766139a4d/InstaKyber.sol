...
contract KyberHelpers is DSMath, Stores  {
    ...
    function kyberTrade(
        ...
        uint unitAmt,
        uint ethAmt
    ) internal returns (uint slippageAmt, uint expectedAmt, uint _buyAmt){
        uint sellAmt18 = convertTo18(getDecimal(sellAddr), sellAmt);
        slippageAmt = convert18ToDec(getDecimal(buyAddr), wmul(unitAmt, sellAmt18));

        (expectedAmt, ) = kyber.getExpectedRate(sellAddr, buyAddr, sellAmt);
        require(expectedAmt >= slippageAmt, "Too much slippage");
...
contract KyberResolver is KyberHelpers {
    ...
    ) external payable
    {
        ...
        (uint slippageAmt, uint expectedAmt, uint _buyAmt) = kyberTrade(
            kyberContract,
            sellAddr,
            buyAddr,
            _sellAmt,
        ...
        emit LogSell(buyAddr, sellAddr, _buyAmt, _sellAmt, slippageAmt, expectedAmt);
    }
}
