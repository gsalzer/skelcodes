...
contract CBDToken {
    ...
    string public name = "Computing Blockchain Development";      //  token name
    ...
    function canTransferBalance(address addr) public view returns (uint256){
        if(initTypes[addr]==0x0){
            ...
            if(initTypes[addr]==0x1){
                if(s >= 2160 minutes){
                    return balanceOf[addr];    
                }else if(s >= 1080 minutes){
                    return balanceOf[addr].add(frozenBalances[addr].mul((s.sub(1080 minutes).div(30 minutes) + 1)).div(36)).sub(frozenBalances[addr]);
                }else{
                ...
                if(s >= 1440 minutes){
                    return balanceOf[addr];    
                }else if(s >= 720 minutes){
                    return balanceOf[addr].add(frozenBalances[addr].mul((s.sub(720 minutes).div(30 minutes) + 1)).div(24)).sub(frozenBalances[addr]);
                }else{
                    return balanceOf[addr].sub(frozenBalances[addr]);
                }
            }else{
