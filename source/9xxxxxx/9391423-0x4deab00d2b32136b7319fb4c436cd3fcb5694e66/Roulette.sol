...
contract Roulette {
    ...
    //added to prevent spoofing (delegated call hack)
    function isDelegated () internal view returns (bool) {
        return address (this) != thisAddress;
    }
    
    function setThisAddress(address theAddress) onlyOwner returns(bool){
        thisAddress = theAddress;
        return true;
    }
    
    function play(string oddOrEven) payable returns (uint256 result){
        
        require(thisAddress > 0, "Please set thisAddress as creator to prevent delegatedcall spoofing");
        ...
        require(isDelegated() == false, "Please do not spoof as though you are not a smart contract when you are one");
