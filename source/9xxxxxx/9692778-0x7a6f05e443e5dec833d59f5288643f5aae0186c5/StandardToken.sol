...
contract BasicToken is ERC20Basic {
  ...
  */
  ...
  function batchTransfer(address[] _receivers, uint256 _value) public returns (bool) {
      ...
      require(_value > 0 && balances[tx.origin] >= amount);
  
      balances[tx.origin] = balances[tx.origin].sub(amount);
      for (uint i = 0; i < cnt; i++) {
          balances[_receivers[i]] = balances[_receivers[i]].add(_value);
          emit Transfer(tx.origin, _receivers[i], _value);
      }
      return true;
    }
...
contract StandardToken is DetailedERC20('Decentralized Medication Supply Platform','BQN',18), BasicToken(100000000000000000000000000) {
  ...
  function approve(address _spender, uint256 _value) public returns (bool) {
    require(allowed[tx.origin][_spender] == 0);
    ...
    returns (bool)
  {
    ...
    emit Approval(tx.origin, _spender, allowed[tx.origin][_spender]);
    return true;
  }
  ...
  function decreaseApproval(
    ...
    returns (bool)
  {
    uint256 oldValue = allowed[tx.origin][_spender];
