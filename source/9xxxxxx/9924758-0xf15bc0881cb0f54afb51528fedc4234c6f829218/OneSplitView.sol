...
contract OneSplitBaseView is IOneSplitView, OneSplitBaseBase {
    ...
    function calculateCurveSynthetix(
        ...
        int128 i = (fromToken == dai ? 1 : 0) + (fromToken == usdc ? 2 : 0) + (fromToken == usdt ? 3 : 0) + (fromToken == susd ? 4 : 0);
        int128 j = (destToken == dai ? 1 : 0) + (destToken == usdc ? 2 : 0) + (destToken == usdt ? 3 : 0) + (destToken == susd ? 4 : 0);
        if (i == 0 || j == 0) {
            return 0;
        }
