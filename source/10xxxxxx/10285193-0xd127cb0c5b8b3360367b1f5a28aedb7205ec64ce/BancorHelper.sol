...
contract BancorHelper {
    ...
    function getConvertersByTokenAddress(address token) public view returns (address[] memory converters) {

        address[] memory smartTokens = registry.getConvertibleTokenSmartTokens(token);

        converters = new address[](smartTokens.length);
