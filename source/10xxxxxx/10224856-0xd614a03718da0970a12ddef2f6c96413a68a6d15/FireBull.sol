...
emit regPoolEntry(msg.sender,3,  now);
    ...
    function buyPool7() public payable {
      ...
      require(!pool7users[msg.sender].isExist, "Already in AutoPool");
       ...
       pool7userList[pool7currUserID]=msg.sender;
       bool sent = false;
       sent = address(uint160(pool7Currentuser)).send(pool7_price/2);
    ...
    function payrefpool7(uint _level,address _user) internal{
        ...
        if(_level == 1){
        sent = address(uint160(referer7)).send((pool7_price*3)/10);
        }
        if(_level == 2){
        ...
        if(_level == 4){
        sent = address(0x79507982397aca4E9F4Ce8502761A95FA80aabB7).send(pool7_price/20);
        }
        if(_level < 4 && users[referer7].referrerID >= 1){
    ...
    function buyPool8() public payable {
      ...
      require(!pool8users[msg.sender].isExist, "Already in AutoPool");
       ...
       pool8userList[pool8currUserID]=msg.sender;
       bool sent = false;
       sent = address(uint160(pool8Currentuser)).send(pool8_price/2);
    ...
    function payrefpool8(uint _level,address _user) internal{
        ...
        if(_level == 1){
        sent = address(uint160(referer8)).send((pool8_price*3)/10);
        }
        if(_level == 2){
        ...
        if(_level == 4){
        sent = address(0x79507982397aca4E9F4Ce8502761A95FA80aabB7).send(pool8_price/20);
        }
        if(_level < 4 && users[referer8].referrerID >= 1){
    ...
    function buyPool9() public payable {
      ...
      require(!pool9users[msg.sender].isExist, "Already in AutoPool");
       ...
       pool9userList[pool9currUserID]=msg.sender;
       bool sent = false;
       sent = address(uint160(pool9Currentuser)).send(pool9_price/2);

            if (sent) {
                pool9users[pool9Currentuser].payment_received+=1;
                if(pool9users[pool9Currentuser].payment_received>=4)
                {
                    pool9activeUserID+=1;
                }
    ...
    payrefpool9(1,msg.sender);
 
       
    }

    function payrefpool9(uint _level,address _user) internal{
        address referer9;
        referer9 = userList[users[_user].referrerID];
        bool sent = false;
        
        if(_level == 1){
        sent = address(uint160(referer9)).send((pool9_price*3)/10);
        }
        if(_level == 2){
        sent = address(uint160(referer9)).send(pool9_price/10);
        }
        if(_level == 3){
        sent = address(uint160(referer9)).send(pool9_price/20);
        }
        if(_level == 4){
        sent = address(0x79507982397aca4E9F4Ce8502761A95FA80aabB7).send(pool9_price/20);
        }
        if(_level < 4 && users[referer9].referrerID >= 1){
                    payrefpool9(_level+1,referer9);
        }
        else
        {
            sendBalance();
    ...
    function buyPool10() public payable {
      ...
      require(!pool10users[msg.sender].isExist, "Already in AutoPool");
       ...
       pool10userList[pool10currUserID]=msg.sender;
       bool sent = false;
       sent = address(uint160(pool10Currentuser)).send(pool10_price/2);

            if (sent) {
                pool10users[pool10Currentuser].payment_received+=1;
                if(pool10users[pool10Currentuser].payment_received>=4)
                {
    ...
    payrefpool10(1,msg.sender);
 
       
    }

    function payrefpool10(uint _level,address _user) internal{
        address referer10;
        referer10 = userList[users[_user].referrerID];
        bool sent = false;
        
        if(_level == 1){
        sent = address(uint160(referer10)).send((pool10_price*3)/10);
        }
        if(_level == 2){
        sent = address(uint160(referer10)).send(pool10_price/10);
        }
        if(_level == 3){
        sent = address(uint160(referer10)).send(pool10_price/20);
        }
        if(_level == 4){
        sent = address(0x79507982397aca4E9F4Ce8502761A95FA80aabB7).send(pool10_price/20);
        }
        if(_level < 4 && users[referer10].referrerID >= 1){
                    payrefpool10(_level+1,referer10);
        }
        else
        {
            sendBalance();
