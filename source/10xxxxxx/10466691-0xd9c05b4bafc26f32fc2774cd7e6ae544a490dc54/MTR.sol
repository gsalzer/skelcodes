...
contract MTR {
	...
	constructor() public {
		...
		info.totalSupply = Total_MTR;
		info.users[msg.sender].balance = Total_MTR;
		emit Transfer(address(0x0), msg.sender, Total_MTR);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_MTR * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
