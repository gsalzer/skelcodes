...
import "./SafeMath.sol";
import "./HBCHStorage.sol";

contract HBCHLogic {

    using SafeMath for uint256;

    string public constant name = "HBCHLogic";
    ...
    function burnLogic(address from, uint256 value,string calldata bchAddress,
        string calldata proof,bytes32 taskHash, address supportAddress, uint256 requireNum)
        external onlyCaller returns(uint256){

        uint256 balance = store.balanceOf(from);
        require(balance >= value,"sender address not have enough HBCH");
        require(value > 0, "value need > 0");
        require(taskHash == keccak256((abi.encodePacked(from,value,bchAddress,proof))),"taskHash is wrong");
    ...
    function resetStoreLogic(address storeAddress) external onlyCaller {
        store = HBCHStorage(storeAddress);
    }
