...
contract Etherbank_Interface_Father {
...
contract Father {
    ...
    function Repurchase() external onlyOwner {
        Etherbank_Interface_Father(etherbank).Repurchase.value(amount)();
    }

    function PayAnotherAccount(uint _sponsorID, address _user) external onlyOwner {
        Etherbank_Interface_Father(etherbank).PayAnotherAccount.value(amount)(_sponsorID, _user);
    }
