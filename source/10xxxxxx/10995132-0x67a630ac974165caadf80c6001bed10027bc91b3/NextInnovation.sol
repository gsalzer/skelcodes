...
/**
 * @dev Extension of {ERC20} that adds a time-based lock to the wallet.
 */
abstract contract ERC20LockableUpgradeSafe is Initializable, ERC20UpgradeSafe, OwnableUpgradeSafe {


    /**
     * @dev Addresses which cant be locked.
     */
    mapping (address => bool) private _unlockableAddresses;

    struct Account {
        uint lockedUntilDate;
        bool exists;
    }

    mapping (address => Account) private _lockableAccounts;


    event RegisteredNewLockableAccount(address accountAddress);
    event RegisteredNewUnlockableAccount(address accountAddress);
    event DisabledUnlockableAccount(address accountAddress);
     ...
     * Requirements:
     *
     * There are only two valid scenarios for these accounts:
     * 1) Unlockable Addresses can make any transactions
     * 2) Locked accounts cant transfer to any accounts until a certain date
     * **Note**: !_lockableAccounts[from].exists... ensures that a third party exchanger cannot transfer token to our locked accounts
     */
    function _beforeTokenTransfer(
        address from,
        address to,
        ...
        require(
            ...
            (!_lockableAccounts[to].exists || (_lockableAccounts[to].exists && _lockableAccounts[to].lockedUntilDate < block.timestamp))
        ...
        require(
            ...
            (!_lockableAccounts[from].exists || (_lockableAccounts[from].exists && _lockableAccounts[from].lockedUntilDate < block.timestamp))
        ,"ERC20Lockable: Sender's wallet is locked. This transaction is not permitted.");

    }

    //TODO: If locked accounts could transfer money, there should be a _transfer override function to forward the lock dates.
...

{
    ...
    function initialize(string memory name, string memory symbol, uint cap) public virtual {
        __ERC20PresetMinterPauser_init(name, symbol, cap);
    }
...
contract NextInnovation is
    ...
    ERC20UpgradeBase
{
    using SafeERC20 for ERC20UpgradeBase;
