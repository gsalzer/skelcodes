...
/**
 * @title Guardian node bridge storage
 */
contract BRIDGE_NodeSave {
    IBMapping mappingContract;                      
    IBRIDGE bridgeContract;
    ...
    */
    constructor (address map) public {
        ...
        bridgeContract = IBRIDGE(address(mappingContract.checkAddress("bridge")));
    }
    ...
    */
    function turnOut(uint256 amount, address to) public onlyMiningCalculation returns(uint256) {
        uint256 leftNum = bridgeContract.balanceOf(address(this));
        if (leftNum >= amount) {
            bridgeContract.transfer(to, amount);
            return amount;
        } else {
            return 0;
        }
 ...
 */
contract BRIDGE_NodeAssignmentData {
    ...
    */
    ...
    //  Add bridge
    function addBridge(uint256 amount) public onlyNodeAssignment {
        nodeAllAmount = nodeAllAmount.add(amount);
    }
    
    //  View cumulative total 累计合计
 ...
 */
contract BRIDGE_NodeAssignment {
    
    using SafeMath for uint256;
    IBMapping mappingContract;  
    IBRIDGE bridgeContract;
    SuperMan supermanContract;                              
    BRIDGE_NodeSave nodeSave;
    BRIDGE_NodeAssignmentData nodeAssignmentData;
    ...
    */
    constructor (address map) public {
        mappingContract = IBMapping(map); 
        bridgeContract = IBRIDGE(address(mappingContract.checkAddress("bridge")));
        supermanContract = SuperMan(address(mappingContract.checkAddress("bridgeNode")));
        nodeSave = BRIDGE_NodeSave(address(mappingContract.checkAddress("bridgeNodeSave")));
        nodeAssignmentData = BRIDGE_NodeAssignmentData(address(mappingContract.checkAddress("nodeAssignmentData")));
    }
    ...
    */
    ...
    /**
    * @dev Deposit in bridge
    * @param amount Quantity deposited in bridge
    */
    function bookKeeping(uint256 amount) public {
        require(amount > 0);
        require(bridgeContract.balanceOf(address(msg.sender)) >= amount);
        require(bridgeContract.allowance(address(msg.sender), address(this)) >= amount);
        require(bridgeContract.transferFrom(address(msg.sender), address(nodeSave), amount));
        nodeAssignmentData.addBridge(amount);
    }
    ...
    */
    function nodeGet() public {
        ...
        uint256 getAmount = amount.mul(supermanContract.balanceOf(address(msg.sender))).div(18000);
        require(bridgeContract.balanceOf(address(nodeSave)) >= getAmount);
    ...
    */
    function nodeCount(address fromAdd, address toAdd) public {
        ...
        uint256 getAmountFrom = amountFrom.mul(supermanContract.balanceOf(address(fromAdd))).div(18000);
        require(bridgeContract.balanceOf(address(nodeSave)) >= getAmountFrom);
        ...
        uint256 getAmountTo = amountTo.mul(supermanContract.balanceOf(address(toAdd))).div(18000);
        require(bridgeContract.balanceOf(address(nodeSave)) >= getAmountTo);
...
contract SuperMan is IERC20 {
    ...
    uint256 private _totalSupply = 18000 ether;
    string public name = "BridgeNode";
    ...
    */
    function _transfer(address from, address to, uint256 value) internal {
        ...
        // BRIDGE_NodeAssignment nodeAssignment = BRIDGE_NodeAssignment(address(mappingContract.checkAddress("nodeAssignment")));
