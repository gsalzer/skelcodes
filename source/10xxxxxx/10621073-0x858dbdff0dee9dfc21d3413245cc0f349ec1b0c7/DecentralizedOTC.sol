...
abstract contract ERC20Vestable is ERC20 {
    ...
    event CreateGrant(address beneficiary, address creator, uint256 id, uint256 endTime);
    event DepositToGrant(address beneficiary, uint256 id, address depositor, uint256 amount);
...
contract DecentralizedOTC is UseOracle, UseBondMaker, UseSafeMath, Time, TransferETH {
    address internal immutable LIEN_TOKEN_ADDRESS;
    ...
    event LogDepositERC20Pool(address indexed deployer, bytes32 indexed poolID, uint256 amount);

    event LogWithdrawERC20Pool(address indexed sender, bytes32 indexed erc20PoolID, uint256 amount);
    ...
    function setPoolMap(
        ...
        poolMap[erc20PoolID] = PoolInfo(ERC20Address, spread, isVestable, vestingEndTime);
    ...
    function getLBTTheoreticalPrice(
        ...
        require(
            ...
            "left time until the maturity must be non-negative"
        );
        uint256 untilMaturity = _getBlockTimestampSec().sub(maturity);
    ...
    function calcRateLBT2ERC20(
        ...
        (uint256 lowestPrice, uint256 strikePrice) = _getLowestPrice(sbtID, etherPrice);
    ...
    function exchangeLBT2ERC20(
        ...
        uint256 ERC20Amount = _exchangeLBT2ERC20(bondGroupID, erc20PoolID, LBTAmount);
        if (expectedAmount != 0) {
            require(
    ...
    function _exchangeLBT2ERC20(
        ...
        bondToken.transferFrom(msg.sender, deployer[erc20PoolID], LBTAmount - feeAmount);
        ...
        ERC20Vestable token = ERC20Vestable(pool.ERC20Address);
        {
            ...
            ERC20Amount = _applyDecimalGap((LBTAmount - feeAmount).mul(rateE4), bondToken, token);
    ...
    function transferEther2LienHolders(bytes32[] memory lbtIDList) public {
        for (uint256 i = 0; i < lbtIDList.length; i++) {
            ...
            BondTokenInterface lbtContract = BondTokenInterface(payable(contractAddress));
