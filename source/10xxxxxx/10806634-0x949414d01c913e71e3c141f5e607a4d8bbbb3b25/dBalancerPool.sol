...
contract LPTokenWrapper {
    ...
    IERC20 public bpt = IERC20(0xCBa435408f3B3A7ad231e0083d7eE2e9ec03764f);
...
contract dBalancerPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    bool public isOpenWithdraw = false;

    event RewardINVPaid(address indexed user, uint256 reward);
    ...
    function getInvAddrs(address addr) public view returns(address[] memory){
        address[] memory list = invAddrs[addr];
        return list;
    }

    function getInvCount(address addr) public view returns(uint256) {
        return  invAddrs[addr].length;
    }
    ...
    function earnedInv(address account) public view returns(uint256) {
        ...
        return total.sub(claimINVRewards[account]);
    }

    function calcInv(address addr,uint256 factor,uint256 cur) internal view returns(uint256) {
        uint256 total = Math.min(earned(addr).add(claimRewards[addr]), cur);
        return total.mul(factor).div(100);
    }
    ...
    modifier checkhalve(){
        if (block.timestamp >= periodFinish && periodFinish != 0) {
            ...
            syfi.mint(address(this),initreward);
