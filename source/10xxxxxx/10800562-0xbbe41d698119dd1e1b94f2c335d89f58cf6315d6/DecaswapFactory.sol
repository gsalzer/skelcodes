...
contract DecaswapFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'decaswap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'decaswap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'decaswap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(decaswapPair).creationCode;
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'decaswap: FORBIDDEN');
        feeTo = _feeTo;
    }
