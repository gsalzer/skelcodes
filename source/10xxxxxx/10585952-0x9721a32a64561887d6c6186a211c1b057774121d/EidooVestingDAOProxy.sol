...
// File: contracts/IDAOStaking.sol
...
interface IDAOStaking {
   ...
   *         From https://github.com/provable-things/aragon-staking-manager/blob/develop/contracts/StakingManager.sol
...
// File: contracts/EidooVestingDAOProxy.sol
...
//interface IBasicEidooVestingHub {
//  function createVesting(uint amount) external;  // it requires pre-approval of the amount from the msg.sender
//  function createVestingFor(address beneficiary, uint amount) external;  // it requires pre-approval of the amount from the msg.sender
//
//  function balanceOf(address beneficiary) external view returns(uint locked, uint unlocked);
//}

//contract EidooVestingTransitionToDAO is IBasicEidooVestingHub, Ownable, Pausable, Withdrawable {
contract EidooVestingDAOProxy is Ownable, Pausable, Withdrawable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  IDAOStaking stakeManager;
  uint64 defaultDuration = 365 days;

  constructor (address _stakeManager) public {
    stakeManager = IDAOStaking(_stakeManager);
  }
  ...
  function setDefaultDuration(uint64 _duration) public onlyOwner {
  ...
  function balanceOf(address beneficiary) external view returns(uint locked, uint unlocked) {
    IDAOStaking.Lock [] memory locks = stakeManager.getStakedLocks(beneficiary);
    locked = 0;
    unlocked = 0;
    for (uint i = 0; i < locks.length; i++) {
      IDAOStaking.Lock memory lock = locks[i];
      if (lock.lockDate + lock.duration > now) {
        locked += lock.amount;
      } else {
  ...
  {
    IERC20 token = IERC20(stakeManager.depositToken());

    token.safeTransferFrom(msg.sender, address(this), _amount);
    token.safeApprove(address(stakeManager), _amount);
    stakeManager.stake(_amount, defaultDuration, _beneficiary);
  }
