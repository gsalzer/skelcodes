...
contract DfTokenizedStrategy is DSMath, Ownable {
    ...
    // curDeposited eth in strategy by owner
    uint256 public ethInDeposit;

    uint256 public entryEthPrice;
    ...
    constructor(
        ...
        // open strategy
        IDfFinanceOpen(DF_FINANCE_OPEN).deal.value(curDeposit)
        (
            address(this),
            _extraCoef,
            _profitPercent,
    ...
    function calculateProfit(address _userAddr) public view returns(
        ...
        // calculate amount for deposit withdrawal by owner
        (uint ethDeposit, uint usdDeposit, ) = calculateWithdrawalOnDeposit();

        uint profitEth = sub(ethBalance, ethDeposit);
        uint profitUsd = sub(usdBalance, usdDeposit);

        uint userTokenBalance = IERC20(profitToken).balanceOf(_userAddr);
        uint userShare = wdiv(userTokenBalance, tokenTotalSupply);

        ethToWithdraw = wmul(profitEth, userShare);
        usdToWithdraw = wmul(profitUsd * 1e12, userShare) / 1e12;
    }
    ...
    // TODO: ?? onlyOwner
    function withdrawProfit(address[] memory _accounts) public onlyOwner {
    ...
    function withdrawDeposit() public onlyOwner {
        uint depositEth = ethInDeposit;
        require(depositEth > 0, "Deposit is withdrawn");

        // calculate withdrawal on deposit
        (uint ethToWithdraw, uint usdToWithdraw, uint restDepositEth) = calculateWithdrawalOnDeposit();
    ...
    function calculateWithdrawalOnDeposit() internal view returns(
        uint ethToWithdraw,
        uint usdToWithdraw,
        ...
        uint ethBalance = IToken(ETH_ADDRESS).universalBalanceOf(address(this));
        uint usdBalance = IToken(USDC_ADDRESS).universalBalanceOf(address(this));
        require(ethBalance > 0 || usdBalance > 0, "There are no ETH or USDC");
    ...
    function _withdrawProfitHelper(address _userAddr) internal {
        ...
        require(ethToWithdraw > 0 || usdToWithdraw > 0, "No funds to withdraw");

        // TODO: ?? burn without eth and usd withdrawal

        // burn all user's tokens
        _burnTokensHelper(_userAddr, IERC20(profitToken).balanceOf(_userAddr));
    ...
    function _withdrawHelper(
        ...
        if (_ethToWithdraw > 0) {
            IToken(ETH_ADDRESS).universalTransfer(_user, _ethToWithdraw);
        }
