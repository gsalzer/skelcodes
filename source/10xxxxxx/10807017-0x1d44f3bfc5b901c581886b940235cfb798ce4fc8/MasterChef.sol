...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to BeerSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // BeerSwap must mint EXACTLY the same amount of BeerSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Beer. He can make Beer and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accBeerPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBeerPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accBeerPerShare; // Accumulated BEERs per share, times 1e12. See below.
    }
    ...
    function pendingBeer(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBeerPerShare = pool.accBeerPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 beerReward = multiplier.mul(beerPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBeerPerShare = accBeerPerShare.add(beerReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBeerPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accBeerPerShare = pool.accBeerPerShare.add(beerReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    function init() public {
        if (beer.totalSupply() == 0) {
            beer.mint(msg.sender, 100000000000000000000000);
        }
    }

    // Deposit LP tokens to MasterChef for BEER allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBeerPerShare).div(1e12).sub(user.rewardDebt);
            safeBeerTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accBeerPerShare).div(1e12);
    ...
    function safeBeerTransfer(address _to, uint256 _amount) internal {
