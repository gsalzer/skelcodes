...
// File: contracts/Utils/AddressTranslation.sol

pragma solidity ^0.5.0;


contract AddressTranslation is Ownable {
  mapping(address => address) public addressTranslationMap;

  function setAddressTranslation(address from, address to) external onlyOwner {
    addressTranslationMap[from] = to;
  }

  function translateAddress(address from) internal view returns(address to) {
    to = addressTranslationMap[from];
    if (to == address(0)) {
      to = from;
    }
  }
...
contract KyberTokenSwap is AddressTranslation, NetworkBasedTokenSwap
{
  using SafeMath for uint;
  using SafeERC20 for IERC20;
  ...
  function setKyberNetwork(address _kyberNetwork) public onlyOwner {
    require(_kyberNetwork != address(0), "_kyberNetwork == address(0)");
    kyberNetwork = IKyberNetwork(_kyberNetwork);
  }
  ...
  function getNetworkRate(address src, address dest, uint256 srcAmount) internal view
    returns(uint expectedRate, uint slippageRate)
  {
    return kyberNetwork.getExpectedRate(translateAddress(src), translateAddress(dest), srcAmount);
  }
    ...
    internal returns(uint256)
  {
    if (src == ETHER) {
      return kyberNetwork.trade
        .value(srcAmount)(translateAddress(src), srcAmount, translateAddress(dest), address(this), maxDestAmount, minConversionRate, kyberFeeWallet);
    } else {
      if (IBadStaticCallERC20(src).allowance(address(this), address(kyberNetwork)) > 0) {
        IBadStaticCallERC20(src).safeApprove(address(kyberNetwork), 0);
      }
      IBadStaticCallERC20(src).safeApprove(address(kyberNetwork), srcAmount);
      return kyberNetwork.trade(translateAddress(src), srcAmount, translateAddress(dest), address(this), maxDestAmount, minConversionRate, kyberFeeWallet);
    }
  }
