...
WORLDS BIGGEST CROWD FUNDING SMART CONTRACT ( FOR THE PUBLIC : BY THE PUBLIC)
...
//******************************************************************************//
//------------------ Main Contract "EtherrMoney" Starts Here -------------------//
//******************************************************************************//
...
contract megaTree is owned
{
    ...
    // in other words if that coin address contains > 0 value in ether will be allowed means if validStableCoin[address] > 0 then can be used here
    ...
    // EtherOutPlan: network owner can define as much plan as possible max up to 256 for ex main, auto-pool, unilevel, dividend and any other
    ...
    // networkId=> etherOutPlanId => main or sub => payoutTo (pool index) => total amount
    ...
    // whenever Your EtherInPlan will create/start a new tree structure,  platform fee will be deducted, so calculate this while designing plan
