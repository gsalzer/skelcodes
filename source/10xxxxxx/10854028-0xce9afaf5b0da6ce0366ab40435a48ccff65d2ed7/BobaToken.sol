...
// BobaToken with Governance.
contract BobaToken is ERC20("BobaToken", "BOBA"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterBarista).
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to BobaSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // BobaSwap must mint EXACTLY the same amount of BobaSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterBarista is the master of Boba. He can make Boba and he is a fair guy.
//
...
contract MasterBarista is Ownable {
    ...
    uint256 public constant INITIAL_BOBA_PER_BLOCK = 5e18; // 5 per block
    ...
    // Update the given pool's BOBA allocation point. Can only be called by the owner.
    ...
    // Return total reward of Boba over the given _from to _to block.
    ...
    // Return Boba reward of a given block height
    ...
    // Deposit LP tokens to MasterBarista for BOBA allocation.
    ...
    // Withdraw LP tokens from MasterBarista.
    ...
    // Update dev admin address by the prevoius dev admin
