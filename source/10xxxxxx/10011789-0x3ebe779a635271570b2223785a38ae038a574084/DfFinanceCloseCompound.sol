...
contract DfFinanceCloseCompound is DSMath, ConstantAddresses, FundsMgr {
    ...
    function collectUsdForStrategies(
        ...
        if (_useExchange) {
            ...
            _exchange(IToken(ETH_ADDRESS), totalLoanEth, IToken(USDC_ADDRESS), add(_amountUsdToRedeem, _amountUsdToBuy), _exData);
        }
    ...
    function collectAndClose(
        ...
        bool _useExchange,
        bytes memory _exData
    ) public {  // onlyOwnerOrAdmin in collectUsdForStrategies function
    ...
    function _collectUsdHelper(
        address _dfWallet, uint256 _amountUsdToBuy, uint256 _usdPrice, bool _onlyProfitInUsd
        ...
        if (_amountUsdToBuy > 0) { // in USD
            ...
            // set strategy value
            strategy.usdToWithdraw = uint64(usdToWithdraw);
    ...
    function _collectAndCloseHelper(
        ...
        _exchangeAndRedeemHelper(_dfWallet, ethForExchange, _minAmountUsd, _exData);
        ...
        if (hasProfit) {
            ...
            systemProfit = sub(fee, _affiliateProcess(strategyOwner, fee));
        }
        ...
        _withdrawProfit(strategyOwner, ethToWithdraw, _minAmountUsd);
    ...
    function _withdrawProfit(
        address payable _strategyOwner, uint256 _ethToWithdraw, uint256 _usdToWithdraw
    ) internal {
        // withdraw ETH to strategy owner
        if (_ethToWithdraw > 0) {
            _strategyOwner.transfer(_ethToWithdraw);
        }
