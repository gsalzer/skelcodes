...
contract HEXREF is Events {
    ...
    uint constant devFee = 10; // 10% of refShare;
    ...
    function stakeHex(uint hearts, uint dayLength, uint refShare, address payable ref)
        internal
        returns(bool)
    {
        //send
        require(hexInterface.transferFrom(msg.sender, address(this), hearts), "Transfer failed");//send hex from user to contract
        //refshare must be max 50%, min 0.1%
        require(refShare >= 2 && refShare <= 1000, "ref share out of range");
        //user info
        ...
        //update stake info
        updateStakeData(hearts, dayLength, refShare, ref, hexStakeIndex);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        if(stakeValue < stake.heartValue)//late or early penalties cut into principal
        {
            ...
            //do not take fee when cut in to principal, incentivise refs to run good accounting
            _profit = 0;
            _principal = stakeValue;
        }
        else{
            stake.stakeValue = stakeValue;//principal + interest
            stake.stakeProfit = stakeValue.sub(stake.heartValue);//interest
            uint _fee = stake.stakeProfit.div(stake.refShare);//calc fee from staking interest
            uint _devFee = _fee.div(devFee);//get devFee (10% of fee)
            uint _refFee = _fee.sub(_devFee);//sub devFee from fee to get refFee (90% of fee)
    ...
    //updates stake data
    function updateStakeData(uint hearts, uint dayLength, uint refShare, address payable ref, uint index)
        internal
    {
        ...
        stake.refShare = refShare;//update ref share
    ...
    //stake HEX (refShare must be >=2 && <=1000)
    function StakeHex(uint _hearts, uint _dayLength, uint _refShare, address payable _ref)
        public
        canStake(_hearts)
        synchronized
    {
        require(stakeHex(_hearts, _dayLength, _refShare, _ref), "Error: could not stake");
    }
