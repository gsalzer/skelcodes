...
contract PSYieldContract is YieldRoles {
    ...
    mapping (address => uint256) public lockBoxIndex;
    ...
    function createLockBoxForWallet3m(address beneficiary, uint256 amount) onlyOwner external {
        createLockBoxForWallet(beneficiary, amount, 90 days);
    }
    
    function createLockBoxForWallet6m(address beneficiary, uint256 amount) onlyOwner external {
        createLockBoxForWallet(beneficiary, amount, 180 days);
    }
    
    function createLockBoxForWallet12m(address beneficiary, uint256 amount) onlyOwner external {
        createLockBoxForWallet(beneficiary, amount, 360 days);
    }
    
    function createLockBoxForWallet(address beneficiary, uint256 amount, uint256 duration) internal {
        require(lockBoxIndex[beneficiary] == 0, "LockBox for user already exists!");
        ...
        // Store lockbox number + 1 (to check if it is not 0 for existence)
        lockBoxIndex[beneficiary] = lockBoxStructs.length + 1;
        
        // Build lockbox
        LockBoxStruct memory l;
        l.beneficiary = beneficiary;
        l.balance = yieldAmount;
        l.tokenAmount = amount;
        l.releaseTime = now + duration;
        l.tokensDeposited = false;
        lockBoxStructs.push(l);
    }

    function deposit(uint256 amount) public {
        require(now < endDepositTime, "Deposit time has ended.");
        require(lockBoxIndex[msg.sender] > 0, "No lockbox for wallet found.");
        
        // Get wallet's lockbox (-1 because we stored +1 above)
        uint256 lockBoxNumber = lockBoxIndex[msg.sender] - 1;
        LockBoxStruct storage l = lockBoxStructs[lockBoxNumber];
        
        require(l.tokensDeposited == false, "Tokens already deposited!");
        
        // Wallet must submit exactly the specified amount
        require(amount == l.tokenAmount, "Wrong amount of tokens to deposit.");
        
        // Get lockable tokens from user
        require(token.transferFrom(msg.sender, address(this), amount), "Tokens cannot be transferred.");
        
        // Update lockbox
        l.balance += amount;
        l.tokenAmount = 0;
        l.tokensDeposited = true;
        lockBoxStructs.push(l);
    ...
    function withdraw(uint256 lockBoxNumber) public {
        ...
        require(l.tokensDeposited == true, "No tokens were deposited, inactive lockbox!");
    ...
    // Retrieve other tokens that were sent to this contract
    function recoverERC20(address tokenAddress, uint256 tokenAmount, address receiver) public onlyOwner {
        // Not possible for DIA tokens to prevent accidential early release
        require(tokenAddress != 0x84cA8bc7997272c7CfB4D0Cd3D55cd942B3c9419);
        IERC20(tokenAddress).transfer(receiver, tokenAmount);
    }
    
    // Do not accept ETH payments
    function() external {
        revert();
    }
}
