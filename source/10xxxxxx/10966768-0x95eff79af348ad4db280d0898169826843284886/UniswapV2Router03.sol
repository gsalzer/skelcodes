...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'6ee5cf2638d8cc5e7c0b363d093bc82bff365a08e0c33b78b578869d85d67d53' // init code hash; update to keccak256(pair.bytecode) upon code update
            ))));
    }
...
contract UniswapV2Router03 is IUniswapV2Router03 {
    ...
    // **** ADD LIQUIDITY ****
    function _reportAddLiquidity(address _user, address _tokenA, uint256 _amountA, address _tokenB, uint256 _amountB, address _referrer) internal virtual {
        address reportingTo = IUniswapV2Factory(factory).reportingTo();
        if (reportingTo != address(0) && _user != address(this)) {
          IUniswapV2Reporting(reportingTo).addedLiquidity(_user, _tokenA, _amountA, _tokenB, _amountB, _referrer);
        }
    }
    function _reportAddLiquidityETH(address _user, address _token, uint256 _amount, uint256 _amountETH, address _referrer) internal virtual {
        address reportingTo = IUniswapV2Factory(factory).reportingTo();
        if (reportingTo != address(0) && _user != address(this)) {
          IUniswapV2Reporting(reportingTo).addedLiquidityETH(_user, _token, _amount, _amountETH, _referrer);
        }
    }
    ...
    function addLiquidity(
        ...
        _reportAddLiquidity(to, tokenA, amountA, tokenB, amountB, address(0));
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        ...
        _reportAddLiquidityETH(to, token, amountToken, amountETH, address(0));
    }
    function addLiquidityWithReferrer(
        address tokenA,
        address tokenB,
        ...
        _reportAddLiquidity(to, tokenA, amountA, tokenB, amountB, referrer);
    }
    function addLiquidityETHWithReferrer(
        address token,
        uint amountTokenDesired,
        ...
        // refund dust eth, if any
        if (msg.value > amountETH) TransferHelper.safeTransferETH(msg.sender, msg.value - amountETH);
        _reportAddLiquidityETH(to, token, amountToken, amountETH, referrer);
    }
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        (, amountETH) = removeLiquidity(
            ...
            address(this),
            deadline
        );  // does NOT report, since the transfer is to address(this)
        uint amountToken = IERC20Uniswap(token).balanceOf(address(this));
        TransferHelper.safeTransfer(token, to, amountToken);
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint amountOut = IERC20Uniswap(path[path.length - 1]).balanceOf(to).sub(balanceBefore);
        require(
