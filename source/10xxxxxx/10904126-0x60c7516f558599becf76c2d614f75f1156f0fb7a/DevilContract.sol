...
contract SoulToken is ERC20("https://devil.finance/", "$SOUL"), Ownable {
...
contract DevilContract is Ownable {
  ...
  // ie when progress = 10% & _amount = 10000
  // ratio = 0.1 * 1e4 = 1000
  // tethered factor =  10000 * 1000 * 1000 * 1000 / 1e12 = 10
  // one can withdraw 10 out of 10000 $SOUL token
  function tetheredFactor(uint256 _amount) public view returns (uint256) {
    if (block.number >= bondingEndBlock) {
      ...
      return _amount.mul(ratio).mul(ratio).mul(ratio).div(1e12);
    }
  }
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 lpSupply = pool.lpToken.balanceOf(address(this)); // 1e18
    if (lpSupply == 0) {
      pool.lastRewardBlock = block.number;
      return;
    }
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number); // 10
    uint256 soulReward = multiplier.mul(soulPerBlock).mul(pool.allocPoint).div(totalAllocPoint); // soulperblock = 1e18
    soul.mint(address(this), soulReward);
    pool.accSoulPerShare = pool.accSoulPerShare.add(soulReward.mul(1e12).div(lpSupply)); // 1e18 * 1e12 / 1e18
    pool.lastRewardBlock = block.number;
  }
