...
// Modified to work in the KASSIAHOME system

// all votes work on underlying _kassiahomeBalances[address], not balanceOf(address)
...
//
// Changes made by KASSIAHOME after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since KASSIAHOME supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match KASSIAHOME as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "KASSIAHOME Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(kassiahome.initSupply(), 4), 100); } // 4% of KASSIAHOME

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(kassiahome.initSupply(), 100); } // 1% of KASSIAHOME
    ...
    KASSIAHOMEInterface public kassiahome;
    ...
    constructor(address timelock_, address kassiahome_) public {
        timelock = TimelockInterface(timelock_);
        kassiahome = KASSIAHOMEInterface(kassiahome_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(kassiahome.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || kassiahome.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = kassiahome.getPriorVotes(voter, proposal.startBlock);
