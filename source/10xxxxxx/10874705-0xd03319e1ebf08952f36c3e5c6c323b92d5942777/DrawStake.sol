...
// token transfers
// ----------------------------------------------------------------------------
contract DrawStake is Owned {
    using SafeMath for uint256;
    
    address public YFFS = 0x90D702f071d2af33032943137AD0aB4280705817;
    ...
    event gotReward(address staker, uint256 reward);
    ...
    function stake(uint256 tokens) external {
        require(IERC20(YFFS).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    // ------------------------------------------------------------------------
    function drawToken(uint256 tokens) external {
        require(IERC20(YFFS).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function getReward() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(YFFS).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
        
            emit gotReward(msg.sender, owing);
    ...
    function withdraw(uint256 tokens) external {
        ...
        require(IERC20(YFFS).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedYFFS(address staker) external view returns(uint256 stakedYFFS){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the YFFS balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourYFFSBalance(address user) external view returns(uint256 YFFSBalance){
        return IERC20(YFFS).balanceOf(user);
    }
}
