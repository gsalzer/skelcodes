...
contract ChadletsGenesisPool is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

	ERC1155Tradable public chadsLtdAddress;
	...
	uint256 public maxChadletsPerDay = 69;

	uint256 public maxStakingTokens = 1104;
	...
	constructor(ERC1155Tradable _chadsLtdAddress, IERC20 _stakingToken) public {
		chadsLtdAddress = _chadsLtdAddress;
	...
	function setMaxStakingTokens(uint256 _maxStakingTokens) external onlyOwner {
		maxStakingTokens = _maxStakingTokens;
	}

	function setMaxChadletsPerDay(uint256 _maxChadletsPerDay) external onlyOwner {
		maxChadletsPerDay = _maxChadletsPerDay;
	}

	function setPoints(address _address, uint256 _points) external onlyOwner {
	...
	function earned(address account) public view returns (uint256) {
		...
		return
			points[account].add(
				blockTime.sub(lastUpdateTime[account])
				.mul(1e18)
				.div(86400)
				.mul((balanceOf(account).mul(maxChadletsPerDay).div(maxStakingTokens))	// max 69e18 (1104e18 * (69/1104)) per day
				.div(1e18))
			);
	}
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused updateReward(msg.sender) {
		...
		require(amount.add(balanceOf(msg.sender)) <= (maxStakingTokens * 1e18), "Cannot stake more than maxStakingTokens");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(chadsLtdAddress.totalSupply(card) < chadsLtdAddress.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		chadsLtdAddress.mint(msg.sender, card, 1, "");
