...
// TacoToken with Governance.
contract TacoToken is ERC20("TacoToken", "TACO"), Ownable {
    ...
    */
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "TACO::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying TACOs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "TACO::_writeCheckpoint: block number exceeds 32 bits");
...
interface IMigratorChef {
    // Perform LP token migration from legacy TacoSwap to TacoSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to TacoSwap SLP tokens.
    // TacoSwap must mint EXACTLY the same amount of TacoSwap LP tokens or
    // else something bad will happen. Traditional TacoSwap does not
...
contract TacoChef is Ownable {
    ...
    // The TACO TOKEN!
    TacoToken public taco;
    // Dev address.
    address public devaddr;
    // Block number when bonus TACO period ends.
    ...
    function pendingTaco(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 tacoReward = multiplier.mul(tacoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTacoPerShare = accTacoPerShare.add(tacoReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accTacoPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tacoReward = multiplier.mul(tacoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        taco.mint(devaddr, tacoReward.div(10));
        taco.mint(address(this), tacoReward);
        pool.accTacoPerShare = pool.accTacoPerShare.add(tacoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTacoPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTacoPerShare).div(1e12);
    ...
    function safeTacoTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > tacoBal) {
            taco.transfer(_to, tacoBal);
        } else {
            taco.transfer(_to, _amount);
        }
    }
