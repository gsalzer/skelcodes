...
interface IngameMoney {
    ...
    function buyProxy(address payable _user, address payable _referrer, uint256 _referralBasisPoint, bytes calldata _signature, bytes32 _hash)
        external
        payable;
}

contract CJOKOIGatewayV1 is Operatable, Withdrawable, IngameMoney {
    ...
    constructor(
        ...
        payableOptions[2 ether] = true;
        payableOptions[3 ether] = true;
        ...
        whenNotPaused()
    {
        ...
        require(validateSig(encodeData(_user, _referrer, _referralBasisPoint), _signature), "Invalid signature");
        if (_hash != bytes32(0)) {
            recordHash(_hash);
        }
        ...
        payable
    {
        require(_referralBasisPoint + ethBackBasisPoint <= BASE, "Invalid basis points");
        require(payableOptions[msg.value], "Invalid msg.value");
        require(validateSig(encodeData(msg.sender, _referrer, _referralBasisPoint), _signature), "Invalid signature");
    ...
    function getRandom(uint16 max, uint256 _nonce, address _sender) internal view returns (uint16) {
        return uint16(
            bytes2(
                keccak256(
                    abi.encodePacked(
    ...
    function encodeData(address _user, address _referrer, uint256 _referralBasisPoint) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                _user,
                _referrer,
