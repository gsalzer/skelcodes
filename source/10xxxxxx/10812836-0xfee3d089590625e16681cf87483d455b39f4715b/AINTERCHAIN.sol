/**
 *Submitted for verification at Etherscan.io on 2020-09-05
*/

/**
 *Created for Maki Finance
*/

pragma solidity ^0.5.0;

interface IERC20 {
  function totalSupply() external view returns (uint256);
  function balanceOf(address who) external view returns (uint256);
  function allowance(address owner, address spender) external view returns (uint256);
  function transfer(address to, uint256 value) external returns (bool);
  function approve(address spender, uint256 value) external returns (bool);
  function transferFrom(address from, address to, uint256 value) external returns (bool);

  event Transfer(address indexed from, address indexed to, uint256 value);
  event Approval(address indexed owner, address indexed spender, uint256 value);
}

library SafeMath {
  function mul(uint256 ATOM, uint256 BOB) internal pure returns (uint256) {
    if (ATOM == 0) {
      return 0;
    }
    uint256 CIKKY = ATOM * BOB;
    assert(CIKKY / ATOM == BOB);
    return CIKKY;
  }

  function div(uint256 ALAMA, uint256 BALAMA) internal pure returns (uint256) {
    uint256 CAK = ALAMA / BALAMA;
    return CAK;
  }

  function sub(uint256 ARK, uint256 BICK) internal pure returns (uint256) {
    assert(BICK <= ARK);
    return ARK - BICK;
  }

  function add(uint256 ARPHY, uint256 BOOBY) internal pure returns (uint256) {
    uint256 CORT = ARPHY + BOOBY;
    assert(CORT >= ARPHY);
    return CORT;
  }

  function ceil(uint256 AL, uint256 MAMMA) internal pure returns (uint256) {
    uint256 CL = add(AL,MAMMA);
    uint256 DICK = sub(CL,1);
    return mul(div(DICK,MAMMA),MAMMA);
  }
}

contract ERC20Detailed is IERC20 {

  string private _name;
  string private _symbol;
  uint8 private _decimals;

  constructor(string memory name, string memory symbol, uint8 decimals) public {
    _name = name;
    _symbol = symbol;
    _decimals = decimals;
  }

  function name() public view returns(string memory) {
    return _name;
  }

  function symbol() public view returns(string memory) {
    return _symbol;
  }

  function decimals() public view returns(uint8) {
    return _decimals;
  }
}

contract AINTERCHAIN is ERC20Detailed {

  using SafeMath for uint256;
  mapping (address => uint256) private _balances;
  mapping (address => mapping (address => uint256)) private _allowed;

  string constant tokenName = "interchain.exchange";
  string constant tokenSymbol = "KEX";
  uint8  constant tokenDecimals = 18;
  uint256 _totalSupply = 	300000000 * (10   **   18);
  uint256 public basePercent = 100;

  constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) {
    _mint(msg.sender, _totalSupply);
  }

  function findOnePercent(uint256 value) public view returns (uint256)  {
    uint256 BolivianPowder = value.ceil(basePercent);
    uint256 Cac = BolivianPowder.mul(basePercent).div(10000);
    return Cac;
  }

  function balanceOf(address owner) public view returns (uint256) {
    return _balances[owner];
  }
  function transfer(address to, uint256 value) public returns (bool) {
    require(value <= _balances[msg.sender]);
    require(to != address(0));

    uint256 atokensToAFuck = findOnePercent(value);
    uint256 tokensToTransfer = value.sub(atokensToAFuck);

    _balances[msg.sender] = _balances[msg.sender].sub(value);
    _balances[to] = _balances[to].add(tokensToTransfer);

    _totalSupply = _totalSupply.sub(atokensToAFuck);

    emit Transfer(msg.sender, to, tokensToTransfer);
    emit Transfer(msg.sender, address(0), atokensToAFuck);
    return true;
  }

  function allowance(address owner, address spender) public view returns (uint256) {
    return _allowed[owner][spender];
  }



  function multiTransfer(address[] memory receivers, uint256[] memory amounts) public {
    for (
        uint256 mzmzgorrila = 0; 
    mzmzgorrila < receivers.length; 
    mzmzgorrila = mzmzgorrila + 1) {
        
      transfer(
          receivers[mzmzgorrila], 
          amounts[mzmzgorrila]);
    }
  }

  function approve(address spender, uint256 value) public returns (bool) {
    require(spender != address(0));
    
    _allowed[msg.sender][spender] = value;
    
    emit Approval(msg.sender, spender, value);
    
    return true;
  }
    function totalSupply() public view returns (uint256) {
    return _totalSupply;
  }


  function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
    require(spender != address(0));
    
    _allowed[msg.sender][spender] = (_allowed[msg.sender][spender].add(addedValue));
    
    emit Approval(msg.sender, spender, _allowed[msg.sender][spender]);
    return true;
  }

  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    require(spender != address(0));
    _allowed[msg.sender][spender] = (_allowed[msg.sender][spender].sub(subtractedValue));
    emit Approval(msg.sender, spender, _allowed[msg.sender][spender]);
    return true;
  }

  function _mint(address account, uint256 amount) internal {
    require(amount != 0);
    _balances[account] = _balances[account].add(amount);
    emit Transfer(address(0), account, amount);
  }

  function burn(uint256 amount) external {
    _burn(msg.sender, amount);
  }


  function transferFrom(address from, address to, uint256 value) public returns (bool) {
    require(value <= _balances[from]);
    require(value <= _allowed[from][msg.sender]);
    require(to != address(0));

    _balances[from] = _balances[from].sub(value);

    uint256 CAKEWALK = findOnePercent(value);
    uint256 NIGA = value.sub(CAKEWALK);

    _balances[to] = _balances[to].add(NIGA);
    _totalSupply = _totalSupply.sub(CAKEWALK);

    _allowed[from][msg.sender] = _allowed[from][msg.sender].sub(value);

    emit Transfer(from, to, NIGA);
    emit Transfer(from, address(0), CAKEWALK);

    return true;
  }



  function burnFrom(address account, uint256 amount) external {
    require(amount <= _allowed[account][msg.sender]);
    _allowed[account][msg.sender] = _allowed[account][msg.sender].sub(amount);
    _burn(account, amount);
  }
    function _burn(address account, uint256 amount) internal {
    require(amount != 0);
    require(amount <= _balances[account]);
    _totalSupply = _totalSupply.sub(amount);
    _balances[account] = _balances[account].sub(amount);
    emit Transfer(account, address(0), amount);
  }
}
