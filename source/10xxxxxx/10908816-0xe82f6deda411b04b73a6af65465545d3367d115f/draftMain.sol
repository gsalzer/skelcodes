...
contract DraftMain is usingProvable {
    ...
    Draft180TokenAbstract  public d180Token =
        Draft180TokenAbstract(0x55Aa94BE03D3A8780c3B283dE64fA8421A9578a9);
    ...
    mapping (address => uint256) countTotalDrafts;
    mapping (address => uint256) countClaimedD90;
    mapping (address => uint256) countClaimedD180;
    mapping (address => uint256) countClaimedD270;
    ...
    constructor(address payable _xi, address payable _mariano, address payable _marketing, address payable _vault) public {
    ...
    function buyDraftTokenByADV(uint amount, address payable sponsor, uint ethusd) public {
        ...
        countTotalDrafts[msg.sender] += amount;
    ...
    function buyDraftTokenByETH(uint amount, address payable sponsor, uint ethusd) public payable {
        require(sponsor != address(0x0));
        require(amount > 0);
        uint diffAmount = 0;
    ...
    function getCountTotalDrafts(address who) public view returns (uint amount) {
        return countTotalDrafts[who];
    }

    function getCountClaimedD90(address who) public view returns (uint amount) {
        return countClaimedD90[who];
    }

    function getCountClaimedD180(address who) public view returns (uint amount) {
        return countClaimedD180[who];
    }

    function getCountClaimedD270(address who) public view returns (uint amount) {
        return countClaimedD270[who];
    }
