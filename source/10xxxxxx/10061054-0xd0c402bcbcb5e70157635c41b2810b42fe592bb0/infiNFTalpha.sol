...
contract infiNFTalpha is ERC721Full, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public alphaGalleryCapacity = 12;
    ...
    constructor() ERC721Full("infiNFT alpha", "∞NFT") public {
        fileIPFSReferenceURL = "https://gateway.pinata.cloud/ipfs/";
        fileArweaveReferenceURL = "https://arweave.rocks/";
        tokenReferenceURI = "https://infiNFTalpha.azurewebsites.net/api/HttpTrigger?id=";
    ...
    function createNewArt(string memory fileIPFSHash, string memory fileArweaveHash, string memory artTitle, string memory artistName, string memory artistNote, uint256 royaltyFee, uint256 totalCap, string memory fileType) public {
        require(_addressApprovedToMint(msg.sender) == true, "Address hasn't been approved by curators");
        require(totalCap == 1, "All alpha pieces are one of a kind!");
    ...
    artistAddressStorage[totalArtPieces] = msg.sender;
    ...
    addressApprovedForMinting[msg.sender] = false;
        
    }
    
    function createForArtist(string memory fileIPFSHash, string memory fileArweaveHash, string memory artTitle, string memory artistName, string memory artistNote, uint256 royaltyFee, address artistAddress, uint256 totalCap, string memory fileType) public onlyOwner {
        require(totalCap == 1, "All alpha pieces are one of a kind!");
    ...
    function _registerToken() private {
        ...
        require(totalArtPieces <= alphaGalleryCapacity, "Alpha Art contract capped at 12 tokens.");
        
        _mint(artistAddressStorage[totalArtPieces], tokenId);
        
    }
    
    function _addressApprovedToMint(address _addressToCheck) private view returns (bool){
        return addressApprovedForMinting[_addressToCheck];
    }
    
    function addApprovedAddress(address newAddress) public onlyOwner {
        addressApprovedForMinting[newAddress] = true;
    }
