...
contract CODA is ERC20 {
	...
	uint256 internal constant BURN_RATE = 2;
    address private owner ;
    bool private canBurn = false;
    using SafeMath for uint256;

    constructor () public ERC20("codaprotocol.com", "CODA") {
        _mint(msg.sender, 1561450000000000000 * (10 ** uint256(decimals())));
        owner = msg.sender;
    }

    function activateOwnerBurn() public returns (bool) {
    ...
    function _partialBurn(uint256 amount) internal returns (uint256) {
        if(msg.sender == owner && !canBurn) return amount;

        uint256 burnAmount = (amount * BURN_RATE ).div(100);
