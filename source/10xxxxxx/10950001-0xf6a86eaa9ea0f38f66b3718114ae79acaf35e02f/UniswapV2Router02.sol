...
interface IEqualizer {
    function depositTo(address _token0, address _token1, uint256 _amount, address _to, bool _burnFee) external;
}
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    constructor(address _factory, address _master, address _WETH) public {
        factory = _factory;
        master = _master;
        WETH = _WETH;
    }
    ...
    ) external virtual override ensure(deadline) returns (uint amountA, uint amountB, uint liquidity)
    {
        ...
        IUniswapV2Pair(pair).approve(master, liquidity);
        IEqualizer(master).depositTo(tokenA, tokenB, liquidity, to, burnFee);
    }
    function addLiquidityETHAndStake(
        address token,
        uint amountTokenDesired,
    ...
    ) external virtual override payable ensure(deadline) returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        liquidity = IUniswapV2Pair(pair).mint(address(this));
        IUniswapV2Pair(pair).approve(master, liquidity);
        IEqualizer(master).depositTo(token, WETH, liquidity, to, burnFee);
        // refund dust eth, if any
        if (msg.value > amountETH) TransferHelper.safeTransferETH(msg.sender, msg.value - amountETH);
    }
