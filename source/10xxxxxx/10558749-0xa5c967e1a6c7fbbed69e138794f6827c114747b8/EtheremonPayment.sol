...
contract EtheremonPayment is EtheremonEnum, BasicAccessControl, SafeMath {
    ...
    function EtheremonPayment(address _dataContract, address _tokenContract, address _transformContract) public {
    ...
    function setContract(address _dataContract, address _tokenContract, address _transformContract) onlyModerators external {
        dataContract = _dataContract;
        tokenContract = _tokenContract;
        transformContract = _transformContract;
    }
    ...
    function payService(address _trainer, uint _tokens, uint32 _type) isActive requireTransformContract  public returns(uint result) {
        if (msg.sender != tokenContract)
            revert();
