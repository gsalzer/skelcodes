...
contract StakePool is Initializable {
    ...
    function _stake(uint256 amount, uint256 fee) internal {
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        stakeToken.safeTransferFrom(msg.sender, address(this), amount.add(fee));
    }
...
contract FomoCow is StakePool {
    ...
    function stake(uint256 amount) public updateReward(msg.sender) checkhalve checkStart{
        ...
        uint256 fee = amount.mul(feeRate).div(feeRateDenominator);
        uint256 realAmount = amount.sub(fee);
        super._stake(realAmount, fee);
        emit Staked(msg.sender, realAmount);

        if(fee > 0 && feeReceiver != address(0)) {
            stakeToken.safeTransfer(feeReceiver, fee);
        }
    }
...
/**
 * @dev https://cow.money
 * MILK distribution will be increased 10% at each period.
 */
...
/**
 * @dev CowFactory https://cow.money
 */
contract CowFactory is Ownable {
	...
	{
		bytes memory bytecode = type(FomoCow).creationCode;
    bytes32 salt = keccak256(abi.encodePacked('fomo'));
    assembly {
        cow := create2(0, add(bytecode, 32), mload(bytecode), salt)
    }
    emit CowBorn(cow);
	}
	...
	{
		bytes memory bytecode = type(GrowingCow).creationCode;
    bytes32 salt = keccak256(abi.encodePacked('growing'));
    assembly {
        cow := create2(0, add(bytecode, 32), mload(bytecode), salt)
    }
    emit CowBorn(cow);
	...
	// More cow for future
	function createCow(string memory id, bytes memory bytecode) public onlyOwner returns (address cow) {
	...
	function initializeCow(
		address cow, 
		address _yieldToken,
		uint256 reward,
		bytes memory params
	) public onlyOwner {
		(bool success, ) = address(cow).call(params);
    require(success, "CowFactory: initialize cow failed");
    IERC20(_yieldToken).transfer(cow, reward);
	}
}
