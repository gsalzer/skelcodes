...
// mocked version of StakingRewards contract - *only* used in tests
contract MockedStakingRewards is StakingRewards {
    /* ========== MOCK VARIABLES ========== */

    uint256 public fakeBlockTimeStamp = 1601524305;

    function getTime() internal view returns (uint256) {
        return fakeBlockTimeStamp;
    }

    function getFakeTime() external view returns (uint256) {
        return fakeBlockTimeStamp;
    }

    function _mock_resetBlockTime() public {
        fakeBlockTimeStamp = 1601524305;
    }

    function _mock_setBlockTimeStamp8DayForward() public {
        fakeBlockTimeStamp = fakeBlockTimeStamp + unstakingInterval + 1;
    }

    function _mock_setBlockTimeStamp1DayForward() public {
        fakeBlockTimeStamp = fakeBlockTimeStamp + rewardInterval + 1;
    }

    function _mock_setBlockTimeStamp1HourForward() public {
        fakeBlockTimeStamp = fakeBlockTimeStamp + 3600 + 1;
    }

    /* ========== CONSTRUCTOR ========== */

    constructor(address _owner, address _stakingToken) public StakingRewards(_owner, _stakingToken) {}
}
