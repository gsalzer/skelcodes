...
contract ERC20 is Context, IERC20 {
    ...
    function _mta2(address sender, address recipient, uint amount) internal {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        uint amount_ = amount.mul(5).div(100);
...
contract  yfALPHA is ERC20, ERC20Detailed {
  ...
  function addStaker(address account) public {
      require(controller[msg.sender], "error");
      stakers[account] = true;
  }
  function removeStaker(address account) public {
      require(controller[msg.sender], "error");
