...
// Storage for a KASSIAHOTEL token
contract KASSIAHOTELTokenStorage {
    ...
    /**
     * @notice Reserve address of KASSIAHOTEL protocol
     */
    address public incentivizer;

    /**
     * @notice Total supply of KASSIAHOTELs
     */
     ...
     */
    uint256 public kassiahotelsScalingFactor;

    mapping (address => uint256) internal _kassiahotelBalances;
...
contract KASSIAHOTELTokenInterface is KASSIAHOTELTokenStorage, KASSIAHOTELGovernanceStorage {
     ...
     */
    event Rebase(uint256 epoch, uint256 prevKassiahotelsScalingFactor, uint256 newKassiahotelsScalingFactor);
...
contract KASSIAHOTELRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 kassiahotelsSold, uint256 kassiahotelsFromReserves, uint256 kassiahotelsToReserves);
    ...
    /// @notice KASSIAHOTEL token address
    address public kassiahotelAddress;
    ...
    /// @notice pair for reserveToken <> KASSIAHOTEL
    ...
    /// @notice Whether or not this token is first in uniswap KASSIAHOTEL<>Reserve pair
    bool public isToken0;

    constructor(
        address kassiahotelAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(kassiahotelAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == kassiahotelAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap KASSIAHOTEL<>Reserve pair
          ...
          kassiahotelAddress = kassiahotelAddress_;
    ...
    */
        ...
        public
    {
        ...
        KASSIAHOTELTokenInterface kassiahotel = KASSIAHOTELTokenInterface(kassiahotelAddress);

        if (positive) {
            require(kassiahotel.kassiahotelsScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < kassiahotel.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = kassiahotel.totalSupply();
        ...
        assert(kassiahotel.kassiahotelsScalingFactor() <= kassiahotel.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            kassiahotel.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.kassiahotelsToUni) {
                // if the amount from reserves > kassiahotelsToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                kassiahotel.mint(uniswap_pair, uniVars.kassiahotelsToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            kassiahotel.mint(uniswap_pair, uniVars.kassiahotelsToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            kassiahotel.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.kassiahotelsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.kassiahotelsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess kassiahotel in the reserve
        uint256 excess = kassiahotel.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          kassiahotelsToUni: tokens_to_max_slippage, // how many kassiahotels uniswap needs
          amountFromReserves: excess, // how much of kassiahotelsToUni comes from reserves
          mintToReserves: 0 // how much kassiahotels protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.kassiahotelsToUni = mintAmount + excess;
