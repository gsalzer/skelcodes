...
contract StrategySwerveUSD {
  ...
  function balanceOfPool() public view returns (uint) {
    uint256 wantTokenUnit = uint256(10) ** ERC20Detailed(want).decimals();
    uint256 swusdTokenUnit = uint256(10) ** ERC20Detailed(swusd).decimals();
    return SSUSDGauge(gauge).balanceOf(address(this)) * wantTokenUnit / swusdTokenUnit;
  }
