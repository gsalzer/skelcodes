...
contract STONK is ERC20Detailed {

  using SafeMath for uint256;
  mapping (address => uint256) private _balances;
  mapping (address => mapping (address => uint256)) private _allowed;

  string constant tokenName = "STONK";
  ...
  constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) {
    _beginTheStonkening(msg.sender, _totalSupply);
  }
  ...
  function transfer(address to, uint256 value) public returns (bool) {
    ...
    uint256 tokensToBurn = findDestonkAmount(value);
  ...
  function _beginTheStonkening(address account, uint256 amount) internal {
    require(amount != 0);
