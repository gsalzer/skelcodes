/*
 * A Decentralized Stack of Financial Rewards
 * 
 * Official Website: 
...
contract DexStackFinance is ERC20Detailed 
{
    ...
    string constant tokenName = "Dex Stack Finance";
    string constant tokenSymbol = "STACK"; 
    ...
    //these addresses won't be affected by network fee 
    ...
    constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) 
    {
        ...
        _start(msg.sender, 100000000*(10**uint256(tokenDecimals)));
    }
    ...
    function _start(address account, uint256 value) internal 
    {
        require(value != 0);
    ...
    /*
    *   transfer attached with 10% fee
    *   the receiver receives 90% of the sent value, and 10% distributed to STACK staking pool
    */
    function _executeTransfer(address from, address to, uint256 value) private
    {
        ...
        //Update sender and receivers rewards 
        updateRewardsFor(from);
        updateRewardsFor(to);
        
        uint256 tenPercent = 0;
        
        if(!whitelistFrom[from] && !whitelistTo[to])
        {
             
            tenPercent = value.mul(10).div(100);
            
            //set a minimum burn rate to prevent no-fee-txs 
            if(tenPercent == 0 && value > 0)
                tenPercent = 1;
        }
        ...
        if(fus_total > 0)
        {
            uint256 stakingRewards = tenPercent;
