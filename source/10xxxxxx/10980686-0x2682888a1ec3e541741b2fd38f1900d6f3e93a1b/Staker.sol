...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0xD879fCA7EC2d3c11Bd2fcadc2AF611bCF777f729;
...
contract Staker is Poolable{
    ...
    function stake() public payable{
        ...
        require(creationTime + 12 hours <= now, "It has not been 12 hours since contract creation yet");
        
        address poolAddress = Uniswap(FACTORY).getPair(ringAddress, WETHAddress);
        
        if(price() >= (1.05 * 10**18) && priceCapped){

            uint t = IERC20(ringAddress).balanceOf(poolAddress); //token in uniswap
            uint a = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
            uint x = (sqrt(9*t*t + 3988000*a*t) - 1997*t)/1994;
            
            IERC20(ringAddress).mint(address(this), x);
            
            address[] memory path = new address[](2);
            path[0] = ringAddress;
            path[1] = WETHAddress;
            IERC20(ringAddress).approve(UNIROUTER, x);
        ...
        uint tokenAmount = IERC20(ringAddress).balanceOf(poolAddress); //token in uniswap

        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(ringAddress).mint(address(this), toMint);
        
        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));
        
        uint amountTokenDesired = IERC20(ringAddress).balanceOf(address(this));
        IERC20(ringAddress).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(ringAddress, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(ringAddress).mint(msg.sender, amount);
    }
    ...
    function ethtimeCalc(uint ring) internal view returns(uint){
        return ( ring.mul(31557600 * DECIMAL) ).div( rewardValue() );
    }
}
