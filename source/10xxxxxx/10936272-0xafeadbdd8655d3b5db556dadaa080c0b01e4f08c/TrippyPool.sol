...
contract MushTokenWrapper {
	using SafeMath for uint256;
	IERC20 public mush;

	constructor(IERC20 _mushAddress) public {
		mush = IERC20(_mushAddress);
	}
	...
	function stake(uint256 amount) public {
		...
		mush.transferFrom(msg.sender, address(this), amount);
	}

	function withdraw(uint256 amount) public {
		_totalSupply = _totalSupply.sub(amount);
		_balances[msg.sender] = _balances[msg.sender].sub(amount);
		mush.transfer(msg.sender, amount);
	}
}

contract TrippyPool is MushTokenWrapper, Ownable {
	ERC1155Tradable public mushie;
	...
	constructor(ERC1155Tradable _mushieAddress, IERC20 _mushAddress) public MushTokenWrapper(_mushAddress) {
		mushie = _mushieAddress;
	}
	...
	// stake visibility is public as overriding MushTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 10000000000, "Cannot stake more than 100 Mushie");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(mushie.totalSupply(card) < mushie.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		mushie.mint(msg.sender, card, 1, "");
