...
contract UpSwingIDO is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public immutable HARDCAP = 6000 ether;

    uint256 public TOKENS_PER_ETH = 1000;

    uint256 public TOKENS_PER_ETH_WITH_BONUS = 1150;
    ...
    function _buyTokens(address _beneficiary, uint256 _amount) internal {
        ...
        uint256 tokensPerEth = whitelists[_beneficiary] ? TOKENS_PER_ETH_WITH_BONUS : TOKENS_PER_ETH;
    ...
    function finalize() external onlyOwner {
        ...
        uint256 liquidityETH = weiRaised.div(3); // 33.3(3)%
        ...
        IUpSwing(address(token)).togglePause(false);
