...
contract CreamUSD is ERC20, ERC20Detailed {
    using SafeMath for uint;
    ...
    constructor() public ERC20Detailed("Cream USD", "crUSD", 18) {
    ...
    function getOutExactIn(address from, address to, uint input, int256 x, int256 y) public view returns (uint output) {
        if (normalizer.getPrice(from) != uint(0)) {
            input = input.mul(normalizer.getPrice(from)).div(1e18);
        }
        int256 _x = x.add(i(input));
        int256 _y = f(_x, x, y);
        output = uint(y.sub(_y));
        if (normalizer.getPrice(to) != uint(0)) {
            output = output.mul(1e18).div(normalizer.getPrice(to));
        }
    }

    // Calculate input given exact output
    function getInExactOut(address from, address to, uint output, int256 x, int256 y) public view returns (uint input) {
        if (normalizer.getPrice(to) != uint(0)) {
            output = output.mul(normalizer.getPrice(to)).div(1e18);
        }
        int256 _y = y.sub(i(output));
        int256 _x = f(_y, y, x);
        input = uint(_x.sub(x));
        if (normalizer.getPrice(from) != uint(0)) {
            input = input.mul(1e18).div(normalizer.getPrice(from));
        }
    }
    ...
    function balance(IERC20 token) public view returns (uint) {
        ...
        if (normalizer.getPrice(_token) != uint(0)) {
            _balance = _balance.mul(normalizer.getPrice(_token)).div(1e18);
        }
    ...
    function swapExactAmountIn(IERC20 from, IERC20 to, uint input, uint minOutput, uint deadline) external returns (uint output) {
        ...
        output = normalize(to, getOutExactIn(address(from), address(to), normalize1e18(from, input.mul(fee).div(BASE)), i(balance(from)), i(balance(to))));
    ...
    function swapExactAmountOut(IERC20 from, IERC20 to, uint maxInput, uint output, uint deadline) external returns (uint input) {
        ...
        input = normalize(from, getInExactOut(address(from), address(to), normalize1e18(to, output), i(balance(from)), i(balance(to))));
    ...
    function addLiquidityExactIn(IERC20 from, uint input, uint minOutput, uint deadline) external returns (uint output) {
        ...
        if (totalSupply() == 0) {
            ...
            output = getOutExactIn(address(from), address(this), normalize1e18(from, input.mul(fee).div(BASE)), i(balance(from)), i(totalSupply().div(count)));
        }
    ...
    function addLiquidityExactOut(IERC20 from, uint maxInput, uint output, uint deadline) external returns (uint input) {
        ...
        if (totalSupply() == 0) {
            ...
            input = normalize(from, getInExactOut(address(from), address(this), output, i(balance(from)), i(totalSupply().div(count))));
            input = input.mul(BASE).divCeil(fee);
        }
    ...
    function removeLiquidityExactIn(IERC20 to, uint input, uint minOutput, uint deadline) external returns (uint output) {
        ...
        output = normalize(to, getOutExactIn(address(this), address(to), input.mul(fee).div(BASE), i(totalSupply().div(count)), i(balance(to))));
    ...
    function removeLiquidityExactOut(IERC20 to, uint maxInput, uint output, uint deadline) external returns (uint input) {
        ...
        input = getInExactOut(address(this), address(to), normalize1e18(to, output), i(totalSupply().div(count)), i(balance(to)));
        input = input.mul(BASE).divCeil(fee);
