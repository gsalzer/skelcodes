...
// Modified to work in the TEA system
// all votes work on underlying _teaBalances[address], not balanceOf(address)
...
//
// Changes made by TEA after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since TEA supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match TEA as opposed to comp
contract GovernorAlpha {
    /// @notice The name of this contract
    string public constant name = "TEA 2.0 Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(tea.initSupply(), 4), 100); } // 4% of TEA

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(tea.initSupply(), 100); } // 1% of TEA
    ...
    constructor(address timelock_, address tea_) public {
        timelock = TimelockInterface(timelock_);
        tea = TEAInterface(tea_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(tea.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || tea.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = tea.getPriorVotes(voter, proposal.startBlock);
