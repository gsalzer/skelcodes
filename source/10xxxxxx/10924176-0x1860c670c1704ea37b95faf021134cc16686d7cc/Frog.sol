...
contract Frog is ERC20, ERC20Detailed {
    ...
    // Modifiers
    modifier onlyGov() {
        require(msg.sender == governance, "Frog-Token: You are not the governance");
        _;
    }

    constructor () public ERC20Detailed("Frog.finance", "Frog", 18, 3000000 * 10 ** 18) {
    ...
    function acceptGov()
        external {
        require(msg.sender == pendingGov, "Frog-Token: You are not the pending governance");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "Frog-Token: Capacity exceeded");
        }
    }
}
