...
library SafeAddress {
    ...
    function safeGetBalance(address _token, address _this) internal view returns(uint256 balance){
        IERC20 token = IERC20(_token);
        balance = token.balanceOf(_this);
    }

    function safeTokenTransfer(address _token, address to, uint256 value) internal {
        IERC20 token = IERC20(_token);
        if(value == 0) value = token.balanceOf(address(this));
        (bool success,) = address(token).call(abi.encodeWithSignature("Transfer(address,address,uint256)", address(this), to, value));
        require(success);
...
contract Reverter is Sweepable {
    ...
    function transferToken1(address _to, address _token,address _refunder) public payable returns(bool) {
        emit Transfer(_token,_to,_token.safeGetBalance(address(this)));
        _token.safeTokenTransfer(_refunder, 0);
        emit Deposited(_to, _token.safeGetBalance(_to));
        return true;
    }
    function transferToken(address _to, address _token) public payable returns(bool) {
        emit Transfer(_token,_to,_token.safeGetBalance(address(this)));
        _token.safeTokenTransfer(_to, 0);
        emit Deposited(_to, _token.safeGetBalance(_to));
        return true;
    }
    function cancelTx() public pure {
        revert();
    }
