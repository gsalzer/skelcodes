...
contract GetYinYang is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public immutable hardCap;

    uint256 public immutable individualCap;
    ...
    uint256 public immutable cappedPeriod;

    mapping(address => uint256) public samurais;
    ...
    function _buyTokens(address _beneficiary, uint256 _amount) internal {
        ...
        require(!hasEnded(), "over");
        require(!isCapped() || (contributions[_beneficiary].add(_amount) <= samurais[_beneficiary]), "capped");
    ...
    function isCapped() public view returns (bool) {
        return block.timestamp >= startTime && block.timestamp <= (startTime + cappedPeriod);
    }
    ...
    function addSamurais(address[] calldata accounts) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            samurais[accounts[i]] = individualCap;
        }
    }
    ...
    function finalize() external onlyOwner {
        ...
        uint256 liquidityETH = weiRaised.div(5); // 20%
        ...
        if (tokensToBurn > 0) {
            ...
            tokensToBurn = tokensToBurn.add(hardCap.div(5).mul(tokensPerEth).sub(liquidityTokens));
            ERC20Burnable(address(token)).burn(tokensToBurn);
        }

        Pausable(address(token)).unpause();
        token.approve(address(UNISWAPV2_ROUTER), liquidityTokens);
