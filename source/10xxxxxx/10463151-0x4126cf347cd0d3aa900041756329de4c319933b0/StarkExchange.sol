    ...
    UpdateState
{
    ...
    */
    ...
    /*
      ...
      Note - current stateful initializer is a full set of init parameters.
      i.e. in order to successfully call initialize, one has to provide ALL parameters,
      including those that change dynamically (e.g. merkle roots).
      Given the fact that you have to provide identical sets of parameters
      in addImplementatoion and in upgradeTo, this is impractical for a working StarkEx.

      So - a real life scenario of upgrade implies a change of the initializer logic,
      to reflect the actual parameters needed to be initialized.

      Additionally, one need to address the governance issue, that with current governance code,
      would fail. (See comments in Governance.sol).
    */
