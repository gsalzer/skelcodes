...
contract GenericPool_YFCC_Finance is Ownable {
	...
	uint256 public _poolHalvingIntervalHours = 24;
	uint256 public _poolRewardDistributionIntervalHours = 24;
	...
	constructor(
		...
		uint256 poolHalvingIntervalHours,
		uint256 poolRewardDistributionIntervalHours,
		...
		pool = address(this);
		_poolHalvingIntervalHours = poolHalvingIntervalHours;
		_poolRewardDistributionIntervalHours = poolRewardDistributionIntervalHours;
	 ...
	 * @param _hours Halving interval hours.
	 */
	function setRewardHalvingInterval(uint256 _hours) public onlyOwner {
		require(_hours > 0, "Halving interval hours must be none zero value.");
		_poolHalvingIntervalHours = _hours;
	}

	/**
	 * @notice A method to set pool reward distribution interval hours.
	 * @param _hours Distribution interval hours.
	 */
	function setRewardDistributionInterval(uint256 _hours) public onlyOwner {
		require(_hours > 0, "Halving interval hours must be none zero value.");
		_poolRewardDistributionIntervalHours = _hours;
	}
	...
	function distributeRewards() public onlyOwner {
		...
		uint256 availableReward = (
			_poolRewardDistributionRate.mul(_poolHalvingIntervalHours)
		)
			.div(_poolRewardDistributionIntervalHours);
		...
		if (_poolRewardHalvingAt == 0 || timestamp >= _poolRewardHalvingAt) {
			...
			uint256 nextHalvingTimestamp = (60 *
				60 *
				_poolHalvingIntervalHours) - 60;
