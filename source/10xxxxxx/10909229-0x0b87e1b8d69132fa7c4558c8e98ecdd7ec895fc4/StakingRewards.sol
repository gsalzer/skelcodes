...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyRewardsDistribution {
        require(IERC20(tokenAddress) != stakingToken, "Can't recover staked tokens in order to prevent abuse");
        IERC20(tokenAddress).transfer(rewardsDistribution, tokenAmount);
    }

    function initializeAddress(address stakeAddress, address rewardAddress) public onlyRewardsDistribution {
        rewardsToken = IERC20(rewardAddress);
        stakingToken = IERC20(stakeAddress);
    } 
