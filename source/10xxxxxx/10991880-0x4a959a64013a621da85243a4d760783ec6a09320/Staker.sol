...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0xF4fAB492478771c1fc2EbcEef8256163A7439Cd6;
...
contract Staker is Poolable{
    ...
    function lpToken() public view returns (address){
        return Uniswap(FACTORY).getPair(hapyAddress, WETHAddress);
    }
    ...
    function stake(address payable ref) public payable{
        ...
        if(ref != address(0)){
            ...
            referralEarned[ref] = referralEarned[ref] + ((address(this).balance/25)*DECIMAL)/price();
        }

        sendValue(_POOLADDRESS, address(this).balance/2);

        address poolAddress = Uniswap(FACTORY).getPair(hapyAddress, WETHAddress);
        uint ethAmount = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
        uint tokenAmount = IERC20(hapyAddress).balanceOf(poolAddress); //token in uniswap

        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(hapyAddress).mint(address(this), toMint);

        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));

        uint amountTokenDesired = IERC20(hapyAddress).balanceOf(address(this));
        IERC20(hapyAddress).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(hapyAddress, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(hapyAddress).mint(msg.sender, amount);
    }
    ...
    function ethtimeCalc(uint hapy) internal view returns(uint){
        return ( hapy.mul(31557600 * DECIMAL) ).div( rewardValue() );
    }
}
