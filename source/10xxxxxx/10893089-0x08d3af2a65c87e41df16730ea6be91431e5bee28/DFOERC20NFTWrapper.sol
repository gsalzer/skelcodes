...
// File: contracts\standalone\ISuperSayanToken.sol
...
interface ISuperSayanToken is IERC1155, IERC1155Receiver, IERC1155Views, IERC1155Data {
    function init(
        address model,
        address source,
...
// File: contracts\standalone\voting\IDFOSuperSayanToken.sol
...
interface IDFOSuperSayanToken is ISuperSayanToken {
...
contract ERC20NFTWrapper is Context, IERC20NFTWrapper {
    ...
    function init(uint256 objectId) public virtual override {
        ...
        (_name, _symbol, _decimals) = ISuperSayanToken(
    ...
    function burn(address owner, uint256 amount) public virtual override {
        require(
            msg.sender == _mainWrapper ||
                (ISuperSayanToken(_mainWrapper).source() == address(0) &&
                    msg.sender == owner),
            "Unauthorized action!"
        );
        _burn(owner, amount);
        ...
        returns (uint256 allowanceAmount)
    {
        ...
        if (
            allowanceAmount == 0 &&
            ISuperSayanToken(_mainWrapper).isApprovedForAll(owner, spender)
        ) {
    ...
    function transferFrom(
        ...
        if (
            !ISuperSayanToken(_mainWrapper).isApprovedForAll(
                sender,
                _msgSender()
            )
        ) {
...
contract DFOERC20NFTWrapper is IDFOERC20NFTWrapper, ERC20NFTWrapper {
    ...
    function transferFrom(
        ...
        if (txSender == _mainWrapper || ISuperSayanToken(_mainWrapper).isApprovedForAll(
                sender,
                txSender
            )) {
            return true;
    ...
    function getProxy() public override view returns (address) {
        address doubleProxy = IDFOSuperSayanToken(_mainWrapper).doubleProxy();
