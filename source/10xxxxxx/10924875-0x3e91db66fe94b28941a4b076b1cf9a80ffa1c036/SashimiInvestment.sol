...
contract SashimiInvestment is ISashimiInvestment, Ownable {
    ...
    function chooseProvider(address _token, uint256 _providerId) onlyAdmin public {
        ...
        if (IERC20(ISashimiVault(provider.vault).profit()).allowance(address(this), router) == 0)
            IERC20(ISashimiVault(provider.vault).profit()).safeApprove(router, type(uint256).max);
