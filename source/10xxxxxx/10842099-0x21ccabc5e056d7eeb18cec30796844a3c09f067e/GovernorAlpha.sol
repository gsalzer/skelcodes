...
// Modified to work in the KASSIAHOTEL system

// all votes work on underlying _kassiahotelBalances[address], not balanceOf(address)
...
//
// Changes made by KASSIAHOTEL after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since KASSIAHOTEL supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match KASSIAHOTEL as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "KASSIAHOTEL Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(kassiahotel.initSupply(), 4), 100); } // 4% of KASSIAHOTEL

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(kassiahotel.initSupply(), 100); } // 1% of KASSIAHOTEL
    ...
    KASSIAHOTELInterface public kassiahotel;
    ...
    constructor(address timelock_, address kassiahotel_) public {
        timelock = TimelockInterface(timelock_);
        kassiahotel = KASSIAHOTELInterface(kassiahotel_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(kassiahotel.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || kassiahotel.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = kassiahotel.getPriorVotes(voter, proposal.startBlock);
...
interface KASSIAHOTELInterface {
