...
contract EnergyChainValidator is ChainValidator {
    ...
    // Max allowed number of active transactors at the same time 
    uint256     public  maxNumOfTransactors;
    
    // Require new validators to be whitelisted
    bool requireWhitelistedValidators = true;

    // Require new transactors to be whitelisted
    ...
    function validateNewTransactor(uint256 deposit, address acc, uint256 actNumOfTransactors) external returns (bool) {
        ...
        if (maxNumOfTransactors != 0 && actNumOfTransactors >= maxNumOfTransactors) {
            return false;
        }
    ...
    /**
     * @notice Sets allowed max num of active transactors at the same time  
     * 
     * @param num
     **/
    function setMaxNumOfTransactors(uint256 num) external {
        require(existAcc(admins, msg.sender) == true, "Only admins can do internal changes");
        maxNumOfTransactors = num;
    }
