...
contract Stake is Ownable {
    ...
    IYFKAController public CONTROLLER = IYFKAController(0x0); // YFKA Controller
    ...
    function unstake() public {
        uint256 amount = stakes[msg.sender].sub(10**3);
    ...
    function YFKAforStake(uint256 stake) public view returns(uint256) {
        uint percentOfLPStaked = stake.mul(precision).div(POOL.totalSupply());
    ...
    function totalYFKAStaked() public view returns(uint points) {
        return YFKAforStake(POOL.balanceOf(address(this)));
    }
    
    function personalYFKAStaked() public view returns(uint points) {
        return YFKAforStake(stakes[msg.sender]);
    }
