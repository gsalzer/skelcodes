...
contract OneSplitUniswapV2PoolTokenView is OneSplitViewWrapBase, OneSplitUniswapV2PoolTokenBase {
        ...
        )
    {
        ...
        path[1] = details.tokens[leftoverIndex == 0 ? 1 : 0].token;

        uint256 exchangeAmount = _calcRebalanceAmount(
            amounts[leftoverIndex].sub(
                amounts[leftoverIndex == 0 ? 1 : 0].mul(
                    details.tokens[leftoverIndex].reserve
                ) / details.tokens[leftoverIndex == 0 ? 1 : 0].reserve
            ),
            path[0].balanceOf(address(uniswapRouter)), // need to recheck
            path[1].balanceOf(address(uniswapRouter))
        );

        (bool success, bytes memory data) = address(uniswapRouter).staticcall.gas(100000)(
            abi.encodeWithSelector(
        ...
        returnAmount = returnAmount.add(
            amountsOutAfterSwap[leftoverIndex]
                .mul(details.totalSupply)
                .div(details.tokens[leftoverIndex].reserve)
        );
...
contract OneSplitUniswapV2PoolToken is OneSplitBaseWrap, OneSplitUniswapV2PoolTokenBase {
    ...
    function _swapToUniswapV2PoolToken(
        ...
        if (redeemAmounts[0] == amounts[0]) {
            return;
        }

        uint256 leftoverIndex = redeemAmounts[0] > amounts[0] ? 0 : 1;
        IERC20[] memory path = new IERC20[](2);
        path[0] = tokens[leftoverIndex];
        path[1] = tokens[leftoverIndex == 0 ? 1 : 0];

        uint256 exchangeAmount = _calcRebalanceAmount(
            amounts[leftoverIndex] - redeemAmounts[leftoverIndex],
            path[0].balanceOf(address(uniswapRouter)),
            path[1].balanceOf(address(uniswapRouter))
        );
        ...
        address(uniswapRouter).call.gas(1000000)(
            abi.encodeWithSelector(
                ...
                amountsOut[leftoverIndex == 0 ? 1 : 0],
                uint256(0),
                uint256(0),
                address(this),
                now.add(1800)
