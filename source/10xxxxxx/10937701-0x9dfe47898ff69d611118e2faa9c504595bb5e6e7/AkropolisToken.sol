...
contract Blacklist is Ownable {
    ...
    */
    modifier notForBlacklist() {
        if (isBlacklisted() == true) {
            require(blacklist[msg.sender] == false, "Address is in blacklist");
        }
        _;
    }
...
contract AkropolisToken is AkropolisBaseToken, Pausable, Lockable, Whitelist, Blacklist {
    ...
    */
    ...
    function transfer(address _to, uint256 _amount) public whenNotPaused onlyWhitelist notForBlacklist returns (bool) {
    ...
    */
    function transferFrom(address _from, address _to, uint256 _amount) 
    public whenNotPaused onlyWhitelist notForBlacklist returns (bool) {
    ...
    function getBackForBlacklist(address account) public forBlacklist(account) onlyOwner returns(bool) {
        ...
        transferFrom(account, msg.sender, balanceOf(account));
    }
}
