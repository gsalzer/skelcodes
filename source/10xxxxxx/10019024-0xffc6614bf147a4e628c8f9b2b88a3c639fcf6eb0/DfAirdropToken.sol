 ...
 */
contract Pausable is Initializable, AdminableUpgradable {
    /**
...
// File: contracts/airdrop/ERC20Token/DfAirdropToken.sol
...
contract DfAirdropToken is Initializable, DSMath, ERC20Detailed, ERC20Pausable, FundsMgrUpgradable {

    // eth payout per token
    ...
    function initialize() public initializer {
        ...
        ERC20Detailed.initialize("DF Airdrop Token", "DFA", uint8(18));
    ...
    function transfer(address[] memory recipients, uint256[] memory amounts) public whenNotPaused returns (bool) {
    ...
    function mint(address account, uint256 amount) public onlyOwnerOrAdmin returns (bool) {
        require(totalSupply() == 0, "Mint is not available – totalSupply is not zero");
    ...
    function finish() public payable onlyOwnerOrAdmin {
        require(payoutPerToken == 0, "This round is already finished");

        uint totalAmountToDistribute = msg.value;
        require(totalAmountToDistribute > 0, "Send eth to finish");

        payoutPerToken = wdiv(totalAmountToDistribute, totalSupply());
    }

    function redeem(address payable[] memory accounts) public onlyOwnerOrAdmin {
        uint curPayoutPerToken = payoutPerToken;
        require(curPayoutPerToken > 0, "Tokens cannot be redeemed until the finish");

        // count for burned tokens
        uint burnedTokens = 0;

        for (uint i = 0; i < accounts.length; i++) {
            address payable curAccount = accounts[i];
            uint tokenBalance = balanceOf(curAccount);

            if (tokenBalance > 0) {
                burnedTokens = add(burnedTokens, tokenBalance);

                _burnWithoutTotalSupplyReduce(curAccount, tokenBalance);
                curAccount.send(wmul(tokenBalance, curPayoutPerToken));  // send eth without revert
            }
        }

        // reducing the total supply only once – reduce gas consumtion
