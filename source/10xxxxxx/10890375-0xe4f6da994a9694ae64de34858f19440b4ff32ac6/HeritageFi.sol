...
contract HeritageFi {
	...
	uint256 constant private INITIAL_SUPPLY = 37000e18;
	uint256 constant private BURN_RATE = 37;
	uint256 constant private SUPPLY_FLOOR = 50;
	uint256 constant private MIN_FREEZE_AMOUNT = 1e18;

	string constant public name = "Heritage Fi";
	string constant public symbol = "HFI";
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		uint256 _burnedAmount = _tokens * BURN_RATE / 100000;
		if (totalSupply() - _burnedAmount < INITIAL_SUPPLY * SUPPLY_FLOOR / 100000 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
	...
	function _unfreeze(uint256 _amount) internal {
		...
		uint256 _burnedAmount = _amount * BURN_RATE / 100000;
		info.scaledPayoutPerToken += _burnedAmount * FLOAT_SCALAR / info.totalFrozen;
		info.totalFrozen -= _amount;
