...
// File: contracts/vaults/dego/IDegoPool.sol
...
// File: contracts/vaults/dego/DegoVault.sol
...
contract DegoVault is SashimiVault{
    ...
    function depositAll() override onlyController external {
        ...
        if (amount > 0)
        {
            ...
            IERC20(token).safeApprove(lend, amount);
            IDegoPool(lend).stake(amount);
        }
    }
    ...
    function earned() override view external returns(uint256){
        return IDegoPool(lend).earned(address(this));
    }
