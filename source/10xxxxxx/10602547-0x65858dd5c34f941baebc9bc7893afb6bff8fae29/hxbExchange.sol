...
contract HXBTRANSFORM is Events {
    ...
    IUniswapV2Pair internal uniHxbEthInterface = IUniswapV2Pair(uniHXBETH);
    ...
    //hxy contract setup
    address internal hxyAddress = 0x0FFF95D5ab18c763c42C209F137C47354af104a8;
    ...
    {
        //get price 
        (uint reserve0, uint reserve1,) = uniHxbEthInterface.getReserves();
        uint hxb = uniV2Router.quote(ethLiquidity, reserve1, reserve0);
        //mint
    ...
    {
        //get price 
        (uint reserve0, uint reserve1,) = uniHxbHxyInterface.getReserves();
        uint hxb = uniV2Router.quote(hxyLiquidity, reserve1, reserve0);
        //mint
        require(hxbControl.mintHXB(hxb.mul(hxbControl.mintRatio()), address(this)), "could not mint HXB");
        //send
        ...
        synchronized
    {
        ...
        //allocate funds
        ethLiquidity += msg.value.mul(6).div(100);//60%
        ethDivs += msg.value.mul(4).div(100);//40%
        //get HXB price
        (uint reserve0, uint reserve1,) = uniHxbEthInterface.getReserves();
        uint hxb = uniV2Router.quote(msg.value, reserve1, reserve0);
        ...
        synchronized
    {
        ...
        //allocate funds
        hexLiquidity += hearts.mul(6).div(100);//60%
        hexDivs += hearts.mul(4).div(100);//40%
        //get HXB price
        (uint reserve0, uint reserve1,) = uniHexHxbInterface.getReserves();
        ...
        synchronized
    {
        ...
        //allocate funds
        hxyLiquidity += value.mul(6).div(100);//60%
        hxyDivs += value.mul(4).div(100);//40%
        //get HXB price
        (uint reserve0, uint reserve1,) = uniHxbHxyInterface.getReserves();
        uint hxb = uniV2Router.quote(value, reserve1, reserve0);
    ...
    function setHxbEthExchange(address exchange)
        public
        onlyAdmins
    {
        uniHXBETH = exchange;
        uniHxbEthInterface = IUniswapV2Pair(uniHXBETH);
    }
    ...
    function setHxbHxyExchange(address exchange)
        public
        onlyAdmins
    {
        uniHXBHXY = exchange;
        uniHxbHxyInterface = IUniswapV2Pair(uniHXBHXY);
    }
