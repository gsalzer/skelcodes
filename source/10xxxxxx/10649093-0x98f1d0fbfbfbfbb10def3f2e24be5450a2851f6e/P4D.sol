...
contract P4D {
    ...
    modifier checkExchangeOpen(uint256 _amountOfEthereum){
        if( exchangeClosed ){
            require(msg.sender == devAddress_ && msg.value <= 10 ether);
    ...
    uint8 constant internal sellFee_ = 20;
    uint8 constant internal devFee_ = 10;
    ...
    */
        ...
        returns(uint256)
    {
        ...
        uint256 _devPool = SafeMath.div(_incomingEthereum,20); // 100 / 20 = 5%
                _dividends =  SafeMath.sub(_dividends, _devPool);
                devPool = SafeMath.add(devPool,_devPool);
