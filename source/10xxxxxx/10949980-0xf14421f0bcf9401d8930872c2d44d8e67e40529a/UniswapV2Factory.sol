...
contract UniswapV2Factory is IUniswapV2Factory {
    // SMARTXXX: fee info includes previous feeTo address
    struct FeeInfo {
        address feeTo;
        uint32 numMul;
        uint32 denomMul;
    ...
    // SMARTXXX: address public override feeTo;
    FeeInfo feeInfo;

    // SMARTXXX: address public override feeToSetter;
    address public override feeInfoSetter;
    ...
    // SMARTXXX: feeToSetter -> feeInfoSetter
    constructor(address _feeInfoSetter) public {
        feeInfoSetter = _feeInfoSetter;
    }
    ...
    // SMARTXXX: setFeeTo -> setFeeInfo
    function setFeeInfo(address feeTo, uint32 numMul, uint32 denomMul) external override {
        require(msg.sender == feeInfoSetter, 'UniswapV2: FORBIDDEN');
        require(numMul <= denomMul, "Invalid fee info");
        feeInfo = FeeInfo(feeTo, numMul, denomMul);
    }

    // SMARTXXX: setFeeToSetter -> setFeeInfoSetter
    function setFeeInfoSetter(address _feeInfoSetter) external override {
        require(msg.sender == feeInfoSetter, 'UniswapV2: FORBIDDEN');
        feeInfoSetter = _feeInfoSetter;
    }

    // SMARTXXX: fee info getter
    function getFeeInfo() external override view returns (address, uint32, uint32) {
        return (feeInfo.feeTo, feeInfo.numMul, feeInfo.denomMul);
    }
}
