...
contract Experiments is Ownable {
  using SafeMath for uint256;
  ...
  address public weth = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    IERC20 public yeldToken = IERC20(0x468ab3b1f63A1C14b361bC367c3cC92277588Da1);
    
    event Generated(uint256 tokens);

  // Converts DAI to ETH and returns how much ETH has been received from Uniswap
  function daiToETH(uint256 _amount) public returns(uint256) {
      ...
      uint[] memory amounts = IUniswap(uniswapRouter).swapExactTokensForETH(_amount, uint(0), path, msg.sender, now.add(1800));
      emit Generated(amounts[1]);
      return amounts[1];
  }

  // Buys YELD tokens paying in ETH on Uniswap and removes them from circulation
  // Returns how many YELD tokens have been burned
  function buyNBurn(uint256 _ethToSwap) public returns(uint256) {
    address[] memory path = new address[](2);
    path[0] = weth;
    path[1] = address(yeldToken);
    // Burns the tokens by taking them out of circulation, sending them to the 0x0 address
    uint[] memory amounts = IUniswap(uniswapRouter).swapExactETHForTokens.value(_ethToSwap)(uint(0), path, msg.sender, now.add(1800));
    emit Generated(amounts[1]);
    return amounts[1];
  }
