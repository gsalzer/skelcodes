...
contract OnyxZap is Secondary{
    ...
    function LetsInvest(address _TokenContractAddress, address payable _towhomtoissue) public payable{
        ...
        require(msg.value < b.mul(10), "You are trying to pool to much ETH");
        
        //Eth to trade for token
        uint c = (msg.value.mul(  b.mul(1597000).add(msg.value.mul(184600))  )).div(  b.mul(3189209).add(msg.value.mul(1166248))  );
        ...
        //pool eth and tokens
        uint amountTokenDesired = Token(_TokenContractAddress).balanceOf(address(this)).sub(tokensBefore);
        Token(_TokenContractAddress).approve(ROUTER, amountTokenDesired); //allow pool to get tokens
        Uniswap(ROUTER).addLiquidityETH.value( msg.value.sub(c) )(_TokenContractAddress, amountTokenDesired, 1, 1, _towhomtoissue, INF);
        
        //send back leftover ETH
        _towhomtoissue.transfer(address(this).balance.sub(balanceBefore.sub(msg.value)));
    }
