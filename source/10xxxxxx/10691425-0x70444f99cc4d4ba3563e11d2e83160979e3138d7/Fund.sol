...
contract Fund {
    ...
    function setAddrContract(address _addr) external onlyOwner {
        addContract = _addr;
    }
    ...
    function Game(uint _turns) external payable {
        ...
        if(game != msg.sender){ require(Contract_Interface(addContract).getUserStatusReferrers(msg.sender, now) == true, "Registered users only"); }
