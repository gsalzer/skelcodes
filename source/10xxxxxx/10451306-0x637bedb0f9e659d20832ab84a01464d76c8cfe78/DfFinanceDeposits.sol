...
//FlashLoanReceiverBase
{
    ...
    function claimComps(address dfWallet) public {
        require(msg.sender == owner || wallets[dfWallet].owner == msg.sender);
        address[] memory cTokens = new address[](2);
        cTokens[0] = CDAI_ADDRESS;
        cTokens[1] = CUSDC_ADDRESS;
        IDfWallet(dfWallet).claimComp(cTokens);
        IToken(COMP_ADDRESS).transfer(msg.sender, IToken(COMP_ADDRESS).balanceOf(address(this)));
    }
    ...
    function closeDepositDAI(address dfWallet, uint256 minDAIForCompound, bytes memory data) public balanceCheck {
        require(msg.sender == owner || wallets[dfWallet].owner == msg.sender);

        uint256 startBalance = ICToken(CDAI_ADDRESS).balanceOf(address(this));
        uint256 totalBorrowed = ICToken(CDAI_ADDRESS).borrowBalanceStored(dfWallet);

        require(startBalance >= totalBorrowed);

        IToken(DAI_ADDRESS).transfer(dfWallet, totalBorrowed);

        IDfWallet(dfWallet).withdraw(DAI_ADDRESS, CDAI_ADDRESS, DAI_ADDRESS, CDAI_ADDRESS);

        if (minDAIForCompound > 0) {
            exchangeInternal(IToken(COMP_ADDRESS), IToken(COMP_ADDRESS).balanceOf(dfWallet), IToken(DAI_ADDRESS), minDAIForCompound, data);
        }
        uint256 userBalance = sub(ICToken(CDAI_ADDRESS).balanceOf(address(this)), startBalance);
        IDfWallet(dfWallet).withdrawToken(DAI_ADDRESS, msg.sender, userBalance);
        if (minDAIForCompound == 0) IDfWallet(dfWallet).withdrawToken(COMP_ADDRESS, msg.sender, IToken(COMP_ADDRESS).balanceOf(dfWallet));
    }

    function createStrategyDeposit3(uint256 amountDAI, uint256 coef, uint256 targetAmount) public balanceCheck {
        address dfWallet = dfWalletFactory.createDfWallet();
        // Transfer tokens to wallet
        IToken(DAI_ADDRESS).universalTransferFrom(msg.sender, dfWallet, amountDAI);

        uint256 flashLoanAmount = amountDAI * coef / 25; // 75 => 3
        uint256 totalFunds = flashLoanAmount + amountDAI;

        IToken(DAI_ADDRESS).transfer(dfWallet, flashLoanAmount);

        IDfWallet(dfWallet).deposit(DAI_ADDRESS, CDAI_ADDRESS, totalFunds, DAI_ADDRESS, CDAI_ADDRESS, totalFunds * coef / 100);

        IDfWallet(dfWallet).withdrawToken(DAI_ADDRESS, address(this), flashLoanAmount);

        wallets[dfWallet] = UserData(msg.sender, amountDAI, targetAmount);

        // set dfWallet's DfFinanceClose2 contract state
        IDfWallet(dfWallet).setDfFinanceClose(address(this));

        emit DfOpenDeposit(dfWallet, amountDAI);
    }

    function createStrategyDeposit(uint256 amountDAI, uint256 targetAmount, uint8 step) public {
        // create dfWallet
        address dfWallet = dfWalletFactory.createDfWallet();
        ...
        uint256 amountToBorrowUSDC = amountDAI / 1e12 * 70 / 100;
        IDfWallet(dfWallet).deposit(
        ...
        if (step == 6) return;
        IDfWallet(dfWallet).deposit(DAI_ADDRESS, CDAI_ADDRESS, amountToBorrowDAI, address(0), address(0), 0);
        if (step == 7) return;
