...
*/
contract PineappleToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    modifier onlyPauser() {
        require(pauser == _msgSender(), "PineappleToken: caller is not the pauser.");
        _;
    }

    modifier whenNotPaused() {
        require(!paused, "PineappleToken: paused");
        _;
    }

    modifier when3DaysBetweenLastSnapshot() {
        require((now - lastRewardTime) >= 3 days, "PineappleToken: not enough days since last snapshot taken.");
        _;
    }
    ...
    function setUniswapPool() external onlyOwner {
        require(uniswapPool == address(0), "PineappleToken: pool already created");
    ...
    function setPauser(address newPauser) public onlyOwner {
        require(newPauser != address(0), "PineappleToken: pauser is the zero address.");
        pauser = newPauser;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        require(!paused || msg.sender == pauser, "PineappleToken: token transfer while paused and not pauser role.");
    }
    ...
    function claimRewards() external {
        require(unclaimedRewards[msg.sender] > 0, "PineappleToken: nothing left to claim.");
