...
contract Normalizer {
    ...
    function getPrice(address token) external view returns (uint) {
        if (yearn[token]) {
            ...
            return Compound(token).exchangeRateStored().mul(1e8).div(underlyingDecimals[token]);
        } else if (cream[token]) {
            return Cream(token).exchangeRateStored().mul(1e8).div(underlyingDecimals[token]);
        }
    }
}
