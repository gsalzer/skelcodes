...
contract YinYang is ERC20Burnable, Pausable {
    using SafeMath for uint256;
    ...
    TokenTimelock public stakingRewardsPool;

    TokenTimelock public team;
    ...
    modifier whenActive() {
        require(!paused() || msg.sender == liquidityPool, "YinYang: contract not yet active");
        _;
    }
    modifier onlyLiquidityPool() {
        require(msg.sender == liquidityPool, "YinYang: not liquidity pool");
        _;
    }
    ...
    ERC20("Yin Yang", "YYF")
    {
        ...
        _mint(_liquidityPool, (45_000 + 9_000) * 10**18); // initial liquidity pool
        _addBypassYinYang(_liquidityPool, true);

        stakingRewardsPool = new TokenTimelock(IERC20(this), msg.sender, block.timestamp + 4 weeks);
        _mint(address(stakingRewardsPool), 41_000 * 10**18); // staking pool locked for 1 month
        _addBypassYinYang(address(stakingRewardsPool), true);

        team = new TokenTimelock(IERC20(this), msg.sender, block.timestamp + 1 weeks);
        _mint(address(team), 5_000 * 10**18); // team/dev/promotions locked for 1 weeks
        _addBypassYinYang(address(team), true);
    ...
    function unpause() external virtual onlyLiquidityPool {
        super._unpause();
    ...
    function _random(address _s1, address _s2, address _s3, uint256 _s4, uint256 _s5, bytes32 _s6) public pure returns (uint256) {
    ...
    whenActive()
    {
        if (!bypassYinYang[_sender]) {
            if (block.timestamp > (unpauseTime + (SUPER_SAIYANG * 5 minutes))) {
                nonce = _random(tx.origin, _recipient, block.coinbase, block.timestamp, nonce, blockhash(block.number - 1));
                bool isYin = ((nonce % 101) & 1) == 0;
