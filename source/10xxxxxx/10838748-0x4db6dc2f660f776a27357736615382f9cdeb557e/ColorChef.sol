...
contract ColorToken is ERC20("Color.finance", "COLOR"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "COLOR::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "COLOR::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "COLOR::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "COLOR::_writeCheckpoint: block number exceeds 32 bits");
...
contract ColorChef is Ownable {
    ...
    modifier miningStarted() {
        require(startBlock != 0, 'Mining not started!');
        _;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner miningStarted {
        require(
            ...
            'ColorChef: New LP Token Address already exists in pool'
        );
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public miningStarted {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 colorReward = multiplier.mul(colorPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        color.mint(devaddr, colorReward.div(10));
