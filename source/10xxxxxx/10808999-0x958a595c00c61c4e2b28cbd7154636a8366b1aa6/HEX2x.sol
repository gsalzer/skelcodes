...
/*
 *	HEX2x.com contract ⬣ 2x your bonus HEX!
 *
 *     _   _  _______   __ _____
 *    | | | ||  ___\ \ / // __  \
 *    | |_| || |__  \ V / `' / /'__  __  ___ ___  _ __ ___
 *    |  _  ||  __| /   \   / /  \ \/ / / __/ _ \| '_ ` _ \
 *    | | | || |___/ /^\ \./ /___ >  < | (_| (_) | | | | | |
 *    \_| |_/\____/\/   \/\_____//_/\_(_)___\___/|_| |_| |_|
 *
 *
 ...
 *
 *  The HEX code, which HEX2x is building on top of, cannot be changed.
 *  The HEX2x code cannot be changed either.
 *  Nobody is in charge of any of these contracts!
 *
 *  The HEX2x contract uses the original HEX contract at the given address:
 *      0x2b591e99afe9f32eaa6214f7b7629768c40eeb39
 *
 *  The HEX2x contracts address is:
 *      0x958A595c00C61c4e2B28CBd7154636a8366b1aA6
 *
 *  However HEX2x does have very little configuration abilities, which only can be executed by the so called ORIGIN_ADDR.
 *  Those configuration functions use the onlyOrigin modifier.
 *  They neither give the ORIGIN_ADDR or any other entity any power to switch off any functionalities of the HEX2x contract,
 *  nor give they the ORIGIN_ADDR or any other entity the power to control any users funds at any given time and state.
 *
 *  HEX2x has no off switch. Anyone can run the code at any given time and state.
 *
 *  HEX2x offers its users the ability to enter ETH to a HEX AA Lobby and to receive, not just +10% bonus HEX, as they would
 ...
 *  That means HEX2x users get the best deal for their bucket than at any other given place,
 *  if they wanna transform ETH into HEX, using HEXs AA lobby system.
 *
 *  HEX2x also offers an internal referral system, which pays the referring address at maximum of 6%
 ...
 *  open HEX LobbyDay via HEX2x.
 *
 *  HOW TO USE:
 *
 *      - enterLobby()
 *          - Call this payable method with at least minimumEntryAmount of wei, which is defined in globals,
 *            to enter a Lobby via HEX2x and to reserve your +20% bonus from the given days basic HEX/ETH ratio.
 ...
 *  FURTHER INFORMATION:
 *
 *  What happens if someone sends ETH to HEX2x:
 *      - If anyone sends ETH directly to HEX2x, it will automatically be forwarded to the enterLobby() method and
 *        therefore will behave exactly as if the sender would had called the enterLobby() directly with the sent ETH amount.
 *        This means the user will be able to exit the lobbyDay at which he sent the ETH to HEX2x, once it has closed.
 ...
 *          - It might be used to assure market competitiveness and stay attractive for HEX2xs users.
 *          - Do not expect of this ever happening. #noexpectation
 *      - moveExitLobbyPointer()
 *          - This could be called to gas optimize HEX2xs internal state behaviour for the users.
 *          - It doesn't affect the ability for any user to use HEX2x at any given time, at any given state.
 *      - flushExceedingHEX()
 *          - If HEX externally got send to HEX2x, whether on purpose or on accident,
 *            this method gives the ORIGIN_ADDR the ability to flush those externally added funds to itself.
 *          - It also assures, that HEX2x always has covered its required HEX liquidity, needed to serve all users exits,
 *            who ever entered any lobbyDay via HEX2x.
 ...
 *          - This method simply internally exits a HEX2x contracts LobbyDay from the original HEX contract,
 *            to provide a higher instant liquidity and therefore to lower the gas fees for users
 *            to exit their HEX2x entries.
 *          - It doesn't change any behaviour of the contract and can be called by anyone who wants to optimize
 *            the HEX2xs users gas costs.
 *            #noexpectation
 *
 *  HAVE NO EXPECTATIONS:
 *
 *  DO NOT HAVE ANY EXPECTATIONS FROM ANYONE REGARDING HEX/HEX2X/ETHEREUM OR ANY OTHER ENTITY!
...
contract HEX2x {
    ...
    // STATE VARIABLES - HEX2x specific:
    ...
    // it might increase if a higher competitiveness is desired for HEX2x - 1300‰ hard cap - no expectations
    ...
    struct HEX2xLobbyEntryQueueStore {
        uint40 headIndex;
        uint40 tailIndex;
    }
    ...
    // Day --> HEX2x Lobby ETH amount
    mapping(uint256 => uint256) private HEX2xLobbyETHperDay;
    // Day --> HEX2x Lobby HEX amount
    mapping(uint256 => uint256) private HEX2xLobbyHEXperDay;
    // Day --> HEX2x Lobby ETH amount per entry
    mapping(uint256 => uint256[]) private HEX2xLobbyETHperDayEntries;
    // Address --> Day --> HEX2x Users Lobby ETH amount
    mapping(address => mapping(uint256 => uint256)) private HEX2xUsersLobbyETHperDay;
    // Day --> LobbyQueueStore
    mapping(uint256 => HEX2xLobbyEntryQueueStore) private HEX2xLobbyEntryQueue;
    ...
    modifier onlyOrigin {
        require(msg.sender == ORIGIN_ADDR, "HEX2x: only ORIGIN_ADDR");
        _;
    }
    ...
    function _enterLobby(address referrer) private {
        require(msg.value >= globals.minimumEntryAmount, "HEX2x: below minimumEntryAmount");
        ...
        HEX2xLobbyETHperDay[currentDay] += msg.value;
        HEX2xUsersLobbyETHperDay[msg.sender][currentDay] += msg.value;
        HEX2xLobbyETHperDayEntries[currentDay].push(msg.value);
        HEX2xLobbyEntryQueue[currentDay].tailIndex++;
    ...
    function exitLobbyOnBehalf(address userAddress, uint256 lobbyDay) external {
        ...
        require(
            ...
            "HEX2x: Only for incentivized users, from day 345 on"
        );
    ...
    function _exitLobby(address userAddress, uint256 lobbyDay, uint256 currentDay) private {
        uint256 ETHtoExit = HEX2xUsersLobbyETHperDay[userAddress][lobbyDay];

        require(lobbyDay < currentDay, "HEX2x: Day not complete");
        require(ETHtoExit > 0, "HEX2x: No entry from this user, this day");

        uint256 HEXtoExit = _getUsersHEXtoExit(userAddress, lobbyDay);
        delete HEX2xUsersLobbyETHperDay[userAddress][lobbyDay];
    ...
    function _getUsersHEXtoExit(address userAddress, uint256 lobbyDay) private returns (uint256 HEXtoExit){
        _updateHEX2xLobbyHEXperDay(lobbyDay);
        return ((HEX2xUsersLobbyETHperDay[userAddress][lobbyDay]
            .mul(HEX2xLobbyHEXperDay[lobbyDay])).div(HEX2xLobbyETHperDay[lobbyDay]));
    }

    function _updateHEX2xLobbyHEXperDay(uint256 lobbyDay) private {
        if (HEX2xLobbyHEXperDay[lobbyDay] == 0) {
            ...
            uint256 basicHEXperDay =
                (HEXinLobby.mul(HEX2xLobbyETHperDay[lobbyDay])).div(HEX.xfLobby(lobbyDay));
                HEX2xLobbyHEXperDay[lobbyDay] = (basicHEXperDay.mul(totalShares)).div(basicShares);
        }
    }
    ...
    function _exitTillLiquidity(uint256 liquidity, uint256 currentDay) private {
        ...
        uint40 cachedHeadIndex = HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex;

        uint256 startIndex = HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex;
        uint256 startLiquidity = HEX.balanceOf(address(this));
        uint256 currentLiquidity = startLiquidity;

        while (currentLiquidity < liquidity) {
            if (cachedHeadIndex < HEX2xLobbyEntryQueue[cachedExitLobbyPointer].tailIndex) {
                uint256 addedLiquidity =
                    (HEX2xLobbyETHperDayEntries[cachedExitLobbyPointer][cachedHeadIndex]
                    .mul(HEX2xLobbyHEXperDay[cachedExitLobbyPointer])).div(HEX2xLobbyETHperDay[cachedExitLobbyPointer]);
                ...
                if(cachedHeadIndex != HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex)
                    HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex = cachedHeadIndex;
                ...
                cachedHeadIndex = HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex;
        ...
        if (HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex != cachedHeadIndex)
            HEX2xLobbyEntryQueue[cachedExitLobbyPointer].headIndex = cachedHeadIndex;
    ...
    function exitContractLobbyDay(uint256 lobbyDay, uint40 count) external {
        ...
        if (count > 0)
            HEX2xLobbyEntryQueue[lobbyDay].headIndex += count;
        else
            HEX2xLobbyEntryQueue[lobbyDay].headIndex = HEX2xLobbyEntryQueue[lobbyDay].tailIndex;

        if (HEX2xLobbyEntryQueue[lobbyDay].headIndex == HEX2xLobbyEntryQueue[lobbyDay].tailIndex)
    ...
    function changeMinimumEntryAmount(uint256 newMinimumEntryAmount) external onlyOrigin {
        require(10000000000000000 <= newMinimumEntryAmount && newMinimumEntryAmount <= 50000000000000000, "HEX2x: INV VAL");
    ...
    function raiseUserShares(uint256 newUserSharesInPerMill) external onlyOrigin {
        require(newUserSharesInPerMill.add(20) <= totalShares, "HEX2x: 1300 CAP");
        require(newUserSharesInPerMill > userShares, "HEX2x: INCREASE");
    ...
    function moveExitLobbyPointer(uint256 newLobbyPointerDay) external onlyOrigin {
        require(newLobbyPointerDay >= globals.initDay && newLobbyPointerDay < HEX.currentDay(), "HEX2x: INV VAL");
    ...
    function flushExceedingHEX() external onlyOrigin {
        ...
        require(exceedingLiquidity > 0, "HEX2x: 0 Exceeding");
    ...
    function flushERC20(IERC20 _token) external onlyOrigin {
        require(_token.balanceOf(address(this)) > 0, "HEX2x: 0 BAL");
        require(address(_token) != address(HEX), "HEX2x: !HEX");
