   ...
   function buyPool(uint _pool) public payable userExist {
       ...
       if(_pool==1){
        ...
        if (sent) {
            ...
            if(_pool==10){
               require(msg.value == 500 ether,'Incorrect Value');
               (,,uint256 payment_received) = dbInst.pool10users(poolCurrentuser);
            }
            if(payment_received>=3)
            {
