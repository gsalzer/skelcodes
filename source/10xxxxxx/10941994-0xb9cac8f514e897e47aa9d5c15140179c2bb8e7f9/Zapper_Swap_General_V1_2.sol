...
contract Zapper_Swap_General_V1_2 is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using Address for address;
        ...
        returns (uint256 tokensUnwrapped, address toToken)
    {
        ...
        if (_FromTokenContractAddress == address(wethContract)) {
            wethContract.withdraw(tokens2Trade);
            tokensUnwrapped = address(this).balance.sub(initialEthbalance);
            toToken = address(0);
        } else if (cToken[_FromTokenContractAddress] != address(0)) {
            require(
