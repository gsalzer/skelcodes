...
contract NORIRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 norisSold, uint256 norisFromReserves, uint256 norisToReserves);
    ...
    /// @notice NORI token address
    address public noriAddress;
    ...
    /// @notice pair for reserveToken <> NORI
    ...
    /// @notice Whether or not this token is first in uniswap NORI<>Reserve pair
    bool public isToken0;

    constructor(
        address noriAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(noriAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == noriAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap NORI<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        NORITokenInterface nori = NORITokenInterface(noriAddress);

        if (positive) {
            require(nori.norisScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < nori.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = nori.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = nori.rebase(epoch, indexDelta, positive);
        assert(nori.norisScalingFactor() <= nori.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            nori.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.norisToUni) {
                // if the amount from reserves > norisToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                nori.mint(uniswap_pair, uniVars.norisToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            nori.mint(uniswap_pair, uniVars.norisToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            nori.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.norisToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.norisToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess nori in the reserve
        uint256 excess = nori.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          norisToUni: tokens_to_max_slippage, // how many noris uniswap needs
          amountFromReserves: excess, // how much of norisToUni comes from reserves
          mintToReserves: 0 // how much noris protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.norisToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire nori reserves, and minting part of total
