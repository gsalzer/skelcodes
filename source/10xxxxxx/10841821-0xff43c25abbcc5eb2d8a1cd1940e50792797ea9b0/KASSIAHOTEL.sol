...
import "./KASSIAHOTELGovernance.sol";

contract KASSIAHOTELToken is KASSIAHOTELGovernanceToken {
    // Modifiers
    modifier onlyGov() {
    ...
    function initialize(
        ...
        public
    {
        require(kassiahotelsScalingFactor == 0, "already initialized");
        name = name_;
        symbol = symbol_;
        decimals = decimals_;
    }
    ...
    */
    ...
    function _maxScalingFactor()
        ...
        returns (uint256)
    {
        // scaling factor can only go up to 2**256-1 = initSupply * kassiahotelsScalingFactor
        // this is used to check if kassiahotelsScalingFactor will be too high to compute balances when rebasing.
    ...
    */
        ...
        internal
    {
      ...
      uint256 kassiahotelValue = amount.mul(internalDecimals).div(kassiahotelsScalingFactor);
    ...
    */
    ...
    {
        if (indexDelta == 0) {
          emit Rebase(epoch, kassiahotelsScalingFactor, kassiahotelsScalingFactor);
          return totalSupply;
        }

        uint256 prevKassiahotelsScalingFactor = kassiahotelsScalingFactor;

        if (!positive) {
           kassiahotelsScalingFactor = kassiahotelsScalingFactor.mul(BASE.sub(indexDelta)).div(BASE);
        } else {
            uint256 newScalingFactor = kassiahotelsScalingFactor.mul(BASE.add(indexDelta)).div(BASE);
            if (newScalingFactor < _maxScalingFactor()) {
                kassiahotelsScalingFactor = newScalingFactor;
            } else {
              kassiahotelsScalingFactor = _maxScalingFactor();
            }
        }

        totalSupply = initSupply.mul(kassiahotelsScalingFactor);
        emit Rebase(epoch, prevKassiahotelsScalingFactor, kassiahotelsScalingFactor);
        return totalSupply;
    }
}

contract KASSIAHOTEL is KASSIAHOTELToken {
    /**
        ...
        public
    {
        ...
        kassiahotelsScalingFactor = BASE;
        _kassiahotelBalances[initial_owner] = initSupply_.mul(10**24 / (BASE));
