...
// Liquidity pool allows a user to stake Uniswap liquidity tokens (tokens representaing shares of ETH and HOPS tokens in the Uniswap liquidity pool)
...
contract LiquidityPool {
    ...
    constructor(address _uniswapPair, address _hopsToken) public {
        uniswapPair = IERC20(_uniswapPair);
        hopsToken = IERC20(_hopsToken);
        minStakeDurationDays = 1;
        owner = msg.sender;
        rewardAdjustmentFactor = 4E23;
        stakingEnabled = true;
    ...
    function withdrawLiquidityTokens(uint256 numPoolTokensToWithdraw) external {
        ...
        hopsToken.transfer(msg.sender, tokensOwed);             
    ...
    function hopsTokenBalance() external view returns (uint256) {
        return hopsToken.balanceOf(address(this));
    }
    ...
    function updateHopsToken(address _hopsToken) external onlyOwner {
        hopsToken = IERC20(_hopsToken);
    }
    ...
    function transferHopsTokens(uint _numTokens) external onlyOwner {
        hopsToken.transfer(msg.sender, _numTokens);
    }
