...
contract SignatureHelper {
    ...
    string constant private EIP712_DOMAIN_NAME = "DF.Help";
    ...
    bytes32 constant private EIP712_DEAL_STRUCT_SCHEMA_HASH = keccak256(abi.encodePacked(
        "DealUsdSignature(",
        "uint256 coefficient,",
    ...
    FundsMgrUpgradable
{
    ...
    modifier onlyVerified(
        ...
        uint[8] memory _params  // coef [0], profitPercent [1], valueUsdc [2],
                                // entryEthPrice [3], maxEntryEthPrice [4], expireTime [5],
                                // signNonce [6], ethType[7]
    ) {
        // check signature
        require(
            isDealUsdVerified(
                /* walletOwner, signature,
                coef, profitPercent, valueUsdc, entryEthPrice, maxEntryEthPrice, expireTime, signNonce */
        ...
        signedNonces[_walletOwner][_params[6]] = true;
        _;
    }
    ...
    function isDealUsdVerified(
        address _addr,
        bytes memory _signature,  // signature is v, r, s for ECDSA
        uint[7] memory _params  // coef [0], profitPercent [1], valueUsdc [2],
                                // entryEthPrice [3], maxEntryEthPrice [4], expireTime [5]
                                // signNonce [6]
    ) public view returns(bool) {
        // sign is expired, used or canceled
        if (now > _params[5] ||
            signedNonces[_addr][_params[6]]) {
            return false;
        }

        // entryEthPrice is too high
        if (_params[3] > _params[4]) {
            return false;
        }

        // This recreates the message hash that was signed on the client
        bytes32 hash = _getDealHash(
            /* coef, profitPercent, valueUsdc, maxEntryEthPrice, expireTime, signNonce */
            [_params[0], _params[1], _params[2], _params[4], _params[5], _params[6]]
        );
    ...
    // ** ONLY_OWNER_OR_ADMIN DEAL WITH USDC functions **
    ...
    useGasToken
    onlyOwnerOrAdmin
    /* _walletOwner, _signature, _params */
    ...
    returns(address dfWallet) {
        dfWallet = dealUsdInternal(
            ...
            _walletOwner, _params[1], _params[0], _params[2], _params[3], _exData, _params[7]
        );
    }

    // with GasToken usage
    function dealUsdWithPromo(
        address _walletOwner,
        bytes memory _signature,  // signature is v, r, s for ECDSA
        uint[8] memory _params,  // coef [0], profitPercent [1], valueUsdc [2],
                                // entryEthPrice [3], maxEntryEthPrice [4], expireTime [5],
    ...
    function dealUsdInternal(
        ...
        // return an eth loan – all eth
        uint balanceEth = address(this).balance;
        require(balanceEth >= totalLoanEth, "Not enough eth to repay the loan");
        transferEthInternal(address(loanPool), balanceEth);
    }
