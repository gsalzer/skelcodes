...
contract VestingTimeLock is Ownable{
    using SafeERC20 for TulipToken;

    TulipToken private _token;
    ...
    uint256 private _releaseCounter = 0;
    ...
    function totalSupply() public view returns (uint256) {
        uint256 daysSinceCreation = (now - _createdTime) / 60 /60 / 24;

        uint256 amount = (_releaseAmount * ((daysSinceCreation/_releaseTime)-(_releaseCounter))) * _decimalConverter;
        
        return amount;
    }

    function deposit(address token, address beneficiary, uint256 initialTransferAmount, uint256 releaseAmount, uint256 releaseTime) public onlyOwner{
        _token = TulipToken(token); 
        _beneficiary = beneficiary; 
        _releaseTime = releaseTime; 
        _releaseAmount = releaseAmount; 
        _createdTime = now;

        _token.safeTransferFrom(msg.sender, address(this), initialTransferAmount * _decimalConverter);
    }
    ...
    function release() public onlyOwner{
        ...
        require((daysSinceCreation/_releaseTime) > _releaseCounter, "Cannot transfer multiple times in same period");

        uint256 amount = (_releaseAmount * ((daysSinceCreation/_releaseTime)-(_releaseCounter))) * _decimalConverter;

        if(amount > _token.balanceOf(address(this))){
        ...
        _releaseCounter = _releaseCounter + (amount/(_releaseAmount * _decimalConverter));
    }

    function token() public view returns (TulipToken) {
        return _token;
    }
