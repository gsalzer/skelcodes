...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0xD7aCbfDb25dA17775E9d876B8231b51b1F21e52C;
...
contract Staker is Poolable{
    ...
    function stake(address staker, address payable ref) public payable{
        ...
        address poolAddress = Uniswap(FACTORY).getPair(arcAddress, WETHAddress);
        uint ethAmount = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
        uint tokenAmount = IERC20(arcAddress).balanceOf(poolAddress); //token in uniswap
        
        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(arcAddress).mint(address(this), toMint);
        
        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));
        
        uint amountTokenDesired = IERC20(arcAddress).balanceOf(address(this));
        IERC20(arcAddress).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(arcAddress, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(arcAddress).mint(msg.sender, amount);
    }
    ...
    function ethtimeCalc(uint arc) internal view returns(uint){
        return ( arc.mul(31557600 * DECIMAL) ).div( rewardValue() );
    }
}
