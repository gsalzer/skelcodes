...
contract Ethermaze {
    ...
    uint public PERIOD_LENGTH = 50 days;
    ...
    function payForLevel(uint _flag, uint _level, address _userAddress, uint _adminPrice, uint256 _amt) internal {
        ...
        if (users[referer[0]].levelExpired[_level] >= now) {
            ...
            emit getMoneyForLevelEvent(msg.sender, users[msg.sender].id, referer[0], users[referer[0]].id, _level, LEVEL_PRICE[_level].sub(_adminPrice), now);
        } else {
            if (loopCheck[msg.sender] < 12) {
                loopCheck[msg.sender] = loopCheck[msg.sender].add(1);

            emit lostMoneyForLevelEvent(msg.sender, users[msg.sender].id, referer[0], users[referer[0]].id, _level, LEVEL_PRICE[_level].sub(_adminPrice),now);
