...
// File: contracts/CryptoAvatars.sol
...
contract CryptoAvatars is Ownable, ERC721Full, MinterRole {
    ...
    function initializeCryptoAvatars() public initializer {
        ERC721.initialize();
        Ownable.initialize(msg.sender);
        ERC721Enumerable.initialize();
        ERC721Metadata.initialize("CryptoAvatars", "AVNFT");
        MinterRole.initialize(msg.sender);
    }

    function mintCryptoAvatar(string memory tokenURI) public onlyMinter returns (uint256) {
        uint256 tokenId = _mintCryptoAvatar(msg.sender, tokenURI);
        return tokenId;
    }

    function _mintCryptoAvatar(address owner, string memory tokenURI) internal returns (uint256) {
        ...
        _tokenCreator[newItemId] = owner;

        return newItemId;
    }

    //Add minter role to an address
    ...
    function burn(uint256 tokenId) public {
        ...
        if (_tokenCreator[tokenId]!=address(0)) {
          delete _tokenCreator[tokenId];
        }
        _burn(tokenId);
    }
    //Return the creator of a token
