/*
    ...
    Copyright 2019 The Zeedex Foundation
...
contract LibOrder is EIP712, LibSignature, LibMath {
    ...
    /**
     * Calculates the Keccak-256 EIP712 hash of the order using the Zeedex Protocol domain.
     *
     * @param order The order data struct.
     * @return Fully qualified EIP712 hash of the order in the Zeedex Protocol domain.
     */
 ...
 *
 * First, Relayers can opt into or out of the Zeedex liquidity incentive system.
 *
...
contract LibRelayer {
    ...
    /**
     * Join the Zeedex incentive system.
     */
    ...
    /**
     * Exit the Zeedex incentive system.
     * For relayers that choose to opt-out, the Zeedex Protocol
    ...
    /**
     * @return true if relayer is participating in the Zeedex incentive system.
     */
...
contract LibDiscount is LibOwnable {
    ...
    address public zdexTokenAddress;

    constructor(address _zdexTokenAddress) internal {
        zdexTokenAddress = _zdexTokenAddress;
    }

    /**
     * Get the ZDEX token balance of an address.
     *
     * @param owner The address to check.
     * @return The ZDEX balance for the owner address.
     */
    function getZdexBalance(address owner) internal view returns (uint256 result) {
        address zdexToken = zdexTokenAddress;

        // IERC20(zdexTokenAddress).balanceOf(owner)
        ...
        assembly {
            ...
            // No need to check the return value because zdexToken is a trustworthy contract
            result := call(
                gas,      // Forward all gas
                zdexToken, // ZDEX token deployment address
     ...
     * rate depends on how much ZDEX token is owned by the user. Values returned will be a percentage
     ...
     *                                              bar    rate
     *
     * Meaning if a user has less than 10000 (0x00002710) ZDEX, they will pay 100%(0x64) of the
     * standard fee.
     *
    ...
    function getDiscountedRate(address user) public view returns (uint256 result) {
        uint256 zdexBalance = getZdexBalance(user);
        ...
        // ZDEX Token has 18 decimals
        zdexBalance = zdexBalance.div(10**18);
...
contract ZeedexExchange is LibMath, LibOrder, LibRelayer, LibDiscount, LibExchangeErrors {
    using SafeMath for uint256;
    ...
    constructor(address _proxyAddress, address zdexTokenAddress)
        LibDiscount(zdexTokenAddress)
        public
    {
     ...
     * @param isParticipantRelayer Whether this relayer is participating in zdex discount
