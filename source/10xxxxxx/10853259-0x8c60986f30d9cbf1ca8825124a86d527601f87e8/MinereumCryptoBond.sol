...
function externalFunction(address _sender, uint256 _mneAmount, address[] memory _addressList, uint256[] memory _uintList) public {
	...
	{
		if (_sender == _addressList[0] || _addressList[0] == address(this) || _addressList[0] == address(mneContract)) revert('invalid referal');
		
		uint bonusAmountReferrer = _mneAmount * ReferrerRate / 100;
		
		BonusDay[_addressList[0]].push(now);
		BonusFrom[_addressList[0]].push(_sender);
		BonusAmount[_addressList[0]].push(bonusAmountReferrer);
		BonusPaid[_addressList[0]].push(false);
		BonusPaidDate[_addressList[0]].push(0);
		
		if (BonusDay[_addressList[0]].length - 1 == 0)
...
function PayoutBonus(address _address, uint i) public {
	if (i > BonusAmount[msg.sender].length -1) revert('no bonus index found');	
