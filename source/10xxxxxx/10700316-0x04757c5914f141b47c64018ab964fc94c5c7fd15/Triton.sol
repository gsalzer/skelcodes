...
contract Triton is ERC20Interface, Owned {
    ...
    constructor() public {
        ...
        _maxMintable = 1000000 * 10**uint(decimals);  
        owner = msg.sender;
    }
    
    mapping(address => bytes32) public Tritonaddress;
    event MigrateEvent(address indexed addr, bytes32 Tritonaddress, uint256 balance);
    function migrateTriton(bytes32 registeraddress) public {
        if (migrationOn){
            Tritonaddress[msg.sender] = registeraddress;  //store the Triton wallet address in the Ethereum blockchain
            ...
            emit ErrorEvent (msg.sender, "Attempted to migrate before Triton Migration has begun.");
        }
    }
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        if(pauseOn){
            emit ErrorEvent(msg.sender, "Contract is paused. Please migrate to the native chain with migrateTriton.");
