...
contract DMEX {
	...
	function closeFuturesContract (bytes32 futuresContract, uint256 price);
...
contract DMEX_Oracle is usingProvable {
	...
	event LogOracleRequest(bytes32 queryId, bytes32 futuresContractHash, string priceUrl, string pricePath);
    event LogOracleCallback(bytes32 queryId, bytes32 futuresContractHash, string result, bytes proof);
