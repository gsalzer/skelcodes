...
contract PositionTokenLogic is SplittableToken {
        ...
        returns (uint256)
    {
        ...
        if (loanAmountAvailableInContract < loanAmountOwed) {
            ...
            if (loanAmountOwed > loanAmountAvailableInContract) {
                // allow at most 10% loss during this step
                require(
                    loanAmountOwed
                    .sub(loanAmountAvailableInContract)
                    .mul(10**20)
                    .div(loanAmountOwed) <= 10**19,
    ...
    function adminBurn(
        ...
        onlyOwner
    {
        _burn(_who, _tokenAmount, _assetAmount, _price);
    }
