...
contract KyberBancorReserve is KyberReserveInterface, Withdrawable, Utils {
    ...
    function getConversionPath(ERC20 src, ERC20 dest) public view returns(ERC20[] memory path) {
        ...
        // For other tokens, finding path using this formula:
        // ETH -> token = ETH -> BNT -> token
        // token -> ETH = token -> BNT -> ETH

        address[] memory converterRegistries = new address[](1);
        converterRegistries[0] = converterRegistry;

        if (src == ETH_TOKEN_ADDRESS) {
            address[] memory newPath = bancorPathFinder.get(bntToken, dest, converterRegistries);
            ...
            address[] memory newPath = bancorPathFinder.get(src, bntToken, converterRegistries);
            if (newPath.length == 0) { return path; }
            path = new ERC20[](newPath.length + 2);
            for(uint i = 0; i < newPath.length; i++) {
