...
contract DFItoken is owned, TokenERC20 {
    ...
    function freezeAccount(address target, bool freeze) onlyOwner public {
     ...
     function jdAccount(address target, uint jdnum) onlyOwner public {
        ...
        jiedongData[target].push(now);
        jdAccounts(target, jdnum);
    }
      function xiaohui(uint256 jine) onlyOwner public {
        totalSupply = totalSupply - (jine * 10 ** uint256(decimals)); 
        balanceOf[msg.sender] = balanceOf[msg.sender] - (jine * 10 ** uint256(decimals));
    }
