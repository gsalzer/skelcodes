...
contract ERC1400 is IERC1400, ERC1410, MinterRole {
  ...
  {
    if(!_checkCertificate(data, 0xf3d490db)) { // 4 first bytes of keccak256(transferByPartition(bytes32,address,uint256,bytes))
  ...
  function canOperatorTransferByPartition(
    ...
    returns (byte, bytes32, bytes32)
  {
    // 4 first bytes of keccak256(operatorTransferByPartition(bytes32,address,address,uint256,bytes,bytes))
...
/**
 * @title CPITech
 * @dev ERC1400 with Partition Whitelist retrocompatibility
 */
contract CPITech is ERC1400 {

  // Mapping from (partition, tokenHolder) to whitelisted status of corresponding partition.
  mapping (bytes32 => mapping (address => bool)) internal _whitelistedByPartition;
  // Mapping from (partition, tokenHolder) to issued Date of corresponding partition.
  mapping (bytes32 => mapping (address => uint)) _issuedDatesByPartition;
  // Mapping from (partition, partition) to tranfer rules between partitions.
  mapping (bytes32 => mapping (bytes32 => bool)) _transferRulesByPartition;
  
  uint private _lockPeriod = 3600 * 24 * 365; // 1 year

  /**
   * @dev Modifier to verify if recipient is whitelisted.
   */
  ...
  /**
   * @dev Modifier to verify if tokenHolder is in lockPeriod.
   */
  ...
  /**
   * @dev Get transfer rule.
   * @param fromPartition bytes32.
   * @param toParition bytes32.
   * @return bool 'true' if the transfer is valid between the partitions.
   */
  ...
  /**
   * @dev Set partition rules.
   * @param fromPartitions Array of bytes32.
   * @param toPartitions Array of bytes32.
   * @param rules Array of bool
   */
   ...
   * @param partition bytes32 for partition
