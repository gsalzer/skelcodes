...
contract TradeEth2DAI {
    ...
    function getOrdersSellAmountInTokens(bool isEthToDai, uint firstOrderToCheck, uint numOrders) public view
        returns(uint [] memory offerPayAmtTokens, uint [] memory offerBuyAmtTokens) 
    {
        uint offerId = INVALID_ID;
        uint i;
        ...
        offerPayAmtTokens = new uint[](numOrders);
        offerBuyAmtTokens = new uint[](numOrders);
        
        for (i = 0; i < firstOrderToCheck; i++) {
            
            (offerId, , ) = getNextBestOffer(dstToken, srcToken, 1, offerId);
            
            if (offerId == 0) return(offerBuyAmtTokens, offerPayAmtTokens);
        }
        ...
        for (i = 0; i < numOrders; i++) {
            ...
            (offerId, offerPayAmt, offerBuyAmt) = getNextBestOffer(dstToken, srcToken, 1, offerId);
            
            offerBuyAmtTokens[i] = offerBuyAmt / 10 ** 18;
            offerPayAmtTokens[i] = offerPayAmt / 10 ** 18;
            
            if(offerId == 0) return(offerBuyAmtTokens, offerPayAmtTokens);
        }
        
        return(offerBuyAmtTokens, offerPayAmtTokens);
    }
}
