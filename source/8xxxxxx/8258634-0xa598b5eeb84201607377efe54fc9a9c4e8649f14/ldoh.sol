...
//-------o 04 - Utility Function
	...
	/** Idata Mapping : 
	[1] Status						[5] IHO - Unix time starts					[9] Total available Balance (Now)				[13] Deposit Vault
	[2] Amount Deposit IHO			[6] Length of hold time in seconds			[10] XXX - Not Use				
	[3] Total reward provided		[7] Current IHO Round						[11] The IHO contract must be paid to user
	[4] Maximum number of winners	[8] Multipler								[12] The amount that can be withdrawn (Nodes)
	**/
	mapping (address => mapping (uint256 => mapping (uint256 => uint256))) public Idata2;		// Data per Round
	/** Idata Mapping : 
	[1] Current Reward ( Per Round )				[4] Number of active participants			
	[2] Reward Vault for All User ( Remaining )		[5] IHO - Unix time starts	
	[3] Total Deposit All User						[6] Status IHO Round >>> 0 = Disabled , 1 = Enabled
	
	[7] Status Pick Winner 0 = Available , 1 = Not Available		[8] Number of winners
	**/
	mapping (address => mapping (address => mapping (uint256 => uint256))) public Idata3;
	// [1] User Balance		[2] IHO Balance		[3] Unlock Time	

	mapping (address => mapping (address => mapping (uint256 => uint256))) public Idata4;		
	// Ticket Number per Round --- >> [TokenAddress][msg.sender][Roundnumber][Ticketnumber]
	
	mapping (address => mapping (uint256 => mapping (uint256 => address))) public Idata5;		
	// ETH Address by Ticket Number per Round --- >> [TokenAddress][Roundnumber][Ticketnumber][msg.sender]
	
	mapping (address => mapping (address => mapping (uint256 => address))) public Idata6;		
	// ETH address status in this round participation --- >> [TokenAddress][msg.sender][Roundnumber][status]
	
	mapping (address => mapping (uint256 => address[]))	public winlist;	
	
	
//-------o 05 - Initial Hold Offering (IHO)
	...
	function InitialHoldOffering(address tokenAddress) public {
		...
		// [1] Status [6] 0 = none, 1 =  The eth address has been registered in this round
