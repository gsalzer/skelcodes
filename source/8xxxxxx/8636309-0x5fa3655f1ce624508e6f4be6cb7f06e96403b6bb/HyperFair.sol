...
contract HyperFair is UtilFairWin, WhitelistAdminRole {
    ...
    function getUserInfo(address user, uint roundId) public isHuman() view returns(
        ...
        ct[0] = userInfo.id;           //id
        ct[1] = userInfo.staticLevel;  //抵押分红等级
        ct[2] = userInfo.dynamicLevel; //节点收益等级
        ct[3] = userInfo.allInvest;    //抵押总额
        ct[4] = userInfo.freezeAmount;  //锁定金额
        ct[5] = userInfo.unlockAmount;  //可提现金额
        ct[6] = userInfo.allStaticAmount;   //分红收益
        ct[7] = userInfo.allDynamicAmount;  //节点收益
        ct[8] = userInfo.hisStaticAmount;   //历史分红
        ct[9] = userInfo.hisDynamicAmount;  //历史节点
        ct[10] = getinvitedCountByCode(userInfo.inviteCode);   //邀请总数
