...
contract AggregatedTokenSwap {
    ...
    function setFee(uint _fee) public onlyOwner {
        ...
        require(_fee <= 1000, "Fee should not exceed 10%"); // <= 10%
        fee = _fee;
    }
    ...
    returns (uint returnAmount)
    {
        ...
        if (returnAmount > guaranteedAmount) {
            ...
            if (feeAmount.mul(10000).div(returnAmount) > 10) { // 0.1%

                feeAmount = returnAmount.div(1000); // 0.1%
            }

            if (referrer != address(0) && referrer != msg.sender && referrer != tx.origin) {
                feeAmount /= 2;
                if (_transfer(toToken, referrer, feeAmount, true)) {
                    returnAmount = returnAmount.sub(feeAmount);
                    emit OneInchFeePaid(toToken, referrer, feeAmount);
                } else {
                    feeAmount *= 2;
