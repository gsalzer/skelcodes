...
contract ERC20 is Context, IERC20 {
    struct lockamount {
        uint256 starttime;
        uint256 overtime;
        uint256 value;          // unlock total amount
    }
    ...
    uint256 private _onceduration;          // unlock once time
    
    uint256 private _lockonceamount;        // unlock once amount
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_lockamounts[sender].value > 0 && _lockamounts[sender].overtime > block.timestamp) {
            ...
            uint256 _val2 = _lockonceamount.mul(_val);  // unlock amount 
            uint256 _amount = _lockamounts[sender].value.sub(_val2);    // lock amount
            
            //require(_balances[sender] >= (_lockamounts[sender].value + amount), "ERC20: transfer lock amount exceeds balance");
...
// main 1000000000
contract HDAOToken is Context, ERC20, ERC20Detailed, Ownable {

    // metadata
    string public constant tokenName = "Test6HDao";
    string public constant tokenSymbol = "T6HDAO";
