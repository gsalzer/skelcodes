...
contract LoanTokenLogicV4 is AdvancedToken, OracleNotifierInterface {
        ...
        returns (uint256 loanAmountPaid)
    {
        ...
        require(loanAmountPaid <= loanAmountAvailableInContract, "38");
    ...
    {
        // borrowAmount
        require (sentAmounts[1] <= ERC20(loanTokenAddress).balanceOf(address(this)), "24");
        ...
        internal
    {
        ...
        if (tradeTokenAddress == address(0)) { // withdrawOnOpen == true
            if (loanTokenAddress == wethContract) {
                WETHInterface(wethContract).withdraw(withdrawalAmount);
                (success, ) = borrower.call.value(withdrawalAmount)("");
            } else {
