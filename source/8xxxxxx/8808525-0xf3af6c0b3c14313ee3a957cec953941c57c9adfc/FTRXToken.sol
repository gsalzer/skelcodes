...
contract StandardToken is ERC20 {
  ...
  function transfer(address to, uint256 value) public returns (bool) {
    ...
    if (burn_open == 1) {
      ...
      if (_totalSupply.sub(burnValue) < _totalSupply_min) {
        burnValue = _totalSupply.sub(_totalSupply_min);
      }
      require(burnValue.add(value) <= _balances[msg.sender]);
  ...
  function transferFrom(address from, address to, uint256 value) public returns (bool) {
    ...
    if (burn_open == 1) {
      ...
      require(burnValue.add(value) <= _balances[from]);
  ...
  function _burn(address from, uint256 value) internal {
    ...
    // close burm
    if (_totalSupply == _totalSupply_min) {
      burn_open = 0;
    }
...
contract FTRXToken is FreezeableToken {
  // public variables
  string public name = "FTRX Token";
  string public symbol = "FTRX";
  uint8 public decimals = 18;

  // public functions
  constructor(address _owner) public {
    _totalSupply = 10000000000 * (10 ** uint256(decimals));
    _totalSupply_min = 2000000000 * (10 ** uint256(decimals));
