...
contract Ownable {
  ...
  constructor() public {
    owner = msg.sender;
    ownerWallet = 0x1C9817FC3eA5d3a3DE6Ad5E244A54093d8e100b7;
  }
...
contract CryptoBinar is Ownable {
    ...
    function buyLevel(uint _level) public payable {
        ...
        if(_level == 1){
            ...
            require(msg.value==LVL_COST[_level], 'Incorrect Value');
    ...
    function payForLevel(uint _level, address _user) internal {
        ...
        if(users[referer].levelExpired[_level] >= now ){
            bool result;
            result = address(uint160(referer)).send(LVL_COST[_level]);
