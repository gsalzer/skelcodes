...
contract XNGAirDrop is Ownable {

  function XNGAirDrop() public {
  }

  function batchAirDrop(address token_address, address[] recipients, uint256 XNG) onlyOwner public {
    XNGToken token = XNGToken(token_address);
    for(uint i = 0 ; i < recipients.length ; i++) {
      address recipient = recipients[i];
      require(token.transfer(recipient, XNG));
    }
  }
