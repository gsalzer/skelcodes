...
contract Trading is Ownable {
    ...
    constructor(address payable _beneficiary, address payable _proveq, uint256 _fee) public {
    ...
    function trade(
        ...
        uint256 feeAmount = returnAmount.mul(fee).div(10000);
        uint256 platformFee = feeAmount.mul(4).div(5);
        _transfer(toToken, beneficiary, platformFee, false);
        _transfer(toToken, proveq, feeAmount.sub(platformFee), false);
    ...
    function setProveQ(address payable _proveq) external {
        require(msg.sender == address(proveq));
        proveq = _proveq;
        emit ProveQSet(_proveq);
    }
