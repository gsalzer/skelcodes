...
contract StablePrice is Managed {
  ...
  constructor(uint8 _priceInUSD) public {
...
contract MinutemanToken is ERC20, Ownable, Managed, StablePrice(10) {
  using SafeMath for uint256;
  ...
  constructor(address whitelistAddress) public {
    ...
    emit Mint(msg.sender, 10000 ether);
  }

  event Mint(address indexed receiver, uint256 value);

  modifier onlyWhitelisted() {
    require(whitelist.isWhitelisted(msg.sender), "Sender must be whitelisted");
    _;
  }
   ...
   */
  function approve(address spender, uint256 value) public returns (bool) {
    require(whitelist.isWhitelisted(spender));
