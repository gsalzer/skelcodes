...
contract ErbCoin {
    ...
    string constant public name = "ErbCoin";      //  token name
    string constant public symbol = "ERC";           //  token symbol
    ...
    //转账到账户的冻结部分
    function transferToFrozen(address _to, uint256 _value) public isRunning isOwner returns (bool success) {
        balances[msg.sender] = balances[msg.sender].sub(_value);
    ...
    //冻结部分释放
    function release(address _target, uint256 _value) public isRunning isOwner returns(bool){
    ...
    //注销冻结代币
    function revoke(address _target, uint256 _value) public isRunning isOwner returns(bool){
        frozenBalances[_target] = frozenBalances[_target].sub(_value);
        balances[zeroaddr] += _value;
        emit Revoke(_target, _value);
        return true;
    }
    ...
    function burn(uint256 _value) public {
        balances[msg.sender] = balances[msg.sender].sub(_value);
        balances[zeroaddr] += _value;
        emit Transfer(msg.sender, zeroaddr, _value);
    }
    ...
    event Revoke(address _target, uint256 _value);
}
