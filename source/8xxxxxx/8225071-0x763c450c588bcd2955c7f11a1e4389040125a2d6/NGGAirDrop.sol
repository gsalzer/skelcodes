...
contract NGGAirDrop is Ownable {
    
    constructor() public {
        
    }

  function batchAirDrop(address token_address, address[] recipients, uint256 NGG) onlyOwner public {
    NGGToken token = NGGToken(token_address);
    for(uint i = 0 ; i < recipients.length ; i++) {
      address recipient = recipients[i];
      require(token.transfer(recipient, NGG));
    }
  }
