  ...
  */
  function transfer(address _to, uint256 _amount) public returns (bool success) {
    ...
    if (!released) { // before exchanged
      if ( (lockupParticipants[msg.sender].lockType == LockupType.Advisor) || (msg.sender == ownerWallet) ) {
 ...
 * @dev Token representing GenieKRW.
 */
 contract GenieKRW is BurnableToken {
    ...
    function lockupforTeam(address team, uint256 daysafter, uint256 amount) public onlyOwner
    {
        lockup = Lockup({
    ...
    function lockupforAdvisor(address advisor) public onlyOwner
    {
        lockup = Lockup({
                          lockupTime:0 days,
                          lockupAmount:0 * 10 ** uint256(Decimals), 
                          lockType:LockupType.Advisor
                          });
        lockupParticipants[advisor] = lockup;
    }
