...
contract LockableToken is ERC20Mintable, AdminRole {
     ...
     */
    function lock(address[] calldata accounts) external onlyAdmin {
        for (uint256 i = 0; i < accounts.length; i++) {
            _locked[accounts[i]] = true;
        }
    }
     ...
     * @param accounts array of addresses.
     */
    function unlock(address[] calldata accounts) external onlyAdmin {
        for (uint256 i = 0; i < accounts.length; i++) {
            _locked[accounts[i]] = false;
        }
    }
 ...
 */
contract GIGJToken is LockableToken {

    // name of the token
    string private _name = "Gigajoule";
    // symbol of the token
    string private _symbol = "GIGJ";
    // decimals of the token
    uint8 private _decimals = 2;

    // initial supply
    uint256 internal constant INITIAL_SUPPLY = 65625000 * 100;

    // if additional minting of tokens is impossible
    bool public mintingFinished;

    // updated modifier to prevent minting of tokens when it is finished.
    modifier onlyMinter() {
        if (mintingFinished) {
            revert();
        }
