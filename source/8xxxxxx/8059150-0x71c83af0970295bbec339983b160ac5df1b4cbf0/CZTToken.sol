...
contract CZTToken is owned {
    string public name;//token name
    string public symbol;//token symbol = czt
    uint8 public decimals = 18;//Tokens to support the number of decimal digits
    uint256 public totalSupply;//token total nums

    mapping (address => uint256) balanceOf;//mapping address balance
    ...
    /**
     ...
     centralMinter 合同所有者。0表示启动时调用本身
     */
    ...
    function getBalance(address sender) view public returns ( uint256 balance) {
