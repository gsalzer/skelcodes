...
// File: contracts/FixedTokenSwap.sol
  ...
  Destructible
{
  ...
  function isOfferAllowed(address _offerTokenAddress)
    public
    view
    returns (bool)
  {
  ...
  function getAmount(
    ...
    returns(uint)
  {
    return _offerTokenAmount.mul(buyRate).div(10 ** buyRateDecimals);
  }
    ...
    returns (bool)
  {
    ...
    require(
      isOfferAllowed(_userOfferTokenAddress),
      "_userOfferTokenAddress not allowed"
    );
    uint toWalletAmount = _userOfferTokenAmount;
    uint toUserAmount = getAmount(_userOfferTokenAmount);
    ...
    require(
      ...
      "the transfer of the quote from user to the Wallet SC failed"
    );
    ...
    require(
      ...
      "the transfer of the base from the Wallet SC to the user failed"
    );
    emit LogTokenSwap(
      msg.sender,
      quoteTokenAddress,
