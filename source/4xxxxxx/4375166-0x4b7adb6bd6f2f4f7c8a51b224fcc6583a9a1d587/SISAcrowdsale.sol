...
contract SISAcrowdsale is Math {

	SISA public SISA_token; 
	address public founder; 
	address public sale_address = 0x1111111111111111111111111111111111111111;

	//Price / 100
	uint public price = 44817927170868350;
	//uint price = div(100 ether, 267750 * ether)

	uint public begins = 1511136000;
	uint public ends = 1513728000;

	function priceChange() {
		if (now > begins) {
			price = 44817927170868350;
			//price = 37348272642390287
		} else if (now > (begins + 1 days)) {
			price = 48715138229204725;
		} else if (now > (begins + 7 days)) {
			price = 50929462694168575;
		} else if ( now > (begins + 14 days)) {
			price = 53354675203414694;
		} else if (now > (begins + 21 days)) {
			price = 56022408963585430;
		}
	}
	modifier isExpired() {
		if (now < begins) {
	...
	function SISAcrowdsale(address tokenAddress, address founderAddress) {
		...
		isExpired 
	{
		...
		if (founder.send(amount)) {
			SISA_token.issueTokens(msg.sender, tokens);
		} else {
			throw;
		}
	}
