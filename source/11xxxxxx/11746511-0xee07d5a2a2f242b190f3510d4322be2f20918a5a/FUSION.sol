...
contract FUSION is Context, IERC20, Ownable {
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "FUSION: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'FUSION: The contract it self cannot be excluded');
        require(!_isExcluded[account], "FUSION: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "FUSION: Account is already included");
