...
// Strategy will take into account slippage now of 1%
...
contract StabilizeStrategySGRArbV3 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public percentTradeTrigger = 3000; // 10% change in value will trigger a trade
    uint256 public maxSlippage = 500; // 0.5% max slippage is ok
    uint256 public maxPercentSell = 100000; // 100% of the tokens are sold to the cheapest token if slippage ok
    ...
    function simulateExchange(address _inputToken, address _outputToken, uint256 _amount) internal view returns (uint256) {
        // We will be using Uniswap for all swaps
    ...
    function getCheaperUniswapToken() internal view returns (uint256) {
    ...
    function checkAndSwapTokens(address _executor) internal {
        ...
        uint256 targetID = getCheaperUniswapToken();
        uint256 length = tokenList.length;
