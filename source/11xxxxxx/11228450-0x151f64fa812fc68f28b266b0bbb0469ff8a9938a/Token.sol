 ...
 */
contract Token is Initializable, ContextUpgradeSafe, AccessControlUpgradeSafe, ERC20BurnableUpgradeSafe, ERC20PausableUpgradeSafe {
    ...
    function initialize(string memory name, string memory symbol, uint8 decimals, uint256 totalSupply) public {
        __Token_init(name, symbol, decimals, totalSupply);
    }

    function __Token_init(string memory name, string memory symbol, uint8 decimals, uint256 totalSupply) internal initializer {
        __Context_init_unchained();
        __AccessControl_init_unchained();
        __ERC20_init_unchained(name, symbol);
        __ERC20Burnable_init_unchained();
    ...
    notBlacklisted(from)
    {
        require(to != address(this), "Token: can't transfer to contract address itself");
        super._beforeTokenTransfer(from, to, amount);
    }
    ...
    function withdrawTokens(address tokenContract) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Token [withdrawTokens]: must have admin role to withdraw");
    ...
    */
    function blacklist(address _account) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Token [blacklist]: must have admin role to blacklist");
    ...
    */
    function unBlacklist(address _account) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Token [unBlacklist]: must have admin role to unBlacklist");
