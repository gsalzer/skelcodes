...
contract PresalecSWOP is Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        cSWOP = _cSWOP;
        cSWOPTarget = _cSWOPTarget;
        weiTarget = _weiTarget;
        multiplier = cSWOPTarget.div(weiTarget);
    ...
    function setcSWOPTarget(uint256 _cSWOPTarget) external onlyOwner { 
        cSWOPTarget = _cSWOPTarget;
        multiplier = cSWOPTarget.div(weiTarget);
    }
    ...
    function setStartTimestamp(uint256 _startTimestamp) external onlyOwner { startTimestamp = _startTimestamp; }
    function setEndTimestamp(uint256 _endTimestamp) external onlyOwner { endTimestamp = _endTimestamp; }
    function setClaimTimesatmp(uint256 _claimTimestamp) external onlyOwner { claimTimestamp = _claimTimestamp; }
    function setMinWei(uint256 _minWei) external onlyOwner { minWei = _minWei; }
    function setMaxWei(uint256 _maxWei) external onlyOwner { maxWei = _maxWei; }
    ...
    function withdrawcSWOP() external onlyOwner {
        require(block.timestamp > endTimestamp, "presale has not yet ended");
        cSWOP.transfer(msg.sender, cSWOP.balanceOf(address(this)).sub(totalOwed));
    }

    function claim() external {
        require(block.timestamp > claimTimestamp, "claiming not allowed yet");
        ...
        require(cSWOP.transfer(msg.sender, amount), "failed to claim");
    }

    function buy() public payable {
        require(block.timestamp > startTimestamp, "presale has not yet started");
        require(block.timestamp < endTimestamp, "presale already ended");
        ...
        require(totalOwed.add(amount) <= cSWOP.balanceOf(address(this)), "sold out");
