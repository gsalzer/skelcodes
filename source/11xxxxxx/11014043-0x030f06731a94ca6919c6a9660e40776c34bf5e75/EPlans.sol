...
contract EPlans {
    ...
    constructor(address _token) public {
        ...
        for (uint8 i = 1; i <= LAST_LEVEL; i++) {
            users[owner].activeQ8Levels[i] = true;
        }
    }
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        if (EPlansToken(ePlansToken).balanceOf(_this) > levelPrice[uint8(1)]) {
            EPlansToken(ePlansToken).transfer(userAddress, levelPrice[uint8(1)]);
        }
    ...
    function buyNewLevel(uint8 level) external payable {
        ...
        if (EPlansToken(ePlansToken).balanceOf(_this) > levelPrice[level]) {
            EPlansToken(ePlansToken).transfer(msg.sender, levelPrice[level]);
        }
    }
    ...
    function updateQ8Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (q8Matrix[level].secondLevelReferrals.length == 3) {//reinvest
    ...
    function withdrawELS(address _user, uint _value) public OnlySuper {
        EPlansToken(ePlansToken).transfer(_user, _value);
    }
}
...
contract EPlansToken is UnboundedRegularToken {

    uint8 constant public decimals = 18;
    string constant public name = "EPlans";
