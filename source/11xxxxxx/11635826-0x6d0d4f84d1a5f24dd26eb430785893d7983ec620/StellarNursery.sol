...
// StellarNursery is BigBang's left hand and kinda a scientific process. It can form Rigel from pretty much anything!
//
// This contract handles "serving up" rewards for xRigel holders by trading tokens collected from fees for Rigel.

contract StellarNursery {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapV2Factory public factory;
    address public orion;
    address public rigel;
    address public weth;

    constructor(IUniswapV2Factory _factory, address _orion, address _rigel, address _weth) public {
        factory = _factory;
        rigel = _rigel;
        orion = _orion;
        weth = _weth;
    ...
    function convert(address token0, address token1) public {
        ...
        // Then we convert the WETH to Rigel
        _toRIGEL(wethAmount);
    }

    // Converts token passed as an argument to WETH
    function _toWETH(address token) internal returns (uint256) {
        // If the passed token is Rigel, don't convert anything
        if (token == rigel) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, orion, amount);
            return 0;
        }
        // If the passed token is WETH, don't convert anything
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, rigel), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, rigel), new bytes(0));
        return amountOut;
    }
