...
contract HDTokenWrapper {
    ...
    //UNI-V2(HD-ETH)
    IERC20 public stakeToken = IERC20(0x187Be5160B6f48a110a1d47e6ff191C3b1FF05F7); 
...
/**
 * XD-UNI-V2(HD-ETH) Pool
 */
contract XDUNIV2HDETHPool is HDTokenWrapper, IRewardDistributionRecipient {
    IERC20 public xd = IERC20(0x2b8b691Ada4D6EdD20325fB45358A25d85Ce7324);
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "XD-UNI-V2(HD-ETH)-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "XD-UNI-V2(HD-ETH)-POOL: Cannot withdraw 0");
    ...
    function getReward() public checkStart updateReward(msg.sender){
        ...
        if (reward > 0) {
            ...
            xd.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    modifier checkStart(){
        require(block.timestamp > startTime,"XD-UNI-V2(HD-ETH)-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "XD-UNI-V2(HD-ETH)-POOL: Pool is closed");
        _;
    }
