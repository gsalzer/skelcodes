...
NONAME Crowdsale Contract.

Token Contract: 0x09Ff2267EDA5F1e99db5b0dDd85555a7B46C76BB
...
contract IncreasingPriceCrowdsale is TimedCrowdsale {
    ...
    function getCurrentRate() public view returns (uint256) {
        ...
        // Early participants get 5% bonus (within first hour)
        if (openingTime() + 1 hours > block.timestamp) {
            _rate = _rate.mul(105).div(100);
        }

        // Referrer get 20% bonus
        if (isReferrer(msg.sender)) {
            _rate = _rate.mul(120).div(100);
        }
...
// File: contracts\NonameCrowdsale.sol
...
{
    uint256 private _finalRate = 100;
