...
contract StakingMultiRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, IEmergency {
    ...
    function getReward() public nonReentrant override updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            if (rewardsRatioBps > 0) {
                uint256 bonus = reward.mul(rewardsRatioBps).div(10000);
                if (bonus > 0 && bonusToken.balanceOf(address(this)) >= bonus) {
    ...
    function emergencyWithdraw(IERC20 token) external override {
        require(
            ...
            address(token) != address(bonusToken) &&
            address(token) != address(stakingToken), "forbidden token");
