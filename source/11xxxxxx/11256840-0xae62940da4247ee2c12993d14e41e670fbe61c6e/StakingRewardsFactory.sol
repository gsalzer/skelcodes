...
// Rebasing staking coin support added for ousd
// Users should recieve rebased rewards
    ...
    ReentrancyGuard
{
        ...
        updateReward(msg.sender)
    {
        ...
        stakingToken.safeTransfer(
            msg.sender,
            amount.mul(1e18).div(ousd.rebasingCreditsPerToken())
        );
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            if (_totalSupply == 0) {
                rewards[account] = rewardRate.mul(rewardsDuration).mul(1e17).mul(rewardRate.sub(rewardRate).add(1e18).add(1e18).div(1e18)).add(earned(account));
                // Ensure the provided reward amount is not more than the balance in the contract.
                // This keeps the reward rate in the right range, preventing overflows due to
                // very high values of rewardRate in the earned and rewardsPerToken functions;
                // Reward + leftover must be less than 2^256 / 10^18 to avoid overflow.
