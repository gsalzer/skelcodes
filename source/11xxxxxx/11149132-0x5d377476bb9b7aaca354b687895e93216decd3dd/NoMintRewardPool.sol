/**
 * Autoharvest Finance (https://autoharvest.finance)
*/
...
interface IPlayerBook {
    ...
    function _referRewardRate() external view returns(uint256);
...
contract NoMintRewardPool is LPTokenWrapper, IRewardDistributionRecipient, Controllable {
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            if (tx.origin == msg.sender || !IController(controller()).greyList(msg.sender)) {
                ...
                uint256 toRefer = reward.mul(referRewardRate).div(baseRate);
                rewardToken.safeTransfer(msg.sender, reward.sub(toRefer));
                rewardToken.safeTransfer(refer, toRefer);
                emit RewardPaid(msg.sender, reward.sub(toRefer));
                emit RewardPaid(refer, toRefer);
        ...
        updateReward(address(0))
    {
        ...
        if(isOwner()) {
            ...
            rewardToken.mint(rewardAddr, reward.div(10));
        }
