...
// File: contracts/strategies/StabilizeStrategyFRAXArb.sol
...
// Strategy takes into account 1% slippage estimate for large sells
...
contract StabilizeStrategyFRAXArb is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxPoolSize = 3000000e18; // The maximum amount of frax tokens this strategy can hold, 3 mil by default
    uint256 public percentTradeTrigger = 3000; // 3% change in value will trigger a trade
    uint256 public maxSlippage = 100; // 0.1% max slippage is ok
        ...
        for(uint256 i = 0; i < length; i++){
            if(i != targetID){
                ...
                if(tokenList[i].token.balanceOf(address(this)) <= _minTradeTarget){
                    ...
                    sellBalance = estimateSellAtMaxSlippage(i, localTarget); // This will select a balance with the max slippage
                }
                if(sellBalance > _maxTradeTarget){
                    // If greater than the maximum trade allowed, match it
                    sellBalance = _maxTradeTarget;
    ...
    function finishChangePercentSlippage() external onlyGovernance timelockConditionsMet(11) {
    ...
    function finishChangeMaximumPoolSize() external onlyGovernance timelockConditionsMet(12) {
