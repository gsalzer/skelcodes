...
contract RFIextremeBattle is Context, Ownable {
    ...
    //bet limit
    uint256 public _minBetAmount = 0.024 ether;
    ...
    modifier finalGame(){
        ...
        if (block.timestamp.sub(lastGameTime) >= 24 hours){
        ...
        if(daoAddress != address(0)){
            ...
            uint256 realAm = (_rfiextremetoken.balanceOf(address(this))).sub(lastPool[0].add(lastPool[1]));
    ...
    function redeem() external finalGame()  {
        
        require(lastPool[looser] > 0 && betBalances[_msgSender()][gameNo.sub(1)][winner] > 0);
       
        uint256 amountOnWinning = (betBalances[_msgSender()][gameNo.sub(1)][winner].mul(100)).div(lastPool[looser]);
