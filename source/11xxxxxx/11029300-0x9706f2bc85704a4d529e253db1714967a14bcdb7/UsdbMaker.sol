...
contract UsdbMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _usdb, address _weth) public {
        factory = _factory;
        usdb = _usdb;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, usdb), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, usdb), new bytes(0));
        return amountOut;
    }

    function _toUSDB(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, usdb));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
