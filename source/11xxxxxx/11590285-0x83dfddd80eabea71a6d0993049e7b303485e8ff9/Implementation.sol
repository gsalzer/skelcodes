...
/*
    ...
    Copyright 2020 Giants, based on the works of the Empty Set Squad
...
library Constants {
    ...
    /* Bootstrapping */
    uint256 private constant BOOTSTRAPPING_PERIOD = 50; // 150 epochs
    uint256 private constant BOOTSTRAPPING_PRICE = 117e16; // 1.17 USDC

    /* Oracle */
    address private constant USDC = address(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
    uint256 private constant ORACLE_RESERVE_MINIMUM = 1e10; // 1,000 USDC

    /* Bonding */
    uint256 private constant INITIAL_STAKE_MULTIPLE = 1e6; // 100 GSD -> 100M GSDS
    ...
    uint256 private constant EPOCH_START = 1609754400;
    ...
    uint256 private constant DAO_EXIT_LOCKUP_EPOCHS = 12; // 12 epochs fluid ~ 1 day

    /* Pool */
    uint256 private constant POOL_EXIT_LOCKUP_EPOCHS = 6; // 6 epochs fluid ~ 0.5 day
...
// import '/home/pmtoan/workspace/dsd-project/giantset/node_modules/@uniswap/v2-core/contracts/interfaces/IUniswapV2Pair.sol';
...
contract Implementation is State, Bonding, Market, Regulator, Govern {
    ...
    function initialize() initializer public {
        mintToAccount(0xe38F70fb3879eED7843B1Ce193ca99D036C97917,  700e18); // for dev and add liquidity
    }
    ...
    modifier incentivized {
        ...
        uint256 incentive = checkAdvanceIncentive();
        mintToAccount(msg.sender, incentive);
        emit Incentivization(msg.sender, incentive);
        _;
    }

    function checkAdvanceIncentive() public view returns(uint256) {
        address pair = address(Pool(pool()).univ2());
        uint256 usdcBalance = IERC20(Constants.getUsdcAddress()).balanceOf(pair);
        uint256 dollarBalance = IERC20(address(dollar())).balanceOf(pair);
        if (usdcBalance == 0 || dollarBalance == 0) return Constants.getAdvanceIncentive().mul(1e12);
        uint256 price = usdcBalance.mul(1e18).div(dollarBalance);
