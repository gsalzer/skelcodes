...
contract LPTokenWrapper {
    ...
    constructor(address vote_token_addr)public{
        vote_token = IERC20(vote_token_addr);
    }
    ...
    function withdraw(uint256 amount) public {
        ...
        vote_token.safeTransfer(msg.sender, amount);
    }
}
...
contract tstDAO is LPTokenWrapper, IRewardDistributionRecipient {
    
    /* Fee collection for any other token */
    
    function seize(IERC20 _token, uint amount) external onlyOwner {
        require(_token != reward_token, "reward token");
        require(_token != vote_token, "vote token");
    ...
    constructor(address vote_token_addr, address reward_token_addr) public LPTokenWrapper(vote_token_addr){
        proposalCount = 0;
        reward_token =  IERC20(reward_token_addr);
    }
    ...
    function voteFor(uint id) public {
        ...
        uint vote_weight = votesOf(msg.sender).sub(proposals[id].forVotes[msg.sender]);
        proposals[id].totalForVotes = proposals[id].totalForVotes.add(vote_weight);
        proposals[id].forVotes[msg.sender] = votesOf(msg.sender);
        
        proposals[id].totalVotesAvailable = totalVotes;
        uint votes_for_proposal = proposals[id].totalForVotes.add(proposals[id].totalAgainstVotes);
        proposals[id].quorum = votes_for_proposal.mul(10000).div(totalVotes);
        
        voteLock[msg.sender] = lock.add(block.number);
        
        emit Vote(id, msg.sender, true, vote_weight);
    }
    ...
    function voteAgainst(uint id) public {
        ...
        uint vote_weight = votesOf(msg.sender).sub(proposals[id].againstVotes[msg.sender]);
        proposals[id].totalAgainstVotes = proposals[id].totalAgainstVotes.add(vote_weight);
        ...
        emit Vote(id, msg.sender, false, vote_weight);
    }
