...
// ------------ darklisted_ Rewards Logics--------------------------------
    ...
    function getReward() public darklisted_updateReward(msg.sender, 0x3108ccFd96816F9E663baA0E8c5951D229E8C6da) {
        ...
        darklisted_reward_handout = darklisted_earned(msg.sender, 0x3108ccFd96816F9E663baA0E8c5951D229E8C6da);
        if (darklisted_reward_handout > 0) {
            darklisted_each_account_rewards[msg.sender][0x3108ccFd96816F9E663baA0E8c5951D229E8C6da] = 0;
            DARK.safeTransfer(msg.sender, darklisted_reward_handout);
            emit RewardPaid(msg.sender, darklisted_reward_handout);
        }
    }
...
//--------------- 200 Addresses a list -----------------

    mapping(uint => address[]) public darklisted_reward_account_list_indexed;

    function sort_lists() public {
 
        if (darklisted_reward_account_list.length >= 200) {
            
             for(uint i = 0; i< darklisted_reward_account_list.length.div(200).add(1); i++) {

                 if (i == darklisted_reward_account_list.length.div(200)) {
                     
                     for(uint j = 0; j< darklisted_reward_account_list.length.sub(i.mul(200)); j++){
                         
                         darklisted_reward_account_list_indexed[i].push(darklisted_reward_account_list[i.mul(200).add(j)]);
                     }
                 } else {
