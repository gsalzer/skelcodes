...
interface IDarkMatterOriginsStory {
  function unlockCard(address _account, uint256 _id) external;
}

contract DarkMatterCollectibleFarm is TokenWrapper, Ownable {
	ERC1155Tradable public dmc;
  IDarkMatterOriginsStory public story;
	...
	constructor(ERC1155Tradable _dmcAddress, IERC20 _erc20Address) public TokenWrapper(_erc20Address) {
		dmc = _dmcAddress;
	}

  function setStory(IDarkMatterOriginsStory _story) public onlyOwner {
    story = _story;
  }
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(dmc.totalSupply(card) < dmc.maxSupply(card), "Max cards minted");
        if (card < 8) {
          story.unlockCard(msg.sender, card);
        }

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		dmc.mint(msg.sender, card, 1, "");
