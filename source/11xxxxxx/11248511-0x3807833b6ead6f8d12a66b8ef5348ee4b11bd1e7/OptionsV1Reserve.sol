...
contract OptionsV1Reserve {
    ...
    uint constant public LN = 3059614075; // ln(SP/ST) NORMSDIST(d1|d2) * sqrt(PERIOD / 365)
    uint constant public BASE = 1e39; // 1e10 * 1e18
    
    function quote(address tokenIn, uint amount, address tokenOut) public view returns (uint minOut) {
        return ORACLE.current(tokenIn, amount, tokenOut);
    }
    ...
    function createCall(address token, address payment, uint amount, uint period) external {
        createOption(token, payment, amount, period, 0);
    }
    
    function createPut(address token, address payment, uint amount, uint period) external {
        createOption(token, payment, amount, period, 1);
    }
    
    function fees(address token, uint cost, uint period) public view returns (uint) {
        return LN.mul(Math.sqrt(period.mul(1e11).div(365))).mul(vol(token)).mul(cost).div(BASE);
    }
    
    function fee(address token) public view returns (uint) {
        return fees(token, price(token), 28 days);
    }
    
    function price(address asset) public view returns (uint) {
        if (asset == WETH) {
            return quote(WETH, 1e18, DAI);
        } else {
            uint _weth = quote(asset, uint(10)**IERC20(asset).decimals(), WETH);
            return quote(WETH, _weth, DAI);
        }
    }
    
    function cost(address token, uint amount, address out) public view returns (uint) {
        return quote(token, amount, out);
    }
    ...
    function approve() external {
        ...
        IERC20(DAI).approve(address(CURVE), uint(0));
        IERC20(USDT).approve(address(CURVE), uint(0));
        IERC20(USDC).approve(address(CURVE), uint(0));
        
        IERC20(CRV3).approve(address(YEARN), uint(0));
        
        IERC20(DAI).approve(address(CURVE), uint(-1));
        IERC20(USDT).approve(address(CURVE), uint(-1));
        IERC20(USDC).approve(address(CURVE), uint(-1));
        
        IERC20(CRV3).approve(address(YEARN), uint(-1));
    }
    
    function toY3P() internal {
    ...
    function volatilityETH(address token) public view returns (uint) {
        return ORACLE.realizedVolatility(token,uint(10)**IERC20(token).decimals(), WETH, 24, 2);
    }
    
    function volatilityDAI() public view returns (uint) {
        return ORACLE.realizedVolatility(WETH, 1e18, DAI, 24, 2);
    }
    ...
    function vol(address token) public view returns (uint) {
        if (token == WETH) {
            ...
            uint volWETH = ORACLE.realizedVolatility(token, uint(10)**IERC20(token).decimals(), WETH, 24, 2);
            uint volDAI = ORACLE.realizedVolatility(WETH, 1e18, DAI, 24, 2);
            return volWETH.div(volDAI);
        }
    }
    
    function convert(address asset, uint total) public view returns (uint) {
        return total.mul(IERC20(asset).decimals()).div(IERC20(address(DAI)).decimals());
    }
    
    function createOption(address token, address payment, uint amount, uint period, uint optionType) public {
        require(payment == DAI || payment == USDC || payment == USDT);
        uint _costInWETH = cost(token, amount, WETH);
        uint _costInDAI = cost(WETH, _costInWETH, DAI);
        uint _fee = fees(token, _costInDAI, period);
        
        require(_fee > 0);
        
        uint _strike = _costInDAI.div(amount);
        
        IERC20(payment).transferFrom(msg.sender, address(this), convert(payment, _fee));
    ...
    function exercise(uint id) external {
        ...
        uint _costInWETH = cost(_pos.asset, uint(10)**IERC20(_pos.asset).decimals(), WETH);
        uint currentPrice = cost(WETH, _costInWETH, DAI);
        
        uint profit;
        if (_pos.optionType == 0) {
            require(_pos.strike <= currentPrice, "Current price is too low");
            profit = currentPrice.sub(_pos.strike).mul(_pos.amount);
        } else if (_pos.optionType == 1) {
            require(_pos.strike >= currentPrice, "Current price is too high");
            profit = _pos.strike.sub(currentPrice).mul(_pos.amount);
        }
