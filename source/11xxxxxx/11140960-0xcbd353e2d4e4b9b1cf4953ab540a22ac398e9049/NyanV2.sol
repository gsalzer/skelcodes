...
contract NyanV2 is ERC20, NyanV2DataLayout, Proxiable {
    ...
    modifier _updateRewards() {
        ...
        //     uint256 stakeNumerator = userStakeAmount.mul(1000);
      
        //     if (stakeNumerator > totalLP) {
        //       uint256 stakeTemp = stakeNumerator / totalLP + 5;
        //       uint256 stakerPoolOwnershipPerc = stakeTemp.div(10);
        //       uint256 stakerFraction = stakerPoolOwnershipPerc.mul(nyanRewardsPerDay).mul(10).add(5);
        //       uint256 userRewardPerDay = stakerFraction.div(10);
        //       uint256 timeSinceCheckpoint = userStake[msg.sender].lastBlockChecked.sub(lastNyanCheckpoint);
        //       if(timeSinceCheckpoint > 300) {
        //         uint256 timeMul = timeSinceCheckpoint.div(300);
        //         uint256 stakerShares = userRewardPerDay.div(21);
        //         userStake[msg.sender].nyanV2Rewards = userStake[msg.sender].nyanV2Rewards.add(stakerShares.mul(timeMul));
    ...
    function viewNyanRewards(address staker) delegatedOnly public view returns(uint256) {
        ...
        if (userStake[staker].stakedNyanV2LP > 0) {
            ...
            if (stakeNumerator > totalLP) {
              uint256 stakerPoolOwnershipPerc = stakeNumerator.div(totalLP).add(5).div(10);
              uint256 userRewardPerDay = stakerPoolOwnershipPerc.mul(nyanRewardsPerDay).mul(10).add(5).div(10);
              uint256 timeSinceCheckpoint = userStake[staker].lastBlockChecked.sub(lastNyanCheckpoint);
              if(timeSinceCheckpoint > 300) {
                uint256 timeMul = timeSinceCheckpoint.div(300);
                uint256 stakerShares = userRewardPerDay.div(21);
                currentRewards = userStake[staker].nyanV2Rewards.add(stakerShares.mul(timeMul));
              }
            }
