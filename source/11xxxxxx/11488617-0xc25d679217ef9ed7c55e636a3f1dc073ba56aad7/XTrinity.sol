...
contract XTrinity is Ownable {
    ...
    */
    ...
    function swap (IERC20 fromToken, IERC20 toToken, uint amount, bool slipProtect) public payable returns (uint returnAmount) {
        if (isETH(fromToken)) {
            amount = msg.value;
        ...
        uint minAmount;
        if (slipProtect) {            
            uint256 feeSlippage = returnQuoteAmount.mul(slippageFee).div(PC_DENOMINATOR);    
