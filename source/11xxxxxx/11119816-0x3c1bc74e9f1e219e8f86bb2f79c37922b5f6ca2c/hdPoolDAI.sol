...
contract hdPoolDAI is ERC20 {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    IERC20 public token = IERC20(0x6B175474E89094C44Da98b954EedeAC495271d0F); // DAI
    IERC20 public HDCORE = IERC20(0x63Da3b945e86FE2D1FCAf4eb138474E934E2B94f);
    ...
    uint public HDCOREMin = 50; // 0.5% default
    uint public constant HDCOREMax = 10000;

    bool public enableHDCOREYield = true;
    ...
    constructor (address _controller)
        public ERC20(
          string(abi.encodePacked("HDCORE ", ERC20(address(token)).name())),
          string(abi.encodePacked("h", ERC20(address(token)).symbol()))
        )
    {
    ...
    function setHDCOREBuyMin(uint _HDCOREMin) external {
        require(msg.sender == governance, "!governance");
        HDCOREMin = _HDCOREMin;
    }
    ...
    function setEnableHDCOREYield(bool _status) public {
        require(msg.sender == governance, "!governance");
        enableHDCOREYield = _status;
    }
    ...
    function withdraw(uint _shares) public {
        ...
        if (!enableHDCOREYield) {
          token.safeTransfer(msg.sender, r);
        } else {
          uint tokenToBuyHDCORE = r.mul(HDCOREMin).div(HDCOREMax);
          r = r.sub(tokenToBuyHDCORE);
          token.safeTransfer(msg.sender, r);

          if (tokenToBuyHDCORE > 0) {
              uint _before = IERC20(HDCORE).balanceOf(address(this));
              token.safeApprove(onesplit, 0);
              token.safeApprove(onesplit, tokenToBuyHDCORE);
              uint[] memory _distribution;
              uint _expected;
              (_expected, _distribution) = OneSplitAudit(onesplit).getExpectedReturn(address(token), address(HDCORE), tokenToBuyHDCORE, onesplitParts, 0);
              uint _expectedAfterSlippage = _expected.sub(_expected.mul(onesplitSlippageMin).div(onesplitSlippageMax));
              OneSplitAudit(onesplit).swap(address(token), address(HDCORE), tokenToBuyHDCORE, _expectedAfterSlippage, _distribution, 0);
              uint _after = IERC20(HDCORE).balanceOf(address(this));
              if (_after > _before) {
                  uint _diff = _after.sub(_before);
                  HDCORE.safeTransfer(msg.sender, _diff);
              }
          }
        }
    }
