...
forked from KIMCHI, SUSHI, KIMBAP, PRINT, AND KINGSWAP
but without public mint() function or bug
...
contract Token is ERC20("Farmerdefi.com", "Farm", 18, 0, 1000000), Ownable, Mintable {
...
contract FarmerChef is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        // claimable Farmer by a user is:
        //
        //   pending reward = (user.amount * pool.accFarmerPerLPShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accFarmerPerLPShare` (and `lastRewardedBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accFarmerPerLPShare; // Accumulated Farmer per share.
        uint256 lpTokenHolding;     // Amount of LP token hold by this pool
    }

    // Info of Farmer minting
    struct MintInfo {
        uint256 FarmerPerBlock;  // Amount of minted Farmer each block.
    ...
    // Amount of block spans for minting different amounts of Farmer
    ...
    function setMint(uint256 _mid, uint256 _block, uint256 _amount) public onlyOwner {
        ...
        mintInfo[_mid].FarmerPerBlock = _amount;
    ...
    function getFarmerAmountInRange(uint256 _from, uint256 _to) public view returns (uint256) {
        if(_from >= _to){
            return 0;
        }
        uint256 reward = 0;
        ...
        for( uint256 i = 0; i < mintInfo.length-1; i++){
            for( uint256 cblock = _from+1; cblock <= _to; cblock++){
                if(cblock >= mintInfo[i].mintStartBlock && cblock < mintInfo[i+1].mintStartBlock){
                    reward += mintInfo[i].FarmerPerBlock;
                    blocksleft--;
                }
            }
        }
        reward += blocksleft.mul(mintInfo[mintInfo.length-1].FarmerPerBlock);
        return reward;
    }

    // View function to see pending Farmer on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        FarmerReward = FarmerReward.mul(1000e18);
        pool.accFarmerPerLPShare = pool.accFarmerPerLPShare.add(FarmerReward.div(lpSupply));
        pool.lastRewardedBlock = block.number;
    }

    // Deposit LP tokens to Chef for allocation.
    // Also used to claim the users pending Farmer tokens (_amount = 0)
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accFarmerPerLPShare).div(1e18).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accFarmerPerLPShare).div(1e18);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from Chef.
    // Also used to claim the users pending Farmer tokens
    ...
    function safeFarmerTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > FarmerBal) {
            Farmer.transfer(_to, FarmerBal);
        } else {
            Farmer.transfer(_to, _amount);
        }
    }
}
