...
contract XCOREVault is OwnableUpgradeSafe {
    ...
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = XCORE.balanceOf(address(this)).sub(coreBalance);
        
        if(newRewards > 0) {
            coreBalance = XCORE.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function safeCoreTransfer(address _to, uint256 _amount) internal {
        uint256 coreBal = XCORE.balanceOf(address(this));
        if (_amount > coreBal) {
            XCORE.transfer(_to, coreBal);
            coreBalance = XCORE.balanceOf(address(this));
        } else {
    ...
    function transferDevFee() public {
        ...
        if (pending_BURN_rewards > coreBal) {
            XCORE.transfer(burnaddr, coreBal);
            coreBalance = XCORE.balanceOf(address(this));
        } else {
            XCORE.transfer(burnaddr, pending_BURN_rewards);
            coreBalance = XCORE.balanceOf(address(this));
        }
        coreBal = XCORE.balanceOf(address(this));
        if (pending_TRESURY_rewards > coreBal) {
            XCORE.transfer(tresuryaddr, coreBal);
            coreBalance = XCORE.balanceOf(address(this));
        } else {
            XCORE.transfer(tresuryaddr, pending_TRESURY_rewards);
            coreBalance = XCORE.balanceOf(address(this));
        }
