...
// File: contracts/microCOREVault.sol
...
// microCORE Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract microCOREVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of microCOREs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accmicroCOREPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accmicroCOREPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. microCOREs to distribute per block.
        uint256 accmicroCOREPerShare; // Accumulated microCOREs per share, times 1e12. See below.
    ...
    function initialize(
        INBUNIERC20 _microCORE,
        address _devaddr,
        address superAdmin
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        DEV_FEE = 1000;
        microCORE = _microCORE;
        devaddr = _devaddr;
    ...
    // Note contract owner is meant to be a governance contract allowing microCORE governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accmicroCOREPerShare: 0,
                withdrawable : _withdrawable
            })
        );
    }

    // Update the given pool's microCOREs allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing microCORE governance consensus
    ...
    function pendingmicroCORE(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accmicroCOREPerShare = pool.accmicroCOREPerShare;

        return user.amount.mul(accmicroCOREPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the microCORE token.
    // ----
    uint256 private microCOREBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = microCORE.balanceOf(address(this)).sub(microCOREBalance);

        if(newRewards > 0) {
            microCOREBalance = microCORE.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 microCORERewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        microCORERewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 microCORERewardFee = microCORERewardWhole.mul(DEV_FEE).div(10000);
        uint256 microCORERewardToDistribute = microCORERewardWhole.sub(microCORERewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(microCORERewardFee);

        pool.accmicroCOREPerShare = pool.accmicroCOREPerShare.add(
            microCORERewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to microCOREVault for microCORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accmicroCOREPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accmicroCOREPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from microCOREVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming microCORE farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accmicroCOREPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safemicroCORETransfer(from, pending);
        }
    ...
    // Safe microCORE transfer function, just in case if rounding error causes pool to not have enough microCOREs.
    function safemicroCORETransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 microCOREBal = microCORE.balanceOf(address(this));
        if (_amount > microCOREBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", microCOREBal);

            microCORE.transfer(_to, microCOREBal);
            microCOREBalance = microCORE.balanceOf(address(this));

        } else {
            microCORE.transfer(_to, _amount);
            microCOREBalance = microCORE.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safemicroCORETransfer(devaddr, devSend);
        }
    ...
    // allowing microCORE governance token holders to do this functions.
