/**
 *Submitted for verification at vitFlex.net on 2021-01-02
*/
        ...
        for(i=0; i < 2; i++) {    // 1 level + 2 level only
	        if (_nextReferrer != address(0x0)) {
                if (i == 0) {
					...
					emit referralCommissionEvent(_playerAddress, companyAddress, amount.mul(5).div(2), now);  // send to company 50%
					emit referralCommissionEvent(_playerAddress, centerAddress,  amount.div(2), now);         // send to center  10%
