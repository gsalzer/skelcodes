...
library ConverterHelper {
    ...
    function calculateAddBptLiquidity(Balancer _balPool,
        ...
        uint _poolOutByAmount0 = _amount0.mul(_balTotalSupply).div(_balToken0Amount);
        uint _poolOutByAmount1 = _amount1.mul(_balTotalSupply).div(_balToken1Amount);
        return Math.min(_poolOutByAmount0, _poolOutByAmount1);
    }
    ...
    function calculateRemoveBptLiquidity(Balancer _balPool, uint _poolAmountIn,
        ...
        uint _balToken0Amount = _balPool.getBalance(_token0);
        uint _balToken1Amount = _balPool.getBalance(_token1);
        _amount0 = _balToken0Amount.mul(_poolAmountIn).div(_balTotalSupply);
        _amount1 = _balToken1Amount.mul(_poolAmountIn).div(_balTotalSupply);
    }
