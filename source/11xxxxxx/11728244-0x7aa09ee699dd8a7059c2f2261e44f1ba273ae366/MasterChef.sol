...
contract ZToken is ERC20("Rocket", "R"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Rocket::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Rocket::delegateBySig: invalid nonce");
        require(now <= expiry, "Rocket::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Rocket::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Rocket's (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "Rocket::_writeCheckpoint: block number exceeds 32 bits");
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to Rocket.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // Rocket must mint EXACTLY the same amount of Rocket LP tokens or
...
// will be transferred to a governance smart contract once ROCKET is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSushiPerShare; // Accumulated ROCKETs per share, times 1e12. See below.
    }
    ...
    // Bonus muliplier for early rockets makers.
