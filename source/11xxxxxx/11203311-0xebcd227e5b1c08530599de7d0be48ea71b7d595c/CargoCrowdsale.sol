...
contract CargoCrowdsale is Ownable, TimedCrowdsale, AllowanceCrowdsale {
    using SafeMath for uint256;

    uint256 _openingTime = now + 1 hours;
    uint256 _closingTime = now + 10 days;
    uint256 _rate = 100000;
    address payable _wallet = address(
        0x7C2A3ca02c8d15fa21cd94b6B560bb7672F03744
    );
    IERC20 _token = IERC20(0x86d70F5b6a6d6D6B570AcAEcbb4Fd9B41C3d850a);
    address _tokenWallet = address(0xDc4ac69d46cc2A67Bd1F2bD440B01C62d80A41A7);
    ...
    function rate() public view returns (uint256) {
        if (now <= _openingTime.add(24 hours)) {
            return _rate.mul(15).div(10);
        } else if (now <= _openingTime.add(96 hours)) {
            return _rate.mul(125).div(100);
        } else return _rate;
    }
