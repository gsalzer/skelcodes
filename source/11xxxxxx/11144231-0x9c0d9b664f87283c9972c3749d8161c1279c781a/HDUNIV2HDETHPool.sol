...
/**
 * HD-UNI-V2(HD-ETH) Pool
 */
contract HDUNIV2HDETHPool is HDTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "HD-UNI-V2(HD-ETH)-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "HD-UNI-V2(HD-ETH)-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"HD-UNI-V2(HD-ETH)-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "HD-UNI-V2(HD-ETH)-POOL: Pool is closed");
        _;
    }
