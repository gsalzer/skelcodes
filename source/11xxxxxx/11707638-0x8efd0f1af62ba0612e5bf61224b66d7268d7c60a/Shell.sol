...
contract Shell is IExchange, TokenFetcher {
  using SafeMath for uint256;
  using Address for address;
  ...
  returns (uint256)
  {
    ...
    uint256 receivedAmount = IShell(exchange).originSwap(
      address(fromToken),
      address(toToken),
      fromAmount,
      toAmount,
  ...
  returns (uint256)
  {
    ...
    uint256 originAmountUsed = IShell(exchange).targetSwap(
      address(fromToken),
      address(toToken),
      fromAmount,
      toAmount,
    ...
    uint256 remainingAmount = fromAmount.sub(originAmountUsed);
    Utils.transferTokens(address(toToken), msg.sender, toAmount);
