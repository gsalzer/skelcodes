...
contract LunaSwap is Ownable, BNum {
    ...
    function _swapLunaToWeth(uint256 _poolAmountIn) internal {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (tokens[i] == address(univ2)) {
                ...
                _removeLiquidityAndRecieveEth(address(twa), tokensOutLuna[i]);
            } else {
                if (tokens[i] != address(weth)) {
    ...
    function _removeLiquidityAndRecieveEth(address token, uint256 lpAmountIn) internal returns (uint256 ethAmountOut) {
        uniswapPairFor(token).approve(address(uniswapV2Router), lpAmountIn);
        ethAmountOut = uniswapV2Router.removeLiquidityETHSupportingFeeOnTransferTokens(
            token,
            lpAmountIn,
            0,
            0,
        ...
        _swapTokenForWethOut(token, TokenInterface(token).balanceOf(address(this)));
    }
