...
contract Bancor is IExchange, TokenFetcher {
    ...
    IBancorNetworkPathFinder public pathFinder;

    bytes32 public constant BANCOR_NETWORK = 0x42616e636f724e6574776f726b00000000000000000000000000000000000000;

    constructor(address _contractRegistry, address _pathFinder, address _etherToken) public {
        contractRegistry = IContractRegistry(_contractRegistry);
        pathFinder = IBancorNetworkPathFinder(_pathFinder);
    ...
    */
    ...
    function onChainSwap(
        ...
        returns (uint256)
    {
        IERC20[] memory path = pathFinder.generatePath(
            address(fromToken) == Utils.ethAddress() ? IERC20(etherToken) : fromToken,
            address(toToken) == Utils.ethAddress() ? IERC20(etherToken) : toToken
        );
        address bancorNetwork = contractRegistry.addressOf(
          BANCOR_NETWORK
        );
