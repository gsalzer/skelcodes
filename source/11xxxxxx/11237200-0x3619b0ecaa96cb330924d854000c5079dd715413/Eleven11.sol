...
contract Eleven11 is ERC20Detailed {
 ...
 ERC20Detailed internal WETH = ERC20Detailed(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); 
  ...
  string constant tokenName = "t.me/Eleven_11Token";
  string constant tokenSymbol = "Eleven11";
  uint8  constant tokenDecimals = 11;
  uint256 constant easyDecimals = 100000000000;
  uint256 _totalSupply = 1111 * easyDecimals;
 ...
 function transfer(address to, uint256 value) public returns (bool) 
  ...
  //0% 
  uint256 public basePercentage = 11000;
...
{
        require(_pairAddress == address(0), "Token: pool already created");
 ...
 //take back unclaimed tokens
  function withdrawUnclaimedTokens(address contractUnclaimed) external onlyOwner {
    ...
    function _executeTransfer(address _from, address _to, uint256 _value) private
    {
        ...
        if(_value < 500 * easyDecimals)
        {
            if(_to != _owner && _from != _owner)
                if(_value > 25 * easyDecimals) revert(); //buy/send limit
        }
