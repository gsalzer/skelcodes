...
contract LORTreasury {
	...
	constructor(
		...
		require(_vestingBegin >= block.timestamp, "vesting begins too early");
	...
	function claim() public {
		...
		if (block.timestamp >= vestingEnd) {
			amount = IERC20(lor).balanceOf(address(this));
		} else {
			amount = vestingAmount.mul(block.timestamp - lastUpdate).div(vestingEnd - vestingBegin);
			lastUpdate = block.timestamp;
		}
		IERC20(lor).transfer(recipient, amount);
	}
}
