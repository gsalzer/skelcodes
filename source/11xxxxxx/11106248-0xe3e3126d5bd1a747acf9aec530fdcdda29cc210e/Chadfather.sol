...
// Chadspawn with Governance.
contract Chadspawn is ERC20Burnable, Ownable {
	...
	uint256 public constant maxSupply = 6969 ether;
	...
	/// @notice Creates `_amount` token to `_to`. Must only be called by the owner (Chadfather).
	function mint(address _to, uint256 _amount) public onlyOwner {
		require(this.totalSupply().add(_amount).add(burnPool) <= maxSupply);
		_mint(_to, _amount);
	...
	constructor() public ERC20('t.me/SOCHAD69', 'CHADSPAWN') {}
	...
	function delegateBySig(
		...
		require(signatory != address(0), 'CHAD::delegateBySig: invalid signature');
		require(nonce == nonces[signatory]++, 'CHAD::delegateBySig: invalid nonce');
		require(now <= expiry, 'CHAD::delegateBySig: signature expired');
		...
		returns (uint256)
	{
		require(
			...
			'CHAD::getPriorVotes: not yet determined'
		);
	...
	function _delegate(address delegator, address delegatee) internal {
		...
		uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CHADs (not scaled);
	...
	function _writeCheckpoint(
		...
		uint32 blockNumber = safe32(
			block.number,
			'CHAD::_writeCheckpoint: block number exceeds 32 bits'
		);
...
contract Chadfather is Ownable {
	...
	constructor(Chadspawn _chad, address _devaddr) public {
		chad = _chad;
		devaddr = _devaddr;
	}
