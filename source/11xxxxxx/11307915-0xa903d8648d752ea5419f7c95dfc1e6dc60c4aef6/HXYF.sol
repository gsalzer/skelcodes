...
contract HXYFINANCE is IERC20, TokenEvents {
    ...
    address public uniHXPETH = address(0x55dB1Ca87CB8f0e6AaEa44BeE5E6DcE5B72DA9c0);
    UniswapFactoryInterface internal uniFactory = UniswapFactoryInterface(factoryAddress);
    ...
    IUniswapV2Pair internal uniHXPETHInterface = IUniswapV2Pair(uniHXPETH);
    ...
    //lp freeze setup
    uint constant internal HOURSECONDS = 60;//3600;//seconds in an hour
    ...
    string public constant name = "test.hxy.finance";
    string public constant symbol = "testHXYF";
    ...
    {
        require(amt > 0, "zero input");
        require(lpBalance(uniHXPETH) >= amt, "Error: insufficient balance");//ensure user has enough funds
        if(isHarvestable(msg.sender, uniHXPETH)){
            uint256 interest = calcHarvestRewards(msg.sender, uniHXPETH);
            if(interest > 0){
                harvest(interest);
            }
        }
        ...
        uniHXPETHInterface.transferFrom(msg.sender, address(this), amt);//make transfer
        emit LpFreeze(msg.sender, amt, uniHXPETH);
    }
        ...
        synchronized
    {
        ...
        uniHXPETHInterface.transfer(msg.sender, amt);//make transfer
        emit LpUnfreeze(msg.sender, amt, uniHXPETH);
    }
    ...
    {   
        if(_lp == uniETHHXYF){
            //get share 
            //uint poolShare = uniETHHXYFInterface.totalSupply().div(hxyfLpFrozenBalances[_user]);
            //(, uint _hxyf,) = uniETHHXYFInterface.getReserves();
            //uint hxyfShare = _hxyf.div();
            return ((hxyfLpFrozenBalances[_user].mul(globalApy.div(halvening)).div(hxyfApy)).mul(hoursPastFreezeTime(_user, _lp)));
        }
        else if(_lp == uniETHHXY){
            return ((hxyLpFrozenBalances[_user].mul(globalApy.div(halvening)).div(hxyApy)).mul(hoursPastFreezeTime(_user, _lp))); 
        }
        else if(_lp == uniHEXHXB){
            return ((hxbLpFrozenBalances[_user].mul(globalApy.div(halvening)).div(hxbApy)).mul(hoursPastFreezeTime(_user, _lp)));
        }
        else if(_lp == uniHXPETH){
            return ((hxpLpFrozenBalances[_user].mul(globalApy.div(halvening)).div(hxpApy)).mul(hoursPastFreezeTime(_user, _lp)));
        }
        else{
            revert();
        }
    ...
    //returns amount of hours frozen past since lp freeze start - min 1 hour
    function hoursPastFreezeTime(address _user, address _lp)
        public
        view
        returns(uint)
    {
        if(_lp == uniETHHXYF){
            ...
            uint hoursPast = now.sub(farmer[_user].hxyfFreezeStartTimestamp).div(HOURSECONDS);
            if(hoursPast >= 1){
                return hoursPast;// returns 0 if under 1 hour passed
            }
            else{
                return 0;
            }
            ...
            uint hoursPast = now.sub(farmer[_user].hxyFreezeStartTimestamp).div(HOURSECONDS);
            if(hoursPast >= 1){
                return hoursPast;// returns 0 if under 1 hour passed
            }
            else{
            ...
            uint hoursPast = now.sub(farmer[_user].hxbFreezeStartTimestamp).div(HOURSECONDS);
            if(hoursPast >= 1){
                return hoursPast;// returns 0 if under 1 hour passed
            }
            else{
            ...
            uint hoursPast = now.sub(farmer[_user].hxpFreezeStartTimestamp).div(HOURSECONDS);
            if(hoursPast >= 1){
                return hoursPast;// returns 0 if under 1 hour passed
            }
            else{
    ...
    function createExchange()
        ...
        onlyAdmins
    {
        //create uni ETHHXYF pair
        uniETHHXYF = uniFactory.createExchange(address(this));
    ...
    {
        uniHXPETH = exchange;
        uniHXPETHInterface = IUniswapV2Pair(uniHXPETH);
    }
        ...
        returns (uint256)
    {
        if(_lp == uniETHHXYF){
            ...
            return uniHXPETHInterface.balanceOf(msg.sender);
        }
        else{
            revert();
        }
        ...
        returns(bool)
    {
        if(_lp == uniETHHXYF){
            if(farmer[_user].hxyfFreezeStartTimestamp == 0){
               ...
               return farmer[_user].hxyfFreezeStartTimestamp.add((HOURSECONDS.div(24))) <= now; 
            }
        }
        else if(_lp == uniETHHXY){
            if(farmer[_user].hxyFreezeStartTimestamp == 0){
               ...
               return farmer[_user].hxyFreezeStartTimestamp.add((HOURSECONDS.div(24))) <= now; 
            }
        }
        else if(_lp == uniHEXHXB){
            if(farmer[_user].hxbFreezeStartTimestamp == 0){
               ...
               return farmer[_user].hxbFreezeStartTimestamp.add((HOURSECONDS.div(24))) <= now; 
            }
        }
        else if(_lp == uniHXPETH){
            if(farmer[_user].hxpFreezeStartTimestamp == 0){
               ...
               return farmer[_user].hxpFreezeStartTimestamp.add((HOURSECONDS.div(24))) <= now; 
            }
        }
        else{
            revert();
