...
contract POLKINDEX is ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    uint256 public _buyFeePct;
    uint256 public _sellFeePct;
    ...
    uint256 public _launchLimitBlocks;
    uint256 public _launchLimitAmount;
    
    
    function initialize(uint256 initialSupply, address dev, address router, address tokenQuote, address tokenUSD)
        public
        initializer
    {
        __ERC20_init("polkindex.finance", "PIDX");
		...
		_buyFeePct = 100; //1.00%
        _sellFeePct = 100; //1.00%
        
        _launchLimitBlocks = 20;
        _launchLimitAmount = 50000 * 10 ** DECIMALS;
    ...
    function setTransferFees(uint256 buyFeePct, uint256 sellFeePct) external onlyOwner() {
        _buyFeePct = buyFeePct;
        _sellFeePct = sellFeePct;
    }
    
    function setLaunchLimits(uint256 launchLimitBlocks, uint256 launchLimitAmount) external onlyOwner() {
        _launchLimitBlocks = launchLimitBlocks;
        _launchLimitAmount = launchLimitAmount;
    }
    ...
    function reflect(uint256 tAmount) public {
        ...
        (uint256 rAmount,,,,) = _getValues(tAmount, _sellFeePct);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,,) = _getValues(tAmount, _sellFeePct);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(_isDistributor[sender] || _isDistributor[recipient]) {
            ...
            if(_isDistributor[sender] && _isExchanger[recipient]) {
            ...
            if (_isExchanger[sender] && !_isExchanger[recipient]) {
                ...
                //buying
                if((block.number - _launchBlock) <= _launchLimitBlocks)
                    require(amount <= _launchLimitAmount, "Buys on initial blocks limited");
                
                _transferFromExcluded(sender, recipient, amount, _buyFeePct);
                
            } else if (!_isExchanger[sender] && _isExchanger[recipient]) {
                //selling
                _transferToExcluded(sender, recipient, amount, _sellFeePct);
            } else if (!_isExchanger[sender] && !_isExchanger[recipient]) {
                //wallet to wallet
                _transferStandard(sender, recipient, amount, _sellFeePct);
            } else if (_isExchanger[sender] && _isExchanger[recipient]) {
