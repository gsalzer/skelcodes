...
contract NFITokenWrapper {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // !! Mainnet NFI-ETH LP TOKEN address
    IERC20 public stakeToken = IERC20(0xf78f3FE99b6A391081AB5ae960866AeC2EF8DCDa);
...
contract LpETHNFIPool is NFITokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){
        require(amount > 0, "LP-ETH-NFI-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "LP-ETH-NFI-POOL: Cannot withdraw 0");
    ...
    function getReward() public checkStart updateReward(msg.sender){
        ...
        if (reward > 0) {
            ...
            nfi.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    modifier checkStart(){
        require(block.timestamp > startTime,"LP-ETH-NFI-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "LP-ETH-NFI-POOL: Pool is closed");
        _;
    }
