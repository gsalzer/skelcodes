...
// TRIFI is a community fork of TRINITY
...
contract Balancer {
    ...
    TRIFI private _tokenContract;
    
    constructor(TRIFI tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract TRIFI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 12000000000000;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _tBurnTotal;

    string private _name = 'TRIFI';
    string private _symbol = 'TRIFI';
    uint8 private _decimals = 9;
    
    uint256 public _feeDecimals = 1;
    uint256 public _taxFee = 0;
    uint256 public _lockFee = 0;
    uint256 public _maxTxAmount = 8000000000000;
    uint256 public _minTokensBeforeSwap = 8000000000;
    uint256 public _minInterestForReward = 10000000;
    uint256 private _autoSwapCallerFee = 240000000;
    ...
    uint256 private _minTokenForAlchemy = 1200000000;
    uint256 private _lastAlchemy;
    uint256 private _alchemyInterval = 20 minutes;
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "TRIFI: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "TRIFI: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'TRIFI: We can not exclude Uniswap router.');
        require(account != address(this), 'TRIFI: We can not exclude contract self.');
        require(account != _rewardWallet, 'TRIFI: We can not exclude reweard wallet.');
        require(!_isExcluded[account], "TRIFI: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "TRIFI: Account is already included");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "TRIFI: approve from the zero address");
        require(spender != address(0), "TRIFI: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "TRIFI: transfer from the zero address");
        require(recipient != address(0), "TRIFI: transfer to the zero address");
        require(amount > 0, "TRIFI: Transfer amount must be greater than zero");
        
        if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            require(amount <= _maxTxAmount, "TRIFI: Transfer amount exceeds the maxTxAmount.");
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_uniswapV2Router)) && !tradingEnabled)
                require(false, "TRIFI: trading is disabled.");
        }
    ...
    function alchemy() public lockTheSwap {
        require(balanceOf(_msgSender()) >= _minTokenForAlchemy, "TRIFI: You have not enough TRIFI to ");
        require(now > _lastAlchemy + _alchemyInterval, 'TRIFI: Too Soon.');
    ...
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        require(feeDecimals >= 0 && feeDecimals <= 2, 'TRIFI: fee decimals should be in 0 - 2');
        _feeDecimals = feeDecimals;
        emit FeeDecimalsUpdated(feeDecimals);
    }
    
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'TRIFI: taxFee should be in 0 - 5');
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'TRIFI: lockFee should be in 0 - 5');
        _lockFee = lockFee;
        emit LockFeeUpdated(lockFee);
    }
    
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 500000e9 , 'TRIFI: maxTxAmount should be greater than 500000e9');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
    
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'TRIFI: minTokenBeforeSwap should be in 50e9 - 25000e9');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'TRIFI: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 1e9, 'TRIFI: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setLiquidityRemoveFee(uint256 liquidityRemoveFee) external onlyOwner() {
        require(liquidityRemoveFee >= 1 && liquidityRemoveFee <= 10 , 'TRIFI: liquidityRemoveFee should be in 1 - 10');
    ...
    function _setAlchemyCallerFee(uint256 alchemyCallerFee) external onlyOwner() {
        require(alchemyCallerFee >= 1 && alchemyCallerFee <= 15 , 'TRIFI: alchemyCallerFee should be in 1 - 15');
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        require(poolAddress != address(0), "TRIFI: Pool address is zero.");
        require(pairTokenAddress != address(0), "TRIFI: Pair token address is zero.");
        require(pairTokenAddress != address(this), "TRIFI: Pair token address self address.");
        require(pairTokenAddress != currentPairTokenAddress, "TRIFI: Pair token address is same as current one.");
