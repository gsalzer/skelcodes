...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        address _PORNAddress,
        address _WETHAddress,
        address _pornVaultAddress
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        pornTokenAddress = _PORNAddress;
        WETHAddress = _WETHAddress;
        tokenUniswapPair = IUniswapV2Factory(address(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f)).getPair(WETHAddress,pornTokenAddress);
        pornVaultAddress = _pornVaultAddress;
        feePercentX100 = 20;
    ...
    address pornTokenAddress;
    address pornVaultAddress;
    ...
    function setPornVaultAddress(address _pornVaultAddress) public onlyOwner {
        pornVaultAddress = _pornVaultAddress;
    }
        ...
        )
    {
        ...
        // No fee when pornVault is sending, or there will be an infinite loop
        // And also when the pair is sending (buys are happening, so do not tax on it)
        if(sender == pornVaultAddress || sender == tokenUniswapPair ) {
