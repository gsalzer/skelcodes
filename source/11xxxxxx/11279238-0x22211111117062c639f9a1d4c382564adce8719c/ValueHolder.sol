...
contract XChanger {
    ...
    uint private constant slippageFee = 150;
    ...
    function swapUni(
        ...
        //slippage protection 1.5% if enabled
        uint minAmount = 0;
        if (slipProtect) {
            uint256 feeSlippage = returnAmount.mul(slippageFee).div(10000);    
            minAmount = amount.sub(feeSlippage);
        } 
...
contract ValueHolder is Ownable, XChanger, CHIBurner {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
