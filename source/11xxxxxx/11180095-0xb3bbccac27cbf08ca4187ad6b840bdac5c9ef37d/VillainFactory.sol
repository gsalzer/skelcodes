...
/// @title VillainFactory Contract

contract VillainFactory is ERC721, Ownable {
    ...
    */
    constructor() ERC721("NodeRunnersVillain", "NDRV") public {}
    ...
    function createFighter(
        ...
        require(_strength >= 0 && _strength <=80, "strength is 80 max");

        fighters.push(Fighter(_strength, _defence, _health, URI));
        fighterToOwner[_tokenIds.current()] = _owner;
        ownerFighterCount[_owner]++;

        uint256 newItemId = _tokenIds.current();
        emit NewFighter(_strength, _defence, _health, URI);
