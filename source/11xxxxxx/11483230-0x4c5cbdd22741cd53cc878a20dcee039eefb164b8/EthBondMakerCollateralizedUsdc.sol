...
contract BondTokenCollateralizedErc20 is BondToken {
    ...
    function _sendCollateralTo(address receiver, uint256 amount) internal override {
        COLLATERALIZED_TOKEN.safeTransfer(receiver, amount); // MODIFIED: BTE-01
    }
}
        ...
        )
    {
        ...
        if (_registeredFnMap[fnMapID].length == 0) {
            ...
            for (uint256 i = 0; i < polyline.length; i++) {
                _registeredFnMap[fnMapID].push(unzipLineSegment(polyline[i])); // NOCHANGE: BMR-01
            }
...
contract BondMakerCollateralizedErc20 is BondMaker {
    ...
    function _sendCollateralTo(address receiver, uint256 amount) internal override {
        COLLATERALIZED_TOKEN.safeTransfer(receiver, amount); // MODIFIED: BME-01
    }
    ...
    function _receiveCollateralFrom(address sender, uint256 amount) internal {
        ...
        COLLATERALIZED_TOKEN.safeTransferFrom(sender, address(this), amount); // MODIFIED: BME-01
