...
contract SpaceMineToken is SpaceMineCore {
    ...
    */
    ...
    modifier taxUnlock() {
        require(block.timestamp >= unlockTime, "Can not set tax now, time is locked.");
        _;
    }
    ...
    function setTaxRateRequest(uint256 rate) public onlyTaxer {
    ...
    function releaseTaxeRate() public onlyTaxer taxUnlock {
        _taxRate = rateWillBeSet;
    }
...
contract MineLiquidityMining is Ownable {
    ...
    uint256 public devFundAmount = 480 * 1e18;
    ...
    function stake(uint amount) external shouldStarted {
        ...
        if (rewards[msg.sender] > 0) {
            mine.safeTransfer(msg.sender, rewards[msg.sender]);
        }
    ...
    function claim(uint amount) external shouldStarted {
        updateRewards(msg.sender);
        checkHalving();
    ...
    function transferDevAddr(address newAddr) external onlyDev {
    ...
    function lpTokenAddress() view external returns(address) {
        return address(lpToken);
    }

    function mineAddress() view external returns(address) {
        return address(mine);
    }
    ...
    function setInitRewards(uint256 reward) external onlyOwner {
        initReward = reward;
    }
}
