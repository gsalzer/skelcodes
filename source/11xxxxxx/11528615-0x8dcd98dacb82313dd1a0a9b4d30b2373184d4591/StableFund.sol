...
// File: localhost/contracts/utils/SafeMath.sol
...
library SafeMath {
  ...
  */
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    require(b > 0, 'zero div'); // Solidity only automatically asserts when dividing by 0
    uint256 c = a / b;
...
// File: localhost/contracts/StableFund.sol
...
contract StableFund is Ownable {
    ...
    uint256 private constant MAX_UINT256 = ~uint256(0) / 10**6;
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 5000 *
        10**6 *
        10**DECIMALS;
     ...
     */
    function mint(address to, uint256 value) public onlyValueHolder {
     ...
     */
    function burn(address from, uint256 value) public onlyValueHolder {
        uint256 gonValue = value.mul(_gonsPerFragment);

        if (gonValue > _gonBalances[from]) {
            gonValue = _gonBalances[from];
