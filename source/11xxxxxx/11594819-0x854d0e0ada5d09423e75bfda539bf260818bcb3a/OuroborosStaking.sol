...
contract OuroborosStaking is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	function addPool(address _token, address _tokenReward, uint256 _rewardBlock, uint256 _rewardRemains) external onlyOwner {
	...
	function stake(uint256 _pid, uint256 _amount) external nonReentrant {    
		...
		pool.totalStakingAmount += _amount;				
	...
	function _updatePool(uint256 _pid) internal {
		...
		if(pool.totalStakingAmount > 0){
			...
			uint256 TotalShare = percent(pool.totalStakingAmount, _decimalConverter, 4);
	...
	function claim(uint256 _pid) public nonReentrant {
		...
		user.pendingReward = pendingRewardsFromPool(_pid,msg.sender);
		require(user.pendingReward > 0, "not have claimable reward");
		
		if(user.pendingReward > 0){
			pool.tokenReward.safeTransfer(msg.sender, user.pendingReward );
			emit ClaimReward(msg.sender, address(pool.tokenReward) ,user.pendingReward );
			
			pool.rewardRemains -= user.pendingReward;
			pool.totalPendingReward -= user.pendingReward;
			user.claimReward += user.pendingReward;
	...
	function withdraw(uint256 _pid) public nonReentrant {
		...
		pool.token.safeTransfer(msg.sender, user.stakingAmount);
		
		emit Withdraw(msg.sender, address(pool.token) ,user.stakingAmount);
		
		
		pool.totalStakingAmount -= user.stakingAmount;
