...
contract StrategyDForceUSDT {
    ...
    constructor() public {
        ...
        swap2YFIIRouting = [output,weth,yfii];    //DF平台代币 换 CM代币 (10%)
        swap2TokenRouting = [output,weth,want];   //DF平台代币 换 USDT=利息 (90%)
        doApprove();
    }
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            ...
            dERC20(d).mint(address(this), _want);   //https://markets.dforce.network/dapp  收条代币
        }
    ...
    function doswap() internal {
        ...
        UniswapRouter(unirouter).swapExactTokensForTokens(_2token, 0, swap2TokenRouting, address(this), now.add(1800)); //USDT
        UniswapRouter(unirouter).swapExactTokensForTokens(_2yfii, 0, swap2YFIIRouting, address(this), now.add(1800));   //CMT
    }
