...
// File: contracts/gladUniswapMiningStorage.sol
...
contract gladUniswapMiningStorage {
  using SafeMath for uint256;

  bool constant public isgladUniswapMining = true;
  ...
  ERC20 public gladToken;
  
  uint public startMiningBlockNum = 11858000;
  uint public totalMiningBlockNum = 1192000;
  uint public endMiningBlockNum = startMiningBlockNum + totalMiningBlockNum;
  uint public gladPerBlock = 3857638800000000;
...
// File: contracts/gladUniswapMining.sol
...
contract gladUniswapMining is Ownable, Pausable, gladUniswapMiningStorage {
  
  event Staked(address indexed user, uint256 amount, uint256 total, uint256 lockedUntil);
  event Unstaked(address indexed user, uint256 amount, uint256 total);
  event Claimedglad(address indexed user, uint amount, uint total);

  function initiate(uint _startMiningBlocknum, uint _totalMiningBlockNum, uint _gladPerBlock, ERC20 _glad, ERC20 _lp) public onlyOwner{
    ...
    _glad.totalSupply(); //sanity check
  ...
  // @param _locked the locking period; option: 0, 30 days (2592000), 90 days (7776000)
    ...
    internal
  {
    ...
    require(_locked == 0 || _locked == 30 days || _locked == 90 days   , "invalid locked period" );
  ...
  function getMiningState(uint _blockNum) public view returns(uint, uint){
    ...
    if (deltaBlocks > 0 && totalStaked > 0) {
        uint gladAccrued = deltaBlocks.mul(gladPerBlock);
        uint ratio = gladAccrued.mul(1e18).div(totalStakedPower); //multiple ratio to 1e18 to prevent rounding error
  ...
  // @notice claim GLAD based on current state 
  function claimglad() public whenNotPaused {
      
      updateMiningState();
      
      uint claimableglad = claimableglad(msg.sender);
      
      stakerIndexes[msg.sender] = miningStateIndex;
      
      if(claimableglad > 0){
          
          stakerClaimed[msg.sender] = stakerClaimed[msg.sender].add(claimableglad);
          totalClaimed = totalClaimed.add(claimableglad);
          gladToken.transfer(msg.sender, claimableglad);
          emit Claimedglad(msg.sender, claimableglad, stakerClaimed[msg.sender]);
          
      }
  }
  
  // @notice calculate claimable glad based on current state
  function claimableglad(address _address) public view returns(uint){
      ...
      uint gladDelta = deltaIndex.mul(stakerPower[_address]).div(1e18);
    ...
    // @notice admin function to send GLAD to external address, for emergency use
    function sendglad(address _to, uint _amount) public onlyOwner{
      gladToken.transfer(_to, _amount);
    }
