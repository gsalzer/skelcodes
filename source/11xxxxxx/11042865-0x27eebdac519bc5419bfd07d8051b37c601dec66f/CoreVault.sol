...
contract DeCoreVault is OwnableUpgradeSafe {
    ...
    uint16 TRESURY_FEE;
    uint16 BURN_FEE;
    function setFee(uint16 _TRESURY_FEE, uint16 _BURN_FEE) public onlyOwner {
        TRESURY_FEE = _TRESURY_FEE;
        BURN_FEE = _BURN_FEE;
    }
    uint256 pending_TRESURY_rewards;
    uint256 pending_BURN_rewards;
    ...
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = decore.balanceOf(address(this)).sub(coreBalance);
        
        if(newRewards > 0) {
            coreBalance = decore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 coreRewardWhole) {
        ...
        uint256 coreRewardFee = coreRewardWhole.mul(TRESURY_FEE).div(10000);
        uint256 coreBurnFee = coreRewardWhole.mul(BURN_FEE).div(10000);
        
        uint256 coreRewardToDistribute = coreRewardWhole.sub(coreBurnFee).sub(coreRewardFee);

        pending_TRESURY_rewards = pending_TRESURY_rewards.add(coreRewardFee);
        pending_BURN_rewards = pending_BURN_rewards.add(coreBurnFee);
    ...
    function safeCoreTransfer(address _to, uint256 _amount) internal {
        uint256 coreBal = decore.balanceOf(address(this));
        if (_amount > coreBal) {
            decore.transfer(_to, coreBal);
            coreBalance = decore.balanceOf(address(this));
        } else {
    ...
    function transferDevFee() public {
        if(pending_TRESURY_rewards == 0) return;
        if(pending_BURN_rewards == 0) return;
        uint256 coreBal = decore.balanceOf(address(this));
        if (pending_BURN_rewards > coreBal) {
            decore.transfer(burnaddr, coreBal);
            coreBalance = decore.balanceOf(address(this));
        } else {
            decore.transfer(burnaddr, pending_BURN_rewards);
            coreBalance = decore.balanceOf(address(this));
        }
        coreBal = decore.balanceOf(address(this));
        if (pending_TRESURY_rewards > coreBal) {
            decore.transfer(tresuryaddr, coreBal);
            coreBalance = decore.balanceOf(address(this));
        } else {
            decore.transfer(tresuryaddr, pending_TRESURY_rewards);
            coreBalance = decore.balanceOf(address(this));
        }
        pending_BURN_rewards = 0;
    ...
    function setFeeReciever(address _devaddr, address _burnaddr) public onlyOwner {
        tresuryaddr = _devaddr;
        burnaddr = _burnaddr;
    }
