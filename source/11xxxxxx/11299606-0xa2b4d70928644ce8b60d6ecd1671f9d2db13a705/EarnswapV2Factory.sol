// Dependency file: contracts/interfaces/IEarnswapV2Factory.sol

// pragma solidity =0.6.12;

interface IEarnswapV2Factory {
...
// Dependency file: contracts/EarnswapV2ERC20.sol

// pragma solidity =0.6.12;

// import 'contracts/libraries/SafeMath.sol';

contract EarnswapV2ERC20 {
    using SafeMathEarnswap for uint;

    string public constant name = 'Earnswap LP Token';
    string public constant symbol = 'Earnswap_LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'EarnswapV2: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'EarnswapV2: INVALID_SIGNATURE');
...
// Dependency file: contracts/interfaces/IEarnswapV2Callee.sol

// pragma solidity =0.6.12;

interface IEarnswapV2Callee {
    function EarnswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external;
}


// Dependency file: contracts/EarnswapV2Pair.sol

// pragma solidity =0.6.12;

// import 'contracts/EarnswapV2ERC20.sol';
// import 'contracts/libraries/Math.sol';
// import 'contracts/libraries/UQ112x112.sol';
// import 'contracts/interfaces/IERC20.sol';
// import 'contracts/interfaces/IyToken.sol';
// import 'contracts/interfaces/IEarnswapV2Factory.sol';
// import 'contracts/interfaces/IEarnswapV2Callee.sol';

contract EarnswapV2Pair is EarnswapV2ERC20 {
    using SafeMathEarnswap for uint;
    ...
    modifier lock() {
        require(unlocked == 1, 'EarnswapV2: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function owner() public view returns (address) {
        return IEarnswapV2Factory(factory).feeTo();
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        if (b < value) {
            if (token == token0) {
                ...
                require(success && (data.length == 0 || abi.decode(data, (bool))), 'EarnswapV2: TRANSFER_FAILED');
            } else {
                _withdrawAll1();
                (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR, to, value));
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'EarnswapV2: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'EarnswapV2: OVERFLOW');
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'EarnswapV2: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'EarnswapV2: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'EarnswapV2: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'EarnswapV2: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IEarnswapV2Callee(to).EarnswapV2Call(msg.sender, amount0Out, amount1Out, data);
        balance0 = b0();
        balance1 = b1();
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'EarnswapV2: INSUFFICIENT_INPUT_AMOUNT');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'EarnswapV2: K');
        }
...
// Root file: contracts/EarnswapV2Factory.sol

pragma solidity =0.6.12;

// import 'contracts/interfaces/IEarnswapV2Factory.sol';
// import 'contracts/EarnswapV2Pair.sol';

contract EarnswapV2Factory is IEarnswapV2Factory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(EarnswapV2Pair).creationCode);
    }

    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'EarnswapV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'EarnswapV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'EarnswapV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(EarnswapV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        EarnswapV2Pair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, 'EarnswapV2: FORBIDDEN');
        feeTo = _feeTo;
    }
