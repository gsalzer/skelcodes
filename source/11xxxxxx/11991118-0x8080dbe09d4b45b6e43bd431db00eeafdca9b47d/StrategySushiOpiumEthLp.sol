// hevm: flattened sources of src/strategies/opium/strategy-sushi-opium-eth-lp.sol
 ...
 */
contract ERC20 is Context, IERC20_2 {
...
abstract contract StrategyBase {
    using SafeERC20_2 for IERC20_2;
    using Address_2 for address;
    using SafeMath_2 for uint256;
    ...
    function balanceOfWant() public view returns (uint256) {
        return IERC20_2(want).balanceOf(address(this));
    }
    ...
    function withdraw(IERC20_2 _asset) external returns (uint256 balance) {
    ...
    function withdraw(uint256 _amount) external {
        require(msg.sender == controller, "!controller");
        uint256 _balance = IERC20_2(want).balanceOf(address(this));
        if (_balance < _amount) {
        ...
        IERC20_2(want).safeTransfer(IController(controller).devfund(), _feeDev);

        uint256 _feeTreasury = _amount.mul(withdrawalTreasuryFee).div(
            withdrawalTreasuryMax
        );
        IERC20_2(want).safeTransfer(
        ...
        IERC20_2(want).safeTransfer(_jar, _amount.sub(_feeDev).sub(_feeTreasury));
    }
        ...
        returns (uint256 balance)
    {
        ...
        balance = IERC20_2(want).balanceOf(address(this));

        address _jar = IController(controller).jars(address(want));
        require(_jar != address(0), "!jar");
        IERC20_2(want).safeTransfer(_jar, balance);
    }
    ...
    function _distributePerformanceFeesAndDeposit() internal {
        uint256 _want = IERC20_2(want).balanceOf(address(this));
...
abstract contract StrategyStakingRewardsBase is StrategyBase {
    ...
    function deposit() public override {
        uint256 _want = IERC20_2(want).balanceOf(address(this));
        if (_want > 0) {
            IERC20_2(want).safeApprove(rewards, 0);
            IERC20_2(want).safeApprove(rewards, _want);
...
////// src/strategies/strategy-opium-farm-base.sol
// SPDX-License-Identifier: MIT
/* pragma solidity ^0.6.7; */
...
abstract contract StrategyOpiumFarmBase is StrategyStakingRewardsBase {
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _opium = IERC20_2(opium).balanceOf(address(this));
        if (_opium > 0) {
            // 10% is locked up for future gov
            uint256 _keepOpium = _opium.mul(keepOpium).div(keepOpiumMax);
            IERC20_2(opium).safeTransfer(
                IController(controller).treasury(),
                _keepOpium
            );
            uint256 _amount = _opium.sub(_keepOpium);

            IERC20_2(token1).safeApprove(sushiRouter, 0);
            IERC20_2(token1).safeApprove(sushiRouter, _amount);

            _swapSushiswap(opium, weth, _amount);
        }

        // Swap half ETH for Opium
        uint256 _weth = IERC20_2(weth).balanceOf(address(this));
        if (_weth > 0) {
            IERC20_2(weth).safeApprove(sushiRouter, 0);
            IERC20_2(weth).safeApprove(sushiRouter, _weth);
            _swapSushiswap(weth, token1, _weth.div(2));
        }

        // Adds in liquidity for WETH/Token
        _weth = IERC20_2(weth).balanceOf(address(this));
        uint256 _token1 = IERC20_2(token1).balanceOf(address(this));
        if (_weth > 0 && _token1 > 0) {
            IERC20_2(weth).safeApprove(sushiRouter, 0);
            IERC20_2(weth).safeApprove(sushiRouter, _weth);

            IERC20_2(token1).safeApprove(sushiRouter, 0);
            IERC20_2(token1).safeApprove(sushiRouter, _token1);
            ...
            // Donates weth
            IERC20_2(weth).safeTransfer(
                IController(controller).treasury(),
                IERC20_2(weth).balanceOf(address(this))
            );
            IERC20_2(token1).safeTransfer(
                IController(controller).treasury(),
                IERC20_2(token1).balanceOf(address(this))
            );
        }
...
////// src/strategies/opium/strategy-sushi-opium-eth-lp.sol
// SPDX-License-Identifier: MIT
/* pragma solidity ^0.6.7; */
