...
contract EXCORElock is Ownable {
    using SafeMath for uint;
    
    address public constant tokenAddress = 0x87D3646B101977de0D2D58dfC5A70e84767A1909;
    
    uint public constant tokensLocked = 240e18;         // 240 Token
    uint public constant unlockRate =   240;          
    ...
    function claim() public onlyOwner {
        ...
        uint contractBalance = EXCORE(tokenAddress).balanceOf(address(this));
        uint amountToSend = pendingUnlocked;
        if (contractBalance < pendingUnlocked) {
            amountToSend = contractBalance;
        }
        require(EXCORE(tokenAddress).transfer(owner, amountToSend), "Could not transfer Tokens.");
        lastClaimedTime = now;
    }
    ...
    function transferAnyERC20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        ...
        EXCORE(_tokenAddr).transfer(_to, _amount);
    }
