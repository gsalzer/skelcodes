...
contract RESONATE is Context, IERC20, Ownable {
    ...
    uint256 _max_transaction_amount = 150000;
    ...
    function setMaxTransactionAmount(uint256 amount) external onlyOwner() {
        require(amount >= 0 && amount <= (_tTotal/_decimals_exponent), "amount is out of range");
        _max_transaction_amount = amount;
    }    
    
    function getMaxTransactionAmount() public view returns (uint256) {
        return _max_transaction_amount;
    }    
    ...
    function _transfer(
        ...
        {
            if(!_approveAllowed || amount < _max_transaction_amount.mul(_decimals_exponent))
