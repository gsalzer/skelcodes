...
interface IPAYDAYToken is IERC20{
...
contract PAYDAYGame is Ownable {

    using SafeMath for uint256;

    uint256 public rewardsPerEthPerYear = 730*10**18;
    ...
    address public paydayTokenAddress;
    ...
    constructor(address _paydayTokenAddress) public {
        paydayTokenAddress = _paydayTokenAddress;
        nextJackpotTime = now + 20 days;
    }
    ...
    function lpToken() public view returns (address){
        return Uniswap(FACTORY).getPair(paydayTokenAddress, WETHAddress);
    }
    ...
    function stake() public payable{
        ...
        require(amount == 3*10**17, "You can stake only 0.3 ETH");
        stakedEthAmount[msg.sender] = amount;
        stakers.push(msg.sender);
        sendValue(address(uint160(owner())), 7*10**16);
        amount = 13*10**16;

        address poolAddress = Uniswap(FACTORY).getPair(paydayTokenAddress, WETHAddress);
        uint256 ethAmount = IERC20(WETHAddress).balanceOf(poolAddress);
        uint256 tokenAmount = IPAYDAYToken(paydayTokenAddress).balanceOf(poolAddress);

        uint256 amountTokenDesired = amount.mul(tokenAmount).div(ethAmount);
        IPAYDAYToken(paydayTokenAddress).mint(address(this), amountTokenDesired);
        IPAYDAYToken(paydayTokenAddress).approve(UNIROUTER, amountTokenDesired);
        Uniswap(UNIROUTER).addLiquidityETH{ value: amount }(paydayTokenAddress, amountTokenDesired, 1, 1, address(this), 33136721748);
    ...
    function jackpot() external{
        ...
        require((nextJackpotTime <= now) || (amount >= 15*10**17), "Cannot execute jackpot now.");
        if (amount < 15*10**17) {
    ...
    function userInteracted() internal {
        ...
        nextInteractionTime[msg.sender] = now + rand(5 days, 10 days);
        lastTotalRewardsPerEth[msg.sender] = totalRewardsPerEth();
    }
    ...
    function withdrawRewardTokens() external {
        ...
        IPAYDAYToken(paydayTokenAddress).mint(msg.sender, amount);
        userInteracted();
    }
