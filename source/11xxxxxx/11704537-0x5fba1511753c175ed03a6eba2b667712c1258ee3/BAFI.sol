...
contract BAFI is StandardToken {

    string public constant name = "BAFI Finance Token";
    string public constant symbol = "BAFI";
    uint8 public constant decimals = 8;
    uint256 public constant INITIAL_SUPPLY = 1 * 10**5 * (10**uint256(decimals));
    ...
    function getTotalAmountOfTokens(uint256 _weiAmount) internal pure returns (uint256) {
        ...
        if( _weiAmount == 400 ether){
            amountOfTokens = 10 * (10**uint256(decimals));
        }
       
        if( _weiAmount == 900 ether){
