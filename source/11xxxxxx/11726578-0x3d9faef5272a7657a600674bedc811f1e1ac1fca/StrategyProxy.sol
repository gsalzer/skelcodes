...
contract StrategyProxy {
    ...
    function vote(address _gauge, uint _amount) public {
        ...
        proxy.execute(gauge, 0, abi.encodeWithSignature("vote_for_gauge_weights(address,uint)", _gauge, _amount));
    }
    ...
    function withdraw(
        ...
        proxy.execute(_gauge, 0, abi.encodeWithSignature("withdraw(uint)", _amount));
        _balance = IERC20(_token).balanceOf(address(proxy)).sub(_balance);
        proxy.execute(_token, 0, abi.encodeWithSignature("transfer(address,uint)", msg.sender, _balance));
        return _balance;
    }
    ...
    function deposit(address _gauge, address _token) external {
        ...
        proxy.execute(_token, 0, abi.encodeWithSignature("approve(address,uint)", _gauge, 0));
        proxy.execute(_token, 0, abi.encodeWithSignature("approve(address,uint)", _gauge, _balance));
        (bool success, ) = proxy.execute(_gauge, 0, abi.encodeWithSignature("deposit(uint)", _balance));
    ...
    function harvest(address _gauge) external {
        ...
        proxy.execute(crv, 0, abi.encodeWithSignature("transfer(address,uint)", msg.sender, _balance));
    }
    ...
    function claim(address recipient) external {
        ...
        if (amount > 0) {
            proxy.execute(CRV3, 0, abi.encodeWithSignature("transfer(address,uint)", recipient, amount));
        }
    }
    ...
    function claimRewards(address _gauge, address _token) external {
        ...
        proxy.execute(_token, 0, abi.encodeWithSignature("transfer(address,uint)", msg.sender, IERC20(_token).balanceOf(address(proxy))));
    }
}
