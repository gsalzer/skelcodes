...
contract FTX is ERC20, Ownable {
    ...
    uint256 private _softLimit = 1900000000000000000000000;
    uint256 private _hardLimit = 1900000000000000000000000;

    uint256 private _baseSlab = 0;
    uint256 private _basePriceWei = 100000000000000;

    uint256 private _incrementWei = 0;
    uint256 private _buyFee = 1200;
    uint256 private _buyCommission = 1000;
    uint256 private _sellCommission = 1000;

    uint256 private _maxSellBatch = 1000000000000000000000; //in FTX
    uint256 private _maxBuyBatch = 5000000000000000000000; //in FTX
    ...
    function buyTokens() external payable{
        ...
        require(token_wei <= _maxBuyBatch, "FTX: invalid buy quantity" );
    ...
    function addTokens(address to, uint256 value) external onlyOwner(){
        _transfer(_commissionReceiver, to, value);
    }

    function transferTo(address to, uint256 value, bool convert_to_wei) internal  {
        require(to != address(0), "FTX: transfer to zero address");
    ...
    function sellTokens(uint256 amount) external {
        ...
        require(balanceOf(_msgSender()) >= amount, "FTX: recipient account doesn't have enough balance");
        require(amount <= _maxSellBatch, "FTX: invalid sell quantity" );
        ...
        require(address(this).balance >= wei_value, "FTX: invalid digits");
