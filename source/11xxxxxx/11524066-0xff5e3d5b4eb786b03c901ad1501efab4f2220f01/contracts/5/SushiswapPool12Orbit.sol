...
// SushiswapPool12Orbit is specific for SUSHI-ETH pool in Sushiswap.
// In this case, fToken = SUSHI and pid = 12.
contract SushiswapPool12Orbit is Ownable, ReentrancyGuard, Orbit {
    /// @notice Libraries
    ...
    function refuel() public onlyEOA nonReentrant {
        ...
        addStrat.operate(address(this), 0, abi.encode(sushi, 0, 0));
    ...
    function destroy(uint256 id, address user) external onlyOperator nonReentrant {
        ...
        liqStrat.operate(address(0), 0, abi.encode(sushi, 0));
