...
contract StrategyCurvemUSDVoterProxy {
    ...
    function harvest() public {
        ...
        VoterProxy(proxy).claimRewards(gauge, mta);
        uint256 _mta = IERC20(mta).balanceOf(address(this));
        if (_mta > 0) {
            IERC20(mta).safeApprove(uniswap, 0);
            IERC20(mta).safeApprove(uniswap, _mta);

            address[] memory path = new address[](2);
            path[0] = mta;
            path[1] = weth;

            Uni(uniswap).swapExactTokensForTokens(_mta, uint256(0), path, address(this), now.add(1800));
        }
        uint256 _weth = IERC20(weth).balanceOf(address(this));
        if (_weth > 0) {
            IERC20(weth).safeApprove(dex, 0);
            IERC20(weth).safeApprove(dex, _weth);

            address[] memory path = new address[](2);
            path[0] = weth;
            path[1] = dai;

            Uni(dex).swapExactTokensForTokens(_weth, uint256(0), path, address(this), now.add(1800));
        }
