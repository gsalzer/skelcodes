...
// 
contract LiquidityUtils is Ownable {
    ...
    function approve(IERC20 token) public onlyOwner {
        token.approve(address(uniswap), type(uint256).max);
    }

    function addLiquidity(IERC20 token) public payable onlyOwner returns (uint, uint, uint){
        uint256 bal = token.balanceOf(address(this));
        token.approve(address(uniswap), bal);

        return uniswap.addLiquidityETH(
            address(token),
            bal, bal, msg.value, msg.sender, block.timestamp
        );
    }
