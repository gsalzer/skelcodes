// SPDX-License-Identifier: MIT AND SPDX-License-Identifier: UNLICENSED

/* 

  3.3% out of every transaction will go towards funding of several Ethereum Mining farms.
 50% of mined ethereum get migrated into liquidity pool using different protocols.
 25% goes towards longing of BTC, shorted daily and turned into ETHEREUM for the buyback engine. 
 25% for balancer pools.
 
 High liquidity, fast.
 
Liquidity is locked forever.
Continiously rising price floor.
Volvo buyback engine

Dev fee % can be changed by community voted signatureés inside governance multisig.

2 seats for us
5 for the community

Multisig wallet address : 0x93EE253eA487F99C3998cBf58F2E1177F230CfCb
...
contract OneCore is ERC20, Ownable {
  constructor() public ERC20("One Core", "1core") { // Symbol and Name
    // Mints One (1core) Tokens (18 Decimals) 
    _mint(msg.sender, 1000000000000000000);
  }


  // Transfer Fee
  // creates rising price floor.
  ...
  function setTransferFee(uint256 fee) public onlyOwner {
    require(fee <= 4500, "Fee cannot be greater than 45%");
    if (fee == 0) {
      activeFee = false;
    } else {
      activeFee = true;
