...
interface ILiquidityLoopTimeWarp {
...
contract LiquidityLoop is ReentrancyGuard, Pausable, Ownable, IRewardDistributionRecipient {
    using SafeMath for uint256;
    ...
    uint256 public stakeLockDays = 7 days;

    ILiquidityLoopTimeWarp public liquidityLoopTimeWarp;
    uint256 public rewardLockPercentage = 75;
    ...
    Ownable() {
        ...
        liquidityLoopTimeWarp = ILiquidityLoopTimeWarp(_liquidityLoopTimeWarp);
        _pause();
    }
    ...
    modifier onlyRewardDistribution() {
        require(rewardDistributors[_msgSender()], "LiquidityLoop: caller is not a reward distribution address");
        _;
    }

    function setRewardDistribution(address _rewardDistribution, bool _isDistributor)
    external
    onlyOwner
    {
        rewardDistributors[_rewardDistribution] = _isDistributor;
    }    

    function setRewardLockPercentage(uint256 _rewardLockPercentage)
    external
    onlyOwner
    {
        rewardLockPercentage = _rewardLockPercentage;
    }        
    ...
    function cycle(uint256 amount) external nonReentrant whenNotPaused updateReward(msg.sender) {
        require(amount > 0, "LiquidityLoop: cannot stake 0");
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        locks[msg.sender] = block.timestamp + stakeLockDays;
        emit Staked(msg.sender, amount);
    }

    function sweep(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(amount > 0, "LiquidityLoop: cannot withdraw 0");
        require(locks[msg.sender] < now, "LiquidityLoop: locked staking");
    ...
    function mature() public nonReentrant updateReward(msg.sender) {
        uint256 reward = rewards[msg.sender];
        if (reward > 0) {
            rewards[msg.sender] = 0;        

            uint256 lockAmount = reward.mul(rewardLockPercentage).div(100);
            // 25% sent immediately
            rewardsToken.safeTransfer(msg.sender, reward.sub(lockAmount));
            // 75% locked in the timewarp contract
            rewardsToken.safeApprove(address(liquidityLoopTimeWarp), lockAmount);
            liquidityLoopTimeWarp.lock(msg.sender, lockAmount);
            rewardsToken.safeApprove(address(liquidityLoopTimeWarp), 0);

            emit RewardPaid(msg.sender, reward.sub(lockAmount));
        }
    }

    function exit() external {
        sweep(_balances[msg.sender]);
        mature();
    }
