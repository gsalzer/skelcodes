...
contract MerkleTreeWithHistory {
  ...
  uint256 public constant ZERO_VALUE = 21663839004416932945382355908790599225266501822907911457504978515578255421292; // = keccak256("unimix") % FIELD_SIZE
...
contract Unimix is MerkleTreeWithHistory, ReentrancyGuard {
...
// File: contracts/ERC20Unimix.sol
...
contract ERC20Unimix is Unimix {
  ...
  constructor(
    ...
    address _depositRewardDistributor,
    address _token
  ) Unimix(_verifier, _denomination, _merkleTreeHeight, _operator, _governance, _protocolFeeDistributor, _depositRewardDistributor) public {
    token = _token;
  }
  ...
  function _processWithdraw(address payable _recipient, address payable _relayer, uint256 _fee, uint256 _refund) internal {
    ...
    uint256 protocolFee = denomination * protocolFee / protocolFeeBase;

    _safeErc20Transfer(_recipient, denomination - _fee - protocolFee);
    if (_fee > 0) {
      _safeErc20Transfer(_relayer, _fee);
    }

    if (protocolFee > 0) {
      _safeErc20Transfer(protocolFeeDistributor, protocolFee);
    }
...
// File: contracts/deployment/UnimixDAI1K.sol

pragma solidity 0.5.17;


contract UnimixDAI1K is ERC20Unimix {
  constructor(
    IVerifier _verifier,
    address _protocolFeeDistributor,
    address _depositRewardDistributor
