...
contract Treasury is ContractGuard, Epoch, AccessControl {
    ...
    uint256 public currentEpochBurnedCeiling;
    uint256 public currentEpochTotalBurned;
    uint256 public totalPendingRewards;
    mapping(address=>mapping(uint256 => uint256)) currentEpochUserBurned;
    ...
    function getMelterOraclePrice() public view returns (uint256) {
        return _getCashPrice(melterOracle);
    }
    ...
    function updateMelterPrice() public onlyOperator {
        try IOracle(melterOracle).update() {
            uint256 melterPrice = getMelterOraclePrice();
            if (melterPrice <= cashPriceFloor) {
                uint256 poolUBCAmount = getTotalPooledUBC();
                uint256 percentage = cashPriceOne.sub(melterPrice);
                currentEpochBurnedCeiling = poolUBCAmount.mul(percentage).div(1e6);
                currentEpochTotalBurned = 0;
            } else {
                currentEpochBurnedCeiling = 0;
                currentEpochTotalBurned = 0;
            }
        } catch {}
    }
    ...
    function melter(uint256 amount) public onlyOneBlock checkMigration {
        require(amount > 0, "can not stake zero");
        uint256 melterPrice = getMelterOraclePrice();
        require(melterPrice <= cashPriceFloor, "melter not open");
        uint256 userStakeAmount = Boardroom(boardroom).balanceOf(msg.sender);
        require( userStakeAmount > 0,"user have not board seat");
        uint256 sharePerBurned = getSharePerBurned();
        uint256 userMaxBurned = sharePerBurned.mul(userStakeAmount);
        uint256 currentEpoch = getMelterEpoch();
        require(currentEpochUserBurned[msg.sender][currentEpoch].add(amount)<=userMaxBurned,"user have not enough UCS right to burn more UBC");
        require(currentEpochTotalBurned.add(amount) <= currentEpochBurnedCeiling,"melter alreay burned too much");
        IUCashAsset(cash).burnFrom(msg.sender, amount);
        currentEpochTotalBurned = currentEpochTotalBurned.add(amount);
        currentEpochUserBurned[msg.sender][currentEpoch] = currentEpochUserBurned[msg.sender][currentEpoch].add(amount);
        uint256 rewards = cashPriceOne.mul(1e18).div(melterPrice).mul(amount).div(1e18);
        CPEarned[msg.sender] = CPEarned[msg.sender].add(rewards);
        totalPendingRewards = totalPendingRewards.add(rewards);
    }

    function redeem() public onlyOneBlock {
        uint256 melterPrice =  getMelterOraclePrice();
        require(melterPrice >= cashPriceCeiling, "price lower than 1.05");
        require(CPEarned[msg.sender] > 0, "earned zero");
        IUCashAsset(cash).mint(msg.sender, CPEarned[msg.sender]);
        totalPendingRewards = totalPendingRewards.sub(CPEarned[msg.sender]);
        CPEarned[msg.sender] = 0;
    }


    function getSharePerBurned() public view returns(uint256){
        uint256 totalCash = IERC20(cash).totalSupply();
        uint256 boardroomStaked = IERC20(share).balanceOf(boardroom);
        uint256 SharePerBurned = totalCash.mul(1e18).div(boardroomStaked).div(1e18);
        return SharePerBurned;
    }

    function getMelterEpoch() public view returns(uint256){
        return Oracle(melterOracle).getCurrentEpoch();
    }
