/**
 * 
 * GearAutomatic is an autonomous DeFi governance system featuring automatic staking & yield farming.
 *
 * Official Website: 
...
contract GearAutomatic is ERC20Detailed 
{
    ...
    string constant tokenName = "Gear Automatic v2";
    ...
    mapping(address => bool) public autoWhitelistTo;
    event WhitelistFrom(address _addr, bool _whitelisted);
    event WhitelistTo(address _addr, bool _whitelisted);
    event AutoWhitelistTo(address _addr, bool _whitelisted);
    ...
    constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) 
    {
        ...
        _supply(msg.sender, 100000*(10**uint256(tokenDecimals)));
    }
    ...
    function _supply(address account, uint256 value) internal 
    {
        require(value != 0);
   ...
   /*
    *   transfer is taxed with a fee of 6%
    *   the receiver gets 94% of the sent value
    *   while 6% is distributed to UPIE staking pool
    */
    function _executeTransfer(address from, address to, uint256 value) private
    {
        ...
        if(!autoWhitelistTo[to] && !isStaking[to])
        {
             _enableStaking(to);
        }
        ...
        if(isStaking[from] && balances[from] < 1)
        {
             updateRewardsFor(from);
             isStaking[from] = false;
             fullUnitsStaked_total = fullUnitsStaked_total.sub(toFullUnits(balances[from]));
             fullUnitsStaked_total = fullUnitsStaked_total.sub(1);
        ...
        if(fus_total > 0)
        {
            uint256 stakingRewards = threePercent;
            //split up to rewards per unit
        ...
        uint256 amountToBurn = threePercent;
    ...
    //no Auto staking if receiver is whitelisted
    function setAutoWhitelistedTo(address _addr, bool _whitelisted) external onlyOwner {
        emit AutoWhitelistTo(_addr, _whitelisted);
        autoWhitelistTo[_addr] = _whitelisted;
    }
