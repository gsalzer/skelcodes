...
contract StakerCapped is Poolable{
    ...
    function stake() public payable{
        ...
        if(price() >= (1.05 * 10**18) && priceCapped){
            ...
            uint t = IERC20(fomoAddress).balanceOf(poolAddress); //token in uniswap
            uint a = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
            uint x = (sqrt(9*t*t + 3988000*a*t) - 1997*t)/1994;

            IERC20(fomoAddress).mint(address(this), x);

            address[] memory path = new address[](2);
            path[0] = fomoAddress;
            path[1] = WETHAddress;
            IERC20(fomoAddress).approve(UNIROUTER, x);
            Uniswap(UNIROUTER).swapExactTokensForETH(x, 1, path, POOLADDRESS, INF);
        }
        ...
        uint amountTokenDesired = IERC20(fomoAddress).balanceOf(address(this));
