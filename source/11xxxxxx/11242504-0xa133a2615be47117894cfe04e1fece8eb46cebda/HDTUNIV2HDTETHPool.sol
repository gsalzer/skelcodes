...
/**
 * HDT-UNI-V2(HDT-ETH) Pool
 */
contract HDTUNIV2HDTETHPool is HDTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "HDT-UNI-V2(HDT-ETH)-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "HDT-UNI-V2(HDT-ETH)-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"HDT-UNI-V2(HDT-ETH)-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "HDT-UNI-V2(HDT-ETH)-POOL: Pool is closed");
        _;
    }
