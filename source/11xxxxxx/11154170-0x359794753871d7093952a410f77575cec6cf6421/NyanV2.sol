...
contract NyanV2 is ERC20, NyanV2DataLayout, Proxiable {
    ...
    modifier _updateRewards() {
        ...
        if (!checkpointReset) {
          lastNyanCheckpoint = lastNyanCheckpoint.sub(6500);
          initialNyanCheckpoint = lastNyanCheckpoint;
          initialContractNyanHeld = ERC20(address(this)).balanceOf(address(this));
          totalNyanV2Held = ERC20(address(this)).balanceOf(address(this));
        ...
        if (userStake[msg.sender].stakedNyanV2LP > 0) {
            ...
            if (rSync[msg.sender].currentContractNyanHeld == 0) {
              rSync[msg.sender].currentContractNyanHeld = initialContractNyanHeld;
              rSync[msg.sender].currentStakerCheckpoint = initialNyanCheckpoint;
            }
            nyanRewardsPerDay = rSync[msg.sender].currentContractNyanHeld.mul(rewardsPercentage).div(10000);
            ...
            uint256 blockAdd = block.number.sub(userStake[msg.sender].lastBlockChecked);
            timeSinceCheckpoint = userStake[msg.sender].lastBlockChecked.add(blockAdd).sub(rSync[msg.sender].currentStakerCheckpoint);
            uint256 percReduction;
            if(timeSinceCheckpoint > 0) {
              if (timeSinceCheckpoint > 6500) {
                percReduction = timeSinceCheckpoint.div(6500);
              }

              uint256 userRewardPerDaySub = userRewardPerDay.mul(percReduction.mul(100)).div(10000);
              userStake[msg.sender].nyanV2Rewards = userStake[msg.sender].nyanV2Rewards.add(userRewardPerDay.mul(timeSinceCheckpoint).sub(userRewardPerDaySub));
              rSync[msg.sender].currentStakerCheckpoint = lastNyanCheckpoint;
              rSync[msg.sender].currentContractNyanHeld = totalNyanV2Held;
            }
    ...
    function viewNyanRewards(address staker) delegatedOnly public view returns(uint256) {
        ...
        if (userStake[staker].stakedNyanV2LP > 0) {
            ...
            uint256 currentContractNyanHeld;
            uint256 currentStakerCheckpoint;
            if (rSync[staker].currentContractNyanHeld == 0) {
              currentContractNyanHeld = initialContractNyanHeld;
              currentStakerCheckpoint = initialNyanCheckpoint;
            } else {
              currentContractNyanHeld = rSync[staker].currentContractNyanHeld;
              currentStakerCheckpoint = rSync[staker].currentStakerCheckpoint;
            }
            uint256 userStakeAmount = userStake[staker].stakedNyanV2LP.add(userStake[staker].stakedDNyanV2LP);
            // get user share of nyanRewardsPerDay
            uint256 userRewardPerDay = currentContractNyanHeld.mul(rewardsPercentage).div(10000)
                                          .div(totalLP.div(userStakeAmount))
                                          .div(6500);
          
            uint256 timeSinceCheckpoint;
            timeSinceCheckpoint = userStake[staker].lastBlockChecked
                                    .add(block.number.sub(userStake[staker].lastBlockChecked))
                                    .sub(currentStakerCheckpoint);
            uint256 percReduction;
            if(timeSinceCheckpoint > 0) {
              if (timeSinceCheckpoint > 6500) {
                percReduction = timeSinceCheckpoint.div(6500);
              ...
              currentRewards = currentRewards
                                .add(userRewardPerDay.mul(timeSinceCheckpoint)
                                .sub(userRewardPerDay.mul(percReduction.mul(100)).div(10000)));
          }
        }
    ...
    function transfer(address _recipient, uint256 _amount) delegatedOnly public override returns(bool) {    
        ...
        if((msg.sender == uniswapRouterV2) || (msg.sender == nyanV2LP)) {
          ...
          // require(userStake[_recipient].nyanV2LPStaked == 0);
          // Slightly decrease poolMax on each send
          uint256 reduction = _amount.div(100);
          nyanPoolMax = nyanPoolMax.sub(reduction);
        }

        if(_recipient == nyanV2LP) {
          require(v2Nyan.add(_amount) <= nyanPoolMax.add(nyanPoolMax.div(2)));
        }

        // Check if LP has increased.
        if (lastLPCount < ERC20(nyanV2LP).totalSupply()) {
          lastLPCount = ERC20(nyanV2LP).totalSupply();
          uint256 nyanDecrease;
          if (v2Nyan > v2ETH) {
            nyanDecrease = v2Nyan.div(v2ETH);
          } else {
            nyanDecrease = v2ETH.div(v2Nyan);
          }
          nyanPoolMax = nyanPoolMax.sub(nyanDecrease);
        }
