...
contract ExchangeSwapV6 is AccessControl {
    using SafeERC20 for IERC20;
    using SafeMath for uint;

    bytes32 public constant EXECUTOR_ROLE = bytes32('Executor');
    uint constant EXTRA_GAS = 21000; // SLOAD * 2, SSTORE update * 2, CALL with value, calc.
    uint constant GAS_SAVE = 30000;
    ...
    function updateFeeCollector(address payable _address) external onlyOwner() {
    ...
    function makeSwap(Request memory _request) external returns(bool) {
        require(hasRole(EXECUTOR_ROLE, _msgSender()), 'Only Executor');
        require(coverFees[_request.user] >= (_request.txGasLimit * tx.gasprice),
            'Cover fees deposit required');

        bool _result = true;
        try this._execute{gas: gasleft().sub(GAS_SAVE)}(_request) {} catch {
            _result = false;
        }
    ...
    function makeSwapETHForTokens(RequestETHForTokens memory _request) external returns(bool) {
        require(hasRole(EXECUTOR_ROLE, _msgSender()), 'Only Executor');
        require(coverFees[_request.user] >= ((_request.txGasLimit * tx.gasprice) + _request.amountFrom),
            'Cover fees deposit required');

        bool _result = true;
        try this._executeETHForTokens{gas: gasleft().sub(GAS_SAVE)}(_request) {} catch {
            _result = false;
        }
        _chargeFee(_request.user, _request.txGasLimit);
        return _result;
    ...
    function makeSwapTokensForETH(RequestTokensForETH memory _request) external returns(bool) {
        require(hasRole(EXECUTOR_ROLE, _msgSender()), 'Only Executor');
        require(coverFees[_request.user] >= (_request.txGasLimit * tx.gasprice),
            'Cover fees deposit required');

        bool _result = true;
        try this._executeTokensForETH{gas: gasleft().sub(GAS_SAVE)}(_request) {} catch {
            _result = false;
        }
        _chargeFee(_request.user, _request.txGasLimit);
        return _result;
