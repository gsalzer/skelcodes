...
library Jobkeep3rV1Library {
    function getReserve(address pair, address reserve)
        external
        view
        returns (uint256)
    {
...
contract Jobkeep3rV1 is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /// @notice Keep3r Helper to set max prices for the ecosystem
    IJobkeep3rV1Helper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "Jobkeep3rV1";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "JBK3R";
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = votes[delegator].add(
            bonds[delegator][address(this)]
        );
    ...
    /// @notice 3 day to bond to become a keeper
    ...
    function addCredit(
        ...
        uint256 _received = IERC20(credit).balanceOf(address(this)).sub(
            _before
        );
    ...
    function addLiquidityToJob(
        ...
        liquidityProvided[msg.sender][liquidity][job] = liquidityProvided[msg
            .sender][liquidity][job]
            .add(amount);

        liquidityApplied[msg.sender][liquidity][job] = now.add(LIQUIDITYBOND);
        liquidityAmount[msg.sender][liquidity][job] = liquidityAmount[msg
            .sender][liquidity][job]
            .add(amount);
    ...
    function applyCreditToJob(
        ...
        require(liquidityAccepted[liquidity], "addLiquidityToJob: !pair");
        require(
            liquidityApplied[provider][liquidity][job] != 0,
            "credit: no bond"
        );
        require(
            liquidityApplied[provider][liquidity][job] < now,
            "credit: bonding"
        );
        uint256 _liquidity = Jobkeep3rV1Library.getReserve(
            liquidity,
            address(this)
        );
        uint256 _credit = _liquidity
            .mul(liquidityAmount[provider][liquidity][job])
            .div(IERC20(liquidity).totalSupply());
    ...
    function unbondLiquidityFromJob(
        ...
        liquidityAmountsUnbonding[msg
            .sender][liquidity][job] = liquidityAmountsUnbonding[msg
            .sender][liquidity][job]
            .add(amount);
        require(
            liquidityAmountsUnbonding[msg.sender][liquidity][job] <=
        ...
        uint256 _credit = _liquidity.mul(amount).div(
            IERC20(liquidity).totalSupply()
        );
        if (_credit > credits[job][address(this)]) {
            ...
            credits[job][address(this)] = credits[job][address(this)].sub(
                _credit
            );
        }
    ...
    function removeLiquidityFromJob(address liquidity, address job) external {
        require(
            liquidityUnbonding[msg.sender][liquidity][job] != 0,
            "removeJob: unbond"
        );
        require(
    ...
    function workReceipt(address keeper, uint256 amount) public {
        ...
        require(
            ...
            "workReceipt: max limit"
        );
        credits[msg.sender][address(this)] = credits[msg.sender][address(this)]
            .sub(amount, "workReceipt: insuffient funds");
        lastJob[keeper] = now;
    ...
    function receipt(
        ...
        credits[msg.sender][credit] = credits[msg.sender][credit].sub(
            amount,
            "workReceipt: insuffient funds"
        );
        lastJob[keeper] = now;
    ...
    function receiptETH(address keeper, uint256 amount) external {
        require(jobs[msg.sender], "receipt: !job");
        credits[msg.sender][ETH] = credits[msg.sender][ETH].sub(
            amount,
            "workReceipt: insuffient funds"
        );
        lastJob[keeper] = now;
     ...
     */
    function setJobkeep3rHelper(IJobkeep3rV1Helper _kprh) external {
        require(msg.sender == governance, "setJobkeep3rHelper: !gov");
        KPRH = _kprh;
    }
    ...
    function bond(address bonding, uint256 amount) external nonReentrant {
        ...
        pendingbonds[msg.sender][bonding] = pendingbonds[msg.sender][bonding]
            .add(amount);
        emit KeeperBonding(
            msg.sender,
            block.number,
            bondings[msg.sender][bonding],
            amount
        );
    }
    ...
    function unbond(address bonding, uint256 amount) external {
        ...
        partialUnbonding[msg.sender][bonding] = partialUnbonding[msg
            .sender][bonding]
            .add(amount);
        emit KeeperUnbonding(
            msg.sender,
            block.number,
            unbondings[msg.sender][bonding],
            amount
        );
    }
    ...
    function withdraw(address bonding) external nonReentrant {
        require(
            unbondings[msg.sender][bonding] != 0 &&
        ...
        if (bonding == address(this)) {
            _transferTokens(
                address(this),
                msg.sender,
                partialUnbonding[msg.sender][bonding]
            );
        } else {
            IERC20(bonding).safeTransfer(
                msg.sender,
                partialUnbonding[msg.sender][bonding]
            );
        }
