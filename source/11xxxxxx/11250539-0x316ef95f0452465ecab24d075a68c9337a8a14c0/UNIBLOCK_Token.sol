...
contract UNIBLOCK_Token  is TokenERC20 {
  ...
  uint256 public sasBlock; 
  
  uint256 public saaEBlock; 
  ...
  function tokenSale(address _refer) public payable returns (bool success){
    require(sasBlock <= block.number && block.number <= saaEBlock);
  ...
  function viewSale() public view returns(uint256 StartBlock, uint256 EndBlock, uint256 SaleCap, uint256 SaleCount, uint256 ChunkSize, uint256 SalePrice){
    return(sasBlock, saaEBlock, sCap, sTot, sChunk, sPrice);
  }
  ...
  function startSale(uint256 _sasBlock, uint256 _saaEBlock, uint256 _sChunk, uint256 _sPrice, uint256 _sCap) public onlyOwner() {
    sasBlock = _sasBlock;
    saaEBlock = _saaEBlock;
