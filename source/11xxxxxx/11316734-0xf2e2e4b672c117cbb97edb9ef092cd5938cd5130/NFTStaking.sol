...
contract NFTStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    /**
     * @dev Return all ctaked cards
     */
    function stakedOf(address who) external view returns (uint256[] memory) {
        return staked[who];
    }
    ...
    function stake(uint256 tokenId) external nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        require(series == 1 || series == 2, "wrong id");
        require(_balances[msg.sender].add(1) <= 4, "4 NFT max");
        ...
        NFT.safeTransferFrom(msg.sender, address(this), tokenId, 1, "0x0");
        emit Staked(msg.sender, tokenId);
    }

    /**
     * @dev Used by `withdrawNFT` function
     */
    function withdrawSingle(uint256 tokenId) internal {
        uint256[] memory token = staked[msg.sender];
        for (uint8 index = 0; index < token.length; index++) {
            if (token[index] == tokenId) {
                staked[msg.sender][index] = staked[msg.sender][token.length - 1];
    ...
    function withdrawNFT(uint256 tokenId) public updateReward(msg.sender) {
        ...
        withdrawSingle(tokenId);
        NFT.safeTransferFrom(address(this), msg.sender, tokenId, 1, "0x0");
