...
interface ILegacyDollar is IERC20 {
...
library Constants {
    ...
    function getPreviousEpochStrategy()
        internal
        pure
...
abstract contract Permittable is ERC20UpgradeSafe {
...
contract Dollar is IDollar, MinterRole, Permittable, ERC20BurnableUpgradeSafe {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    ILegacyDollar public legacyDollar; // vUSD v1

    event Migrate(address indexed account, uint256 amount);

    function initialize(ILegacyDollar _legacyDollar) public initializer {
        legacyDollar = _legacyDollar;
        __ERC20_init("Value USD", "vUSD");
    ...
    function migrate(uint256 _amount) public {
        legacyDollar.transferFrom(msg.sender, address(this), _amount);
        legacyDollar.burn(_amount);
        _mint(msg.sender, _amount.mul(1e9));
    ...
    function burn(uint256 amount)
        public
        override(IDollar, ERC20BurnableUpgradeSafe)
    {
        super.burn(amount);
    }
