...
// File: contracts/SunflowerToken.sol

pragma solidity 0.6.12;

// SunflowerToken with Governance.
contract SunflowerToken is ERC20("SunflowerToken", "SFR"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (SunflowerMain).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "SFR::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SFR::delegateBySig: invalid nonce");
        require(now <= expiry, "SFR::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "SFR::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SFRs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "SFR::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts/SunflowerMain.sol
