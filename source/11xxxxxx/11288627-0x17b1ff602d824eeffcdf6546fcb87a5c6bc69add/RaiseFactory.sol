...
library RaiseDeployer {
    ...
    function initializeRaise(
        address _proxy, IERC20 _dchf, 
        address _issuer, uint256 _min,
        uint256 _max, uint256 _price, 
        uint256 _subscription, uint256 _opening, 
        uint256 _closing, address _baseOperators, 
        address _raiseOperators) 
        public
    {
        Raise(_proxy).initialize(
            _dchf, _issuer, _min,
            _max, _price, _subscription, 
            _opening, _closing, _baseOperators, 
            _raiseOperators);
    }
}
...
library ProxyDeployer {
    ...
    */
    function deployProxy(address _implementation, address _proxyAdmin, bytes memory _data)
        public
        returns(address)
    {
 ...
 * @title RaiseFactory
 * @author Connor Howe <connor.howe@sygnum.com>
 * @dev Riae factory to be used by operators to deploy arbitrary Sygnum Capital Raise.
 */
...
contract RaiseFactory is Initializable, RaiseOperatorable {
    ...
    event ProposalAccepted(address indexed issuer, bytes32 indexed identifier, address raise, address proxy, address token);
    event ProposalDeclined(address indexed issuer, bytes32 indexed identifier, address indexed operator, uint256 timestamp);
     ...
     */
    function initialize(IERC20 _dchf, address _baseOperators, address _raiseOperators, address _proxyAdmin, address _implementation)
        public
        initializer
    {
    ...
    */
    function newRaiseProposal(bytes32 _identifier)
        public
        onlyIssuer
    {
    ...
    */
    function operatorProposal(
        bytes32 _identifier, bool _accept,
        uint256 _min, uint256 _max, uint256 _price,
        uint256 _subscription, uint256 _open,
        uint256 _close, address _token
    )
        public
        onlyOperator
        returns(address proxy)
    {
        ...
        RaiseDeployer.initializeRaise(
            proxy, dchf, info.issuer,
            _min, _max, _price, _subscription,
            _open, _close, getOperatorsContract(),
    ...
    */
    function implementationExists(bytes32 _identifier)
        public
        view
        returns(bool)
    {
    ...
    */
    function getImplementationAndProxy(bytes32 _identifier)
        public
        view
    ...
    */
    function getToken(bytes32 _identifier)
        public
        view
        returns(address)
    {
    ...
    */
    function getIssuer(bytes32 _identifier)
        public
        view
        returns(address)
    {
