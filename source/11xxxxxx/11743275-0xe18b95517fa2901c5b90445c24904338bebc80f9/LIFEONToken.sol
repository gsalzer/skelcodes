...
contract TokenLock is ERC20, Ownable {
  ...
  // checks if the address can transfer tokens
  modifier canMultiTransfer(address _sender, uint256[] memory _value) {
    require(_sender != address(0));
    
    uint256 ui;
    uint256 amountSum = 0;
    
    for (ui = 0; ui < _value.length; ui++) {
        amountSum = amountSum.add(_value[ui]);
    }
    ...
    require(
      (_sender == owner()) || (
        transferEnabled && (
          noTokenLocked ||
          canTransferIfLocked(_sender, amountSum)
        )
      )
    );
  ...
  function canTransferIfLocked(address _sender, uint256 _value) public view returns(bool) {
    uint256 after_math = super.balanceOf(_sender).sub(_value);
 ...
 * bug.
 */
contract ERC20Pausable is Pausable, TokenLock, MultiTransferToken {
    function transfer(address to, uint256 value) public whenNotPaused canTransfer(msg.sender, value) returns (bool) {
        return super.transfer(to, value);
    }

    function transferFrom(address from, address to, uint256 value) public whenNotPaused canTransfer(msg.sender, value) returns (bool) {
        return super.transferFrom(from, to, value);
    }
    
    function multiTransfer(address[] memory to, uint256[] memory value) public whenNotPaused canMultiTransfer(msg.sender, value) returns (bool) {
        return super.multiTransfer(to, value);
    }
...
contract ERC20Mintable is ERC20, MinterRole {
    ...
    constructor() internal {
        _mint(msg.sender, 15300000000000000000000000000);
    }
...
contract LIFEONToken is ERC20Pausable, ERC20Burnable, ERC20Mintable {
  string public constant name = "LIFEON CASH";
  string public constant symbol = "LIFEON";
