...
contract CHIBurner {
    address internal constant CHI_ADDRESS = 0x0000000000004946c0e9F43F4Dee607b0eF1fA1c;
    ICHI internal constant chi = ICHI(0x0000000000004946c0e9F43F4Dee607b0eF1fA1c);
...
contract ValueHolder is Ownable, CHIBurner, XChangerUser{
    ...
    function topUpChi(IERC20 Token, uint amountAvailable) public returns (uint spendAmount) {
    ...
    function burnUniLq(uint amount, address pool) internal returns (IERC20 token0, IERC20 token1, uint bal0, uint bal1) {
        ...
        uint lq = pair.totalSupply().mul(I1).div(reserve1); // might be min of either of those token0/token1
    ...
    function getHolderPc(IUniswapV2Exchange uniPool) internal view returns (uint holderPc) {
