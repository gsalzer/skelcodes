...
Website: bonzai.cc
Tele: t.me/bonzai_farm

forked from KIMCHI, SUSHI, KIMBAP
...
contract Ownable is Context {
     ...
     * at any time but cannot mint anymore as soon as the 'minter' gets changes (to the farm contract)
     */
...
// BonzaiToken with Governance.
//                       ERC20 (name, symbol, decimals, burnrate, initSupply)
contract Token is ERC20("BONZAI.CC", "BONZAI", 18, 0, 1200), Ownable, Mintable {
...
contract BonzaiFarm is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        // claimable BONZAI by a user is:
        //
        //   pending reward = (user.amount * pool.accBonzaiPerLPShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBonzaiPerLPShare` (and `lastRewardedBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accBonzaiPerLPShare; // Accumulated Bonzai per share.
        uint256 lpTokenHolding;     // Amount of LP token hold by this pool
    }

    // Info of Bonzai minting
    struct MintInfo {
        uint256 BonzaiPerBlock;  // Amount of minted Bonzai each block.
        uint256 mintStartBlock; // Minting of this amount starts here.
    }

    // The Bonzai token contract
    Token public Bonzai;
    ...
    // Info of Bonzai minting block span.
    ...
    constructor(
        Token _Bonzai,
        address _devaddr,
        uint256 _BonzaiPerBlock,
        uint256 _mintStartBlock
    ) public {
        Bonzai = _Bonzai;
        devaddr = _devaddr;
        mintInfo.push(MintInfo({
            BonzaiPerBlock: _BonzaiPerBlock,
    ...
    // Amount of block spans for minting different amounts of Bonzai
    ...
    function setMint(uint256 _mid, uint256 _block, uint256 _amount) public onlyOwner {
        ...
        mintInfo[_mid].BonzaiPerBlock = _amount;
    ...
    // Also used to claim the users pending Bonzai tokens
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accBonzaiPerLPShare).div(1e18).sub(user.rewardDebt);
        safeBonzaiTransfer(msg.sender, pending);
        user.amount = user.amount.sub(_amount);
        user.rewardDebt = user.amount.mul(pool.accBonzaiPerLPShare).div(1e18);
    ...
    function getBonzaiAmountInRange(uint256 _from, uint256 _to) public view returns (uint256) {
        if(_from >= _to){
            return 0;
        }
        uint256 reward = 0;
        ...
        for( uint256 i = 0; i < mintInfo.length-1; i++){
            for( uint256 cblock = _from+1; cblock <= _to; cblock++){
                if(cblock >= mintInfo[i].mintStartBlock && cblock < mintInfo[i+1].mintStartBlock){
                    reward += mintInfo[i].BonzaiPerBlock;
                    blocksleft--;
                }
            }
        }
        reward += blocksleft.mul(mintInfo[mintInfo.length-1].BonzaiPerBlock);
        return reward;
    }

    // View function to see pending Bonzai on frontend.
    function pendingBonzai(uint256 _pid, address _user) public returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBonzaiPerLPShare = pool.accBonzaiPerLPShare;
        ...
        if (block.number > pool.lastRewardedBlock && lpSupply > 0) {
            uint256 BonzaiReward = getBonzaiAmountInRange(pool.lastRewardedBlock, block.number).mul(pool.allocPoints).div(totalAllocPoints);
            BonzaiReward = BonzaiReward.mul(1e18);
            accBonzaiPerLPShare = accBonzaiPerLPShare.add(BonzaiReward.div(lpSupply));
        }
        return user.amount.mul(accBonzaiPerLPShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 BonzaiReward = getBonzaiAmountInRange(pool.lastRewardedBlock, block.number).mul(pool.allocPoints).div(totalAllocPoints);
        Bonzai.mint(devaddr, BonzaiReward.div(5)); 
        Bonzai.mint(address(this), BonzaiReward);
        BonzaiReward = BonzaiReward.mul(1e18);
        pool.accBonzaiPerLPShare = pool.accBonzaiPerLPShare.add(BonzaiReward.div(lpSupply));
        pool.lastRewardedBlock = block.number;
    }

    // Deposit LP tokens to Farm for Bonzai allocation.
    // Also used to claim the users pending Bonzai tokens (_amount = 0)
    ...
    // Safe Bonzai transfer function, just in case if rounding error causes pool to not have enough bonzai.
    function safeBonzaiTransfer(address _to, uint256 _amount) internal {
        uint256 BonzaiBal = Bonzai.balanceOf(address(this));
        if (_amount > BonzaiBal) {
            Bonzai.transfer(_to, BonzaiBal);
        } else {
            Bonzai.transfer(_to, _amount);
        }
    }
    
    // Update dev address by the previous dev.
    function dev(address _devaddr) public {
    require(msg.sender == devaddr, "dev: nonono?");
    devaddr = _devaddr;
    }
}
