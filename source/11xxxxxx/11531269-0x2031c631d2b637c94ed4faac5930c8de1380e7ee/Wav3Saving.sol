...
contract Wav3Saving is Ownable, StakingInterface {
    ...
    // a + b*x + c*x^2
    uint256 public A = 146663; // 1.46663
    uint256 public B = 44259;  // 0.4442953*x
    uint256 public C = 107;    // 0.00107407*x^2

    uint256 public maxDays = 360;
    uint256 public minDays = 10;

    uint256 public totalStaked = 0;
    uint256 public totalRewards = 0;

    uint256 public earlyStop = 0;
    ...
    constructor(address _wav3) public {
        wav3 = IERC20(_wav3);
    }
    ...
    function save(uint256 _amount, uint256 _days) public {
        ...
        require(_amount > 10, "amount to small");
        ...
        require(wav3.balanceOf(address(this)) > totalOwedValue().add(_reward).add(_amount), "insufficient contract bal");

        require(wav3.transferFrom(msg.sender, address(this), _amount), "transfer failed");
    ...
    function earn() public {
        ...
        require(block.timestamp > stakes[msg.sender].payday.sub(earlyStop), "too early");
        ...
        require(wav3.transfer(msg.sender, owed), "transfer failed");
    }
    ...
    function setEarlyStop(uint256 _earlyStop) public onlyOwner {
        require(_earlyStop < 31104000, "too big");
        close(true);
    ...
    function ownerReclaim(uint256 _amount) public onlyOwner {
        require(_amount < wav3.balanceOf(address(this)).sub(totalOwedValue()), "cannot withdraw owed funds");
        wav3.transfer(msg.sender, _amount);
    }
