...
contract PriceConsumerV3 is Ownable {
  ...
  returns(int256, uint256, uint256, uint256)
  {
    ...
    uint256 result = price.div(10 ** 8).toUint256(); //priceFeed.decimals().toUint256());
    uint256 response = uint256(10e18).div(result);

    return (price, price.toUint256(), result, response);
  }
