// hevm: flattened sources of src/strategies/basis/strategy-basis-bas-dai-lp.sol
...
library SafeERC20 {
     ...
     */
    function safeApprove(IERC20_1 token, address spender, uint256 value) internal {
    ...
    function safeDecreaseAllowance(IERC20_1 token, address spender, uint256 value) internal {
...
abstract contract StrategyBase {
    ...
    function balanceOfWant() public view returns (uint256) {
        return IERC20_1(want).balanceOf(address(this));
    }
    ...
    function withdraw(IERC20_1 _asset) external returns (uint256 balance) {
    ...
    function _swapUniswap(
        ...
        // Swap with uniswap
        IERC20_1(_from).safeApprove(univ2Router2, 0);
        IERC20_1(_from).safeApprove(univ2Router2, _amount);
    ...
    function _swapUniswapWithPath(
        ...
        // Swap with uniswap
        IERC20_1(path[0]).safeApprove(univ2Router2, 0);
        IERC20_1(path[0]).safeApprove(univ2Router2, _amount);
    ...
    function _swapSushiswap(
        ...
        // Swap with uniswap
        IERC20_1(_from).safeApprove(sushiRouter, 0);
        IERC20_1(_from).safeApprove(sushiRouter, _amount);
    ...
    function _swapSushiswapWithPath(
        ...
        // Swap with uniswap
        IERC20_1(path[0]).safeApprove(sushiRouter, 0);
        IERC20_1(path[0]).safeApprove(sushiRouter, _amount);
    ...
    function _distributePerformanceFeesAndDeposit() internal {
        uint256 _want = IERC20_1(want).balanceOf(address(this));
...
abstract contract StrategyStakingRewardsBase is StrategyBase {
    ...
    function deposit() public override {
        uint256 _want = IERC20_1(want).balanceOf(address(this));
        if (_want > 0) {
            IERC20_1(want).safeApprove(rewards, 0);
            IERC20_1(want).safeApprove(rewards, _want);
...
////// src/strategies/strategy-basis-farm-base.sol
/* pragma solidity ^0.6.7; */
...
abstract contract StrategyBasisFarmBase is StrategyStakingRewardsBase {
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _bas = IERC20_1(bas).balanceOf(address(this));
        if (_bas > 0) {
            // 10% is locked up for future gov
            uint256 _keepBAS = _bas.mul(keepBAS).div(keepBASMax);
            IERC20_1(bas).safeTransfer(
                IController(controller).treasury(),
                _keepBAS
            );
            path[0] = bas;
        ...
        uint256 _dai = IERC20_1(dai).balanceOf(address(this));
        if (_dai > 0) {
            path[0] = dai;
            path[1] = token1;
        ...
        _dai = IERC20_1(dai).balanceOf(address(this));
        uint256 _token1 = IERC20_1(token1).balanceOf(address(this));
        if (_dai > 0 && _token1 > 0) {
            IERC20_1(dai).safeApprove(univ2Router2, 0);
            IERC20_1(dai).safeApprove(univ2Router2, _dai);

            IERC20_1(token1).safeApprove(univ2Router2, 0);
            IERC20_1(token1).safeApprove(univ2Router2, _token1);
            ...
            // Donates DUST
            IERC20_1(dai).transfer(
                IController(controller).treasury(),
                IERC20_1(dai).balanceOf(address(this))
            );
            IERC20_1(token1).safeTransfer(
                IController(controller).treasury(),
                IERC20_1(token1).balanceOf(address(this))
            );
        }
...
////// src/strategies/basis/strategy-basis-bas-dai-lp.sol
/* pragma solidity ^0.6.7; */
