...
contract BMonTwapable {
    using FixedPoint for *;

    event TwapUpdated(uint256 newTwap, uint256 priceCumulative, uint32 blockTimestamp);

    uint256 public constant PRESALE_PRICE_MULTIPLIER = 20; // 1 ETH = 20 $BMON
    uint256 public constant LISTING_PRICE_MULTIPLIER = 10; // 1 ETH = 10 $BMON
    ...
    function _getPresalePrice() internal pure returns (uint256) {
        return PRESALE_PRICE_MULTIPLIER * 1 ether;
    }
