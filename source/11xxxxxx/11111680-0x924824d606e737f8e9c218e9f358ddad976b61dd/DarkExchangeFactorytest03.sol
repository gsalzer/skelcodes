...
// File: contracts\DarkExchange\DarkExchange_ERC20.sol

// 
pragma solidity =0.6.12;


contract DarkExchangeERC20 {
    using SafeMath for uint;

    string public constant name = "DarkExchange LP Token";
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, "DarkExchange: EXPIRED");
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, "DarkExchange: INVALID_SIGNATURE");
        _approve(owner, spender, value);
    }
}

// File: contracts\DarkExchange\libraries\Math.sol
...
// File: contracts\DarkExchange\libraries\UQ112x112.sol
...
// File: contracts\DarkExchange\interfaces\IDarkExchange_Callee.sol

// 
pragma solidity >=0.5.0;

interface IDarkExchangeCallee {
    function DarkExchangeCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}
...
// File: contracts\DarkExchange\DarkExchange_SlippageToken.sol

// 
pragma solidity =0.6.12;


contract DarkExchangeSlippageToken {
    using SafeMath for uint;

    string public constant name = "Dark Slippage Token";
...
// File: contracts\DarkExchange\DarkExchange_Pair.sol
...
contract DarkExchangePair is DarkExchangeERC20 {
    ...
    DarkExchangeSlippageToken public stoken;
    ...
    modifier lock() {
        require(unlocked == 1, "DarkExchange: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint256 value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "DarkExchange: TRANSFER_FAILED");
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "DarkExchange: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
        stoken = new DarkExchangeSlippageToken(0);
    }

    // update reserves and, on the first call per block, price accumulators
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), "DarkExchange: OVERFLOW");
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IDarkExchangeFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            address migrator = IDarkExchangeFactory(factory).migrator();
            if (msg.sender == migrator) {
                liquidity = IMigrator(migrator).desiredLiquidity();
                require(liquidity > 0 && liquidity != uint256(-1), "DarkExchange: Bad desired liquidity");
            } else {
                require(migrator == address(0), "DarkExchange: Must not have migrator");
        ...
        require(liquidity > 0, "DarkExchange: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1);
        {
            ...
            require(amount0 > 0 && amount1 > 0, "DarkExchange: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, "DarkExchange: INSUFFICIENT_OUTPUT_AMOUNT");
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "DarkExchange: INSUFFICIENT_LIQUIDITY");
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, "DarkExchange: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0) IDarkExchangeCallee(to).DarkExchangeCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint256 amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint256 amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, "DarkExchange: INSUFFICIENT_INPUT_AMOUNT");
        {
...
// File: contracts\DarkExchange\DarkExchange_Factory.sol

// 
pragma solidity ^0.6.12;


contract DarkExchangeFactorytest03 {
    ...
    constructor() public {
        feeToSetter = 0x0EfE62b7ecA25523d75C547fDC8aA8B6086D88Da;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, "DarkExchange: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "DarkExchange: ZERO_ADDRESS");
        require(getPair[token0][token1] == address(0), "DarkExchange: PAIR_EXISTS"); // single check is sufficient
        bytes memory bytecode = type(DarkExchangePair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IDarkExchangePair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, "DarkExchange: FORBIDDEN");
        feeTo = _feeTo;
    }
    ...
    function getCodeHash() external pure returns (bytes32) {
        return keccak256(type(DarkExchangePair).creationCode);
   }
}
