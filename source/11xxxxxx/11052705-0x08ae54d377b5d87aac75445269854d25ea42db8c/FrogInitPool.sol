...
contract FrogInitPool is FrogTokenWrapper, IRewardDistributionRecipient {
    ...
    uint256 public/* constant*/ startTime = 1999652800; //utc+8 2020-09-09 20:00:00
    ...
    mapping(address => uint256) public claimedRewards; // claimed rewards
    ...
    function getReward() internal{
        ...
        uint256 userClaimedReward = claimedRewards[msg.sender];
        if (reward > 0 && claimedReward.add(reward) <= maxReward) {
            rewards[msg.sender] = 0;
            claimedReward = claimedReward.add(reward);
            frog.mint(msg.sender, reward);
            if(reward >= 10) frog.mint(devPool, reward.div(10));
            claimedRewards[msg.sender] = userClaimedReward.add(reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    function withdraw(uint256 amount) public checkOpen checkStart checkEnd checkStakeToken checkEmergency{
        require(amount > 0, "FROG-POOL: Cannot withdraw 0");
        super.withdraw(amount);
        emit Withdrawn(msg.sender, amount);
    }
