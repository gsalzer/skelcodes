...
/**
  * @title Gama's InterestRateModel Interface
  * @author gama
  */
...
contract ComptrollerV3Storage is ComptrollerV2Storage {
    ...
    //uint public gamaRate;

    /// @notice The portion of gamaRate that each market currently receives
    //mapping(address => uint) public gamaSpeeds;

    /// @notice The GAMA market supply state for each market
    //mapping(address => GamaMarketState) public gamaSupplyState;

    /// @notice The GAMA market borrow state for each market
    //mapping(address => GamaMarketState) public gamaBorrowState;

    /// @notice The GAMA borrow index for each market for each supplier as of the last time they accrued GAMA
    //mapping(address => mapping(address => uint)) public gamaSupplierIndex;

    /// @notice The GAMA borrow index for each market for each borrower as of the last time they accrued GAMA
    //mapping(address => mapping(address => uint)) public gamaBorrowerIndex;

    /// @notice The GAMA accrued but not yet transferred to each user
    //mapping(address => uint) public gamaAccrued;
}

/**
 * @title Gama's Comptroller Contract
 * @author gama
 */
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    /// @notice Emitted when GAMA rate is changed
    //event NewGamaRate(uint oldCompRate, uint newCompRate);

    /// @notice Emitted when a new GAMA speed is calculated for a market
    //event GamaSpeedUpdated(GToken indexed gToken, uint newSpeed);

    /// @notice Emitted when GAMA is distributed to a supplier
   // event DistributedSupplierGama(GToken indexed gToken, address indexed supplier, uint compDelta, uint compSupplyIndex);

    /// @notice Emitted when GAMA is distributed to a borrower
    //event DistributedBorrowerGama(GToken indexed gToken, address indexed borrower, uint compDelta, uint compBorrowIndex);

    /// @notice The threshold above which the flywheel transfers GAMA, in wei
    // uint public constant gamaClaimThreshold = 0.001e18;

    /// @notice The initial GAMA index for a market
    // uint224 public constant gamaInitialIndex = 1e36;
    ...
    */
    ...
    function mintAllowed(address gToken, address minter, uint mintAmount) external returns (uint) {
        ...
        // updateGamaSupplyIndex(gToken);
        // distributeSupplierGama(gToken, minter, false);
    ...
    function redeemAllowed(address gToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        /*
        updateGamaSupplyIndex(gToken);
        distributeSupplierGama(gToken, redeemer, false);
        */
    ...
    function borrowAllowed(address gToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        updateGamaBorrowIndex(gToken, borrowIndex);
        distributeBorrowerGama(gToken, borrower, borrowIndex, false);
        */
    ...
    function seizeAllowed(
        ...
        // Keep the flywheel moving
        /*
        updateGamaSupplyIndex(gTokenCollateral);
        distributeSupplierGama(gTokenCollateral, borrower, false);
        distributeSupplierGama(gTokenCollateral, liquidator, false);
        */
    ...
    function transferAllowed(address gToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        /*
        updateGamaSupplyIndex(gToken);
        distributeSupplierGama(gToken, src, false);
        distributeSupplierGama(gToken, dst, false);
        */
...
*/
    ...
    /**
     * @notice Recalculate and update GAMA speeds for all GAMA markets
     */
    /*
    function refreshGamaSpeeds() public {
        require(msg.sender == tx.origin, "only externally owned accounts may refresh speeds");
        refreshGamaSpeedsInternal();
    }

    function refreshGamaSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            updateGamaSupplyIndex(address(gToken));
            updateGamaBorrowIndex(address(gToken), borrowIndex);
        }
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            GToken gToken = allMarkets_[i];
            if (markets[address(gToken)].isComped) {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            uint newSpeed = totalUtility.mantissa > 0 ? mul_(gamaRate, div_(utilities[i], totalUtility)) : 0;
            gamaSpeeds[address(gToken)] = newSpeed;
            emit GamaSpeedUpdated(gToken, newSpeed);
        }
    }
    */

    /**
     * @notice Accrue GAMA to the market by updating the supply index
     * @param gToken The market whose supply index to update
     */
    /*
    function updateGamaSupplyIndex(address gToken) internal {
        GamaMarketState storage supplyState = gamaSupplyState[gToken];
        uint supplySpeed = gamaSpeeds[gToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = GToken(gToken).totalSupply();
            uint gamaAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(gamaAccrued, supplyTokens) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: supplyState.index}), ratio);
            gamaSupplyState[gToken] = GamaMarketState({
    ...
    */
    ...
    /**
     * @notice Accrue GAMA to the market by updating the borrow index
     * @param gToken The market whose borrow index to update
     */
    /*
    function updateGamaBorrowIndex(address gToken, Exp memory marketBorrowIndex) internal {
        GamaMarketState storage borrowState = gamaBorrowState[gToken];
        uint borrowSpeed = gamaSpeeds[gToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(GToken(gToken).totalBorrows(), marketBorrowIndex);
            uint gamaAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(gamaAccrued, borrowAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: borrowState.index}), ratio);
            gamaBorrowState[gToken] = GamaMarketState({
            index: safe224(index.mantissa, "new index exceeds 224 bits"),
            block: safe32(blockNumber, "block number exceeds 32 bits")
            });
        } else if (deltaBlocks > 0) {
    ...
    /**
     * @notice Calculate GAMA accrued by a supplier and possibly transfer it to them
     * @param gToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute GAMA to
     */
    /*
    function distributeSupplierGama(address gToken, address supplier, bool distributeAll) internal {
        GamaMarketState storage supplyState = gamaSupplyState[gToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: gamaSupplierIndex[gToken][supplier]});
        gamaSupplierIndex[gToken][supplier] = supplyIndex.mantissa;

        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa > 0) {
            supplierIndex.mantissa = gamaInitialIndex;
        }

        Double memory deltaIndex = sub_(supplyIndex, supplierIndex);
        uint supplierTokens = GToken(gToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(gamaAccrued[supplier], supplierDelta);
        gamaAccrued[supplier] = transferGama(supplier, supplierAccrued, distributeAll ? 0 : gamaClaimThreshold);
        emit DistributedSupplierGama(GToken(gToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    */

    /**
     * @notice Calculate GAMA accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param gToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute GAMA to
     */
    /*
    function distributeBorrowerGama(address gToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        GamaMarketState storage borrowState = gamaBorrowState[gToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({mantissa: gamaBorrowerIndex[gToken][borrower]});
        gamaBorrowerIndex[gToken][borrower] = borrowIndex.mantissa;

        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint borrowerAmount = div_(GToken(gToken).borrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(gamaAccrued[borrower], borrowerDelta);
            gamaAccrued[borrower] = transferGama(borrower, borrowerAccrued, distributeAll ? 0 : gamaClaimThreshold);
            emit DistributedBorrowerGama(GToken(gToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
    */

    /**
     * @notice Transfer GAMA to the user, if they are above the threshold
     * @dev Note: If there is not enough GAMA, we do not perform the transfer all.
     * @param user The address of the user to transfer GAMA to
     * @param userAccrued The amount of GAMA to (possibly) transfer
     * @return The amount of GAMA which was NOT transferred to the user

    function transferGama(address user, uint userAccrued, uint threshold) internal returns (uint) {
        if (userAccrued >= threshold && userAccrued > 0) {
            Gama GAMA = Gama(getGamaAddress());
            uint gamaRemaining = GAMA.balanceOf(address(this));
            if (userAccrued <= gamaRemaining) {
                GAMA.transfer(user, userAccrued);
                return 0;
            }
        }
        return userAccrued;
    ...
    */
    ...
    /**
     * @notice Claim all the GAMA accrued by holder in all markets
     * @param holder The address to claim GAMA for
     */ /*
    function claimGama(address holder) public {
        return claimGama(holder, allMarkets);
    } */

    /**
     * @notice Claim all the GAMA accrued by holder in the specified markets
     * @param holder The address to claim GAMA for
     * @param gTokens The list of markets to claim GAMA in
     */  /*
    function claimGama(address holder, GToken[] memory gTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimGama(holders, gTokens, true, true);
    } */

    /**
     * @notice Claim all GAMA accrued by the holders
     * @param holders The addresses to claim GAMA for
     * @param gTokens The list of markets to claim GAMA in
     * @param borrowers Whether or not to claim GAMA earned by borrowing
     * @param suppliers Whether or not to claim GAMA earned by supplying
     */
    /*
    function claimGama(address[] memory holders, GToken[] memory gTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < gTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerGama(address(gToken), holders[j], borrowIndex, true);
                }
            }
            if (suppliers == true) {
                updateGamaSupplyIndex(address(gToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierGama(address(gToken), holders[j], true);
                }
            }
        }
    }
    */
    ...
    /*** Gama Distribution Admin ***/

    /**
     * @notice Set the amount of GAMA distributed per block
     * @param gamaRate_ The amount of GAMA wei per block to distribute
     */
    /*
    function _setGamaRate(uint gamaRate_) public {
        require(adminOrInitializing(), "only admin can change GAMA rate");

        uint oldRate = gamaRate;
        gamaRate = gamaRate_;
        emit NewGamaRate(oldRate, gamaRate_);
    ...
    */
    ...
    /**
     * @notice Add markets to gamaMarkets, allowing them to earn GAMA in the flywheel
     * @param gTokens The addresses of the markets to add
     */
    /*
    function _addGamaMarkets(address[] memory gTokens) public {
        require(adminOrInitializing(), "only admin can add GAMA market");

        for (uint i = 0; i < gTokens.length; i++) {
            _addGamaMarketInternal(gTokens[i]);
        }
    ...
    */
    ...
    /*
    function _addGamaMarketInternal(address gToken) internal {
        Market storage market = markets[gToken];
        require(market.isListed == true, "GAMA market is not listed");
        require(market.isComped == false, "GAMA market already added");

        market.isComped = true;
        emit MarketComped(GToken(gToken), true);

        if (gamaSupplyState[gToken].index == 0 && gamaSupplyState[gToken].block == 0) {
            gamaSupplyState[gToken] = GamaMarketState({
            index: gamaInitialIndex,
            block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
        }

        if (gamaBorrowState[gToken].index == 0 && gamaBorrowState[gToken].block == 0) {
            gamaBorrowState[gToken] = GamaMarketState({
            index: gamaInitialIndex,
            block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
    ...
    */
    ...
    /**
     * @notice Remove a market from gamaMarkets, preventing it from earning GAMA in the flywheel
     * @param gToken The address of the market to drop
     */
    /*
    function _dropGamaMarket(address gToken) public {
        require(msg.sender == admin, "only admin can drop GAMA market");

        Market storage market = markets[gToken];
        require(market.isComped == true, "market is not a GAMA market");

        market.isComped = false;
        emit MarketComped(GToken(gToken), false);
    ...
    */
    ...
    /**
     * @notice Return the address of the GAMA token
     * @return The address of GAMA
     */

/*
    function getGamaAddress() public view returns (address) {
        return 0xaC07E31A6778c3CA9A5523924300D1b828b698f6;
    }
*/
