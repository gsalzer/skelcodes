...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    event LPBounceTokenClaimed(address dst, uint value);
    ...
    uint256 public dayDuration = 7 days;

    function updateDay(uint256 dayUpdate) public onlyOwner {
        dayDuration = dayUpdate;
    }
    ...
    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        ...
        return contractStartTimestamp.add(dayDuration).sub(block.timestamp);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        // console.log("days since start is", contractStartTimestamp.add(2 days), "Time now is", block.timestamp);
        // console.log("liquidity generation ongoing", contractStartTimestamp.add(2 days) < block.timestamp);
        return contractStartTimestamp.add(dayDuration) > block.timestamp;
    }
    ...
    function emergencyDrain24hAfterLiquidityGenerationEventIsDone() public onlyOwner {
        require(contractStartTimestamp.add(dayDuration).add(1 days) < block.timestamp, "Liquidity generation grace period still ongoing"); // About 24h after liquidity generation happens        
    ...
    address public bounceTokenAddr;
    
    function setBounceTokenAddr(address addr) public onlyOwner{
        bounceTokenAddr = addr;
    }

    function claimBounceLPTokens() public {
        require(LPGenerationCompleted, "Event not over yet");
        uint256 bounceTokenAmount = IERC20(bounceTokenAddr).balanceOf(msg.sender);
        require(bounceTokenAmount > 0 , "Nothing to claim, move along");
        IUniswapV2Pair pair = IUniswapV2Pair(tokenUniswapPair);
        uint256 amountLPToTransfer = bounceTokenAmount.mul(LPperETHUnit).div(1e18);
        pair.transfer(msg.sender, amountLPToTransfer); // stored as 1e18x value for change
        IERC20(bounceTokenAddr).transferFrom(msg.sender,address(this),bounceTokenAmount);
        emit LPBounceTokenClaimed(msg.sender, amountLPToTransfer);
    }
