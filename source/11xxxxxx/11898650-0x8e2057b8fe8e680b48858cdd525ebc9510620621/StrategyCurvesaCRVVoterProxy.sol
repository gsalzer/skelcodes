...
contract StrategyCurvesaCRVVoterProxy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address public constant want = address(0x02d341CcB60fAaf662bC0554d13778015d1b285C);
    address public constant crv = address(0xD533a949740bb3306d119CC777fa900bA034cd52);

    address public constant curve = address(0xEB16Ae0052ed37f479f7fe63849198Df1765a733);
    address public constant gauge = address(0x462253b8F74B72304c145DB0e4Eebd326B22ca39);
    address public constant voter = address(0xF147b8125d2ef93FB6965Db97D6746952a133934);
    address public constant spool = address(0xA5407eAE9Ba41422680e2e00537571bcC53efBfD);
    ...
    function getName() external pure returns (string memory) {
        return "StrategyCurvesaCRVVoterProxy";
    }
    ...
    function harvest() public {
        ...
        if (_dai > 0) {
            IERC20(dai).safeApprove(spool, 0);
            IERC20(dai).safeApprove(spool, _dai);
            ICurveFi(spool).exchange(0, 3, _dai, 0);
        }
        uint256 _susd = IERC20(susd).balanceOf(address(this));
        if (_susd > 0) {
            IERC20(susd).safeApprove(curve, 0);
            IERC20(susd).safeApprove(curve, _susd);
            ICurveFi(curve).add_liquidity([0, _susd], 0, true);
        }
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
