...
contract GOF is ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    function initialize()
        ...
        initializer
    {
        __ERC20_init("GoForth", "GOF");
        _setupDecimals(uint8(DECIMALS));
        __Ownable_init();
        
        _totalSupply = 6000000 * 10**9 ;
        ...
        _presaleTimestamp = 1609866300;
        endSale = false;
        _presaleEth = 50 ether;
        _presaleRate = 3333;
    ...
    receive() external payable {
        ...
        require(now > 1609866300, "Presale has not started");
        require(_presaleEth >= msg.value, "Sold out");
        saleAmt[msg.sender] += msg.value;
        require(saleAmt[msg.sender] <=  2000000000000000000, "Max presale limit has been reached");
    ...
    function listToken() external onlyOwner() {
        ...
        uint256 tokenToAdd = 1333333 * 10**9;
        if(_presaleEth != 0) {
