...
contract PoolGetters is PoolState {
    ...
    function dollar() public view returns (address) {
        return _state.provider.dollar;
    }

    function univ2() public view returns (IERC20) {
        return IERC20(_state.provider.uniV2);
    }
    ...
    function totalRewarded() public view returns (uint256) {
        return IDollar(dollar()).balanceOf(address(this)).sub(totalClaimable());
    }
...
contract Pool is PoolSetters, Liquidity {
    ...
    constructor(address _uniV2, address _oracle, address _dollar, address _usdc) public { 
        _state.provider.uniV2 = _uniV2;
        _state.provider.oracle = _oracle;
        _state.provider.dollar = _dollar;
        _state.provider.usdc = _usdc;
        owner = msg.sender;
    }
