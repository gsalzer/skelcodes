...
contract ERC20 is Context, IERC20 {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(_totalSupply.add(amount) <= _cap);
...
contract KohimaTokenCore is ERC20("Kohima Token", "KHM"), Ownable {
    using SafeMath for uint256;
...
contract KohimaToken is KohimaTokenCore {
...
contract LPToKHM is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for KohimaToken;
    using SafeERC20 for IERC20;
    using Address for address;

    KohimaToken private khm;
    ...
    uint256 public devFundAmount = 100000 * 1e18;
    uint256 public devDistributeDuration = 1825 days;
    ...
    constructor(address _khm, address _lpToken) {
        khm = KohimaToken(_khm);
    ...
    function claim(uint amount) public shouldStarted {
        ...
        khm.safeTransfer(msg.sender, amount);
        distributeDevFund();
    ...
    function checkHalving() internal {
        if (block.timestamp >= halvingTime) {
            ...
            if(khm.totalSupply().add(initReward) <= khm.cap()){
                khm.mint(address(this), initReward);
            }
    ...
    function startFarming() external onlyOwner {
        ...
        if(khm.totalSupply().add(initReward) <= khm.cap()){
            khm.mint(address(this), mintAmount);
        }
    ...
    function distributeDevFund() internal {
        ...
        if(balanceOf(address(this)) >= fundAmount){
            khm.safeTransfer(devAddr, fundAmount);
        }
    ...
    function khmAddress() view public returns(address) {
        return address(khm);
    }
