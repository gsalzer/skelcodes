...
// Curve has good rates, no need to take into account slippage, just sell as much as possible
...
contract StabilizeStrategyBTCArbV4 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public percentTradeTrigger = 3000; // 3% change in value will trigger a trade
    uint256 public maxPercentSell = 100000; // 100% of the tokens are sold to the cheapest token
    uint256 public maxAmountSell = 10e18; // The maximum amount of tokens that can be sold at once, 10 BTC equivalent
    uint256 public percentDepositor = 50000; // 1000 = 1%, depositors earn 50% of all gains
    uint256 public percentExecutor = 30000; // 30000 = 30% of WETH goes to executor, 15% of total profit
    uint256 public percentStakers = 50000; // 50% of non-depositors WETH goes to stakers, can be changed
    uint256 public minTradeSplit = 1e17; // If the balance of a btc token (normalized) is less than or equal to this, it trades the entire balance
    ...
    function setupWithdrawTokens() internal {
        // Start with renBTC
        // Most liquid pair on Uniswap renBTC / ETH
        ...
        // wBTC
        // Most liquid pair on Uniswap wBTC / ETH
        ...
        // sBTC
        // Illiqiud on Uniswap, curve is best
    ...
    function simulateExchange(address _inputToken, address _outputToken, uint256 _amount) internal view returns (uint256) {
        if(_outputToken != wethAddress){
            // We will be using curve to swap
            CurvePool pool = CurvePool(curve3PoolAddress);
            int128 inputID = 0;
            int128 outputID = 0;
            for(uint256 i = 0; i < tokenList.length; i++){
                if(_inputToken == address(tokenList[i].token)){
                    inputID = tokenList[i].curveID;
                }
                if(_outputToken == address(tokenList[i].token)){
                    outputID = tokenList[i].curveID;
                }
            }
            if(inputID == outputID){return 0;} // Can't exchange for self
            _amount = pool.get_dy(inputID, outputID, _amount); // Returns amount of other token
            return _amount;            
        }else{
            // Selling for WETH
            UniswapRouter router = UniswapRouter(uniswapRouterAddress);
            address sbtc = address(tokenList[2].token);
            if(_inputToken == sbtc){
                // Map inputs for curve
                CurvePool pool = CurvePool(curve3PoolAddress);
                int128 inputID = 0;
                int128 outputID = 0;
                for(uint256 i = 0; i < tokenList.length; i++){
                ...
                // From sBTC to WETH, need to convert to wBTC first
                // By first going to wBTC
                _amount = pool.get_dy(inputID, tokenList[1].curveID, _amount); // Returns amount of wBTC
                address[] memory path = new address[](2);
                path[0] = address(tokenList[1].token); // wBTC address
                path[1] = wethAddress;
                ...
                // Can use Uniswap directly
                address[] memory path = new address[](2);
                path[0] = _inputToken;
                path[1] = wethAddress; // Tokens share WETH as main liquidity pair
                uint256[] memory estimates = router.getAmountsOut(_amount, path);
                _amount = estimates[estimates.length - 1]; // This is the amount of WETH returned
                return _amount; 
            }
    ...
    function getCheapestToken() internal view returns (uint256) {
        ...
        // We will estimate the return for trading 0.01 BTC
        // The higher the return, the lower the price of the other token
        uint256 targetID = 0; // Our target ID is renBTC first
        uint256 renBTCAmount = uint256(1).mul(10**tokenList[0].decimals).div(100); // We start with 0.01 Bitcoin
        uint256 highAmount = renBTCAmount;
        for(uint256 i = 1; i < tokenList.length; i++){
            uint256 estimate = simulateExchange(address(tokenList[0].token),address(tokenList[i].token),renBTCAmount);
    ...
    function exchange(address _inputToken, address _outputToken, uint256 _amount) internal {
        if(_outputToken != wethAddress){
            ...
            if(inputID == outputID){return;} // Can't exchange for self
            IERC20(_inputToken).safeApprove(curve3PoolAddress, 0);
            IERC20(_inputToken).safeApprove(curve3PoolAddress, _amount);
            pool.exchange(inputID, outputID, _amount, 1);
            return;           
        }else{
            // Selling for WETH
            UniswapRouter router = UniswapRouter(uniswapRouterAddress);
            ...
            if(_inputToken == sbtc){
                ...
                IERC20(_inputToken).safeApprove(curve3PoolAddress, 0);
                IERC20(_inputToken).safeApprove(curve3PoolAddress, _amount);
                uint256 _before = tokenList[1].token.balanceOf(address(this)); // get WBTC balance before
                pool.exchange(inputID, tokenList[1].curveID, _amount, 1); // Get WBTC from sBTC
                _amount = tokenList[1].token.balanceOf(address(this)).sub(_before); // This is the gain of wBTC
                address[] memory path = new address[](2);
                path[0] = address(tokenList[1].token); // wBTC address
                path[1] = wethAddress;
                tokenList[1].token.safeApprove(uniswapRouterAddress, 0);
                tokenList[1].token.safeApprove(uniswapRouterAddress, _amount);
                router.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60)); // Get WETH from wBTC
                return;
            }else{
                // Can use Uniswap directly
                address[] memory path = new address[](2);
    ...
    function checkAndSwapTokens(address _executor) internal {
        ...
        for(uint256 i = 0; i < length; i++){
            if(i != targetID){
                ...
                uint256 _normBal = tokenList[i].token.balanceOf(address(this)).mul(1e18).div(10**tokenList[i].decimals);
                uint256 _maxTradeTarget = maxAmountSell.mul(10**tokenList[i].decimals).div(1e18); // De-normalize and determine the maximum amount of tokens to sell at once
                ...
                if(_normBal <= minTradeSplit){
                    ...
                    sellBalance = tokenList[i].token.balanceOf(address(this)).mul(maxPercentSell).div(divisionFactor);
                }
    ...
    function finishChangePercentExecutor() external onlyGovernance timelockConditionsMet(10) {
