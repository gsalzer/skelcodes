...
contract BullyPresale is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public immutable HARDCAP = 500 ether;

    uint256 public TOKENS_PER_ETH = 16000;
    ...
    function _buyTokens(address _beneficiary, uint256 _amount) internal {
        ...
        require(contributions[_beneficiary].add(_amount) <= 10 ether, "individual cap is 10 eth");
    ...
    function addWhitelists(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            whitelists[_addresses[i]] = 5 ether;
        }
    }
    ...
    function finalize() external virtual onlyOwner {
        ...
        uint256 liquidityFraction = 5; // 20%

        uint256 liquidityETH = weiRaised.div(liquidityFraction);
        ...
        if (tokensToBurn > 0) {
            ...
            IBully(address(token)).burn(tokensToBurn);
        }

        IBully(address(token)).togglePause(false);
