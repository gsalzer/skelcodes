...
abstract contract StrategyBase {
    ...
    function _swapUniswap(
        ...
        if (_amount > 0){
            ...
            UniswapRouterV2(sushiRouter).swapExactTokensForTokens(_amount, 0, sushipath, address(this), now);
        }
    }
...
contract StrategyAlchemixALCX is StrategyBase {
    ...
    uint256 public constant stakingPoolId = 1;

    constructor(address _governance, address _strategist, address _controller, address _timelock) public
    StrategyBase(alcx, _governance, _strategist, _controller, _timelock)
    {
        IERC20(alcx).safeApprove(stakingPool, uint256(-1));
        IERC20(alcx).safeApprove(sushiRouter, uint256(-1));
        IERC20(weth).safeApprove(sushiRouter, uint256(-1));
    }

        // **** Views ****

    function getName() external override pure returns (string memory) {
        return "StrategyAlchemixALCXV1";
    }
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        _swapUniswap(alcx, weth, _lpAmount);
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _buybackLpAmount = _profit.mul(performanceFee).div(performanceMax);
        if (_profit > 0) {
