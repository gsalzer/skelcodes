    ...
    LnRewardCalculator
{
        ...
        returns (uint256)
    {
        ...
        uint256 stakingBalance = super.amountOf(account).add(oldStaking).add(extenStaking);
        return stakingBalance;
    }
    ...
    function _widthdrawFromExtensionStaking(address _addr, uint256 amount) internal {
        uint256 blockNb = block.number;
        if (blockNb > mEndBlock) {
            blockNb = mEndBlock;
        }

        uint256 extensionStakingAmount = super.amountOf(mStakingExtensionAddress);
        super._withdraw(blockNb, mStakingExtensionAddress, amount);
        // sub already withraw reward, then cal portion
        uint256 reward = super
            .rewardOf(mStakingExtensionAddress)
            .sub(
            ...
            .div(1e20);
        mWidthdrawRewardFromExtensionStaking = mWidthdrawRewardFromExtensionStaking.add(
            reward
        );
    ...
    function _cancelStaking(address user, uint256 amount) internal {
        ...
        if (newAmount >= amount) {
            ...
            if (mExtensionAmount > amount && amount > 0){
                _widthdrawFromExtensionStaking(user, amount);
                amount = 0;
            } else {
                 _widthdrawFromExtensionStaking(user, mExtensionAmount);
                amount = amount.sub(
                    mExtensionAmount,
                    "_cancelStaking amount sub overflow"
                );               
            ...
            if (mOldAmount > amount && amount > 0){
                _widthdrawFromOldStaking(user, amount);
                amount = 0;
            } else {
                  _widthdrawFromOldStaking(user, mOldAmount);
                amount = amount.sub(
                    mOldAmount,
                    "_cancelStaking amount sub overflow"
                );              
    ...
    function claim() public override whenNotPaused returns (bool) {
        ...
        uint256 iAmount = super.amountOf(msg.sender);
        _cancelStaking(msg.sender, iMyOldStaking.add(iAmount).add(iMyExtStaking));
        ...
        emit Claim(msg.sender, iReward, iMyOldStaking.add(iAmount).add(iMyExtStaking));
        return true;
    }
