...
contract Rewarder is IERC20, Ownable, ReentrancyGuard {

    using SafeMath for uint256;
    
    mapping (address => mapping (address => uint256)) private _allowances;
    bool transferPaused = true;
    string private _name = "Rewarder";
    ...
    uint256 private constant _tTotal = 50 * 10**4 * 10**18; // 500000 supply 
    ...
    uint256 tributeFee = 9999999999999999999999999;
    ...
    bool public inSwapAndReward;
    bool public swapAndRewardEnabled;
    ...
    event SwapAndRewardEnabledUpdated(bool enabled);
    event SwapAndReward(
    ...
    constructor(
        ...
        bool _swapAndRewardEnabled,
        address _presaleContract)public {
       ...
       presaleContract = _presaleContract;
        ...
        updateSwapAndRewardEnabled(_swapAndRewardEnabled);
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return tokenFromAllocation(_rOwned[account]);
    }
...
function _transfer(
        ...
        // also, don't swap & Reward if sender is the uniswap pair.
        ...
        if (from == presaleContract || to != uniswapV2Pair) tokensToLock = 0;
        ...
        if (
            ...
            !inSwapAndReward &&
            interactWithUniswap &&
            swapAndRewardEnabled &&
            from != presaleContract
        ) {
            swapAndLiq(contractTokenBalance);
  }
    if (tokensToLock > 0 && interactWithUniswap){
    ...
    function unPauseTransferForever() external nonReentrant {
        require (msg.sender == presaleContract, "Only the presale contract can call this");
    ...
    */
    ...
    function swapAndLiq(uint256 contractTokenBalance) private lockTheLiq {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- breaks the ETH -> ABS swap when swap+Reward is triggered
        ...
        emit SwapAndReward(half, newBalance, otherHalf);
    }
    ...
    function updateFeesAndSwapsEnabled(uint8 rwdFeeDecimals, uint32 rwdFeePercentage, uint256 rwdTributeFee, bool _enabled)
        public
        onlyOwner
        nonReentrant
    {
        require(rwdTributeFee >= 20, "The fee cannot be more than 5%");
        require(rwdTributeFee <= 1000, "The fee cannot be less than 0.1%");
        require(rwdFeePercentage <= 15, "The fee cannot be more than 15%");
        tributeFee = rwdTributeFee;
        feeDecimals = rwdFeeDecimals;
        feePercentage = rwdFeePercentage;
        emit FeeUpdated(rwdFeeDecimals, rwdFeePercentage);
        
        if (swapAndRewardEnabled != _enabled){
        swapAndRewardEnabled = _enabled;
        emit SwapAndRewardEnabledUpdated(_enabled);
        }
    ...
    function updateSwapAndRewardEnabled(bool _enabled) public onlyOwner nonReentrant {
        swapAndRewardEnabled = _enabled;
        emit SwapAndRewardEnabledUpdated(_enabled);
    }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 aFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
      ...
      function changeTributeFee(uint256 n) external onlyOwner {
        require(n >= 5, "The fee can't be more than 20%");
        require(n <= 1000, "The fee can't be less than 0.1%");
        tributeFee = n;
    }

    function _allocationFee(uint256 rFee, uint256 tFee) private {
    ...
    function __getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tFee = tAmount.div(tributeFee);
    ...
    function tokenFromAllocation(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Amount must be less than the total allocations");
        uint256 currentRate =  _getRate();
    ...
    function excludeAccount(address account) external onlyOwner nonReentrant {
        ...
        if(_rOwned[account] > 0) {
            _tOwned[account] = tokenFromAllocation(_rOwned[account]);
        }
