...
contract ClaimTokenSupplyPercentage is Ownable {
    ...
    function multiSend(address[] memory recipients) public onlyOwner {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            uint256 owed = pendingClaim(recipient);
            
            if(owed > 0 && _token.balanceOf(address(this)) >= owed) {
                
                uint256 owedSupplyPct = _owedSupplyPct[recipient];
                
                _token.transfer(recipient, owed);
                emit Claimed(recipient, owedSupplyPct, owed);
                
               _owedSupplyPct[recipient] = 0;
