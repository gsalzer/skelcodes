...
contract AaveResolver is AaveImportHelpers {
    ...
    function _borrowOne(AaveInterface aave, address token, uint amt, uint rateMode) private {
        aave.borrow(token, amt, rateMode, getReferralCode(), address(this));
    }
...
contract AaveImportResolver is AaveResolver {
    ...
    function importAave(address userAccount, address[] calldata tokens, bool convertStable) external payable {
        ...
        for (uint i = 0; i < _length; i++) {
            ...
            _aaveData.tokens[i] = _token;

            (_aaveData.atokens[i], ,) = aaveData.getReserveTokensAddresses(_token);
...
contract ConnectAaveV2Import is AaveImportResolver {
    string public name = "AaveV2-Import-v1.2";
}
