...
contract CardRedeemer is Ownable {
    ...
    function redeem(uint256 _packId) public onlyEOA {
        ...
        uint256[] memory _cardIdList = pack.cardIdList;

        uint256 cardIdWon = 0;
        uint256 cumul = 0;
        for (uint256 i = 0; i < _cardIdList.length; ++i) {
            uint256 cardId = _cardIdList[i];
            uint256 cardsLeft = ropeMaker.maxSupply(cardId).sub(ropeMaker.totalSupply(cardId));

            cumul = cumul.add(cardsLeft);
            if (rng < cumul) {
                cardIdWon = cardId;
                break;
            }
