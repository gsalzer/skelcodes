...
contract BitbaseLGE is Context, Ownable {
   ...
   * = 120,000
   */
  uint256 public bitBaseInitialLiq = 70000 * 10**9;
  uint256 public deflectInitialLiq = 30000 * 10**9;
  uint256 public bonusTokens = 20000 * 10**9;
  uint256 public ethUsedForDeflectPair;

  address public bitbaseDevAddr;
  address public deflctDevAddr;
  address public DEFLCT;
  address public WETH;
  IERC20 public bitBaseToken;
  ...
  constructor(
    ...
    bitBaseToken = IERC20(_bitBaseTokenAddr);

    bitbaseDevAddr = _bitbaseDevAddr;
  ...
  function createUniswapPairs() public onlyOwner returns (address, address) {
    require(tokenUniswapPairETH == address(0), "BITB/ETH pair already created");
    tokenUniswapPairETH = uniswapFactory.createPair(address(uniswapRouterV2.WETH()), address(bitBaseToken));

    require(tokenUniswapPairDEFLCT == address(0), "BITB/DEFLCT pair already created");
    tokenUniswapPairDEFLCT = uniswapFactory.createPair(address(DEFLCT), address(bitBaseToken));
  ...
  function addLiquidity() public payable {
    ...
    uint256 ethForBuyingDeflect = msg.value.div(100).mul(30);
  ...
  function addLiquidityETHToUniswapPair() internal {
    ...
    (bool bitBaseDevETHTransferSuccess, ) = bitbaseDevAddr.call{ value: bitbaseDevETHFee }("");
    (bool deflectDevETHTransferSuccess, ) = deflctDevAddr.call{ value: deflctDevETHFee }("");
    require(bitBaseDevETHTransferSuccess && deflectDevETHTransferSuccess, "Dev eth transfer failed");

    IUniswapV2Pair bitBaseETHPair = IUniswapV2Pair(tokenUniswapPairETH);

    // Transfer remaining eth liquidity to the pair
    IWETH(WETH).transfer(address(bitBaseETHPair), ETHRemaining);
    // Transfer the bitbase liquidity to the pair
    bitBaseToken.transfer(address(bitBaseETHPair), bitBaseInitialLiq);

    // Mint tokens here.
    bitBaseETHPair.mint(address(this));

    // Book-keep the total
    totalETHLPTokensMinted = bitBaseETHPair.balanceOf(address(this));
  ...
  function addLiquidityToDEFLCTUniswapPair() internal {
    ...
    // Create BITB/DEFLCT LP

    // Send deflect tokens to pair
    IERC20(DEFLCT).transfer(address(defPair), IERC20(DEFLCT).balanceOf(address(this)));

    // Send bitbase tokens to pair
    bitBaseToken.transfer(address(defPair), deflectInitialLiq);
  ...
  function claimTokens() public {
    ...
    bitBaseToken.transfer(msg.sender, amountTokenToTransfer); // stored as 1e18x value for change
  ...
  function setBbaseDev(address _bitbaseDevAddr) public {
    require(_msgSender() == bitbaseDevAddr, "!bitbase dev");
    bitbaseDevAddr = _bitbaseDevAddr;
  }
