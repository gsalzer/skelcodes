...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    uint256 public rewardsDuration = 10 weeks;
    ...
    function withdraw(uint256 amount) public nonReentrant {
        ...
        _lastRewardPaidTime[msg.sender] = Math.min(block.timestamp, periodFinish);
    ...
    function _getReward() private {
        ...
        _lastRewardPaidTime[msg.sender] = Math.min(block.timestamp, periodFinish);
        if (reward > 0) {
            uint256 twoPercent = reward.div(100).mul(2);
            uint256 transferAmount = twoPercent.mul(49);
            address treasury = PoorRug(rewardsTokenAddress).treasury();
            rewardsToken.safeTransfer(msg.sender, transferAmount);
            rewardsToken.safeTransfer(treasury, twoPercent);
    ...
    function clearDust() external onlyRewardsDistribution {
        require(block.timestamp > periodFinish);
        uint balance = rewardsToken.balanceOf(address(this));

        rewardsToken.safeTransfer(rewardsDistribution, balance);
        emit DustCleared();
    }
