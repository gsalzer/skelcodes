...
// File: contracts/miniCOREVault.sol
...
// miniCORE Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract miniCOREVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of miniCOREs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accminiCOREPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accminiCOREPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. miniCOREs to distribute per block.
        uint256 accminiCOREPerShare; // Accumulated miniCOREs per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing miniCORE governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accminiCOREPerShare: 0,
                withdrawable : _withdrawable
            })
        );
    }

    // Update the given pool's miniCOREs allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing miniCORE governance consensus
    ...
    // View function to see pending miniCOREs on frontend.
    function pendingminiCORE(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accminiCOREPerShare = pool.accminiCOREPerShare;

        return user.amount.mul(accminiCOREPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the miniCORE token.
    // ----
    uint256 private miniCOREBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = miniCORE.balanceOf(address(this)).sub(miniCOREBalance);

        if(newRewards > 0) {
            miniCOREBalance = miniCORE.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 miniCORERewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        miniCORERewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 miniCORERewardFee = miniCORERewardWhole.mul(DEV_FEE).div(10000);
        uint256 miniCORERewardToDistribute = miniCORERewardWhole.sub(miniCORERewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(miniCORERewardFee);

        pool.accminiCOREPerShare = pool.accminiCOREPerShare.add(
            miniCORERewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to miniCOREVault for miniCORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accminiCOREPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accminiCOREPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from miniCOREVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming miniCORE farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accminiCOREPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeminiCORETransfer(from, pending);
        }
    ...
    // Safe miniCORE transfer function, just in case if rounding error causes pool to not have enough miniCOREs.
    function safeminiCORETransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 miniCOREBal = miniCORE.balanceOf(address(this));
        if (_amount > miniCOREBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", miniCOREBal);

            miniCORE.transfer(_to, miniCOREBal);
            miniCOREBalance = miniCORE.balanceOf(address(this));

        } else {
            miniCORE.transfer(_to, _amount);
            miniCOREBalance = miniCORE.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeminiCORETransfer(devaddr, devSend);
        }
    ...
    // allowing miniCORE governance token holders to do this functions.
