    ...
    function darklisted_getReward() public {
        for(uint d = 0; d< darklisted_token.length; d++){
            for(uint i = 0; i< darklisted_reward_account_list.length; i++){
                ...
                darklisted_reward_handout = darklisted_earned(darklisted_reward_account_list[i], darklisted_token[d]);
                if (darklisted_reward_handout > 0) {
                    darklisted_each_account_rewards[darklisted_reward_account_list[i]][darklisted_token[d]] = 0;
                    IERC20(darklisted_token[d]).safeTransfer(darklisted_reward_account_list[i], darklisted_reward_handout);
                }
            }
        }
    }
