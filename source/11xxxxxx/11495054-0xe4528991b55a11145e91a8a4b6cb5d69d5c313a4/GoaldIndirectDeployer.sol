...
// File: contracts\GoaldIndirect.sol
...
contract GoaldIndirectDeployer {
    address constant PROXY_ADDRESS = 0x1c7e0b9a33b83c7bBCF068958887bE4292637F3c;

    address[] deployedGoalds;
    ...
    function deploy(
        ...
        require(latestProxy.getLatestDeployerAddress() == address(this), "Not latest deployer");

        // Create the goald.
        GoaldIndirect goald = new GoaldIndirect(address(latestProxy), msg.sender, collateralToken, paymentToken, fee, feeIntervalDays, finalizationIntervals);
        address goaldAddress = address(goald);
        deployedGoalds.push(goaldAddress);

        // Tell the proxy we created a goald.
        latestProxy.notifyGoaldCreated(msg.sender);
...
contract GoaldIndirect is ERC721 {
    /** @dev WETH is used as an intermediary within Uniswap for greater liquidity. */
    ...
    constructor(
        ...
        require(collateralToken != WETH_CONTRACT,   "COL: WETH not allowd");
        ...
        require(paymentToken    != WETH_CONTRACT,   "PAY: WETH not allowd");
        ...
        _uniPath           = [paymentToken, WETH_CONTRACT, collateralToken];
    ...
    function payFeeLateOnly(uint256 deadline) external {
        require(block.timestamp >= _nextFeeTimestamp, "Not late");
    ...
    function _payFee(uint256 amount, uint256 deadline) private {
        ...
        { // Scoped to prevent stack too deep error.
            ...
            if (currentFeeBalance >= fee) {
                // This will floor to at last 1 period so we also don't need to worry about underflowing.
    ...
    /**
     * @dev See {IERC721-safeTransferFrom}. We override so we can add a check to make sure it hasn't been finalized.
     */
    ...
    /**
     * @dev See {IERC721-transferFrom}. We override so we can add a check to make sure it hasn't been finalized.
     */
