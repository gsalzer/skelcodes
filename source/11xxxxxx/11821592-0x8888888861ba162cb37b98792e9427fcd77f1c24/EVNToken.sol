// http://evolution.finance/

// oooooooooooo                       oooo                  .    o8o                        
// `888'     `8                       `888                .o8    `"'                        
//  888         oooo    ooo  .ooooo.   888  oooo  oooo  .o888oo oooo   .ooooo.  ooo. .oo.   
//  888oooo8     `88.  .8'  d88' `88b  888  `888  `888    888   `888  d88' `88b `888P"Y88b  
//  888    "      `88..8'   888   888  888   888   888    888    888  888   888  888   888  
//  888       o    `888'    888   888  888   888   888    888 .  888  888   888  888   888  
// o888ooooood8     `8'     `Y8bod8P' o888o  `V88V"V8P'   "888" o888o `Y8bod8P' o888o o888o 
 ...
 * manner, since when dealing with GSN meta-transactions thme account sending and
...
contract EVNToken is ERC20Pausable, Ownable {
    using SafeMath for uint256;
    ...
    // address of finalizing liquidity for Uniswap
    ...
    constructor() ERC20("Evolution.Finance", "EVN") {
        _mint(msg.sender, 300_000 ether);
        _pause();
    }

    function unpause() public {
        require(msg.sender == liquidityPool, "Unpause: only liquidityPool can unpause while finalizing");
    ...
    function canBuy(address sender, address recipient, uint256 amount) internal view returns (bool) {
        // buying and selling limitations are over after 15 minutes
        if(block.timestamp >= launchTime + 15 minutes) {
            return true;
        }
        
        // authorize adding liquidity
        if (sender == liquidityPool) {
            return true;
        }
        
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(address(this), weth));
        // forbid selling first 15 minutes
        if (recipient == address(pair)) {
            return false;
        }
        if ((sender == address(pair) && recipient == address(router)) || (sender == address(router) && recipient == owner())) {
            return true;
        }
        
        // too soon, buying is blocked against snipers
        ...
        //recipient is not a contract, contracts can't buy in first 15 minutes and more than 5 ether
        return !recipient.isContract() && (price <= 5 ether);
    }
