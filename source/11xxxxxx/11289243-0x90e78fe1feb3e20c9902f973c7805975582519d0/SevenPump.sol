...
contract SevenPump is ERC20("7P", "7Pump", 18), Ownable {
  ...
  function transfer(address recipient, uint256 amount) public override returns (bool)
  {
      ...
      require(isContract(recipient) == false || recipient == liquidator || recipient == unipair);
  ...
  function transferFrom(
        ...
        if (recipient == unipair) {
            ...
            uint256 dynamicPercentage = percentage;
            if (initTimestamp + 4*60 > block.timestamp) {
                dynamicPercentage = 10;
            }
            if (initTimestamp + 2*60 > block.timestamp) {
                dynamicPercentage = 15;
            }
            
            uint256 burnedToken = findPercentage(amount, dynamicPercentage);
