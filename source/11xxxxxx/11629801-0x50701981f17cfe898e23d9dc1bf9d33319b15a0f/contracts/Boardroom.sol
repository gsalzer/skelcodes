...
contract ShareWrapper {
    ...
    function totalSupply() public view returns (uint256) {
        return share.tokenFromReflection(_totalSupply);
    }

    function balanceOf(address account) public view returns (uint256) {
        return share.tokenFromReflection(_balances[account]);
    }

    function stake(uint256 amount) public virtual {
        share.transferFrom(msg.sender, address(this), amount);

        uint256 rAmount = share.reflectionFromToken(amount, false);
...
contract Boardroom is ShareWrapper, ContractGuard, Operator {
    ...
    constructor(IERC20 _cash, IShare _share) public {
        cash = _cash;
        share = _share;
