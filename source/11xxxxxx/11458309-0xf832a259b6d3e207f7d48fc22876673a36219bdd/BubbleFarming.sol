...
contract BubbleFarming is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	function addPool(address _lptoken, address _token, address _tokenReward, uint256 _rewardBlock, uint256 _rewardRemains) external onlyOwner {
	...
	function _updateUsers(uint256 _pid) internal {
		...
		for(uint256 i = 0; i < pooldatas[_pid].length; i++){
			...
			if(_user != address(0)){
				userList[_pid][_user].pendingReward = pendingRewardsFromPool(_pid,msg.sender);
