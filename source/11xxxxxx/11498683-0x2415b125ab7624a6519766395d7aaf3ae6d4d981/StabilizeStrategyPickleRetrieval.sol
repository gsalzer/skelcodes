...
// File: contracts/strategies/StabilizeStrategyPickleRetrieval.sol

pragma solidity ^0.6.6;

// This strategy recovers pickle from the token vault
contract StabilizeStrategyPickleRetrieval is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;
    
    address public zsTokenAddress; // The address of the controlling zs-Token
    address constant mainRewardToken = address(0x429881672B9AE42b8EbA0E26cD9C73711b891Ca5); // Pickle
    ...
    function setupRewardTokens() internal {
        // Reward tokens
        rewardTokenList.push(mainRewardToken); // Pickle token
    }
    ...
    function balance() public view returns (uint256) {
        return IERC20(mainRewardToken).balanceOf(address(this));
    }
    
    function pricePerToken() external pure returns (uint256) {
        return 1e18;
    }
    ...
    function enter() external onlyZSToken {
        // Send this token to governance
        IERC20(mainRewardToken).safeTransfer(governance(), balance());
    }
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total) public onlyZSToken returns (uint256) {
        require(balance() > 0, "There are no tokens in this strategy");
        return 0; // The withdraw will fail
    }
