...
contract CaesarTokenWrapper {
    ...
    function stake(uint256 amount) public {
        ...
        caesar.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        caesar.safeTransfer(msg.sender, amount);
    }
}


contract CaesarPool is CaesarTokenWrapper, IRewardDistributionRecipient {

    uint256 public constant DURATION = 7 days;                            
    uint256 public constant FROZEN_STAKING_TIME = 24 hours;

    uint256 public constant startTime = 1603200602; 
    ...
    function earned(address account) public view returns (uint256) {
        uint256 calculatedEarned = 
            balanceOf(account)
                ...
                .div(_gunit)
                .add(rewards[account]);
        uint256 poolBalance = caesar.balanceOf(address(this));
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "Caesar:CAESAR POOL - Cannot stake 0");
        super.stake(amount);
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"Caesar:CAESAR POOL - not start");
        _;
    }

    modifier checkOpen() {
        require(open, "Caesar:CAESAR POOL - Pool is closed");
        _;
    }
