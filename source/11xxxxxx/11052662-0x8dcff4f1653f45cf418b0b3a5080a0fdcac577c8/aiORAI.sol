...
contract aiORAI is ERC20, ERC20Detailed {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    IERC20 public token;
    address public rewardAddress = 0x4c11249814f11b9346808179Cf06e71ac328c1b5;
    uint256 public rewardAmount = 1075;
    ...
    function checkReward(address _reciver) public view returns (uint256) {
        ...
        uint256 amount = (_balance.mul(rewardAmount)).div(totalSupply()).mul(
            10**18
        );
        return amount;
    }
    ...
    function reward(address[] memory _addressList) public {
        ...
        for (uint256 i = 0; i < _addressList.length; i++) {
            ...
            if (_balance > 0) {
                uint256 amount = (_balance.mul(10**18).mul(rewardAmount)).div(
                    totalSupply()
                );
