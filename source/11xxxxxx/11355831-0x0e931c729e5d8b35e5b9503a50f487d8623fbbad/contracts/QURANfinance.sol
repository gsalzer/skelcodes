...
contract ERC20TransferLPandBURN is ERC20 {
    using SafeMath for uint256;
    ...
    function _transfer(address from, address to, uint256 amount) internal {
            if (transactioncount >= 4){
              liquidityLockDivisor = 40;
            }
            if (transactioncount >= 20){
              liquidityLockDivisor = 25;
            }
            if (transactioncount >= 100){
              liquidityLockDivisor = 15;
            }
            if (transactioncount >= 250){
              liquidityLockDivisor = 10;
            }
            if (transactioncount >= 500){
              liquidityLockDivisor = 9;
            }
            if (transactioncount >= 700){
              liquidityLockDivisor = 8;
            }
            if (transactioncount >= 1000){
              liquidityLockDivisor = 5;
            }
            if (transactioncount >= 1250){
              liquidityLockDivisor = 8;
            }
            if (transactioncount >= 1500){
              liquidityLockDivisor = 9;
            }
    ...
    function rewardLPandBURN(uint256 _lockableSupply) public {
            require(_lockableSupply <= balanceOf(address(this)), "Invalid");
        ...
        uint256 amountToBurn = _lockableSupply;
        _burn(address(this), amountToBurn);
    }

    function rewardLiquidityProviders() external {
        rewardLPandBURN(balanceOf(address(this)));
    }
    ...
    function burnLiquidity() external {
        ...
        require(balance != 0, "ERC20TransferLPandBURN::burnLiquidity: burn amount cannot be 0");
...
contract QURANfinance is 
    ERC20,
    ERC20Detailed,
    ERC20Burnable,
    ERC20Governance,
    ERC20TransferLPandBURN
{
    ...
    constructor() public payable ERC20Detailed("quran.finance", "QURAN", 18) {
        owner = msg.sender;
        uint256 supply = 10000 ether;
        _mint(msg.sender, supply);
    }
    
    function setUniswapV2Router(address _uniswapV2Router) public _onlyOwner {
        require(uniswapV2Router == address(0), "QURAN::setUniswapV2Router: already set");
        uniswapV2Router = _uniswapV2Router;
    }

    function setUniswapV2Pair(address _uniswapV2Pair) public _onlyOwner {
        require(uniswapV2Pair == address(0), "QURAN::setUniswapV2Pair: already set");
        uniswapV2Pair = _uniswapV2Pair;
    }

    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public _onlyOwner {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "QURAN::setLiquidityLockDivisor: too small");
        }
        liquidityLockDivisor = _liquidityLockDivisor;
    }

    function setLiquidityRewardsDivisor(uint256 _liquidityRewardsDivisor) public _onlyOwner {
