...
contract PiranhasAttack is Ownable {
    using SafeMath for uint256;
    ...
    // game parameters
    uint256 public KillerLockDays = 7;
    uint256 public KilledLockDays = 14;
    uint256 public divider = 100;
    uint256 public KillerRewardPercentage = 1;
    ...
    event AttackPiranhas(address indexed killer, address indexed killed, uint256 amount, uint256 reward);

    constructor (PiranhasToken _pir, PiranhasGame _gameContract) public {
        pir = _pir;
    ...
    function getDivider() public view returns (uint256){
        return divider.mul(10 ** 18);
    }

    function AttackPir(address killed, uint256 amount) public {
        address killer = msg.sender;

        require(amount > 0, "PiranhasGame: Amount must be bigger than zero");
        require(!gameContract.IsInWhiteList(killed), "PiranhasGame: Attack on this address forbidden");
        require(killer != killed, "PiranhasGame: Can't attack yourself");

        // check balances
        require(pir.balanceOf(killer) >= amount, "PiranhasGame: Killer balance is insufficient");
        require(pir.balanceOf(killed) >= amount, "PiranhasGame: Killed balance is insufficient");

        uint256 killerUnlockDate = block.timestamp.add(OneDay.mul(KillerLockDays.add(amount.div(getDivider()))));
        uint256 killedUnlockDate = block.timestamp.add(OneDay.mul(KilledLockDays.add(amount.div(getDivider()))));
        
    
        uint256 KillerReward = amount.div(100).mul(KillerRewardPercentage);
        
        gameContract.AddNewAttack(killer, amount, KillerReward, killerUnlockDate, true);
        gameContract.AddNewAttack(killed, amount, 0, killedUnlockDate, false);
        emit AttackPiranhas(msg.sender, killed, amount, KillerReward);
    }


    function changeRules(uint _killerDays, uint _killedDays, uint _divider, uint256 _reward) public onlyOwner {
        KillerLockDays = _killerDays;
        KilledLockDays = _killedDays;
        divider = _divider;
        KillerRewardPercentage = _reward;
    }
