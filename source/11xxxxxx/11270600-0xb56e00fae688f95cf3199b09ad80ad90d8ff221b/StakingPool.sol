...
contract DoublePool is StakingPool {
    ...
    function rewardDelta2() public view returns (uint amt) {
        if(lastUpdateTime2 >= now)
            return 0;
            
        amt = stakingPool2.earned(address(this)).sub(stakingPool2.rewards(address(this)));
    }
    ...
    function rewardPerToken2() virtual public view returns (uint256) {
        ...
        return
            rewardPerTokenStored2.add(
                rewardDelta2().mul(1e18).div(_totalSupply)
            );
    }
