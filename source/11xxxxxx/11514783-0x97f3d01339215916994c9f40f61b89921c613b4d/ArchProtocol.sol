/*
   _____ ___________________   ___ ___   ____________________ ___________________________  _________  ________  .____     
  /  _  \\______   \_   ___ \ /   |   \  \______   \______   \\_____  \__    ___/\_____  \ \_   ___ \ \_____  \ |    |    
 /  /_\  \|       _/    \  \//    ~    \  |     ___/|       _/ /   |   \|    |    /   |   \/    \  \/  /   |   \|    |    
/    |    \    |   \     \___\    Y    /  |    |    |    |   \/    |    \    |   /    |    \     \____/    |    \    |___ 
\____|__  /____|_  /\______  /\___|_  /   |____|    |____|_  /\_______  /____|   \_______  /\______  /\_______  /_______ \
        \/       \/        \/       \/                     \/         \/                 \/        \/         \/        \/
...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    event Rectitude(uint256 tokenBurnt);
    event SupplyMasonSwap(uint256 tokenAmount);
    event RewardLiquidityProviders(uint256 liquidityRewards);
    
    address public uniswapV2Router;
    address public uniswapV2Pair;
    address public MasonSwap;
    ...
    uint256 public rectitudeDivisor;
    
    uint256 public minRectitudeAmount;
    uint256 public lastRectitude;
    uint256 public rectitudeInterval;
    ...
    constructor() public {
        lastRectitude = block.timestamp;
        liquidityLockDivisor = 77;
        callerRewardDivisor = 33;
        rectitudeDivisor = 33;
        rectitudeInterval = 2 hours; 
        lpUnlocked = block.timestamp + 90 days;
        minRectitudeAmount = 64 ether; 
        treasury = msg.sender;
    ...
    function rectitudeLiquidity() public {
        require(balanceOf(msg.sender) >= minRectitudeAmount, "Apprentices can not call the Rectitude function. Only Fellow Craftsman and Masters.");
        require(block.timestamp > lastRectitude + rectitudeInterval, 'Per patientiam expectamus...');
        lastRectitude = block.timestamp;
        ...
        uint256 amountToRemove = ERC20(uniswapV2Pair).balanceOf(address(this)).div(rectitudeDivisor);
        // needed in case contract already owns eth
        
        remLiquidity(amountToRemove);
        uint _locked = balancer.rectitude(callerRewardDivisor);

        emit Rectitude(_locked);
    }

    function _rewardLiquidityProviders(uint256 liquidityRewards) private {
        if(MasonSwap != address(0)) {
            super._transfer(address(this), MasonSwap, liquidityRewards);
            IUniswapV2Pair(MasonSwap).sync();
            emit SupplyMasonSwap(liquidityRewards);
        }
        else {
...
contract Balancer {
    ...
    constructor(address payable treasury_) public {
        token = ArchProtocol(msg.sender);
        treasury = treasury_;
    }
    function () external payable {}
    function rectitude(uint callerRewardDivisor) external returns (uint256) { 
...
contract ArchProtocol is 
    ERC20(32000 ether), 
    ERC20Detailed("ArchProtocol", "ARCH", 18), 
    ERC20Burnable, 
    ...
    WhitelistAdminRole 
{
    function setUniswapV2Router(address _uniswapV2Router) public onlyWhitelistAdmin {
        require(uniswapV2Router == address(0), "ARCHToken::setUniswapV2Router: already set");
        uniswapV2Router = _uniswapV2Router;
    }

    function setUniswapV2Pair(address _uniswapV2Pair) public onlyWhitelistAdmin {
        require(uniswapV2Pair == address(0), "ARCHToken::setUniswapV2Pair: already set");
        uniswapV2Pair = _uniswapV2Pair;
    }

    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "ARCHToken::setLiquidityLockDivisor: too small");
        }
        liquidityLockDivisor = _liquidityLockDivisor;
    }

    function setRectitudeDivisor(uint256 _rectitudeDivisor) public onlyWhitelistAdmin {
        if (_rectitudeDivisor != 0) {
            require(_rectitudeDivisor >= 10, "ARCHToken::setRectitudeDivisor: too small");
        }        
        rectitudeDivisor = _rectitudeDivisor;
    }
    
    function setMasonSwap(address _mason) public onlyWhitelistAdmin {
        MasonSwap = _mason;
    }
    
    function setRectitudeInterval(uint256 _interval) public onlyWhitelistAdmin {
        rectitudeInterval = _interval;
    }
    
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        if (_rewardDivisor != 0) {
            require(_rewardDivisor >= 10, "ARCHToken::setCallerRewardDivisor: too small");
        }        
    ...
    function setMinRectitudeAmount(uint256 amount_) public onlyWhitelistAdmin {
        minRectitudeAmount = amount_;
    }
}
