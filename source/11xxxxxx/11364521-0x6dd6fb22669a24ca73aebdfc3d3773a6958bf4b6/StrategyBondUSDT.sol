...
contract StrategyBondUSDT {
    ...
    address constant public weth = address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);          // Wrapped Ether (WETH) 
    ...
    constructor() public {
        ...
        controller = 0x03Cc018e952Ee7d0F4Ea3444785F0fA910cc73A6;    // 更新后

        getName = string(
            abi.encodePacked("CM:ABS.Bond.", 
                abi.encodePacked(IERC20(want).name(),".1"
                )
            ));
    ...
    function doApprove () public{
        IERC20(cmt).safeApprove(unirouter, 0);
        IERC20(cmt).safeApprove(unirouter, uint(-1));
    }
    ...
    function deposit() public {
        ...
        /*
        if (_want > 0) {
            IERC20(want).safeTransfer(fundAddress, _want);
        }*/
    }
