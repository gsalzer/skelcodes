...
contract TLAMB is StandardToken, DetailedERC20("Wrapped lamb main network", "tLAMB", 18),
    MintableToken, BurnableToken, PausableToken, OwnableContract {
    event TransferRemarks(address indexed from, address  to, uint256 amount,bytes32 lambdaaddress);
    event BurnRemarks(address indexed from, uint256 amount,bytes32 lambdaaddress);
  ...
  function userBurnRemarks(uint256 _value,bytes32 _remarks) public whenNotPaused returns (bool) {
     burn(_value);
     emit BurnRemarks(msg.sender, _value,_remarks);
  }
}
