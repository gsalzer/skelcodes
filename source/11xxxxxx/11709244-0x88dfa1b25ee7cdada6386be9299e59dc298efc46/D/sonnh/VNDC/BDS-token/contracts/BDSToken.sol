...
contract BDSToken is ERC20Detailed, ERC20, Ownable {
    address directors = 0xd235d90775eaC34BDbceD870C72bD2735ED306C1;
    address managers = 0xDbAc76646732000a6E3ce964CB322f901d5D749A;
    address advisors = 0x999DFC0Ce1F180dddf8214e12E6f2a461ce0F48f;
    address shareholders = 0x12b2580De6F8E2Db0550A25125D5799E3F6B8A0F;
    address funds = 0x3Ec321Ee8ab5bC1F374eA6AB749df3C17Da73536;
    address marketing = 0x0DBaD6899bCDD79B17F66067b2E6109205b0De51;
    address investors = 0x4326Dd1810CEd23820f39FE3D5CEc2Ccb88d3b86;
    
    uint256 timeline = 1643500800; // 2022-01-30T00:00:00Z;
    ...
    constructor() public ERC20Detailed("Big Digital Shares", "BDS", 8) {  

        totalTokens = 1000000000 * 10 ** uint256(decimals());
        _mint(owner(), totalTokens); // total supply fixed at 1 billion token

        transferWithLock(directors, 300000000 * 10 ** uint256(decimals()), timeline);
        transferWithLock(managers, 50000000 * 10 ** uint256(decimals()), timeline);
        transferWithLock(advisors, 100000000 * 10 ** uint256(decimals()), timeline);
        transferWithLock(shareholders, 100000000 * 10 ** uint256(decimals()), timeline);
        transfer(funds, 150000000 * 10 ** uint256(decimals()));
        transfer(marketing, 100000000 * 10 ** uint256(decimals()));
        transfer(investors, 200000000 * 10 ** uint256(decimals()));
    }
    ...
    function transferWithLock(address _receiver, uint256 _amount, uint256 _releaseDate) public returns (bool success) {
        require(msg.sender == directors || msg.sender == managers || msg.sender == advisors || msg.sender == shareholders || msg.sender == funds || msg.sender == marketing || msg.sender == investors || msg.sender == owner());
