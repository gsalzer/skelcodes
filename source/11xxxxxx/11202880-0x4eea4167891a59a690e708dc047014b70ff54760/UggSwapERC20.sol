...
contract UggSwapERC20 is ERC20('UGGSWAP', 'UGG'), IUggSwapERC20, Ownable {
    ...
    function setFactory(address _factory) external onlyOwner {
        require(_factory != address(0), "address can not be null");
        factory = _factory;
    }
    ...
    function collect(address to, uint256 value) external override {
        require(to != address(0), "address can not be null");
        require(msg.sender == factory, "must be called by factory");

        _mint(to, value);
    }

    function additional(address receiver, uint256 amount) external onlyOwner {
        require(receiver != address(0), "receiver cannot be null");
