...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0x76c1685B21bADFD709b015A6c7D68527328411f5;
...
contract Staker is Poolable{
    ...
    mapping (address => uint256) public reduced_time;
    mapping (address => uint256) public new_time;
    ...
    function randomHour() internal returns (uint) {
        uint randomnumber = uint(keccak256(abi.encodePacked(now, msg.sender, nonce))) % 86100;
        randomnumber = randomnumber + 7200;
        nonce++;
        return randomnumber;
    }


    function reduce_time() public payable returns (uint) {
        address gambler = msg.sender;
        LPTokenBalance[gambler];
        reduced_time[msg.sender] == 0;
        new_time[msg.sender] == 0;
        require(timePooled[msg.sender] >= 1 seconds);
        //require(LPTokenBalance[gambler] != 0);
        reduced_time[msg.sender] = timePooled[msg.sender];
        new_time[msg.sender] = reduced_time[msg.sender] - randomHour();
        if(new_time[msg.sender] > timePooled[msg.sender]){
            reduce_time();
        }
        else
        timePooled[msg.sender] = new_time[msg.sender];
        return timePooled[msg.sender];
        }
         
        function stake() public payable{
        address staker = msg.sender;
        require(creationTime + 10 hours  <= now, "It has not been 8 hours since contract creation yet");
        ...
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance/2 }(orbAddress, amountTokenDesired, 1, 1, address(this), INF);
        ...
        if (msg.value >= 0.01 ether){
            ...
            path[0] = WETHAddress;
            path[1] = orbAddress;
            uint[] memory amounts = Uniswap(UNIROUTER).swapExactETHForTokens{ value: address(this).balance*2/10 }(1, path, address(this), INF);
            IERC20(orbAddress).burn(amounts[1]);
        }
        sendValue(_POOLADDRESS, address(this).balance);
    ...
    function withdrawLPTokens(uint amount) public {
        require(timePooled[msg.sender] + 1 days <= now, "It has not been 24 hours since you staked yet");
