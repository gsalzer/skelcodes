...
// ZOM with Governance.
contract ZOMHealth is ERC20("zom.health", "ZOM"), Ownable {
   ...
   modifier onlyPayloadSize(uint256 size) {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (ZOM_Obsidian stake contract).
     ...
     */
     ...
     function transfer(address recipient, uint256 amount) public override onlyPayloadSize(2 * 32) returns (bool) {
     ...
     * - burn 5% and send rest
     */
    function transferFrom(address sender, address recipient, uint256 amount) public override onlyPayloadSize(3 * 32) returns (bool) {
        // reduce 5% from the amount 
        uint256 amounttoburn = _getamount(amount);
        // send rest amount to receiver
        uint256 amounttosend = amount.sub(amounttoburn);
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "ZOM::delegateBySig: signature expired");
...
interface IMigratorObsidian {
    // Perform LP token migration from legacy UniswapV2 to zomhealth.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // zomhealth must mint EXACTLY the same amount of zomhealth LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// ZOM_Obsidian is the master of Zom. He can make Zom and he is a fair guy.
//
...
contract ZOM_Obsidian is Ownable {
    using SafeMath for uint256;
    ...
    IMigratorObsidian public migrator;
    ...
    function setMigrator(IMigratorObsidian _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    // Deposit LP tokens to ZOM_Obsidian for ZOM allocation.
    ...
    // Withdraw LP tokens from ZOM_Obsidian.
    ...
    function safeZomTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > zomBal) {
            require(zom.transfer(_to, zomBal), "zom transfer failed");
        } else {
            require(zom.transfer(_to, _amount), "zom transfer failed");
        }
    }
