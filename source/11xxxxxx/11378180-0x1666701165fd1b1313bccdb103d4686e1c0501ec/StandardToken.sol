...
contract LockBalance is Manager {
    ...
    function deleteLockUserInfos(address _to, eLockType _lockType, uint256[] _startTime, uint256[] _endTime) onlyManagers public {
        require(_startTime.length == _endTime.length);
        require(_lockType != eLockType.None);
        for(uint256 i = 0; i < _startTime.length; i++){
            deleteLockUserInfo(_to, _lockType, _startTime[i], _endTime[i]);
        }
    }
...
contract StandardToken is Token, LockBalance {
    ...
    function transferWithtLockUser(eLockType _type, address _to, uint256 _amount, uint256[] _value, uint256[] _endTime) onlyManagers public {  
        require(_value.length > 0);
        require(_value.length == _value.length);
        require(_value.length == _endTime.length);
        ...
        for(uint256 i = 0; i < _value.length; i++){
            require(!isProtect(_to));
            tmpValue = tmpValue.add(_value[i]);
            setLockUser(_to, _type, _value[i], _endTime[i]);
        }
        require(tmpValue == _amount);
