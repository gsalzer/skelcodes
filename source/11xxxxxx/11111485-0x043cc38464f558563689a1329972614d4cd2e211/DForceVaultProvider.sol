...
// File: contracts/vaults/dforce/DForce.sol
...
// File: contracts/vaults/dforce/DForceStrategyBase.sol
...
contract DForceStrategyBase is VaultBase{
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function _deposit(uint256 _amount) internal {
        if (_amount > 0) {
            IERC20(token).safeApprove(dtoken, 0);
            IERC20(token).safeApprove(dtoken, _amount);
            dERC20(dtoken).mint(address(this), _amount);
        }

        uint256 _d = IERC20(dtoken).balanceOf(address(this));
        if (_d > 0) {
            IERC20(dtoken).safeApprove(lend, 0);
            IERC20(dtoken).safeApprove(lend, _d);
            dRewards(lend).stake(_d);
        }
    }
    ...
    function _withdrawAll() internal { 
        if(dRewards(lend).balanceOf(address(this)) == 0) return;
        dRewards(lend).exit();
        uint256 _d = IERC20(dtoken).balanceOf(address(this));
        if (_d > 0) {
    ...
    function _harvest() internal {
        dRewards(lend).getReward();
    }

    function _earned() view internal returns(uint256){
        return dRewards(lend).earned(address(this));
    }
}

// File: contracts/vaults/dforce/DForceVaultProvider.sol
...
contract DForceVaultProvider is VaultProvider, DForceStrategyBase {
    
    event Withdrew(uint256 amount,uint256 realAmount);

    constructor(address _token, address _dtoken, address _profit, address _lend, address _controller) public{
        token = _token;
        dtoken = _dtoken;
        profit = _profit;
        lend = _lend;
    ...
    function withdraw(uint256 _amount) override onlyController external {
        uint256 amount = _amount.mul(101).div(100);
        uint256 realAmount = _withdraw(amount);
        emit Withdrew(amount, realAmount);
    ...
    function harvest() override onlyController external returns (uint256 amount){
        ...
        if (amount > 0)
            IERC20(profit).safeTransfer(controller, amount);
    }
