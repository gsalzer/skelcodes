...
contract Ownable {
    ...
    modifier onlyPendingOwner() {
        require(
            ...
            "msg.sender should be pending owner"
        );
        _;
    }
    /**
    ...
    */
    function setOwner(address _newOwner) internal {
        require(_newOwner != address(0x0), "Owner should not be zero address");
        _owner = _newOwner;
    }
    /**
...
contract Refundable is Ownable {
    ...
    function refundETH(address payable payee, uint256 amount) public onlyOwner {
        require(payee != address(0), "Can not refund to address 0x0");
    ...
    function refundERC20All(address tokenContract, address payee)
        public
        onlyOwner
    {
...
contract TokenBBTestToken is Refundable, ERC20Mintable, ERC20Burnable {
    using SafeMath for uint256;
    uint256 private _faucetFlow;
    uint256 private _exchangeRate;
    constructor() public ERC20("TokenBB.com Test Token", "TBBTT") {
        _faucetFlow = 100 * (10**18);
        _exchangeRate = 100000;
        mint(address(this), 100000000 * 10**18);
    }
    receive() external payable {
        faucet(msg.sender);
    }
    function faucet(address _payee) public payable {
        uint256 amount = msg.value.mul(_exchangeRate).add(_faucetFlow);
        this.transfer(_payee, amount);
    }
    function faucetFlow() public view returns (uint256) {
        return _faucetFlow;
    }
    function exchangeRate() public view returns (uint256) {
        return _exchangeRate;
    }
    function setFaucetFlow(uint256 amount) public onlyOwner {
        _faucetFlow = amount;
    }
