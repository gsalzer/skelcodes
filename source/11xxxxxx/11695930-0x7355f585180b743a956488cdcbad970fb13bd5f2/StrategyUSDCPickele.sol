...
contract StrategyUSDCPickele  {
    ...
    function redelivery() internal {
        ...
        if (reward > redeliverynum)
        {
		    ...
		    uint256 _2dms = reward.sub(_2token);  //20%
            _swapUniswap(pickletoken,want,_2token,unirouter);
            _swapUniswap(pickletoken,weth,_2dms,unirouter);
            _swapUniswap(weth,dms,IERC20(weth).balanceOf(address(this)),dmsrouter);
            IERC20(dms).safeTransfer(Controller(controller).rewards(), IERC20(dms).balanceOf(address(this)));
		}
        deposit();
    }
