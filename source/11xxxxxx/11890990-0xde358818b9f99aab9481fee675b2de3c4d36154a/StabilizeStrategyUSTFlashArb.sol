...
// File: contracts/strategies/StabilizeStrategyUSTFlashArb.sol
...
contract StabilizeStrategyUSTFlashArb is Ownable {
    ...
    function getCheaperToken() internal view returns (uint256, uint256) {
        ...
        // We will estimate the return for trading 1000 UST
        ...
        uint256 ustAmount = uint256(1000).mul(10**tokenList[0].decimals);
        ...
        // Now on Curve
        estimate = simulateExchange(address(tokenList[0].token),address(tokenList[1].token),ustAmount,false);
        estimate = estimate.mul(10**tokenList[0].decimals).div(10**tokenList[1].decimals);
        if(estimate > ustAmount){
            // This token is worth less than UST on curve
            targetID_2 = 1;
        }
        return (targetID_1, targetID_2);
    }
    ...
    function checkAndSwapTokens(address _executor) internal {
        ...
        (uint256 targetID_1, uint256 targetID_2) = getCheaperToken(); // Normally both these values should point to the same ID
        ...
        for(uint256 i = 0; i < length; i++){
            if(i != targetID){
                ...
                if(sellBalance > 0){
                    ...
                    uint256 estimate = simulateExchange(address(tokenList[i].token), address(tokenList[targetID].token), sellBalance, true);
                    uint256 estimate2 = simulateExchange(address(tokenList[i].token), address(tokenList[targetID].token), sellBalance, false);
        ...
        for(uint256 i = 0; i < length; i++){
            if(i != targetID){
                ...
                if(tokenList[i].token.balanceOf(address(this)) <= _minTradeTarget){
                    ...
                    sellBalance = estimateSellAtMaxSlippage(i, targetID, tokenList[i].token.balanceOf(address(this))); // This will select a balance with the max slippage
                }
                if(sellBalance > _maxTradeTarget){
                    // If greater than the maximum trade allowed, match it
                    sellBalance = _maxTradeTarget;
