...
// File: contracts/interfaces/IChainLinkFeed.sol
...
// File: contracts/interfaces/Keep3r/IKeep3rV1Mini.sol
...
// File: contracts/Keep3rV1HelperOld.sol
pragma solidity ^0.6.6;
...
contract Keep3rV1HelperLegacyV1Fixed is Ownable {
    using SafeMath for uint;

    IChainLinkFeed public constant FASTGAS = IChainLinkFeed(0x169E633A2D1E6c10dD91238Ba11c4A708dfEF37C);
    IKeep3rV1Mini public KP3R = IKeep3rV1Mini(0x0E3EF895c59E7Db27214AB5bbf56347cE115A3f4);

    uint constant public BOOST = 50;
    uint constant public BASE = 10;
    uint public TARGETBOND = 300e18;

    uint constant public PRICE = 10;
    
    uint256 public ReductionAmount = 0;
    uint256 public IncreaseAmount = 0;
    uint256 internal BASEC = 10000;
    ...
    function getQuoteLimitFor(address origin, uint gasUsed) public view returns (uint) {
        ...
        uint result =  Math.max(_min, _boost.mul(_bond).div(TARGETBOND));
        if(ReductionAmount > 0) result = result.sub(result.mul(ReductionAmount).div(BASEC));
        if(IncreaseAmount > 0) result = result.add(result.mul(IncreaseAmount).div(BASEC));
        return result;
    }
    ...
    function setReductionAmount(uint256 newRed) public onlyOwner {
        ReductionAmount = newRed;
    }

    function setIncreaseAmount(uint256 newInc) public onlyOwner {
        IncreaseAmount = newInc;
    }
    
    function setTargetBond(uint newTarget) public onlyOwner {
