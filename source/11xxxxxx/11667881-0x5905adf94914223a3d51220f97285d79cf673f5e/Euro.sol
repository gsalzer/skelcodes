...
/*
    ...
    limitations under the License.
*/
contract Euro is IEuro, MinterRole, ERC20Detailed, Permittable, ERC20Burnable {

    constructor()
    ERC20Detailed("Elastic Euro", "eEUR", 18)
    Permittable()
    public
    { }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if (allowance(sender, _msgSender()) != uint256(-1)) {
            _approve(
                sender,
                _msgSender(),
                allowance(sender, _msgSender()).sub(amount, "Euro: transfer amount exceeds allowance"));
        }
        return true;
    }
}
