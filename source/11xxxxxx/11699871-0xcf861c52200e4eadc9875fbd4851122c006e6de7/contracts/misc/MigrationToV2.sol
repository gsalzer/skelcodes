...
contract MigrationToV2 is IFlashLoanReceiver {
  ...
  address immutable LENDING_POOL_V1_CORE;
  ...
  function _payDebt(
    ...
    if (debtReserve == ETH) {
      ...
      IERC20(debtReserve).approve(address(LENDING_POOL_V1_CORE), amount);
    }
  ...
  function _migrateAToken(
    ...
    if (underlying == USDT) {
      IERC20(underlying).approve(address(LENDING_POOL), 0);
    }
    IERC20(underlying).approve(address(LENDING_POOL), underlyingBalance);
