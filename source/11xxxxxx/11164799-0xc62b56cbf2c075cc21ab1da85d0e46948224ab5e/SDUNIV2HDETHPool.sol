...
/**
 * SD-UNI-V2(SD-ETH) Pool
 */
contract SDUNIV2HDETHPool is SDTokenWrapper, IRewardDistributionRecipient {
    IERC20 public SD = IERC20(0x6ce654aC973D326F89f0685E7459542641410eD9);
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "SD-UNI-V2(SD-ETH)-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "SD-UNI-V2(SD-ETH)-POOL: Cannot withdraw 0");
    ...
    function getReward() public checkStart updateReward(msg.sender){
        ...
        if (reward > 0) {
            ...
            SD.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    modifier checkStart(){
        require(block.timestamp > startTime,"SD-UNI-V2(SD-ETH)-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "SD-UNI-V2(SD-ETH)-POOL: Pool is closed");
        _;
    }
