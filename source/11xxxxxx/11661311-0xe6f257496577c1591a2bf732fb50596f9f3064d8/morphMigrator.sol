...
contract morphMigrator is Ownable {
    ...
    IAsset public oldToken;
    IAsset public newToken;
    bool public enabled;

    event Migrate(IAsset oldToken, IAsset newToken, uint256 amount);

    constructor (IAsset _oldToken, IAsset _newToken, uint256 _limit) public {
        oldToken = _oldToken;
        newToken = _newToken;
    ...
    function migrate(uint256 _amount) public {
        require(enabled, 'OneMigrator: Not enabled');
        require(totalMigrated.add(_amount) <= migrationLimit, 'OneMigrator: Exceeds Cap');
        oldToken.burnFrom(msg.sender, _amount);
        newToken.mint(msg.sender, _amount);
        totalMigrated = totalMigrated.add(_amount);
        emit Migrate(oldToken, newToken, _amount);
    }
