...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        return Strings.strConcat(_uri, Strings.uint2str(_tokenId), ".json");
    }
    string private constant _contractURI = "https://kool.lol/api/nft.json";
...
/// @title Kool NFT Contract

contract KoolNFT is ERC1155, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() public ERC1155("https://kool.lol/api/token/") {
    ...
    event CardAdded(uint256[] ids, uint256[] maxSupply);

    function initiateCards(uint256[] calldata ids, uint256[] calldata maxSupply) external returns (uint256[] memory, uint256[] memory) {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not an admin");
        require(ids.length == maxSupply.length);
        uint256 length = ids.length;
        uint i;
        for (i=0; i < length; i++) {
            totalSupply[ids[i]] = maxSupply[i];
        }
        emit CardAdded(ids, maxSupply);
        return (ids, maxSupply);
    }

    function mint(address to, uint256 id, uint256 amount) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "Caller is not a minter");
        require(circulatingSupply[id].add(amount) <= totalSupply[id] || totalSupply[id] == 0, "Total supply reached.");
