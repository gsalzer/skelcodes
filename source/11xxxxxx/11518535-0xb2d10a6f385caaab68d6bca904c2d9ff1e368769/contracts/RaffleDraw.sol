...
contract RaffleDraw is Ownable {
    ...
    uint256 public participantIndex = 0;
    ...
    function buyTicket(address _referrer, uint _noOfTicket) external {
        ...
        uint256 index = participantsNum; // 0 
        uint256 newIndex = index + (noOfTicket-1); // no of ticket 10 new index = 0 + 9
        uint256 participantTotal = index + noOfTicket; // participant total = 10  0 + 10
        require( participantTotal <= maxParticipants, "No of Ticket Reached Limit"); // Max Participant = 10
        participantsNum = participantTotal;

        participants[game][participantIndex] = 
        Participant({
        ...
        emit Ticket(game, msg.sender, index,newIndex);
    ...
    function draw() public onlyOwner {
        ...
        address winner;
        for(uint i = 0 ; i <= participantIndex ; i++){
            Participant memory p  = participants[game][i];

            if(_placeParticipantInGame(p,winningNumber)){
                winner = p.participantAddress;
                break;
            }
    ...
    function _placeParticipantInGame(Participant memory _user, uint _winningNumber) internal pure returns(bool)  {
        if(_winningNumber >= _user.startIndex &&  _winningNumber <= _user.endIndex){
            return true;
        }else{
            return false;
        }
