...
//import "https://github.com/smartcontractkit/chainlink/blob/develop/evm-contracts/src/v0.6/VRFCoordinator.sol";
...
contract LottoChainLink is VRFConsumerBase {
    
    address payable owner;
    address ETHaddress = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    //address LINKaddress = 0xa36085F69e2889c224210F603D836748e7dC0088; //kovan
    address LINKaddress = 0x514910771AF9Ca656af840dff83E8264EcF986CA;
    
    //address VRFCoordinatorAddress = 0xdD3782915140c8f3b190B5D67eAc6dc5760C46E9; //kovan
    address VRFCoordinatorAddress = 0xf0d54349aDdcf704F77AE15b96510dEA15cb7952; //mainnet
    
    address[] contendersList;
    uint totalRound = 0;
    uint constant LOTOPRICE = 1000000000000000000;
    uint constant LOTOFEE = 100; //1%
    uint constant LOTORUNTIME = 86400; //1 days (or 24 hours)
    //uint constant LOTORUNTIME = 3600; //1 hour for kovan testing
    //uint constant LOTORUNTIME = 600; //10 minutes for kovan
    uint constant MINCONTS = 10;
    
    IERC20 LinkToken;
    
    //bytes32 internal keyHash=0x6c3699283bda56ad74f6b855546325b68d482e983852a7a82979cc4807b641f4; //kovan
    bytes32 internal keyHash=0xAA77729D3466CA35AE8D28B3BBAC7CC36A5031EFDC430821C02BC31A238AF445; //mainnet
    uint256 internal LINKfee;
    
    uint256 randomResult=0;
    uint256 public lastRandomResult=0;
    
    
    uint256 public randomWinnerNumber=0;
    uint256 public roundAmount=0;
    uint256 public roundContenders=0;
    address roundWinnerAddress;
    bytes32 lastRequestId;
    address public lastRoundWinner;
    uint256 public lastRoundContenders=0;
    ...
    constructor() 
        ...
        LINKfee = 2 * 10 ** 18; // 2 LINK
        owner = msg.sender;
        LinkToken = IERC20(LINKaddress);
    }
    
    function approveDepositLINK() public {
        require(LinkToken.balanceOf(msg.sender) >= LOTOPRICE, "Not enough LINK to approve the deposit");
        LinkToken.approve(msg.sender, LOTOPRICE);
    }
    
    function depositLINK() public {
        for (uint i; i< contendersList.length;i++){
            if (contendersList[i] == msg.sender) {
                revert('You can only deposit 1 LINK');
            }
        }
        
        LinkToken.transferFrom(msg.sender, address(this), LOTOPRICE);
        contendersList.push(msg.sender);
        totalRound += LOTOPRICE;
        roundAmount += LOTOPRICE;
        roundContenders = roundContenders+1;
        
        //if (getRoundFee() >= LINKfee) {
        if (roundContenders >= MINCONTS) { //start the competition when we reach MINCONTS people
            getRandomNumber(random());
        }
    }
   
    function random() private view returns(uint){
        return uint256(keccak256(abi.encodePacked(block.difficulty, now, block.timestamp, block.number)));
    }
    ...
    function checkCompetitionStatus() public view returns(bool, uint256) {
        //need to check if
        if (competitionRunning) {
            if (block.timestamp >= endingDate) {
                return (false, endingDate);
            }
            return (true, endingDate);
        } else {
            return (false, endingDate);
        }
    }
    function callFinishCompetition() public returns(bool) {
        //need to check if
        if (competitionRunning) {
            if (block.timestamp >= endingDate) {
                //Round is over!
    ...
    function startCompetition() private {
        //set end date to now + LOTORUNTIME
        endingDate = block.timestamp+LOTORUNTIME;
    ...
    function finishCompetition() private {
        //need to start the competition.. based on the random number we got from chainlink
        //we will decide of a winnder from the balances. winner will get all the LINK's in
        //the contract minus fee's sent to owner
        
        randomWinnerNumber = randomResult % (roundContenders+1);
        roundWinnerAddress = contendersList[randomWinnerNumber];
        
        uint256 roundFeeContract = getRoundFee(); //Contract Fee for making the competition
        uint256 roundFeeOwner = getRoundFee(); //Owner fee
        uint256 roundWinnings = roundAmount.sub(roundFeeOwner).sub(roundFeeContract);
        
        LinkToken.approve(address(this), LinkToken.balanceOf(address(this)));
        LinkToken.transferFrom(address(this), owner, roundFeeContract);
        LinkToken.transferFrom(address(this), roundWinnerAddress, roundWinnings);
        
        resetCompetition();
    }
    function getRoundFee() private view returns(uint) {
        return roundAmount.div(LOTOFEE);
    }
    
    function resetCompetition() private {
        lastRandomResult = randomResult; //make sure we dont run again with same random
        lastRoundWinner = roundWinnerAddress; //save last round winner
        lastRoundContenders = roundContenders; //save number of contenders
        
        randomWinnerNumber = 0;
        roundAmount = 0;
        roundContenders = 0;
        competitionRunning = false;
        delete contendersList; //remove all contenders
        delete roundWinnerAddress; //remove last winner
        checkLeftoversAndReturnToOwner();
    }
    
    function checkLeftoversAndReturnToOwner() private {
        if (LinkToken.balanceOf(address(this)) > 0) {
            LinkToken.transferFrom(address(this), owner, LinkToken.balanceOf(address(this)));
        }
    }
    
    //take care of getting ETH or other tokens
