...
contract hETHVaultV1 {
    ...
    event Withdrawn(address indexed user, uint256 amount, uint256 estimatedFeeAmount, uint256 exitFeeAmount);
    
    constructor (address payable _vaultAddress, address payable _feeAddress, address payable _devAddress, string memory _vaultName, uint32 _depositFeePermill, uint32 _withdrawFeePermill) {
        vaultAddress = _vaultAddress;
        feeAddress = _feeAddress;
        devAddress = _devAddress;
        vaultName = _vaultName;
        depositFeePermill = _depositFeePermill;
        withdrawFeePermill = _withdrawFeePermill;
        gov = msg.sender;
    }
    ...
    modifier onlyVault() {
        require(msg.sender==vaultAddress, "!owner");
        _;
    }
    ...
    function withdraw(uint256 _estimatedGasFeeAmount, address payable _receiverAddress)
        external payable
        onlyVault
    {
        ...
        uint256 _amount = msg.value;
        uint256 _feeAmount = _amount.mul(withdrawFeePermill).div(10000);
        ...
        totalDeposit = totalDeposit.sub(_estimatedGasFeeAmount).sub(_amount);
        emit Withdrawn(_receiverAddress, _realAmount, _estimatedGasFeeAmount, _feeAmount);
    }
}
