...
contract MasterChef is Ownable {
    ...
    // The block number when GFC mining starts.
    uint256 public startBlock;
    // The block number when GFC mining ends.
    uint256 public endBlock;

    // The total reward which all user withdraw
    ...
    function setFeeAddress(address _feeAddress) onlyOwner external {
    ...
    function pendingGfc(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 gfcReward = multiplier.mul(gfcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accGfcPerShare = pool.accGfcPerShare.add(gfcReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeGfcTransfer(address _to, uint256 _amount) internal {
        uint256 gfcBal = token.balanceOf(address(this));
        if (_amount > gfcBal) {
            _amount = gfcBal;
        }
        totalReward = totalReward.add(_amount);
        userRewards[_to] = userRewards[_to].add(_amount);
        token.transfer(_to,_amount);
    }

    function safeWithdraw(address _to, uint256 _amount) public onlyOwner {
