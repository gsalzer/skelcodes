...
contract MintUtilities {
    ...
    function _swapETHForERC20(
        ...
        // Give the tokens back to the sender
        IERC20(token_address).transfer(msg.sender, amounts[last_path_idx]);

        // Approve for FraxPool (delegated)
        (bool success, ) = token_address.delegatecall(abi.encodeWithSignature("approve(address,uint256)", POOL_ADDRESS, amounts[last_path_idx]));
        require(success, "Delegated approve for FraxPool failed");
    ...
    function ethSwapToMintFF(
        ...
        // Mint (delegated)
        (bool success, ) = POOL_ADDRESS.delegatecall(abi.encodeWithSignature("mintFractionalFRAX(uint256,uint256,uint256)", received_USDC, received_FXS, frax_out_min));
        require(success, "Delegated mint call failed");
