...
Created and tested by the Seed team!
...
contract DIBS2 is IERC20 {
    ...
    uint256 private _totalSupply = 72 ether;

    string private _name = "Dibsv2 Token";
    string private _symbol = "Dibsv2";
    ...
    uint256 private _minimum_balance_amount = 5 ether;
    // uint256 public amount_to_sell_limit = 200;
    // bool public begin_stop_selling = true;
   ...
   function reward_holder(address holder, address sender, uint256 _reward_per_holder) private{
    ...
    function findOnePercent(uint256 value) public pure returns (uint256)  {
        ...
        uint256 onePercent = roundValue.mul(1000).div(10000); // burn 10
        return onePercent;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (stopBots) {
            if (amount > 2 ether && sender != __owner) {
                revert('stop the bots!');
            }
        }
        
        _beforeTokenTransfer(sender, recipient, amount);
        uint256 to_burn = amount.div(100).mul(2);
        uint256 to_everyone = amount.div(100).mul(5);
