/**
 *Submitted for verification at Etherscan.io on 2021-1-10
*/
...
contract OSY is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Osyraa";
    string private _symbol = "OSY";
    ...
    //@dev The tax fee contains two decimal places so 400 = 4.0%
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "OSY: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'OSY: The contract it self cannot be excluded');
        require(!_isExcluded[account], "OSY: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "OSY: Account is already included");
    ...
    function _transfer(
        ...
        //@dev Limit the transfer to 4500 tokens for first 5 minutes
        require(now > liquidityAddedAt + 6 minutes  || amount <= 4500e9, "You cannot transfer more than 4500 tokens.");
