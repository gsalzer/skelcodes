...
contract Bot is Ownable {
    ...
    event WithdrawFund(address indexed withdrawer, address indexed receiver, uint amount);
    event WithdrawReward(address indexed receiver, uint amount);
    ...
    constructor(address _weth) public Ownable() {
        weth = _weth;
    }
    ...
    function withdrawFund(address payable receiver, uint amount) external payable {
        require (funders[msg.sender], "not funder");
        payable(receiver).transfer(amount);
        funds = funds.sub(amount);
        emit WithdrawFund(msg.sender, receiver, amount);
    }

    function withdrawReward(address payable receiver) external payable onlyOwner {
        uint amount = address(this).balance.sub(funds);
        require (amount > 0, "no reward");
        payable(receiver).transfer(amount);
        emit WithdrawReward(receiver, amount);
    }

    function setFunder(address _funder, bool status) external onlyOwner {
        funders[_funder] = status;
    }

    function ethToToken(uint ethAmount, address token, uint minAmount, uint deadline) external returns (uint) {
    ...
    function approveToken(address token) external onlyOwner {
        IERC20(token).approve(address(UNI_V2_ROUTER02), uint256(-1));
    }

    function tokenToEth(address token, uint deadline) external returns (uint) {
        address pair = UNI_V2_FACTORY.getPair(weth, token);
        require (pair != address(0), "No pair exist");
        address[] memory path = new address[](2);
