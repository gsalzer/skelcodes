...
// decore Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract decoreVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of decores
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accdecorePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accdecorePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accdecorePerShare; // Accumulated decores per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing decore governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's decores allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing decore governance consensus
    ...
    uint16 treasury_FEE;
    uint16 BURN_FEE;
    function setTreasuryFee(uint16 _treasury_FEE) public onlyOwner {
        treasury_FEE = _treasury_FEE;
    }
    
    function setBurnFee(uint16 _BURN_FEE) public onlyOwner {
        BURN_FEE = _BURN_FEE;
    }
    
    uint256 pending_TRESURY_rewards;
    uint256 pending_burn_amount;


    // View function to see pending decores on frontend.
    function pendingdecore(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accdecorePerShare = pool.accdecorePerShare;

        return user.amount.mul(accdecorePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the decore token.
    // ----
    uint256 private decoreBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = decore.balanceOf(address(this)).sub(decoreBalance);

        if(newRewards > 0) {
            decoreBalance = decore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 decoreRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        decoreRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 decoreRewardFee = decoreRewardWhole.mul(treasury_FEE).div(10000);
        uint256 decoreBurnFee = decoreRewardWhole.mul(BURN_FEE).div(10000);
        uint256 decoreRewardToDistribute = decoreRewardWhole.sub(decoreRewardFee).sub(decoreBurnFee);

        pending_TRESURY_rewards = pending_TRESURY_rewards.add(decoreRewardFee);
        pending_burn_amount = pending_burn_amount.add(decoreBurnFee);

        pool.accdecorePerShare = pool.accdecorePerShare.add(
            decoreRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to decoreVault for decore allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accdecorePerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accdecorePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from decoreVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming decore farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accdecorePerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safedecoreTransfer(from, pending);
        }
    ...
    // Safe decore transfer function, just in case if rounding error causes pool to not have enough decores.
    function safedecoreTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 decoreBal = decore.balanceOf(address(this));
        if (_amount > decoreBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", decoreBal);

            decore.transfer(_to, decoreBal);
            decoreBalance = decore.balanceOf(address(this));

        } else {
            decore.transfer(_to, _amount);
            decoreBalance = decore.balanceOf(address(this));

        }

        if(pending_TRESURY_rewards > 0) {
            uint256 devSend = pending_TRESURY_rewards; // Avoid recursive loop
            pending_TRESURY_rewards = 0;
            safedecoreTransfer(treasuryaddr, devSend);
        }
        
        if(pending_burn_amount > 0) {
            uint256 burnSend = pending_burn_amount; // Avoid recursive loop
            pending_burn_amount = 0;
            safedecoreTransfer(burnaddr, burnSend);
        }

    }

    // Update dev address by the previous treasury.
    // Note onlyOwner functions are meant for the governance contract
    // allowing decore governance token holders to do this functions.
    function setTreasuryFeeReciever(address _treasuryaddr) public onlyOwner {
        treasuryaddr = _treasuryaddr;
    }
