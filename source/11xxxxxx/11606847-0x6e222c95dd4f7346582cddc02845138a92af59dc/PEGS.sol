...
contract PUSDShares is ERC20Custom, AccessControl {
    ...
    address public PUSDStablecoinAdd;
    
    uint256 public constant genesis_supply = 30000000e18; // 100M is printed upon genesis
    uint256 public PEGS_DAO_min; // Minimum PEGS required to join DAO groups 
    ...
    modifier onlyPools() {
       require(PUSD.pusd_pools(msg.sender) == true, "Only pusd pools can mint new PUSD");
        _;
    } 
    ...
    function setPUSDAddress(address pusd_contract_address) external onlyByOwnerOrGovernance {
        PUSD = PUSDStablecoin(pusd_contract_address);
    }
    
    function setPEGSMinDAO(uint256 min_PEGS) external onlyByOwnerOrGovernance {
        PEGS_DAO_min = min_PEGS;
    }
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "PEGS::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address voter, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "PEGS::_writeCheckpoint: block number exceeds 32 bits");
    ...
    // Track PEGS burned
    event PEGSBurned(address indexed from, address indexed to, uint256 amount);

    // Track PEGS minted
    event PEGSMinted(address indexed from, address indexed to, uint256 amount);
