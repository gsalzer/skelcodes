/**
 * 
 * UniPastry Finance is a full stack DeFi platform featuring staking, yield farming and NFT gaming.
 *
 * Official Website: 
...
contract UniPastryFinance is ERC20Detailed 
{
    ...
    string constant tokenName = "UniPastry Finance";
    string constant tokenSymbol = "UPIE"; 
    ...
    */
    function _executeTransfer(address from, address to, uint256 value) private
    {
        ...
        if(isStaking[from] && balances[from] < 1)
        {
             ...
             fullUnitsStaked_total = fullUnitsStaked_total.sub(toFullUnits(balances[from])).sub(1);
        ...
        if(fus_total > 0)
        {
            uint256 stakingRewards = sixPercent;
    ...
    function viewUnpaidRewards(address staker) public view returns (uint256)
    {
        ...
        uint256 newRewards = newRewardsPerUnit.mul((toFullUnits(balances[staker])).add(1));
    ...
    function _enableStaking(address staker) private
    {
        ...
        fullUnitsStaked_total = fullUnitsStaked_total.add(toFullUnits(balances[staker])).add(1);
    ...
    function _disableStaking(address staker) private
    {
        ...
        fullUnitsStaked_total = fullUnitsStaked_total.sub(toFullUnits(balances[staker])).sub(1);
        totalStakers = totalStakers.sub(1);
    }
