...
contract VendingMachine is Ownable {
    ...
    constructor(ERC1155Tradable _RMUAddress, ToshiCashNonTradable _ToshiCashAddress) public {
        RMU = _RMUAddress;
    ...
    function redeem(uint256 card) public {
        ...
        require(ToshiCash.balanceOf(msg.sender) >= cardCosts[card], "Not enough points to redeem for card");
        require(RMU.totalSupply(card) < RMU.maxSupply(card), "Max cards minted");

        ToshiCash.burn(msg.sender, cardCosts[card]);
        RMU.mint(msg.sender, card, 1, "");
