...
contract VotingDataLayout is LibraryLock {
    ...
    modifier _updateState(address voter) {
        if (block.number.sub(lastVotePropogationBlock) > votePropogationBlocks) {
            userVoteTracker[voter].votes = userVoteTracker[voter].defaultVoteCount;
        }
...
contract NyanVoting is Proxiable, VotingDataLayout{
    ...
    /** @notice Updates the contract owner.
      ...
      */
    function setOwner(address _owner) public _onlyOwner _updateState(msg.sender) delegatedOnly {
        owner = _owner;
    ...
    /** @notice Updates the connector address.
      ...
      */
    function setConnector(address _connector) public _onlyConnector _updateState(msg.sender) delegatedOnly {
    ...
    /** @notice Updates the funds address.
      ...
      */
    function setFundAddress(address _fund) public _onlyConnector _updateState(msg.sender) delegatedOnly {
        fundAddress = _fund;
    ...
    /** @notice Updates the voting period block length.
      ...
      */
    function setVotingPeriodBlockLength(uint256 _blocks) public _onlyConnector _updateState(msg.sender) delegatedOnly {
    ...
    /** @notice Updates the NyanV2 address.
      ...
      */
    function setNyanV2Address(address _addr) public _onlyConnector _updateState(msg.sender) delegatedOnly {
        nyanV2 = _addr;
        
        emit NewNyanAddress(nyanV2);
    }
    
    /** @notice Updates the catnipV2 address.
      * @param _addr Address of the new Catnip contract.
      */
    function setCatnipV2Address(address _addr) public _onlyConnector _updateState(msg.sender) delegatedOnly {
        catnipV2 = _addr;
        
        emit  NewCatnipAddress(catnipV2);
    }
    
    /** @notice Updates the dNyanV2 address.
      * @param _addr Address of the new dNyanV2 contract.
      */
    function setdNyanV2Address(address _addr) public _onlyConnector _updateState(msg.sender) delegatedOnly {
        dNyanV2 = _addr;
        
        emit NewDNyanAddress(dNyanV2);
    }
    
    /** @notice Updates the catnip fee divider.
      * @param _val Number to divide votes by.
      */
    function setVoteDiv(uint256 _val) public _onlyConnector delegatedOnly _updateState(msg.sender) {
        voteDiv = _val;
    }
    ...
    /** @notice Triggers when a user stakes NyanV2 LP tokens.
      ...
      */
     function nyanV2LPStaked(uint256 totalV2LPStaked, address voter) public _onlyNyanV2 delegatedOnly _updateState(msg.sender) {
        userVoteTracker[voter].defaultVoteCount = totalV2LPStaked.div(1e18);
        userVoteTracker[voter].lastBlockStaked = block.number;
    }
    ...
    function proposeBid(
        ...
        ) public  _updateState(msg.sender) {
    ...
    function addChainBid(
        ...
        string[] memory _strings, 
        bytes[] memory _bytesArr) 
        public _updateState(msg.sender) delegatedOnly {
    ...
    function claimDistribution() public _updateState(msg.sender) delegatedOnly {
        ...
        uint256 nyanV2LPStakedAmount = userVoteTracker[msg.sender].defaultVoteCount.mul(1e18);
