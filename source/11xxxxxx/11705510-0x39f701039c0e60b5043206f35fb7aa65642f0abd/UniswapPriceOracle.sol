...
// File: contracts/UniswapPriceOracle.sol
...
contract UniswapPriceOracle {
    ...
    uint256 internal constant MANTISSA = 10**18;
    ...
    constructor(
        ...
        require(
            ...
            "UniswapPriceOracle: NO_RESERVES"
    ...
    function update() external {
        ...
        require(
            timeElapsed >= PERIOD,
            "UniswapPriceOracle: PERIOD_NOT_ELAPSED"
        );
        ...
        emit Update(blockTimestampLast);
    }

    // note this will always return 0 before update has been called successfully for the first time.
    function consult(address token) external view returns (uint256 amountOut) {
        if (token == token0) {
            amountOut = price0Average.mul(MANTISSA).decode144();
        } else {
            require(token == token1, "UniswapPriceOracle: INVALID_TOKEN");
            amountOut = price1Average.mul(MANTISSA).decode144();
        }
    }
}
