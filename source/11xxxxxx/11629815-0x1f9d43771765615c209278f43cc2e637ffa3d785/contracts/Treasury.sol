...
contract Treasury is ContractGuard, Epoch {
    ...
    uint256 private accumulatedSeigniorage;
    uint256 public fundAllocationRate = 2; // 2%
    ...
    function setFundAllocationRate(uint256 rate) external onlyOperator {
        require(rate <= 5, 'Treasury: Invalid rate'); // Max. 5%
        fundAllocationRate = rate;
        emit ContributionPoolRateChanged(msg.sender, rate);
    }

    function setCashPriceCeiling(uint256 ceiling) external onlyOperator {
        cashPriceCeiling = uint256(ceiling).mul(cashPriceOne).div(10**2);
        emit CashPriceCeilingChanged(msg.sender, ceiling);
    }
    ...
    event CashPriceCeilingChanged(address indexed operator, uint256 ceiling);
