...
contract PooledESDCoupons is ReentrancyGuard {
  ...
  string public constant name = "CPOOL";
  string public constant symbol = "CPOOL";
  uint8 public constant decimals = 18;
  
  // Looks weird in Uniswap UI to have 2M CPOOL tokens for just 10k of ESD (10k * 200 epoch = 2M CPOOL)
  ...
  function wrap(uint _epoch, uint _couponAmount) public nonReentrant {
    uint expiresIn = ESDS.couponsExpiration(_epoch).sub(ESDS.epoch());
    require(expiresIn >= 30, "PooledESDCoupons: coupon expires in less than 30 epochs");
    require(_couponAmount > 1e3);

    _mint(msg.sender, _couponAmount.mul(_epoch).div(SCALE));
    ESDS.transferCoupons(msg.sender, address(this), _epoch, _couponAmount);
  }

  function unwrap(uint _epoch, uint _tokenAmount) public nonReentrant {
    uint couponAmount = _tokenAmount.mul(SCALE).div(_epoch);
    _burn(msg.sender, _tokenAmount);
    ESDS.transferCoupons(address(this), msg.sender, _epoch, couponAmount);
  }
