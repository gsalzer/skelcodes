 ...
 *    ╚═════╝  ╚═════╝ ╚══════╝╚═╝     ╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝
 * 
 * https://dolphintoken.dev/
 * https://t.me/dolphintoken
 * 
 * Experimental project by Seal Ze.
 * 
 * Contract has been tested, but could contain flaws.
 * I am not liable if these flaws exist. Have fun!
 * 
 *
 *                                                          
 */
...
contract Token is ERC20Detailed, ERC20Mintable, ERC20Burnable, WhitelistedRole {
    ...
    ERC20Detailed(_name, _symbol, _decimals)
    {
        ...
        _mint(msg.sender, _initialSupplyWithoutDecimals * (10 ** uint256(decimals())));
    }
    ...
    function transfer(address _recipient, uint256 _amount) public returns (bool) {
        ...
        if (!isWhitelisted(_recipient)) {
            // calculate and burn transfer fee
            uint256 fee = _amount.mul(feePercentage).div(100);
            actualAmount = _amount.sub(fee);

            burn(fee);
        }

        super.transfer(_recipient, actualAmount);
        return true;
    }

    function addWhitelisted(address[] calldata _accounts)
    external
    {
    ...
    function removeWhitelisted(address[] calldata _accounts)
    external
    {
        for (uint256 i = 0; i < _accounts.length; i++) {
            removeWhitelisted(_accounts[i]);
        }
    }

    function transferAndCall(address _to, uint256 _tokens, bytes calldata _data)
    external
    returns (bool)
    {
