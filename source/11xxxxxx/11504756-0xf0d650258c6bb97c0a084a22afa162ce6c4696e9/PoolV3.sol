...
contract PoolV3 {
    ...
    function unmine(uint256 amount) public payable {
        require(amount > 0, "zero");
        uint256 fee =
            amount.mul(unminingFeeNumerator).div(unminingFeeDenominator);
        require(msg.value == fee, "invalid fee");
    ...
    function getReward(address miner) public view returns (uint256) {
        ...
        uint256 rewardPerBlockResult =
            blockResult.mul(rewardPerBlockNumerator).div(
                rewardPerBlockDenominator
            );
        return rewardPerBlockResult.mul(minerBalance[miner]).div(totalBalance);
    }

    function getRewardByBlock(address miner, uint256 blockNumber)
        public
        view
        returns (uint256)
    {
