...
// File: contracts/ImicroCOREVault.sol

pragma solidity 0.6.12;

interface ImicroCOREVault {
...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        address _microCOREAddress,
        address _WETHAddress,
        address _uniswapFactory
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        microCORETokenAddress = _microCOREAddress;
        WETHAddress = _WETHAddress;
        tokenUniswapPair = IUniswapV2Factory(_uniswapFactory).getPair(WETHAddress,microCORETokenAddress);
        feePercentX100 = 11;
    ...
    address microCORETokenAddress;
    address microCOREVaultAddress;
    ...
    function setmicroCOREVaultAddress(address _microCOREVaultAddress) public onlyOwner {
        microCOREVaultAddress = _microCOREVaultAddress;
    }
    ...
    function calculateAmountsAfterFee(
        ...
        ) public  returns (uint256 transferToAmount, uint256 transferToFeeDistributorAmount)
        {
            ...
            if(sender == microCOREVaultAddress  || sender == tokenUniswapPair ) { // Dont have a fee when microCOREVault is sending, or infinite loop
...
contract NBUNIERC20 is INBUNIERC20, OwnableUpgradeSafe {
    ...
    function initialSetup(address router_) internal {
        ...
        uint256 initialSupply = 50e18; // 50
        
        //20% to time-locked wallet for microCORE airdrops 
        _mint(0xC4eec19D7BFb2106400A506f777bFaff4DB1061a, initialSupply.div(5));
        //80% to be sent to uniswap 
        _mint(address(this), initialSupply.div(100).mul(80));
...
// File: contracts/microCORE.sol

pragma solidity 0.6.12;


// microCOREToken with Governance.
contract microCORE is NBUNIERC20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "microCORE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "microCORE::delegateBySig: invalid nonce");
        require(now <= expiry, "microCORE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "microCORE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying microCORE tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "microCORE::_writeCheckpoint: block number exceeds 32 bits");
