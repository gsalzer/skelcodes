...
contract StakingRewardsFactory is Ownable {
    ...
    function deploy(address stakingToken, uint rewardAmount, uint rewardPerDay) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards == address(0), 'StakingRewardsFactory::deploy: already deployed');

        info.stakingRewards = address(new StakingRewards(/*_rewardsDistribution=*/ address(this), rewardsToken, stakingToken, rewardPerDay));
        info.rewardAmount = rewardAmount;
        stakingTokens.push(stakingToken);
    }

    // Update pool status to closed
    function closePool(address stakingToken) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards != address(0), 'StakingRewardsFactory::notifyRewardAmount: not deployed');
        StakingRewards(info.stakingRewards).closePool();
    }
    
    // Allow claims of rewards
    function unblockClaims(address stakingToken) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards != address(0), 'StakingRewardsFactory::notifyRewardAmount: not deployed');
        StakingRewards(info.stakingRewards).unblockClaims();
    }

    // Block claim of rewards
    function blockClaims(address stakingToken) public onlyOwner {
        StakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards != address(0), 'StakingRewardsFactory::notifyRewardAmount: not deployed');
        StakingRewards(info.stakingRewards).blockClaims();
    }
