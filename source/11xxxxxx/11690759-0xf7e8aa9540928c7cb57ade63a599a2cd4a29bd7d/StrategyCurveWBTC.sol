...
contract StrategyCurveWBTC {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    
    address constant public want = address(0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599); // 
    address constant public liquidity_pool = address(0x4CA9b3063Ec5866A4B82E437059D2C43d1be596F);
    address constant public hcrv = address(0xb19059ebb43466C323583928285a49f558E572Fd);
    address constant public pool = address(0x4c18E409Dc8619bFb6a1cB56D114C3f592E0aE79);
    ...
    address constant public hbtc = address(0x0316EB71485b0Ab14103307bf65a021042c6d380);
    ...
    address public controller = 0x5055cfADfbF9927deD44c183979085e2eC79Ed9d;
    address public foundationAddress = 0x1250E38187Ff89d05f99F3fa0E324241bbE2120C;
    address public burnAddress = 0x767575b1c3f653b3842cf1f77a1E6105C3744952;

    string public getName = "Golff:Strategy:WBTC:CRV";
    ...
    constructor() public {
        ...
        swap2GOFRouting = [output,weth,gof];
        swap2TokenRouting = [output,weth,wbtc];
        doApprove();
    }
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            IERC20(want).safeApprove(liquidity_pool, 0);
            IERC20(want).safeApprove(liquidity_pool, _want);
            ICurveFi(liquidity_pool).add_liquidity([0,_want],0);
        }
    ...
    function depositPool() public {
        uint _hcrv = IERC20(hcrv).balanceOf(address(this));
        if (_hcrv > 0) {
            IERC20(hcrv).safeApprove(pool, 0);
            IERC20(hcrv).safeApprove(pool, _hcrv);
            Gauge(pool).deposit(_hcrv);
        }
    }
    ...
    function withdraw(IERC20 _asset) external returns (uint balance) {
        ...
        require(output != address(_asset), "Golff:crv");
        require(hcrv != address(_asset), "Golff:hcrv");
    ...
    function withdrawWBTC(uint256 _amount) internal returns (uint256) {
        uint256 _before = IERC20(want).balanceOf(address(this));

        IERC20(hcrv).safeApprove(liquidity_pool, 0);
        IERC20(hcrv).safeApprove(liquidity_pool, _amount);
        ICurveFi(liquidity_pool).remove_liquidity(_amount, [uint256(0),0]);

        uint256 _hbtc = IERC20(hbtc).balanceOf(address(this));
        if (_hbtc > 0) {
            IERC20(hbtc).safeApprove(liquidity_pool, 0);
            IERC20(hbtc).safeApprove(liquidity_pool, _hbtc);
            ICurveFi(liquidity_pool).exchange(0, 1, _hbtc, 0);
        }
    ...
    function _withdrawAll() internal {
        ...
        withdrawWBTC(IERC20(hcrv).balanceOf(address(this)));
    }
    ...
    function doswap() internal {
        ...
        uint _hbtc = IERC20(hbtc).balanceOf(address(this));
        if (_wbtc > 0 || _hbtc > 0) {
            IERC20(wbtc).safeApprove(liquidity_pool, 0);
            IERC20(wbtc).safeApprove(liquidity_pool, _wbtc);
            IERC20(hbtc).safeApprove(liquidity_pool, 0);
            IERC20(hbtc).safeApprove(liquidity_pool, _hbtc);
            ICurveFi(liquidity_pool).add_liquidity([_hbtc,_wbtc],0);
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint) {
        // calculate amount of hcrv to withdraw for amount of want
        uint _hcrv = _amount.mul(1e18).div(ICurveFi(liquidity_pool).get_virtual_price());
        Gauge(pool).withdraw(_hcrv);
        return withdrawWBTC(IERC20(hcrv).balanceOf(address(this)));
    }
    ...
    function balanceOfHBTCinIiquidity() public view returns (uint) {
        return balanceOfPool().mul(ICurveFi(liquidity_pool).get_virtual_price()).div(1e18);
    }
    
    function balanceOf() public view returns (uint) {
        return balanceOfWant()
               .add(balanceOfHBTCinIiquidity());
    }
