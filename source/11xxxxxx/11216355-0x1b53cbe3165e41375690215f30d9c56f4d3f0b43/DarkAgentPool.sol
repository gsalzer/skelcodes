...
// ------------ darklisted_ Rewards Logics--------------------------------
    ...
    function darklisted_rewardPerToken(address token) public returns(uint256) {

        if (totalSupply() == 0) {
            darklisted_reward_per_token_stored[token] = 0;
        }
    ...
    function darklisted_earned(address account, address token) public returns(uint256) {
    ...
    function darklisted_notifyRewardAmount() external {
        ...
        for(uint d = 0; d< darklisted_token.length; d++){
            ...
            if (darklisted_token[d] == 0x3108ccFd96816F9E663baA0E8c5951D229E8C6da) {
                ...
                darklisted_newreward = IERC20(d).balanceOf(address(this));
            }
