...
// File: contracts/StabinolClaimer.sol
...
contract StabinolClaimer is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    // variables
    address public stolAddress; // The address for the STOL tokens
    address public wethAddress; // The address of WETH
    uint256 public maxPercentClaim = 4000; // Initial conditions are 4% of USD value of STOL holdings can be claimed
    ...
    function claim() external nonReentrant
    {
        ...
        if(currentBalance < allUsersInfo[_user].ethBalance){
            // Can only claim STOL if spending ETH/WETH and balance is less than previous balance
            uint256 spent = allUsersInfo[_user].ethBalance.sub(currentBalance); // Returns ETH spent since last claim
            allUsersInfo[_user].ethBalance = currentBalance; // Set the balance to the current balance
            ...
            claimedAmount = _stake.getSTOLInLP(_user).mul(maxPercentClaim).div(DIVISION_FACTOR); // Returns STOL in user's LP share, then takes percent
            ...
            allUsersInfo[_user].ethBalance = currentBalance; // Only set the balance to the current balance
        }
    }
    ...
    function finishChangeMaxPercent() external onlyGovernance timelockConditionsMet(4) {
        maxPercentClaim = _timelock_data;
    }
    // --------------------
    ...
    function startChangeMinSTBZ(uint256 _stbz) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 5;
        _timelock_data = _stbz;
    }
    
    function finishChangeMinSTBZ() external onlyGovernance timelockConditionsMet(5) {
        _minSTBZStaked = _timelock_data;
    }
    // --------------------
    
    // Change the min claim interval
    // --------------------
    function startChangeMinClaimTime(uint256 _interval) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 6;
        _timelock_data = _interval;
    }
    
    function finishChangeMinClaimTime() external onlyGovernance timelockConditionsMet(6) {
        minClaimWindow = _timelock_data;
    }
    // --------------------
    ...
    function finishTransferVaultToNewClaimer() external onlyGovernance timelockConditionsMet(7) {
