...
//Original codebase from SushiswapV2SingleSidedILProtection,Staking code Taken from v3 deployment of StableCredit

//Modified to distribute half of the fees to stakers

//Dont ape without understanding what sil or slt is for,no one will be responsible for your losses for aping at a premium
...
contract SushiswapV2ILTest is ReentrancyGuard {
    using SafeERC20 for IERC20;

    /// @notice EIP-20 token name for this token
    string public constant name = "SushiswapV2 ILTest";
    ...
    address public constant RLR = address(0x0E3EF895c59E7Db27214AB5bbf56347cE115A3f4);
    ...
    constructor () {
        ...
        PAIR = FACTORY.createPair(address(this), RLR);
    }
    ...
    function _transferTokens(address src, address dst, uint amount) internal {
        ...
        if ((pairs[src] && dst != address(this))||(pairs[dst] && src != address(this))) {
            //Unlike sil we send it here for fee claims,half goes to stakers,half goes to pool
            uint _fee = amount / FEE / 2;
            //Transfer half for fee dist
            _transferTokens(dst, address(this),_fee);
            //Transfer other half to rlr pair
            _transferTokens(dst, PAIR, _fee);
            //Update fee reward amount
