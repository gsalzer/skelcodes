...
contract Stake is Owned {
    ...
    address public YFICG = 0x9080e92296a176883aAB1d7d1B7e50BC055B0cAa;
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(YFICG).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function ADDFUNDS(uint256 tokens) external {
        require(IERC20(YFICG).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(YFICG).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(YFICG).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedYFICG(address staker) external view returns(uint256 stakedYFICG){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the YFICG balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourYFICGBalance(address user) external view returns(uint256 YFICGBalance){
        return IERC20(YFICG).balanceOf(user);
    }
}
