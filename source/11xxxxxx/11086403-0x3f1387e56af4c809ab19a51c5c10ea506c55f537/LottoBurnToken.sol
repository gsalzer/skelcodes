...
contract Heap is Ownable {
    ...
    uint256 public constant TOP_SIZE = 50;
...
// File: contracts/LottoBurnToken.sol
...
contract LottoBurnToken is Ownable, GasPump, IERC20 {
    ...
    // metadata
    string public name = "Lottoburn.finance";
    string public constant symbol = "`LOBU";
    ...
    function _transferFrom(address _operator, address _from, address _to, uint256 _value, bool _payFee) internal {
        ...
        if (_payFee || !_isWhitelisted(_from, _to)) {
            ...
            _setBalance(winner, _balanceOf(winner).add(LOBU));
            emit Winner(winner, LOBU);
            emit Transfer(_from, winner, LOBU);
        }

        // Sanity checks
        // no tokens where created
        assert(burn.add(LOBU).add(receive) == _value);
