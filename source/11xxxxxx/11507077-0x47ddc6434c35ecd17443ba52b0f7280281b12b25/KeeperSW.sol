...
interface IKeeperSWHelper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract KeeperSW is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice KeeperSW Helper
    IKeeperSWHelper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "KeeperSW";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "KPRS";
    ...
    /// @notice 2 day to bond to become a keeper
    uint constant public BOND = 2 days;
    /// @notice 14 days to unbond to remove funds from being a keeper
    uint constant public UNBOND = 7 days;
    /// @notice 3 days till liquidity can be bound
    uint constant public LIQUIDITYBOND = 2 days;
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = KeeperSWLibrary.getReserve(liquidity, address(this));
    ...
    function setKeeperSWHelper(IKeeperSWHelper _kprh) external {
        require(msg.sender == governance, "setKeeperSWHelper: !gov");
        KPRH = _kprh;
    }
