...
contract ZeeLock is Ownable {
    ...
    //Fee per lock in zeedex token
    uint256 private _zeedexTokenFee;
    ...
    event LockTokenUpdated(address indexed zeedexTokenAddress);
    ...
    modifier tokenExist(address token) {
        require(_tokenVsIndex[token] > 0, "Zeelock: Token does not exist!!");
        _;
    }

    modifier tokenDoesNotExist(address token) {
        require(_tokenVsIndex[token] == 0, "Zeelock: Token already exist!!");
        _;
    }
    ...
    modifier canLockAsset(address token) {
        ...
        require(index > 0, "Zeelock: Token does not exist!!");

        require(
            _tokens[index.sub(1)].status == TokenStatus.ACTIVE,
            "Zeelock: Token not active!!"
        );

        require(
            !_tokens[index.sub(1)].emergencyUnlock,
            "Zeelock: Token is in emergency unlock state!!"
        );
        _;
    }
    ...
    modifier canClaim(uint256 id) {
        ...
        require(claimable(id), "Zeelock: Can't claim asset");

        require(
            _idVsLockedAsset[id].beneficiary == msg.sender,
            "Zeelock: Unauthorized access!!"
        );
        _;
    }
    ...
    */
    modifier whenNotPaused() {
        require(!_paused, "Zeelock: paused");
        _;
    }
    ...
    */
    modifier whenPaused() {
        require(_paused, "Zeelock: not paused");
        _;
    }
    ...
    * @param zeedexTokenAddress Address of the zeedex token
    * @param zeedexTokenFee Fee for each lock in zeedex token
    */
    constructor(
        address payable wallet,
        address zeedexTokenAddress,
        uint256 zeedexTokenFee
    )
        public
    {
        require(
            wallet != address(0),
            "Zeelock: Please provide valid wallet address!!"
        );
        require(
            zeedexTokenAddress != address(0),
            "Zeelock: Invalid lock token address"
        );
        _lockToken = IERC20(zeedexTokenAddress);
        _wallet = wallet;
        _zeedexTokenFee = zeedexTokenFee;
    }
    ...
    */
    ...
    /**
    * @dev Returns fee per lock in zeedex token
    */
    function getLockTokenFee() external view returns(uint256) {
        return _zeedexTokenFee;
    }
    ...
    */
    ...
    )
    {
        ...
        require(start.add(length) <= _tokens.length, "Zeelock: Invalid input");
        require(length > 0 && length <= 15, "Zeelock: Invalid length");
        uint256 count = 0;
    ...
    */
    ...
    {
        require(destToken != address(0), "Zeelock: Invalid destination token!!");
        require(numerator > 0, "Zeelock: Invalid numerator!!");
        require(denominator > 0, "Zeelock: Invalid denominator!!");
        require(isActive(baseToken), "Zeelock: Base token is not active!!");
    ...
    /**
    * @dev Update zeedex token address
    * @param zeedexTokenAddress New lock token address
    */
    function updateZeedexToken(address zeedexTokenAddress) external onlyOwner {
        require(
            zeedexTokenAddress != address(0),
            "Zeelock: Invalid zeedex token address"
        );
        _lockToken = IERC20(zeedexTokenAddress);
        emit LockTokenUpdated(zeedexTokenAddress);
    }

    /**
    * @dev Update fee in zeedex token
    * @param zeedexTokenFee Fee per lock in zeedex token
    */
    function updateZeedexTokenFee(uint256 zeedexTokenFee) external onlyOwner {
        _zeedexTokenFee = zeedexTokenFee;
        emit LockTokenFeeUpdated(zeedexTokenFee);
    }
    ...
    */
    function setWallet(address payable wallet) external onlyOwner {
        require(
            wallet != address(0),
            "Zeelock: Please provider valid wallet address!!"
        );
        _wallet = wallet;
    ...
    */
        ...
        tokenExist(token)
    {
        ...
        require(
            ...
            "Zeelock: Token already inactive!!"
        );
    ...
    * @param lockFee Bool to check if fee to be paid in zeedex token or not
    */
    function lock(
        address tokenAddress,
        uint256 amount,
        ...
        canLockAsset(tokenAddress)
    {
        ...
        require(
            ...
            "Zeelock: Sent more ethers then required"
        );
    ...
    */
        ...
        canLockAsset(tokenAddress)
    {
        ...
        require(amounts.length == durations.length, "Zeelock: Invalid input");
        require(amounts.length == beneficiaries.length, "Zeelock: Invalid input");
        require(
            amounts.length == amountThresholds.length,
            "Zeelock: Invalid input"
        );
    ...
    */
        ...
        whenNotPaused
    {
        ...
        require(lockedAsset.status == Status.OPEN, "Zeelock: Lock is not open");
        ...
        if(lockFee) {
            _lockToken.safeTransferFrom(msg.sender, _wallet, _zeedexTokenFee);
        }
        if(ETH_ADDRESS == lockedAsset.token) {
            require(amount == msg.value, "Zeelock: Insufficient value sent");

            if(!lockFee) {
                (bool success,) = _wallet.call.value(fee)("");
                require(success, "Zeelock: Transfer of fee failed");
            }
        }
        else {
            if(!lockFee){
    ...
    */
        ...
        returns(uint256)
    {
        require(
            ...
            "Zeelock: Provide valid beneficiary address!!"
        );
        ...
        require(
            ...
            "Zeelock: Please provide minimum amount of tokens!!"
        );
    ...
    */
        ...
        private
    {
        ...
        if(lockFee){
	    require(value >= amount, "Zeelock: Enough ETH not sent!!");
            _lockToken.safeTransferFrom(msg.sender, _wallet, fee);
        }
        else {
            require(value >= amount.add(fee), "Zeelock: Enough ETH not sent!!");
            (bool success,) = _wallet.call.value(fee)("");
            require(success, "Zeelock: Transfer of fee failed");
        }
    ...
    */
    function _claimETH(uint256 id) private {
        ...
        require(success, "Zeelock: Failed to transfer eth!!");
