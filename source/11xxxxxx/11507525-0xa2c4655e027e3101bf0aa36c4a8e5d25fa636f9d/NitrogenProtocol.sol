...
contract NitrogenProtocol is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Nitrogen Protocol";
    string private _symbol = "NGEN";
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        require(account != address(this), 'Nitrogen: The contract it self cannot be excluded');
        require(!_isExcluded[account], "Nitrogen: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Nitrogen: Account is already included");
    ...
    function _transfer(
       ...
       require(now > liquidityAddedAt + 10 minutes  || amount <= 100e9, "You cannot transfer more than 100 tokens.");
