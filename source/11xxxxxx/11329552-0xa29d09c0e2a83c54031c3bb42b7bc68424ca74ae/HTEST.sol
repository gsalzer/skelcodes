...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    // assign a new transactionfee.. if 1% give 10, if 2% give 20
    function setRewardFee(uint8 _newTxFee) public onlyOwner {
        txFee = _newTxFee;
    }
    
    // assign a new transaction Ownerfee .. if 1% give 10, if 2% give 20
    function setOwnerFee(uint8 _newTxFee) public onlyOwner {
        OwnertxFee = _newTxFee;
    }

    // assign a new fee distributor address
    function setRewardFeeDistributor(address _distributor) public onlyOwner {
    ...
    function calculateAmountsAfterFee(
        ...
        uint256 fee = amount.mul(txFee).div(10);
        uint256 Ownerfee = amount.mul(OwnertxFee).div(10);
        return (amount.sub(fee).sub(Ownerfee), fee, Ownerfee);
    }
 ...
 */
contract HTEST is DeflationaryERC20 {

    constructor() DeflationaryERC20("Hype TEST", "HTEST") {
