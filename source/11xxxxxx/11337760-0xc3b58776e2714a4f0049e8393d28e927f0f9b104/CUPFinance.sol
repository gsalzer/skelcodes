...
contract CUPFinance {
        ...
        if (ALLDIFFICULTIES.length > 1) {
            ...
            for (uint256 i = 0; i < ALLDIFFICULTIES.length; i++) {
                if (ALLDIFFICULTIES[i] == d.Difficulty) {
                    difficultyIndex = i;
                    // 直接结束，有可能 调整过难度，又调整回来,所以只找第一个出现的，比如20 * 1e8,15 * 1e8,20 * 1e8
                    break;
                }
            }
    ...
    function withdraw() public afterStarted {
        ...
        poolToken.safeTransfer(msg.sender, amount.sub(burnAmount));
        poolToken.safeTransfer(CONTRACT_DEVELOPER, burnAmount);
