...
contract ServiceV6 {
    ...
    function payFee() public payable {
        ...
        feeCollector.transfer(msg.value);
        paidOnBlock[msg.sender] = paidOnBlock[msg.sender].add(
            recurringPaymentCycleInBlocks
        );
    }
    ...
    function claim(uint256 blockNumber) public payable {
        ...
        if (
            (entityIsNaaS[msg.sender] && recurringNaaSFeeInWei != 0) ||
            (!entityIsNaaS[msg.sender] && recurringFeeInWei != 0)
        ) {
            require(
                blockNumber <
                    paidOnBlock[msg.sender].add(recurringPaymentCycleInBlocks),
                "pay fee"
            );
        }
