...
/*
    Copyright 2019 dYdX Trading Inc.
    Copyright 2021 DaiDynamic Devs, based on the works of the Empty Set Squad Devs
...
library Constants {
    ...
    /* Bootstrapping */
    uint256 private constant INITIALSWAP_SUPPLY_TARGET = 100000e18; // 100K INITIALSWAP
    uint256 private constant BOOTSTRAP_SUPPLY_TARGET = 3000000e18; // 3M BOOTSTRAP
    uint256 private constant FINAL_SUPPLY_TARGET = 30000000e18; // 30M FINAL
    uint256 private constant BOOTSTRAPPING_PERIOD = 100; // 100 Epochs
    uint256 private constant BOOTSTRAPPING_PRICE = 154e16; // 1.54 DAI (4.5% inflation)
    ...
    /* Bonding */
    uint256 private constant INITIAL_STAKE_MULTIPLE = 1e7; // 1000 DAID
    ...
    uint256 private constant ORACLE_POOL_RATIO = 70; // 70% LP 30%DAO
    ...
    function getInitialSwapSupplyTarget()
        internal
        pure
        returns (Decimal.D256 memory)
    {
        return Decimal.from(INITIALSWAP_SUPPLY_TARGET);
    }

    function getBootstrapSupplyTarget()
        internal
        pure
        returns (Decimal.D256 memory)
    {
        return Decimal.from(BOOTSTRAP_SUPPLY_TARGET);
    }

    function getFinalSupplyTarget()
        internal
        pure
        returns (Decimal.D256 memory)
    {
        return Decimal.from(FINAL_SUPPLY_TARGET);
    }
...
contract Dollar is
    ...
    Permittable,
    ERC20Burnable{
    constructor() public ERC20Detailed("DaiDynamic", "DAID", 18) Permittable() {}
...
contract DaiSwap {
    string public name = "DaiDynamic DAI-DAID Fixed Swap";
    Dollar public daidToken;
    Dai public daiToken;
    ...
    event TokensPurchased(address account, address daidToken, uint256 amount);
    ...
    constructor(
        ...
        daidToken = _daidToken;
        daiToken = _daiToken;
        wallet = _wallet;
    }

    function buyTokens(uint256 daiAmount) public {
        require(daidToken.balanceOf(address(this)) >= daiAmount,  "this contract has less DAID tokens than the sent DAI tokens."
        );
        //Get dai tokens from the user
        daiToken.transferFrom(msg.sender, wallet, daiAmount);

        // Transfer daid tokens to the user
        daidToken.transfer(msg.sender, daiAmount);

        uint256 amount = userInfo[msg.sender];
        if (amount == 0) {
            addressList.push(msg.sender);
        }
        amount = amount + daiAmount;
        userInfo[msg.sender] = amount;

        // Emit an event
        emit TokensPurchased(msg.sender, address(daidToken), daiAmount);
    }
}
