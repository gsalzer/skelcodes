...
/**
 * @title DEFIBASE ERC20 token
 * @dev This is part of an implementation of the DEFIBASE Index Fund protocol.
 *      DEFIBASE is a normal ERC20 token, but its supply can be adjusted by splitting and
 *      combining tokens proportionally across all wallets.
 *
 *      DEFIBASE balances are internally represented with a hidden denomination, 'shares'.
 *      We support splitting the currency in expansion and combining the currency on contraction by
 *      changing the exchange rate between the hidden 'shares' and the public 'BASE'.
 */
contract DEFIBaseToken is ERC20UpgradeSafe, ERC677Token, OwnableUpgradeSafe {
    // PLEASE READ BEFORE CHANGING ANY ACCOUNTING OR MATH
    // Anytime there is division, there is a risk of numerical instability from rounding errors. In
    // order to minimize this risk, we adhere to the following guidelines:
    // 1) The conversion rate adopted is the number of shares that equals 1 DEFIBASE.
    //    The inverse rate must not be used--totalShares is always the numerator and _totalSupply is
    //    always the denominator. (i.e. If you want to convert shares to DEFIBASE instead of
    //    multiplying by the inverse rate, you should divide by the normal rate)
    // 2) Share balances converted into DEFIBaseToken are always rounded down (truncated).
    //
    // We make the following guarantees:
    // - If address 'A' transfers x DEFIBaseToken to address 'B'. A's resulting external balance will
    //   be decreased by precisely x DEFIBaseToken, and B's external balance will be precisely
    //   increased by x DEFIBaseToken.
    //
    ...
    uint256 private constant INITIAL_SUPPLY = 5000000 * 10**DECIMALS;
    uint256 private constant INITIAL_SHARES = (MAX_UINT256 / (10 ** 36)) - ((MAX_UINT256 / (10 ** 36)) % INITIAL_SUPPLY);
    uint256 private constant MAX_SUPPLY = ~uint128(0);  // (2^128) - 1

    uint256 private _totalShares;
    uint256 private _totalSupply;
    uint256 private _sharesPerDEFIBASE;
    mapping(address => uint256) private _shareBalances;


    // This is denominated in DEFIBaseToken, because the shares-DEFIBASE conversion might change before
    // it's fully paid.
    mapping (address => mapping (address => uint256)) private _allowedDEFIBASE;
    ...
    /**
     * @dev Notifies DEFIBaseToken contract about a new rebase cycle.
     * @param supplyDelta The number of new DEFIBASE tokens to add into circulation via expansion.
     * @return The total number of DEFIBASE after the supply adjustment.
     */
        ...
        returns (uint256)
    {
        ...
        _sharesPerDEFIBASE = _totalShares.div(_totalSupply);

        // From this point forward, _sharesPerDEFIBASE is taken as the source of truth.
        // We recalculate a new _totalSupply to be in agreement with the _sharesPerDEFIBASE
        // conversion rate.
    ...
    function initialize()
        ...
        initializer
    {
        __ERC20_init("DEFIBase Protocol", "DEFIBASE");
        ...
        returns (bool)
    {
        ...
        uint256 shareValue = value.mul(_sharesPerDEFIBASE);
        ...
        returns (bool)
    {
        ...
        _allowedDEFIBASE[from][msg.sender] = _allowedDEFIBASE[from][msg.sender].sub(value);
        ...
        returns (bool)
    {
        ...
        _allowedDEFIBASE[msg.sender][spender] = value;
        ...
        returns (bool)
    {
        ...
        _allowedDEFIBASE[msg.sender][spender] = _allowedDEFIBASE[msg.sender][spender].add(addedValue);
        emit Approval(msg.sender, spender, _allowedDEFIBASE[msg.sender][spender]);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        uint256 oldValue = _allowedDEFIBASE[msg.sender][spender];
        if (subtractedValue >= oldValue) {
            _allowedDEFIBASE[msg.sender][spender] = 0;
        } else {
            _allowedDEFIBASE[msg.sender][spender] = oldValue.sub(subtractedValue);
        }
        emit Approval(msg.sender, spender, _allowedDEFIBASE[msg.sender][spender]);
        return true;
    }
...
/**
 * @title DEFIBaseToken Monetary Supply Policy
 * @dev This is an implementation of the DEFIBaseToken Index Fund protocol.
 *      DEFIBaseToken operates symmetrically on expansion and contraction. It will both split and
 *      combine coins to maintain a stable unit price.
 *
 *      This component regulates the token supply of the DEFIBaseToken ERC20 token in response to
 *      market oracles.
 */
contract DEFIBaseTokenMonetaryPolicy is OwnableUpgradeSafe {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DEFIBaseToken public DEFIBASE;

    // Provides the current defi market cap, as an 18 decimal fixed point number.
    IOracle public mcapOracle;

    // Market oracle provides the token/USD exchange rate as an 18 decimal fixed point number.
    // (eg) An oracle value of .15e18 it would mean 1 DEFIBASE is trading for $.150.
    ...
    function setDEFIBASEToken(address _DEFIBASE)
        public
        onlyOwner
    {
        DEFIBASE = DEFIBaseToken(_DEFIBASE);
    }
     ...
     *      and targetPrice is McapOracleRate / defibaseMcap
     */
    function rebase() external {
        ...
        if (supplyDelta > 0 && DEFIBASE.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(DEFIBASE.totalSupply())).toInt256Safe();
        }

        uint256 supplyAfterRebase = DEFIBASE.rebase(epoch, supplyDelta);
     ...
     */
    function initialize(DEFIBaseToken DEFIBASE_)
        public
        initializer
    {
        __Ownable_init();
        ...
        rebaseWindowOffsetSec = 79200;  // 6PM UTC
    ...
    function computeSupplyDelta(uint256 price, uint256 mcap)
        ...
        returns (int256)
    {
        if (withinDeviationThreshold(price, mcap.div(100000000000))) {
            return 0;
        }

        // supplyDelta = totalSupply * (price - targetPrice) / targetPrice
        // 100,000,000,000:1 ratio
        int256 pricex       = price.mul(100000000000).toInt256Safe();
        int256 targetPricex = mcap.toInt256Safe();
        return DEFIBASE.totalSupply().toInt256Safe()
            .mul(pricex.sub(targetPricex))
            .div(targetPricex);
    }
