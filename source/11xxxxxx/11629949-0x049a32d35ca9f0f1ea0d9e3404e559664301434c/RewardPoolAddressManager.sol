...
// File: contracts/lendingpool/LendingPoolConfigurator.sol
...
// File: contracts/lendingpool/LendingPoolDataProvider.sol
...
contract LendingPoolDataProvider is VersionedInitializable {
    ...
    uint256 public constant DATA_PROVIDER_REVISION = 0x5;
    ...
    //calculates a users collateral in ETH for a specific reserve
    function calculateUserReserveCollateralETH(address _reserve, address _user)
        public
        view
        returns (
            uint256 totalCollateralBalanceETH
    ...
    //calculates a users collateral in ETH for invoice pool collateral reserves with status = true
    function calculateUserInvoiceCollateralETH(address _user)
        public
        view
        returns (
            uint256 totalCollateralBalanceETH
        )
    {
        ...
        LendingPoolConfigurator poolConfig = LendingPoolConfigurator(
        ...
        address[] memory reserves = poolConfig.getInvoiceCollateralReserves();//array of invoice lending pool collateral reserves

        for (uint256 i = 0; i < reserves.length; i++) {

            vars.currentReserve = reserves[i];
            //check if reserve is enabled or disabled for use as collateral in invoice lending pool
            if (poolConfig.getCollateralTokenAddressStatus(vars.currentReserve) == true) {
                (
                vars.compoundedLiquidityBalance,
                vars.compoundedBorrowBalance,
                vars.originationFee,
...
// File: contracts/lendingpool/LendingPoolLiquidationManager.sol
...
// File: contracts/lendingpool/LendingPool.sol
...
contract LendingPool is ReentrancyGuard, VersionedInitializable {
    ...
    **/
    ...
    /**
    * @dev check that reserve borrowed from is not enabled for invoice pool
    * @param _reserve the address of the reserve
    **/
    modifier isNotInvoiceLendingPool(address _reserve){
        LendingPoolConfigurator poolConfig = LendingPoolConfigurator(
            addressesProvider.getLendingPoolConfigurator()
        );
        require(
            poolConfig.isReserveEnabledForInvoicePool(_reserve) != true, 
            "reserve is an invoice lending pool reserve, cannot swap borrow rate from stable"
            );
        _;
    }
    ...
    */
    ...
    //invoice lending pool checks
    function calculateUserReserveCollateralETHInvoicePool(
        address _reserveToBorrowFrom,
        uint256 _amount,
    ...
    { //TODO: change to internal or modifier
        ...
        //if condition is true, then run this whole function internally or as modifier
        //check if reserve to borrow from is enabled for invoice pool
        ...
        ( , , , interestRateStrategyAddress, , , , ) = dataProvider.getReserveConfigurationData(_reserveToBorrowFrom);
        
        DefaultReserveInterestRateStrategy irs = DefaultReserveInterestRateStrategy(interestRateStrategyAddress);
    
        require((irs.getStableRateSlope1() != 0) || (irs.getStableRateSlope2() != 0), "no stable interest rates set for reserve to borrow from"); //check that reserve to borrow from has stable interest rates set in interest rate strategy contract
 
        require(_interestRateMode == uint256(CoreLibrary.InterestRateMode.STABLE), "only stable rate mode allowed when borrowing from invoice lending pool reserves");
        
        //get users total collateral balance in ETH for invoice lending pool collateral reserves/tokens
        vars.borrowFee = feeProvider.calculateLoanOriginationFee(msg.sender, _amount); //the fee to pay based on the amount to borrow
        
        //calculate total collateral balance in ETH for a user based on their balances of ALL reserves that have been set as invoice lending pool collateral reserves e.g., GBP using calculateUserInvoiceCollateralETH function in LendingPoolDataProvider
        vars.userCollateralBalanceETH = dataProvider.calculateUserInvoiceCollateralETH(msg.sender); 
        ...
        vars.amountOfCollateralNeededETH = dataProvider.calculateCollateralNeededInETH( //checking collateral amount needed based on the reserve in collateral
            _reserveToBorrowFrom,
            _amount,
            vars.borrowFee,
            vars.userBorrowBalanceETH,
            vars.userTotalFeesETH,
        ...
        require(vars.amountOfCollateralNeededETH <= vars.userCollateralBalanceETH, "There is not enough collateral to cover a new invoice lending pool stable borrow");

		return (vars.userCollateralBalanceETH, vars.amountOfCollateralNeededETH);
        //_;
    } 
    ...
    **/
        ...
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        //checks _reserve to borrow from is invoice lending pool reserve set in lending pool configurator by lending pool manager
        //and if so, checks that interest rate mode selected and passed by the user is stable 
        //and user has enough collateral in ETH based on their invoice lending pool collateral reserves/token balance
        calculateUserReserveCollateralETHInvoicePool(_reserve, _amount, _interestRateMode);
    ...
    * @dev the target user is defined by _onBehalfOf. If there is no repayment on behalf of another account, _onBehalfOf must be equal to msg.sender.
    ...
    **/
        ...
        onlyAmountGreaterThanZero(_amount)
    {
        ...
        vars.isETH = EthAddressLib.ethAddress() == _reserve; //true if ETH is reserve
    ...
    **/
    function swapBorrowRateMode(address _reserve)
        ...
        onlyActiveReserve(_reserve)
        onlyUnfreezedReserve(_reserve)
        isNotInvoiceLendingPool(_reserve)
    {
...
// File: contracts/tokenization/PToken.sol
...
// File: contracts/reward/RewardPool.sol
...
contract RewardPool is Ownable {
    ...
    {   
        core = _core;
        pToken = PToken(core.getReservePTokenAddress(_reserve));
    ...
    function withdraw(uint256 amount) public updateReward(msg.sender) {
        ...
        IERC20(pToken).safeTransfer(address(msg.sender), amount);
...
// File: contracts/reward/RewardPoolAddressManager.sol

pragma solidity ^0.5.0;



contract RewardPoolAddressManager is Ownable {

    mapping(address => address) public rewardPools; //reserve => reward pool for reserve

    event RewardPoolAdded(address _rewardPool, address _reserve);

    function getRewardPool(address _reserve) public view returns (address) {
        return rewardPools[_reserve];
    }
        ...
        external onlyOwner returns (address) 
    {
        ...
        //validate reserve => reward pool existence
        require(rewardPools[_reserve] == address(0), "reserve has already been added");
        //create reward pool contract
        RewardPool newPool = new RewardPool(_core, _rewardToken, _reserve, _duration);
        //add to mapping/list - map reserve address to reward pool address
        rewardPools[_reserve] = address(newPool);
        //transfer ownership to admin
        newPool.transferOwnership(msg.sender);
        //emit event
        emit RewardPoolAdded(address(newPool), _reserve);
