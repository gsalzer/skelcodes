...
contract MozartCoreV2 is Adminable, MozartCoreStorage {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        // The growth rate of the interest is simply how much has been accumulated
        // then adding 1e18 to the result.
        uint256 growthRate = accumulatedInterest().add(BASE);
        uint256 newIndex = currentBorrowIndex();

        // Update the new borrowIndex directly
        borrowIndex = newIndex;

        // Update the total borrows based on the proportional rate of interest
        // applied to the entire system
        totalBorrowed = totalBorrowed.mul(growthRate).div(BASE);
    ...
    function currentBorrowIndex()
        ...
        returns (uint256)
    {
        uint256 interestAccumulated = accumulatedInterest();

        // We add the existing index by the accumulated rate to get a compounded interest rate.
