...
contract INV {
    ...
    string public constant name = "Inverse Finance";
    ...
    function setMinter(address minter_) external {
        require(msg.sender == minter, "INV::setMinter: only the minter can change the minter address");
    ...
    function mint(address dst, uint rawAmount) external {
        require(msg.sender == minter, "INV::mint: only the minter can mint");
        require(block.timestamp >= mintingAllowedAfter, "INV::mint: minting not allowed yet");
        ...
        require(amount <= SafeMath.div(SafeMath.mul(totalSupply, mintCap), 100), "INV::mint: exceeded mint cap");
    ...
    function permit(address owner, address spender, uint rawAmount, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory == owner, "INV::permit: unauthorized");
