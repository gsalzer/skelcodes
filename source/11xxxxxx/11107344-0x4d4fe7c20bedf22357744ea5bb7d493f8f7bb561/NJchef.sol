...
// File: contracts/NJswapToken.sol
pragma solidity 0.6.12;
contract NJswapToken is ERC20("NJswap.org", "NJS"), Ownable {
    using SafeMath for uint256;
...
// File: contracts/NJchef.sol
pragma solidity 0.6.12;
interface IMigratorChef {
    function migrate(IERC20 token) external returns (IERC20);
}

contract NJchef is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accNJSPerShare; // Accumulated token per share
    }

    // NJswapToken
    NJswapToken public njswap;
    ...
    constructor(
        ...
        njswapPerBlock = 100000000000000000; // 0.1 /Block
        bonusEndBlock = _bonusEndBlock;
        startBlock = _startBlock;
    }

    function changeReward(uint256 _njswapPerBlock) public onlyOwner {
        njswapPerBlock = _njswapPerBlock;
    }
    ...
    // View function
    function pendingNJ(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accNJSPerShare = pool.accNJSPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 njswapReward = multiplier.mul(njswapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accNJSPerShare = accNJSPerShare.add(njswapReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accNJSPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 njswapReward = multiplier.mul(njswapPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        njswap.distribute(devaddr, njswapReward.div(20));
        njswap.distribute(address(this), njswapReward);
        pool.accNJSPerShare = pool.accNJSPerShare.add(njswapReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accNJSPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeNJSTransfer(msg.sender, pending);
                safeNJSTransfer(eventaddr, pending.div(20));
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accNJSPerShare).div(1e12);
    ...
    // Safe NJ transfer
    function safeNJSTransfer(address _to, uint256 _amount) internal {
        uint256 njswapBal = njswap.balanceOf(address(this));
        if (_amount > njswapBal) {
            njswap.transfer(_to, njswapBal);
        } else {
            njswap.transfer(_to, _amount);
        }
    }
