...
contract RewardedPltcWethUniV2Pair is Ownable {
    ...
    event Unstaked(address indexed user, uint256 pltcAmount, uint256 ethAmount);
    ...
    constructor(address _uniV2, address _modifiedUnipool) public {
        require(Address.isContract(_uniV2), "RewardedPltcWethUniV2Pair: _uniV2 not a contract");

        uniV2 = IUniswapV2Pair(_uniV2);
        modifiedUnipool = ModifiedUnipool(_modifiedUnipool);
        pltc = IERC20(uniV2.token0());
    ...
    function() external payable {
        require(msg.sender == address(weth), "RewardedPltcWethUniV2Pair: msg.sender is not weth");
    }
    ...
    function stake() public payable returns (bool) {
        require(msg.value > 0, "RewardedPltcWethUniV2Pair: msg.value must be greater than 0");
