...
// Kernel Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract KernelVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KERNELs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accKernelPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accKernelPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. KERNELs to distribute per block.
        uint256 accKernelPerShare; // Accumulated KERNELs per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing KERNEL governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's KERNELs allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing KERNEL governance consensus
    ...
    // View function to see pending KERNELs on frontend.
    function pendingKernel(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accKernelPerShare = pool.accKernelPerShare;

        return user.amount.mul(accKernelPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the KERNEL token.
    // ----
    uint256 private kernelBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = kernel.balanceOf(address(this)).sub(kernelBalance);

        if(newRewards > 0) {
            kernelBalance = kernel.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 kernelRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        kernelRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 kernelRewardFee = kernelRewardWhole.mul(DEV_FEE).div(10000);
        uint256 kernelRewardToDistribute = kernelRewardWhole.sub(kernelRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(kernelRewardFee);

        pool.accKernelPerShare = pool.accKernelPerShare.add(
            kernelRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to Kernel for KERNEL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accKernelPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accKernelPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from KernelVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming KERNEL farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accKernelPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeKernelTransfer(from, pending);
        }
    ...
    // Safe kernel transfer function, just in case if rounding error causes pool to not have enough KERNELs.
    function safeKernelTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 kernelBal = kernel.balanceOf(address(this));
        if (_amount > kernelBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", kernelBal);

            kernel.transfer(_to, kernelBal);
            kernelBalance = kernel.balanceOf(address(this));

        } else {
            kernel.transfer(_to, _amount);
            kernelBalance = kernel.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeKernelTransfer(devaddr, devSend);
        }
    ...
    // allowing KERNEL governance token holders to do this functions.
