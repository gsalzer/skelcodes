...
contract SushiMaker {
    ...
    function convert(address token0, address token1) public {
        ...
        (bool success, bytes memory data) = address(token0).call(abi.encodeWithSelector(0x95d89b41)); // symbol()
        require(success && keccak256(bytes(abi.decode(data, (string)))) != keccak256(bytes("SLP")), "SushiMaker: Token is SLP");        
        
        (success, data) = address(token1).call(abi.encodeWithSelector(0x95d89b41)); // symbol()
        require(success && keccak256(bytes(abi.decode(data, (string)))) != keccak256(bytes("SLP")), "SushiMaker: Token is SLP");        
