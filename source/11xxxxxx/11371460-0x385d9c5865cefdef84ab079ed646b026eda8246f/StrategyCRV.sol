...
contract StrategyCRV  {
    ...
    address constant public pickling = address(0x1BB74b5DdC1f4fC91D6f9E7906cf68bc93538e33);
    ...
    function deposit() public { 
		...
		if (_crvpla > 0)
		{
		    IERC20(crvPla).safeApprove(pickling, 0);				
            IERC20(crvPla).safeApprove(pickling, _crvpla);
		    yERC20(pickling).deposit(_crvpla);							
		}
		
		uint _pickling = IERC20(pickling).balanceOf(address(this));
		if (_pickling > 0)
		{
	        IERC20(pickling).safeApprove(PICKLE, 0);				
            IERC20(pickling).safeApprove(PICKLE, _pickling);
	    	pERC20(PICKLE).deposit(14,_pickling);						
    		pledgePickles = pledgePickles.add(_pickling);
		}
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns(uint256){
		...
		if (crvBefore < _crvpla)
		{
            ...
            uint256 _pickling = _crvpla.mul(1e18).div(yERC20(pickling).getRatio());
            uint _before = IERC20(pickling).balanceOf(address(this));
            if (_before < _pickling) {
                _pickling = _pickling.sub(_before);
                //require(_pickling <= pledgePickles, "Invalid amount!");
				if (_pickling > pledgePickles)
				{
					_pickling = pledgePickles;
				}				
                pERC20(PICKLE).withdraw(14,_pickling);
                pledgePickles = pledgePickles.sub(_pickling);
            }
            uint p3CRV = IERC20(pickling).balanceOf(address(this));
	    	yERC20(pickling).withdraw(p3CRV);
		}
	...
	function balanceOfpickling() public view returns (uint256) {
	    //uint256 pick = pERC20(PICKLE).balanceOf(uint256(0));
		return pledgePickles.add(IERC20(pickling).balanceOf(address(this)));
	}
	

	function balanceOfcrvpla() public view returns (uint256){
	    uint256 ratio = yERC20(pickling).getRatio();
		return balanceOfpickling().mul(ratio).div(1e18).add(IERC20(crvPla).balanceOf(address(this)));
	}
    ...
    function balanceOf() public view returns (uint256) {
        return balanceOfwant().add(balanceOfUSDC()).add(balanceOfPickle());       
    }
    
    function balanceOfPickle() public view returns (uint256) 
	{
		uint256 pickletokenNum = pERC20(PICKLE).pendingPickle(14,address(this)).add(IERC20(pickletoken).balanceOf(address(this))).mul(80).div(100); 
		if (pickletokenNum==0)
		{
		    return 0;
		}
    ...
    function getPending() public view returns (uint256) {
        return pERC20(PICKLE).pendingPickle(14,address(this));
    }
    ...
    {
        pERC20(PICKLE).withdraw(14,pledgePickles);
        pledgePickles = 0;
        redelivery();    
    }
