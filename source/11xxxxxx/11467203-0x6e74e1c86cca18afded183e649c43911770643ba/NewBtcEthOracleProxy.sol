...
// NewBtcEthOracleProxy.sol
contract NewBtcEthOracleProxy is Ownable {
    ...
    function getChainlinkPrice() external view returns (uint256, uint256) {
        // BTC/USD price feeder
        address btc_usd = 0xF4030086522a5bEEa4988F8cA5B36dbC97BeE88c;
        (, int256 btc_usd_answer, , uint256 btc_usd_timestamp, )
            = AggregatorV3Interface(btc_usd).latestRoundData();
        uint8 btc_usd_Decimal = AggregatorV3Interface(btc_usd).decimals();
        uint256 btc_usd_price = uint256(btc_usd_answer);
        if (btc_usd_Decimal > oracleDecimal) {
            btc_usd_price = btc_usd_price.div(10 ** uint256(btc_usd_Decimal - oracleDecimal));
        } else if (btc_usd_Decimal < oracleDecimal) {
            btc_usd_price = btc_usd_price.mul(10 ** uint256(oracleDecimal - btc_usd_Decimal));
        }

        // ETH/USD price feeder
        address eth_usd = 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419 ;
        (, int256 eth_usd_answer, , uint256 eth_usd_timestamp, )
            = AggregatorV3Interface(eth_usd).latestRoundData();
        uint8 eth_usd_Decimal = AggregatorV3Interface(eth_usd).decimals();
        uint256 eth_usd_price = uint256(eth_usd_answer);
        if (eth_usd_Decimal > oracleDecimal) {
            eth_usd_price = eth_usd_price.div(10 ** uint256(eth_usd_Decimal - oracleDecimal));
        } else if (eth_usd_Decimal < oracleDecimal) {
            eth_usd_price = eth_usd_price.mul(10 ** uint256(oracleDecimal - eth_usd_Decimal));
        }

        uint256 price = 10 ** 18;
        price = price.mul(btc_usd_price).div(eth_usd_price);
        // take the earlier one
        uint256 timestamp = (btc_usd_timestamp < eth_usd_timestamp) ? btc_usd_timestamp : eth_usd_timestamp;
