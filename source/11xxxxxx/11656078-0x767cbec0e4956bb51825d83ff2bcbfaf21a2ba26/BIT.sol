...
contract BIT is ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    function initialize()
        ...
        initializer
    {
        __ERC20_init("Binance Index Token", "BIT");
        _setupDecimals(uint8(DECIMALS));
        __Ownable_init();
        
        _totalSupply = 50000 * 10**9 ;
        _rTotal = (MAX - (MAX % _totalSupply));
        
        _rebaser = _msgSender();
        
        _tFeePercent = 273; //2.7300%
  ...
  function addWhitelistAddresses(address[] calldata _whitelistAddresses) external onlyOwner {
    ...
    function listToken() external onlyOwner() {
        ...
        endSale = true;
        _transfer(address(this), _msgSender(), 7500 * 10**9);
