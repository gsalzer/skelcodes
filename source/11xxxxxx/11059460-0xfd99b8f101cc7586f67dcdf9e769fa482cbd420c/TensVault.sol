...
// Tens Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. Papa bless.
contract TensVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TENSs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTensPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accTensPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. TENSs to distribute per block.
        uint256 accTensPerShare; // Accumulated TENSs per share, times 1e12. See below.
    ...
    // Update the given pool's TENSs allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing TENS governance consensus
    ...
    // View function to see pending TENSs on frontend.
    function pendingTens(uint256 _pid, address _user)
        public
        view
        returns (uint256)
    {
        ...
        uint256 accTensPerShare = pool.accTensPerShare;

        return user.amount.mul(accTensPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the TENS token.
    // ----
    uint256 private tensBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = tens.balanceOf(address(this)).sub(tensBalance);
        
        if(newRewards > 0) {
            tensBalance = tens.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 tensRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        tensRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 tensRewardFee = tensRewardWhole.mul(DEV_FEE).div(10000);
        uint256 tensRewardToDistribute = tensRewardWhole.sub(tensRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(tensRewardFee);

        pool.accTensPerShare = pool.accTensPerShare.add(
            tensRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to TensVault for TENS allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accTensPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accTensPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from TensVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid, from); // Update balances of from this is not withdrawal but claiming TENS farmed
    ...
    function updateAndPayOutPending(uint256 _pid, address from) internal {
        ...
        uint256 pending = pendingTens(_pid, from);

        if(pending > 0) {
            safeTensTransfer(from, pending);
        }
    ...
    // Safe tens transfer function, just in case if rounding error causes pool to not have enough TENSs.
    function safeTensTransfer(address _to, uint256 _amount) internal {

        uint256 tensBal = tens.balanceOf(address(this));
        
        if (_amount > tensBal) {
            tens.transfer(_to, tensBal);
            tensBalance = tens.balanceOf(address(this));

        } else {
            tens.transfer(_to, _amount);
            tensBalance = tens.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 tensBal = tens.balanceOf(address(this));
        if (pending_DEV_rewards > tensBal) {

            tens.transfer(devaddr, tensBal);
            tensBalance = tens.balanceOf(address(this));

        } else {

            tens.transfer(devaddr, pending_DEV_rewards);
            tensBalance = tens.balanceOf(address(this));
    ...
    // allowing TENS governance token holders to do this functions.
