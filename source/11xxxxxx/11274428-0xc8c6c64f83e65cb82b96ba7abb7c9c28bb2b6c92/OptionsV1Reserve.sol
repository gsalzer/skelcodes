...
contract OptionsV1Reserve {
    ...
    OptionsV1Library public constant LIB = OptionsV1Library(0x5854a86B67F7EC80DAD9083B98758b01049b4532);
    OptionsV1Feeds public constant FEED = OptionsV1Feeds(0x73353801921417F465377c8d898c6f4C0270282C);
    ...
    function sp(address tokenIn, address tokenOut) public view returns (uint) {
        if (tokenIn == WETH) {
            return FEED.current(WETH, 1e18, tokenOut);
        } else {
            uint _weth = FEED.current(tokenIn, uint(10)**IERC20(tokenIn).decimals(), WETH);
            return FEED.current(WETH, _weth, tokenOut);
        }
    }
    
    function quote(address tokenIn, uint amountIn, address tokenOut) public view returns (uint) {
        return FEED.current(tokenIn, amountIn, tokenOut);
    }
    
    function vol(address token) public view returns (uint) {
        return LIB.rVol(token, WETH, 24, 2);
    }
    
    function fees(address token) public view returns (uint, uint) {
        (uint call, uint put) = LIB.quote(token, WETH, 7);
        return (FEED.current(WETH, call, DAI), FEED.current(WETH, put, DAI));
    }
    ...
    function fee(address payment, address token, uint amount, uint st, uint t, uint optionType) public view returns (uint) {
       (uint _call, uint _put) = LIB.quotePrice(token, WETH, t, sp(token, payment), st);
