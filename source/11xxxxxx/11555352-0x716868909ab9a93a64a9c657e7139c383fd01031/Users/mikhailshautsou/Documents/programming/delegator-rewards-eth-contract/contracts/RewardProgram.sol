...
contract RewardProgram is Ownable {
    ...
    mapping(address => mapping(string => User)) private ethereumAddressOnSidechainAddress;

    mapping(string => mapping(address => User)) private sidechainAddressOnEthereumAddress;
    ...
    function sidechainUser(address ethereumAddress, string memory target) public view returns (User memory) {
        return ethereumAddressOnSidechainAddress[ethereumAddress][target];
    }

    function ethereumUser(address ethereumAddress, string memory target) public view returns (User memory) {
        return sidechainAddressOnEthereumAddress[target][ethereumAddress];
    }
    ...
    function linkAddresses(string memory target, bytes memory signature, string memory message) public {
        User memory user = User(target, signature, message);
        ethereumAddressOnSidechainAddress[msg.sender][target] = user;
        sidechainAddressOnEthereumAddress[target][msg.sender] = user;
