...
contract DeflationaryERC20 is Context, IERC20, Ownable {
    ...
    */
    ...
    uint8 public constant seFee = 100; 
    ...
    function calculateAmountsAfterFee(
        ...
        if(UniSwapReciever[recipient] && UniSwapSaleEnds){
            ...
            uint256 sfee = 10; //10% fee default if time not met in loop. 
            for(uint i=1; i < 10; i++){
                if(timeNow <= UniSwapSaleEndsTime.add(i * 5 minutes)){
                    sfee = amount.mul(seFee - (i*10)).div(100);
...
contract Vitamin is DeflationaryERC20 {

    constructor() DeflationaryERC20("Vitamin", "VTMM") {
        
        _transferToOwner(msg.sender, 100000000e18);
    }
