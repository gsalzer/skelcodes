...
contract MintUtilities {
    ...
    function _swapETHForUSDC(
        ...
        // Get the estimated value of USDC back
        (uint256 estimated_USDC, uint256[] memory distribution_ETH_USDC) = OneSplit_Inst.getExpectedReturn(
            ETH_ERC20,
            USDC_ERC20,
            amountETH,
            100, 
        ...
        // Make sure enough USDC was estimated
        require(estimated_USDC >= usdc_out_min, "[ETH -> USDC getExpectedReturn] Not enough USDC expected");

        // Swap some ETH for USDC
        (uint256 received_USDC) = OneSplit_Inst.swapWithReferral(
            ETH_ERC20,
            USDC_ERC20,
            estimated_USDC,
            usdc_out_min,
            ...
            0,
            REFERRAL_REWARD_ADDRESS,
            max_slippage
        );
        
        // Make sure enough USDC was recieved
        require(received_USDC >= usdc_out_min, "[ETH -> USDC swapWithReferral] Not enough USDC received from swap");
    ...
    function _swapUSDCForFXS(
        ...
        uint256 fxs_for_mint,
        uint256 max_slippage
    ) internal notPaused returns (uint256) {
        // Get the estimated value of FXS back
        (uint256 estimated_FXS, uint256[] memory distribution_USDC_FXS) = OneSplit_Inst.getExpectedReturn(
            USDC_ERC20,
            FXS_ERC20,
            received_USDC.sub(usdc_for_mint),
            100, 
            0
        );

        // Make sure enough FXS was estimated
        require(estimated_FXS >= fxs_out_min, "[USDC -> FXS getExpectedReturn] Not enough FXS expected");

        // Swap some USDC for FXS
        (uint256 received_FXS) = OneSplit_Inst.swapWithReferral(
            USDC_ERC20,
            FXS_ERC20,
            estimated_FXS,
            fxs_out_min,
        ...
        require(received_FXS >= fxs_for_mint, "[USDC -> FXS swapWithReferral] Not enough FXS received from swap");
    ...
    function ethSwapToMintFF(
        ...
        // =================== ETH -> USDC via 1inch ===================
        (uint256 received_USDC) = _swapETHForUSDC(amountETH, usdc_out_min, max_slippage);

        // =================== USDC -> FXS via 1inch ===================
        (uint256 received_FXS) = _swapUSDCForFXS(received_USDC, fxs_out_min, usdc_for_mint, fxs_for_mint, max_slippage);
