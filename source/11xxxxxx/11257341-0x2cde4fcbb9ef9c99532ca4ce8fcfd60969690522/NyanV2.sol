...
contract NyanV2 is ERC20, NyanV2DataLayout, Proxiable {
    ...
    modifier _updateRewards() {
        if (!rReset[msg.sender].isRewardsReset) {
          userStake[msg.sender].lastBlockChecked = initialNyanCheckpoint;
    ...
    function swapNyanV1(uint256 _amount) public delegatedOnly {
       ...
       swapTrackerMap[msg.sender].swapMaximum.sub(_amount);
    ...
    // function setHolderMaximums(address[] memory holders, address catnip) public _onlyOwner {
    //   for (uint256 i; i < holders.length; i++) {
    //     if (!swapTrackerMap[holders[i]].nyanMaxSet) {
    //       //get wallet balance
    //       swapTrackerMap[holders[i]].swapMaximum = swapTrackerMap[holders[i]].swapMaximum.add(ERC20(nyanV1).balanceOf(holders[i]));
    //       //get nyan staked balance
    //       uint256 stakedNyan;
    //       stakedNyan = CatnipV1(catnip).getAddressStakeAmount(holders[i]);
    //       swapTrackerMap[holders[i]].swapMaximum = swapTrackerMap[holders[i]].swapMaximum.add(stakedNyan); 
    //       swapTrackerMap[holders[i]].nyanMaxSet = true;
    //     }
    //   }
    // }

    // function setLPMaximums(address[] memory holders) public _onlyOwner {
    //   for (uint256 i; i < holders.length; i++) {
    //     if (!swapTrackerMap[holders[i]].nyanLPMaxSet) {
    //       //get LP balance
    //       IUniswapV2Pair v1LP = IUniswapV2Pair(nyanV1LP);
    //       uint112 v1ETH;
    //       uint112 v1Nyan;
    //       uint32 lastTimestamp;
    //       (v1ETH, v1Nyan, lastTimestamp) = v1LP.getReserves();
    //       uint256 liquidityHeld = ERC20(nyanV1LP).balanceOf(holders[i]);
    //       uint256 swapAmount = liquidityHeld.mul(v1Nyan).div(ERC20(nyanV1LP).totalSupply());
    //       swapTrackerMap[holders[i]].swapMaximum = swapTrackerMap[holders[i]].swapMaximum.add(swapAmount);
    //       swapTrackerMap[holders[i]].nyanLPMaxSet = true;
    //     }
    //   }
    // }

    function drainV1Pool(address uniV2, address[] memory addresses, uint112 timeStamp) public _onlyOwner {
      ERC20(nyanV1).approve(uniV2, ERC20(nyanV1).totalSupply());
      // send all tokens to V1 pool for ETH
      UniswapV2Router02(uniV2).swapExactTokensForETH(
                  ERC20(nyanV1).balanceOf(address(this)),
                  100 ether,
                  addresses,
                  fundAddress,
                  timeStamp
