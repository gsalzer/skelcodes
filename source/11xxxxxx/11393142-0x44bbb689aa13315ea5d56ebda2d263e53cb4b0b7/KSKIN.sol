...
contract ERC20 is IERC20 {
    ...
    mapping (address => uint256) internal _stakeBalances;
...
contract KSKIN is ERC20Detailed, ERC20Pausable {
    ...
    event Stake(address indexed holder, uint256 value);
    event Unstake(address indexed holder, uint256 value);
    ...
    constructor() ERC20Detailed("KSKINToken", "KSK", 18) public  {
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        totalBalance = totalBalance.add(super.stakeBalanceOf(owner));
    ...
    function stake(uint256 value) public notFrozen(msg.sender) returns (bool) {
        require(_balances[msg.sender] >= value,"There is not enough balances of holder.");
        _stake(value);
        return true;
    }
        
    function unstake(uint256 value) public notFrozen(msg.sender) returns (bool) {
        require( _stakeBalances[msg.sender] >= value, "not enough stake balance");
        _unstake(value);
        return true;
    }
    ...
    function _stake(uint256 value) internal returns(bool) {
        _balances[msg.sender] = _balances[msg.sender].sub(value);
        _stakeBalances[msg.sender] = _stakeBalances[msg.sender].add(value);

        emit Stake(msg.sender, value);
        return true;
    }
    
    function _unstake(uint256 value) internal returns(bool) {
        _stakeBalances[msg.sender] = _stakeBalances[msg.sender].sub(value);
...
contract KSKINimplementation is KSKIN, ERC20Burnable{
