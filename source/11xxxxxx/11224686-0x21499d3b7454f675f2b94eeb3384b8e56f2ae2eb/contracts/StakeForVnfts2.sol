...
contract StakeForVnfts2 is Roles {
    ...
    uint256 public musePrice = 50 * 1 ether;
    ...
    event MusePriceOfvnftChanged(uint256 newAmount);

    constructor(address _vNFT, address _museToken) public {
        vNFT = IERC721(_vNFT);
        museToken = IMuseToken(_museToken);
    }
    ...
    function changeMusePriceOfNFT(uint256 _newAmount) external onlyOperator {
        musePrice = _newAmount;
        emit MusePriceOfvnftChanged(_newAmount);
    }
