...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0xaf0d782cCbE0479129Ca95Ea15FBC961c282DBE0;
...
contract Staker is Poolable{
    ...
    function lpToken() public view returns (address){
        return Uniswap(FACTORY).getPair(apymAddress, WETHAddress);
    }
    ...
    function stake(address payable ref) public payable{
        ...
        address poolAddress = Uniswap(FACTORY).getPair(apymAddress, WETHAddress);
        uint ethAmount = IERC20(WETHAddress).balanceOf(poolAddress); //Eth in uniswap
        uint tokenAmount = IERC20(apymAddress).balanceOf(poolAddress); //token in uniswap

        uint toMint = (address(this).balance.mul(tokenAmount)).div(ethAmount);
        IERC20(apymAddress).mint(address(this), toMint);

        uint poolTokenAmountBefore = IERC20(poolAddress).balanceOf(address(this));

        uint amountTokenDesired = IERC20(apymAddress).balanceOf(address(this));
        IERC20(apymAddress).approve(UNIROUTER, amountTokenDesired ); //allow pool to get tokens
        Uniswap(UNIROUTER).addLiquidityETH{ value: address(this).balance }(apymAddress, amountTokenDesired, 1, 1, address(this), INF);
    ...
    function withdrawRewardTokens(uint amount) public {
        ...
        IERC20(apymAddress).mint(msg.sender, amount);
    }
    ...
    function ethtimeCalc(uint apym) internal view returns(uint){
        return ( apym.mul(31557600 * DECIMAL) ).div( rewardValue() );
    }
}
