...
// ------------ Listing Tokens ---------------------------------------
    ...
    function revoke_darkListing(address _darklisted_token) public onlyOwner {
        require(_darklisted_token != address(0), "Cannot be adddress(0)");
...
// ------------ darklisted_ Rewards Logics--------------------------------
    ...
    function getReward() public {
        for(uint d = 0; d< darklisted_token.length; d++) {
            getReward_For_All(msg.sender, darklisted_token[d]);
        }
    }
...
//--------------- 200 Addresses a list -----------------
    ...
    function sort_lists() public {
        ...
        if (darklisted_reward_account_list.length >= 2) {
            
             for(uint i = 0; i< darklisted_reward_account_list.length.div(2).add(1); i++) {

                 if (i == darklisted_reward_account_list.length.div(2)) {
                     
                     for(uint j = 0; j< darklisted_reward_account_list.length.sub(i.mul(2)); j++){
                         
                         darklisted_reward_account_list_indexed[i].push(darklisted_reward_account_list[i.mul(2).add(j)]);
                     }
                 } else {
