...
contract MintThenUni is ICallee, DydxFlashloanBase {
    ...
    function callFunction(
        ...
        IERC20(cust_params.token).approve(cust_params.routerAddress, (cust_params.collatForFXSUni).mul(110).div(100));

        address[] memory USDC_FXS_PATH = new address[](2);
        USDC_FXS_PATH[0] = USDC_ADDRESS;
        USDC_FXS_PATH[1] = FXS_ADDRESS;
        ...
        IERC20(cust_params.token).approve(cust_params.poolAddress, (cust_params.collatForMint).mul(110).div(100));

        // Approve the FXS for the FraxPool
        IERC20(FXS_ADDRESS).approve(cust_params.poolAddress, (cust_params.fxsAmountForMint).mul(110).div(100));
        ...
        IERC20(FRAX_ADDRESS).approve(cust_params.routerAddress, (cust_params.expectedFrax).mul(110).div(100));
