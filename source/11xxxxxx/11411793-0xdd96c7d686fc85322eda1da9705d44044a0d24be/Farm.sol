...
contract Emoji is ERC20Mintable, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _presale_address) public {
        ...
        symbol = "EMOJI";
        decimals = 18;
        _mint(_presale_address, 500 * (10 ** 18));
    }
...
contract Farm is Ownable {
    ...
    Emoji public emoji; //white emoji
    IUniswapV2Pair public cEmoji; //colored emoji
    ...
    constructor(uint256 _spawnRate, uint256 _withdrawRate, uint256 _timeLock, address _emoji, address _cEmoji, address _token) public {
        today = now / 1 days;
        spawnRate = _spawnRate;
        withdrawRate = _withdrawRate;
        timeLock = _timeLock;
        emoji = Emoji(_emoji);
        cEmoji = IUniswapV2Pair(_cEmoji);
        token = ERC20(_token);
    }
    ...
    function totalValue() public view returns(uint256) {
        return cEmoji.balanceOf(address(this));
    }
    ...
    function deposit(uint256 amount) external returns (uint256 share) {
        ...
        require(cEmoji.transferFrom(msg.sender, address(this), amount));
    }
    ...
    function withdraw(address to, uint256 share) external returns (uint256 amount) {
        ...
        amount = share.mul(totalValue()).div(totalSupply()).mul(19).div(20);
        ...
        require(cEmoji.transfer(to, amount));
    }

    function rescueToken(ERC20 _token, uint256 _amount) onlyOwner public {
        require(_token != ERC20(address(cEmoji)));
    ...
    function breed() external {
        ...
        uint256 emojiPairAmount = emoji.balanceOf(address(cEmoji));
        uint256 tokenPairAmount = token.balanceOf(address(cEmoji));
        uint256 newEmoji = emojiPairAmount.mul(spawnRate).div(1e18);
        uint256 amount = UniswapV2Library.getAmountOut(newEmoji, emojiPairAmount, tokenPairAmount);

        emoji.mint(address(cEmoji), newEmoji);
        if(address(emoji) < address(token))
            cEmoji.swap(0, amount, address(this), "");
        else
            cEmoji.swap(amount, 0, address(this), "");
        token.transfer(address(cEmoji), amount);
        emoji.mint(address(cEmoji), newEmoji);
        cEmoji.mint(address(this));
    }
}
