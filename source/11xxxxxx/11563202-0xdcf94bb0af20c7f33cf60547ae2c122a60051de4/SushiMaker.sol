...
contract SushiMaker {
    ...
    modifier notPair(address token) {
        (bool success, bytes memory data) = token.staticcall(abi.encodeWithSelector(IUniswapV2Pair.factory.selector));
        require(!success || abi.decode(data, (address)) != address(factory), "cannot convert a pair");
        _;
    }
    ...
    function convert(address token0, address token1) notPair(token0) notPair(token1) public {
