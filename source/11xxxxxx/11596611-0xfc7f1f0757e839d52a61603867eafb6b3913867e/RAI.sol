...
contract RAI is Context, IERC20, Ownable {
    ...
    string private _name = "Radiance Protocol";
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "RAI: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'RAI: The contract it self cannot be excluded');
        require(!_isExcluded[account], "RAI: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "RAI: Account is already included");
