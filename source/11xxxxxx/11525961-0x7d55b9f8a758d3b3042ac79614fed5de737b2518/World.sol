...
contract World is ERC20Burnable, Ownable {

    IUniswapV2Factory internal uniswapFactory = IUniswapV2Factory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);

    IUniswapV2Router internal uniswapRouter = IUniswapV2Router(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    ...
    public
    Ownable()
    ERC20("destroyer.market", "WORLD")
    {
        _mint(msg.sender, 100 * 10**18); // sale
        _mint(msg.sender, 900 * 10**18); // farming rewards
        uniswapPair = createUniswapPairMainnet();
        setWhitelistedSender(msg.sender, true);
        setWhitelistedSender(uniswapPair, true);
    }

    function createUniswapPairMainnet() public virtual returns (address) {
        require(uniswapPair == address(0), "WORLD: pool already exists");
        return uniswapFactory.createPair(
    ...
    function setWhitelistedRecipient(address _recipient, bool _whitelisted) public onlyOwner {
        whitelistedRecipients[_recipient] = _whitelisted;
    }    

    function _isWhitelistedRecipient(address _recipient) internal view returns (bool) {
    ...
    override 
    {
        if (!_isWhitelistedSender(_sender) && !_isWhitelistedRecipient(_recipient)) {
            uint256 burnAmount = _amount.div(20);
