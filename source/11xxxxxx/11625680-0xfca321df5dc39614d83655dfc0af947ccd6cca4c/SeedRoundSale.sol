...
interface IStakeContract{
    ...
    function SetRewardClaimDate(uint256 _rewardClaimDate) external returns(bool);
}
...
contract SeedRoundSale is Owned{
    ...
    constructor(address _tokenAddress, address _stakingAddress) public{
        ...
        wbtcPriceFeed = AggregatorV3Interface(0xF4030086522a5bEEa4988F8cA5B36dbC97BeE88c);
    ...
    receive() external payable {
        uint256 tokens = getTokenAmount(msg.value, 1);
    ...
    function buyViaUSDT(uint256 amount) external {
        address USDT = 0xdAC17F958D2ee523a2206206994597C13D831ec7;
        require(IERC20(USDT).transferFrom(msg.sender, owner, amount), "transfer failed"); 
        
        uint256 tokens = amount; // 1:1
        tokens = tokens.mul(10 ** 12); //RBT has 12 extra decimals than USDT
        _preValidatePurchase(msg.sender, tokens);
        totalTokensSold = totalTokensSold.add(tokens);
        purchasedTokens[msg.sender].purchased = purchasedTokens[msg.sender].purchased.add(tokens);
        require(IStakeContract(stakingAddress).StakeTokens(msg.sender, tokens), "token stake unsuccessful");
    ...
    function buyViaWBTC(uint256 amount) external {
        address WBTC = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599;
        require(IERC20(WBTC).transferFrom(msg.sender, owner, amount), "transfer failed"); 
        
        uint256 tokens = getTokenAmount(amount, 2);
        _preValidatePurchase(msg.sender, tokens);
        totalTokensSold = totalTokensSold.add(tokens);
        purchasedTokens[msg.sender].purchased = purchasedTokens[msg.sender].purchased.add(tokens);
        require(IStakeContract(stakingAddress).StakeTokens(msg.sender, tokens), "token stake unsuccessful");
    ...
    function getTokenAmount(uint256 amount, uint256 isEther) public view returns(uint256){
        int latestPrice;
        uint256 decimals;
        
        if(isEther == 1){
            latestPrice = getETHLatestPrice(); 
            decimals = 18;
        }
        else{
            latestPrice = getWBTCLatestPrice();
            decimals = 8;
        }
            
        latestPrice = latestPrice / 1e8; // 591$ = 1 ETHER OR 23444 $ = 1 WBTC
     ...
     * Returns the latest price
     */
    function getWBTCLatestPrice() public view returns (int) {
        (
            uint80 roundID, 
            int price,
            uint startedAt,
    ...
    function StartCliff() external onlyOwner{
        ...
        require(IStakeContract(stakingAddress).SetRewardClaimDate(tokenUnLockDate), "failed to set reward claim date in staking");
    ...
    function withdrawTokens() external {
        ...
        uint256 monthsPassed = (block.timestamp.sub(tokenUnLockDate)).div(30 days);
        if(monthsPassed > 12)
            monthsPassed = 12;
