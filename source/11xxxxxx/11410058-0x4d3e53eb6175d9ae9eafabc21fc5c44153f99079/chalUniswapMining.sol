...
// File: contracts/chalUniswapMiningStorage.sol
...
contract chalUniswapMiningStorage {
  using SafeMath for uint256;

  bool constant public ischalUniswapMining = true;
  ...
  uint public totalMiningBlockNum = 589952;
  uint public endMiningBlockNum = startMiningBlockNum + totalMiningBlockNum;
  uint public chalPerBlock = 2644280000000000;
...
// File: contracts/chalUniswapMining.sol
...
contract chalUniswapMining is Ownable, Pausable, chalUniswapMiningStorage {
  
  event Staked(address indexed user, uint256 amount, uint256 total, uint256 lockedUntil);
  event Unstaked(address indexed user, uint256 amount, uint256 total);
  event Claimedchal(address indexed user, uint amount, uint total);

  function initiate(uint _startMiningBlocknum, uint _totalMiningBlockNum, uint _chalPerBlock, ERC20 _chal, ERC20 _lp) public onlyOwner{
  ...
  function getMiningState(uint _blockNum) public view returns(uint, uint){
    ...
    if (deltaBlocks > 0 && totalStaked > 0) {
        uint chalAccrued = deltaBlocks.mul(chalPerBlock);
        uint ratio = chalAccrued.mul(1e18).div(totalStakedPower); //multiple ratio to 1e18 to prevent rounding error
  ...
  // @notice claim CHAL based on current state 
  function claimchal() public whenNotPaused {
      ...
      if(claimablechal > 0){
          ...
          stakerClaimed[msg.sender] = stakerClaimed[msg.sender].add(claimablechal);
          totalClaimed = totalClaimed.add(claimablechal);
          chalToken.transfer(msg.sender, claimablechal);
          emit Claimedchal(msg.sender, claimablechal, stakerClaimed[msg.sender]);
          
      }
  }
  
  // @notice calculate claimable chal based on current state
  function claimablechal(address _address) public view returns(uint){
      ...
      uint chalDelta = deltaIndex.mul(stakerPower[_address]).div(1e18);
    ...
    // @notice admin function to send CHAL to external address, for emergency use
    function sendchal(address _to, uint _amount) public onlyOwner{
      chalToken.transfer(_to, _amount);
    }
