...
// File: contracts/Dynamite.sol

pragma solidity ^0.6.0;



contract Dynamite is ERC20UpgradeSafe, OwnableUpgradeSafe {

    address[] public canTransfer;

    function initialize() public initializer {
        canTransfer.push(msg.sender);
        ERC20UpgradeSafe.__ERC20_init("Dynamite", "DNMT");
...
contract GiveawayDNMT is OwnableUpgradeSafe {
    Dynamite public token;

    constructor(Dynamite _token) public {
        token = _token;
    }

    function swap(address[] calldata holders) public onlyOwner {
        for(uint i = 0; i<holders.length; i++){
            token.transfer(holders[i],1000000000000000000);
        }
    }
