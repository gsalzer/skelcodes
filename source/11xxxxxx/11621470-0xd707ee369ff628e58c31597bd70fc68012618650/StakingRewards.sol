...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    uint256 private pool_weight; // This staking pool's percentage of the total PEGS being distributed by all pools, 6 decimals of precision
    ...
    constructor(
        ...
        rewardRate = 380517503805175038; // (uint256(12000000e18)).div(365 * 86400); // Base emission rate of 12M PEGS over the first year
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            uint256 minting_fee_amount = reward.mul(minting_fee).div(MULTIPLIER_BASE);
            ...
            totalClaimedRewards[msg.sender] = totalClaimedRewards[msg.sender].add(reward.sub(minting_fee_amount));
            emit RewardPaid(msg.sender, reward);
        }
    }
    
    function getUserTotalRewards(address _addr) public view returns(uint256 userTotalClaimedRewards) {
    ...
    function retroCatchUp() internal {
        ...
        require(rewardRate.mul(rewardsDuration).mul(crBoostMultiplier()).mul(num_periods_elapsed + 1).div(PRICE_PRECISION) <= balance, "Not enough PEGS available for rewards!");
    ...
    function setMintingFee(uint256 _minting_fee) external onlyByOwnerOrGovernance {
        minting_fee = _minting_fee;
    }

    function setMintingFeeAddress(address _feeAddress) external onlyByOwnerOrGovernance {
