...
contract ZenterestPriceFeed is TwoStageOwnable {
    ...
    {
        return _prices[token].value.toTuple();
    }
        ...
        returns (uint256 updatedPricesCount)
    {
        ...
        for (uint256 i = 0; i < updatesCount; i++) {
            ...
            checkUpdatingTime(update.updatedAt);
    ...
    function _delegatedPriceUpdate(DelegatedPriceUpdate memory update) internal {
        bytes32 hash_ = keccak256(abi.encodePacked(update.token, update.newPriceMantissa, update.updatedAt));
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 prefixedHash = keccak256(abi.encodePacked(prefix, hash_));
        address signer = ecrecover(prefixedHash, update.signature.v, update.signature.r, update.signature.s);
