...
/**
 ...
 Machine with ETH. Machine version 2
 */
contract GachaMachineETH is Ownable, ERC1155Holder {
    ...
    uint256 public totalAmountForBuying;
    ...
    address payable public artistAccount;
    address payable public dokiBuybackAccount;
    uint256 public forArtistRate = 65;
    uint256 public forDokiBuybackRate = 10;
    ...
    event RunMachineSuccessfully(address account, uint256 times, uint256 value);
    ...
    constructor(uint256 _machineId,
                ...
                address payable _buybackManager,
        ...
        administrator = owner();
        dokiBuybackAccount = _buybackManager;
        artistAccount = _artistAccount;
        _salt = uint256(keccak256(abi.encodePacked(_momijiToken, block.timestamp))).mod(10000);
    }
    ...
    function runMachine(uint256 userProvidedSeed, uint256 times) public payable onlyHuman unbanned {
        ...
        require(msg.value == playOncePrice.mul(times), "ETH amount is not correct.");
        _createARound(times);
    ...
    function _transferToken(uint256 amount) private {
        ...
        artistAccount.transfer(forArtistAmount);
        totalAmountForArtist = totalAmountForArtist.add(forArtistAmount);

        // For doki buyback
        uint256 forDokiBuybackAmount = amount.mul(forDokiBuybackRate).div(100);
        dokiBuybackAccount.transfer(forDokiBuybackAmount);
        totalAmountForBuying = totalAmountForBuying.add(forDokiBuybackAmount);

        uint256 forDevAmount = amount.sub(forArtistAmount).sub(forDokiBuybackAmount);
        totalAmountForDev = totalAmountForDev.add(forDevAmount);
        uint256 devAccountCount = _devAccountSet.length();
        uint256 everyDevAmount = forDevAmount.div(devAccountCount);
        for (uint256 i = 0; i < devAccountCount; i ++) {
            address payable devAccount = payable(_devAccountSet.at(i));
            devAccount.transfer(everyDevAmount);
        }
    }
    ...
    function _distributePrize() private {
        for (uint i = 0; i < gameRounds[msg.sender].times; i ++) {
            ...
            if (mintedCardAmountWithId[cardId] > 0) {
                momijiToken.safeTransferFrom(address(this), msg.sender, cardId, 1, "Add to Machine");
    ...
    function changeBuybackAddress(address payable account) public onlyAdministrator {
        dokiBuybackAccount = account;
    }

    function changeForArtistRate(uint256 newRate) public onlyAdministrator {
        forArtistRate = newRate;
    }

    function changeForDokiBuybackRate(uint256 newRate) public onlyAdministrator {
        forDokiBuybackRate = newRate;
    }

    function changeArtisAccount(address payable account) public onlyOwner {
