...
abstract contract IStaking {
    ...
    function stakeFor(address user, uint256 amount, bytes calldata data) external virtual;
 ...
 */
contract Cradle is IStaking, OwnableUpgradeSafe {
    ...
    function setup(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
                uint256 startBonus_, uint256 bonusPeriodSec_, uint256 initialSharesPerToken) external onlyOwner {
    ...
    function stake(uint256 amount, bytes calldata data) external override  {
        _stakeFor(_msgSender(), _msgSender(), amount);
    }
    ...
    function stakeFor(address user, uint256 amount, bytes calldata data) external override {
        _stakeFor(_msgSender(), user, amount);
    }
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        require(totalStakedFor(_msgSender()) >= amount,
        ...
        // 1. User Accounting
        UserTotals storage totals = _userTotals[_msgSender()];
        Stake[] storage accountStakes = _userStakes[_msgSender()];
        ...
        // interactions
        require(_stakingPool.transfer(_msgSender(), amount),
            'Cradle: transfer out of staking pool failed');
        require(_unlockedPool.transfer(_msgSender(), rewardAmount),
            'Cradle: transfer out of unlocked pool failed');

        emit Unstaked(_msgSender(), amount, totalStakedFor(_msgSender()), "");
    ...
    function lockTokens(uint256 amount, uint256 durationSec) external onlyOwner {
        ...
        require(_lockedPool.token().transferFrom(_msgSender(), address(_lockedPool), amount),
