...
contract StrategyCompoundUNI {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    address constant public want = address(0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984); // UNI
    address constant public cUNI = address(0x35A18000230DA775CAc24873d00Ff85BccdeD550); // compound cUNI
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            IERC20(want).safeApprove(cUNI, 0);
            IERC20(want).safeApprove(cUNI, _want);
            cERC20(cUNI).mint(_want);
        }
    }
    ...
    function _withdrawAll() internal {
        _withdrawSome(balanceOfCUNI());
    }

    function _withdrawSome(uint256 _amount) internal returns (uint) {
        uint _before = IERC20(want).balanceOf(address(this));
        cERC20(cUNI).redeemUnderlying(_amount);
    ...
    function balanceOfCUNI() public view returns (uint) {
        return IERC20(cUNI).balanceOf(address(this)).mul(cERC20(cUNI).exchangeRateStored()).div(1e18);
    }

    function getExchangeRate() public view returns (uint) {
        return cERC20(cUNI).exchangeRateStored();
    }
