...
contract Vault {
    ...
    constructor(address _tokenAddress, address _beneficiary, uint256 _releasetime) public {
...
contract YUIToken is Ownable, StandardToken {
    ...
    event VaultCreated(address indexed _vault, address indexed _beneficiary);
    ...
    constructor() public {
        ...
        crowdSaleContract = 0x9905fD0351265790f1465dec86eC476cB48f61f1;                // contract for ICO tokens
        ...
        uint256 teamReleaseTime = now + 180 days;                                      // lock team tokens for 6 months
        uint256 marketingReleaseTime = now + 90 days;                                  // lock marketing tokens - 1k tokens for 3 months
        Vault teamVault = new Vault(address(this), teamWallet, teamReleaseTime);               // team vault contract
        emit VaultCreated(address(teamVault), teamWallet);
        Vault marketingVault = new Vault(address(this), marketingWallet, marketingReleaseTime);   // marketing vault contract
        emit VaultCreated(address(marketingVault), marketingWallet);
        
        balances[address(teamVault)] = 3000 * (uint256(10) ** decimals);
        balances[privateSaleWallet] = 1500 * (uint256(10) ** decimals);
        balances[crowdSaleContract] = 5000 * (uint256(10) ** decimals);
        balances[marketingWallet] = 1000 * (uint256(10) ** decimals);
        balances[address(marketingVault)] = 1000 * (uint256(10) ** decimals);
        balances[exchangesLiquidity] = 9000 * (uint256(10) ** decimals);
        balances[stakeWallet] = 7500 * (uint256(10) ** decimals);
        _totalSupply = 28000  * (uint256(10) ** decimals);
        txUnlocked = false;
    }
