...
contract XplosiveEthereum is Ownable, Pausable, Rebasable
{
    ...
    string public name     = "Xplosive EthereumG";
    string public symbol   = "xETHG";
    ...
    */
    ...
    returns (uint256)
    {
        uint256 clEthValue = this.getChainlinkEthDollarValue();
        return this.rebase(now, clEthValue, true);
    }

    function clRebaseWtihMultiplier(uint256 _multiplier)
    external
    onlyRebaser
    whenNotPaused
    returns (uint256)
    {
        uint256 clEthValue = this.getChainlinkEthDollarValue();
        return this.rebase(now, clEthValue.mul(_multiplier), true);
    }

    function getChainlinkEthDollarValue()
    external
    view
    returns (uint256)
    {
        ( uint80 roundId, int price, uint startedAt, uint timeStamp, uint80 answeredInRound ) = priceFeed.latestRoundData();
        return uint256(10e18).divRound(price.toUint256()).mul(10e6);
    }

    function getChainlinkEthDollarValueWithMultiplier(uint256 _multiplier)
    external
    view
    returns (uint256)
    {
        uint256 clEthValue = this.getChainlinkEthDollarValue();
        return clEthValue.mul(_multiplier);
    }
