...
contract Curve_ZapIn_General_V1_10 is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    function _getIntermediate(
        ...
        if (curveReg.isMetaPool(swapAddress)) {
            for (uint8 i = 0; i < 4; i++) {
                if (curveReg.metaPools(poolTokens[i]) != address(0)) {
                    address intermediateSwapAddress = curveReg.metaPools(
                        poolTokens[i]
                    );
                    (bool isUnderlying, uint8 underlyingIndex) = curveReg
                        .isUnderlyingToken(intermediateSwapAddress, fromToken);
                    if (isUnderlying) {
                    ...
                    if (curveReg.isBtcPool(intermediateSwapAddress)) {
            ...
            if (curveReg.isBtcPool(swapAddress)) {
                ...
                (, index) = curveReg.isUnderlyingToken(swapAddress, wbtcToken);
