...
contract NFITokenWrapper {
    ...
    // !! Mainnet NFI-USDT LP TOKEN address
    IERC20 public stakeToken = IERC20(0x3c974e0332e7FB61F9e9dEC1338ba6DFe25dB8D6);
...
contract LpUSDTNFIPool is NFITokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){
        require(amount > 0, "LP-USDT-NFI-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "LP-USDT-NFI-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"LP-USDT-NFI-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "LP-USDT-NFI-POOL: Pool is closed");
        _;
    }
