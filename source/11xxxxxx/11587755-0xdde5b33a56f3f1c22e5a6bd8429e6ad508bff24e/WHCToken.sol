...
contract WHCToken is ERC20Detailed, ERC20, Ownable {
    address foundationWallet = 0x1cC026584Be090C144Ac63d2831a7030E89b18Ad;
    address communityDevWallet = 0x3c2dF1227cd4FEe79d003Eb353031051E5Fe24d7;
    address salesWallet = 0xA2B707db217AB32F7dE646770B6A286eB8A1793A;
    ...
    mapping (uint => uint) public communityDevMap;
    mapping (uint => uint) public salesMap;

    address [] private lockedAddressList;

    constructor() public ERC20Detailed("WhiteHub", "WHC", 8) {  

		foundationMap[1]=1617235200; // 2021-04-01T00:00:00Z
		foundationMap[2]=1625097600; // 2021-07-01T00:00:00Z
		foundationMap[3]=1633046400; // 2021-10-01T00:00:00Z
		foundationMap[4]=1640995200; // 2022-01-01T00:00:00Z
		foundationMap[5]=1648771200; // 2022-04-01T00:00:00Z
		foundationMap[6]=1656633600; // 2022-07-01T00:00:00Z
		foundationMap[7]=1664582400; // 2022-10-01T00:00:00Z
		foundationMap[8]=1672531200; // 2023-01-01T00:00:00Z
		foundationMap[9]=1680307200; // 2023-04-01T00:00:00Z
		foundationMap[10]=1688169600; // 2023-07-01T00:00:00Z

		communityDevMap[1]=1609459200; // 2021-01-01T00:00:00Z
		communityDevMap[2]=1617235200; // 2021-04-01T00:00:00Z
		communityDevMap[3]=1625097600; // 2021-07-01T00:00:00Z
		communityDevMap[4]=1633046400; // 2021-10-01T00:00:00Z
		communityDevMap[5]=1640995200; // 2022-01-01T00:00:00Z

		salesMap[1]=1609459200; // 2021-01-01T00:00:00Z
		salesMap[2]=1612137600; // 2021-02-01T00:00:00Z
		salesMap[3]=1614556800; // 2021-03-01T00:00:00Z
		salesMap[4]=1617235200; // 2021-04-01T00:00:00Z
		salesMap[5]=1619827200; // 2021-05-01T00:00:00Z
		salesMap[6]=1622505600; // 2021-06-01T00:00:00Z
		salesMap[7]=1625097600; // 2021-07-01T00:00:00Z

        totalTokens = 100000000 * 10 ** uint256(decimals());
        _mint(owner(), totalTokens); // total supply fixed at 1 billion token

        for(uint i = 1; i<= 10; i++) {
            transferWithLock(foundationWallet, 2000000 * 10 ** uint256(decimals()), foundationMap[i]);
        }

        for(uint i = 1; i<= 5; i++) {
            transferWithLock(communityDevWallet, 2000000 * 10 ** uint256(decimals()), communityDevMap[i]);
        }

        for(uint i = 1; i<= 7; i++) {
            transferWithLock(salesWallet, 10000000 * 10 ** uint256(decimals()), salesMap[i]);
        }
    }
    ...
    function transferWithLock(address _receiver, uint256 _amount, uint256 _releaseDate) public returns (bool success) {
	    require(msg.sender == foundationWallet || msg.sender == communityDevWallet || msg.sender == salesWallet || msg.sender == owner());
