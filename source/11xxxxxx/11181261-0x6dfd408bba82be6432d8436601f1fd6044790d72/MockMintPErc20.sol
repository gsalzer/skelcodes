...
// File contracts/mock/MockPMintToken.sol


pragma solidity ^0.6.0;

contract MockMintPEther is ERC20 {
    ...
    function mintForMigrate(uint mintTokens) external payable {
        mintInternalForMigrate(msg.value, mintTokens);
    }
    ...
    function mintInternal(uint amount) internal {
        _mint(msg.sender, amount * 2);
    }

    function mintInternalForMigrate(uint amount, uint mintTokens) internal {
    ...
    function doTransferOut(address payable to, uint amount) external {
        to.transfer(amount);
    }

}

contract MockMintPErc20 is ERC20 {

    address public underlying;

    constructor(address _underlying, string memory name, string memory symbol, uint256 initialSupply) public ERC20(name, symbol){
    ...
    function mintForMigrate(uint amount, uint mintTokens) external {
        mintInternalForMigrate(amount, mintTokens);
    }
