...
contract StrategyProxy {
    ...
    function claimRewards(address _gauge) external {
        require(strategies[msg.sender], "!strategy");
        address _rewarded_token = Gauge(_gauge).rewarded_token();
        uint256 _balance = IERC20(_rewarded_token).balanceOf(address(proxy));
        Gauge(_gauge).claim_rewards(address(proxy));
        _balance = (IERC20(_rewarded_token).balanceOf(address(proxy))).sub(_balance);
        if (_balance > 0) {
            proxy.execute(_rewarded_token, 0, abi.encodeWithSignature("transfer(address,uint256)", msg.sender, _balance));
        }
