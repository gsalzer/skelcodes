    ...
    LnRewardCalculator
{
    ...
    function _cancelStaking(address user, uint256 amount) internal {
        ...
        uint256 returnAmount = amount;          //想要unstake的数目
        uint256 newAmount = super.amountOf(user); //用户可stake的数目
        if (newAmount >= amount) {
    ...
    LnRewardCalculator
{
    ...
    function _cancelStaking(address user, uint256 amount) internal {
        ...
        if (newAmount >= amount) {
            ...
            oldStakingAmount = mOldSimpleStaking.stakingBalanceOf(user);
    ...
    LnRewardCalculator
{
    ...
    // event TestCancelStaking(address indexed who, uint256 amount, uint256 newAmount, uint256 blockNb, uint256 oldStaking, uint256 extenStaking, uint256 returnAmount);
    ...
    function _cancelStaking(address user, uint256 amount) internal {
        ...
        uint256 returnAmount = 0;            //最终transfer回用户的数额
        uint256 newAmount = super.amountOf(user);
        uint256 oldStaking = 0;
        uint256 extenStaking = 0;
        if (newAmount >= amount) {
            super._withdraw(blockNb, user, amount);
            returnAmount.add(amount);              //20201114 0433
            amount = 0;
        } else {
            if (newAmount > 0) {
                super._withdraw(blockNb, user, newAmount);
                ...
                returnAmount.add(newAmount);        //20201114 0433
            }
            oldStaking = mStaking.stakingBalanceOf(user);
            if (oldStaking > 0){
                oldStaking = oldStaking.sub(mOldReward[user],"_cancelStaking amount sub overflow");
            }
            extenStaking = mStakingExtension.stakingBalanceOf(user);
            if (extenStaking > 0){
                ...
                if (extenStaking > amount && amount > 0) {
                    ...
                    if (amount > 0){
                        ...
                        returnAmount.add(extenStaking);        //20201114 0433
                        amount = amount.sub(
                            extenStaking,
                            "_cancelStaking amount sub overflow"
                        );
            ...
            if (oldStaking > amount && amount > 0) {
                ...
                if (amount > 0){
                    ...
                    returnAmount.add(oldStaking);        //20201114 0433
                    amount = amount.sub(
                        oldStaking,
                        "_cancelStaking amount sub overflow"
                    );
        ...
        // emit TestCancelStaking(user, amount, newAmount, blockNb, oldStaking, extenStaking, returnAmount);
