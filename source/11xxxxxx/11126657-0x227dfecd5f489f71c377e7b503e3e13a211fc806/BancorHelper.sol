...
contract BancorHelper {
    ...
    function getConverters() public view returns (IConverter[] memory goodPools) {
        ...
        for (uint i = 0; i < smartTokens.length; i++) {
            ...
            converter = getGoodPool(converter);
            if (converter != IConverter(address(0))) {
                goodPoolsCount++;
    ...
    function getGoodPool(IConverter converter) public view returns (IConverter) {
        ...
        if (!ok || data.length == 0) {
            return IConverter(address(0));
        }
        IERC20 firstToken = abi.decode(data, (IERC20));
        if (firstToken.balanceOf(address(converter)) > 0) {
              return converter;
        }
        return IConverter(address(0));
    }
