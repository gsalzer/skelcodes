...
contract EcoceliumDataManager is Initializable {
   ...
   function newOrder(address payable userAddress,string memory _tokenSymbol ,uint _amount,uint _dID, uint _yield, bool _status) external {
        ...
        while(amountLeft>0){
            ...
            if((_status && _yield>iOrder.yield) || (!_status && _yield<iOrder.yield) || nextID[indexID] == uint64(0) || iOrder.amount == 0){
                pushOrder(userAddress, _tokenSymbol, amountLeft, (_dID+1)*6, _yield, _status);
                break;
            } 
            if(iOrder.amount <= amountLeft) {
                    matchOrder(userAddress, _tokenSymbol, amountLeft, (_dID+1)*6, iOrder.yield, nextID[indexID]);
                    amountLeft -= openOrderMap[nextID[indexID]].amount;    
            } else { 
                matchOrder(userAddress, _tokenSymbol, amountLeft, (_dID+1)*6, iOrder.yield, nextID[indexID]);
		pushOrder(iOrder.creator, _tokenSymbol, iOrder.amount-amountLeft, (_dID+1)*6, iOrder.yield, !_status ? true : false);                
		amountLeft=0;
            }
        }
    }
    
    function pushOrder(address payable userAddress,string memory _tokenSymbol ,uint _amount,uint _duration, uint _yield, bool _status) public returns (uint){
    ...
    function poolSorter(uint64 _orderId, string memory _tokenSymbol, bool _status, uint dID) public {        //Status here signifies Insertion if True, and Deletion if false
        ...
        while(!task && (index != uint64(0))) {
            if(_status && 
            ((orderStatus[_orderId] == Status.OPENCREATOR && openOrderMap[nextID[index]].yield >= openOrderMap[_orderId].yield) 
            || (orderStatus[_orderId] == Status.OPENBORROW && openOrderMap[nextID[index]].yield <= openOrderMap[_orderId].yield) 
            || nextID[index] == uint64(0))) {
    ...
    function matchOrder(address payable userAddress, string memory _tokenSymbol ,uint _amount,uint _duration,uint _yield, uint64 _orderId) public    {
    ...
    function cancelOrder(uint64 _orderId) external{
        ...
        if(orderStatus[_orderId]==Status.OPENCREATOR) {
            ...
            for((uint i,uint j)=(0,0); i<users[openOrderMap[_orderId].creator].createdOrders.length; i++) {
                if(users[openOrderMap[_orderId].creator].createdOrders[i] == _orderId || openOrderMap[users[openOrderMap[_orderId].creator].createdOrders[i]].amount == 0) { continue; }
   ...
   /* function updateHeads(string memory token, uint dID, bool status) external {
    ...
    mapping (uint64 => uint64) public nextID; //Linked List Map for All Orders
    mapping (string => uint64[6]) public hDID; // Head ID for Deposit
    mapping (string => uint64[6]) public hBID; // Head ID for Borrow

    function pushHeadOrder(address payable userAddress,string memory _tokenSymbol ,uint _amount,uint _duration, uint _yield, bool _status) public returns (uint64) {
        (uint64 _orderId,uint newAmount,) = _ordersub(_amount, userAddress, _duration, _tokenSymbol);
        openOrderMap[_orderId] = Order({       
                                            creator : userAddress,
                                            duration : _duration,
                                            amount : newAmount,
        ...
        if(_status) {
            orderStatus[_orderId] = Status.OPENCREATOR;
            hDID[_tokenSymbol][(_duration/6)-1] = _orderId; } else {
            orderStatus[_orderId] = Status.OPENBORROW;
            hBID[_tokenSymbol][(_duration/6)-1] = _orderId; }
        return _orderId;
    }
...
contract Ecocelium is Initializable{
  ...
  /*  function initialize(address _owner,address payable EMMaddress,address payable AbacusAddress,address payable EDMaddress, address payable Initaddress)public payable initializer {
        owner = _owner;
        EMM = EcoMoneyManager(EMMaddress);
    ...
    function borrowOrder(address userAddress, string memory rtoken, uint amount, uint duration, uint yield) public {//Rewrite this part
        ...
        EDM.newOrder(msg.sender,EMM.getWrapped(rtoken), amount,duration,yield,false);
    }
