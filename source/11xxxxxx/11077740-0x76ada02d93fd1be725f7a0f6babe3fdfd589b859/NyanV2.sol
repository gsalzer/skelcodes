// File: browser/UniswapV2-nyan.sol
...
contract NyanV2 is ERC20, ERC20DataLayout, Proxiable {
    ...
    modifier _updateRewards() {
        if (userStake[msg.sender].stakedNyanV2LP > 0) {
            uint256 totalLP = ERC20(nyanV2LP).totalSupply().add(ERC20(dNyanV2LP).totalSupply());
            uint256 totalNyanV2Held = balanceOf(address(this));
            uint256 rewardsInDay = totalNyanV2Held.mul(rewardsPercentage).div(10000);
            uint256 rewardsPerBlock = rewardsInDay.div(6500);
            uint256 userStakeAmount = userStake[msg.sender].stakedNyanV2LP.add(userStake[msg.sender].stakedDNyanV2LP);
            uint256 userRewardPerBlock = rewardsPerBlock.mul(userStakeAmount).div(totalLP);
            uint256 blocksClaimed = block.number.sub(userStake[msg.sender].lastBlockChecked);
            userStake[msg.sender].nyanV2Rewards = userStake[msg.sender].nyanV2Rewards.add(userRewardPerBlock.mul(blocksClaimed));
    ...
    function viewNyanRewards(address staker) delegatedOnly public view returns(uint256) {
        ...
        if (userStake[staker].nyanV2Rewards > 0) {
            ...
            uint256 userRewardPerBlock = rewardsPerBlock.mul(userStakeAmount).div(totalLP);
            uint256 blocksClaimed = block.number.sub(userStake[staker].lastBlockChecked);
            currentRewards = userStake[staker].nyanV2Rewards.add(userRewardPerBlock.mul(blocksClaimed));
        }
    ...
    function transfer(address _recipient, uint256 _amount) delegatedOnly public override returns(bool) {
        ...
        super.transfer(_recipient, _amount.sub(transferFee));
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) delegatedOnly public override returns(bool) {
        ...
        super.transferFrom(_sender, _recipient, _amount.sub(transferFee));
    ...
    /** @notice Add NyanV1 and ETH to the contract.
      ...
      */
    function addNyanAndETH(uint256 _nyanAmount) public payable _onlyOwner delegatedOnly {
    ...
    function initializeV2ETHPool() public {
        ...
        ERC20(address(this)).transfer(address(v2LP), ERC20(address(this)).balanceOf(address(this)));
