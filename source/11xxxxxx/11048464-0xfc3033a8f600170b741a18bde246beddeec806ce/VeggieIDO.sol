...
contract VeggieIDO is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public immutable HARDCAP = 65 ether;

    uint256 public TOKENS_PER_ETH = 10;
    ...
    IUNIv2 internal constant uniswap = IUNIv2(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    ...
    function _buyTokens(address _beneficiary, uint256 _amount) internal {
        require(isOpen(), "sale did not start yet!");
        require(!hasEnded(), "it's over frens");
        require(isFcFs() || contributions[_beneficiary].add(_amount) <= whitelists[_beneficiary], "not whitelisted or fcfs");
        ...
        uint256 tokenAmount = _amount.mul(tokensPerEth);
    ...
    function isFcFs() public view returns (bool) {
        return block.timestamp > startTime + 2 hours;
    }
    ...
    function addWhitelists(address[] calldata _addresses, uint256 _cap) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            whitelists[_addresses[i]] = _cap;
        }
    }

    function finalize() external virtual onlyOwner {
        require(hasEnded(), "not over yet");

        uint256 liquidityFraction = 2; // 50%

        uint256 liquidityETH = weiRaised.div(liquidityFraction); // 50%
        ...
        if (tokensToBurn > 0) {
            ...
            tokensToBurn = tokensToBurn.add(HARDCAP.div(liquidityFraction).mul(tokensPerEth).sub(liquidityTokens));
            IVeggie(address(token)).burn(tokensToBurn);
        }

        IVeggie(address(token)).togglePause(false);
