...
contract Balancer {
    ...
    OVERTHEMOON private _tokenContract;
    
    constructor(OVERTHEMOON tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract OVERTHEMOON is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 32000e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _tBurnTotal;

    string private _name = 'OVERTHEMOON';
    string private _symbol = 'OTM';
    ...
    uint256 private _minTokenForAlchemy = 100e9;
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "OVERTHEMOON: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "OVERTHEMOON: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'OVERTHEMOON: We can not exclude Uniswap router.');
        require(account != address(this), 'OVERTHEMOON: We can not exclude contract self.');
        require(account != _rewardWallet, 'OVERTHEMOON: We can not exclude reweard wallet.');
        require(!_isExcluded[account], "OVERTHEMOON: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "OVERTHEMOON: Account is already included");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "OVERTHEMOON: approve from the zero address");
        require(spender != address(0), "OVERTHEMOON: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "OVERTHEMOON: transfer from the zero address");
        require(recipient != address(0), "OVERTHEMOON: transfer to the zero address");
        require(amount > 0, "OVERTHEMOON: Transfer amount must be greater than zero");
        
        if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            require(amount <= _maxTxAmount, "OVERTHEMOON: Transfer amount exceeds the maxTxAmount.");
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_uniswapV2Router)) && !tradingEnabled)
                require(false, "OVERTHEMOON: trading is disabled.");
        }
    ...
    function alchemy() public lockTheSwap {
        require(balanceOf(_msgSender()) >= _minTokenForAlchemy, "OVERTHEMOON: You have not enough OVERTHEMOON to ");
        require(now > _lastAlchemy + _alchemyInterval, 'OVERTHEMOON: Too Soon.');
    ...
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        require(feeDecimals >= 0 && feeDecimals <= 2, 'OVERTHEMOON: fee decimals should be in 0 - 2');
        _feeDecimals = feeDecimals;
        emit FeeDecimalsUpdated(feeDecimals);
    }
    
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'OVERTHEMOON: taxFee should be in 0 - 5');
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'OVERTHEMOON: lockFee should be in 0 - 5');
        _lockFee = lockFee;
        emit LockFeeUpdated(lockFee);
    }
    
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 500000e9 , 'OVERTHEMOON: maxTxAmount should be greater than 500000e9');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
    
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'OVERTHEMOON: minTokenBeforeSwap should be in 50e9 - 25000e9');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'OVERTHEMOON: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 1e9, 'OVERTHEMOON: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setLiquidityRemoveFee(uint256 liquidityRemoveFee) external onlyOwner() {
        require(liquidityRemoveFee >= 1 && liquidityRemoveFee <= 10 , 'OVERTHEMOON: liquidityRemoveFee should be in 1 - 10');
    ...
    function _setAlchemyCallerFee(uint256 alchemyCallerFee) external onlyOwner() {
        require(alchemyCallerFee >= 1 && alchemyCallerFee <= 15 , 'OVERTHEMOON: alchemyCallerFee should be in 1 - 15');
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        require(poolAddress != address(0), "OVERTHEMOON: Pool address is zero.");
        require(pairTokenAddress != address(0), "OVERTHEMOON: Pair token address is zero.");
        require(pairTokenAddress != address(this), "OVERTHEMOON: Pair token address self address.");
        require(pairTokenAddress != currentPairTokenAddress, "OVERTHEMOON: Pair token address is same as current one.");
