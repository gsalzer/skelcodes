...
contract BerezkaPriceTracker {
        ...
        returns (int256) 
    {
        if (berezkaPriceOverride.hasOverride(_token)) {
            ...
            return result;
        } else if (berezkaPriceOverride.hasTokenOverride(_token)) {
            (address token, int256 amount) = berezkaPriceOverride.replaceToken(_token, _amount);
            return getExchangeTokenPrice(amount, token);
        } else {
