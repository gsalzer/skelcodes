...
//import "./XChangerV2.0.sol";
//import "./XChanger.sol";



//import "./utils/SafeERC20.sol";
//import "./utils/UniswapV2Lib.sol";
...
contract ValueHolder is Ownable, CHIBurner, XChangerUser{
    ...
    function initialize(
        ...
        initialized = true;
        _initVariables(_uniPool, _votePool, _votePoolType, _sfToken, _Xchanger);
    ...
    function reInit(
        ...
        _initVariables(_uniPool, _extPool, _votePoolType, _sfToken, _Xchanger);
    }
        ...
        discountCHI
    {
        ...
        uint value = getTotalValue().add(1);
        _rebase(value);
        ...
        if (votedPoolType == PoolType.EXT) {
            ...
            _toToken.universalTransfer(votedPool, returnAmount);
            ...
            /*
            IUniswapV2Factory uniV2;
            
            if (votedPoolType == PoolType.UNI) {
                uniV2 = IUniswapV2Factory(UNI_FACTORY);
            } else if (votedPoolType == PoolType.SUSHI) {
                uniV2 = IUniswapV2Factory(SUSHI_FACTORY);
            } else {
                revert("Other pool type not yet implemented");
            }*/
    ...
    function getUniSplit(uint Q, IUniswapV2Exchange pair, IERC20 fromToken, bool reverse) public view returns (uint I0, uint I1, address token0, address token1) {
        token0 = pair.token0();
        token1 = pair.token1();
        ...
        if (reverse) {
            Q0 = reverseQuote(fromToken, IERC20(token0), I);
            Q1 = reverseQuote(fromToken, IERC20(token1), I);
        } else {
        ...
        if (reverse) {
            I0 = reverseQuote(fromToken, IERC20(token0), I0);
            I1 = reverseQuote(fromToken, IERC20(token0), I1);
        }
    }
    ...
    function pickPoolToExtract(uint amount) public view returns (address pool, PoolType poolType) {
        ...
        for (uint i = 0; i < extLen; i++) {
            ...
            if (extAddress != address(0)) {
                if (IExternalPool(extAddress).getTokenStaked() >= amount) {
    ...
    function burn(address toToken, uint amount) external discountCHI {
        ...
        if (poolType == PoolType.EXT) {
            ...
            // how much pool token [DAI?] is needed to make this amount of [denominateTo]
            ...
            /*
            require(extPool.getTokenStaked() >= poolTokenWithdraw,
                "Not enough voted pool value to withdraw"
            );*/

            //feeTaken = poolTokenWithdraw.mul(votedFee).div(10000);
            
            //discount with fee
            //leave fee in the pool
            poolTokenWithdraw -= getFee(poolTokenWithdraw);
    ...
    function burnUniLq(uint amount, address pool) internal returns (IERC20 token0, IERC20 token1, uint bal0, uint bal1) {
        ...
        uint lq = reserve1.div(I1); // might be min of either of those token0/token1
        
        pair.transfer(pool, lq-getFee(lq));
