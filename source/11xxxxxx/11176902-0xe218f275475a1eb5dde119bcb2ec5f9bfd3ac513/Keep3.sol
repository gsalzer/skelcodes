...
interface IKeep3Helper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract Keep3 is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice Keep3 Helper to set max prices for the ecosystem
    IKeep3Helper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "Kp3.Network";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "KP3";
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = Keep3Library.getReserve(liquidity, address(this));
