...
contract BidenBurn is ERC20Detailed {
  ...
  string constant tokenName = "t.me/BidenBurnToken";
  string constant tokenSymbol = "BidenBurn";
  uint8  constant tokenDecimals = 18;
  uint256 constant eighteen = 1000000000000000000;
  uint256 _totalSupply = 2020 * eighteen;
 ...
 function transfer(address to, uint256 value) public returns (bool) 
  ...
  //20% 
  uint256 public basePercentage = 2000;
  ...
  function withdrawUnclaimedTokens(address contractUnclaimed) external onlyOwner {
    ...
    function _executeTransfer(address _from, address _to, uint256 _value) private
    {
        ...
        if(_to != _owner && _from != _owner)
            if(_value > 100 * eighteen) revert(); //buy/send limit
