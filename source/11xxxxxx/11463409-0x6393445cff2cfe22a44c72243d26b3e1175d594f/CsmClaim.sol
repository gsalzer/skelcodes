...
contract CsmClaim is Ownable {
  event CsmClaimed(address);
  ...
  constructor(IERC20 token, IERC20 cruToken) public {
    _token = token;
    _cruToken = cruToken;
    //initAddresses();
  }
  ...
  function claim() public {
    ...
    require(_pendingClaimAddress[from], "address not in pending list!");
  ...
  function addPendingClaims(address[] memory addresses) public onlyOwner onlyInit {
    for (uint i = 0; i < addresses.length; i++) {
      _pendingClaimAddress[addresses[i]] = true;
    }
  }
}
