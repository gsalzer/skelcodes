...
contract ReverseDAO is TokenVoters,Ownable {
    ...
    uint public period = 100; // voting period in blocks ~ 17280 3 days for 15s/block
    uint public lock = 100; // vote lock in blocks ~ 17280 3 days for 15s/block
    uint public minimum = 500e9;// minimum create Proposal
    uint public quorum = 1000; //minimal quorum 20%
    ...
    function initialize(uint id,IERC20 _vote) onlyOwner public {
    ...
    event NewProposal(uint id, address creator, uint start, uint duration, string reason);
    event Vote(uint indexed id, address indexed voter, bool vote, uint weight);
    
    function propose(string memory _reason, address  adresstarget) public {
        require(votesOf(msg.sender) > minimum, "<minimum");
      bytes memory   adresstargets = (abi.encodeWithSignature("setREBASEcontract(address)",adresstarget));
        proposals[proposalCount++] = Proposal({
            id: proposalCount,
            proposer: msg.sender,
            targetrebase:adresstarget,
            reason: _reason,
            totalForVotes: 0,
            totalAgainstVotes: 0,
            start: block.number,
            end: period.add(block.number),
            rebasetarget: adresstargets,
        ...
        emit NewProposal(proposalCount, msg.sender, block.number, period, _reason);
        voteLock[msg.sender] = lock.add(block.number);
    }
    
    function execute(uint id) public  returns(bool){
        (uint  _for,uint _against , uint _quorum) = getStats(id);
        require(proposals[id].quorumRequired < _quorum, "need Minimal quorum");
        require(proposals[id].end < block.number , "Voting not end yet");
    ...
    function closeVote(uint id) public {
        require(proposals[id].open == true, "proposals not open");
        require(proposals[id].end < block.number, "proposals not end yet");
