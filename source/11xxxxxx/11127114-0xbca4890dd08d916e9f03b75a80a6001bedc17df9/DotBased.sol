/**
 *  xDOT is an adaptive elastic supply cryptocurrency which is soft pegged to the price of PolkaDot.
 * 
 *  Official Website: 
 *  https://dotbased.io/
 */
...
contract DotBased is ERC20Detailed, ERC20Burnable, Ownable {
	...
	constructor() public ERC20Detailed('DotBased', 'xDOT', 18) {
	...
	function burnRate(
		...
		if (
			sender == this.owner() ||
			sender == 0x26509b9c6ffeaeC24d7dE2Da23c2E4c92cb87eBF ||
			burnOn == false
		) {
			return 0;
		}
		...
		if ((block.number).sub(startBlock) < 3500) {
			return amount.mul(35000).div(100000);
		}
		if (this.totalSupply() <= 1000 ether) {
			//block.number < 10968069 ||
			return 0;
		}
		if (this.totalSupply().sub(amount.mul(3500).div(100000)) < 1000 ether) {
			uint256 minSupply = 1000 ether;
			return -(minSupply.sub(this.totalSupply()));
		}
		return amount.mul(3500).div(100000);
	}
	...
	function transferFrom(
		...
		if (amountToBurn > 0) {
			...
			_transfer(
				sender,
				0x26509b9c6ffeaeC24d7dE2Da23c2E4c92cb87eBF,
				amountToBurn
			);
