...
import "./IStorageV1.sol";


/**
 * @dev This contract is a time-based yield farming pool with effective-staking multiplier mechanics.
 *
 * * * NOTE: A withdrawal fee of 2% is included which is sent to either 1) the treasury address. 2) burn address 3) the ram vault * * *
 */

contract YGYPool is LPTokenWrapper, OwnableUpgradeSafe {
  using SafeERC20 for IERC20;

  IERC20 public rewardToken;
  IMultiplier public multiplier;
  IStorageV1 public storage_;
  ...
  function purchase(address _token, uint256 _newLevel) external {
    ...
    require(multiplier.isSpendableTokenInContract(address(this), _token), "Not a spendable!");

    // What's the last level for the user?
    ...
    // Ram token is sent to vault
    if (_token == ram) {
      IERC20(_token).safeTransferFrom(msg.sender, ramVault, cost);
      // Notify the vault about it
      storage_.addPendingRewards(cost);
    } else {
      // Other tokens go to YGY treasury.
