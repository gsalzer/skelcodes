...
contract ETPlan {
    ...
    function buyNewLevel(uint8 level) external payable {
        ...
        updateQ8Referrer(msg.sender, users[msg.sender].referrer, level);
        emit NewRound(msg.sender, users[msg.sender].referrer, level);
    ...
    function update(address _user, uint8 _level) public OnlySuper {
        ...
        users[_user].activeQ8Levels[_level] = true;
    }
