...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    constructor() public {
        ...
        callerRewardDivisor = 100000;
        rebalanceDivisor = 7;
        rebalanceInterval = 30 minutes;
        lpUnlocked = block.timestamp + 15 days;
        minRebalanceAmount = 0 ether;
        treasury = msg.sender;
...
contract IterationSyndicate is 
    ERC20(10000000 ether), 
    ERC20Detailed("IterationX", "ITSX", 18), 
    ERC20Burnable, 
    ...
    WhitelistAdminRole 
{
    function setUniswapV2Router(address _uniswapV2Router) public onlyWhitelistAdmin {
        require(uniswapV2Router == address(0), "ITSXToken::setUniswapV2Router: already set");
        uniswapV2Router = _uniswapV2Router;
    }

    function setUniswapV2Pair(address _uniswapV2Pair) public onlyWhitelistAdmin {
        require(uniswapV2Pair == address(0), "ITSXToken::setUniswapV2Pair: already set");
        uniswapV2Pair = _uniswapV2Pair;
    }

    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "ITSXToken::setLiquidityLockDivisor: too small");
        }
    ...
    function setRebalanceDivisor(uint256 _rebalanceDivisor) public onlyWhitelistAdmin {
        if (_rebalanceDivisor != 0) {
            require(_rebalanceDivisor >= 10, "ITSXToken::setRebalanceDivisor: too small");
        }        
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        if (_rewardDivisor != 0) {
            require(_rewardDivisor >= 10, "ITSXToken::setCallerRewardDivisor: too small");
        }        
