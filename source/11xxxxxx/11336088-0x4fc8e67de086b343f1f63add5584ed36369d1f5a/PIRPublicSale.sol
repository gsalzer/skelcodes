...
contract PiranhasToken is ERC20("Piranhas", "$PIR") {
    ...
    external
    {
        ...
        require(signatory != address(0), "PIR::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PIR::delegateBySig: invalid nonce");
        require(now <= expiry, "PIR::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "PIR::getPriorVotes: not yet determined");
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "PIR::_writeCheckpoint: block number exceeds 32 bits");
...
contract PIRPublicSale is Ownable {
    using SafeMath for uint256;
    ...
    function buy(uint256 amount) public payable {
        ...
        if(PreSaleClosed || !PreSaleStarted || amount<minAmount){
            msg.sender.transfer(amount);
        } else {
            uint256 receivedPir = amount.mul(PricePerEther).div(10**18);
            if(UnSoldTokens > receivedPir){
                UnSoldTokens = UnSoldTokens.sub(receivedPir);
                sendMoney(msg.sender, amount,receivedPir);
            } else {
                uint256 extraEthers = receivedPir.sub(UnSoldTokens).div(PricePerEther);
                uint256 ethers = amount.sub(extraEthers);
                receivedPir = UnSoldTokens;
                UnSoldTokens = 0;
                PreSaleClosed = true;
                msg.sender.transfer(extraEthers);
                sendMoney(msg.sender, ethers,receivedPir);
            }
        }
    }
