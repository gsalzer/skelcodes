 ...
 *
 */
contract NFTeGG_Farm_1a is Ownable {
    using SafeMath for uint256;
    ...
    address public constant T3_ADDRESS = address(0x1C27c489F5304c6C171115c7d11a807533F6c5ff); // address of T3 contract (ERC20) used for distributing rewards
    ...
    function calcCurrentPayoutT1(address token, address user, uint256 tokenid) public view returns (uint256) {
        ...
        if (T1(token).someBool(tokenid)) {
            base = base * t1StakeFactors[token].baseMulti / t1StakeFactors[token].baseDivi;
        }
    ...
    function unstakeERC721(address token, uint256 tokenid) external {
        ...
        uint256[] memory gegoIds = stakedERC721ByUser[token][user];
        ...
        stakedERC721ByUser[token][user][gegoArrayLength] = 0;
    ...
    function stakeT1(address token, uint256 tokenid) external {
        require(t1StakeFactors[token].baseMulti != 0, "T1 token not stakeable");
        address user = msg.sender;
        IERC721(token).transferFrom(user, address(this), tokenid);
    ...
    function unstakeT1(address token, uint256 tokenid) external {
        ...
        uint256[] memory gegoIds = stakedT1ByUser[token][user];
        uint256 nftIndex = t1ArrayIndex[token][tokenid]; 
        uint256 gegoArrayLength = gegoIds.length-1;
        uint256 tailId = gegoIds[gegoArrayLength];
        stakedT1ByUser[token][user][nftIndex] = tailId;
        stakedT1ByUser[token][user][gegoArrayLength] = 0;
