...
contract cmVault is ERC20, ERC20Detailed {
    ...
    constructor (address _token,uint _earnLowerlimit) public ERC20Detailed(
        string(abi.encodePacked("contractmint ", ERC20Detailed(_token).name())),
        string(abi.encodePacked("cm", ERC20Detailed(_token).symbol())),
        ERC20Detailed(_token).decimals()
    ) {
        token = IERC20(_token);
        governance = tx.origin;
        
        controller = 0xd3CaC4008e9c11737A901d4c1F551Eb46DeB42A7;
    ...
    function deposit(uint _amount) public {
        ...
        _mint(msg.sender, shares); //返回了iUSDT 
