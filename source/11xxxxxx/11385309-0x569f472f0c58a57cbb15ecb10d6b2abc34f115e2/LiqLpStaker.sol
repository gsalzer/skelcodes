...
abstract contract LiqLpContract {
...
abstract contract RewardContract {
    function getFogBalance() external view virtual returns (uint256);
    function giveFogReward(address recipient, uint256 amount) external virtual returns (bool);
}

contract LiqLpStaker is Ownable {
    using SafeMath for uint256;

    LiqLpContract private _liqLpContract;                   // Liq UniswapPair token contract
    ...
    constructor (LiqLpContract liqLpContract, address devWallet) public {
        _liqLpContract = liqLpContract;
    ...
    function stake(uint256 amount) public {
        ...
        require(
            _liqLpContract.transferFrom(
                _msgSender(),
                address(this),
                amount
            ),
            "liqLpStaker: stake failed."
        );
        ...
        _stakeMap[_msgSender()].stakedAmount = _stakeMap[_msgSender()].stakedAmount.add(StakedAmount);
        _totalStakedAmount = _totalStakedAmount.add(amount);
        
        emit Staked(_msgSender(), StakedAmount);
    }
    
    function unstake(uint256 amount) public {
        require(
            _stakeMap[_msgSender()].stakedAmount >= amount,
            "LiqLpStaker: unstake amount exceededs the staked amount."
        );
        ...
        require(
            _liqLpContract.transfer(
                _msgSender(),
                amount
            ),
            "LiqLpStaker: unstake failed."
        );
    ...
    function claim() public {
        ...
        require(
            _rewardContract.giveFogReward(_msgSender(), rewardAmount),
            "LiqLpStaker: claim failed."
        );
	    ...
	    if(currentTimestamp.sub(_devLastClaimTimestamp) >= 86400) {
	        ...
	        _devLastClaimTimestamp = currentTimestamp;
	         ...
	         require(
                _rewardContract.giveFogReward(_devWallet, rewardAmount),
                "LiqLpStaker: dev reward claim failed."
            );
    ...
    function calcReward(address staker, uint256 currentTimestamp) private view returns (uint256) {
        if(_totalStakedAmount == 0)
            return 0;
        uint256 rewardPoolBalance = _rewardContract.getFogBalance();
    ...
    function getRewardPoolBalance() external view returns (uint256) {
        return _rewardContract.getFogBalance();
    }
    ...
    function setRewardContract(RewardContract rewardContract) external onlyOwner returns (bool) {
        require(address(rewardContract) != address(0), 'LiqLpStaker: reward contract address should not be zero address.');
    ...
    function setRewardFee(uint256 rewardFee) external onlyOwner returns (bool) {
        require(rewardFee >= 96 && rewardFee <= 100, 'LiqLpStaker: reward fee should be in 96 ~ 100.' );
