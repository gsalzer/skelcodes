...
contract OneChad is IERC20 {
    ...
    // Two minutes minimum diff between selling and buying time
    uint256 private minimumDiffSellBuyTime = 2 minutes;
    
    uint256 private _totalSupply = 500 ether;

    string private _name = "OneChad.network";
    string private _symbol = "1Chad";
    ...
    event Chad(address indexed who);
    ...
    function getBiggestholder() private returns (address){
        ...
        for(uint256 i=0; i < holderIndex; i++){
            ...
            if(bought > max){
                if(canWin(curAddress) && isVirgin(curAddress) == false && curAddress != sidepotAddress && !shouldIgnore(curAddress)) {
    ...
    function isVirgin(address holderAddress) public view returns(bool){
