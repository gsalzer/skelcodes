...
// Selling will occur via Curve or Uniswap and buying WETH via Uniswap
...
contract StabilizeStrategyBTCArbV3 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxAmountSell = 3e18; // The maximum amount of tokens that can be sold at once, 3 BTC equivalent
    uint256 public percentDepositor = 50000; // 1000 = 1%, depositors earn 50% of all gains
    uint256 public percentExecutor = 30000; // 30000 = 30% of WETH goes to executor
    ...
    function simulateExchange(address _inputToken, address _outputToken, uint256 _amount) internal view returns (uint256) {
        ...
        if(_inputToken == sbtc || _outputToken == sbtc){
            // Map the addresses to curveIDs
            ...
            if(_outputToken != wethAddress){
                // We are not trading for weth, so use curve
                // Must use Curve for trading
            ...
            if(_outputToken != wethAddress){
                // Simple Uniswap routes
                address[] memory path = new address[](3);
                path[0] = _inputToken;
                path[1] = wethAddress; // Tokens share WETH as main liquidity pair
                path[2] = _outputToken;
                uint256[] memory estimates = router.getAmountsOut(_amount, path);
                _amount = estimates[estimates.length - 1]; // This is the amount of other token returned
                return _amount;
            }else{
                address[] memory path = new address[](2);
                path[0] = _inputToken;
    ...
    function exchange(address _inputToken, address _outputToken, uint256 _amount) internal {
        ...
        if(_inputToken == sbtc || _outputToken == sbtc){
            ...
            if(_outputToken != wethAddress){
                ...
                router.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60)); // Get other token from token
                return;
            }else{
                address[] memory path = new address[](2);
                path[0] = _inputToken;
