...
contract WSPair is IWSPair, WSERC20, IWSImplementation {
    ...
    function getRouter() internal pure virtual returns(address) {
        return address(0x463672ffdED540f7613d3e8248e3a8a51bAF7217);
    } 
    ...
    function _swap(uint amount0Out, uint amount1Out, address to, bytes calldata data, uint discount) internal {
        require(discount == 0 || msg.sender == getRouter(), 'WSwap: DISCOUNT COULD BE CALCULATED ONLY ON ROUTER');
        ...
        uint balanceOrAmountOut0; // spike to avoid depp arrors
        uint balanceOrAmountOut1; // spike to avoid depp arrors
        ...
        balanceOrAmountOut0 = IERC20(_token0).balanceOf(address(this));
        balanceOrAmountOut1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balanceOrAmountOut0 > _reserve0 - amount0Out ? balanceOrAmountOut0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balanceOrAmountOut1 > _reserve1 - amount1Out ? balanceOrAmountOut1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'WSwap: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = balanceOrAmountOut0.mul(10_000).sub(amount0In.mul(30 - discount));
        uint balance1Adjusted = balanceOrAmountOut1.mul(10_000).sub(amount1In.mul(30 - discount));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10_000**2), 'WSwap: K');
        }

        _update(balanceOrAmountOut0, balanceOrAmountOut1, _reserve0, _reserve1);
        balanceOrAmountOut0 = amount0Out;
        balanceOrAmountOut1 = amount1Out;
        emit Swap(msg.sender, amount0In, amount1In, balanceOrAmountOut0, balanceOrAmountOut1, to);
    }

    // this low-level function should be called from a contract which performs important safety checks
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
        _swap(amount0Out, amount1Out, to, data, 0);
    }

    function swapDiscount(uint amount0Out, uint amount1Out, address to, bytes calldata data, uint discount) external override lock {
        _swap(amount0Out, amount1Out, to, data, discount);
    }
