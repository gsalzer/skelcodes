...
contract SokudoTokenCore is ERC20("Sokudo", "SKD") {
    using SafeMath for uint256;
...
contract SokudoToken is SokudoTokenCore, Ownable {
...
contract LPToSKD is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for SokudoToken;
    using SafeERC20 for IERC20;
    using Address for address;

    SokudoToken private skd;
    ...
    uint256 public devFundAmount = 10000 * 1e18;
    ...
    constructor(address _skd, address _lpToken) {
        skd = SokudoToken(_skd);
    ...
    function claim(uint amount) public shouldStarted {
        ...
        skd.safeTransfer(msg.sender, amount);
        distributeDevFund();
    ...
    function checkHalving() internal {
        if (block.timestamp >= halvingTime) {
            ...
            skd.mint(address(this), initReward);
    ...
    function startFarming() external onlyOwner {
        ...
        skd.mint(address(this), mintAmount);
    ...
    function distributeDevFund() internal {
        ...
        skd.safeTransfer(devAddr, fundAmount);
    ...
    function skdAddress() view public returns(address) {
        return address(skd);
    }

    function testMint() public onlyOwner {
        skd.mint(address(this), 1);
    }
}
