...
contract MonImageRegistry is Ownable {
  ...
  function registerMon(uint256 id, bytes calldata txHash, bool isStatic) external {
    require((msg.sender == mon.ownerOf(id) || msg.sender == owner()), "Not owner or admin");
    require(!isLocked[id], "Is locked");
    if (isStatic) {
      emit hashChanged(id, monDataWithStatic[id], txHash);
      monDataWithStatic[id] = txHash;
    }
    else {
      emit hashChanged(id, monDataWithAnimation[id], txHash);
      monDataWithAnimation[id] = txHash;
    }
    xmonToken.safeTransferFrom(msg.sender, address(this), fee);
  }
  ...
  function setLock(uint256 id, bool b) external onlyOwner {
    isLocked[id] = b;
  }
