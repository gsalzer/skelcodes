...
contract SnailToken is BEP20('SnailToken', 'SNAIL') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (SnailSwapRouter).
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        // balance of underlying SNAILs (not scaled);
...
contract SnailMaster is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SNAILs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (userInfo.amount * pool.accSnailPerShare) - userInfo.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSnailPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // How many allocation points assigned to this pool. SNAILs to distribute per block.
        uint256 lastRewardBlock; // Last block number that SNAILs distribution occurs.
        uint256 accSnailPerShare; // Accumulated SNAILs per share, times 1e12. See below.
        bool exists; //
    }
    ...
    // The block number when SNAIL mining starts.
    uint256 public startBlock;
    
    uint256 public snailPerBlock;
    
    uint256 public devSnailRatio;//奖励给开发者的比例100代表1%
   
    // Accumulated SNAILs per share, times 1e12.
    ...
    constructor(
        ...
        address _devAddr,
        uint256 _startBlock,
        uint256 _snailAccount,
        uint256 _oneDayBlockNo,
        uint256 _day,
        uint256 _devSnailRatio
        ...
        //开发者地址
        devAddr = _devAddr;
        uint256 _maxBlock = _startBlock.add(_day.mul(_oneDayBlockNo));
        uint256 _snailPerBlock = _snailAccount.mul(1E18).div(_day).div(_oneDayBlockNo);
        startBlock = _startBlock;
        snailPerBlock = _snailPerBlock;
        snailMaxBlock = _maxBlock;
        devSnailRatio = _devSnailRatio;
    }
    ...
    function getTotalRewardInfo(uint256 _from, uint256 _to) public pure returns (uint256 totalReward) {
        totalReward =  _to.sub(_from);
        return totalReward;
    }
    ...
    // Update the given pool's SNAIL allocation point. Can only be called by the owner.
    function set(
        address _pair,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending SNAILs on frontend.
    function pendingSnail(address _pair, address _user) external view returns (uint256) {
        ...
        //流动性token的供应数量
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < snailMaxBlock) {
            uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
            //计算还没有分配奖励的总奖励数量
            uint256 snailReward = totalReward.mul(snailPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            //计算这个池子应该分配的比例
    ...
    function updatePool(address _pair) public {
        ...
        if (pool.lastRewardBlock >= snailMaxBlock) {
            return;
        }
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 snailReward = totalReward.mul(snailPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        snail.mintTo(devAddr, snailReward.div(devSnailRatio));
    ...
    // Deposit LP tokens to SnailMaster for SNAIL allocation.
    ...
    // Withdraw LP tokens from SnailMaster.
    ...
    // Withdraw without caring about rewards. EMERGENCY ONLY.  不计较报酬就退出。仅限紧急情况。
    ...
    // Safe snail transfer function, just in case if rounding error causes pool to not have enough SNAILs.
