...
contract ZStake is ERC20("ZKP Stake Token", "ZSTAKE"), Ownable {
    using SafeMath for uint256;
    IERC20 public erc20;

    // whether ZKPStake is opened or not
    bool public isOpen = true;
    bool public isAllow = false;
    // amount of blocks between each unlock
    uint256 public blocksBetween; // 27500
    // next period that ZStake will remain opened
    uint256[2] public openPeriod; // ["11900000", "12700000"]
    ...
    constructor(uint256 _blocksBetween, uint256[2] memory _openPeriod, IERC20 _erc20) public {
      blocksBetween = _blocksBetween;
      openPeriod = _openPeriod;
      erc20 = _erc20;
    }

    modifier validateZKPStake() {
      require(isOpen, "zkpstake closed");
      _;
    }
    ...
    function pendingERC20() external view returns (uint256) {
      ...
      uint256 zstakeShare = balanceOf(msg.sender);
      uint256 what = zstakeShare.mul(erc20.balanceOf(address(this))).div(totalShares);

      return what;
    }

    function openZKPStake() external validateAllow {
    ...
    function closeZKPStake() external validateAllow {
      ...
      // zkpstake remains open for ~1 day
    ...
    // allows governance to start or stop zkpstake
    ...
    // Opens the ZKPStake and collects ERC20
    function enter(uint256 _amount) public validateZKPStake {
    ...
    // Leave the zkpstake. Claim back your ERC20s.
    function leave(uint256 _share) public validateZKPStake {
        uint256 totalShares = totalSupply();
