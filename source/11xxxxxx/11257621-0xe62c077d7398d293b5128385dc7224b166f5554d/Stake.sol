...
// ----------------------------------------------------------------------------
// 'RAKWETH' Staking smart contract
// ----------------------------------------------------------------------------
...
contract Stake is Owned {
    ...
    address public RAKWETH = 0x83a40563A19c6f19dFB93E5c496280129D97E423;
    ...
    function STAKE(uint256 tokens) external {
        require(IERC20(RAKWETH).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user account");
    ...
    function ADDFUNDS(uint256 tokens) external {
        require(IERC20(RAKWETH).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from funder account");
        _addPayout(tokens);
    }
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(RAKWETH).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(RAKWETH).transfer(msg.sender, tokens.sub(_unstakingFee)), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedRAKWETH(address staker) external view returns(uint256 stakedRAKWETH){
        return stakers[staker].stakedTokens;
    }
    
    // ------------------------------------------------------------------------
    // Get the RAKWETH balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourRAKWETHBalance(address user) external view returns(uint256 RAKWETHBalance){
        return IERC20(RAKWETH).balanceOf(user);
    }
}
