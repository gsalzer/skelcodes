...
contract RewardedPbtcWethUniV2Pair is Ownable {
    ...
    constructor(address _uniV2, address _modifiedUnipool) public {
        require(Address.isContract(_uniV2), "RewardedPbtcWethUniV2Pair: _uniV2 not a contract");
    ...
    function() external payable {
        require(msg.sender == address(weth), "RewardedPbtcWethUniV2Pair: msg.sender is not weth");
    }
    ...
    function stake() public payable returns (bool) {
        require(msg.value > 0, "RewardedPbtcWethUniV2Pair: msg.value must be greater than 0");
    ...
    function _stakeFor(address _user, uint256 _amount) internal {
        ...
        (uint256 pbtcReserve, uint256 wethReserve, ) = uniV2.getReserves();
        uint256 pbtcAmountOut = UniswapV2Library.getAmountOut(wethAmountIn, wethReserve, pbtcReserve);

        require(
            allowedSlippage >= UniswapV2Library.calculateSlippage(wethAmountIn, wethReserve, pbtcReserve),
            "RewardedPbtcWethUniV2Pair: too much slippage"
        );

        weth.deposit.value(_amount)();
        weth.transfer(address(uniV2), wethAmountIn);
        uniV2.swap(pbtcAmountOut, 0, address(this), "");

        pbtc.safeTransfer(address(uniV2), pbtcAmountOut);
        weth.transfer(address(uniV2), wethAmountIn);
