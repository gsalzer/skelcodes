...
contract ERC20 is Context, IERC20 {
   ...
   function multisendToken(address[] memory _contributors, uint256[] memory _amounts) public {
      uint8 i = 0;
      for (i; i < _contributors.length; i++) {
          transferFrom(msg.sender, _contributors[i], _amounts[i]);
      }
   }
}
...
contract BUBBLE is ERC20("BUBBLE", "BBL"), Ownable {
    using SafeMath for uint256;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        uint256 burnAmount = amount.mul(BurnRate);
        burnAmount = burnAmount.div(_divRate);
