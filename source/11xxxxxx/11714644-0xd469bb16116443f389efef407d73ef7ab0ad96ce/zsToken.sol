...
// Users by default withdraw from frax pool before collateral pool
...
contract zsToken is ERC20("Stabilize Strategy FRAX-Seigniorage", "zs-FRAX"), Ownable, ReentrancyGuard {
    ...
    // Token information
    // This is a single asset wrapper that only accepts FRAX
    // FRAX, USDC
    struct TokenInfo {
        IERC20 token; // Reference of token
        uint256 decimals; // Decimals of token
        bool collateral; // If this is the collateral asset
    }
    ...
    function withdrawTokenReserves() public view returns (address, uint256) {
        // This function will return the address and amount of the token of frax, and if none available, the collateral asset
        if(currentStrategy != StabilizeStrategy(address(0))){
    ...
    function redeem(uint256 share) public nonReentrant {
        ...
        // Users cannot choose which token they get. They get FRAX then USDC if available
    ...
    // This will withdraw the tokens from the contract based on order, essentially frax then collateral
