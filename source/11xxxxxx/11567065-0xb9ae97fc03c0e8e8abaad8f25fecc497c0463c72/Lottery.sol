...
contract Lottery is _Ownable_, _OpenClose, _Guest_{
    uint32 public constant version = 20201226;

    // Required function
    receive()   external payable{/* require(msg.data.length == 0); */   $deposit(msg.sender, msg.value);}
    fallback()  external payable{/* require(msg.data.length == 0); */   $deposit(msg.sender, msg.value);}
    ...
    constructor() public {
        ...
        // fix 0.1 ether(100 finney)
        $config[0].slotPrice    = 100 finney;
        $config[1].slotPrice    = 100 finney;
        $config[2].slotPrice    = 100 finney;

        $global.findPage     = 10000;
...
// [ ■■■ deprecated ■■■ 
    /*
    function sampleCall(address _g, uint8 _c, uint8[40] memory _ns) external onlyOwner{
        $guests[_c].votes[_g].turn = $_state[_c].turn;

        for(uint8 i = 0; i < 5; i++){
            uint8 $ = i * 8;
            $guests[_c].votes[_g].slot[i].vote = _ns[$ + 1];
            $guests[_c].votes[_g].slot[i].numbers = [_ns[$ + 2], _ns[$ + 3], _ns[$ + 4], _ns[$ + 5], _ns[$ + 6], _ns[$ + 7]];

            $_state[_c].weights[_ns[$ + 1]-1]++;
            $_state[_c].fullAmounts += $config[_c].slotPrice;
        }

        $guests[_c].lists.push(_g);
    }
    function sampleWinnumbers(uint8 _c, uint8[6] memory _n) external onlyOwner{_lastgame[_c].winNumbers = _n;}
    function sampleDeposit(address g, uint v) external onlyOwner{_guestDeposits[g] = v * 1 finney;}
    */
// ] ■■■ deprecated ■■■ 
