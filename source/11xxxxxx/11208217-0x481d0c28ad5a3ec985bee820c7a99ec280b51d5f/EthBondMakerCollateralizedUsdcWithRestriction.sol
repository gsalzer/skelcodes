...
// File: contracts/bondMaker/BondMakerCollateralizedErc20.sol

pragma solidity 0.6.6;



contract BondMakerCollateralizedErc20 is BondMaker, UseBondTokenName {
    ERC20 internal immutable COLLATERALIZED_TOKEN;
    ...
    {
        COLLATERALIZED_TOKEN = ERC20(collateralizedTokenAddress);
        uint8 decimalsOfCollateral = ERC20(collateralizedTokenAddress).decimals();
    ...
    function issueNewBonds(uint256 bondGroupID) public returns (uint256 bondAmount) {
        uint256 allowance = COLLATERALIZED_TOKEN.allowance(msg.sender, address(this));
        COLLATERALIZED_TOKEN.transferFrom(msg.sender, address(this), allowance);
        return _issueNewBonds(bondGroupID, allowance);
    }
...
// File: contracts/bondMaker/EthBondMakerCollateralizedUsdc.sol
...
contract EthBondMakerCollateralizedUsdc is BondMakerCollateralizedErc20 {
        ...
        returns (string memory symbol, string memory name)
    {
        ...
        if (sbtStrikePrice != 0) {
            return
                _bondTokenNameContract.genBondTokenName(
                    "SBT",
                    "SBT",
                    maturity,
...
// File: contracts/bondMaker/EthBondMakerCollateralizedUsdcWithRestriction.sol

pragma solidity 0.6.6;



contract EthBondMakerCollateralizedUsdcWithRestriction is EthBondMakerCollateralizedUsdc {
    constructor(
        address collateralizedTokenAddress,
        address oracleAddress,
        address feeTaker,
        ...
        public
        EthBondMakerCollateralizedUsdc(
            collateralizedTokenAddress,
            oracleAddress,
            feeTaker,
            bondTokenNameAddress,
    ...
    function registerNewBondGroup(bytes32[] calldata bondIDs, uint256 maturity)
        ...
        returns (uint256 bondGroupID)
    {
        require(bondIDs.length == 2, "a bond group must consists of only 2 bonds"); // restriction
        _assertBondGroup(bondIDs, maturity);
    ...
    function reverseBondGroupToCollateral(uint256, uint256) external override returns (bool) {
        revert("this function is not available now"); // restriction
    }
