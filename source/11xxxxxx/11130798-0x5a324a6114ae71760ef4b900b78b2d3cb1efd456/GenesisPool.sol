...
contract PepeTokenWrapper {
	using SafeMath for uint256;
	IERC20 public pepe;

	constructor(IERC20 _pepeAddress) public {
...
contract GenesisPool is PepeTokenWrapper, Ownable {
	ERC1155Tradable public pepes;
	...
	constructor(ERC1155Tradable _pepesAddress, IERC20 _pepeAddress) public PepeTokenWrapper(_pepeAddress) {
		pepes = _pepesAddress;
	}
	...
	// stake visibility is public as overriding PepeTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 5000000000000000000, "Cannot stake more than 5 pepe");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(pepes.totalSupply(card) < pepes.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		pepes.mint(msg.sender, card, 1, "");
