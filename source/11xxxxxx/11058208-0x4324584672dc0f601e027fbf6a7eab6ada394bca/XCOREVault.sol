...
// XCORE Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract XCOREVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of XCOREs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accXCOREPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accXCOREPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. XCOREs to distribute per block.
        uint256 accXCOREPerShare; // Accumulated XCOREs per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing XCORE governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's XCOREs allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing XCORE governance consensus
    ...
    function setDevFee(uint16 _DEV_FEE) public onlyOwner {
        require(_DEV_FEE <= 4000, 'Dev fee clamped at 40%');
        DEV_FEE = _DEV_FEE;
    }
    uint256 pending_DEV_rewards;


    // View function to see pending XCOREs on frontend.
    function pendingXCORE(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accXCOREPerShare = pool.accXCOREPerShare;

        return user.amount.mul(accXCOREPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the XCORE token.
    // ----
    uint256 private XCOREBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = XCORE.balanceOf(address(this)).sub(XCOREBalance);

        if(newRewards > 0) {
            XCOREBalance = XCORE.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 XCORERewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        XCORERewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 XCORERewardFee = XCORERewardWhole.mul(DEV_FEE).div(10000);
        uint256 XCORERewardToDistribute = XCORERewardWhole.sub(XCORERewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(XCORERewardFee);

        pool.accXCOREPerShare = pool.accXCOREPerShare.add(
            XCORERewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to XCOREVault for XCORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accXCOREPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accXCOREPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from XCOREVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming XCORE farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accXCOREPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeXCORETransfer(from, pending);
        }
    ...
    // Safe XCORE transfer function, just in case if rounding error causes pool to not have enoughXCOREs.
    function safeXCORETransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 XCOREBal = XCORE.balanceOf(address(this));
        if (_amount > XCOREBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", XCOREBal);

            XCORE.transfer(_to, XCOREBal);
            XCOREBalance = XCORE.balanceOf(address(this));

        } else {
            XCORE.transfer(_to, _amount);
            XCOREBalance = XCORE.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeXCORETransfer(devaddr, devSend);
        }
    ...
    // allowing XCORE governance token holders to do this functions.
