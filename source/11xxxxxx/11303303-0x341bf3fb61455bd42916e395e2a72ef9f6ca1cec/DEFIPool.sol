...
contract DEFIPool is StakeTokenWrapper, IRewardDistributionRecipient {
    ...
    uint256 public constant startTime = 1605718800; //2020-11-23 13:00:00 UTC
    uint256 public periodFinish = 0;
    uint256 public rewardRate = 0;
    uint256 public lastUpdateTime; 
    uint256 public rewardPerTokenStored = 0;
    uint256[] public stakePool = new uint256[](1); 
    uint256[] public withdrawPool = new uint256[](1);
   ...
   struct User {
    ...
    function getUser(address adrr,uint256 index) public view returns (uint256,uint256,uint256,uint256,uint256) {
        return (users[adrr].deposits[index].amount,users[adrr].deposits[index].withdrawAmount,users[adrr].deposits[index].stakeTime,users[adrr].deposits[index].stakeRewardPerTokenStored,users[adrr].deposits[index].round);
    }
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        ...
        _autoExendArray(withdrawPool,round);
        stakePool[round] = stakePool[round].add(amount);
        uint256 updateTime = lastTimeRewardApplicable();
        users[msg.sender].deposits.push(Deposit(amount, 0, updateTime, rewardPerTokenStored,round));
    ...
    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        ...
        _autoExendArray(withdrawPool,round);
        _autoExendArray(stakePool,round);
        withdrawPool[round] = withdrawPool[round].add(amount);
        ...
        for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
            ...
            if(userAmount >= totalAmount){
                users[msg.sender].deposits[i].withdrawAmount = totalAmount;
                totalAmount = 0;
            }else{
                users[msg.sender].deposits[i].withdrawAmount = userAmount;
    ...
    function getReward() public checkStart updateReward(msg.sender){
        ...
        if (reward > 0) {
            ...
            uint256 burnRewardAmount = burnReward(msg.sender);
            uint256 round = getRound(startTime);
            _autoExendArray(burnPool,round);
            burnPool[round] = burnPool[round].add(taxAmount);
            defi.safeTransfer(projectAddress, reward.div(10));
            defi.safeTransfer(msg.sender, reward.mul(9).div(10).sub(taxAmount).add(burnRewardAmount));
            uint256 updateTime = lastTimeRewardApplicable();
            for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
                users[msg.sender].deposits[i].amount = users[msg.sender].deposits[i].amount.sub(users[msg.sender].deposits[i].withdrawAmount);
                users[msg.sender].deposits[i].withdrawAmount = 0;
                ...
                users[msg.sender].deposits[i].round = round;
            }
    ...
    function tax(address account) public view returns(uint256) {
     ...
     function burnReward(address account) public view returns(uint256) {
        if(stakePool.length <= 3 || withdrawPool.length <= 3 || users[account].deposits.length == 0){
            return 0;
        }
        uint256 burnClaim = 0;
        uint256 lastTime = lastTimeRewardApplicable();
        uint256 endIndex =  getRound(lastTime);
        
        for (uint256 i = 0; i < users[account].deposits.length; i++) {
            if(users[account].deposits[i].amount != 0){
                uint256[] memory stakeRealAmountArr = new uint256[](3);
                if(users[account].deposits[i].round == 0){
                    uint256 stakeRealAmount = 0;
                    uint256 reward = 0;
                    for(uint256 j = 0; j < 3; j++){
                        stakeRealAmount = stakeRealAmount.add(stakePool[i]).sub(withdrawPool[i]);
                        stakeRealAmountArr[j] = stakeRealAmount;
                        reward = reward.add(burnPool[i]);
                    }
                    stakeRealAmount =  Math.min(Math.min(stakeRealAmountArr[0],stakeRealAmountArr[1]),stakeRealAmountArr[2]);
                    burnClaim = burnClaim.add(
                        reward.mul(users[account].deposits[i].amount).div(stakeRealAmount)
                    );
                    
                }
                else{
                    uint256 stakeRealAmount = 0;
                    for(uint256 j = 0; j < users[account].deposits[i].round; j++){
                        stakeRealAmount = stakeRealAmount.add(stakePool[i]).sub(withdrawPool[i]);
                        stakeRealAmountArr[j%3] = stakeRealAmount;
                    }
                    for(uint256 j = users[account].deposits[i].round; j < endIndex; j++){
                        if(stakeRealAmount != 0) {
                            stakeRealAmount =  Math.min(Math.min(stakeRealAmountArr[0],stakeRealAmountArr[1]),stakeRealAmountArr[2]);
                            uint256 round = users[account].deposits[i].round;
                            uint256 amount = users[account].deposits[i].amount;
                            burnClaim = burnClaim.add( burnPool[round].mul(amount).div(stakeRealAmount) );                            
                        }
                    }
                }
            }
