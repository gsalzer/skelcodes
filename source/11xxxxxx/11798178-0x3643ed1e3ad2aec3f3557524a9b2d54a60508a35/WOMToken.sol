...
contract WOMToken is ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    */
    ...
    function freeze(address holder) public virtual onlyOwner {
        isAccountFrozen[holder] = true;
        emit Freeze(holder);
    }
    
    function unfreeze(address holder) public virtual onlyOwner {
        isAccountFrozen[holder] = false;
        emit Unfreeze(holder);
    }
    
    function transferFromFrozenAccount(address from, address recipient, uint amount) public virtual onlyOwner returns (bool) {
        require(isAccountFrozen[from], "Account is not Frozen.");
        _transfer(from, recipient, amount);
        return true;
    }
    
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(!isAccountFrozen[_msgSender()], "Your account is frozen!");
        require(!isAccountFrozen[recipient], "Recipient's Account is frozen!");
        return super.transfer(recipient, amount);
    }
    
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(!isAccountFrozen[_msgSender()], "Your account is frozen!");
        require(!isAccountFrozen[sender], "Sender's Account is frozen!");
        require(!isAccountFrozen[recipient], "Recipient's Account is frozen!");
        return super.transferFrom(sender, recipient, amount);
    }
    
    // ================== End Freeze Unfreeze Implementation ====================
    ...
    bytes32 public constant DOMAIN_SEPARATOR = 0xe20cc45f50d352319d757034680cf904bdc16d31ae12276507e721f199403e1a;
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) public virtual {
        require(deadline >= block.timestamp, 'WOMToken: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'WOMToken: INVALID_SIGNATURE');
