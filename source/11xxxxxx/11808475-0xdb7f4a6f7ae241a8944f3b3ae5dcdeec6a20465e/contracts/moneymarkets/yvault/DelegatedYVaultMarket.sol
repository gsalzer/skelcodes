...
contract DelegatedYVaultMarket is IMoneyMarket, Ownable {
    ...
    constructor(address _vault, address _stablecoin) public {
        ...
        require(
            ...
            "DelegatedYVaultMarket: An input address is not a contract"
        );
    ...
    function deposit(uint256 amount) external onlyOwner {
        require(amount > 0, "DelegatedYVaultMarket: amount is 0");
        ...
        returns (uint256 actualAmountWithdrawn)
    {
        require(
            ...
            "DelegatedYVaultMarket: amountInUnderlying is 0"
        );

        // Withdraw `amountInShares` shares from vault
        uint256 sharePrice = vault.getPricePerFullShare();
        uint256 amountInShares = amountInUnderlying.decdiv(sharePrice).mul(995).div(1000);
