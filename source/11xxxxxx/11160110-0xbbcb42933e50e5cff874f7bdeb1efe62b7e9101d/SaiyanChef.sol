...
website: saiyan.rocks
chat: https://t.me/saiyan_token

||        SUPER SAIYAN          ||
// 100% Locked Liquidity
// 1000 Tokens + Dynamic burning
// Deflational Tokenomics
// No Team or Dev funds
//
// Farming Token
// NFT Marketplace 
...
contract Token is ERC20("Saiyan.Rocks", "SAIYAN", 18, 0, 1000), Ownable, Mintable {
...
contract SaiyanChef is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        // claimable SAIYAN by a user is:
        //
        //   pending reward = (user.amount * pool.accSaiyanPerLPShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSaiyanPerLPShare` (and `lastRewardedBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardedBlock;  // Last block number where SAIYAN distribution occured.
        uint256 accSaiyanPerLPShare; // Accumulated Saiyan per share.
        uint256 lpTokenHolding;     // Amount of LP token hold by this pool
    }

    // Info of Saiyan minting
    struct MintInfo {
        uint256 SaiyanPerBlock;  // Amount of minted Saiyan each block.
        uint256 mintStartBlock; // Minting of this amount starts here.
    }

    // The Saiyan token contract
    Token public Saiyan;
    
    // Dev address. (Only needed during deployment)
    address public devaddr;

    // Info of each pool.
    PoolInfo[] public poolInfo;

    // Info of Saiyan minting block span.
    ...
    constructor(
        Token _Saiyan,
        address _devaddr,
        uint256 _SaiyanPerBlock,
        uint256 _mintStartBlock
    ) public {
        Saiyan = _Saiyan;
        devaddr = _devaddr;
        mintInfo.push(MintInfo({
            SaiyanPerBlock: _SaiyanPerBlock,
    ...
    // Amount of block spans for minting different amounts of Saiyan
    ...
    function setMint(uint256 _mid, uint256 _block, uint256 _amount) public onlyOwner {
        ...
        mintInfo[_mid].SaiyanPerBlock = _amount;
    ...
    function getSaiyanAmountInRange(uint256 _from, uint256 _to) public view returns (uint256) {
        if(_from >= _to){
            return 0;
        }
        uint256 reward = 0;
        ...
        for( uint256 i = 0; i < mintInfo.length-1; i++){
            for( uint256 cblock = _from+1; cblock <= _to; cblock++){
                if(cblock >= mintInfo[i].mintStartBlock && cblock < mintInfo[i+1].mintStartBlock){
                    reward += mintInfo[i].SaiyanPerBlock;
                    blocksleft--;
                }
            }
        }
        reward += blocksleft.mul(mintInfo[mintInfo.length-1].SaiyanPerBlock);
        return reward;
    }

    // View function to see pending Saiyan on frontend.
    function pendingSaiyan(uint256 _pid, address _user) public returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accSaiyanPerLPShare = pool.accSaiyanPerLPShare;
        // Users LP tokens who are connected to the site
        ...
        if (block.number > pool.lastRewardedBlock && lpSupply > 0) {
            uint256 SaiyanReward = getSaiyanAmountInRange(pool.lastRewardedBlock, block.number).mul(pool.allocPoints).div(totalAllocPoints);
            SaiyanReward = SaiyanReward.mul(1e18);
            accSaiyanPerLPShare = accSaiyanPerLPShare.add(SaiyanReward.div(lpSupply));
        }
        return user.amount.mul(accSaiyanPerLPShare).div(1e18).sub(user.rewardDebt);
    }

    // Update reward vairables for all pools. WARNING: Be careful of gas spending!
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 SaiyanReward = getSaiyanAmountInRange(pool.lastRewardedBlock, block.number).mul(pool.allocPoints).div(totalAllocPoints);
        Saiyan.mint(devaddr, SaiyanReward.div(8));
        Saiyan.mint(address(this), SaiyanReward);
        SaiyanReward = SaiyanReward.mul(1e18);
        pool.accSaiyanPerLPShare = pool.accSaiyanPerLPShare.add(SaiyanReward.div(lpSupply));
        pool.lastRewardedBlock = block.number;
    }

    // Deposit LP tokens to SaiyanChef for Saiyan allocation.
    // Also used to claim the users pending Saiyan tokens (_amount = 0)
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSaiyanPerLPShare).div(1e18).sub(user.rewardDebt);
            safeSaiyanTransfer(msg.sender, pending);
        }
        pool.lpTokenContract.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accSaiyanPerLPShare).div(1e18);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from SaiyanChef.
    // Also used to claim the users pending Saiyan tokens
    ...
    // Safe Saiyan transfer function, just in case if rounding error causes pool to not have enough saiyan.
    function safeSaiyanTransfer(address _to, uint256 _amount) internal {
        uint256 SaiyanBal = Saiyan.balanceOf(address(this));
        if (_amount > SaiyanBal) {
            Saiyan.transfer(_to, SaiyanBal);
        } else {
            Saiyan.transfer(_to, _amount);
        }
    }
    
    // Update dev address by the previous dev.
     function dev(address _devaddr) public {
       require(msg.sender == devaddr, "dev: fud?");
       devaddr = _devaddr;
    }
}
