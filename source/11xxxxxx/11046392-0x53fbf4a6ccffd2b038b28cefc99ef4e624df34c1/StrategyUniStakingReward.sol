...
// File: contracts/SashimiPlates/interfaces/IUniStakingRewards.sol
...
// File: contracts/SashimiPlates/strategies/StrategyUniStakingReward.sol
...
contract StrategyUniStakingReward {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    // Staking rewards address for LP providers
    ...
    function getName() external pure returns (string memory) {
        return "StrategyUniStakingReward";
    }
    ...
    function setUniThreshold(uint256 _uniThreshold) external {
        require(msg.sender == governance, "!governance");
        uniThreshold = _uniThreshold;
    }
    ...
    function harvest() public {
        ...
        // Swap half WETH for token
        uint256 _weth = IERC20(weth).balanceOf(address(this));
        if (_weth > 0) {
            _swap(weth, token, _weth.div(2));
        }

        // Adds in liquidity for ETH/token
        ...
        if (_weth > 0 && _token > 0) {
            ...
            IERC20(token).safeApprove(univ2Router2, 0);
            IERC20(token).safeApprove(univ2Router2, _token);
        ...
        // We want to get back UNI ETH/token LP tokens
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            // Performance fee
            IERC20(want).safeTransfer(
