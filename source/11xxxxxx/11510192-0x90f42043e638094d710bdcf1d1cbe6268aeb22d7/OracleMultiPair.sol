...
contract OracleMultiPair is Ownable {
    ...
    function updateCumulative() public {
        ...
        uint256 totalMainPoolBal;

        for (uint256 i = 0; i < pools.length; i++) {
            uint256 _decimalFactor = 10**(uint256(18).sub(sideTokenDecimals[i]));
            uint256 tokenMainPrice = pools[i].getSpotPrice(sideTokens[i], mainToken).mul(_decimalFactor);
            require(tokenMainPrice != 0, "!price");

            uint256 reserveBal = pools[i].getBalance(sideTokens[i]).mul(_decimalFactor);
            require(reserveBal >= ORACLE_RESERVE_MINIMUM, "!min reserve");

            uint256 tokenBal = pools[i].getBalance(mainToken);
        ...
        if (timeElapsed > 0 && totalMainPoolBal != 0) {
            ...
            priceCumulative += uint256(UQ112x112.encode(uint112(totalMainPriceWeight)).uqdiv(uint112(totalMainPoolBal))) * timeElapsed;
