...
contract StakeUSDCtoPSC is Ownable {
	using SafeMath for uint256;
    using SafeERC20 for IERC20;
	IERC20 public token;
	IERC20 public polkaStableCash;
	...
	constructor(
		...
		address _polkaStableCash
	) public {
		token = IERC20(_token);
        tokenDecimal = _tokenDecimal;
		polkaStableCash = IERC20(_polkaStableCash);
	}
	...
	function rewardsOf(address stakeholder)
		public
		view
		returns (uint256 userRewardPSC, uint256 totalRewarPSC)
	{
		uint256 totalRewardPSC = 0;
		for (uint256 i = 0; i < _StakeSlots[stakeholder].length; i++) {
			StakeInfo storage infopsc = _StakeSlots[stakeholder][i];
			uint256 currentTime =
				infopsc.endedAt > block.timestamp
					? block.timestamp
					: infopsc.endedAt;
			uint256 spentTime = currentTime.sub(infopsc.startAt);

			uint256 newRewards =
				(infopsc.snapshotAmount * spentTime * _PoolInterestRate) /
					(duration * 10000);
			totalRewardPSC = totalRewardPSC.add(newRewards);
		}

		uint256 availableRewards =
			totalRewardPSC - _PoolClaimedRewards[stakeholder];
		return (availableRewards, totalRewardPSC);
	}
	...
	function Claimd() public {
		...
		polkaStableCash.mint(msg.sender, availableTransfer);
	...
	function SwapPSCtoUSDC(uint256 _amount) public {
		require(polkaStableCash.balanceOf(msg.sender) >= _amount && _amount > 0, "not enough");
        polkaStableCash.burn(msg.sender, _amount);
        uint256 amountTransfer = _amount.mul(10**tokenDecimal).div(uint256(1e18));
