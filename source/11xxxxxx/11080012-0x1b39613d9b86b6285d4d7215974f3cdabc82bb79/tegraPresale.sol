...
contract tegraCoinCore is ERC20("TEGRA Finance", "TEGRA") {
    using SafeMath for uint256;
...
contract tegraCoin is tegraCoinCore, Ownable {
...
contract tegraPresale is Ownable {
    using SafeMath for uint256;
    ...
    uint public tegraPrice = 50;
    uint public buyLimit = 100 * 1e18;
    bool public presaleStart = false;
    bool public onlyWhitelist = false;
    uint public presaleLastSupply = 10000 * 1e18;

    tegraCoin private tegra = tegraCoin(0x7d2ee2F55C0c5A1Fb21E083c9757771Cd58cF616);

    event BuytegraSuccess(address account, uint ethAmount, uint tegraAmount);
    ...
    function settegraPrice(uint newPrice) public onlyOwner {
        tegraPrice = newPrice;
    }
    ...
    receive() payable external presaleHasStarted needHaveLastSupply {
        ...
        uint tegraAmount = msg.value.mul(tegraPrice);
        require(tegraAmount <= presaleLastSupply, "insufficient presale supply");
        presaleLastSupply = presaleLastSupply.sub(tegraAmount);
        tegra.mint(msg.sender, tegraAmount);
        ethSupply[msg.sender] = ethTotalAmount;
        devAddress.transfer(msg.value);
        emit BuytegraSuccess(msg.sender, msg.value, tegraAmount);
    }
