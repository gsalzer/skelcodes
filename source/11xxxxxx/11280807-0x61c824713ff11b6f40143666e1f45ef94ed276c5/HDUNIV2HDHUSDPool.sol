...
contract HDTokenWrapper {
    ...
    //UNI-V2(HD-HUSD)
    IERC20 public stakeToken = IERC20(0x982c99466E420B0aBb60848afC7Ae9411B9f2F48); 
...
/**
 * HD-UNI-V2(HD-HUSD) Pool
 */
contract HDUNIV2HDHUSDPool is HDTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "HD-UNI-V2(HD-HUSD)-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "HD-UNI-V2(HD-HUSD)-POOL: Cannot withdraw 0");
    ...
    function getReward() public checkStart updateReward(msg.sender){
        ...
        if (reward > 0) {
            ...
            hd.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    modifier checkStart(){
        require(block.timestamp > startTime,"HD-UNI-V2(HD-HUSD)-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "HD-UNI-V2(HD-HUSD)-POOL: Pool is closed");
        _;
    }
