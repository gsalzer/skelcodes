...
contract PermissionedExecutors is Proxied {
    ...
    event LogExecutorPayoutFailed(address indexed executor, string error);
    ...
    function _exec(TaskReceipt calldata _taskReceipt) internal virtual {
        ...
        try IGelatoCore(gelatoCore).exec(_taskReceipt) {} catch Error(
            ...
            err.revertWithInfo("PermissionedExecutors.exec:");
        } catch {
            revert("PermissionedExecutors.exec:unknown error");
        }

        (uint256 withdrawAmount, string memory error) =
            _withdrawExcessExecutorStake(type(uint256).max);

        if (withdrawAmount == 0) {
            emit LogExecutorPayoutFailed(msg.sender, error);
            return;
        }

        (bool success, ) = msg.sender.call{value: withdrawAmount}("");
        if (!success)
            emit LogExecutorPayoutFailed(msg.sender, "sendValue failed");
    }
