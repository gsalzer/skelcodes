...
contract Registry is IRegistry, ERC721Burnable, AdminControl {
    ...
    //uint256 private constant _BNB_ROOT_HASH = 0xdba5666821b22671387fe7ea11d7cc41ede85a5aa67c3e7b3d68ce6a661f389c;
    ...
    constructor() ERC721("Defi Name Service (.defi)", "TD") public {
        _mint(address(0xDeF1000000000000000000000000000000000000), _DEFI_ROOT_HASH);
        _tokenURIs[_DEFI_ROOT_HASH] = "defi";
    }
...
library StringEnumerableMap {
    ...
    // implementations (such as Uint256ToAddressMap) are just wrappers around
    // the underlying Map.
    // This means that we can only create new StringEnumerableMaps for types that fit
    // in bytes32.
     ...
     * already present.
     */
    function _set(Map storage map, bytes32 key, string memory value) private returns (bool) {
        // We read and store the key's index to prevent multiple reads from the same storage slot
        uint256 keyIndex = map._indexes[key];
    ...
    */
    function _at(Map storage map, uint256 index) private view returns (bytes32, string memory) {
        require(map._entries.length > index, "StringEnumerableMap: index out of bounds");
    ...
    function _get(Map storage map, bytes32 key) private view returns (string memory) {
        return _get(map, key, "StringEnumerableMap: nonexistent key");
    }

    /**
     * @dev Same as {_get}, with a custom error message when `key` is not in the map.
     */
    function _get(Map storage map, bytes32 key, string memory errorMessage) private view returns (string memory) {
        uint256 keyIndex = map._indexes[key];
        require(keyIndex != 0, errorMessage); // Equivalent to contains(map, key)
        return map._entries[keyIndex - 1]._value; // All indexes are 1-based
        //if (keyIndex == 0) {
     ...
     * already present.
     */
    function set( UintToStringMap storage map, uint256 key, string memory value) internal returns (bool) {
     ...
     * Returns true if the key was removed from the map, that is if it was present.
     */
    function remove( UintToStringMap storage map, uint256 key) internal returns (bool) {
        return _remove(map._inner, bytes32(key));
    }
     ...
     * @dev Returns true if the key is in the map. O(1).
     */
    function contains( UintToStringMap storage map, uint256 key) internal view returns (bool) {
        return _contains(map._inner, bytes32(key));
    }
     ...
     * @dev Returns the number of elements in the map. O(1).
     */
    function length( UintToStringMap storage map) internal view returns (uint256) {
        return _length(map._inner);
    }
    ...
    * - `index` must be strictly less than {length}.
    */
    function at( UintToStringMap storage map, uint256 index) internal view returns (uint256, string memory) {
     ...
     * - `key` must be in the map.
     */
    function get( UintToStringMap storage map, uint256 key) internal view returns (string memory) {
     ...
     * @dev Same as {get}, with a custom error message when `key` is not in the map.
     */
    function get( UintToStringMap storage map, uint256 key, string memory errorMessage) internal view returns (string memory) {
