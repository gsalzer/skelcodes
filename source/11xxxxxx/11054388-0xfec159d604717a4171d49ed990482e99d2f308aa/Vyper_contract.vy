...
contract yERC20:
    ...
    def mint(dst: address, _pie: uint256): modifying
    def redeem(_src: address, _wad: uint256):modifying
    #returns (decimals, exchangeRate, mintFeeRate, redeemFeeRate, totalUnderlying)
    def getBaseData() -> (uint256, uint256, uint256, uint256, uint256):modifying
    def getExchangeRate() -> uint256 : constant
...
def add_liquidity(uamounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    for i in range(N_COINS):
        ...
        if uamount > 0:
            ...
            yERC20(self.coins[i]).mint(self, uamount)
    ...
    for i in range(N_COINS):
        if (one < 0) or (i == one):
            ...
            if _balance == 0:  # Do nothing for 0 coins
                continue
            yERC20(_coin).redeem(self, _balance)
...
@private 
def getPricePerFullShare(dTokenAddr: address) -> uint256: 
    decimals : uint256 = 0
    exchangeRate : uint256 = 0
    mintFeeRate : uint256 = 0
    redeemFeeRate : uint256 = 0
    total_underlying : uint256 = 0
    (decimals, exchangeRate, mintFeeRate, redeemFeeRate, total_underlying) = yERC20(dTokenAddr).getBaseData()
    total_supply : uint256 = yERC20(dTokenAddr).totalSupply()

    return total_underlying / total_supply

@private
@constant
def getPricePerFullShareView(dTokenAddr: address) ->uint256:
    return yERC20(dTokenAddr).getExchangeRate()
...
def remove_liquidity_imbalance(uamounts: uint256[N_COINS], max_burn_amount: uint256):
    ...
    for i in range(N_COINS):
        if amounts[i] > 0:
            rate: uint256 = self.getPricePerFullShare(self.coins[i])
...
def calc_withdraw_one_coin(_token_amount: uint256, i: int128) -> uint256:
    ...
    for j in range(N_COINS):
        rates[j] = self.getPricePerFullShareView(self.coins[j])
...
def remove_liquidity_one_coin(_token_amount: uint256, i: int128, min_uamount: uint256, donate_dust: bool = False):
    ...
    for j in range(N_COINS):
        rates[j] = self.getPricePerFullShare(self.coins[j])
