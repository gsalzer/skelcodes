...
contract ETFXPool is ERC20, ReentrancyGuard, Ownable {
  ...
  // total 50k = 50 000 00000000
  // first month = 25k / 30days  = 833
  uint256 private mintSpeed = 833 * 1e8; //ETFX daily
  ...
  constructor () public ERC20("ETFX-FARM", "FETFX", 8)  {
  ...
  function stake(uint256 amount) external {
    ...
    _mint(sender, amount);
    ETFX.safeTransferFrom(sender, address(this), amount);
  }
  ...
  function withdraw(uint256 amount) external {
    ...
    ETFX.safeTransfer(sender, amount);
    _burn(sender, amount);
  }
  ...
  function earned(address account) public view returns (uint256) {
    ...
    if (accountStartStaking != 0) {
      ...
      deltaRewards = balanceOf(account).mul(period).mul(rewardPerToken()).div(1e8);
    }
    deltaRewards = deltaRewards.add(rewards[account]);
    if (ETFX.balanceOf(address(this)).sub(totalSupply()) < deltaRewards) {
      deltaRewards = ETFX.balanceOf(address(this)).sub(totalSupply());
    }
    return deltaRewards;
  }
  ...
  function exit() external {
    ...
    ETFX.safeTransfer(sender, unstake.add(earned(sender)));
  ...
  function rewardPerToken() public view returns (uint256) {
    ...
    return
        mintSpeed.mul(1e8).div(totalSupply());
  }
