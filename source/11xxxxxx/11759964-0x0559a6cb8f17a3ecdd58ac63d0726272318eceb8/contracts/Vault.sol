...
contract Vault is AccessControlUpgradeSafe {
  ...
  function startNewEpoch() public {
    require(epochCalculationStartBlock + epochPeriod < block.number, "!Epochready");
  ...
  function pendingRewards(uint256 _pid, address _user) external view returns (uint256 zzzRewards, uint256 napRewards) {
    ...
    uint256 zzzCurrent = user.amount.add(user.boost[epoch]).add(user.timelockBoost).mul(vault.accZZZPerShare).div(1e12);
    uint256 napCurrent = user.amount.add(user.boost[epoch]).add(user.timelockBoost).mul(vault.accNAPPerShare).div(1e12);

    if (user.ZZZRewardDebt >= zzzCurrent) {
      zzzRewards = 0;
    } else {
      zzzRewards = zzzCurrent.sub(user.ZZZRewardDebt);
    }
    if (user.NAPRewardDebt >= napCurrent) {
      napRewards = 0;
    } else {
      napRewards = napCurrent.sub(user.NAPRewardDebt);
    }
  }
  ...
  function massUpdateVaults() public {
    ...
    if (allZZZRewards < pendingZZZRewards) {
      pendingZZZRewards = pendingZZZRewards.sub(allZZZRewards);
    } else {
      pendingZZZRewards = 0;
    }

    if (allNAPRewards < pendingNAPRewards) {
  ...
  function updateVault(uint256 _pid) internal returns (uint256 zzzRewardWhole, uint256 napRewardWhole) {
    ...
    if (address(vault.token) == address(zzz)) {
      tokenSupply = zzz.balanceOf(address(this)).sub(userZZZ);
    } else if (address(vault.token) == address(nap)) {
      tokenSupply = nap.balanceOf(address(this)).sub(userNAP);
    } else {
      tokenSupply = vault.token.balanceOf(address(this));
    }
    if (tokenSupply == 0) {
  ...
  function _withdraw(
    ...
    if (user.timelockEnd > 0 && user.timelockEnd <= now - timelockGracePeriod) {
      ...
      uint256 boostAmount = user.boost[epoch];
      uint256 timelockBoost = user.timelockBoost;
      user.ZZZRewardDebt = user.amount.add(boostAmount).add(timelockBoost).mul(vault.accZZZPerShare).div(1e12);
      user.NAPRewardDebt = user.amount.add(boostAmount).add(timelockBoost).mul(vault.accNAPPerShare).div(1e12);
  ...
  function updateAndPayOutPending(
    ...
    uint256 effectiveAmountZZZ = user.amount.add(boostAmount).add(timelockBoost).mul(vault.accZZZPerShare).div(1e12);
    uint256 effectiveAmountNAP = user.amount.add(boostAmount).add(timelockBoost).mul(vault.accNAPPerShare).div(1e12);

    if (user.ZZZRewardDebt < effectiveAmountZZZ) {
      uint256 pendingZZZ = effectiveAmountZZZ.sub(user.ZZZRewardDebt);
      safeZZZTransfer(from, pendingZZZ);
    }

    if (user.NAPRewardDebt < effectiveAmountNAP) {
      uint256 pendingNAP = effectiveAmountNAP.sub(user.NAPRewardDebt);
