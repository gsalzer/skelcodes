...
import "./interfaces/IPlanetFarmConfig.sol";
...
contract PlanetFarm is Ownable, ReentrancyGuard, IFarm {
    using SafeMath for uint256;
    ...
    // Info of pool.
    address public testa; // The Testa TOKEN!
    ...
    function setParams(
        ...
        startLiquidity = config.getLiquidity();
    }
    ...
    modifier checkPayToEntry() {
        require(users[tx.origin], "pay to entry first");
        _;
    }
    ...
    function harvest() public nonReentrant checkPayToEntry {
        UserInfo storage user = userInfo[msg.sender];
        uint256 lpSupply = lpToken.balanceOf(address(this));
	...
	function getUserReward(address userAddress) public returns(uint256) {
        UserInfo storage user = userInfo[userAddress];
    ...
    function deposit(uint256 amount) public checkPayToEntry {
        require(amount > 0 , "invalid amount");
        UserInfo storage user = userInfo[msg.sender];
		updateUserReward(msg.sender);
        lpToken.safeTransferFrom(address(msg.sender), address(this), amount);
        jETHToken.safeTransferFrom(address(msg.sender), address(this), amount);
    ...
    function withdraw(uint256 _amount) public checkPayToEntry {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function setJETHToken(address _jETHToken) public onlyOwner {
