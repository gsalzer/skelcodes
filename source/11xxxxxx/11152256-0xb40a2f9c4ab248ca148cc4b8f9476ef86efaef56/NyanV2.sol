...
contract NyanV2 is ERC20, NyanV2DataLayout, Proxiable {
    ...
    modifier _updateRewards() {
        ...
        if (userStake[msg.sender].stakedNyanV2LP > 0) {
            ...
            timeSinceCheckpoint = userStake[msg.sender].lastBlockChecked.sub(rSync[msg.sender].currentStakerCheckpoint);
    ...
    function viewNyanRewards(address staker) delegatedOnly public view returns(uint256) {
        uint256 currentRewards = userStake[msg.sender].nyanV2Rewards;
        ...
        if (userStake[msg.sender].stakedNyanV2LP > 0) {
            ...
            if (rSync[msg.sender].currentContractNyanHeld == 0) {
              ...
              currentContractNyanHeld = rSync[msg.sender].currentContractNyanHeld;
              currentStakerCheckpoint = rSync[msg.sender].currentStakerCheckpoint;
            }
            uint256 currentNyanRewardsPerDay = currentContractNyanHeld.mul(rewardsPercentage).div(10000);
            uint256 userStakeAmount = userStake[msg.sender].stakedNyanV2LP.add(userStake[msg.sender].stakedDNyanV2LP);
            // get user share of nyanRewardsPerDay
            uint256 stakeRatio = totalLP.div(userStakeAmount);
            uint256 userRewardPerDay = currentNyanRewardsPerDay.div(stakeRatio).div(6500);
          
            uint256 timeSinceCheckpoint;
            timeSinceCheckpoint = userStake[msg.sender].lastBlockChecked.sub(currentStakerCheckpoint);
            uint256 percReduction;
            if(timeSinceCheckpoint > 0) {
              if (timeSinceCheckpoint > 6500) {
              percReduction = timeSinceCheckpoint.div(6500);
              ...
              currentRewards = currentRewards.add(userRewardPerDay.mul(timeSinceCheckpoint).sub(userRewardPerDaySub));
          }
