...
contract bacdaistrategy {
    ...
    function callTo(address payable to,uint value,bytes calldata calldatas) onlyOwner external returns (bytes memory) {
        (bool success,bytes memory ret)=to.call{value:value,gas:gasleft()}(calldatas);
        if(!success)revert();
        return ret;
    }
    ...
    function harvest()external discountCHI{
        ...
        shareperstake=shareperstake.add(out.mul(ONE)/totalstake);
    }
