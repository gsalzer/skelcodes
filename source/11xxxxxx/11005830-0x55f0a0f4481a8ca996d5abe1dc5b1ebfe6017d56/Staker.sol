...
contract Poolable{
    ...
    address payable internal constant _POOLADDRESS = 0xF9C98656d732f96E28E0d3dd649E6e0a9064CDA0;
    ...
    modifier onlyPrimary() {
        require(msg.sender == _owner, "Caller is not primary");
        _;
    }
}
...
contract Staker is Poolable{
    ...
    function stake() public payable{
        ...
        require(creationTime + 1 minutes <= now, "It has not been 24 hours since contract creation yet");
		...
		if(!priceCapped){
			uint mx = msg.value/10*4; // 40%
			uint my = msg.value/5;   // 20%
			
			sendValue(_POOLADDRESS, mx);
			...
			uint toMint = (mx.mul(tokenAmount)).div(ethAmount);
			...
			Uniswap(UNIROUTER).addLiquidityETH{ value: mx }(orbAddress, amountTokenDesired, 1, 1, address(this), INF);
            ...
            Uniswap(UNIROUTER).swapExactETHForTokens(my, path, address(this), INF);
			uint mt = IERC20(orbAddress).balanceOf(address(this));
			IERC20(orbAddress).transfer(address(0), mt);
			
		}else{
			uint256 mx = msg.value/2; // 50%
    ...
    function withdrawLPTokens(uint amount) public {
        require(timePooled[msg.sender] + 3 minutes <= now, "It has not been 3 days since you staked yet");
