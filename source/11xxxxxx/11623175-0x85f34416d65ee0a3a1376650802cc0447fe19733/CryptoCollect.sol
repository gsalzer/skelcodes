...
contract CryptoCollect {
  ...
  string public constant NAME = "Crypto-collectUSA";
  ...
  uint256 constant START_PRICE_COIN = .1 ether;
  uint256 constant START_PRICE_SETS = 1 ether;
  uint256 constant START_PRICE_KING = 10 ether;


  /*** CONSTRUCTOR ***/
  function CryptoCollect() public {
  ...
  function getInitialPriceOfToken(uint _tokenId) public pure returns (uint) {
    if (_tokenId > MAX_SETS_INDEX)
      return START_PRICE_COIN;
    if (_tokenId > 0)
      return START_PRICE_SETS;
    return START_PRICE_KING;
  }
  ...
  function buyToken(uint _tokenId) public payable {
    ...
    if (_tokenId > 0) {
      if (collectibleIndexToOwner[BASE_TOKEN_ID]!=address(0))
        collectibleIndexToOwner[BASE_TOKEN_ID].transfer(feeOnce*2);
      if (_tokenId > MAX_SETS_INDEX) {
        if (collectibleIndexToOwner[_tokenId % MAX_SETS_INDEX]!=address(0))
          collectibleIndexToOwner[_tokenId % MAX_SETS_INDEX].transfer(feeOnce*2);
