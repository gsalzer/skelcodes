...
/**
 * @title SmartWalletRevertReasonHelperV3
 * @author 0age
 ...
 */
contract SmartWalletRevertReasonHelperV3 {
  /**
  ...
  function reason(uint256 code) external pure returns (string memory) {
    ...
    if (code == 33) return "This function has been deprecated.";
    if (code == 34) return "Only the Trade Bot may call this function.";
    if (code == 35) return "Simulation code must revert!";
    if (code == 36) return "Cannot replace value using call that has not yet been performed.";
    if (code == 37) return "Return values are too short to give back a value at supplied index.";
    if (code == 38) return "Cannot replace data using call that has not yet been performed.";
    if (code == 39) return "Invalid implementation.";
