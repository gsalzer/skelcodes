...
contract WrappedWeAreCrowds is Ownable {
   
    using SafeMath for uint256;
  
    string public name     = "Wrapped CROWDS";
    string public symbol   = "WCRWDS";
    uint8  public decimals = 0;
    
    address CRWDS_CONTRACT = 0xDC808091006687284da5F7407cfb851F3Cb3b2Bc;
    
    uint256 public _totalSupply = 0;
    uint256 basePercent = 200;
    ...
    event SetBurnPercentage(uint256 _percent);
    ...
    function setBurnPercentage(uint256 _percent) external onlyOwner {
        emit SetBurnPercentage(_percent);
        basePercent = _percent.mul(100);
    }
    
    function deposit(uint256 amount) public returns(uint256){ //deposit burn is intrinsic to CRWDS
    
        require(ERC20Token(CRWDS_CONTRACT).transferFrom(address(msg.sender),address(this),amount),"TransferFailed");
            
        //calc actual deposit amount due CRWDS burn
        uint256 tokensToBurn = findBurnPercent(amount);
    ...
    function withdraw(uint256 amount) public returns(uint256){ //
        ...
        ERC20Token(CRWDS_CONTRACT).approve(address(this),amount);
        ERC20Token(CRWDS_CONTRACT).transferFrom(address(this),address(msg.sender),amount);
        return amount;
    ...
    function transfer(address to, uint256 amount) public returns (bool) { //CRWDS
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        ...
        if(!_isWhitelisted(from, to)){
            ...
            ERC20Token(CRWDS_CONTRACT).approve(address(this),value);
            ERC20Token(CRWDS_CONTRACT).transferFrom(address(this),address(this),value); //burn
            //
            }
