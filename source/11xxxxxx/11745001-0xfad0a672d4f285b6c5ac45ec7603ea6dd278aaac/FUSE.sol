...
contract FUSE is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Fusion Protocol";
    string private _symbol = "FUSE";
    ...
    uint256 internal _tokenTotal = 1_000_000e9;
    ...
    uint256 public rebalanceInterval = 5 minutes;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "FUSE: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'FUSE: The contract it self cannot be excluded');
        require(!_isExcluded[account], "FUSE: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "FUSE: Account is already included");
