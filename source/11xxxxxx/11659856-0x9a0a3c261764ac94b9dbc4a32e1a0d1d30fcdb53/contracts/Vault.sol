...
contract Vault is AccessControlUpgradeSafe {
  ...
  function updateAndPayOutPending(
    ...
    uint256 effectiveAmount = user.amount.add(user.boost[epoch]).add(user.timelockBoost).mul(vault.accZZZPerShare).div(1e12);
    if (user.amount == 0 || effectiveAmount < user.ZZZRewardDebt || effectiveAmount < user.NAPRewardDebt) return;

    uint256 pendingZZZ = effectiveAmount.sub(user.ZZZRewardDebt);
    uint256 pendingNAP = effectiveAmount.sub(user.NAPRewardDebt);
