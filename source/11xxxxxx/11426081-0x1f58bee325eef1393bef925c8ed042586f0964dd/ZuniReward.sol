...
// File: contracts/balancer/IBFactory.sol
...
// File: contracts/balancer/PCToken.sol
...
// File: contracts/balancer/BalancerReentrancyGuard.sol
...
// File: contracts/balancer/BalancerOwnable.sol
...
// File: contracts/balancer/libraries/RightsManager.sol
...
// File: contracts/balancer/libraries/SafeApprove.sol
...
// File: contracts/balancer/libraries/SmartPoolManager.sol
...
// File: contracts/balancer/ConfigurableRightsPool.sol
...
// File: contracts/balancer/CRPFactory.sol
...
// File: contracts/ZuniTreasury.sol
...
contract ZuniReward is Ownable {
    ...
    ZuniTreasury public zuniTreasury;

    mapping(address => uint256) zuniReward;
    ...
    function updateReward(address[] calldata _users, uint256[] calldata _reward)
        external
        onlyController
    {
        require(_users.length == _reward.length);
        for (uint256 i = 0; i < _users.length; i++) {
            zuniReward[_users[i]] = zuniReward[_users[i]].add(_reward[i]);
        }
    }

    function claimReward(address _receiver) external {
        uint256 reward = zuniReward[_receiver];

        require(reward > 0);

        zuniReward[_receiver] = 0;

        zuniTreasury.giveZuniReward(_receiver, reward);
    }

    function claimableReward(address _user) external view returns (uint256) {
        return zuniReward[_user];
    }

    function migrateTo(address _to) external {
        require(_to != msg.sender);

        zuniReward[_to] = zuniReward[_to].add(zuniReward[msg.sender]);
        zuniReward[msg.sender] = 0;
    }
}
