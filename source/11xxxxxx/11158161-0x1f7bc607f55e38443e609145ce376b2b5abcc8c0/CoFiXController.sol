...
Commit hash: v0.9.5-10-g9306e6e
...
interface INest_3_OfferPrice {
    ...
    function checkPriceNow(address token) external view returns (uint256 ethAmount, uint256 erc20Amount, uint256 blockNum);
...
contract CoFiXController is ICoFiXController {
    ...
    constructor(address _voteFactory, address _nest, address _factory, address _kTable) public {
        ...
        factory = _factory;
        kTable = _kTable;
        callerAllowed[0x97A016B51efAe15FAB099cAae0a42Ac18996635f] = true;
        callerAllowed[0x513631ae343efCCa94CfE6879F9F8D19027E423E] = true;
        address oracle = INest_3_VoteFactory(address(_voteFactory)).checkAddress("nest.v3.offerPrice");
        // address token, address to, uint value
        TransferHelper.safeApprove(_nest, oracle, DESTRUCTION_AMOUNT);
    ...
    function queryOracle(address token, bytes memory data) external override payable returns (uint256 _k, uint256 _ethAmount, uint256 _erc20Amount, uint256 _blockNum, uint256 _theta) {
        require(callerAllowed[msg.sender], "CoFiXCtrl: caller not allowed");
        (_ethAmount, _erc20Amount, _blockNum) = getLatestPrice(token);
        return (K_EXPECTED_VALUE, _ethAmount, _erc20Amount, _blockNum, KInfoMap[token][2]);
    }
