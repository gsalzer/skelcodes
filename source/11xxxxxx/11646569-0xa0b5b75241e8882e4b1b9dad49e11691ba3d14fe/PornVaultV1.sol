...
// Porn Vault distributes the fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PornVaultV1 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    // Update the given pool's PORNs allocation point. Can only be called by the owner.
    ...
    function pendingPorn(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
    ...
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = porn.balanceOf(address(this)).sub(pornBalance);

        if(newRewards > 0) {
            pornBalance = porn.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 pornRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        pornRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 pornRewardFee = pornRewardWhole.mul(DEV_FEE).div(10000);
        uint256 pornRewardToDistribute = pornRewardWhole.sub(pornRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(pornRewardFee);

        pool.accPornPerShare = pool.accPornPerShare.add(
            pornRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit tokens to PornVault for PORN allocation.
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        // Transfer pending tokens
        // to user
        updateAndPayOutPending(_pid, pool, user, depositFor); // Update the balances of the person that the amount is being deposited for

        if(_amount > 0) {
            pool.token.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount); // This is deposited for address
        }

        user.rewardDebt = user.amount.mul(pool.accPornPerShare).div(1e12); // This is deposited for address
    ...
    // Withdraw tokens from PornVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming PORN farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accPornPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safePornTransfer(from, pending);
        }
    ...
    function safePornTransfer(address _to, uint256 _amount) internal {
        ...
        uint256 pornBal = porn.balanceOf(address(this));
        if (_amount > pornBal) {
            porn.transfer(_to, pornBal);
            pornBalance = porn.balanceOf(address(this));

        } else {
            porn.transfer(_to, _amount);
            pornBalance = porn.balanceOf(address(this));
    ...
    function transferDevFee() public {
        ...
        uint256 pornBal = porn.balanceOf(address(this));
        if (pending_DEV_rewards > pornBal) {
            porn.transfer(devaddr, pornBal);
            pornBalance = porn.balanceOf(address(this));
        } else {
            porn.transfer(devaddr, pending_DEV_rewards);
            pornBalance = porn.balanceOf(address(this));
        }
