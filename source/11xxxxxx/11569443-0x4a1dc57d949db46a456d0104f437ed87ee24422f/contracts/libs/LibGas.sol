...
library LibGas {
    ...
    function total(Types.GasBalances storage gs, address account) internal view returns (uint256) {
        Types.Gas storage gas = gs.balances[account];
        //NOTE: this is what's currently live
        //return gas.balance.add(gas.locked);


        //NOTE: This version has NOT been deployed as of 12/19/2020
        //we can only burn funds that are in the balance or thawing. If the thaw
        //period is over, we should be allowed to use those funds.
        if(gas.lockedUntil > 0 && gas.lockedUntil < block.number) {
            //we have some funds thawing, which are usable
            return gas.balance.add(gas.locked);
        }
