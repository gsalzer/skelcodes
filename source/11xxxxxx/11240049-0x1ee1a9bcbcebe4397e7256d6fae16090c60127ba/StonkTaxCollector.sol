...
// File: contracts\StonkTaxCollector.sol
...
contract StonkTaxCollector is Ownable {
  using SafeMath for uint256;
  ...
  struct PoolInfo {
    ...
    uint256 pendingRewards; // Rewards pending to be claimed by this pool
    uint256 totalRewards; // Total rewards earned by this pool
    uint256 startBlock; // Block the pool was added for average rewards calculation
    bool withdrawable; // Allows future pools to lock liquidity for increased rewards.
  }
  ...
  constructor(StonkERC20 _stonk, address _treasury) public {
    stonk = _stonk;
    treasury = _treasury;
  }
  ...
  function updatePendingRewards(uint256 _amount) internal {
    if (_amount > 0) {
      for (uint256 pid = 0; pid < poolInfo.length; pid++) {
        PoolInfo storage pool = poolInfo[pid];
        uint256 poolReward = _amount.mul(pool.allocPoint).div(
          totalAllocPoint
        );

        pool.pendingRewards = pool.pendingRewards.add(poolReward);
        pool.totalRewards = pool.totalRewards.add(poolReward);
      }
    }
  }
  ...
  function addRewards(uint256 _amount) external {
    require(
      stonk.transferFrom(msg.sender, address(this), _amount),
      'addRewards: Transfer failed'
    );
    updatePendingRewards(_amount);
  }

  // Update treasury address by the previous treasury.
  function setTreasury(address _treasury) external {
    require(msg.sender == treasury, 'setTreasury: ser?');
    treasury = _treasury;
  }
  ...
  function setPoolAlloc(uint256 _pid, uint256 _allocPoint) public onlyOwner {
    updatePool(_pid);
  ...
  function averageFeesPerBlock(uint256 _pid) external view returns (uint256) {
    PoolInfo memory pool = poolInfo[_pid];
    return pool.totalRewards.div(block.number.sub(pool.startBlock));
  }
    ...
    returns (uint256)
  {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      uint256 stonkReward = pool.pendingRewards;
  ...
  function transferPendingRewards(uint256 _pid)
    public
  {
    PoolInfo storage pool = poolInfo[_pid];
  ...
  // Deposit LP tokens to StonkTaxCollector for STONK allocation.
  ...
  // Withdraw LP tokens from StonkTaxCollector.
  function withdraw(uint256 _pid, uint256 _amount) public {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][msg.sender];

    require(pool.withdrawable, 'withdraw: cannot withdraw from locked pool');
  ...
  function emergencyWithdraw(uint256 _pid) public {
    ...
    require(
      pool.withdrawable,
      'emergencyWithdraw: cannot withdraw from locked pool'
    );
  ...
  // Emergency function to unlock a pool's liquidity
  function emergencyUnlockPool(uint256 _pid) external onlyOwner {
    PoolInfo storage pool = poolInfo[_pid];
