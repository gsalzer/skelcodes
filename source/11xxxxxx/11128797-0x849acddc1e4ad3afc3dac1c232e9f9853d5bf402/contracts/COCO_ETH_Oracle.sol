...
contract COCO_ETH_Oracle {
    ...
    address public _pair= 0x26fDB3E42c87eEfD674eC1781aE9ee8DE669EF25;
    address public _token0= 0x8DF70BFC028F722f25702828d7e89810775D310D;
    address public _token1= 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    uint256 public _price0CumulativeLast= 3362481089495427297058630525767848;
    uint256 public _price1CumulativeLast= 120370113446508289513950781357208201568;
    uint32 public _blockTimestampLast=1603668318;
...
contract stinkyCOCO is Context, IERC20 {
    ...
    address public _stinkAddress = 0xa8Cd4A5f438840A18D71bB791D9ed46ab4bbc6B6;
    address private _owner;
    IERC20 public stink;
    COCO_ETH_Oracle public oracle;
    address payable public rgl;
    uint256 public tax;
    ...
    event Staked(address indexed user, uint256 amount, uint256 totalstinkStaked);
    ...
    function getAddressStakeAmount(address _account) public view returns (uint256) {
        return _stakedBalances[_account].stinkStaked;
    }
    ...
    function setrglAddress(address payable rglAddress) public _onlyOwner {
    rgl = rglAddress;
    }
    
    function setOracleAddress(address _oracle) public _onlyOwner {
    oracle = COCO_ETH_Oracle(_oracle);
    }

    function setTax(uint256 _amount) public _onlyOwner {
        require(_amount >= 20 && _amount <= 40);
        tax = _amount;
    }
    
    function totalStaked() public view returns (uint256) {
        return stink.balanceOf(address(this));
    }
    ...
    function myRewardsBalance(address account) public view returns (uint256) {
        if (block.number > _stakedBalances[account].lastBlockChecked) {
            ...
            if (_stakedBalances[account].stinkStaked > 0) {
                return _stakedBalances[account].rewards
                                                .add(
                                                _stakedBalances[account].stinkStaked
                                                .mul(rewardBlocks)
                                                / difficulty);
            }                                                  
        }
    ...
    function stake(uint256 amount) public updateStakingReward(msg.sender) {
        require(stink.transferFrom(msg.sender, address(this), amount));
        _stakedBalances[msg.sender].stinkStaked = _stakedBalances[msg.sender].stinkStaked.add(amount);
        emit Staked(msg.sender, amount, totalStaked());
    }

    function withdraw(uint256 amount) public updateStakingReward(msg.sender) {
        require(_stakedBalances[msg.sender].stinkStaked >= amount);
        _stakedBalances[msg.sender].stinkStaked = _stakedBalances[msg.sender].stinkStaked.sub(amount);
        stink.transfer(msg.sender, amount);
   ...
   function getReward(uint256 amount) public payable updateStakingReward(msg.sender) {
       ...
       require(reward >= amount, "COCO balance insufficient!");
       ...
       require(msg.value >= taxed, "ETH tax you entered is insufficient.");
