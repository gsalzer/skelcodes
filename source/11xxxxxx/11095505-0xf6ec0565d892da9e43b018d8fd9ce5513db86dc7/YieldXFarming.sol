...
// YieldToken with Governance.
contract YieldToken is ERC20("yield-farming.io", "YIELDX"), Ownable {
...
contract YieldXFarming is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. YIELDs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that YIELDs distribution occurs.
        uint256 accYieldPerShare; // Accumulated YIELDs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus YIELD period ends.
    uint256 public bonusEndBlock;
    // YIELD tokens created per block.
    uint256 public yieldPerBlock;
    // Bonus muliplier for early yield makers.
    ...
    // The block number when YIELD mining starts.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 yieldReward = multiplier.mul(yieldPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        yield.mint(devaddr, (yieldReward.mul(75)).div(1000)); // 7.5%
