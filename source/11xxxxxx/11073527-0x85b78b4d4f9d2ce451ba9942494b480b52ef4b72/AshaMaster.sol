...
// AshaToken with Governance.
contract AshaToken is ERC20("AshaToken", "ASHA"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ASHA::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ASHA::delegateBySig: invalid nonce");
        require(now <= expiry, "ASHA::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ASHA::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ASHAs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ASHA::_writeCheckpoint: block number exceeds 32 bits");
...
contract AshaMaster is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accAshaPerShare; // Accumulated SAKEs per share, times 1e12. See below.
    }

    // The SAKE TOKEN!
    AshaToken public asha;
    // Dev address.
    ...
    uint256 public constant BETATEST_BLOCKNUM = 14400;
    // Bonus block num,about 13 days.
    uint256 public constant BONUS_BLOCKNUM = 35500;
    // mint end block num,about 30 days.
    uint256 public constant MINTEND_BLOCKNUM = 72000;
    ...
    constructor(
        AshaToken _asha,
        address _devaddr,
        uint256 _ashaPerBlock,
        uint256 _startBlock
    ) public {
        asha = _asha;
        devaddr = _devaddr;
        ashaPerBlock = _ashaPerBlock;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        require(lpTokenPID[address(_lpToken)] == 0, "AshaMaster:duplicate add.");
    ...
    function handoverAshaMintage(address newOwner) public onlyOwner {
        asha.transferOwnership(newOwner);
    }
    ...
    function pendingAsha(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAshaPerShare = pool.accAshaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ashaReward = multiplier.mul(ashaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAshaPerShare = accAshaPerShare.add(ashaReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accAshaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 ashaReward = multiplier.mul(ashaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        asha.mint(devaddr, ashaReward.div(10));
        asha.mint(address(this), ashaReward);
        pool.accAshaPerShare = pool.accAshaPerShare.add(ashaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accAshaPerShare).div(1e12).sub(user.rewardDebt);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accAshaPerShare).div(1e12);
        if (pending > 0) safeAshaTransfer(msg.sender, pending);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accAshaPerShare).div(1e12);
        safeAshaTransfer(msg.sender, pending);
    ...
    function safeAshaTransfer(address _to, uint256 _amount) internal {
        uint256 ashaBal = asha.balanceOf(address(this));
        if (_amount > ashaBal) {
            asha.transfer(_to, ashaBal);
        } else {
            asha.transfer(_to, _amount);
        }
    }
