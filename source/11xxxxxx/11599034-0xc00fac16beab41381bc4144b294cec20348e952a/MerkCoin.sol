...
import "./MerkStake.sol";
import "./ERC20Capped.sol";
import "./ERC20Mintable.sol";

contract MerkCoin is ERC20Capped {
    using SafeMath for uint256;

    string public name = "MERK";
    string public symbol = "MERK";
    ...
    constructor (uint256 cap, uint256 init) public ERC20Capped(cap) {
        require(cap > 1000000000000000000, "MerkCoin: cap must greater than 10^18");
    ...
    function stake(uint32 id, address beneficiary, uint256 amount, uint256 releaseAmount, uint32 releaseTime)
        ...
        require(_stakes[id] == address(0), "MerkCoin: stake with ID already exist");
        require(balanceOf(msg.sender) >= amount, "MerkCoin: there is not enough tokens to stake");
        require(amount >= releaseAmount, "MerkCoin: there is not enough tokens to stake");

        // create new stake
        MerkStake newStake = new MerkStake(MerkCoin(address(this)), beneficiary, releaseAmount, releaseTime);

        emit NewStake(address(newStake));

        // transfer amount of token to stake address
        require(transfer(address(newStake), amount), "MerkCoin: transfer tokens to new stake failed");
