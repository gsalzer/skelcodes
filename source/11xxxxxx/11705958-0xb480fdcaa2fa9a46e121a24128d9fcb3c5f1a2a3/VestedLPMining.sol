    ...
    IVestedLPMining
{
    ...
    function initialize(
        ...
        twaVestingPeriodInBlocks = SafeMath32.fromUint(_twaVestingPeriodInBlocks, "VLPMining: too big vest period");
    }
    ...
    function setTwaVestingPeriodInBlocks(uint256 _twaVestingPeriodInBlocks) public override onlyOwner {
        twaVestingPeriodInBlocks = SafeMath32.fromUint(
            ...
            "VLPMining: too big twaVestingPeriodInBlocks"
        );
    ...
    function _doPoolUpdate(Pool storage pool) internal {
        ...
        if (twaReward != 0) {
            twaVestingPool = twaVestingPool.add(
                SafeMath96.fromUint(twaReward, "VLPMining::_doPoolUpdate:1"),
    ...
    function _vestUserTwa(User storage user, uint256 accTwaPerLpt) internal {
        ...
        if (newlyVested != 0) {
            ...
            twaVestingPool = twaVestingPool.sub(
                SafeMath96.fromUint(newlyVested, "VLPMining::_vestUserTwa:1"),
                "VLPMining::_vestUserTwa:2"
            );
    ...
    function _getTwaPerBlock(uint256 blockNum) internal view returns(uint256 twaPerBlock) {
        if (startBlock <= blockNum && blockNum < startBlock.add(80640)) {
            return 2170138900000000000; //2.1701389
        }
        if (startBlock.add(80640) <= blockNum && blockNum < (startBlock.add(80640).add(345600))) {
            return 217013900000000000; //0.2170139
        }
        if ((startBlock.add(80640).add(345600)) <= blockNum && blockNum < (startBlock.add(80640).add(345600).add(4204800))) {
            return 59455800000000000; // 0.0594558
        }
        return 0;
    }
    ...
    function _computeUserVotes(uint192 userData, uint192 sharedData) internal override pure returns (uint96 votes) {
        ...
        if (pooledTwaShare == 0) {
            ...
            votes = ownTwa.add(SafeMath96.fromUint(pooledTwa, "VLPMining::_computeVotes"));
        }
    }

    function _computeTwaToEntitle(uint256 userLpt, uint96 userTwaAdjust, uint256 poolAccTwaPerLpt) private pure returns (uint96) {
        return userLpt == 0 ? 0 : (
            SafeMath96.fromUint(userLpt.mul(poolAccTwaPerLpt).div(SCALE), "VLPMining::computeTwa:1")
                .sub(userTwaAdjust, "VLPMining::computeTwa:2")
        );
    }
