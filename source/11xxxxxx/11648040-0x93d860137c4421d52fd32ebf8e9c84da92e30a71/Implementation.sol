...
/*
    Copyright 2021 Foreign Set Token <team@fs.finance>
    Copyright 2020 Dynamic Dollar Devs, based on the works of the Empty Set Squad
...
library Constants {
    ...
    uint256 private constant BOOTSTRAPPING_PRICE = 196e16; // 1.96 USDC (targeting 5% inflation) (EUR)

    /* Oracle */
    // Mainnet: 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48
    // Rinkeby: 0x0Ec948d52B08479b74573430114C50469cc7855E
    ...
    uint256 private constant EPOCH_START = 1610604000;      // 1610582400
...
contract IOracle {
    ...
    function getNormalPrice() public view returns (Decimal.D256 memory);
}
...
contract Getters is State {
    ...
    function dev() public view returns (address) {
        return address(0xFcCc146b160B0Bb812578fC2B2A33766E6f14Ab5);
    }
...
contract Comptroller is Setters {
    ...
    function mintToBonded(uint256 amount) private {
        ...
        uint256 devAmount = amount.div(100);
    ...
    function mintToDev(uint256 amount) private {
        if (amount > 0) {
            dollar().mint(dev(), amount);
        }
    }
...
contract Regulator is Comptroller {
    ...
    function step() internal {
        Decimal.D256 memory normalPrice = oracle().getNormalPrice();
        Decimal.D256 memory price = oracleCapture();

        if (price.greaterThan(normalPrice)) {
            setDebtToZero();
            growSupply(price, normalPrice);
            return;
        }

        if (price.lessThan(normalPrice)) {
            shrinkSupply(price, normalPrice);
            return;
        }
    ...
    function shrinkSupply(
        Decimal.D256 memory price,
        Decimal.D256 memory normalPrice
    ) private {
        Decimal.D256 memory delta = limit(
            (Decimal.one()).sub(Decimal.ratio(price.value, normalPrice.value))
        );
    ...
    function growSupply(
        ...
        Decimal.D256 memory delta = limit(
            Decimal
            .ratio(price.value, normalPrice.value).sub(Decimal.one())
            .div(Constants.getSupplyChangeDivisor())
        );
...
contract Implementation is State, Bonding, Market, Regulator {
    using SafeMath for uint256;
    ...
    function initialize() initializer public {
        // committer reward:
        mintToAccount(address(0x76d394ea49910fc2f22CC61972942a9e8a8bdE53), 10000e18); // 10000 DSD to dev
    }
