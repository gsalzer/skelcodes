...
contract UniswapV2Router02 is IUniswapV2Router02 {
  ...
  function _swap(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      IUniswapV2Pair(IUniswapV2Factory(factory).getPair(input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
    }
  }
  ...
  function swapExactTokensForTokens(
    ...
    TransferHelper.safeTransferFrom(path[0], msg.sender, IUniswapV2Factory(factory).getPair(path[0], path[1]), amounts[0]);
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    TransferHelper.safeTransferFrom(path[0], msg.sender, IUniswapV2Factory(factory).getPair(path[0], path[1]), amountIn);
