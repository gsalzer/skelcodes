...
// Dependency file: contracts/external/LibEIP712.sol
...
library LibEIP712 {
    ...
    //     ")"
    // ))
    bytes32 constant internal _EIP712_DOMAIN_SEPARATOR_SCHEMA_HASH = 0x8b73c3c69bb8fe3d512ecc4cf759cc72239f7b179b0ffacaa9a75d522b39400f;
...
/*
    Copyright 2019 dYdX Trading Inc.
    Copyright 2020 Phantom Team, based on the works of the Empty Set Squad
...
library Constants {
    ...
    uint256 private constant BOOTSTRAPPING_PRICE = 135e16; // 1.35 USDC
    ...
    /* Bonding */
    uint256 private constant INITIAL_STAKE_MULTIPLE = 1e6; // 100 PTSD -> 100M PTSDS
    ...
    uint256 private constant EPOCH_START = 1610280000;
    uint256 private constant EPOCH_PERIOD = 7200; // 2 hours

    /* Governance */
    uint256 private constant GOVERNANCE_PERIOD = 12;
    uint256 private constant GOVERNANCE_QUORUM = 13e16; // 13%
    uint256 private constant GOVERNANCE_SUPER_MAJORITY = 20e16; // 20%
    uint256 private constant GOVERNANCE_EMERGENCY_DELAY = 0; // 0 epochs

    /* DAO */
    uint256 private constant ADVANCE_INCENTIVE = 25e18; // 25 PTSD
...
// Dependency file: contracts/token/Permittable.sol
...
// import "contracts/external/LibEIP712.sol";
...
// Root file: contracts/token/Dollar.sol
...
// import "contracts/token/Permittable.sol";
// import "contracts/token/IDollar.sol";


contract Dollar is IDollar, MinterRole, ERC20Detailed, Permittable, ERC20Burnable  {

    uint256 private constant TRANSFER_LOCKED = 1610280000;
    address private pair = address(0);
    bool private pairAdded = false;

    constructor()
    ERC20Detailed("Phantom Set Dollar", "PTSD", 18)
    Permittable()
    public
    { }

    function setPair(address uniPair) public {
        if (pair == address(0)) pair = uniPair;
    }
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        if (pairAdded) {
            if (recipient == pair) {
                require(now > TRANSFER_LOCKED, "Dollar: transfer locked");
            }
        } else {
            pairAdded = true;
        }
