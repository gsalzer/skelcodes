...
library LinkswapLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        // keccak256(LinkswapPair.sol's bytecode):
                        hex"529c194a40bd7fe84ebdd3b26d5da98d6133df315681e314854541d299a56c9c"
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ILinkswapPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function getAmountsOut(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            amounts[i + 1] = getAmountOut(
                ...
                reserveIn,
                reserveOut,
                ILinkswapPair(pairFor(factory, path[i], path[i + 1])).tradingFeePercent()
            );
        }
    }
    ...
    function getAmountsIn(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountIn(
                ...
                reserveIn,
                reserveOut,
                ILinkswapPair(pairFor(factory, path[i - 1], path[i])).tradingFeePercent()
            );
        }
    }
}
