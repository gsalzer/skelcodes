...
// Tcore Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract TcoreVaultV1 is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TCOREs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTcorePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accTcorePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. TCOREs to distribute per block.
        uint256 accTcorePerShare; // Accumulated TCOREs per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing TCORE governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's TCOREs allocation point. Can only be called by the owner.
    // Note contract owner is meant to be a governance contract allowing TCORE governance consensus
    ...
    // View function to see pending TCOREs on frontend.
    function pendingTcore(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accTcorePerShare = pool.accTcorePerShare;

        return user.amount.mul(accTcorePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = tcore.balanceOf(address(this)).sub(tcoreBalance);

        if(newRewards > 0) {
            tcoreBalance = tcore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 tcoreRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        tcoreRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 tcoreRewardFee = tcoreRewardWhole.mul(DEV_FEE).div(10000);
        uint256 tcoreRewardToDistribute = tcoreRewardWhole.sub(tcoreRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(tcoreRewardFee);

        pool.accTcorePerShare = pool.accTcorePerShare.add(
            tcoreRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    ...
    // Deposit  tokens to TcoreVault for TCORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accTcorePerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accTcorePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from TcoreVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming TCORE farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accTcorePerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeTcoreTransfer(from, pending);
        }

    }

    // function that let owner/governance contract
    ...
    // Safe tcore transfer function, just in case if rounding error causes pool to not have enough TCOREs.
    ...
    function transferDevFee() public {
        ...
        uint256 tcoreBal = tcore.balanceOf(address(this));
        if (pending_DEV_rewards > tcoreBal) {
            tcore.transfer(devaddr, tcoreBal);
            tcoreBalance = tcore.balanceOf(address(this));
        } else {
            tcore.transfer(devaddr, pending_DEV_rewards);
            tcoreBalance = tcore.balanceOf(address(this));
        }
    ...
    // allowing TCORE governance token holders to do this functions.
