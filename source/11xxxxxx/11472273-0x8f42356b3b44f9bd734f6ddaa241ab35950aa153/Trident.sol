...
contract Trident is Context, IERC20, Ownable {
    ...
    uint256 internal _tokenTotal = 10_000_000e9;
    ...
    uint256 public _swapCallerFee = 200e9;
    ...
    uint256 public maxTxAmount = 2000e9;
    uint256 public minTokensBeforeSwap = 10_000e9;
    uint256 public minTokenBeforeReward = 10e9;

    uint256 public lastRebalance = now - 1 hours;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "Trident: We can not exclude Uniswap router."
        );
        require(account != address(this), 'Trident: We can not exclude contract self.');
        require(account != rewardWallet, 'Trident: We can not exclude reweard wallet.');
        require(!_isExcluded[account], "Trident: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Trident: Account is already included");
    ...
    function rebalance() public {
        require(now > lastRebalance + rebalanceInterval && balanceOf(_msgSender()) >= 100e9, 'Trident: Not allowed Rebalance.');
